...
export class SimpleSemanticTokensProvider implements vscode.DocumentSemanticTokensProvider {
    ...
    private collectStrings(line: string, lineNumber: number, matches: TokenMatch[], covered: Set<number>): void {
        for (let index = 0; index < line.length; index++) {
    ...
    private isCovered(covered: Set<number>, start: number, length: number): boolean {
    ...
    private markCovered(covered: Set<number>, start: number, length: number): void {
        for (let index = start; index < start + length; index++) {
            covered.add(index);
        }
    }
