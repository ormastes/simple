...
var RichEditorWebview = (() => {
  ...
  var MATH_PREFIXES = /* @__PURE__ */ new Set(["m", "loss", "nograd", "img", "graph"]);
  function tokenSimple(stream, state) {
  ...
  var simpleLanguage = StreamLanguage.define({
    name: "simple",
    startState() {
  ...
  var ImagePlaceholderWidget = class extends WidgetType {
    constructor(path) {
      super();
      this.path = path;
    }
  ...
  var ImageErrorWidget = class extends WidgetType {
  ...
  function buildDecorations(view, renderedBlocks) {
  ...
  function createDecorationPlugin(getRenderedBlocks) {
  ...
  function createEditor(parent, initialText, renderedBlocksRef, symbolsRef, testsRef, onEdit, onSelectionChange, onRunTest) {
    ...
    return {
      ...
      setDoc(text, selStart, selEnd) {
        ...
        isApplyingSync = true;
        try {
          const changes = view.state.doc.toString() !== text ? { from: 0, to: view.state.doc.length, insert: text } : void 0;
          const selection = selStart !== void 0 ? { anchor: Math.min(selStart, text.length), head: Math.min(selEnd ?? selStart, text.length) } : void 0;
          view.dispatch({
            ...changes ? { changes } : {},
            ...selection ? { selection } : {}
          });
