...
mod tests {
    ...
    fn test_string_create() {
        let s = b"Hello, World!";
        let str_val = unsafe { rt_string_new(s.as_ptr(), s.len() as u64) };
    ...
    fn test_string_concat() {
        let a = b"Hello, ";
        let b = b"World!";
        let str_a = unsafe { rt_string_new(a.as_ptr(), a.len() as u64) };
        let str_b = unsafe { rt_string_new(b.as_ptr(), b.len() as u64) };
    ...
    fn test_empty_string() {
        let str_val = unsafe { rt_string_new(std::ptr::null(), 0) };
        assert!(!str_val.is_nil());
