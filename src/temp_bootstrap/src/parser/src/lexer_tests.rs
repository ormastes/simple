...
#[test]
fn test_triple_quoted_in_args() {
    let tokens = tokenize(r#"foo("a", """hello""", "c")"#);
    // DocComment token is produced for """...""" triple-quoted strings
    assert_eq!(tokens, vec![
        TokenKind::Identifier("foo".to_string()),
        TokenKind::LParen,
        TokenKind::FString(vec![FStringToken::Literal("a".to_string())]),
        TokenKind::Comma,
        TokenKind::DocComment("hello".to_string()),
        TokenKind::Comma,
        TokenKind::FString(vec![FStringToken::Literal("c".to_string())]),
        TokenKind::RParen,
        TokenKind::Eof,
    ]);
}
...
fn test_arithmetic_operators() {
    ...
    // `//` at line start is treated as a C-style line comment, not floor division.
    // Test floor division in expression context instead:
