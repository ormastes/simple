...
pub enum TokenKind {
    ...
    Fn,
    Let,
    Var,  // var (mutable binding) — Simple syntax
    Mut,
    Me,   // me (mutable method keyword) — Simple syntax
    If,
    Elif,
    Else,
    For,
    ...
    // Simple-specific keywords
    PassDoNothing,  // pass_do_nothing (intentional no-op)
    PassDn,         // pass_dn (alias for pass_do_nothing)
    PassTodo,       // pass_todo (TODO marker)
    Pass,           // pass (generic no-op)
    Bitfield,       // bitfield (bit-level data structures)
    Alias,          // alias (class/type alias)
    Def,            // def (synonym for fn)
    WalrusAssign,   // := (val synonym)
    ...
    // Documentation
    DocComment(String), // /** ... */ or ## line comment or /// line comment or """..."""
