...
// ============================================================================
// Batch parse test - parses all .spl files in src/compiler
// ============================================================================

fn walk_spl_files(dir: &std::path::Path, files: &mut Vec<std::path::PathBuf>) {
    if let Ok(entries) = std::fs::read_dir(dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.is_dir() {
                walk_spl_files(&path, files);
...
fn test_struct_with_docstring() {
    ...
    match parser.parse() {
        Ok(module) => {
            assert!(!module.items.is_empty(), "Should parse struct");
            if let simple_parser::ast::Node::Struct(s) = &module.items[0] {
                assert_eq!(s.fields.len(), 2, "Should have 2 fields, got: {:?}", s.fields);
            } else {
...
#[test]
fn batch_parse_compiler_spl_files() {
    // Find the project root
    // CARGO_MANIFEST_DIR = .../src/temp_bootstrap/src/parser
    // Go up: src/parser -> src -> temp_bootstrap -> src -> (project root)
    let manifest_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    let project_root = manifest_dir
        .parent().unwrap()  // src/temp_bootstrap/src
        .parent().unwrap()  // src/temp_bootstrap
        .parent().unwrap()  // src
        .parent().unwrap(); // project root
    let compiler_dir = project_root.join("src").join("compiler");

    if !compiler_dir.exists() {
        eprintln!("Skipping batch parse: {:?} not found", compiler_dir);
        return;
    }

    let mut files = Vec::new();
    walk_spl_files(&compiler_dir, &mut files);
    files.sort();
    ...
    for path in &files {
        let content = match std::fs::read_to_string(path) {
            ...
            Err(e) => {
                fail += 1;
                errors.push((path.display().to_string(), format!("read error: {}", e)));
                continue;
            }
        };
        ...
        match parser.parse() {
            ...
            Err(e) => {
                fail += 1;
                // Include line:col info from span
                let loc = if let Some(span) = e.span() {
                    format!(" [L{}:C{}]", span.line, span.column)
                } else {
                    String::new()
                };
                errors.push((path.display().to_string(), format!("{}{}", e, loc)));
            }
        }
    }

    let total = ok + fail;
    eprintln!("\nParsed: {}/{} ({}%)", ok, total, if total > 0 { ok * 100 / total } else { 0 });
    eprintln!("Failed: {}\n", fail);

    // Count error types
    let mut error_counts: std::collections::HashMap<String, Vec<String>> = std::collections::HashMap::new();
    for (file, e) in &errors {
        ...
        error_counts.entry(pattern).or_default().push(file.clone());
    }
    ...
    for (pattern, ref files) in sorted.iter().take(50) {
        eprintln!("  {:3} x {}", files.len(), pattern);
