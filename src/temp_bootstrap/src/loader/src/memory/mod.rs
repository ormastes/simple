...
impl ExecutableMemory {
    ...
    ///
    /// # Panics
    /// Panics if `offset` exceeds the memory region size.
    ///
    /// # Safety
    /// The caller must ensure that the memory at `offset` contains a valid
    /// function with the signature matching type `F`.
    pub unsafe fn get_fn<F>(&self, offset: usize) -> F {
        assert!(
            offset < self.size,
            "get_fn: offset {} out of bounds for memory region of size {}",
            offset, self.size
        );
