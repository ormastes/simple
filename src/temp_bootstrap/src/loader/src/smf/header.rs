...
impl SmfHeader {
    ...
    pub fn read<R: Read>(reader: &mut R) -> std::io::Result<Self> {
        ...
        // Safety: buf is a stack-allocated array of exactly size_of::<SmfHeader>() bytes.
        // read_exact filled the buffer completely before reaching here, and
        // SmfHeader is repr(C) with only primitive fields, so ptr::read is sound.
        debug_assert!(buf.len() == std::mem::size_of::<SmfHeader>());
        let header = unsafe { std::ptr::read(buf.as_ptr() as *const SmfHeader) };

        // Validate critical fields to catch corrupted headers early
        if header.section_count > 65536 {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                format!("SMF section count {} is unreasonably large", header.section_count),
            ));
        }
        if header.symbol_count > 1_000_000 {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                format!("SMF symbol count {} is unreasonably large", header.symbol_count),
            ));
        }
