...
// Safety: Settlement owns all its memory regions exclusively. The code_region and
// data_region pointers are allocated by the MemoryAllocator and are not shared with
// other threads. All mutation is through &mut self methods, and the indirection tables
// use atomic operations for hot-reload updates. The native library manager uses
// OS-level thread-safe dlopen/dlsym. No raw pointers escape the Settlement without
// going through the atomic indirection table.
...
impl Settlement {
    ...
    pub fn add_module_with_linking(
        ...
        let (code_ptr, code_slot_capacity) = self.code_slots.get_memory(code_slots);

        // Bounds check: ensure source module code fits within allocated slot memory
        if code_size > code_slot_capacity {
            return Err(SettlementError::MemoryError(format!(
                "code size {} exceeds allocated slot capacity {}",
                code_size, code_slot_capacity
            )));
        }
        // Bounds check: ensure source module code_mem is large enough
        if code_size > module.code_mem.size() {
            return Err(SettlementError::InvalidModule(format!(
                "code size {} exceeds module code memory size {}",
                code_size, module.code_mem.size()
            )));
        }
        ...
        if let Some(ref data_mem) = module.data_mem {
            let (data_ptr, data_slot_capacity) = self.data_slots.get_memory(data_slots);

            // Bounds check: ensure data fits within allocated slot memory
            if data_size > data_slot_capacity {
                return Err(SettlementError::MemoryError(format!(
                    "data size {} exceeds allocated slot capacity {}",
                    data_size, data_slot_capacity
                )));
            }
            // Bounds check: ensure source data_mem is large enough
            if data_size > data_mem.size() {
                return Err(SettlementError::InvalidModule(format!(
                    "data size {} exceeds module data memory size {}",
                    data_size, data_mem.size()
                )));
            }
        ...
        if module.header.is_executable() {
            ...
            // Bounds check: entry_offset must be within the code region
            if entry_offset >= code_size {
                return Err(SettlementError::InvalidModule(format!(
                    "entry point offset {} exceeds code size {}",
                    entry_offset, code_size
                )));
            }

            let settlement_addr = (code_ptr as usize).checked_add(entry_offset)
                .ok_or_else(|| SettlementError::MemoryError(format!(
                    "entry point address overflow: code_ptr {:p} + offset {}",
                    code_ptr, entry_offset
                )))?;
        ...
        for sym in module.symbols.exports() {
            if sym.sym_type == crate::smf::SymbolType::Function {
                ...
                // Bounds check: symbol offset must be within code region
                if offset >= code_size {
                    return Err(SettlementError::InvalidModule(format!(
                        "export symbol offset {} exceeds code size {}",
                        offset, code_size
                    )));
                }

                let settlement_addr = (code_ptr as usize).checked_add(offset)
                    .ok_or_else(|| SettlementError::MemoryError(format!(
                        "symbol address overflow: code_ptr {:p} + offset {}",
                        code_ptr, offset
                    )))?;
        ...
        let dependencies = if resolve_imports {
            ...
            if !link_result.resolved_imports.is_empty() {
                // Bounds check: code_size must fit within the slot capacity
                if code_size > code_slot_capacity {
                    return Err(SettlementError::MemoryError(format!(
                        "code size {} exceeds slot capacity {} for relocation",
                        code_size, code_slot_capacity
                    )));
                }
    ...
    pub fn replace_module(
        ...
        // Copy new code
        let (new_code_ptr, new_code_slot_capacity) = self.code_slots.get_memory(new_code_slots);

        // Bounds check: ensure new code fits within allocated slot memory
        if new_code_size > new_code_slot_capacity {
            return Err(SettlementError::MemoryError(format!(
                "new code size {} exceeds allocated slot capacity {}",
                new_code_size, new_code_slot_capacity
            )));
        }
        if new_code_size > new_module.code_mem.size() {
            return Err(SettlementError::InvalidModule(format!(
                "new code size {} exceeds module code memory size {}",
                new_code_size, new_module.code_mem.size()
            )));
        }
        ...
        if let Some(ref data_mem) = new_module.data_mem {
            let (new_data_ptr, new_data_slot_capacity) = self.data_slots.get_memory(new_data_slots);

            // Bounds check: ensure new data fits within allocated slot memory
            if new_data_size > new_data_slot_capacity {
                return Err(SettlementError::MemoryError(format!(
                    "new data size {} exceeds allocated slot capacity {}",
                    new_data_size, new_data_slot_capacity
                )));
            }
            if new_data_size > data_mem.size() {
                return Err(SettlementError::InvalidModule(format!(
                    "new data size {} exceeds module data memory size {}",
                    new_data_size, data_mem.size()
                )));
            }
            ...
            if self.func_table.get_entry(old_func_idx).is_some() {
                ...
                for sym in new_module.symbols.exports() {
                    if sym.sym_type == crate::smf::SymbolType::Function {
                        ...
                        // Bounds check: symbol offset must be within new code region
                        if offset >= new_code_size {
                            return Err(SettlementError::InvalidModule(format!(
                                "export symbol offset {} exceeds new code size {}",
                                offset, new_code_size
                            )));
                        }

                        let new_addr = (new_code_ptr as usize).checked_add(offset)
                            .ok_or_else(|| SettlementError::MemoryError(format!(
                                "symbol address overflow: code_ptr {:p} + offset {}",
                                new_code_ptr, offset
                            )))?;
    ...
    pub fn add_native_lib(&mut self, spec: NativeLibSpec) -> Result<NativeHandle, SettlementError> {
        match spec {
            NativeLibSpec::Static { name, data, .. } => {
                ...
                let (ptr, slot_capacity) = self.data_slots.get_memory(slots);

                // Bounds check: data must fit within allocated slot capacity
                if data.len() > slot_capacity {
                    return Err(SettlementError::MemoryError(format!(
                        "native lib data size {} exceeds allocated slot capacity {}",
                        data.len(), slot_capacity
                    )));
                }
    ...
    pub fn compact(&mut self) -> Result<(), SettlementError> {
        ...
        for (old_range, new_range) in &code_moves {
            let (old_ptr, old_size) = self.code_slots.get_memory(*old_range);
            let (new_ptr, new_capacity) = self.code_slots.get_memory(*new_range);

            // Bounds check: old region size must fit in new region capacity
            if old_size > new_capacity {
                return Err(SettlementError::MemoryError(format!(
                    "compact: code move size {} exceeds destination capacity {}",
                    old_size, new_capacity
                )));
            }

            // Bounds check: both pointers must be within code region
            let code_base = self.code_region.as_ptr() as usize;
            let code_end = code_base + self.code_region.size();
            let old_end = (old_ptr as usize).checked_add(old_size)
                .ok_or_else(|| SettlementError::MemoryError(
                    "compact: old code range address overflow".to_string()
                ))?;
            let new_end = (new_ptr as usize).checked_add(old_size)
                .ok_or_else(|| SettlementError::MemoryError(
                    "compact: new code range address overflow".to_string()
                ))?;
            if (old_ptr as usize) < code_base || old_end > code_end {
                return Err(SettlementError::MemoryError(format!(
                    "compact: old code range {:p}..{:#x} outside code region {:#x}..{:#x}",
                    old_ptr, old_end, code_base, code_end
                )));
            }
            if (new_ptr as usize) < code_base || new_end > code_end {
                return Err(SettlementError::MemoryError(format!(
                    "compact: new code range {:p}..{:#x} outside code region {:#x}..{:#x}",
                    new_ptr, new_end, code_base, code_end
                )));
            }

            // Move the data (ptr::copy handles overlapping regions)
            unsafe {
                std::ptr::copy(old_ptr, new_ptr, old_size);
            }
        ...
        for (old_range, new_range) in &data_moves {
            let (old_ptr, old_size) = self.data_slots.get_memory(*old_range);
            let (new_ptr, new_capacity) = self.data_slots.get_memory(*new_range);

            // Bounds check: old region size must fit in new region capacity
            if old_size > new_capacity {
                return Err(SettlementError::MemoryError(format!(
                    "compact: data move size {} exceeds destination capacity {}",
                    old_size, new_capacity
                )));
            }

            // Bounds check: both pointers must be within data region
            let data_base = self.data_region.as_ptr() as usize;
            let data_end = data_base + self.data_region.size();
            let old_end = (old_ptr as usize).checked_add(old_size)
                .ok_or_else(|| SettlementError::MemoryError(
                    "compact: old data range address overflow".to_string()
                ))?;
            let new_end = (new_ptr as usize).checked_add(old_size)
                .ok_or_else(|| SettlementError::MemoryError(
                    "compact: new data range address overflow".to_string()
                ))?;
            if (old_ptr as usize) < data_base || old_end > data_end {
                return Err(SettlementError::MemoryError(format!(
                    "compact: old data range {:p}..{:#x} outside data region {:#x}..{:#x}",
                    old_ptr, old_end, data_base, data_end
                )));
            }
            if (new_ptr as usize) < data_base || new_end > data_end {
                return Err(SettlementError::MemoryError(format!(
                    "compact: new data range {:p}..{:#x} outside data region {:#x}..{:#x}",
                    new_ptr, new_end, data_base, data_end
                )));
            }
    ...
    ///
    /// # Safety invariant
    /// The code_region pointer is allocated by the MemoryAllocator in `new()` and
    /// is guaranteed to be valid for `code_region.size()` bytes until Settlement is dropped.
    pub fn code_region_slice(&self) -> &[u8] {
        let ptr = self.code_region.as_ptr();
        let size = self.code_region.size();
    ...
    ///
    /// # Safety invariant
    /// The data_region pointer is allocated by the MemoryAllocator in `new()` and
    /// is guaranteed to be valid for `data_region.size()` bytes until Settlement is dropped.
    pub fn data_region_slice(&self) -> &[u8] {
        let ptr = self.data_region.as_ptr();
        let size = self.data_region.size();
        if ptr.is_null() || size == 0 {
            return &[];
        }
        unsafe { std::slice::from_raw_parts(ptr, size) }
