...
impl SlotAllocator {
    ...
    ///
    /// # Panics
    /// Panics if the slot index is out of bounds for this allocator.
    pub fn slot_to_addr(&self, slot: usize) -> *mut u8 {
        assert!(
            slot <= self.total_slots,
            "slot_to_addr: slot {} out of bounds (total_slots: {})",
            slot, self.total_slots
        );
        let byte_offset = slot.checked_mul(self.slot_size)
            .expect("slot_to_addr: byte offset overflow");
        assert!(
            byte_offset <= self.region_size,
            "slot_to_addr: byte offset {} exceeds region size {}",
            byte_offset, self.region_size
        );
        unsafe { self.base.add(byte_offset) }
    }
