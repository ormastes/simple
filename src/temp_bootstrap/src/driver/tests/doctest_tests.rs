...
fn doctest_matches_errors() {
    // Use a construct that triggers a compile/runtime error in the bootstrap
    // interpreter: division by zero or type error.
    // The bootstrap interpreter treats undefined variables as nil for cross-module compat.
    let text = r#"
>>> 1 / 0
...
"#;
    ...
    let results = run_examples(&examples);
    assert!(matches!(results[0].status, DoctestStatus::Passed), "status: {:?}", results[0].status);
    assert!(
