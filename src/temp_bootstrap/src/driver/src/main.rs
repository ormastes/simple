...
fn run_code(code: &str, gc_log: bool, gc_off: bool) -> i32 {
    ...
    let full_code = if code.contains("main") || code.contains("fn ") || code.contains("let ") || code.contains("val ") || code.contains("var ") {
        code.to_string()
    } else {
        format!("main = {}", code)
    };
    ...
    match first.as_str() {
        ...
        "compile" => {
            if args.len() < 2 {
                ...
                eprintln!("       simple compile <source.spl> --emit-c [-o <output.c>]");
                std::process::exit(1);
            }
