    ...
    pub fn infer_expr(&mut self, expr: &Expr) -> Result<Type, TypeError> {
        match expr {
            ...
            Expr::Identifier(name) => {
                // If the identifier is in the environment, use its type
                if let Some(ty) = self.env.get(name).cloned() {
                    return Ok(ty);
                }
                // For bootstrap mode: treat unknown identifiers as fresh type variables
                // instead of errors. This allows single-file compilation without module
                // resolution — external types (from other modules) are simply inferred.
