...
#[test]
fn allows_undefined_variable_in_bootstrap_mode() {
    // In bootstrap mode, undefined identifiers are treated as fresh type variables
    // to allow single-file compilation without module resolution
    let items = parse_items("main = y + 1");
    check(&items).expect("type check ok — bootstrap mode allows unknown identifiers");
}
