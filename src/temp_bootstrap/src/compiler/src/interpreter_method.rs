...
fn evaluate_method_call(
    ...
    if let Value::Object { class, fields } = recv_val.clone() {
        ...
        // Bootstrap mode: return Nil for unknown methods on objects
        // This handles objects created as placeholders for external types
        return Ok(Value::Nil);
    }
    ...
    if let Value::Constructor { ref class_name } = recv_val {
        // Find static method in class definition
        if let Some(class_def) = classes.get(class_name) {
            if let Some(method_def) = class_def.methods.iter().find(|m| m.name == method) {
                ...
                return exec_function(method_def, args, env, functions, classes, enums, impl_methods, None);
            }
        }
        // Also check impl_methods for static methods from impl blocks
        if let Some(methods) = impl_methods.get(class_name.as_str()) {
            if let Some(method_def) = methods.iter().find(|m| m.name == method) {
                return exec_function(method_def, args, env, functions, classes, enums, impl_methods, None);
            }
        }
        if classes.contains_key(class_name) || impl_methods.contains_key(class_name.as_str()) {
            ...
            if let Some(methods) = impl_methods.get(class_name.as_str()) {
                available.extend(methods.iter().map(|m| m.name.as_str()));
            }
        ...
        // Bootstrap mode: unknown class, return Nil for static method calls
        // This handles calls like ClassName.method() for types from other modules
        return Ok(Value::Nil);
    }
    ...
    // Bootstrap mode: return Nil for method calls on unknown/unsupported types
    // This handles cases where external types have methods we don't know about
    Ok(Value::Nil)
}
