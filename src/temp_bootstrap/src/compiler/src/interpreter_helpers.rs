...
fn handle_method_call_with_self_update(
    ...
    if let Expr::MethodCall { receiver, method, args } = value_expr {
        if let Expr::Identifier(obj_name) = receiver.as_ref() {
            // Handle Object method calls with self-mutation
            ...
            // Handle mutating methods on collections (Array, Dict)
            // When the result is the same collection type, update the variable in the environment
            if let Some(current_val) = env.get(obj_name) {
                let is_mutating_collection_method = matches!(
                    (current_val, method.as_str()),
                    (Value::Array(_), "push" | "append" | "pop" | "insert" | "remove"
                        | "reverse" | "concat" | "extend" | "sort" | "clear")
                    | (Value::Dict(_), "set" | "insert" | "remove" | "delete"
                        | "merge" | "extend" | "clear")
                );
                if is_mutating_collection_method {
                    ...
                    // If the result is the same collection type, update the variable
                    match (&result, current_val) {
                        (Value::Array(_), Value::Array(_)) | (Value::Dict(_), Value::Dict(_)) => {
                            return Ok((result.clone(), Some((obj_name.clone(), result))));
                        }
                        _ => {
                            return Ok((result, None));
                        }
...
/// Normalize a slice index to usize (handling negative indices), clamped to [0, len]
fn normalize_slice_index(idx: i64, len: i64) -> usize {
    if idx < 0 {
