...
/// These are the "prelude" functions - print, math, conversion, assertion, and FFI utilities.
...
pub fn evaluate_module(items: &[Node]) -> Result<i32, CompileError> {
    ...
    // Auto-call main() if it was defined as a function
    if let Some(main_func) = functions.get("main") {
        let result = exec_function(main_func, &[], &env, &functions, &classes, &enums, &impl_methods, None)?;
    ...
    match node {
        ...
        Node::Assignment(assign) if assign.op == AssignOp::Assign => {
            if let Expr::Identifier(name) = &assign.target {
                ...
                // Handle field assignment: obj.field = value, obj.inner.field = value
                let value =
                    evaluate_expr(&assign.value, env, functions, classes, enums, impl_methods)?;
                // Collect field access chain: walk receiver chain to find root identifier
                let mut chain = vec![field.clone()];
                let mut current = receiver.as_ref();
                loop {
                    match current {
                        Expr::Identifier(root_name) => {
                            // Found the root identifier
                            chain.reverse();
                            if let Some(root_val) = env.get(root_name).cloned() {
                                let updated = set_nested_field(root_val, &chain, value)?;
                                env.insert(root_name.clone(), updated);
                                break Ok(Control::Next);
                            } else {
                                let known_names: Vec<&str> = env
                                    ...
                                    .collect();
                                let mut msg = format!("undefined variable '{root_name}'");
                                if let Some(suggestion) =
                                    crate::error::typo::format_suggestion(root_name, known_names)
                                {
                                    msg.push_str(&format!("; {}", suggestion));
                                }
                                break Err(CompileError::Semantic(msg));
                            }
                        }
                        Expr::FieldAccess { receiver: inner_recv, field: inner_field } => {
                            chain.push(inner_field.clone());
                            current = inner_recv.as_ref();
                        }
                        _ => {
                            break Err(CompileError::Semantic(
                                "field assignment requires identifier as root object".into(),
                            ));
                        }
                    }
                }
            } else if let Expr::Index { receiver, index } = &assign.target {
                // Handle index assignment: dict["key"] = val, arr[0] = val, obj.field[i] = val
                let value =
                    evaluate_expr(&assign.value, env, functions, classes, enums, impl_methods)?;
                let idx_val =
                    evaluate_expr(index, env, functions, classes, enums, impl_methods)?;

                // Helper closure to perform the actual index mutation on an array or dict
                fn apply_index_assign(mut container: Value, idx_val: &Value, value: Value) -> Result<Value, CompileError> {
                    match container {
                        ...
                        Value::Array(ref mut arr) => {
                            ...
                            if idx < arr.len() {
                                ...
                                Err(CompileError::Semantic(format!(
                                    "array index out of bounds: {raw_idx}"
                                )))
                            }
                        }
                        _ => Err(CompileError::Semantic(
                            "index assignment requires dict or array".into(),
                        )),
                    }
                }

                // Try to resolve the receiver: simple identifier or field access chain
                if let Expr::Identifier(var_name) = receiver.as_ref() {
                    if let Some(recv_val) = env.get(var_name).cloned() {
                        let updated = apply_index_assign(recv_val, &idx_val, value)?;
                        env.insert(var_name.clone(), updated);
                        Ok(Control::Next)
                    } else {
                        Err(CompileError::Semantic(format!(
                    ...
                    // Handle field access chain as receiver: obj.field[i] = val
                    // Walk the receiver to find root identifier and field path
                    ...
                    loop {
                        match current {
                            Expr::Identifier(root_name) => {
                                field_chain.reverse();
                                if let Some(root_val) = env.get(root_name).cloned() {
                                    // Navigate down to get the container, apply index assignment, write back
                                    let container = get_nested_field(&root_val, &field_chain)?;
                                    let updated_container = apply_index_assign(container, &idx_val, value)?;
                                    let updated_root = set_nested_field(root_val, &field_chain, updated_container)?;
                                    env.insert(root_name.clone(), updated_root);
                                    break Ok(Control::Next);
                                } else {
                                    break Err(CompileError::Semantic(format!(
                                        "undefined variable '{root_name}'"
                                    )));
                                }
                            }
                            Expr::FieldAccess { receiver: inner_recv, field } => {
                                field_chain.push(field.clone());
                                current = inner_recv.as_ref();
                            }
                            _ => {
                                break Err(CompileError::Semantic(
                                    "index assignment requires identifier or field access as receiver".into(),
                                ));
                            }
                        }
                    }
...
/// Navigate into nested fields of a Value::Object, returning the value at the given field path.
/// e.g., get_nested_field(root, &["inner", "items"]) returns root.inner.items
fn get_nested_field(val: &Value, fields: &[String]) -> Result<Value, CompileError> {
    let mut current = val.clone();
    for field in fields {
        match current {
            Value::Object { fields: ref flds, .. } => {
                current = flds
                    .get(field)
                    .cloned()
                    .unwrap_or(Value::Nil);
...
/// Set a nested field on a Value::Object, returning the updated root object.
/// e.g., set_nested_field(root, &["inner", "val"], new_value) sets root.inner.val = new_value
fn set_nested_field(root: Value, path: &[String], value: Value) -> Result<Value, CompileError> {
    if path.is_empty() {
        return Ok(value);
    }
    match root {
        Value::Object { class, mut fields } => {
            ...
            if path.len() == 1 {
                ...
                let inner = fields
                    ...
                    .cloned()
                    .unwrap_or(Value::Nil);
                let updated_inner = set_nested_field(inner, &path[1..], value)?;
                fields.insert(field_name.clone(), updated_inner);
                Ok(Value::Object { class, fields })
            }
        }
        _ => {
            Err(CompileError::Semantic(
                "cannot assign field on non-object value".into(),
            ))
        }
    }
}
...
// NOTE: helpers must come before expr because expr uses normalize_slice_index etc.
