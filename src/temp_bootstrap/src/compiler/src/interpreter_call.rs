...
fn evaluate_call(
    ...
    if let Expr::FieldAccess { receiver, field } = callee.as_ref() {
        if let Expr::Identifier(module_name) = receiver.as_ref() {
            // Check if module_name is a type with static methods (Type.method() convention)
            if let Some(methods) = impl_methods.get(module_name.as_str()) {
                if let Some(func) = methods.iter().find(|m| &m.name == field) {
                    return exec_function(func, args, env, functions, classes, enums, impl_methods, None);
                }
            }
            if let Some(class_def) = classes.get(module_name.as_str()) {
    ...
    match callee_val {
        ...
        // Bootstrap mode: treat calls on Nil/unknown values as no-ops
        Value::Nil => Ok(Value::Nil),
        // Bootstrap mode: any other value type called as function returns Nil
        _ => Ok(Value::Nil),
    }
}
...
fn get_iterator_values(iterable: &Value) -> Result<Vec<Value>, CompileError> {
    match iterable {
        ...
        Value::Object { class, fields } => {
            ...
            // Bootstrap mode: treat non-Range objects as empty iterables
            Ok(Vec::new())
        }
        // Bootstrap mode: treat Nil and unknown values as empty iterables
        _ => Ok(Vec::new()),
    }
}
    ...
    for node in nodes {
        match node {
            Node::Expression(expr) => {
                // Handle collection-mutating method calls (push, set, etc.)
                let (val, update) = handle_method_call_with_self_update(
                    expr, &local_env, functions, classes, enums, impl_methods,
                )?;
            ...
            Node::Let(let_stmt) => {
                if let Some(ref value_expr) = let_stmt.value {
                    ...
                    // Extract name from pattern, handling typed patterns like `val x: Type = expr`
                    let inner_pat = match &let_stmt.pattern {
                        simple_parser::ast::Pattern::Typed { pattern, .. } => pattern.as_ref(),
                        other => other,
                    };
                    if let simple_parser::ast::Pattern::Identifier(name) = inner_pat {
                        local_env.insert(name.clone(), val);
                    } else if let simple_parser::ast::Pattern::MutIdentifier(name) = inner_pat {
                        local_env.insert(name.clone(), val);
                    }
                }
            ...
            Node::Assignment(assign_stmt) => {
                ...
                if let simple_parser::ast::Expr::Identifier(name) = &assign_stmt.target {
                    ...
                    // Handle index assignment: dict["key"] = val, arr[0] = val
                    let idx_val = evaluate_expr(index, &local_env, functions, classes, enums, impl_methods)?;
                    if let simple_parser::ast::Expr::Identifier(var_name) = receiver.as_ref() {
                        if let Some(recv_val) = local_env.get(var_name).cloned() {
                            match recv_val {
                                Value::Dict(mut map) => {
                                    let key = idx_val.to_key_string();
                                    map.insert(key, val);
                                    local_env.insert(var_name.clone(), Value::Dict(map));
                                }
                                Value::Array(mut arr) => {
                                    ...
                                    if idx < arr.len() {
                                        arr[idx] = val;
                                        local_env.insert(var_name.clone(), Value::Array(arr));
                                    }
                                }
                                _ => {}
                            }
...
fn instantiate_class(
    ...
    let class_def = match classes.get(class_name) {
        Some(def) => def,
        None => {
            // Bootstrap mode: create a simple object with named fields from args
            // This handles constructors for types from other modules
            let mut fields: HashMap<String, Value> = HashMap::new();
            for (i, arg) in args.iter().enumerate() {
                let val = evaluate_expr(&arg.value, env, functions, classes, enums, impl_methods)?;
                let field_name = arg.name.clone().unwrap_or_else(|| format!("_{}", i));
