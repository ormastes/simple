...
pub(crate) fn evaluate_expr(
    ...
    match expr {
        ...
        Expr::Identifier(name) => {
            ...
            // Check if this is an extern function (registered in EXTERN_FUNCTIONS)
            let is_extern = EXTERN_FUNCTIONS.with(|cell| cell.borrow().contains(name));
            if is_extern {
                // Return a placeholder function value for extern functions
                return Ok(Value::Nil);
            }
            // Bootstrap mode: if the name looks like a type (starts with uppercase letter),
            // treat it as an external type reference and return a Constructor placeholder.
            // This allows files to reference types from other modules without module resolution.
            if name.chars().next().map_or(false, |c| c.is_uppercase()) {
                return Ok(Value::Constructor {
                    class_name: name.clone(),
                });
            }
            // For lowercase names that might be functions from other modules,
            // return Nil in bootstrap mode instead of hard errors
            Ok(Value::Nil)
        }
        Expr::New { kind, expr } => {
        ...
        Expr::Binary { op, left, right } => {
            ...
            match op {
                BinOp::Add => match (left_val, right_val) {
                    ...
                    (Value::Float(a), b) => Ok(Value::Float(a + b.as_float()?)),
                    (a, Value::Float(b)) => Ok(Value::Float(a.as_float()? + b)),
                ...
                BinOp::MatMul => Ok(Value::Int(left_val.as_int()? * right_val.as_int()?)), // MatMul stub
                BinOp::In | BinOp::NotIn => {
                    // Membership test: check if left is in right (array, tuple, dict, or string)
                    let is_not_in = matches!(op, BinOp::NotIn);
        ...
        Expr::Match { subject, arms } => {
            ...
            // Bootstrap mode: return Nil instead of erroring when no pattern matches
            Ok(Value::Nil)
        }
        ...
        Expr::FieldAccess { receiver, field } => {
            ...
            match recv_val {
                Value::Object { ref fields, ref class, .. } => {
                    ...
                    // Bootstrap mode: return Nil for unknown fields
                    Ok(Value::Nil)
                }
                Value::Constructor { ref class_name } => {
                    ...
                    if classes.contains_key(class_name) {
                        Err(CompileError::Semantic(format!("unknown static method {field} on class {class_name}")))
                    } else {
                        // Bootstrap mode: unknown class, return Nil for field access
                        Ok(Value::Nil)
                    }
                }
                ...
                Value::Str(ref s) => {
                    match field.as_str() {
                        ...
                        // Bootstrap mode: return Nil for unknown string properties
                        _ => Ok(Value::Nil),
                    }
                }
                ...
                // Bootstrap mode: return Nil for field access on non-object types
                _ => Ok(Value::Nil),
            }
        }
        ...
        Expr::Index { receiver, index } => {
            ...
            // Check if idx_val is a __range__ object (e.g., arr[1..3] slicing)
            if let Value::Object { ref class, ref fields } = idx_val {
                if class == BUILTIN_RANGE {
                    let start = fields.get("start").and_then(|v| if let Value::Int(i) = v { Some(*i) } else { None }).unwrap_or(0);
                    let end_val = fields.get("end").and_then(|v| if let Value::Int(i) = v { Some(*i) } else { None }).unwrap_or(0);
                    let inclusive = matches!(fields.get("inclusive"), Some(Value::Bool(true)));
                    return match recv_val {
                        Value::Array(arr) => {
                            let len = arr.len() as i64;
                            let s = normalize_slice_index(start, len);
                            let e = if inclusive { normalize_slice_index(end_val + 1, len) } else { normalize_slice_index(end_val, len) };
            ...
            match recv_val {
                ...
                // Bootstrap mode: return Nil for index access on non-indexable types
                // This handles cases where external types are used as containers
                _ => Ok(Value::Nil),
            }
        }
        ...
        // Optional check operator: expr.? — returns true if Option has Some value
        Expr::OptionalCheck(inner) => {
            ...
            match &val {
                Value::Enum { enum_name, variant, .. }
                    if enum_name == "Option" =>
                {
                    Ok(Value::Bool(variant == OptionVariant::Some.as_str()))
                }
                Value::Nil => Ok(Value::Bool(false)),
                _ => Ok(Value::Bool(true)), // Non-nil values are considered "present"
            }
        }
        ...
        Expr::NullCoalesce { expr, default } => {
            ...
            match &val {
                Value::Nil => evaluate_expr(default, env, functions, classes, enums, impl_methods),
                Value::Enum { enum_name, variant, payload }
                    if enum_name == "Option" =>
                {
                    match variant.as_str() {
                        v if v == OptionVariant::None.as_str() => {
                            evaluate_expr(default, env, functions, classes, enums, impl_methods)
                        }
                        v if v == OptionVariant::Some.as_str() => {
                            // Unwrap the Some payload
                            Ok(payload.as_ref().map(|p| *p.clone()).unwrap_or(Value::Nil))
                        }
                        _ => Ok(val),
                    }
                }
        ...
        // Type cast: expr as Type — in interpreter, mostly a no-op (type annotations are compile-time)
        Expr::TypeCast { expr, .. } => {
            evaluate_expr(expr, env, functions, classes, enums, impl_methods)
        }
        // Pattern binding in if conditions: if val pattern = expr
        Expr::LetBinding { pattern: _, value } => {
            let val = evaluate_expr(value, env, functions, classes, enums, impl_methods)?;
            // LetBinding as an expression evaluates to the truthiness of the binding
            // For Option: Some(x) is truthy, None is falsy
            // For other values: non-nil is truthy
            match &val {
                Value::Nil => Ok(Value::Bool(false)),
                Value::Enum { enum_name, variant, .. }
                    if enum_name == "Option" && variant == OptionVariant::None.as_str() =>
                {
                    Ok(Value::Bool(false))
                }
