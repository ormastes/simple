...
Simple includes a compile-time macro system to enable metaprogramming. Macros are programs that run at compile time to generate code (values, functions, types, or identifiers) which is then compiled as part of the program.

### Key Characteristics

**Hygiene and Explicit Declarations**: Macros operate on the AST to produce new code. Simple's macros are hygienic by default (they won't accidentally capture local variables), but require macros to declare any new names and their types that they introduce. This makes the macro system IDE-friendly - development tools can know about macro-generated constructs ahead of time.
...
The macro `define_counter(X)` produces a static `X: i64` and two functions `X_increment()` and `X_reset()`. The `{ ... }` syntax splices the provided identifier into generated code.

### Macro Capabilities

Macros can generate:

- **Values/Expressions**: Compute constants at compile time
- **Functions or Methods**: Generate function definitions
- **Types**: Generate struct, class, enum definitions or trait implementations
- **Identifiers/Names**: Create or transform identifiers
...
The compiler recognizes `define_counter` is a macro and expands it. Macros may also be used as attributes:
...
Then use `make_property(Person, age, i32)` to generate getter and setter.

### Safety and Limits

- Expanded code must still compile and type-check
- Macros cannot arbitrarily break typing rules
- Heavy computations in macros can slow compilation
