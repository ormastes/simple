...
const COMMAND_TABLE: &[CommandEntry] = &[
    ...
    CommandEntry {
        ...
        rust_handler: Handler::Custom(|_| {
            eprintln!(
                "error: llm-runtime-control app not found (run from project root or rebuild deployed app sources)"
            );
            1
        }),
        env_override: "",
...
fn run_file_with_interpreter_mode(path: &Path, gc_log: bool, gc_off: bool, args: Vec<String>) -> i32 {
