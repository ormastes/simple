...
fn find_unmanaged_files(
    ...
    for (spipe_doc, _) in parsed_files {
        managed.insert(format!("{}.md", output_stem(spipe_doc)));
    }
...
fn group_by_category(parsed_files: &[(SspecDoc, ValidationResult)]) -> Vec<Category> {
    ...
    for (spipe_doc, validation) in parsed_files {
        // Determine category
        let category_name = if let Some(ref cat) = spipe_doc.metadata.category {
            cat.clone()
        } else {
            // Infer from file path
            infer_category_from_path(&spipe_doc.file_path)
        };
        ...
        let status = if let Some(status) = &spipe_doc.metadata.status {
            status.clone()
        ...
        let entry = FeatureEntry {
            ...
            difficulty,
            test_count: count_test_cases(&spipe_doc.raw_content),
            summary: extract_summary(spipe_doc),
        };
...
fn extract_summary(spipe_doc: &SspecDoc) -> String {
...
fn render_doc_body(spipe_doc: &SspecDoc) -> String {
    ...
    if let Some(first_block) = spipe_doc.doc_blocks.first() {
        ...
        for block in spipe_doc.doc_blocks.iter().skip(1) {
