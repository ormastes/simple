...
pub fn generate_feature_doc(spipe_doc: &SspecDoc, output_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    let file_name = output_stem(spipe_doc);

    let feature_name = preferred_feature_name(spipe_doc, &file_name);

    let mut md = String::new();

    md.push_str(&format!("# {}\n\n", feature_name));
    if let Some(summary) = extract_summary(spipe_doc) {
        md.push_str(&format!("{}\n\n", summary));
    }

    let metadata_rows = build_metadata_rows(spipe_doc);
    ...
    let scenarios = extract_scenarios(&spipe_doc.raw_content);
    ...
    let body = render_doc_body(spipe_doc);
    if !body.is_empty() {
        md.push_str(&body);
        md.push_str("\n\n");
    } else {
        md.push_str("## Overview\n\n");
        md.push_str("Documentation was generated from executable SPipe scenarios.\n\n");
    }

    append_extracted_examples_section(&mut md, spipe_doc);
    append_evidence_section(&mut md, spipe_doc);
    ...
    append_related_docs_section(&mut md, spipe_doc);
...
fn build_metadata_rows(spipe_doc: &SspecDoc) -> Vec<(String, String)> {
    let mut rows = Vec::new();

    if let Some(id) = spipe_doc
        .metadata
        .id
        .clone()
        .or_else(|| (!spipe_doc.feature_ids.is_empty()).then(|| spipe_doc.feature_ids.join(", ")))
    {
        rows.push(("Feature IDs".to_string(), id));
    }
    if let Some(category) = &spipe_doc.metadata.category {
        rows.push(("Category".to_string(), category.clone()));
    }
    if let Some(difficulty) = &spipe_doc.metadata.difficulty {
        rows.push(("Difficulty".to_string(), difficulty.clone()));
    }
    if let Some(status) = &spipe_doc.metadata.status {
        rows.push(("Status".to_string(), status.clone()));
    }
    if let Some(doc_type) = &spipe_doc.metadata.doc_type {
        rows.push(("Type".to_string(), doc_type.clone()));
    }
    if let Some(source_doc) = &spipe_doc.metadata.source_doc {
        rows.push(("Reference".to_string(), source_doc.clone()));
    }
    if let Some(requirements) = &spipe_doc.metadata.requirements {
        rows.push(("Requirements".to_string(), requirements.clone()));
    }
    if let Some(plan) = &spipe_doc.metadata.plan {
        rows.push(("Plan".to_string(), plan.clone()));
    }
    if let Some(design) = &spipe_doc.metadata.design {
        rows.push(("Design".to_string(), design.clone()));
    }
    if let Some(research) = &spipe_doc.metadata.research {
        rows.push(("Research".to_string(), research.clone()));
    }
    if !spipe_doc.metadata.related.is_empty() {
        rows.push(("Related".to_string(), spipe_doc.metadata.related.join(", ")));
    }
    if !spipe_doc.metadata.dependencies.is_empty() {
        rows.push(("Dependencies".to_string(), spipe_doc.metadata.dependencies.join(", ")));
    }
    rows.push(("Source".to_string(), format!("`{}`", spipe_doc.file_path.display())));
    rows.push(("Updated".to_string(), Local::now().format("%Y-%m-%d").to_string()));
    rows.push(("Generator".to_string(), "`simple spipe-docgen` (Rust)".to_string()));
...
fn render_doc_body(spipe_doc: &SspecDoc) -> String {
    let mut sections = Vec::new();

    if let Some(first_block) = spipe_doc.doc_blocks.first() {
        let first_content = strip_header_metadata(&first_block.content);
        if !first_content.is_empty() {
            sections.push(first_content);
        }

        for doc_block in spipe_doc.doc_blocks.iter().skip(1) {
...
pub(crate) fn output_stem(spipe_doc: &SspecDoc) -> String {
    if let Some(source_doc) = &spipe_doc.metadata.source_doc {
    ...
    let path_str = spipe_doc.file_path.to_string_lossy();
...
fn preferred_feature_name(spipe_doc: &SspecDoc, stem: &str) -> String {
    if let Some(title) = &spipe_doc.feature_title {
...
fn extract_summary(spipe_doc: &SspecDoc) -> Option<String> {
    let body = render_doc_body(spipe_doc);
    if body.is_empty() {
        return None;
    }
...
fn append_evidence_section(md: &mut String, spipe_doc: &SspecDoc) {
    let evidence_groups = [
        ("Artifacts", &spipe_doc.metadata.artifacts),
        ("Screenshots", &spipe_doc.metadata.screenshots),
        ("TUI Captures", &spipe_doc.metadata.tui_captures),
        ("Logs", &spipe_doc.metadata.logs),
    ];
...
fn append_extracted_examples_section(md: &mut String, spipe_doc: &SspecDoc) {
    let examples = extract_examples(&spipe_doc.raw_content);
...
fn append_related_docs_section(md: &mut String, spipe_doc: &SspecDoc) {
    let mut links = Vec::new();

    if let Some(req) = &spipe_doc.metadata.requirements {
...
"#
        ...
        assert!(output.contains("| Generator | `simple spipe-docgen` (Rust) |"));
