...
/// Generate SPipe documentation from spec files
///
/// Returns statistics about the generation process
pub fn generate_spipe_docs(spipe_files: &[PathBuf], output_dir: &Path) -> Result<DocStats, Box<dyn std::error::Error>> {
    ...
    println!("  Input files: {}", spipe_files.len());
    println!("  Output dir: {}\n", output_dir.display());

    println!("Processing specs:");

    // Parse all spipe files
    for file_path in spipe_files {
        match parser::parse_spipe_file(file_path) {
            Ok(mut parsed) => {
                // Extract metadata
...
fn print_spec_status(spipe_doc: &SspecDoc, validation: &ValidationResult) {
    let filename = spipe_doc
        .file_path
        .file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("unknown");
