...
/// Marker every generator-owned Lean file must carry (emitted by LeanCodegen).
/// Existing files without it are treated as hand-written and never overwritten.
const GENERATED_MARKER: &str = "GENERATED by `simple gen-lean` -- DO NOT EDIT.";
...
fn write_lean_files(opts: &GenLeanOptions) -> i32 {
    ...
    for (rel_path, content) in files.iter() {
        ...
        if full_path.exists() {
            // Regeneration-safety guard: never overwrite a file that lacks the
            // generated-ownership marker — it is hand-written proof work.
            // Applies even with --force; move manual theorems to the sibling
            // *Constraints.lean file instead.
            let existing = fs::read_to_string(&full_path).unwrap_or_default();
            if !existing.contains(GENERATED_MARKER) {
                eprintln!(
                    "  [error]     {} - GenLeanUnmarkedOverwrite: existing file lacks the generated marker; \
                     move manual theorems to the sibling *Constraints.lean or restore the marker header",
                    rel_path
                );
                skipped += 1;
                continue;
