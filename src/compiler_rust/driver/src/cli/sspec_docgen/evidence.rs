//! Evidence discovery for generated SSpec documentation.
...
pub fn enrich_with_discovered_evidence(sspec_doc: &mut SspecDoc) {
    ...
    collect_evidence_files(
        &screenshot_dir_for_spec(&sspec_doc.file_path),
        &mut screenshots,
        &mut tui_captures,
        &mut logs,
        &mut artifacts,
    ...
    collect_evidence_files(
        &artifact_dir_for_spec(&sspec_doc.file_path),
        &mut screenshots,
        &mut tui_captures,
        &mut logs,
        &mut artifacts,
    ...
    if sspec_doc.metadata.screenshots.is_empty() {
        sspec_doc.metadata.screenshots = screenshots;
    }
    if sspec_doc.metadata.tui_captures.is_empty() {
        sspec_doc.metadata.tui_captures = tui_captures;
    }
    if sspec_doc.metadata.logs.is_empty() {
        sspec_doc.metadata.logs = logs;
    }
    if sspec_doc.metadata.artifacts.is_empty() {
        sspec_doc.metadata.artifacts = artifacts;
    }
}
