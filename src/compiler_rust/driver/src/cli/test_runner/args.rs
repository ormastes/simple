    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--mode" => {
                i += 1;
                if i < args.len() {
                    if let Some((mode, spec)) = TestExecutionMode::parse_str(&args[i]) {
                        options.execution_mode = mode;
                        options.composite_spec = spec;
                        // Non-interpreter modes require safe mode (subprocess execution)
            ...
            arg if arg.starts_with("--mode=") => {
                ...
                if let Some((mode, spec)) = TestExecutionMode::parse_str(mode_str) {
                    options.execution_mode = mode;
                    options.composite_spec = spec;
                    if mode != TestExecutionMode::Interpreter {
                        options.safe_mode = true;
