...
//! Example:
//!   print('describe User auth:')  →  describe "User auth":
//!                                        """Documentation"""
...
/// Migrate print-based SSpec tests to docstring format
pub fn migrate_sspec_docstrings(path: &Path, dry_run: bool) -> i32 {
    if !path.exists() {
        eprintln!("error: path does not exist: {}", path.display());
        return 1;
    }

    let files = file_walker::collect_spec_files(path);
    ...
    for file in &files {
        match migrate_file_sspec(file, dry_run) {
            Ok(true) => {
                modified_count += 1;
                if dry_run {
...
fn is_manual_tracking_line(line: &str) -> bool {
    ...
    if (line.contains("passed") || line.contains("failed")) && (line.contains("= 0") || line.contains("+ 1")) {
        return true;
    }
