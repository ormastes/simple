...
impl Parser {
    ...
    ///
    /// NOTE: This is a two-phase implementation. It first parses to SdnValue,
    /// then replays events through the handler. Full single-pass parsing with
    /// handlers is planned for Phase 2 of the handler implementation.
    pub fn parse_with_handler<H: SdnHandler>(&mut self, mut handler: H) -> Result<H::Output> {
        // Phase 1: Parse to SdnValue
        let value = self.parse()?;

        // Phase 2: Replay through handler
        crate::handlers::replay_value(&value, &mut handler)?;
...
///
/// This performs two-pass validation:
/// 1. First pass validates structure without allocation (depth/size limits)
/// 2. Second pass builds the value tree (already validated)
///
/// Default limits:
/// - max_depth: 50 (prevents stack overflow)
/// - max_string_len: 1 MB (prevents memory exhaustion)
/// - max_cell_count: 1 million (prevents DoS)
///
/// # Example
///
/// ```
/// use simple_sdn::parser::parse_untrusted;
///
/// let untrusted = "name: Alice\nage: 30";
/// let value = parse_untrusted(untrusted)?;
/// # Ok::<(), simple_sdn::SdnError>(())
/// ```
pub fn parse_untrusted(source: &str) -> Result<SdnValue> {
    // First pass: validate without allocating
    let mut parser = Parser::new(source);
    let noop = crate::NoopHandler::with_limits(50, 1_048_576, 1_000_000);
...
///
/// This enforces a strict policy:
/// - Only allows top-level key-value pairs
/// - Rejects nested dicts, arrays, and tables
/// - Useful for simple configuration files
///
/// # Example
///
/// ```
/// use simple_sdn::parser::parse_config;
///
...
pub fn parse_config(source: &str) -> Result<SdnValue> {
    let mut parser = Parser::new(source);
    let handler = crate::RestrictedHandler::flat_dict_only();
    parser.parse_with_handler(handler)
}

/// Parse with safe key validation.
///
/// This validates all dictionary keys and table field names to prevent:
...
///
/// ```
/// use simple_sdn::parser::parse_safe;
///
/// let src = "user_name: Alice\nuser_id: 42";
/// let value = parse_safe(src)?;
/// # Ok::<(), simple_sdn::SdnError>(())
/// ```
pub fn parse_safe(source: &str) -> Result<SdnValue> {
    let mut parser = Parser::new(source);
    let handler = crate::SafeKeyHandler::new();
    parser.parse_with_handler(handler)
}
...
"#;
    ...
    fn test_parse_test_db_runs_file() {
        let path =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../doc/08_tracking/test/test_db_runs.sdn");
        if !path.exists() {
            return;
        }
        let content = std::fs::read_to_string(&path).unwrap();
