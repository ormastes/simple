...
use super::{
    ...
    pattern_matches, preprocess_macro_contract_at_definition, register_trait_impl, slice_collection,
...
fn record_flattened_import_binding(marker: &str) {
    ...
    if local_name == "*" {
        MODULE_GLOBALS_BY_OWNER.with(|globals| {
            if let Some(source_globals) = globals.borrow().get(&source_owner) {
                for name in source_globals.keys() {
                    entries.insert(name.clone(), (Arc::clone(&source_owner), name.clone()));
                }
            }
        });
        ...
        match item {
            Node::Function(f) => {
                ...
                let owner: Arc<str> = f
                    ...
                    .find_map(|a| {
                        a.name
                            .strip_prefix(FLATTEN_MODULE_OWNER_ATTR_PREFIX)
                            .map(|raw| Arc::from(normalize_path_key(Path::new(raw)).to_string_lossy().as_ref()))
                    })
                ...
                // Both mutable (var) and immutable (val) need to be accessible
                {
                    if let Some(name) = get_pattern_name(&let_stmt.pattern) {
                        if let Some(value) = env.get(&name) {
                            record_flattened_global(pending_flattened_global_owner.as_ref(), name, value.clone());
                        }
                    }
                }
