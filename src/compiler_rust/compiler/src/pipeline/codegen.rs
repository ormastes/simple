...
impl CompilerPipeline {
    ...
    pub fn compile_file_to_opencl(&mut self, source: &Path, output: &Path) -> Result<(), CompileError> {
        ...
        let mir_module =
            self.type_check_and_lower_with_context_and_project_hint(&ast_module, source, project_hint.as_deref())?;
        let opencl = generate_opencl(&mir_module);
        std::fs::write(output, opencl).map_err(|e| CompileError::Io(format!("{e}")))?;
        Ok(())
    }
...
/// Generate OpenCL C source from a MIR module.
fn generate_opencl(module: &mir::MirModule) -> String {
    let mut out = String::new();
    out.push_str("//\n// Generated by Simple compiler (OpenCL backend)\n//\n\n");
    ...
    for func in &module.functions {
        ...
        emit_opencl_function(&mut out, func);
    }
...
fn emit_opencl_function(out: &mut String, func: &MirFunction) {
    let is_kernel = is_gpu_kernel_function(func);
    let return_type = if is_kernel { "void" } else { "long" };
    let prefix = if is_kernel { "__kernel " } else { "" };
    ...
    for block in &func.blocks {
        ...
        for inst in &block.instructions {
            emit_opencl_instruction(out, inst);
        }
        emit_opencl_terminator(out, &block.terminator, is_kernel);
        out.push('\n');
    }
...
fn emit_opencl_instruction(out: &mut String, inst: &MirInst) {
    match inst {
        ...
        MirInst::Load { dest, ptr } => out.push_str(&format!("    v{} = locals[v{}];\n", dest.0, ptr.0)),
        MirInst::Store { ptr, value } => out.push_str(&format!("    locals[v{}] = v{};\n", ptr.0, value.0)),
        MirInst::BinOp { dest, op, left, right } => {
            let expr = match op {
                BinOp::Add => "+",
                BinOp::Sub => "-",
        ...
        MirInst::GpuGlobalId { dest, dim } => {
            out.push_str(&format!("    v{} = get_global_id({});\n", dest.0, dim));
        }
        MirInst::GpuLocalId { dest, dim } => {
            out.push_str(&format!("    v{} = get_local_id({});\n", dest.0, dim));
        }
        MirInst::GpuGroupId { dest, dim } => {
            out.push_str(&format!("    v{} = get_group_id({});\n", dest.0, dim));
        }
        _ => out.push_str(&format!("    // unsupported OpenCL MIR instruction: {:?}\n", inst)),
    }
}

fn emit_opencl_terminator(out: &mut String, term: &Terminator, is_kernel: bool) {
    match term {
