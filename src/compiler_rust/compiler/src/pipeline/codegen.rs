...
impl CompilerPipeline {
    ...
    /// Compile a source file to OpenCL C source.
    pub fn compile_file_to_opencl(&mut self, source: &Path, output: &Path) -> Result<(), CompileError> {
        self.compile_file_to_gpu_source(source, output, GpuSourceDialect::OpenCl)
    }

    /// Compile a source file to HIP C++ source.
    pub fn compile_file_to_hip(&mut self, source: &Path, output: &Path) -> Result<(), CompileError> {
        self.compile_file_to_gpu_source(source, output, GpuSourceDialect::Hip)
    }

    fn compile_file_to_gpu_source(
        &mut self,
        source: &Path,
        output: &Path,
        dialect: GpuSourceDialect,
    ) -> Result<(), CompileError> {
        let ast_module = load_module_with_imports(source, &mut HashSet::new())?;
        let ast_module = monomorphize_module(&ast_module);
        let project_hint = native_single_file_project_hint(source);
        let mir_module =
            self.type_check_and_lower_with_context_and_project_hint(&ast_module, source, project_hint.as_deref())?;
        let source = generate_gpu_source(&mir_module, dialect);
        std::fs::write(output, source).map_err(|e| CompileError::Io(format!("{e}")))?;
        Ok(())
    }
...
enum GpuSourceDialect {
    OpenCl,
    Hip,
}

impl GpuSourceDialect {
    ...
    fn kernel_prefix(self) -> &'static str {
        match self {
            GpuSourceDialect::OpenCl => "__kernel ",
            GpuSourceDialect::Hip => r#"extern "C" __global__ "#,
        }
    }
...
fn generate_gpu_source(module: &mir::MirModule, dialect: GpuSourceDialect) -> String {
    let mut out = String::new();
    out.push_str(&format!(
        "//\n// Generated by Simple compiler ({} backend)\n//\n\n",
        dialect.label()
    ));
    ...
    for func in &module.functions {
        ...
        emit_gpu_source_function(&mut out, func, dialect);
    }
...
fn emit_gpu_source_function(out: &mut String, func: &MirFunction, dialect: GpuSourceDialect) {
    ...
    for (i, _param) in func.params.iter().enumerate() {
        ...
        out.push_str(&format!("long p{}", i));
    }
    out.push_str(") {\n");

    let max_vreg = opencl_max_vreg(func);
    for i in 0..=max_vreg.max(0) {
        out.push_str(&format!("    long v{} = 0;\n", i));
    }
    let num_locals = func.params.len() + func.locals.len();
    if num_locals > 0 {
        out.push_str(&format!("    long locals[{}];\n", num_locals));
        for i in 0..func.params.len() {
            out.push_str(&format!("    locals[{}] = p{};\n", i, i));
        }
    }
    out.push('\n');
    ...
    for block in &func.blocks {
        ...
        for inst in &block.instructions {
            emit_gpu_source_instruction(out, inst, dialect);
        }
        emit_gpu_source_terminator(out, &block.terminator, is_kernel);
        out.push('\n');
    }
...
fn opencl_max_vreg(func: &MirFunction) -> usize {
    func.blocks
        .iter()
        ...
        .filter_map(|inst| match inst {
            ...
            | MirInst::GpuLoadI64 { dest, .. } => Some(dest.0),
            _ => None,
        })
        .max()
...
fn emit_gpu_source_instruction(out: &mut String, inst: &MirInst, dialect: GpuSourceDialect) {
    match inst {
        MirInst::ConstInt { dest, value } => out.push_str(&format!("    v{} = {};\n", dest.0, value)),
        MirInst::ConstBool { dest, value } => {
            let value = if *value { 1 } else { 0 };
            out.push_str(&format!("    v{} = {};\n", dest.0, value));
        }
        MirInst::Copy { dest, src } => out.push_str(&format!("    v{} = v{};\n", dest.0, src.0)),
        MirInst::LocalAddr { dest, local_index } => {
            out.push_str(&format!("    v{} = {};\n", dest.0, local_index));
        }
        MirInst::Load { dest, addr, .. } => out.push_str(&format!("    v{} = locals[v{}];\n", dest.0, addr.0)),
        MirInst::Store { addr, value, .. } => out.push_str(&format!("    locals[v{}] = v{};\n", addr.0, value.0)),
        MirInst::BinOp { dest, op, left, right } => {
            if let Some(expr) = gpu_source_binop(op) {
                out.push_str(&format!("    v{} = v{} {} v{};\n", dest.0, left.0, expr, right.0));
            } else {
                out.push_str(&format!(
                    "    // {} binary operator requires dedicated lowering: {:?}\n",
                    dialect.unsupported_label(),
                    op
                ));
            }
        }
        MirInst::GpuGlobalId { dest, dim } => {
            out.push_str(&format!("    v{} = {};\n", dest.0, gpu_source_global_id(dialect, *dim)));
        }
        MirInst::GpuLocalId { dest, dim } => {
            out.push_str(&format!("    v{} = {};\n", dest.0, gpu_source_local_id(dialect, *dim)));
        }
        MirInst::GpuGroupId { dest, dim } => {
            out.push_str(&format!("    v{} = {};\n", dest.0, gpu_source_group_id(dialect, *dim)));
        }
...
fn gpu_source_binop(op: &BinOp) -> Option<&'static str> {
    match op {
        BinOp::Add => Some("+"),
        BinOp::Sub => Some("-"),
        BinOp::Mul => Some("*"),
        ...
        BinOp::BitOr => Some("|"),
        BinOp::BitXor => Some("^"),
        BinOp::ShiftLeft => Some("<<"),
        BinOp::ShiftRight => Some(">>"),
        BinOp::Lt => Some("<"),
        BinOp::LtEq => Some("<="),
        BinOp::Gt => Some(">"),
        BinOp::GtEq => Some(">="),
...
fn gpu_source_axis(axis: u8) -> &'static str {
    match axis {
        0 => "x",
        1 => "y",
        _ => "z",
...
fn gpu_source_global_id(dialect: GpuSourceDialect, dim: u8) -> String {
    match dialect {
        GpuSourceDialect::OpenCl => format!("get_global_id({dim})"),
        GpuSourceDialect::Hip => {
            let axis = gpu_source_axis(dim);
            format!("blockIdx.{axis} * blockDim.{axis} + threadIdx.{axis}")
        }
    }
}

fn gpu_source_local_id(dialect: GpuSourceDialect, dim: u8) -> String {
    match dialect {
        GpuSourceDialect::OpenCl => format!("get_local_id({dim})"),
        GpuSourceDialect::Hip => format!("threadIdx.{}", gpu_source_axis(dim)),
    }
}

fn gpu_source_group_id(dialect: GpuSourceDialect, dim: u8) -> String {
    match dialect {
        GpuSourceDialect::OpenCl => format!("get_group_id({dim})"),
        GpuSourceDialect::Hip => format!("blockIdx.{}", gpu_source_axis(dim)),
    }
}

fn emit_gpu_source_terminator(out: &mut String, term: &Terminator, is_kernel: bool) {
    match term {
        ...
        Terminator::Jump(target) => out.push_str(&format!("    goto BB{};\n", target.0)),
        Terminator::Branch {
            cond,
            then_block,
            else_block,
        ...
        Terminator::Unreachable => out.push_str("    return;\n"),
    }
}
...
",
    ...
    #[test]
    fn compile_file_to_opencl_writes_opencl_source_output() {
        let source_file = NamedTempFile::new().expect("temp source");
        std::fs::write(
            source_file.path(),
        ...
        assert!(
            opencl.contains("Generated by Simple compiler (OpenCL backend)"),
            "expected OpenCL header:\n{opencl}"
        );
        assert!(
            opencl.contains("__kernel void add_kernel(long p0, long p1)"),
            "expected OpenCL kernel signature:\n{opencl}"
        );
