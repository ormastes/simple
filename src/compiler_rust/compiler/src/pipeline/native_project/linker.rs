...
impl NativeProjectBuilder {
    ...
    pub(crate) fn freestanding_bare_to_qualified_defsyms(
        ...
        for reference in &undefined {
            ...
            // Runtime ABI names belong to the selected freestanding provider.
            // Aliasing a dead hosted fallback keeps its process/file imports live.
            if reference.trim_start_matches('_').starts_with("rt_") {
                continue;
            }
