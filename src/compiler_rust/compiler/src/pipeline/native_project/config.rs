...
use super::tools::{
    archive_defined_symbols, build_core_c_runtime_library, build_host_gpu_c_runtime_library, find_abi_complete_simple_core_runtime_library,
...
impl NativeProjectBuilder {
    ...
    pub(crate) fn selected_runtime_library(&self, temp_dir: &Path) -> Result<Option<(PathBuf, bool)>, String> {
        ...
        if runtime_bundle_requests_host_gpu(&self.config.runtime_bundle) {
            ...
            let core = build_host_gpu_c_runtime_library(&core_dir)
