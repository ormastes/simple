...
thread_local! {
    ...
    /// Newtype definitions visible to the interpreter.
    pub(crate) static NEWTYPES: RefCell<HashMap<String, Arc<NewtypeDef>>> = RefCell::new(HashMap::new());
...
/// Set the concurrent provider registry (called at interpreter startup).
pub(crate) fn set_concurrent_registry(registry: ConcurrentProviderRegistry) {
    CONCURRENT_REGISTRY.with(|cell| {
        *cell.borrow_mut() = Arc::new(registry);
    });
}

/// Get the concurrent provider registry.
pub(crate) fn get_concurrent_registry() -> Arc<ConcurrentProviderRegistry> {
    CONCURRENT_REGISTRY.with(|cell| cell.borrow().clone())
}
