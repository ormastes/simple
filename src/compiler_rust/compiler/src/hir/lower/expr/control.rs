...
/// An `if`/`match` arm contributes nothing to the value-position type JOIN
/// when it is BOTTOM: a bare `nil` literal, a call whose result is
/// definitionally absent (`panic(...)` lowers to a `rt_panic` `BuiltinCall`
/// typed `TypeId::NIL` — see `calls.rs` `"panic" => ... TypeId::NIL`), or the
/// arm definitely transfers control away (`return`, or a nested `if` whose
/// both branches return). Such an arm must never win the join over a sibling
/// that actually produces the value: `if c: make() else: nil` and
/// `match x: Err(e): panic(e); Ok(v): v` must both report the OTHER arm's
/// type, not `Nil`/`ANY`.
fn hir_arm_is_bottom(expr: &HirExpr) -> bool {
    expr.ty == TypeId::NIL || hir_expr_definitely_returns(expr)
}

/// Placeholder-lambda parameter names generated by
/// `parser/expressions/placeholder.rs` (`_` / `_1`/`_2`/... -> `__p0`,
/// `__p1`, ...). Used to scope the ANY-default fix (class 3) to exactly the
/// synthetic params the placeholder desugar introduces, never a
/// user-written unannotated lambda parameter.
fn is_placeholder_lambda_param_name(name: &str) -> bool {
    match name.strip_prefix("__p") {
...
impl Lowerer {
    ...
    pub(super) fn lower_if(
        ...
        // Class 5: a bottom then-arm (bare `nil`, `panic(...)`, or a
        // definitely-returning branch) must not win the join over a real
        // else-arm value. The symmetric case (else bottom, then real) needs
        // no extra handling: `ty` already defaults to `then_hir.ty` above.
        if let Some(other) = else_hir.as_ref() {
            if hir_arm_is_bottom(&then_hir) && !hir_arm_is_bottom(other) {
                ty = other.ty;
            }
        }
    ...
    pub(super) fn lower_lambda_with_param_types(
        ...
        let param_info: Vec<(String, TypeId)> = params
            ...
            .map(|(index, p)| {
                ...
                } else if let Some(inferred) = inferred_param_types.get(index).copied() {
                    // An expected fn/callee param type IS available at this
                    // call site (e.g. `array_sort_by`, `.map`/`.filter`) —
                    // always prefer it, for placeholder params exactly as for
                    // a named unannotated one.
                    inferred
                } else if is_placeholder_lambda_param_name(&p.name) {
                    // Class 3: a placeholder-lambda param (`_`/`_1`/`_2` desugared
                    // to `__p0`/`__p1`/...) with no expected type available
                    // must default to ANY, not I64. Defaulting to I64 made
                    // `_.flag` receiver type "proven scalar", so even the
                    // ANY-safe codegen guard (class 1/2) took the raw `icmp`/
                    // field-access path instead of the dynamic one, and a
                    // non-numeric receiver (e.g. a struct's `.flag` field)
                    // silently mis-executed instead of routing dynamically.
                    // No behavior change for a NAMED unannotated lambda param
                    // (e.g. `\x: x.flag`), which is intentionally excluded.
                    TypeId::ANY
                } else {
                    TypeId::I64
                };
    ...
    fn lower_match_arms(
        ...
        // Class 5: a BOTTOM arm — a return-only arm, a bare `nil`, or
        // `panic(...)` (which lowers to `rt_panic` typed `TypeId::NIL`) —
        // does not contribute a value to the match join. Using its type as
        // the whole match type used to degrade `Err(_): return Err(...);
        // Ok(value): value` to ANY (arithmetic then re-boxed the otherwise
        // raw i64 before a direct i64 call, 6 -> 48), and separately made
        // `case Err(e): panic(e); case Ok(v): v` report `Nil` instead of
        // `v`'s real type.
        let then_is_bottom = hir_arm_is_bottom(&then_branch);
        let result_ty = if then_is_bottom { else_branch.ty } else { then_ty };
    ...
    pub(super) fn lower_try(
        ...
        // Class 2: a bare `!` (never `?`, so `propagate_absence` is false)
        // whose subject the compiler could not prove is a Result/Option enum
        // (untyped/ANY, or some other unresolved shape not caught by the
        // Pointer-typed branches above) must NOT take the propagate-style
        // "is-Err -> early return" branch below. At runtime a non-enum value
        // safely fails `rt_enum_check_variant` (answers false), but the
        // else-arm `rt_enum_payload` then asks that NON-enum for its payload
        // and silently answers a bare `nil`, discarding the real value —
        // `x == nil or f(x!)` on a plainly-present `x` evaluated `f(nil)`.
        // `!` never propagates absence, so route it through the same
        // tag-aware normalizer the Pointer-typed branches above already use:
        // identity on a bare/flat word, payload extraction only for the
        // reserved Option tag, and identity on any other (e.g. Result) enum —
        // never a `return`.
        if !propagate_absence && self.result_like_payload_type(subject_ty).is_none() {
            let unwrap_dyn = HirExpr {
                kind: HirExprKind::BuiltinCall {
                    name: "rt_unwrap_or_self".to_string(),
                    args: vec![subject_ref],
                },
