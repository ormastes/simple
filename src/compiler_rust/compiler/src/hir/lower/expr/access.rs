...
impl Lowerer {
        ...
        match self.get_field_info(recv_hir.ty, field) {
            ...
            Err(LowerError::CannotInferFieldType { struct_name, .. }) => {
                ...
                for candidate_struct_name in &candidate_struct_names {
                    ...
                    {
                        if std::env::var("SIMPLE_TRACE_FIELD_GET").is_ok() { let fpath = self.current_file.as_ref().and_then(|p| p.file_name()).and_then(|n| n.to_str()).unwrap_or("unknown"); eprintln!("[FT2] CAND-BYNAME/{field} idx={field_index} in {fpath}"); }
                        return Ok(HirExpr {
                            kind: HirExprKind::FieldAccess {
                                receiver: recv_hir,
                                field_index,
                ...
                if candidate_struct_names.is_empty() && !self.is_ambiguous_global_field(field) {
                    if let Some((field_index, field_ty, _count, _sname)) = self.resolve_global_field_info(field) {
                        if std::env::var("SIMPLE_TRACE_FIELD_GET").is_ok() { let fpath = self.current_file.as_ref().and_then(|p| p.file_name()).and_then(|n| n.to_str()).unwrap_or("unknown"); eprintln!("[FT2] CLASS2-GLOBAL/{field} idx={field_index} in {fpath}"); }
                        return Ok(HirExpr {
                            kind: HirExprKind::FieldAccess {
                                receiver: recv_hir,
                                field_index,
                ...
                if !has_known_method {
                    ...
                    if let Some((field_index, field_ty, _count, _sname)) = self.resolve_global_field_info(field) {
                        if std::env::var("SIMPLE_TRACE_FIELD_GET").is_ok() { let fpath = self.current_file.as_ref().and_then(|p| p.file_name()).and_then(|n| n.to_str()).unwrap_or("unknown"); eprintln!("[FT2] NKM-GLOBAL/{field} idx={field_index} in {fpath}"); }
                        return Ok(HirExpr {
                            kind: HirExprKind::FieldAccess {
                                receiver: recv_hir,
                                field_index,
                    ...
                    if let Some((field_index, field_ty, _)) = best {
                        if std::env::var("SIMPLE_TRACE_FIELD_GET").is_ok() { let fpath = self.current_file.as_ref().and_then(|p| p.file_name()).and_then(|n| n.to_str()).unwrap_or("unknown"); eprintln!("[FT2] NKM-LOCALBEST/{field} idx={field_index} in {fpath}"); }
                        return Ok(HirExpr {
                            kind: HirExprKind::FieldAccess {
                                receiver: recv_hir,
                                field_index,
    ...
    fn try_resolve_global_field_type_by_name(&mut self, struct_name: &str, field_name: &str) -> Option<TypeId> {
        ...
        let field_type_name = {
            let fields = self.global_struct_fields_for_name(struct_name);
            if let Some(fields) = fields {
                fields
                    .iter()
                    .find_map(|(fname, ftype)| if fname == field_name { Some(ftype.clone()) } else { None })?
            } else if !has_nominal_owner {
                return self
                    .resolve_duplicate_global_field_variant(struct_name, field_name)
    ...
    fn try_resolve_global_field_index_by_name(&self, struct_name: &str, field_name: &str) -> Option<usize> {
        ...
        if self.global_struct_key_for_name(struct_name).is_some() {
            return None;
        }
