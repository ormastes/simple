        ...
        if let Some(hir_ty) = self.module.types.get(struct_ty).cloned() {
            match hir_ty {
                ...
                HirType::Struct { name, fields, .. } => {
                    ...
                    {
                        if std::env::var("SIMPLE_TRACE_FIELD_GET").is_ok() { let fpath = self.current_file.as_ref().and_then(|p| p.file_name()).and_then(|n| n.to_str()).unwrap_or("unknown"); eprintln!("[FT2] S-GLOBAL/{field} struct={name} idx={global_idx} in {fpath}"); }
                    ...
                    {
                        if std::env::var("SIMPLE_TRACE_FIELD_GET").is_ok() { let fpath = self.current_file.as_ref().and_then(|p| p.file_name()).and_then(|n| n.to_str()).unwrap_or("unknown"); eprintln!("[FT2] S-SAMENAME/{field} struct={name} idx={variant_idx} in {fpath}"); }
                        return Ok((variant_idx, variant_field_ty));
                    }
                    if let Some((idx, owner_field_ty)) = self.try_resolve_current_owner_field(field) {
                        if std::env::var("SIMPLE_TRACE_FIELD_GET").is_ok() { let fpath = self.current_file.as_ref().and_then(|p| p.file_name()).and_then(|n| n.to_str()).unwrap_or("unknown"); eprintln!("[FT2] S-OWNER/{field} struct={name} idx={idx} in {fpath}"); }
                        return Ok((idx, owner_field_ty));
                    }
                    if let Some((idx, field_ty, _, _)) = self.resolve_global_field_info(field) {
                        if std::env::var("SIMPLE_TRACE_FIELD_GET").is_ok() { let fpath = self.current_file.as_ref().and_then(|p| p.file_name()).and_then(|n| n.to_str()).unwrap_or("unknown"); eprintln!("[FT2] S-GLOBALINFO/{field} struct={name} idx={idx} in {fpath}"); }
                        return Ok((idx, field_ty));
                    }
                    let mut best: Option<(usize, TypeId, usize)> = None;
                    for (_, search_ty) in self.module.types.iter() {
                    ...
                    if let Some((idx, field_ty, _)) = best {
                        if std::env::var("SIMPLE_TRACE_FIELD_GET").is_ok() { let fpath = self.current_file.as_ref().and_then(|p| p.file_name()).and_then(|n| n.to_str()).unwrap_or("unknown"); eprintln!("[FT2] S-BEST/{field} struct={name} idx={idx} in {fpath}"); }
                        return Ok((idx, field_ty));
                    }
