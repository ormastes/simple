...
pub enum LintName {
    ...
    TodoFormat,
    /// Print-based BDD tests (use proper SPipe syntax)
    ...
    /// Tautology or placeholder-success patterns in SPipe examples
...
impl LintName {
    ...
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            LintName::SPipeNoPrintBasedTests => "spipe_no_print_based_tests",
            LintName::SPipeMissingDocstrings => "spipe_missing_docstrings",
            LintName::SPipeMinimalDocstrings => "spipe_minimal_docstrings",
            LintName::SPipeManualAssertions => "spipe_manual_assertions",
    ...
    pub fn from_str(s: &str) -> Option<Self> {
        match s {
            ...
            "spipe_no_print_based_tests" => Some(LintName::SPipeNoPrintBasedTests),
            "spipe_missing_docstrings" => Some(LintName::SPipeMissingDocstrings),
            "spipe_minimal_docstrings" => Some(LintName::SPipeMinimalDocstrings),
            "spipe_manual_assertions" => Some(LintName::SPipeManualAssertions),
    ...
    pub fn default_level(&self) -> LintLevel {
        match self {
            ...
            // SPipe lints: no_print_based_tests is Deny (error), others are Warn
            LintName::SPipeNoPrintBasedTests => LintLevel::Deny,
            LintName::SPipeMissingDocstrings => LintLevel::Warn,
            LintName::SPipeMinimalDocstrings => LintLevel::Warn, // Info level not supported, use Warn
...
"#
            .to_string(),
            LintName::SPipeNoPrintBasedTests => "Lint: spipe_no_print_based_tests\nLevel: deny\n\nDetects print-based assertions in SPipe tests. Use expect() matchers instead.".to_string(),
            LintName::SPipeMissingDocstrings => "Lint: spipe_missing_docstrings\nLevel: warn\n\nWarns when describe/context/it blocks lack docstrings.".to_string(),
            LintName::SPipeMinimalDocstrings => "Lint: spipe_minimal_docstrings\nLevel: warn\n\nWarns when test files have minimal documentation.".to_string(),
...
"#.to_string(),
    ...
    pub fn all_lints() -> Vec<Self> {
        vec![
            ...
            LintName::TodoFormat,
            LintName::SPipeNoPrintBasedTests,
            LintName::SPipeMissingDocstrings,
            LintName::SPipeMinimalDocstrings,
            LintName::SPipeManualAssertions,
