//! Lint checker: SPipe, TODO format, source quality, and stub pattern checks.
...
impl LintChecker {
    ...
    /// Check for print-based BDD tests (spipe_no_print_based_tests)
    pub(super) fn check_spipe_print_based_tests(&mut self, items: &[Node]) {
        use simple_parser::ast::Expr;
                                    ...
                                    for keyword in BDD_KEYWORDS {
                                        if s.contains(keyword) {
                                            checker.emit(
                                                ...
                                                Some("use proper SPipe syntax: describe/context/it blocks".to_string()),
                                            );
                                            return;
                                        }
                                    }
    ...
    /// Check for minimal docstring usage in SPipe files
    pub(super) fn check_spipe_minimal_docstrings(&mut self, source_file: &std::path::Path) {
        let source = match std::fs::read_to_string(source_file) {
            Ok(s) => s,
            Err(_) => return,
        };
    ...
    pub(super) fn check_spipe_manual_assertions(&mut self, items: &[Node]) {
    ...
    pub(super) fn check_source_backed_quality_patterns(&mut self, source_file: &std::path::Path) {
        ...
        if Self::is_test_like_path(source_file) {
            self.check_spipe_placeholder_patterns(&source);
            self.check_spipe_example_bodies_from_source(&source);
        } else {
        ...
        for (idx, line) in source.lines().enumerate() {
            ...
            if Self::is_false_boolean_wrapper_assertion(&normalized) {
                ...
                self.emit_with_fix(
                    ...
                    Span::new(0, 0, line_num, 1),
                    "false boolean matcher wrapper in spec/example; use expect_not(condition) instead of .to_equal(false) or .to_be(false)".to_string(),
                ...
                self.emit_with_fix(
                    ...
                    Span::new(0, 0, line_num, 1),
                    "verbose true boolean assertion in spec/example; use expect(condition) instead of .to_equal(true) or .to_be(true)"
                        .to_string(),
    ...
    pub(super) fn check_spipe_example_bodies_from_source(&mut self, source: &str) {
        ...
        while i < lines.len() {
            ...
            if statements.is_empty() {
                self.emit(
                    ...
                    Span::new(0, 0, line_num, 1),
                    "SPipe example has no body".to_string(),
            ...
            if !has_real_assertion && !has_sanctioned_skip {
                self.emit(
                    LintName::SPipeEmptyExamples,
                    Span::new(0, 0, line_num, 1),
                    "SPipe example has no real assertion or sanctioned skip".to_string(),
