...
mod winit_sffi_thread;
mod winit_sffi_window;
mod winit_sffi_events;
mod winit_sffi_input;
mod winit_sffi_display;
mod winit_sffi_buffer;
...
pub fn dispatch(name: &str, args: &[Value]) -> Result<Value, CompileError> {
    match name {
        n if n.starts_with("rt_winit_event_loop_") || n.starts_with("rt_winit_window_") => {
            winit_sffi_window::dispatch_window(name, args)
        }
        ...
        {
            winit_sffi_display::dispatch_display(name, args)
        }
