...
//! This avoids serializing/deserializing entire AST trees across the SFFI boundary.
//!
//! ## Handle Lifecycle
...
// ============================================================================
// Expression Accessors (SFFI functions callable from Simple)
// ============================================================================
...
/// Clear all AST SFFI registries between test runs.
pub fn clear_ast_sffi_registries() {
    EXPR_REGISTRY.with(|r| r.borrow_mut().clear());
    NODE_REGISTRY.with(|r| r.borrow_mut().clear());
    PATTERN_REGISTRY.with(|r| r.borrow_mut().clear());
    BLOCK_REGISTRY.with(|r| r.borrow_mut().clear());
