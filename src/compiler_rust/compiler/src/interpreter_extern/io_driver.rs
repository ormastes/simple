...
extern "C" {
    ...
    fn rt_driver_submit_sendfile(
        handle: i64, sock_fd: i64, file_fd: i64, offset: i64, len: i64,
    ) -> i64;
    fn rt_driver_submit_read(handle: i64, fd: i64, buf_size: i64, offset: i64) -> i64;
    fn rt_driver_submit_write(
        handle: i64, fd: i64, data: *const i8, len: i64, offset: i64,
    ) -> i64;
...
pub fn dispatch(name: &str, args: &[Value]) -> Option<Result<Value, CompileError>> {
    let result = match name {
        "rt_driver_create" => {
            Ok(Value::Int(unsafe { rt_driver_create(get_i64(args, 0)) }))
        }
        "rt_driver_destroy" => {
        ...
        "rt_driver_submit_accept" => {
            Ok(Value::Int(unsafe { rt_driver_submit_accept(get_i64(args, 0), get_i64(args, 1)) }))
        }
        ...
        "rt_driver_submit_recv" => {
            Ok(Value::Int(unsafe { rt_driver_submit_recv(get_i64(args, 0), get_i64(args, 1), get_i64(args, 2)) }))
        }
        "rt_driver_submit_send" => {
            let (ptr, _) = get_str_raw(args, 2);
            let len = get_i64(args, 3);
            Ok(Value::Int(unsafe { rt_driver_submit_send(get_i64(args, 0), get_i64(args, 1), ptr, len) }))
        }
        "rt_driver_submit_sendfile" => {
            Ok(Value::Int(unsafe {
                rt_driver_submit_sendfile(
                    get_i64(args, 0), get_i64(args, 1), get_i64(args, 2),
                    get_i64(args, 3), get_i64(args, 4),
                )
            }))
        }
        ...
        "rt_driver_submit_write" => {
            ...
            Ok(Value::Int(unsafe {
                rt_driver_submit_write(
                    get_i64(args, 0), get_i64(args, 1), ptr,
                    get_i64(args, 3), get_i64(args, 4),
                )
            }))
        }
        ...
        "rt_driver_submit_close" => {
            Ok(Value::Int(unsafe { rt_driver_submit_close(get_i64(args, 0), get_i64(args, 1)) }))
        }
        "rt_driver_submit_fsync" => {
            Ok(Value::Int(unsafe { rt_driver_submit_fsync(get_i64(args, 0), get_i64(args, 1)) }))
        }
        "rt_driver_submit_timeout" => {
            Ok(Value::Int(unsafe { rt_driver_submit_timeout(get_i64(args, 0), get_i64(args, 1)) }))
        }
        "rt_driver_flush" => {
            Ok(Value::Int(unsafe { rt_driver_flush(get_i64(args, 0)) }))
        }
        "rt_driver_poll" => {
            Ok(Value::Int(unsafe { rt_driver_poll(get_i64(args, 0), get_i64(args, 1), get_i64(args, 2)) }))
        }
        "rt_driver_poll_id" => {
            Ok(Value::Int(unsafe { rt_driver_poll_id(get_i64(args, 0), get_i64(args, 1)) }))
        }
        "rt_driver_poll_result" => {
            Ok(Value::Int(unsafe { rt_driver_poll_result(get_i64(args, 0), get_i64(args, 1)) }))
        }
        "rt_driver_poll_flags" => {
            Ok(Value::Int(unsafe { rt_driver_poll_flags(get_i64(args, 0), get_i64(args, 1)) }))
        }
        "rt_driver_poll_data" => {
        ...
        "rt_driver_poll_data_len" => {
            Ok(Value::Int(unsafe { rt_driver_poll_data_len(get_i64(args, 0), get_i64(args, 1)) }))
        }
        "rt_driver_cancel" => {
        ...
        "rt_driver_supports_sendfile" => {
            Ok(Value::Bool(unsafe { rt_driver_supports_sendfile(get_i64(args, 0)) }))
        }
        "rt_driver_supports_zero_copy" => {
            Ok(Value::Bool(unsafe { rt_driver_supports_zero_copy(get_i64(args, 0)) }))
        }
        _ => return None,
    };
    Some(result)
...
pub fn dispatch(name: &str, _args: &[Value]) -> Option<Result<Value, CompileError>> {
    if name.starts_with("rt_driver_") {
        Some(Err(CompileError::runtime("Async I/O driver not available on this platform")))
    } else {
        None
    }
}
