...
pub struct MirModule {
    ...
    /// emitted as placed LLVM constant arrays, never tagged heap values.
    pub raw_data_items: Vec<crate::hir::HirRawDataItem>,
    ...
    /// Globals assigned by the synthesized `__module_init_dynamic` function
    /// (see hir/lower/module_lowering/module_pass.rs) rather than one of the
    /// const-foldable shapes above. Must be treated as writable data by
    /// codegen, the same as the other global_init_* categories.
    pub dynamic_init_globals: std::collections::HashSet<String>,
...
impl MirModule {
    pub fn new() -> Self {
        Self {
            ...
            dynamic_init_globals: std::collections::HashSet::new(),
