...
pub struct PreLexInfo {
    /// `"..."` and `'...'` string literal regions
    pub string_spans: Vec<TextSpan>,
    /// Line/block comment regions
    pub comment_spans: Vec<TextSpan>,
    /// Backslash escape positions
    pub escape_positions: Vec<usize>,
    /// Matched `{` `}` pairs (start, end) with nesting
