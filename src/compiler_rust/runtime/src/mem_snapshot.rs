...
//!
//! SINGLE authoritative Rust definition of the `rt_mem_snapshot_*` ABI.
//! It previously existed twice -- here (as `simple-runtime::mem_snapshot`)
//! and again as `simple-native-all::mem_snapshot_provider` -- and because
//! `simple-native-all` depends on `simple-runtime`, both `#[no_mangle]`
//! sets landed in one link unit and rust-lld rejected the whole
//! `simple-native-all` test binary with `duplicate symbol`, aborting
//! `cargo test` before any test ran. The two bodies were NOT equivalent:
//! the old `simple-runtime` copy omitted the `run_id=` field that both C
//! copies (`runtime.c`, `runtime_legacy_core.c`) and the native-all copy
//! emit, so the native-all body is the canonical one and is what survives
//! here. `rt_phase_profile_record` came with it and is now available to
//! every crate that links `simple-runtime`, not just native-all.
...
pub unsafe extern "C" fn rt_mem_snapshot_record(
    ...
    #[cfg(unix)]
    {
        ...
        let ok = write!(&mut line,
            ...
            crate::value::heap::rt_heap_live_bytes(),
            crate::value::heap::rt_heap_peak_bytes(),
...
mod tests {
    ...
    #[test]
    fn record_line_carries_run_id_and_escapes_tokens() {
        let dir = tempfile::tempdir().unwrap();
