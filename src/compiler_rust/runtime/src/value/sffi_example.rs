//! Example SFFI wrapper demonstrating the object-based SFFI system.
//!
//! This module shows how to wrap a Rust type using the SFFI system.
//!
...
use super::sffi_macros::{get_arg_as, IntoRuntimeValue};
use super::sffi_object::{fnv1a_hash, method_flags, FfiMethodEntry, FfiVtable};
use super::sffi_registry::get_registry;
...
// ============================================================================
// SFFI Wrapper Implementation using OnceLock (safe pattern)
// ============================================================================

/// Counter SFFI state, initialized once.
struct CounterFfi {
...
static COUNTER_SFFI: OnceLock<CounterFfi> = OnceLock::new();

/// Get or initialize the Counter SFFI state.
fn get_counter_sffi() -> &'static CounterFfi {
    COUNTER_SFFI.get_or_init(|| {
        ...
        let mut methods = vec![
            ...
            FfiMethodEntry::new(
                ...
                counter_increment_sffi as *const (),
            ),
            FfiMethodEntry::new(
                ...
                method_flags::MUTABLE,
                counter_increment_by_sffi as *const (),
            ),
            FfiMethodEntry::new(
        ...
        let vtable = FfiVtable::new(
            ...
            methods.len() as u32,
            methods.as_ptr(),
            Some(counter_drop_sffi),
            Some(counter_clone_sffi),
        );
...
#[no_mangle]
pub extern "C" fn counter_new_sffi(initial: i64) -> RuntimeValue {
    // Ensure SFFI is initialized
    let _ = get_counter_sffi();
...
/// Drop function called when the SFFI object is destroyed.
unsafe extern "C" fn counter_drop_sffi(handle: u32) {
...
unsafe extern "C" fn counter_clone_sffi(handle: u32) -> u32 {
...
unsafe extern "C" fn counter_get_sffi(handle: u32, _argc: u32, _argv: *const RuntimeValue) -> RuntimeValue {
    let type_id = get_registry().get_type_id("Counter");
    if type_id == 0 {
        return RuntimeValue::NIL;
    }
...
/// Increment the counter by 1.
unsafe extern "C" fn counter_increment_sffi(handle: u32, _argc: u32, _argv: *const RuntimeValue) -> RuntimeValue {
    let type_id = get_registry().get_type_id("Counter");
    if type_id == 0 {
        return RuntimeValue::NIL;
    }
...
/// Increment the counter by a specific amount.
unsafe extern "C" fn counter_increment_by_sffi(handle: u32, argc: u32, argv: *const RuntimeValue) -> RuntimeValue {
    let type_id = get_registry().get_type_id("Counter");
    if type_id == 0 {
        return RuntimeValue::NIL;
    }
...
/// Reset the counter to 0.
unsafe extern "C" fn counter_reset_sffi(handle: u32, _argc: u32, _argv: *const RuntimeValue) -> RuntimeValue {
    let type_id = get_registry().get_type_id("Counter");
    if type_id == 0 {
        return RuntimeValue::NIL;
    }
...
/// Set the counter to a specific value.
unsafe extern "C" fn counter_set_sffi(handle: u32, argc: u32, argv: *const RuntimeValue) -> RuntimeValue {
    let type_id = get_registry().get_type_id("Counter");
    if type_id == 0 {
        return RuntimeValue::NIL;
    }
...
mod tests {
    use super::*;
    use crate::value::sffi_object::rt_sffi_object_is_ffi;
    use crate::value::sffi_registry::rt_sffi_clear_registry;
    ...
    fn test_counter_create_and_use() {
        ...
        // Create a counter
        let counter = counter_new_sffi(10);
        assert!(!counter.is_nil());
        assert!(rt_sffi_object_is_ffi(counter));

        // Get value
        unsafe {
            let handle = super::super::sffi_object::rt_sffi_object_handle(counter);
            let value = counter_get_sffi(handle, 0, std::ptr::null());
            assert!(value.is_int());
    ...
    fn test_counter_increment() {
        ...
        let counter = counter_new_sffi(0);
        assert!(!counter.is_nil());
        ...
        unsafe {
            ...
            // Increment
            counter_increment_sffi(handle, 0, std::ptr::null());
            counter_increment_sffi(handle, 0, std::ptr::null());
            counter_increment_sffi(handle, 0, std::ptr::null());
    ...
    fn test_counter_increment_by() {
        ...
        unsafe {
            ...
            counter_increment_by_sffi(handle, 1, args.as_ptr());
    ...
    fn test_counter_reset() {
        ...
        let counter = counter_new_sffi(100);

        unsafe {
            let handle = super::super::sffi_object::rt_sffi_object_handle(counter);

            // Reset
            counter_reset_sffi(handle, 0, std::ptr::null());
    ...
    fn test_counter_set() {
        ...
        unsafe {
            ...
            counter_set_sffi(handle, 1, args.as_ptr());
    ...
    fn test_counter_drop() {
        ...
        // Create counters
        let _c1 = counter_new_sffi(1);
        let _c2 = counter_new_sffi(2);
        let _c3 = counter_new_sffi(3);
