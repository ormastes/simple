//! SFFI Type Registry for object-based SFFI system.
//!
//! This module provides the central registry for SFFI types, managing:
...
lazy_static::lazy_static! {
    /// Global SFFI type registry.
    static ref SFFI_TYPE_REGISTRY: FfiTypeRegistry = FfiTypeRegistry::new();
}

/// Get a reference to the global SFFI type registry.
pub fn get_registry() -> &'static FfiTypeRegistry {
    &SFFI_TYPE_REGISTRY
}

// ============================================================================
// Storage for SFFI Objects
// ============================================================================

/// Thread-safe storage for SFFI objects of a specific type.
///
...
/// Metadata for a registered SFFI type.
...
/// Central registry for SFFI types.
///
...
impl FfiTypeRegistry {
    ...
    ///
    /// # Returns
    /// A RuntimeValue containing the SFFI object, or NIL if the type is not registered.
    pub fn create_object<T: Any + Send + Sync>(&self, type_id: u32, object: T) -> RuntimeValue {
        ...
        // Create the SFFI object on the heap
        unsafe { super::sffi_object::rt_sffi_object_new(type_id, handle, metadata.vtable) }
    }
    ...
    pub unsafe fn drop_object(&self, value: RuntimeValue) -> bool {
        ...
        if !sffi_obj.vtable.is_null() {
            if let Some(drop_fn) = (*sffi_obj.vtable).drop_fn {
                drop_fn(handle);
            }
        }
        ...
        super::sffi_object::rt_sffi_object_free(value);

        true
    }

    /// Clone an SFFI object.
    ///
    ...
    pub unsafe fn clone_object(&self, value: RuntimeValue) -> RuntimeValue {
        ...
        let clone_fn = (*sffi_obj.vtable).clone_fn;
        let Some(clone_fn) = clone_fn else {
            return RuntimeValue::NIL;
        };

        // Call the clone function to get a new handle
        let new_handle = clone_fn(sffi_obj.handle);
        if new_handle == 0 {
            return RuntimeValue::NIL;
        }

        // Create a new SFFI object with the new handle
        super::sffi_object::rt_sffi_object_new(sffi_obj.type_id, new_handle, sffi_obj.vtable)
    }
...
// ============================================================================
// SFFI Functions for Type Registry
// ============================================================================
...
#[no_mangle]
pub extern "C" fn rt_sffi_type_register(name_ptr: *const u8, name_len: u32, vtable: *const FfiVtable) -> u32 {
...
#[no_mangle]
pub extern "C" fn rt_sffi_type_id(name_ptr: *const u8, name_len: u32) -> u32 {
    if name_ptr.is_null() {
        return 0;
    }
...
#[no_mangle]
pub extern "C" fn rt_sffi_type_name(type_id: u32) -> RuntimeValue {
...
/// A RuntimeValue containing the SFFI object, or NIL if the type is not registered.
#[no_mangle]
pub extern "C" fn rt_sffi_new(type_id: u32, handle: u32) -> RuntimeValue {
    let vtable = get_registry().get_vtable(type_id);
    if vtable.is_null() {
        return RuntimeValue::NIL;
    }

    unsafe { super::sffi_object::rt_sffi_object_new(type_id, handle, vtable) }
}
...
#[no_mangle]
pub extern "C" fn rt_sffi_drop(value: RuntimeValue) -> bool {
...
#[no_mangle]
pub extern "C" fn rt_sffi_clone(value: RuntimeValue) -> RuntimeValue {
    unsafe { get_registry().clone_object(value) }
}

/// Check if a value is an SFFI object of a specific type.
#[no_mangle]
pub extern "C" fn rt_sffi_is_type(value: RuntimeValue, type_id: u32) -> bool {
    super::sffi_object::rt_sffi_object_type_id(value) == type_id
}

/// Call a method on an SFFI object.
///
/// # Parameters
/// - `value`: The SFFI object
...
#[no_mangle]
pub unsafe extern "C" fn rt_sffi_call_method(
    value: RuntimeValue,
    name_ptr: *const u8,
    name_len: u32,
    argc: u32,
    ...
    super::sffi_object::rt_sffi_object_call_method(value, name_ptr, name_len, argc, argv)
}

/// Check if an SFFI object has a specific method.
#[no_mangle]
pub extern "C" fn rt_sffi_has_method(value: RuntimeValue, name_ptr: *const u8, name_len: u32) -> bool {
    super::sffi_object::rt_sffi_object_has_method(value, name_ptr, name_len)
}

/// Clear the SFFI type registry (for testing).
#[no_mangle]
pub extern "C" fn rt_sffi_clear_registry() {
