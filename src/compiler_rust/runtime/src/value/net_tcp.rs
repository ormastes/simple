...
/// Simple-facing bind for a pre-created TCP socket handle.
#[no_mangle]
pub extern "C" fn rt_io_tcp_bind_fd(handle: i64, addr: crate::value::RuntimeValue) -> bool {
    let Some((ptr, len)) = runtime_text_ptr_len(addr) else {
        return false;
    };
    let addr = unsafe {
        match parse_socket_addr(ptr, len) {
            Ok(a) => a,
            Err(_) => return false,
        }
    };
    let mut registry = SOCKET_REGISTRY.lock().unwrap();
    let Some(SocketEntry::TcpSocket(socket)) = registry.get_mut(&handle) else {
        return false;
    };
    socket.bind(&addr.into()).is_ok()
}

/// Simple-facing listen for a pre-created and bound TCP socket handle.
#[no_mangle]
pub extern "C" fn rt_io_tcp_listen(handle: i64, backlog: i64) -> bool {
    let mut registry = SOCKET_REGISTRY.lock().unwrap();
    let Some(entry) = registry.remove(&handle) else {
        return false;
    };
    match entry {
        SocketEntry::TcpSocket(socket) => {
            let backlog_i32 = if backlog <= 0 { 128 } else { backlog.min(i32::MAX as i64) as i32 };
            match socket.listen(backlog_i32) {
                Ok(()) => {
                    let listener: TcpListener = socket.into();
                    registry.insert(handle, SocketEntry::TcpListener(listener));
                    true
                }
                Err(_) => false,
            }
        ...
        other => {
            registry.insert(handle, other);
            false
        }
    }
}

/// Simple-facing nonblocking setter for TCP sockets, listeners, and streams.
#[no_mangle]
pub extern "C" fn rt_io_tcp_set_nonblocking(handle: i64, enabled: bool) -> bool {
    let registry = SOCKET_REGISTRY.lock().unwrap();
    let Some(entry) = registry.get(&handle) else {
        return false;
    };
    match entry {
        SocketEntry::TcpSocket(socket) => socket.set_nonblocking(enabled).is_ok(),
        SocketEntry::TcpListener(listener) => listener.set_nonblocking(enabled).is_ok(),
        SocketEntry::TcpStream(stream) => stream.set_nonblocking(enabled).is_ok(),
        _ => false,
    }
}

/// Simple-facing SO_REUSEADDR setter for pre-bind TCP sockets.
#[no_mangle]
pub extern "C" fn rt_io_tcp_set_reuseaddr(handle: i64, enabled: bool) -> bool {
    let registry = SOCKET_REGISTRY.lock().unwrap();
    let Some(SocketEntry::TcpSocket(socket)) = registry.get(&handle) else {
        return false;
    };
    socket.set_reuse_address(enabled).is_ok()
}

/// Simple-facing SO_REUSEPORT setter for pre-bind TCP sockets.
#[no_mangle]
pub extern "C" fn rt_io_tcp_set_reuseport(handle: i64, enabled: bool) -> bool {
    #[cfg(unix)]
    {
