//! SFFI Wrapper Generation Macros.
//!
//! This module provides macros for easily wrapping Rust types for SFFI.
//!
//! # Usage
//!
//! ```rust,ignore
//! use simple_runtime::sffi_wrap_type;
//!
...
//! - SFFI functions for each method
...
// ============================================================================
// Macro for Generating SFFI Wrappers
// ============================================================================
...
#[macro_export]
macro_rules! sffi_method_entry {
    ($name:literal, $flags:expr, $func:expr) => {{
        $crate::value::sffi_object::FfiMethodEntry::new(
            $crate::value::sffi_object::fnv1a_hash($name.as_bytes()),
            $name.as_ptr(),
            $name.len() as u32,
            $flags,
            $func as *const (),
...
/// Declare an SFFI type with its methods.
///
/// This macro generates:
/// - A vtable for the type
/// - SFFI-compatible method wrappers
/// - Type registration function
///
/// # Example
///
    ...
    (
        ...
        paste::paste! {
            ...
            static mut [<$rust_type:upper _METHODS>]: [
                $crate::value::sffi_object::FfiMethodEntry;
                {
                ...
                $(
                    $crate::value::sffi_object::FfiMethodEntry::new(
                        $crate::value::sffi_object::fnv1a_hash($method_name.as_bytes()),
                        $method_name.as_ptr(),
                        $method_name.len() as u32,
                        $crate::value::sffi_object::method_flags::$flags,
                        $func as *const (),
                    ),
                )*
            ];
            ...
            static mut [<$rust_type:upper _VTABLE>]: $crate::value::sffi_object::FfiVtable =
                $crate::value::sffi_object::FfiVtable {
            ...
            /// Initialize the SFFI type (call once before use).
            fn [<init_ $rust_type:lower _sffi>]() {
                [<$rust_type:upper _INIT>].call_once(|| {
                    unsafe {
                        // Sort methods by hash
                        $crate::value::sffi_macros::sort_method_entries(
                            &mut [<$rust_type:upper _METHODS>]
                        );

                        // Initialize vtable
                        [<$rust_type:upper _VTABLE>] = $crate::value::sffi_object::FfiVtable::new(
            ...
            /// Register the SFFI type with the global registry.
            /// Returns the type ID (or 0 if already registered).
            pub fn [<register_ $rust_type:lower _type>]() -> u32 {
                [<init_ $rust_type:lower _sffi>]();
                unsafe {
                    $crate::value::sffi_registry::get_registry()
                        .register_type($type_name, &[<$rust_type:upper _VTABLE>])
                }
            }

            /// Get the vtable for this type.
            pub fn [<get_ $rust_type:lower _vtable>]() -> *const $crate::value::sffi_object::FfiVtable {
                [<init_ $rust_type:lower _sffi>]();
...
/// Result type for SFFI operations.
pub type FfiResult<T> = Result<T, FfiError>;

/// Error type for SFFI operations.
#[derive(Debug, Clone)]
pub enum FfiError {
