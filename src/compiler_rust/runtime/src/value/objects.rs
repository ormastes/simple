...
pub(crate) fn rt_option_some(payload: RuntimeValue) -> RuntimeValue {
    rt_enum_new(OPTION_ENUM_ID, hash_variant_discriminant("Some"), payload)
}
...
/// Get the stable runtime type identifier of an enum value.
#[no_mangle]
pub extern "C" fn rt_enum_id(value: RuntimeValue) -> i64 {
    get_typed_ptr::<RuntimeEnum>(value, HeapObjectType::Enum).map_or(-1, |p| unsafe { (*p).enum_id as i64 })
}
...
pub extern "C" fn rt_heap_ref_wellformed(value: RuntimeValue) -> i8 {
    // Native class references can be raw untagged pointers. Formation is the
    // masked address floor, not membership in one representation's tag set.
