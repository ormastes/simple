...
// ============================================================================
// Super-Instructions (fused opcodes for performance)
// ============================================================================

/// Fused: CONST_I64 + PUSH (load constant and push to eval stack).
///
/// **Encoding:** `CONST_I64_PUSH value:i64`
/// **Size:** 10 bytes
/// **Effect:** `eval_stack.push(value)`
pub const CONST_I64_PUSH: Opcode = 0x00B0;

/// Fused: LOAD + PUSH (load local and push to eval stack).
///
/// **Encoding:** `LOAD_PUSH local_idx:u16`
/// **Size:** 4 bytes
/// **Effect:** `eval_stack.push(locals[local_idx])`
pub const LOAD_PUSH: Opcode = 0x00B1;

/// Fused: POP + STORE (pop from eval stack and store to local).
///
/// **Encoding:** `POP_STORE local_idx:u16`
/// **Size:** 4 bytes
/// **Effect:** `locals[local_idx] = eval_stack.pop()`
pub const POP_STORE: Opcode = 0x00B2;

/// Fused: ADD_I64 + POP (add and store result to register).
///
/// **Encoding:** `ADD_I64_POP dest:u16`
/// **Size:** 4 bytes
/// **Effect:** `b = pop(); a = pop(); stack[dest] = a + b`
pub const ADD_I64_POP: Opcode = 0x00B3;

/// Fused: SUB_I64 + POP (subtract and store result to register).
///
/// **Encoding:** `SUB_I64_POP dest:u16`
/// **Size:** 4 bytes
/// **Effect:** `b = pop(); a = pop(); stack[dest] = a - b`
pub const SUB_I64_POP: Opcode = 0x00B4;

/// Fused: MUL_I64 + POP (multiply and store result to register).
///
/// **Encoding:** `MUL_I64_POP dest:u16`
/// **Size:** 4 bytes
/// **Effect:** `b = pop(); a = pop(); stack[dest] = a * b`
pub const MUL_I64_POP: Opcode = 0x00B5;

/// Fused: LT + JMP_IF_NOT (compare and branch if not less than).
///
/// **Encoding:** `LT_JMP_IF_NOT offset:i32`
/// **Size:** 6 bytes
/// **Effect:** `b = pop(); a = pop(); if !(a < b): ip += offset`
pub const LT_JMP_IF_NOT: Opcode = 0x00B6;

/// Fused: EQ + JMP_IF_NOT (compare and branch if not equal).
///
/// **Encoding:** `EQ_JMP_IF_NOT offset:i32`
/// **Size:** 6 bytes
/// **Effect:** `b = pop(); a = pop(); if !(a == b): ip += offset`
pub const EQ_JMP_IF_NOT: Opcode = 0x00B7;
...
pub fn opcode_name(opcode: Opcode) -> &'static str {
    match opcode {
        ...
        NOT => "NOT",
        // Super-Instructions
        CONST_I64_PUSH => "CONST_I64_PUSH",
        LOAD_PUSH => "LOAD_PUSH",
        POP_STORE => "POP_STORE",
        ADD_I64_POP => "ADD_I64_POP",
        SUB_I64_POP => "SUB_I64_POP",
        MUL_I64_POP => "MUL_I64_POP",
        LT_JMP_IF_NOT => "LT_JMP_IF_NOT",
        EQ_JMP_IF_NOT => "EQ_JMP_IF_NOT",
        _ => "UNKNOWN",
    }
}
