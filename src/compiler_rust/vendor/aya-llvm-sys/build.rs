...
fn link_windows_llvm_c(llvm_config_path: &Path) {
    assert!(
        cfg!(feature = "force-dynamic"),
        "Windows LLVM 23 must use force-dynamic with the LLVM-C import library"
    );

    let prefix = env::var_os(&*ENV_LLVM_PREFIX)
        .map(PathBuf::from)
        .expect("LLVM_SYS_231_PREFIX is required for the Windows LLVM 23 build");
    let expected_config = prefix.join("bin").join("llvm-config.exe");
    let actual_config = llvm_config_path
        .canonicalize()
        .expect("failed to canonicalize llvm-config");
    let expected_config = expected_config
        .canonicalize()
        .expect("LLVM_SYS_231_PREFIX/bin/llvm-config.exe is missing");
    assert_eq!(
        actual_config, expected_config,
        "llvm-config escaped LLVM_SYS_231_PREFIX"
    );

    let import_lib = prefix.join("lib").join("LLVM-C.lib");
    let runtime_dll = prefix.join("bin").join("LLVM-C.dll");
    assert!(
        import_lib.is_file(),
        "missing LLVM 23 C API import library: {}",
        import_lib.display()
    );
    assert!(
        runtime_dll.is_file(),
        "missing LLVM 23 C API runtime: {}",
        runtime_dll.display()
    );

    println!("cargo:rustc-link-search=native={}", prefix.join("lib").display());
    println!("cargo:rustc-link-lib=dylib=LLVM-C");
    println!("cargo:link_kind=dynamic-c-api");
    println!("cargo:runtime_dll={}", runtime_dll.display());
}
...
fn main() {
    ...
    require_pinned_llvm_version(&llvm_config_path);
    ...
    // The official Windows LLVM 23.1.1 archive ships the complete C API as
    // LLVM-C.dll plus LLVM-C.lib. Its llvm-config static list contains
    // development-only archives that are absent from that archive, while its
    // shared query names an LLVM-23.dll that is also absent. Bind the shipped
    // C API import library directly and do not consume either stale list.
    if target_os_is("windows") {
        link_windows_llvm_c(&llvm_config_path);
        return;
    }
