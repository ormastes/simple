//! Minimal `fs` implementation — enough for `File::open` / `read` / `write`
//! and `metadata` size/kind. Built on the POSIX-ish syscalls libsimpleos_c
//! re-exports. Directory iteration, rename, unlink, and symlink ops all
...
impl File {
    pub fn open(p: &Path, opts: &OpenOptions) -> io::Result<File> {
        ...
        let fd = unsafe { open(c.as_ptr(), opts.as_flags(), opts.mode) };
...
pub use unsupported_ops::*;

impl FromInner<i32> for File {
    fn from_inner(fd: i32) -> File { File(fd) }
}
