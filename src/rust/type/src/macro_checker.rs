// Macro type checking and const evaluation
//
// This module handles:
// - Macro definition order validation
// - Macro intro application (compile-time code generation)
// - Const expression evaluation for macros

/// Compile-time constant value for macro evaluation
...
impl TypeChecker {
    /// Validates that macro invocations respect block position constraints.
    ...
    /// Extracts macro anchors from a macro invocation statement
    pub(crate) fn macro_invocation_anchors(
        &self,
        node: &Node,
    ...
    /// Apply all macro intro declarations from a macro definition
    pub(crate) fn apply_macro_intros(
        &mut self,
        macro_def: &MacroDef,
    ...
    /// Apply a macro intro spec recursively (handles Decl, For, If)
    pub(crate) fn apply_macro_intro_spec(
        &mut self,
        spec: &MacroIntroSpec,
        const_bindings: &std::collections::HashMap<String, String>,
    ) -> Result<(), TypeError> {
    ...
    /// Apply a single macro intro declaration (Fn, Field, Var, Type)
    pub(crate) fn apply_macro_intro_decl(
        &mut self,
        decl: &MacroIntroDecl,
        const_bindings: &std::collections::HashMap<String, String>,
    ) -> Result<(), TypeError> {
    ...
    /// Apply a for-loop macro intro, iterating over a const range
    pub(crate) fn apply_macro_intro_for(
        &mut self,
        spec: &MacroIntroSpec,
        const_bindings: &std::collections::HashMap<String, String>,
    ) -> Result<(), TypeError> {
    ...
    /// Evaluate a const range expression (start..end or start..=end)
    pub(crate) fn eval_const_range(
        &self,
    ...
    /// Evaluate a const expression as a boolean
    pub(crate) fn eval_const_bool(
        &self,
        expr: &Expr,
        const_bindings: &std::collections::HashMap<String, String>,
    ) -> Option<bool> {
    ...
    /// Evaluate a const expression as an integer
    ...
    /// Evaluate a comparison operation on const values
    pub(crate) fn eval_const_compare(
        &self,
        left: ConstValue,
        right: ConstValue,
    ...
    /// Parse a const value from a string binding
    pub(crate) fn const_value_from_binding(&self, text: &str) -> Option<ConstValue> {
        if text == "true" {
    ...
    /// Get the return type from a macro's contract
    pub(crate) fn macro_return_type(&mut self, macro_def: &MacroDef) -> Type {
        for item in &macro_def.contract {
    ...
    /// Build const bindings from macro invocation arguments
    ...
    /// Convert a simple expression to its string representation
    pub(crate) fn const_expr_to_string(&self, expr: &Expr) -> Option<String> {
        match expr {
...
/// Substitute {key} placeholders in a template string with values from bindings
pub(crate) fn substitute_macro_template(
    input: &str,
    const_bindings: &std::collections::HashMap<String, String>,
) -> String {
