...
fn main() {
    ...
    // CARGO_MANIFEST_DIR = .../simple/src/rust/driver
    ...
    // Generate tests for test/ (system tests)
    let simple_test_root = project_root.join("test");
...
fn generate_test_file(test_root: &Path, dest_path: &Path, prefix: &str) {
    ...
    if !test_root.exists() {
        ...
        let is_test_file = |path: &Path| -> bool {
            ...
            // Skip integration/spec/ directory (tests depend on spec stdlib which has lint errors)
            if path.to_string_lossy().contains("/integration/spec/")
                || path.to_string_lossy().contains("\\integration\\spec\\")
            {
                return false;
            }
