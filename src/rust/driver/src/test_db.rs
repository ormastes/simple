...
mod tests {
    ...
    fn test_validate_status_inconsistent_running_with_end_time() {
        ...
        run.end_time = chrono::Utc::now().to_rfc3339();  // Should not have end_time
    ...
    fn test_validate_invalid_timestamp_format() {
        ...
        let report = run.validate_record();
        assert!(report.has_violations());
        let has_invalid_value = report.violations.iter()
    ...
    fn test_validate_multiple_violations() {
        use crate::unified_db::{ViolationType, Severity};
        ...
        run.test_count = 10;
        run.passed = 8;
        run.failed = 5;  // Count overflow: 8 + 5 = 13 > 10

        let report = run.validate_record();
        assert!(report.has_violations());
        assert!(report.violations.len() >= 2);  // At least stale + count inconsistent

        let has_stale = report.violations.iter()
            .any(|v| v.violation_type == ViolationType::StaleRunning);
        let has_count = report.violations.iter()
    ...
    fn test_validate_auto_fixable_marking() {
        ...
        let report = run.validate_record();
        assert!(report.auto_fixable);  // Stale runs are auto-fixable

        // Timestamp errors are NOT auto-fixable
    ...
    fn test_validate_partial_counts_accepted() {
        ...
        run.crashed = 2;
        run.timed_out = 1;
        // Total: 10 + 3 + 2 + 1 = 16 < 20 (4 skipped)

        let report = run.validate_record();
        let has_count_error = report.violations.iter()
            .any(|v| v.violation_type == crate::unified_db::ViolationType::CountInconsistent);
        assert!(!has_count_error);  // Should be valid
    }
    ...
    fn test_validate_exact_count_match() {
        ...
        run.crashed = 2;
        run.timed_out = 0;
        // Total: 10 + 3 + 2 = 15 = 15 (exact match)
