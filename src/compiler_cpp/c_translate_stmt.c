...
const char* translate_interpolated_print(const char* inner, const char* types) {
    const char* fmt = "";
    SimpleIntArray interp_args = simple_new_int_array();
    long long i = 0;
    ...
    const char* args_str = "";
    for (long long _idx_current_arg = 0; _idx_current_arg < interp_args_len; _idx_current_arg++) { long long current_arg = interp_args[_idx_current_arg];
        args_str = simple_str_concat(args_str, simple_str_concat(", ", current_arg));
    }
    return simple_str_concat(r"printf(", simple_str_concat("\"", simple_str_concat(fmt, simple_str_concat(r"\n", simple_str_concat("\"", simple_str_concat(args_str, r");"))))));
}
...
const char* translate_print(const char* arg, const char* types) {
    ...
    //  Check for string concatenation: "str" + expr + "str"
        ...
        if (has_concat) {
            ...
            return simple_str_concat(r"printf(", simple_str_concat("\"", simple_str_concat(r"%s\n", simple_str_concat("\",", c_arg + r");"))));
    ...
    if (is_text_var(arg, types)) {
        return simple_str_concat(r"printf(", simple_str_concat("\"", simple_str_concat(r"%s\n", simple_str_concat("\",", simple_str_concat(arg, r");")))));
    ...
    return simple_str_concat(r"printf(", simple_str_concat("\"", simple_str_concat(r"%lld\n", simple_str_concat("\",(long long)", c_arg + r");"))));
}
...
const char* translate_for_loop(const char* trimmed, const char* types) {
    ...
    if (in_idx < 0) {
        return r"/* unsupported for: " + trimmed + r" */";
    }
    ...
    if (simple_starts_with(iterable, "range(") && simple_ends_with(iterable, ")")) {
        ...
        if (comma_idx >= 0) {
            ...
            return r"for (long long " + loop_var + r" = " + translate_expr(start_expr, types) + r"; " + loop_var + r" < " + translate_expr(end_expr, types) + r"; " + loop_var + r"++) " + "\{";
        } else {
            return r"for (long long " + loop_var + r" = 0; " + loop_var + r" < " + translate_expr(range_args, types) + r"; " + loop_var + r"++) " + "\{";
    ...
    if (simple_starts_with(iterable, "[") && simple_ends_with(iterable, "]")) {
        ...
        const char* init = simple_str_concat(r"SimpleIntArray _arr_", simple_str_concat(loop_var, r" = simple_new_int_array();"));
        for (long long _idx_elem = 0; _idx_elem < elements_len; _idx_elem++) { long long elem = elements[_idx_elem];
            ...
            init = simple_str_concat(init, simple_str_concat(r" simple_int_push(&_arr_", simple_str_concat(loop_var, r", " + e + r");")));
        }
    ...
    if (simple_starts_with(iterable, "[") && simple_ends_with(iterable, "]")) {
        ...
        for (long long _idx_ia_elem = 0; _idx_ia_elem < ia_elements_len; _idx_ia_elem++) { long long ia_elem = ia_elements[_idx_ia_elem];
            const char* ia_e = translate_expr(simple_trim(ia_elem), types);
            ia_init = simple_str_concat(ia_init, simple_str_concat(r" simple_int_push(&_arr_", simple_str_concat(loop_var, r", " + ia_e + r");")));
        }
...
const char* translate_case(const char* trimmed, const char* types) {
    ...
    if (simple_starts_with(case_body, "\"")) {
        return r"if (strcmp(_match_val, " + case_body + r") == 0) " + "\{";
    ...
    //  Generate tag check + field extraction
            ...
            long long fi = 0;
            for (long long _idx_field = 0; _idx_field < fields_len; _idx_field++) { long long field = fields[_idx_field];
    ...
    //  Check if named or positional
                ...
                if (fc >= 0) {
                    ...
                    result = simple_str_concat(result, r" const char* " + fvar + r" = _match_val.data." + variant_name + r"." + fvar + r";");
                } else {
                    result = simple_str_concat(result, r" const char* " + fname + r" = _match_val.data." + variant_name + r"._" + fi + r";");
                }
                fi = fi + 1;
            }
            return result;
    ...
    if (strcmp(resolved, "") != 0) {
        return r"if (_match_val == (long long)" + resolved + r") " + "\{";
    }
    return r"if (_match_val == (long long)" + case_body + r") " + "\{";
}
...
const char* translate_statement(const char* trimmed, const char* types) {
    ...
    //  Reuse print interpolation logic but redirect to stderr
            ...
            return simple_str_concat(r"fprintf(stderr, ", simple_str_concat("\"", simple_str_concat(inner, simple_str_concat(r"\n", "\");"))));
        }
        return simple_str_concat(r"fprintf(stderr, ", simple_str_concat("\"", simple_str_concat(r"%s\n", simple_str_concat("\",", translate_expr(rest, types) + r");"))));
        ...
        if (eq_idx >= 0) {
            ...
            if (simple_starts_with(lhs, "(") && simple_ends_with(lhs, ")")) {
                ...
                if (tparts.len == 2) {
                    ...
                    return r"SimpleTuple2 _tmp_tuple = " + c_rhs + r"; long long " + v0 + r" = (long long)_tmp_tuple._0; long long " + v1 + r" = (long long)_tmp_tuple._1;";
                }
                if (tparts.len == 3) {
                    const char* v0 = simple_trim(tparts[0]);
                    const char* v1 = simple_trim(tparts[1]);
                    const char* v2 = simple_trim(tparts[2]);
                    return r"SimpleTuple3 _tmp_tuple = " + c_rhs + r"; long long " + v0 + r" = (long long)_tmp_tuple._0; long long " + v1 + r" = (long long)_tmp_tuple._1; long long " + v2 + r" = (long long)_tmp_tuple._2;";
    ...
    //  Use translate_condition for expressions with and/or/comparisons
        ...
        if (has_and || has_or) {
            return r"return " + translate_condition(expr, types) + r";";
        }
        return r"return " + translate_expr(expr, types) + r";";
    }
    ...
    if (simple_starts_with(trimmed, "if ") && simple_ends_with(trimmed, ":")) {
        ...
        return r"if (" + translate_condition(cond, types) + r") " + "\{";
    ...
    if (simple_starts_with(trimmed, "elif ") && simple_ends_with(trimmed, ":")) {
        ...
        return r"} else if (" + translate_condition(cond, types) + r") " + "\{";
    }
    ...
    //  Build the if-check for the pattern
            long long case_check_trimmed = r"case " + case_pattern + r":";
    ...
    if (push_pos >= 0) {
        ...
        if (arg_end >= 0) {
            ...
            if (is_string_array_var(obj, types)) {
                return simple_str_concat(r"simple_string_push(&", simple_str_concat(obj, r", " + translate_expr(arg, types) + r");"));
            }
            if (is_int_array_var(obj, types)) {
                return simple_str_concat(r"simple_int_push(&", simple_str_concat(obj, r", " + translate_expr(arg, types) + r");"));
            }
            if (is_str_arr_arr_var(obj, types)) {
                ...
                return simple_str_concat(r"simple_string_array_push(&", simple_str_concat(obj, r", " + push_arg + r");"));
            }
            if (is_int_arr_arr_var(obj, types)) {
                ...
                return simple_str_concat(r"simple_int_array_push(&", simple_str_concat(obj, r", " + push_arg + r");"));
    ...
    if (simple_ends_with(trimmed, ".pop()")) {
        ...
        if (is_string_array_var(obj, types)) {
            return simple_str_concat(r"simple_string_pop(&", simple_str_concat(obj, r");"));
        }
        if (is_int_array_var(obj, types)) {
            return simple_str_concat(r"simple_int_pop(&", simple_str_concat(obj, r");"));
    //  Compound assignment
        }
    }
    ...
    //  String concatenation assignment
        if (is_text_var(lhs, types)) {
            return simple_str_concat(lhs, simple_str_concat(r" = simple_str_concat(", simple_str_concat(lhs, r", " + translate_expr(rhs, types) + r");")));
        }
        return simple_str_concat(lhs, r" += " + translate_expr(rhs, types) + r";");
    }
    ...
    //  Detect value type
                    ...
                    if (rhs_is_str || rhs_text) {
                        return simple_str_concat(r"simple_dict_set_str(", simple_str_concat(dict_name, r", " + c_key + r", " + c_rhs + r");"));
                    }
                    return simple_str_concat(r"simple_dict_set_int(", simple_str_concat(dict_name, r", " + c_key + r", " + c_rhs + r");"));
        ...
        if (lhs_bracket >= 0) {
            ...
            if (arr_is_any) {
                ...
                if (lhs_close > lhs_bracket + 1) {
                    ...
                    long long c_rhs = translate_expr(rhs, types);
                    return simple_str_concat(arr_name, r".items[" + c_idx + r"] = " + c_rhs + r";");
    //  Struct field assignment: obj.field = value
        ...
        if (lhs_dot >= 0) {
            ...
            if (strcmp(lhs_class, "") != 0) {
                long long c_rhs = translate_expr(rhs, types);
                return simple_str_concat(lhs_obj, r"." + lhs_field + r" = " + c_rhs + r";");
            }
        }
    ...
    //  Handle array reset: arr = []
            if (strcmp(rhs, "[]") == 0) {
                if (is_str_arr_arr_var(lhs, types)) {
                    return simple_str_concat(lhs, r" = simple_new_string_array_array();");
                }
                if (is_int_arr_arr_var(lhs, types)) {
                    return simple_str_concat(lhs, r" = simple_new_int_array_array();");
                }
                if (is_string_array_var(lhs, types)) {
                    return simple_str_concat(lhs, r" = simple_new_string_array();");
                }
                if (is_int_array_var(lhs, types)) {
                    return simple_str_concat(lhs, r" = simple_new_int_array();");
                }
                if (strcmp(is_struct_array_var(lhs, types), "") != 0) {
                    return simple_str_concat(lhs, r" = simple_new_struct_array();");
                }
                return simple_str_concat(lhs, r" = simple_new_int_array();");
    //  Handle array concat: arr = arr + [elem] -> direct push (optimization)
            ...
            if (rhs_concat_pos >= 0 && simple_ends_with(rhs, "]")) {
                ...
                if (strcmp(arr_part, lhs) == 0) {
                    ...
                    if (is_int_array_var(lhs, types)) {
                        return simple_str_concat(r"simple_int_push(&", simple_str_concat(lhs, r", " + translate_expr(concat_elem, types) + r");"));
                    }
                    if (is_string_array_var(lhs, types)) {
                        return simple_str_concat(r"simple_string_push(&", simple_str_concat(lhs, r", " + translate_expr(concat_elem, types) + r");"));
                    }
                }
            }
            long long c_rhs = translate_expr(rhs, types);
    //  Track if assigning a string to a var (e.g., name = "Alice")
            ...
            if (rhs_is_str || rhs_is_text) {
                return simple_str_concat(lhs, r" = " + c_rhs + r";
            }
            return simple_str_concat(lhs, r" = " + c_rhs + r";");
    //  While loop
        }
    ...
    if (simple_starts_with(trimmed, "while ") && simple_ends_with(trimmed, ":")) {
        ...
        return r"while (" + translate_condition(cond, types) + r") " + "\{";
    ...
    //  If the part before ( is a simple name (no spaces = no operators), it's a function call
        ...
        if (!(fn_has_space && simple_strlen(fn_part) > 0)) {
            return translate_expr(trimmed, types) + r";";
    ...
    if (has_cmp || has_logic) {
        return r"return " + translate_condition(trimmed, types) + r";";
    }
    ...
    return r"/* unsupported: " + trimmed + r" */";
}
