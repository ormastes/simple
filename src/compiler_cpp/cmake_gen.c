...
// --- Global Variables ---
static long long CMAKE_HEADER = r"# Auto-generated by Simple compiler -- DO NOT EDIT";
...
long long generate_cmake_for_modules(const char* output_dir, long long module_files, long long text) {
    ...
    SimpleIntArray all_dirs = simple_new_int_array();
    SimpleIntArray root_sources = simple_new_int_array();
    SimpleIntArray object_libs = simple_new_int_array();
    ...
    //  Top-level dir -> register under root
            ...
            long long top_dir = dir_parts[0];
            if ("" not in dir_subdirs) {
                dir_subdirs[""] = simple_new_int_array();
            }
    ...
    for (long long _idx_dir_path = 0; _idx_dir_path < all_dirs_len; _idx_dir_path++) { long long dir_path = all_dirs[_idx_dir_path];
        long long files = dir_files[dir_path];
    ...
    //  Collect child subdirs for this directory (if any)
        SimpleIntArray child_subdirs = simple_new_int_array();
        ...
        if (!(rt_file_write_text(cmake_path, cmake_content))) {
            return simple_str_concat("Failed to write ", cmake_path);
    ...
    for (long long _idx_dir_path = 0; _idx_dir_path < dir_subdirs.keys()_len; _idx_dir_path++) { long long dir_path = dir_subdirs.keys()[_idx_dir_path];
        if (strcmp(dir_path, "") == 0) {
            return pass_dn;
        } else if (dir_path not in dir_files) {
            long long subdirs = dir_subdirs[dir_path];
    ...
    SimpleIntArray top_dirs = simple_new_int_array();
    if ("" in dir_subdirs) {
    ...
    if (!(rt_file_write_text(root_cmake_path, root_cmake))) {
        return simple_str_concat("Failed to write ", root_cmake_path);
    }
    return NULL;
}

const char* generate_leaf_cmake(const char* lib_name, SimpleStringArray files, SimpleStringArray child_subdirs) {
    long long runtime_dir = r"${RUNTIME_DIR}";
    const char* content = simple_str_concat(CMAKE_HEADER, "\n");
    content = simple_str_concat(content, simple_str_concat(simple_str_concat("add_library(", lib_name), " OBJECT\n"));
    for (long long _idx_file = 0; _idx_file < files.len; _idx_file++) { const char* file = files.items[_idx_file];
        content = simple_str_concat(content, simple_str_concat(simple_str_concat("    ", file), "\n"));
    }
    content = simple_str_concat(content, ")\n\n");
    content = simple_str_concat(content, simple_str_concat(simple_str_concat("target_include_directories(", lib_name), " PRIVATE\n"));
    content = simple_str_concat(content, simple_str_concat(simple_str_concat("    ", simple_int_to_str(runtime_dir)), "\n"));
    content = simple_str_concat(content, simple_str_concat(simple_str_concat("    ", simple_int_to_str(runtime_dir)), "/platform\n"));
    long long gen_root = r"${CMAKE_SOURCE_DIR}";
    content = simple_str_concat(content, simple_str_concat(simple_str_concat("    ", simple_int_to_str(gen_root)), "\n"));
    content = simple_str_concat(content, ")\n");
    ...
    if (child_subdirs.len > 0) {
        ...
        for (long long _idx_subdir = 0; _idx_subdir < child_subdirs.len; _idx_subdir++) { const char* subdir = child_subdirs.items[_idx_subdir];
            content = simple_str_concat(content, simple_str_concat(simple_str_concat("add_subdirectory(", subdir), ")\n"));
        }
    }
    return content;
}
...
const char* generate_root_cmake(SimpleStringArray top_dirs, SimpleStringArray root_sources, SimpleStringArray object_libs) {
    long long cmake_list_dir = r"${CMAKE_CURRENT_LIST_DIR}";
    long long cmake_bin_dir = r"${CMAKE_CURRENT_BINARY_DIR}";
    const char* content = simple_str_concat(CMAKE_HEADER, "\n");
    ...
    for (long long _idx_dir = 0; _idx_dir < top_dirs.len; _idx_dir++) { const char* dir = top_dirs.items[_idx_dir];
        content = simple_str_concat(content, simple_str_concat(simple_str_concat(simple_str_concat(simple_str_concat(simple_str_concat(simple_str_concat(simple_str_concat(simple_str_concat("add_subdirectory(", simple_int_to_str(cmake_list_dir)), "/"), dir), " "), simple_int_to_str(cmake_bin_dir)), "/generated_"), dir), ")\n"));
    }
    content = simple_str_concat(content, "\n");
    //  Root source files
    ...
    for (long long _idx_src = 0; _idx_src < root_sources.len; _idx_src++) { const char* src = root_sources.items[_idx_src];
        content = simple_str_concat(content, simple_str_concat(simple_str_concat(simple_str_concat(simple_str_concat("    ", simple_int_to_str(cmake_list_dir)), "/"), src), "\n"));
    }
    content = simple_str_concat(content, ")\n\n");
    //  Object library list
    ...
    for (long long _idx_lib = 0; _idx_lib < object_libs.len; _idx_lib++) { const char* lib = object_libs.items[_idx_lib];
        content = simple_str_concat(content, simple_str_concat(simple_str_concat("    ", lib), "\n"));
    }
    content = simple_str_concat(content, ")\n");
    return content;
}
