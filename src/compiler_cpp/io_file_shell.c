...
long long rt_process_run(const char* cmd, SimpleStringArray args);
long long shell(const char* command);
const char* shell_output(const char* command);
int file_write(const char* path, const char* content);
void file_delete(const char* path);
long long file_size(const char* path);

long long shell(const char* command) {
    return rt_process_run("sh", ["-c", command]);
}

const char* shell_output(const char* command) {
    SimpleTuple3 _tmp_tuple = rt_process_run("sh", ["-c", command]); long long out = (long long)_tmp_tuple._0; long long err = (long long)_tmp_tuple._1; long long code = (long long)_tmp_tuple._2;
    return simple_trim(out);
}
...
long long file_size(const char* path) {
    ...
    const char* trimmed = simple_trim(size_str);
    if (strcmp(trimmed, "") == 0) {
        return 0;
    }
    return atoll(trimmed);
}
