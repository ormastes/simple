        ...
        for node in nodes {
            match node {
                ...
                Node::Class(class) => {
                    ...
                    for method in &class.methods {
                        children.push(SymbolInfo::new(&method.name, SymbolKind::Method, span_to_range(&method.span)).with_container(&class.name));
                    }
                    for field in &class.fields {
                        children.push(SymbolInfo::new(&field.name, SymbolKind::Field, span_to_range(&field.span)).with_container(&class.name));
                    }
                ...
                Node::Struct(s) => {
                    ...
                    for field in &s.fields {
                        fields.push(SymbolInfo::new(&field.name, SymbolKind::Field, span_to_range(&field.span)).with_container(&s.name));
                    }
                ...
                Node::Enum(e) => {
                    ...
                    for variant in &e.variants {
                        variants.push(SymbolInfo::new(&variant.name, SymbolKind::EnumMember, span_to_range(&variant.span)).with_container(&e.name));
                    }
                ...
                Node::Trait(t) => {
                    ...
                    for method in &t.methods {
                        methods.push(SymbolInfo::new(&method.name, SymbolKind::Method, span_to_range(&method.span)).with_container(&t.name));
                    }
            ...
            match node {
                Node::Function(func) => {
                    if func.params.len() > 10 {
                        diagnostics.push(Diagnostic::warning(span_to_range(&func.span), format!("Function '{}' has many parameters", func.name)).with_code("W001"));
                    }
                }
                Node::Class(class) => {
                    if class.methods.is_empty() && class.fields.is_empty() {
                        diagnostics.push(Diagnostic::warning(span_to_range(&class.span), format!("Class '{}' has no fields or methods", class.name)).with_code("W002"));
                    }
                }
                _ => {}
            }
...
fn path_to_uri(path: &str) -> String {
    if path.starts_with("file://") {
        ...
        let abs_path = Path::new(path).canonicalize().map(|p| p.to_string_lossy().to_string()).unwrap_or_else(|_| path.to_string());
...
fn format_function_signature(func: &FunctionDef) -> String {
    ...
    let async_kw = if func.effects.iter().any(|e| matches!(e, simple_parser::ast::Effect::Async)) { "async " } else { "" };
...
fn type_to_string(ty: &Type) -> String {
    match ty {
        ...
        Type::Generic { name, args } => {
            if args.is_empty() { name.clone() } else { format!("{}<{}>", name, args.iter().map(type_to_string).collect::<Vec<_>>().join(", ")) }
        }
