...
impl SpecCoverageReport {
    ...
    pub fn load(path: &Path) -> Result<Self, String> {
        ...
        let data: SpecCoverageFile = serde_yaml::from_str(&content)
    ...
    pub fn display_by_category(&self) {
        ...
        for (category_id, coverage) in &self.data.category_coverage {
            ...
            let category_name = self.data.categories
                .iter()
    ...
    pub fn generate_html(&self) -> String {
        ...
        html.push_str(&format!("      <div class=\"progress-bar\" style=\"width: {:.1}%\"></div>\n", 
            self.data.summary.overall_coverage));
        html.push_str("    </div>\n");
        html.push_str(&format!("    <p>Total Features: {} | Implemented: {} | Partial: {} | Planned: {}</p>\n",
        ...
        for category in &self.data.categories {
            ...
            html.push_str(&format!("    <div class=\"category-header\"><h2>{}</h2></div>\n", category.name));
            html.push_str("    <table>\n");
