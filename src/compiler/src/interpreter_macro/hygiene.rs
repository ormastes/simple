//! Macro hygiene transformations
//!
//! Implements hygienic macro expansion by renaming identifiers to prevent
//! variable capture. Uses gensym counters to generate unique names for
//! variables introduced in macro expansions.
...
impl MacroHygieneContext {
    ...
    pub(super) fn resolve(&self, name: &str) -> Option<String> {
        self.scopes.iter().rev().find_map(|scope| scope.get(name).cloned())
    }

    pub(super) fn bind(&mut self, name: &str, reuse_if_bound: bool) -> String {
        if reuse_if_bound {
