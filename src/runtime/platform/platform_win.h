...
#include "windows_command_line_private.h"
...
static bool win_is_path_sep(char ch) {
    return ch == '\\' || ch == '/';
}
...
bool rt_dir_create(const char* path, bool recursive) {
    ...
    char* scratch = (char*)malloc(path_len + 1);
    if (!scratch) return false;
    strcpy(scratch, path);

    size_t start = 0;
    if (path_len >= 3 && scratch[1] == ':' && win_is_path_sep(scratch[2])) {
        /* Drive-rooted path: skip "C:\" prefix. */
        start = 3;
    } else if (path_len >= 2 && win_is_path_sep(scratch[0]) && win_is_path_sep(scratch[1])) {
        /* UNC path: skip "\\server\share\" prefix before creating subdirectories. */
        size_t slash_seen = 0;
        start = 2;
        while (start < path_len) {
            if (win_is_path_sep(scratch[start])) {
                slash_seen++;
                if (slash_seen == 2) {
                    start++;
                    break;
    ...
    } else if (win_is_path_sep(scratch[0])) {
        start = 1;
    }

    bool ok = true;
    for (size_t i = start; i <= path_len; i++) {
        if (!win_is_path_sep(scratch[i]) && scratch[i] != '\0') continue;
...
char* rt_windows_build_command_line(const char* cmd, const char** args, int64_t arg_count) {
    return win_cmd_build_line(cmd, args, arg_count);
}
