 ...
 * own -- only the prefix lifts the ceiling, to ~32767). Caller frees.
 * Self-contained rather than sharing another function's widen helper -- see
 * the call site below for why. Path separator and prefix are built from the
 * numeric code point (92) to keep this free of escape sequences. */
static wchar_t* rt_file_publish_widen_long_path(const char* path) {
...
int64_t rt_file_publish_noreplace(const uint8_t* staged_ptr, uint64_t staged_len, const uint8_t* destination_ptr, uint64_t destination_len) {
     ...
     * endpoints can exceed MAX_PATH; a self-contained widen helper is used
     * here (rather than a shared one) because this file's only prior
     * instance of the same technique, on the bounded reader above, has
     * regressed back to its pre-fix ANSI-only form upstream -- a separate,
     * unrelated defect this change does not attempt to repair. */
    {
        wchar_t* wide_staged = rt_file_publish_widen_long_path(staged);
        wchar_t* wide_dest = wide_staged ? rt_file_publish_widen_long_path(destination) : NULL;
