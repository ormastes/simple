...
static char* rt_file_text_arg_to_cstr(const uint8_t* ptr, uint64_t len) {
    if ((!ptr && len != 0) || len > (uint64_t)(SIZE_MAX - 1) ||
        (len != 0 && memchr(ptr, '\0', (size_t)len) != NULL)) return NULL;
...
int         rt_file_rename(const uint8_t* src_ptr, uint64_t src_len,
...
static atomic_uint_fast64_t rt_file_move_sequence = 0;

static int rt_file_move_cross_device(const char* src, const char* dst) {
    FILE* in = fopen(src, "rb");
    if (!in) return 0;
    ...
    for (int attempt = 0; attempt < 64 && fd < 0; attempt++) {
        unsigned long long sequence = (unsigned long long)atomic_fetch_add_explicit(
            &rt_file_move_sequence, 1, memory_order_relaxed);
...
int         rt_file_move(const uint8_t* src_ptr, uint64_t src_len,
    ...
    int ok = rename(src, dst) == 0;
    if (!ok && errno == EXDEV) ok = rt_file_move_cross_device(src, dst);
    free(src);
    free(dst);
    return ok ? 1 : 0;
}
