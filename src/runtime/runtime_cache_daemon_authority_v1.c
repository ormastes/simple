...
struct receipt { int64_t token, owner_token; enum receipt_kind kind; int root_fd, aux_fd; pid_t pid; uid_t uid; dev_t dev; ino_t ino; char boot[37]; struct receipt *next; };
...
int64_t rt_cache_host_acquire_exclusive_lock_v1(int64_t root,int64_t peer){int rfd=rt_cache_host_duplicate_root_fd_internal_v1(root);if(rfd<0)return-1;struct stat s;if(fstat(rfd,&s)){close(rfd);return-1;}pthread_mutex_lock(&receipt_mu);struct receipt*p=find_receipt(peer);int ok=p&&p->kind==R_PEER&&p->dev==s.st_dev&&p->ino==s.st_ino;pthread_mutex_unlock(&receipt_mu);if(!ok){close(rfd);return-1;}int lfd=openat(rfd,".simple-cache-writer.lock",O_RDWR|O_CREAT|O_NOFOLLOW|O_CLOEXEC,0600);if(lfd<0||flock(lfd,LOCK_EX|LOCK_NB)){if(lfd>=0)close(lfd);close(rfd);return-1;}struct receipt*r=calloc(1,sizeof*r);if(!r){close(lfd);close(rfd);return-1;}r->kind=R_LOCK;r->root_fd=rfd;r->aux_fd=lfd;r->pid=getpid();r->uid=geteuid();r->dev=s.st_dev;r->ino=s.st_ino;return add_receipt(r);}
int64_t rt_cache_host_boot_identity_v1(int64_t lock){char boot[37];if(!read_boot(boot))return-1;pthread_mutex_lock(&receipt_mu);struct receipt*l=find_receipt(lock);int ok=l&&l->kind==R_LOCK;pthread_mutex_unlock(&receipt_mu);if(!ok)return-1;struct receipt*r=calloc(1,sizeof*r);if(!r)return-1;r->kind=R_BOOT;r->owner_token=lock;memcpy(r->boot,boot,37);return add_receipt(r);}
int64_t rt_cache_host_advance_writer_epoch_v1(int64_t lock,int64_t boot){char current[37];if(!read_boot(current))return-1;pthread_mutex_lock(&receipt_mu);struct receipt*l=find_receipt(lock),*b=find_receipt(boot);int rfd=l&&l->kind==R_LOCK&&b&&b->kind==R_BOOT&&b->owner_token==lock&&!memcmp(b->boot,current,37)?fcntl(l->root_fd,F_DUPFD_CLOEXEC,3):-1;pthread_mutex_unlock(&receipt_mu);if(rfd<0)return-1;int fd=openat(rfd,".simple-cache-writer.epoch",O_RDWR|O_CREAT|O_NOFOLLOW|O_CLOEXEC,0600);struct stat s;struct epoch_record rec={{0}};uint64_t prev=0;if(fd<0||fstat(fd,&s)||!S_ISREG(s.st_mode)||s.st_nlink!=1){if(fd>=0)close(fd);close(rfd);return-1;}if(s.st_size==(off_t)sizeof rec){if(pread(fd,&rec,sizeof rec,0)!=(ssize_t)sizeof rec||memcmp(rec.magic,"SCEPOC1",7)){close(fd);close(rfd);return-1;}prev=rec.epoch;}else if(s.st_size!=0){close(fd);close(rfd);return-1;}if(prev==INT64_MAX){close(fd);close(rfd);return-1;}memset(&rec,0,sizeof rec);memcpy(rec.magic,"SCEPOC1",7);rec.epoch=prev+1;memcpy(rec.boot,current,37);int ok=write_all_at(fd,&rec,sizeof rec)&&fsync(rfd)==0;close(fd);close(rfd);return ok?(int64_t)rec.epoch:-1;}
