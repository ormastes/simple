...
void worker_loop_entry(int64_t pool_id) {
    (void)pool_id;
    assert(!"this probe does not start pool workers");
}
...
static void wait_for(atomic_int *value, int expected) {
    ...
    assert(!"thread lifetime probe timed out");
}
...
int main(void) {
    ...
    atomic_store(&proceed, 0);
    int64_t closure[] = {(int64_t)(intptr_t)blocked_worker, 0};
    assert(rt_thread_spawn_isolated((int64_t)(intptr_t)closure, 0) == 0);
    wait_for(&entered, 1);
    assert(atomic_load(&records_live) == 1);
    atomic_store(&proceed, 1);
    wait_for(&records_live, 0);
