...
//
// Older runtime builds had no host-linkable definition for the six pre-bind
// socket functions below. They existed only in runtime_native.c, which is
// compiled into the AOT/native-project lane (an independent ABI universe) and
// is NOT linked into the host runtime. Any SMF that linked the http_server path
// therefore failed at load: "Undefined symbol: rt_io_tcp_socket_create".
//
// The Rust runtime now owns these symbols in runtime/src/value/net_tcp.rs, so
// build.rs no longer links this file. Keep it as historical fallback source
// only; relinking it with the Rust runtime creates duplicate symbols in
// libsimple_native_all.a on Darwin.
//
