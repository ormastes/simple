...
- backend-aware serve-plan metadata with vLLM default and SGLang planning
  fields
...
OpenCode CLI support follows the same owner rule. `opencode_cli` may construct
`opencode run` arguments, spawn through the process facade, poll a known pid,
and kill that pid through `process_kill`. It must not kill by process name or
use shell commands. SGLang support is currently serve-plan metadata only; live
SGLang readiness needs a focused host proof before it can satisfy completion
gates.
...
per-lane `llm_goal_evidence_<lane>_hint`,
