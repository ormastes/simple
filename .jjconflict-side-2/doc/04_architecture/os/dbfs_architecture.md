...
- `src/os/services/nvfs/core/pmap_btree.spl` — full CLRS B-tree (delete/rebalance), key type generalized
- `src/os/services/nvfs/core/intent_log.spl` — intent log structure (must add disk persistence)
- `src/os/services/nvfs/core/checkpoint.spl` — 4096-slot ring (must add META_DURABLE persistence)
...
**NVFS-arena backend implementation:** Existing concrete NVFS Arena impl in `src/os/services/nvfs/core/arena.spl` already satisfies the trait. No changes needed to existing NVFS consumers.
...
| Spec file | Covers |
|-----------|--------|
| `test/02_integration/storage/dbfs/dbfs_engine_pager_spec.spl` | Buffer manager: page eviction, dirty tracking, clock-sweep |
| `test/02_integration/storage/dbfs/dbfs_engine_btree_spec.spl` | Namespace B-tree: insert/lookup/delete/range-scan with Ino/DirEntryKey |
| `test/02_integration/storage/dbfs/dbfs_engine_wal_spec.spl` | WAL: append, flush, LSN monotonicity, CRC32C correctness |
| `test/02_integration/storage/dbfs/dbfs_recovery_spec.spl` | Mount recovery: clean mount, crash-then-recover, orphan reclaim |
| `test/02_integration/storage/dbfs/dbfs_driver_spec.spl` | FsDriver trait: mount, open, read, write, stat, readdir, mkdir, unlink, rename |
| `test/02_integration/storage/dbfs/dbfs_capability_spec.spl` | Extension probe: COW/Xattr/Acl/LargeFiles/PosixCompat reported correctly |
| `test/02_integration/storage/dbfs/arena_as_blob_backend_spec.spl` | NVFS-arena-as-backend: DbFsDriver with NVFS Arena impl (AC-4) |
| `test/02_integration/storage/dbfs/dbfs_hw_passthrough_spec.spl` | RawNvmeArena: sector reads/writes, arena verbs on BlockDevice mock |
| `test/02_integration/storage/dbfs/fat32_no_regression_spec.spl` | FAT32 still mounts, reads dirs, opens files after DBFS code lands (AC-7) |
| `test/02_integration/storage/dbfs/mount_table_dbfs_dispatch_spec.spl` | MountTable dispatches to DbFs variant; exhaustive match compiles |
...
- 15 spec files under `test/02_integration/storage/dbfs/` as listed in D11
