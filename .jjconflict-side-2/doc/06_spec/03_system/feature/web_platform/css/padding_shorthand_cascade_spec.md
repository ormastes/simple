...
> Mirrored manual for `test/03_system/feature/web_platform/css/padding_shorthand_cascade_spec.spl`.
...
This scenario proves authored ordering for nonnegative integer-pixel and zero
values across physical padding plus horizontal-LTR `padding-block` and
`padding-inline` mappings. The shorthand includes a CSS tab separator, and a
malformed shorthand does not replace earlier valid sides. Percentages, other
units, CSS-wide keywords, vertical writing modes, and full CSS cascade
conformance remain outside scope.

## Requirement traceability

- `REQ-WEB-BROWSER-003`: resolves selected physical/logical declarations and
  rejects one malformed shorthand.
- `REQ-WEB-BROWSER-004`: lowers three discriminating boxes through canonical
  Draw IR and Engine2D without skipped commands.
- `REQ-WEB-BROWSER-021`: supplies this executable modern SSpec and mirror.

## Scenario

### should resolve physical and logical padding in authored order

1. **Resolve authored padding order in canonical Web semantic style**
   - Three semantic `div` nodes retain padding `[4,5,4,5]`, `[1,2,3,5]`, and
     `[6,7,8,9]` in top/right/bottom/left order.
2. **Carry the source-order winners into exact layout geometry**
   - Their boxes are `[0,0,18,16]`, `[0,16,15,12]`, and `[0,28,24,22]`.
3. **Retain the winners in canonical Draw IR**
   - Source kind, geometry, and every physical padding side remain exact.
4. **Render the distinct cascade outcomes through Engine2D**
   - No command is skipped; exact blue/red/green counts are 288/180/528.

## Evidence boundary

This is a complete handwritten mirror pending `simple spipe-docgen`. A
qualified pure-Simple runner and provenance remain required before doc
generation or treating it as runtime PASS.

## Complete executable scenario reproduction

Runnable source:
`test/03_system/feature/web_platform/css/padding_shorthand_cascade_spec.spl`.
