...
for i in 1 2 3; do
  /usr/bin/time -f "sample=$i elapsed_s=%e max_rss_kb=%M" -a -o /tmp/wm_backend_measurement_time.txt \
    ...
    test test/03_system/wm_compare/backend_measurement_report_spec.spl --mode=interpreter --clean
done
...
(backend_measurement
  ...
  selected: "cpu_simd"
  status: "Initialized"
  command: "simple test test/03_system/wm_compare/backend_measurement_report_spec.spl"
  host: "linux-x86_64"
  warmup_count: 0
  sample_count: 3
  p50_us: 38580000
