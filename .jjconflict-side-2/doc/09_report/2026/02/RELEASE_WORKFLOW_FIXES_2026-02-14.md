...
2. **Core Unit Tests (INFORMATIONAL):**
   - `test/01_unit/compiler_core/` with 60s timeout
...
- name: Run Tests
  run: |
    $SIMPLE_RUNTIME test test/01_unit/ --timeout=300 || echo "⚠ Some unit tests failed"
...
- name: Run Smoke Tests
  ...
  run: |
    ...
    if $SIMPLE_RUNTIME test test/01_unit/compiler_core/ --timeout=60; then
