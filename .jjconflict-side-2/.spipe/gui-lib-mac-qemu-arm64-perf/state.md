...
- probe: Added `test/05_perf/graphics_2d/fnv1a_native_repro.spl` and `--debug-pixels` runner diagnostics; LLVM matches the small FNV repro and representative pixels, so the remaining native mismatch is full-frame render/hash behavior rather than a trivial checksum arithmetic failure.
