...
- [x] AC-7: `bin/simple test test/unit/os/memory/` passes; two new specs added — DONE (6 + 11 tests pass)
...
**AC-7:** No `test/unit/os/memory/` directory existed. Created new. Two existing OS test patterns studied: `crash_record_spec.spl` (inline class defs, interpreter-safe), `compiled_logic_test.spl`.
...
### 4-spec
Files written:
- `test/unit/os/memory/mold_linker_spec.spl` — 6 tests: install script exists, preference chain order, local path suffix, binary-conditional selection
- `test/unit/os/memory/mimalloc_os_spec.spl` — 11 tests: mimalloc_init, mi_malloc sizes, mi_free, MimallocAllocator allocate/deallocate before+after init
...
### 7-verify
**Test results:**
- `bin/simple test test/unit/os/memory/mold_linker_spec.spl` → PASSED (6/6, 170ms)
- `bin/simple test test/unit/os/memory/mimalloc_os_spec.spl` → PASSED (11/11, 174ms)
