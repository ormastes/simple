...
```bash
# Linux
./script/bootstrap-from-scratch.sh --qemu-freebsd --qemu-vm=/data/vms/freebsd.qcow2
...
bin/release/simple script/bootstrap-multiphase.spl \
  --qemu-freebsd \
  --no-verify \
  --qemu-port=10022
```
...
5. **Remote Build**: Executes `script/bootstrap-from-scratch-freebsd.sh` in VM
...
**Build fails in VM**
- Check FreeBSD script exists: `script/bootstrap-from-scratch-freebsd.sh`
