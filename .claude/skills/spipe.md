...
For LLM Caret agent/team/advisor/goal work, start with `src/app/llm_caret/agent_plan.spl`, `src/app/llm_caret/agent_files.spl`, `src/app/llm_caret/agent_vcs.spl`, `src/app/llm_caret/agent_discovery.spl`, `src/app/llm_caret/agent_mailbox.spl`, `src/app/llm_caret/agent_tui.spl`, `src/app/llm_caret/agent_tmux.spl`, `src/app/llm_caret/agent_runtime.spl`, `test/03_system/app/llm_caret/feature/llm_caret_agent_tui_handoff_spec.spl`, `test/03_system/app/llm_caret/feature/llm_caret_embedded_tmux_tui_spec.spl`, `doc/07_guide/app/llm/llm_caret_agent_teams.md`, and `doc/07_guide/app/llm/llm_caret_embedded_tmux.md`. The current supported surface is static prompt/argv planning, existing-file hash snapshots, `jj diff --name-only` changed-file discovery, simple MCP/plugin manifest parsing, plugin install argv planning, pure `btw`/`side` team mailbox routing, TUI-readable handoff rendering, embedded tmux-style process panes with caller-supplied CPU/memory usage, non-persistent team launch, and explicit agent, skill, MCP server, and plugin capability lists. Keep the SPipe capability shape homogeneous with `AgentCapabilitySet`: `agent_paths`, `skill_paths`, `mcp_servers`, and `plugins` are the canonical field/helper terms across state files, prompts, generated manuals, and review notes. Persistent live teams, background VCS watching, plugin install execution, live MCP registry discovery, live process sampling, and cross-agent transport require separate requirements.
