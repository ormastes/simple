...
**SFFI (Simple FFI)** is the Simple language's approach to FFI wrappers. All SFFI wrappers are written in Simple using a two-tier pattern to distinguish them from raw FFI code.

**Terminology:**
- **SFFI**: Simple FFI - FFI wrappers written in Simple
- **Raw FFI**: Direct `extern fn` declarations or Rust FFI code
- **SFFI wrapper**: The Simple wrapper function (tier 2)
- **SFFI-gen**: Tool to generate Rust FFI from Simple specs (`simple sffi-gen`)
...
✅ Use `simple sffi-gen` to generate Rust FFI from specs
```
...
- Write Rust files manually (use `simple sffi-gen` instead)
...
- Confuse SFFI wrappers (Simple) with raw FFI (Rust)
...
| `src/std/io/*.spl` | Stdlib I/O modules using SFFI |
...
| **Pattern** | Two-tier (extern fn + wrapper) | Direct FFI |
| **Prefix** | `rt_` for extern, clean name for wrapper | `rt_` exposed directly |
| **Generation** | `simple sffi-gen` from specs | Manual Rust code |
