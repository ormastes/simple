...
- **No `Results:` line at all = the run ABORTED, and the counts are UNKNOWN** (2026-08-23): a preflight gate that aborts before any spec executes used to print a fully-formed `Results: 587 total, 0 passed, 587 failed` with `Time: 0ms` and zero `SPEC FILE VERDICT:` lines, indistinguishable from a real mass failure. Fixed in `cd9dfa107d4`: a zero-executed run now prints `ABORTED BEFORE EXECUTION` and NO `Results:`/`Time:`/verdict line. If you see that block, do not classify any spec from it. See `doc/08_tracking/bug/test_runner_preflight_abort_reads_as_mass_failure_2026-08-23.md`.
...
- **`SIMPLE_NO_JIT=1` is a DECOY** — no reader anywhere in `src/compiler_rust/`; it is read only by the pure-Simple interpreter. Any A/B done with it proves nothing. The working knob is `SIMPLE_EXECUTION_MODE=interpreter|jit`, and since 2026-08-23 the *check* that it took effect is `SIMPLE_ENGINE_RECEIPT=1` (next bullet).
- **One unsupported operation silently demotes the WHOLE program to the interpreter** — demotion triggers on source *text* (`std.cli`, `get_cli_args`, `sys_get_args`, `rt_get_args`, `window_winit`), on the `.shs` extension, and on any JIT compile bail (unsupported lambda, named-fn-as-value, unresolved import, generator, no `fn main`). There is no per-function split: when a gate fires, `run_file_jit` is never called and *nothing* is JIT-compiled. A 44-line combined probe reported 38/38 AGREE for exactly this reason. Keep each probe in its own minimal file. Note the gates match SOURCE TEXT, so a **comment** naming a trigger token de-JITs the file — this bit the fixtures for the receipt below, where `jit_clean.spl` de-JIT'd itself because its own comment listed the tokens it was avoiding.
- **The engine receipt is how you check which engine ran — A/B alone still proves nothing** (2026-08-23). Two runs printing the same thing is *not* evidence they used the same engine, and never was; `SIMPLE_NO_JIT` cannot establish it and neither can comparing output. Set `SIMPLE_ENGINE_RECEIPT=1` and read the line the engine itself emits on stderr:

  ```
  [engine-receipt] engine=cranelift-jit requested=default demoted=no  reason=-                    file=.../jit_clean.spl
  [engine-receipt] engine=interpreter   requested=default demoted=yes reason=jit-unsafe-graphics: ... file=.../demote_graphics_text.spl
  ```

  `engine` is stamped from INSIDE the engine that executed (`evaluate_module`, the JIT's `execute`, the SMF loader), never by the CLI layer that requested a lane, so it reports what RAN and a silent demotion cannot be laundered into a green "jit" claim. It is a closed Rust enum with no setter reachable from a flag, so it cannot be forged. Values: `interpreter | cranelift-jit | llvm-jit | native | wasm | unstamped`.

  **A demotion is announced whether or not you asked for a receipt**, as `[engine-demotion] reason=<token> detail=<text>`, with no env var that disables it — including on the default lane, which is the JIT. `SIMPLE_JIT_COVERAGE=1` remains a separate, suppressible census; do not mistake its silence for the absence of a demotion.

  Any claim of the form "this behaves the same on both engines" needs receipts from both runs quoted alongside it. Guard + discriminating fixture pair: `sh scripts/check/check-engine-receipt-discriminates.shs`, `test/fixtures/engine_receipt/`. Background: `doc/08_tracking/bug/no_engine_receipt_silent_jit_demotion_2026-08-23.md`.
