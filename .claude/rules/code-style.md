...
- **Never mutate a collection through a temporary alias.** Simple's value semantics are copy-on-write, so `val t = self.table; t.push(x); self.table = t`, `self.xs = f(self.xs, v)`, and `.keys()`/`.values()` inside a loop body each deep-copy the WHOLE collection per write — O(n) per operation, invisible on small fixtures, catastrophic at real scale. Mutate through the single owner (`self.table.push(x)`, `self.a[i].b[k] = v`) and hoist `.keys()` above the loop. Ratcheted by `sh scripts/check/check-cow-alias-hotpath.shs` (push-time, `src/compiler/**` only) and reported at AUTHORING time over every file by lint rule `cow_alias_hotpath` — `warning[PERF-COW-001]` (round trip), `[PERF-COW-002]` (by-value helper store-back), `[PERF-COW-003]` (`.keys()`/`.values()` on a loop-INVARIANT receiver in a loop; a receiver rebound each iteration is exempt and must not be flagged). Rule doc: `doc/07_guide/tooling/lint/cow_alias_hotpath_rule.md`; analysis in `doc/08_tracking/bug/value_semantics_cow_alias_perf_class_2026-08-21.md`; open population (297 of 299 findings, incl. 101 the ratchet cannot see) in `doc/08_tracking/bug/cow_alias_hotpath_lint_findings_backlog_2026-08-23.md`.
