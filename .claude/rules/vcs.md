...
- **Directory fan-out — run `sh scripts/check/check-directory-fanout.shs` (exit 0 = safe).** Not one of the five range-scoped push guards above: it enforces structure.md's "≤10 files per directory" rule, which doesn't have a meaningful "outgoing range" shape (a directory's file count is a property of the whole tree, not of a diff) — so it always scans the full committed tree (`git ls-tree -r`, ~5-8s measured on 112,697 tracked files), not a `BASE..NEW` range. `doc/08_tracking/bug/architecture_rule_audit_2026-08-10.md` (Finding 2) found the rule as literally written is unenforceable: 1,599 directories exceed 10 files repo-wide, 1,006 even after excluding vendored trees (`src/compiler_rust/vendor/**` etc. per CLAUDE.md's Owned-Code Scope). So this guard is baseline-relative like the divergence guard: `scripts/check/directory_fanout_baseline.txt` records the 1,006 owned-code directories already over the limit as of 2026-08-10 (`path<TAB>count`), and the guard FAILs only on (a) a directory NOT in the baseline crossing 10 files (a genuine new violation), or (b) a baselined directory growing PAST its recorded count (the exact failure the audit measured: `scripts/check` grew by 30 files in one night with nothing checking it). Shrinking a baselined entry is free and encouraged; there is deliberately **no `--generate-baseline` flag** — regenerating it blind would launder every fan-out regression landed since. No directory is excluded for being "agent-owned"; only vendored third-party trees are excluded, repo-wide, per CLAUDE.md. Verdict: `PASS — <n> director(ies) checked, 0 over limit` exit 0 / `FAIL — <n> over limit: <names>` exit 1 / `ERROR — nothing was checked` exit 2. Both-direction fixture-proven (new-violation FAIL, grown-baseline FAIL, compliant-tree PASS with non-zero count) in an isolated fixture repo. `scripts/check` itself remains a known, un-fixed 585-file violation — recorded in the baseline, not reorganized here; that's a separate, larger decision.
