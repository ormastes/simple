...
|---|------|---------|---------|
| 1 | [fetch_user](#fetch_user) | Motivation | `Fetch`, `user`, `FetchUser` +7 |
| 2 | [design_overview_3](#design_overview_3) | Design Overview | `design`, `Design`, `DesignOverview` +10 |
| 3 | [load_data](#load_data) | Syntax | `load_data`, `data`, `load` +12 |
| 4 | [caller](#caller) | Syntax | `caller`, `Caller`, `compute` +8 |
| 5 | [promise_type_6](#promise_type_6) | Promise Type | `promise_type`, `Type`, `PromiseType` +10 |
| 6 | [get_cached_or_fetch](#get_cached_or_fetch) | Promise Type | `Fetch`, `GetCachedOrFetch`, `get` +12 |
| 7 | [process_pipeline](#process_pipeline) | Combinators (Promise Chaining) | `process`, `ProcessPipeline`, `process_pipeline` +14 |
| 8 | [parallel_fetch](#parallel_fetch) | Combinators (Promise Chaining) | `parallel`, `ParallelFetch`, `Fetch` +17 |
| 9 | [try_all](#try_all) | Combinators (Promise Chaining) | `All`, `TryAll`, `try` +12 |
| 10 | [example](#example) | Sync Functions | `Example`, `example`, `load_config` +8 |
...
| 13 | [double](#double) | Effect Inference (Automatic async/sync Detection) | `double`, `Double`, `map` +15 |
| 14 | [caller](#caller) | Effect Inference (Automatic async/sync Detection) | `caller`, `Caller`, `Assigning` +5 |
| 15 | [example](#example) | Effect Inference (Automatic async/sync Detection) | `Example`, `example`, `fetch_user` +2 |
| 16 | [process](#process) | Effect Inference (Automatic async/sync Detection) | `process`, `Process`, `Data` +2 |
| 17 | [ping](#ping) | Effect Inference (Automatic async/sync Detection) | `Ping`, `ping`, `Calls` +3 |
| 18 | [async_ping](#async_ping) | Effect Inference (Automatic async/sync Detection) | `AsyncPing`, `ping`, `Ping` +9 |
| 19 | [process](#process) | Interaction with ~= Operator | `process`, `Process`, `parse_async` +3 |
| 20 | [foo](#foo) | Interaction with ~= Operator | `Foo`, `foo`, `Can` +1 |
| 21 | [foo](#foo) | Migration Strategy | `Foo`, `foo` |
| 22 | [foo](#foo) | Migration Strategy | `Foo`, `foo`, `Returns` +2 |
| 23 | [handle_request](#handle_request) | Examples | `HandleRequest`, `handle_request`, `Handle` +7 |
| 24 | [main](#main) | Examples | `Main`, `main`, `load_config` +4 |
| 25 | [hash_stream](#hash_stream) | Examples | `hash_stream`, `hash`, `stream` +7 |
| 26 | [examples_32](#examples_32) | Examples | `examples`, `Examples`, `Pid` +14 |
| 27 | [unnamed_test](#unnamed_test) | Promise Type Details | `unnamed`, `Unnamed`, `is_rejected` +13 |
| 28 | [fetch_or_error](#fetch_or_error) | Promise Type Details | `FetchOrError`, `Error`, `Fetch` +16 |
| 29 | [might_fail](#might_fail) | Error Handling | `fail`, `might`, `Might` +13 |
| 30 | [error_handling_36](#error_handling_36) | Error Handling | `handling`, `Error`, `ErrorHandling` +13 |
| 31 | [technically_async](#technically_async) | Performance Considerations | `TechnicallyAsync`, `technically`, `technically_async` +7 |
