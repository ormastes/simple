...
Platform differences are **completely isolated** to header files in `seed/platform/`.
...
│  - Input:  seed/seed.cpp + platform headers                 │
...
The `seed/platform/platform.h` header uses preprocessor macros to select the correct platform:
...
Unix platforms (Linux, macOS, FreeBSD) share 95% of their implementation:
...
- Consistent behavior across Linux, macOS, FreeBSD
- Minimal maintenance burden (one implementation, not three)
...
grep "^[a-z_]*\s*rt_" seed/platform/unix_common.h | sed 's/ {$/;/' > /tmp/unix_api.txt
grep "^[a-z_]*\s*rt_" seed/platform/platform_win.h | sed 's/ {$/;/' > /tmp/win_api.txt
...
Create `seed/platform/platform_newos.h`:
