...
fn main() {
    ...
    code.push_str("#[allow(clippy::single_component_path_imports)]\n");
    code.push_str("use phf;\n\n");
    code.push_str("// Type alias to simplify complex phf::Map types\n");
    code.push_str("type MessageTuple = (&'static str, &'static str, Option<&'static str>, Option<&'static str>, Option<&'static str>);\n\n");
...
fn parse_message_catalog(content: &str) -> Option<SimpleCatalog> {
    ...
    for line in content.lines() {
        ...
        if let Some(id_part) = trimmed.strip_suffix(": {") {
...
fn generate_messages_map(code: &mut String, domain: &str, catalog: &SimpleCatalog) {
    ...
    code.push_str(&format!(
        "/// Default locale {} messages (embedded at compile time)\n",
        domain
    ));
    code.push_str("#[allow(clippy::type_complexity)]\n");
    code.push_str(&format!(
        "pub static {}: phf::Map<&'static str, MessageTuple> = phf::phf_map! {{\n",
        map_name
    ));
