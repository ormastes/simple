...
use super::memory::{ExecutableMemory, MemoryAllocator, PlatformAllocator, Protection};
use super::settlement::{FunctionTable, GlobalTable, NativeLibManager, TableIndex};
use super::smf::settlement::{
...
impl StartupLoader {
    ...
    fn read_function_table(
        ...
        let entry_count =
            header.func_table_size as usize / std::mem::size_of::<super::smf::settlement::FuncTableEntry>();

        for _ in 0..entry_count {
            let mut entry_buf = [0u8; std::mem::size_of::<super::smf::settlement::FuncTableEntry>()];
            file.read_exact(&mut entry_buf)?;

            let entry: super::smf::settlement::FuncTableEntry =
    ...
    fn read_global_table(
        ...
        let entry_count =
            header.global_table_size as usize / std::mem::size_of::<super::smf::settlement::GlobalTableEntry>();

        for _ in 0..entry_count {
            let mut entry_buf = [0u8; std::mem::size_of::<super::smf::settlement::GlobalTableEntry>()];
            file.read_exact(&mut entry_buf)?;

            let entry: super::smf::settlement::GlobalTableEntry =
                unsafe { std::ptr::read_unaligned(entry_buf.as_ptr() as *const _) };
