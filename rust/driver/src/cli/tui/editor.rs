        ...
        match action {
            EditorAction::InsertChar(c) => {
                ...
                if self.debug_mode {
                    ...
                    eprintln!(
                        "[DEBUG]   after len={}, cap={}, reallocated={}",
                        new_len,
                        new_cap,
                        new_cap != old_cap
                    );
            ...
            EditorAction::Backspace => {
                if self.cursor > 0 {
                    ...
                    if self.debug_mode {
                        eprintln!(
                            "[DEBUG] Backspace: cursor={}, buffer='{}', len={}, cap={}",
...
mod tests {
    ...
    fn test_backspace_deletes_full_indent_with_prevention() {
        ...
        assert_eq!(
            editor.buffer(),
            "   ",
            "Should leave 3 spaces (empty buffer prevention)"
        );
    ...
    fn test_backspace_deletes_4_spaces_with_text_after() {
        ...
        assert_eq!(
            editor.buffer(),
            "    x",
            "Should delete 4 spaces when content after cursor"
        );
        assert_eq!(editor.cursor(), 4);
    }
    ...
    fn test_multiline_stays_active_on_non_colon_line() {
        ...
        // Now should execute
        assert!(
            matches!(result4, EditorResult::Accepted(_)),
            "Should execute on empty line"
        );
