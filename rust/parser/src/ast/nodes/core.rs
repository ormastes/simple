...
pub enum Expr {
    ...
    /// Format string with interpolation: "hello {name}!"
    /// Includes compile-time metadata with const_keys extracted from placeholders.
    /// The type_meta.const_keys contains ["name"] for the example above.
    FString {
        parts: Vec<FStringPart>,
        /// Compile-time metadata including const_keys extracted from placeholders
        type_meta: super::const_meta::TypeMeta,
    },
    Bool(bool),
    Nil,
    Symbol(String),
