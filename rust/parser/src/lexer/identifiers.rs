...
impl<'a> super::Lexer<'a> {
    ...
    /// Handles nested braces correctly. Performs Tier 1+2 pre-scanning to
    /// track strings, comments, escapes, and brace pairs within the payload.
    pub(super) fn scan_custom_block_payload(&mut self, kind: &str) -> TokenKind {
        ...
        while let Some(ch) = self.peek() {
            ...
            // Tier 1: Track escape sequences
            if ch == '\\' {
                pre_lex.escape_positions.push(pos);
                payload.push(ch);
                self.advance();
                // Push the escaped char too (if any)
            ...
            // Tier 1: Track double-quoted strings
            if ch == '"' {
                let start = pos;
                payload.push(ch);
                self.advance();
                while let Some(sc) = self.peek() {
                    let spos = payload.len();
                    if sc == '\\' {
                        pre_lex.escape_positions.push(spos);
                        payload.push(sc);
                        self.advance();
            ...
            // Tier 1: Track single-quoted strings
            if ch == '\'' {
                let start = pos;
                payload.push(ch);
                self.advance();
            ...
            // Tier 2: Track line comments (# to EOL)
            if ch == '#' {
                let start = pos;
                payload.push(ch);
                self.advance();
            ...
            // Tier 2: Track -- SQL comments (for sql blocks)
            if ch == '-' && kind == "sql" {
                if let Some('-') = self.peek_ahead(1) {
                    let start = pos;
                    payload.push(ch);
                    self.advance();
                    while let Some(cc) = self.peek() {
