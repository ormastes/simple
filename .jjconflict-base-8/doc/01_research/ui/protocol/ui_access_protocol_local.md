...
## Concrete Domain-Guided Next Interfaces

The research points to one additive order for the remaining lanes:

1. add a value-specific semantic interface over canonical nodes
2. add visual fallback outputs that reference the same node/surface ids
3. add adapter-backed snapshot producers targeting the same canonical structs
4. add perf verification on snapshot/query/ensure/value hot paths

This order fits the repo because it preserves the current internal snapshot as
the compatibility center while expanding around it.
