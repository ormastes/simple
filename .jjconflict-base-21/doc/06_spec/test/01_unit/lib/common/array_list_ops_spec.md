...
<!-- sdn-diagram:id=array_list_ops_spec.arch -->
<details class="sdn-source">
<summary>SDN source</summary>

```sdn id=array_list_ops_spec.arch hash=sha256:auto render=ascii
@layout dag
@direction LR

array_list_ops_spec -> std
```
...
```ascii generated-from=array_list_ops_spec.arch hash=sha256:auto
...
#### drops nothing for negative count
...
```simple
expect(array_drop([1, 2, 3], -1)).to_equal([1, 2, 3])
```
...
#### returns empty array for invalid chunk size
...
```simple
expect(array_chunk([1, 2, 3], 0)).to_equal([])
expect(array_chunk([1, 2, 3], -1)).to_equal([])
```
...
| Status | Active |
| Source | `test/01_unit/lib/common/array_list_ops_spec.spl` |
| Updated | 2026-06-01 |
