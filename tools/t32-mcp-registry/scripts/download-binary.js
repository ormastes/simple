...
// The package should contain the wrapper/native assets required by T32 MCP.
...
async function main() {
  ...
  if (fs.existsSync(marker)) {
    ...
    if (installed === `${VERSION}-${platform}`) {
      console.log('Simple T32 MCP package already installed.');
      return;
    }
  }
  ...
  console.log(`Downloading Simple T32 MCP package v${VERSION} for ${platform}...`);
  console.log(`URL: ${url}`);
  ...
  try {
    ...
    console.log(`Installed Simple T32 MCP package to ${nativeDir}`);
  } catch (err) {
    console.warn(`Warning: Could not download T32 MCP package assets: ${err.message}`);
    console.warn('T32 MCP will remain unavailable until wrapper/native assets are installed.');
