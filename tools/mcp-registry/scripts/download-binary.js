...
// The package should contain a native `simple_mcp_server` binary for the
...
async function main() {
  ...
  if (fs.existsSync(marker)) {
    ...
    if (installed === `${VERSION}-${platform}`) {
      console.log('Simple MCP package already installed.');
      return;
    }
  }
  ...
  console.log(`Downloading Simple MCP package v${VERSION} for ${platform}...`);
  console.log(`URL: ${url}`);
  ...
  try {
    ...
    if (entries.length > 0) {
      ...
      const binaries = new Set([
        ...
        path.join(pkgDir, 'src', 'compiler_rust', 'bin', 'release', `simple_mcp_server${ext}`)
      ]);
      ...
      if (fs.existsSync(legacyReleaseRoot)) {
        for (const entry of fs.readdirSync(legacyReleaseRoot)) {
          ...
          binaries.add(path.join(legacyReleaseRoot, entry, `simple_mcp_server${ext}`));
        }
      }
    ...
    console.log(`Installed Simple MCP package to ${nativeDir}`);
  } catch (err) {
