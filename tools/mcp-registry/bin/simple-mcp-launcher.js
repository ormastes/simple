...
// Prefer the native MCP binary in all normal cases.
// Hosted `simple run src/app/mcp/main.spl` fallback is legacy-only and must be
...
function resolveNativeServer(rootDir) {
  ...
  for (const platformDir of platformDirs()) {
    ...
    candidates.push(path.join(rootDir, 'src', 'compiler_rust', 'bin', 'release', platformDir, `simple_mcp_server${ext}`));
  }
  candidates.push(
...
function findRuntime() {
  ...
  throw new Error(
    'No native Simple MCP server binary found. Build or install `simple_mcp_server`, ' +
