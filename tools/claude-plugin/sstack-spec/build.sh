#!/usr/bin/env bash
# build.sh -- Validate and package the sstack-spec plugin
set -euo pipefail
...
for f in "$SKILLS_DIR/sstack-spec/SKILL.md" "$AGENTS_DIR/spec.md"; do
    if [ ! -f "$f" ]; then
...
# 5. Package (tar.gz)
PACKAGE_NAME="sstack-spec-$(python3 -c "import json; print(json.load(open('$PLUGIN_JSON'))['version'])" 2>/dev/null || echo "0.1.0")"
