#!/usr/bin/env bash
# build.sh -- Validate and package the sstack-arch plugin
set -euo pipefail
...
for f in "$SKILLS_DIR/sstack-arch/SKILL.md" "$AGENTS_DIR/arch.md"; do
    if [ ! -f "$f" ]; then
...
# 5. Package (tar.gz)
PACKAGE_NAME="sstack-arch-$(python3 -c "import json; print(json.load(open('$PLUGIN_JSON'))['version'])" 2>/dev/null || echo "0.1.0")"
