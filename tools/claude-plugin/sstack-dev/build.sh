#!/usr/bin/env bash
# build.sh -- Validate and package the sstack-intake plugin
set -euo pipefail
...
echo "=== sstack-intake plugin build ==="
...
# 5. Package (tar.gz)
PACKAGE_NAME="sstack-intake-$(python3 -c "import json; print(json.load(open('$PLUGIN_JSON'))['version'])" 2>/dev/null || echo "0.1.0")"
