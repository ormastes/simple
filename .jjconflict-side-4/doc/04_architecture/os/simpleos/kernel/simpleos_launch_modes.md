...
The kernel resolves executable bytes through `resolve_executable_bytes(path, arch)` and then creates the task through `create_user_task(...)`. This is the right proof lane for `test/03_system/browser_engine_in_qemu_spec.spl` and similar x86_64 baremetal ELF tests.
