...
expect(script).to_contain("invalid-multiboot-elf32-little-endian")
expect(script).to_contain("invalid-multiboot-elf-machine")
...
expect(script).to_contain("guest-input-press-state-frame-correlation-missing")
expect(script).to_contain("guest-pointer-irq-state-frame-correlation-missing")
...
expect(shell).to_contain("input_compositor.maximize_window")
expect(shell).to_contain("input_compositor.restore_window")
...
expect(web).to_contain(
    "simple_web_content_render_request_with_theme(" +
    "theme, title, body_html, width, full_h).with_pixel_output()")
...
expect(kernel_output).to_contain("simpleos_wm_fullscreen_kernel_build_status=external-elf-invalid-multiboot-elf32-little-endian")
