...
| 2 | Native/SMF placement contracts and provider provenance adapters; real callable mapping with ten stable refusal codes (`callable_map_v1`), artifact bytes hashed before AND after the mapping so a rename-swap inside the hash->load window is refused (`SwappedDuringLoad`, code 11), operation family widened to `i64(i64...)` arity 0..6 with executed native==static parity at 0/1/2/3/6, and a typed-token session callable adapter (`session_callable_adapter_v1`) whose capability cannot be replayed across sibling sessions [agents U, U2] | SMF placement still unexecuted — `smf_dlsym` returns a section offset and `exec_mapping_map_rx` maps bytes with no entry ABI, and `src/os/smf/smf_dynlib.spl` is Codex's `egl-prod-integration` lane; artifact identity is not fd-pinned (three shared-descriptor routes measured unavailable on this seed/kernel); the one-statement production hook is stated, not made, because every candidate call site is Codex-owned |
...
| 4 | Stable parse-result provider seam over a frozen five-source corpus (`parse_result_provider_seam_v1`, a struct of callables, no registry); dialect identity COMPUTED from the tables a dialect executes rather than declared — four independent digests (grammar/actions/schema/semantic) whose probe domain is derived from the tables themselves; SDN admitted as a second dialect through the same seam; and a compatibility example proving the seam result is byte-identical to what the frontend's public entry already produces [agents V, V2] | The seam is still not CALLED by `parse_full_frontend_with_scope` — `src/compiler/10.frontend/frontend.spl` is Codex-owned, and the hook must not be applied until per-request identity verification (~2s/request on this seed) is replaced by an admit-once entry; the sosh dialect is BLOCKED on an enum-name collision (`doc/08_tracking/bug/shell_stmtkind_collides_with_frontend_stmtkind_2026-09-12.md`); the SIMD provider slot is still unmapped |
| 5 | SIMD promotion contract; exact-evidence V2 lifecycle; sealed stepwise transaction; truthful routing receipts; partition-safe scalar summaries; sealed 22-state transition-table monoid with exhaustive unsealed semantic verification and canonical admission digests; plus nine published candidate classes derived two independent ways, scalar state resolution through sealed and unsealed rows, issue/verify routing receipts that recompute masks and resolved state from the claimed bytes rather than trusting any receipt field, an advisory frontend wrapper, and boundary-state qualification with an AVX2 classifier primitive and its contract spec [agents W, P] | Native x86 / self-host qualification (this host is aarch64 with no AVX2, so the AVX2 primitive is contract-verified, not executed), broader lexical coverage, and the emitted v3/v4 siblings |
| 6 | GPU admission and Engine2D completion bridges with negative specs; hosted ABI-v1 mapper/lifetime owner with sealed Linux provider bytes, bounded capability tokens, retained unload fences, terminal receipt/resource correlation, staged checksum readback, and a task-owner consumer that requires live typed provider/resource/completion projections | Runtime-owned exact device-program image token, Vulkan parser kernel, and real device fixture |
| 7 | A real admit -> resident -> retire execution island (`environment_resident_executor_v1`) with transfer-aware scheduling; a deferral feedback loop where one round's `deferred` set feeds the next and starvation is bounded, proven over a live loop with a task that can never fit; a device execution body joined to the packed completion authority route so the CPU rasterizer refuses a device-lane lease (`LaneMismatch`) and a spent authority stays spent; and measured transfer costs taken from the backend's own damage/readback receipts instead of declared numbers [agents X, X2, T] | Authority ISSUANCE from a real Vulkan fence is Codex-fenced — nothing in this tree can tell a real fence from a fabricated record, and the adapter from the completion-port owner to the packed authority record does not exist; device-lane measured bytes return zeros because the island moves none itself; incremental (non-sticky) upload measurement needs per-crossing damage clearing, which only `present()` owns |
| 8 | Requirements, plans, guide, manual, pure policy resolver, presence-aware supplied-layer collector, authenticated sealed collect-to-prepare boundary, catalog projection, and preparation bundle; plus a host-capability ledger PROBE (`scripts/audit/egl-host-capability-ledger.shs`, 444 lines) that describes a host in four explicit states and never blocks a push — deliberately an audit, not a gate [agent N] | Trusted command/config owner wiring, package integration, explain output, and measured NFR evidence all remain Codex's: the owner must preserve `{present, value, source}` per the handoff below, and this branch adds no consumer |
