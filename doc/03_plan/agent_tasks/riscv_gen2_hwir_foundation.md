...
| A13 | Scalar completion/product closure | `/root` with bounded parallel reviews | ALU/control/LSU completion, skid, arbitration, trap, sole retirement owner, fault aggregation, mixed sequential emission, binding closure, and combined GHDL scenario are implemented; admitted self-hosted execution evidence remains pending |
| A14 | M/Zmmul provider | `/root` with `muldiv_critical_review` | declarative arithmetic projection and host oracle are active; identity/W-mode hardening landed after critical review, while the iterative owner, product binding, cycle simulation, and GHDL evidence remain pending |
| A15 | Bootstrap qualification authority | `full_cli_evidence`, merge owner `/root` | bootstrap5 compiled 839/839 units but its post-build sanity rejected and deleted the candidate without retaining diagnostics; evidence-retention hardening landed, and a fresh non-competing post-fix authority is still required |
| A16 | Zicsr access and state ownership | fast audits `gen2_gap_fast_audit`/`csr_owner_fast_review`, merge/final review `/root` | all six projections, privilege/read-only/identity gates, atomic one-entry owner, product composition, focused scenarios, and RV32/RV64 GHDL closure rows are implemented; admitted self-hosted cycle qualification remains pending |
