...
| --- | --- | --- | --- | --- |
| 1 | B1 Stage-3 recovery / `/root` | **STOP/WARN**: durable HIR sink is source-green, but process-tree sampler redesign exhausted three cycles; no current candidate | Prior Stage-3 logs, the current-source RSS bug, and admitted local output `build/restart12-stage3-admitted` (Stage2/admitted SHA `2ec71042dd69cf0001fc3f61640c28038a450048f34e416103988b1627431950`) | Do not start Stage 3 yet. In a fresh scoped session implement the non-Python, v3-compatible, descriptor-bound process-group sampler/analyzer and its strict correlation/deliberate-red contract recorded in the bug. After review passes, run `scripts/bootstrap/bootstrap-from-scratch.sh --resume-stage3-from-admitted=build/restart12-stage3-admitted --jobs=1` once. |
