...
| 1 | B3 receipt authority / `stage3_admission`; merge owner `/root` | **source PASS / executable WARN** | Composer exact-key parsing, ledger/path-list retention, source revalidation, destination rehash; deterministic LD_PRELOAD writer-red interposer and host-native write/rename/pass-through fixture PASS; independent highest review | After B1/B1.1 produce admitted Stage 3/4, run the focused composer spec once. The writer-red harness is source-valid but not yet canonically reachable: the runner leaves a PID-suffixed staging manifest without publishing its unique path or invoking the harness. Wire it after manifest revalidation and before successful composition, or publish a unique retained-manifest receipt; then execute copy/publication cleanup reds. Keep the broad executable checkbox open. |
...
| 4 | B8 integration / `/root`; final reviewer `/root` | **PASS after final locked transaction** | The facade, writer-red, memory-attribution, and plan delta are committed in the final detached HEAD; its reachable hash and WARN status are recorded in `/tmp/restart12-vhdl.done` after push | No resume. The final transaction uses the required lock, fetch/rebase/push/refetch/reachability proof, never forces, and never creates a branch. |
