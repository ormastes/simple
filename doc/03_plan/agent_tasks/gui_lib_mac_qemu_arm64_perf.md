...
- Output: update `doc/01_research/local/gui_lib_mac_qemu_arm64_perf.md` with evidence deltas only.
...
- Compare against `test/baselines/*` and emit mismatch count, first mismatch coordinate, checksum, and markdown report.
...
- Run `test/perf/graphics_2d/perf_2d_runner.spl`, `test/integration/rendering/perf_smoke_spec.spl`, and WM perf specs.
