...
| Runtime theme switching | PROTOCOL/CACHE + K1 OWNED-QUEUE SOURCE ACCEPTED; K2/CATALOG SERIES REJECTED; PACKAGE SYNC OWNER BLOCKED; fail-fast system contract | `ThemeChangedV1`, BrowserBackend cache identity, and K1 copied-payload queues are landed. K2 exhausted three cycles on cross-architecture registration/entry stability gaps; generated catalog exhausted three cycles on stale active/frame authority; package transactions await an injected hosted synchronization store. ThemeService/consumer wiring remains unsafe; see the linked K2, catalog, and package blocker docs |
