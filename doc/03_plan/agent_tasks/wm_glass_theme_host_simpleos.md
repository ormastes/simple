...
| Simple Web theme authority | HOSTED APPLICATION PENDING; default Web CSS pixel route SOURCE FIXED; REVIEW ACCEPTED | The default `ui.browser.backend.render_frame` pure-Simple route now sends request HTML/CSS through canonical layout -> Draw IR -> Engine2D, with a CSS-only material/readback regression; Chromium compatibility and GUI widget composition remain separate. Production hosted registry construction still calls `HostedBrowserRendererProcess.create` (handshake disabled). The held hosted-runtime owner must instead drive `create_with_theme -> theme_ready -> begin_init(themed_html)` using an exact accepted projection. The worker currently stores only revision/material SHA and opens raw initial/replay HTML; it must receive producer-generated themed HTML before `BrowserSession.open_html`. Identity-only acknowledgements are not CSS/material application. Add projection, registry state-machine, and worker ordering/render regressions after that owner implementation; live runtime/capture evidence remains open. |
