...
| Simple Web theme authority | TWO APPLICATION GAPS PENDING; canonical producer REVIEW ACCEPTED | `simple_web_content_render_request_with_theme` correctly appends package CSS and emits the `wm-app-content` material wrapper, but production registry construction still calls `HostedBrowserRendererProcess.create` (handshake disabled). The held hosted-runtime owner must instead drive `create_with_theme -> theme_ready -> begin_init(themed_html)` using an exact accepted projection. The worker currently stores only revision/material SHA and opens raw initial/replay HTML; it must receive producer-generated themed HTML before `BrowserSession.open_html`. Identity-only acknowledgements are not CSS/material application. Separately, `ui.browser.backend.render_frame` sends a GUI/widget `DrawIrComposition` directly to the default pure-Simple pixel backend; that route does not consume request CSS/HTML, so package-only material token changes cannot alter default Simple Web pixels. The browser-backend owner must route the default Web pixel artifact through the canonical HTML/CSS layout-to-DrawIR path while keeping widget composition as the GUI producer. Add projection, registry state-machine, worker ordering/render, and default-Web snapshot-to-computed-style/DrawIR regressions after the owner implementations. |
