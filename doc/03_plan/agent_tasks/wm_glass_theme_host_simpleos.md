...
| Simple Web glass | SEMANTICS IMPLEMENTED; INPUT/OVERLAY DRAW IR SOURCE FIXED; LIVE PROOF PENDING | CPU and Draw IR share transformed/aligned/clipped theme-derived input text plus selection/caret overlays; textarea remains explicitly open, and current-source computed-style/Draw-IR/framebuffer proof has not passed |
...
| Runtime theme switching | PROTOCOL/CACHE + K1 OWNED-QUEUE SOURCE ACCEPTED; K2 ABI BLOCKED; fail-fast system contract | `ThemeChangedV1` fixes the bounded notification wire format and BrowserBackend cache identity; K1 now owns copied bounded payloads, reports exact receive states, and rejects unsupported capability transfer without claiming authentication. K2 dispatcher-bound current-task/source authentication and syscall copy-in/out, transactional package refresh, generated SimpleOS snapshot catalog, ThemeService, and consumer wiring remain prerequisites |
