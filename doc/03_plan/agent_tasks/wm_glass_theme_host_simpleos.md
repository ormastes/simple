...
| Simple Web theme authority | TWO APPLICATION GAPS PENDING; canonical producer REVIEW ACCEPTED | `simple_web_content_render_request_with_theme` correctly appends package CSS and emits the `wm-app-content` material wrapper, but the hosted browser worker still opens raw initial/replay HTML after the theme handshake. Its owner must apply the accepted snapshot through that canonical wrapper (or an equivalent owner-owned helper) before `BrowserSession.open_html`; identity-only acknowledgements are not CSS/material application. Separately, `ui.browser.backend.render_frame` sends a GUI/widget `DrawIrComposition` directly to the default pure-Simple pixel backend; that route does not consume the request CSS/HTML, so package-only material token changes cannot alter default Simple Web pixels. The browser-backend owner must route the default Web pixel artifact through the canonical HTML/CSS layout-to-DrawIR path while keeping widget composition as the GUI producer. Add hosted-worker and default-Web snapshot-to-computed-style/DrawIR regressions after the owner implementations. |
