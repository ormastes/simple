...
1. Render parity agent
   - Scope: `test/integration/rendering/`, `src/os/compositor/screenshot_compare.spl`,
...
2. WM/web capture agent
   - Scope: `src/app/wm_compare/`, `test/system/wm_compare/`, Electron evidence
...
- Replace or repair the missing
  ...
  dependency in `test/integration/rendering/screenshot_compare_helpers.spl`.
- Do not claim Vulkan parity from
  `test/integration/rendering/engine2d_cpu_vulkan_parity_spec.spl` until real
  Vulkan pixels are compared against CPU/software readback.
- Use `test/integration/rendering/engine2d_cpu_metal_parity_run.spl` as the
