...
- `InstalledInterfaceActionV1`
- `InterfaceActionArchiveErrorV1`
- `PinnedArchiveCapability`
- `ArchiveFileIdentityV1`
- `PinnedArchiveErrorV1`
- `CasBatchTransactionV1`
- `CasBatchTransactionErrorV1`
...
## Archive cutover implementation lanes

The signatures, invariants, and call order are frozen in
`doc/05_design/compiler/perf/persistent_package_module_index_compile_optimization.md`.
The following three agents work in parallel and must not edit each other's
write sets or shared driver integration files.

| Agent lane | Exclusive write set | Exact owned API | Required fail-fast placeholder |
|---|---|---|---|
| ARCHIVE-INGEST | `src/compiler/20.hir/archive/interface_action_archive.spl`; `test/01_unit/compiler/archive/interface_action_archive_spec.spl` | `InterfaceActionArchiveV1`, `InstalledInterfaceActionV1`, `InterfaceActionArchiveErrorV1`, `interface_action_archive_decode_v1`, `interface_action_archive_install_v1` | `PKG-ARCHIVE-E-INGESTION-NOT-WIRED` |
| ARCHIVE-PIN | `src/compiler/80.driver/cache/pinned_archive_capability.spl`; `test/01_unit/compiler/cache/pinned_archive_capability_spec.spl` | `ArchiveFileIdentityV1`, `PinnedArchiveCapability`, `PinnedArchiveErrorV1`, `pinned_archive_open_verified_v1`, `pinned_archive_read_member_v1`, `pinned_archive_close_v1` | `PKG-ARCHIVE-E-PINNED-CAPABILITY-NOT-WIRED` |
| SCC-BATCH | `src/compiler/80.driver/cache/cas_batch_transaction.spl`; `test/01_unit/compiler/cache/cas_batch_transaction_spec.spl` | `CasBatchTransactionV1`, `CasBatchTransactionErrorV1`, `cas_batch_begin_v1`, `cas_batch_stage_object_v1`, `cas_batch_publish_v1`, `cas_batch_abort_v1` | `PKG-ARCHIVE-E-SCC-BATCH-NOT-WIRED` |

### Lane acceptance invariants

- **ARCHIVE-INGEST:** no source-loader, path-open, package-index mutation, or CAS
  publication imports; typed decode and semantic/action installation are
  all-or-nothing and equivalent to admitted frozen-source compilation.
- **ARCHIVE-PIN:** one no-follow descriptor identity spans verification through
  consumption; no path reopen, symlink acceptance, copyable capability, or
  descriptor leak on error.
- **SCC-BATCH:** exact SCC membership is declared before staging; all mappings
  become visible through one generation/CAS of `CURRENT`; partial, stale, or
  conflicting transactions leave the prior generation unchanged.
- Every lane defines exhaustive error mapping and mutation-red unit coverage.
  No lane may report PASS while its fail-fast placeholder is reachable.

### Merge-owner-only write set and sequence

Only `MERGE`, after all three lanes pass independent review, may edit:
...
- imports/registries needed to expose the three new modules

`MERGE` wires the sequence: pin generation -> open pinned archive -> typed
decode -> atomic install -> close; rebuilt SCC -> begin -> stage exact members
-> publish once, or abort. It then removes all three fail-fast placeholders and
runs the focused archive/package-index gate once. Provider agents must not make
these integration edits.

**MERGE status (2026-09-02): implemented; focused static and working/staged
environment guards PASS.** The
shared receipt boundary is `package_archive_types.spl`; clean-package routing is
typed and capability-based; dirty-SCC publication uses one CAS batch; the
environment-only archive path handoff and sequential action publication have
been removed. Runtime verification remains intentionally deferred to the
separate admitted-compiler lane.
