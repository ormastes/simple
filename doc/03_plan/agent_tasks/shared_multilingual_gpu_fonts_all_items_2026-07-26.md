...
| Runtime | `e480c48da71` contains the reviewed `ExistsCheck` repair. Its fresh P0 attempt retained an isolated A failure, a successful temporary seed-built Stage 2 (`e714d284...`), one failed case-25 run, and one failed Stage-3 producer run, each with the same nil-receiver/SIGILL. No Stage 4 candidate exists; Stage 2 is diagnostic only. | retain the e480 receipts, localize the seed/self-host transition still compiling the compiler's own optional metadata path with pre-repair lowering, then use a fresh bounded P0 window to produce a current pure-Simple Stage 3 parent and incremental Stage 4; no unchanged rerun or full bootstrap |
