...
| `interpreter-startup-parity` | performance-team; controlled cold/warm host | Generic signed receipts are fail-closed. Implement `scripts/check/check-interpreter-startup-parity.shs`; retain at least 50 raw monotonic launch samples per cold/warm × Simple/Python/Bun/Go cell, environment and executable identity, exact interpreter mode receipts, semantic outputs, recomputed p50/p95, and strict Simple-below-each-reference verdicts. Warm means warmed process launch, not in-process fib throughput. |
