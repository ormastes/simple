...
| `rust-go-benchmark-parity` | performance-team; semantic-equivalence oracle | Generic signed receipts are fail-closed. Implement `scripts/check/check-rust-go-benchmark-parity.shs`; retain canonical Stage 4 authority, runtime-fed equivalent Simple/Rust/Go fixtures with equal optimization barriers, at least 30 interleaved raw samples, executable/toolchain/environment identity, recomputed statistics, and the explicit `Simple p50 <= Rust p50 && Simple p50 <= Go p50` verdict. |
