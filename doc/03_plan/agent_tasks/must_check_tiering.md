...
| `rust-go-benchmark-parity` | performance-team; semantic-equivalence oracle | Run `scripts/check/check-rust-go-benchmark-parity.shs` through the external importer with its nine lane-specific committed blobs. Retain canonical Stage 4 authority, the committed runtime-fed 100,000-operation Simple/Rust/Go fixture, 50–1000 rotated trial-interleaved samples, Linux ELF executable/shared-object type plus target identity, and recomputed p50/p95/max RSS. The v1 schema rejects non-Linux environments and requires signed `target_kind=native-host` with `target_id` equal to the environment `host_id`. Promotion derives `Simple p50+p95 <= Rust p50+p95 && Simple p50+p95 <= Go p50+p95`; a signed ratio or Markdown table is not evidence. |
