...
| `interpreter-startup-parity` | performance-team; controlled cold/warm host | Run `scripts/check/check-interpreter-startup-parity.shs` through the external receipt importer with its six committed attachments. Retain 50–1000 ordered monotonic launch samples per cold/warm × Simple/Python/Bun/Go cell, environment and executable identity, exact interpreter mode receipts, semantic outputs, and recomputed p50/p95. Warm means warmed process launch, not in-process fib throughput. Promotion requires strict Simple-below-each-reference p50+p95 plus the exact live canonical Stage 4 chain. |
