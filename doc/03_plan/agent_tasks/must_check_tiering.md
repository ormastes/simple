...
| `binary-size-go-parity` | performance-team; equivalent stripped programs | Run `RUNS=50 WORKERS=100 sh scripts/check/check-cross-language-perf.shs`, then produce the committed bundle required by `scripts/check/check-binary-size-go-parity.shs`: the exact Stage 4 compiler blob, stripped Simple/Go ELF blobs, exact size metrics, semantic oracle/sources/output hashes, compile trace, and canonical 21-field pure-Simple Stage 4 provenance. The checker recomputes compiler/source/helper bindings, hashes, sizes, target identity, and `Simple <= Go`; custom admission summaries, Rust-seed, or compressor evidence fail. |
