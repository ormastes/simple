...
| `doc/03_plan/compiler/dependency_analysis/plan.md` | `needs-evidence` | Compiler Spark | W2-A2 lazy loader bridge is now checked in the source plan and focused `module_loader_lazy_spec` evidence passes; next closure action is record `SIMPLE_LAZY_PARSE=1` completion evidence plus full dependency-flow smoke before marking Wave 2 done. |
