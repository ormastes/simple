...
| `doc/03_plan/runtime/default_native_runtime_shift_phase2_plan.md` | `needs-evidence` | Runtime Spark | Simple-core runtime smoke passed on 2026-06-18 after adding the missing string-builder ABI and no-op bootstrap safepoint; `SIMPLE_LIB=src bin/simple check src/runtime/simple_core` also passed. The row remains open because `check-mcp-native-smoke.shs` still fails: startup/direct-rt gates are green, but stale-stamp re-probe and MCP/LSP framing/schema validation fail. Next step is fix that native smoke/output path, then rerun MCP/LSP package criteria. |
...
| `doc/03_plan/app/mcp/mcp_scenario_manual_quality.md` | `needs-evidence` | App/UI Spark | Reconcile the current one-scenario source with the richer recovered manual/source shape, then make `test/02_integration/app/mcp_stdio_integration_spec.spl` pass. Attempted 2026-06-18: source-mode `bin/simple_mcp_server` emitted no protocol stdout for initialize/tools-list JSONL input, while packaged native responded to initialize/tools-list but still failed `tools/call` argument extraction, so the row cannot be marked done. |
