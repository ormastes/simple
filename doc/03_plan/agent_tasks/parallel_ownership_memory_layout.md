...
| P2 runtime transport | Active: scalar pool-state ABI is internal-only and bounded; public facade is deliberately not admitted. The legacy actor facade is also not an ownership authority: `ActorRef` retains a value mailbox while the scheduler retains an actor value. | Finish a bounded self-hosted native `PoolStateV1` callback run, then migrate the generic pool separately. Replace the actor pair with one stable scheduler-owned mailbox/state handle before relying on actor send for parent/child transport. | Native callback result, Full-until-release, close/idle/destroy, and alternate-provider execution; one actor send/ask path must prove it reaches the same scheduler-owned mailbox, and no `GLOBAL_*` path may be presented as the new pool. |
