...
| 3 | Confucius | Offload break-even: `test/03_system/app/simpleos_gpu_host/processing_ir_offload_break_even_spec.spl` | Native receipt records CPU/device/transfer/total/RSS medians and measured decisions. | Linux CUDA GPU | POLICY ACTIVE, NATIVE PARTIAL: generated-CUDA round trip selects CPU at 64/65,536 and GPU at 1,048,576/8,388,608. The direct ProcessingIR candidate completes exactly at 1,048,576 with device provenance and no fallback; bulk readback reduced cold execution from 1044501 us to 593323 us, and a retained session measured 861499 us cold versus 69331 us warm. The rebuilt diagnostic probe proves a valid correlated CUDA receipt whose only failure is 8x output parity. A fresh 217-module daemon exposed a trait-erased Engine2D shutdown crash; concrete lifecycle dispatch fixes it and restores the valid receipt. Disassembly localizes the 8x wire value to tagged `[u32]` storage read as unboxed. A one-call runtime bulk copy+checksum replaces the million-iteration Simple loop and passes its focused unit; exact 3-warmup/5-sample live medians remain open. |
...
| 6 | Kierkegaard | Injection matrix: `doc/03_plan/sys_test/gpu_backend_failure_injection_matrix.md` | Maps unavailable, submit, readback, mismatch, and fallback cases to hooks/evidence. | Linux for CUDA/Vulkan; prepared macOS for Metal | PARTIAL: writable mmap ABI smoke passes, the retained native Vulkan candidate's default plus all five fault phases pass, and deterministic Linux CUDA submit fallback completes with exact reason `16` under separate HELLO/request deadlines and hard poll ceilings. Prepared-macOS Metal phases remain open. |
