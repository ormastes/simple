...
|---:|---|---|---|---|---|
| 1 | McClintock | Linux CUDA/Vulkan parity: `test/03_system/app/simple_2d/native_processing_ir_cuda_vulkan_readback_parity_spec.spl` | Real device receipts, exact pixels/checksums, zero mismatches, no CPU fallback. | Linux CUDA/Vulkan GPU | PARTIAL: 2026-07-27 live generated CUDA C/PTX fill/copy/alpha/scroll device readback passes exact checksums with two positive identities. The ProcessingIR CUDA probe reaches device readback with positive provenance and no CPU fallback but still returns 64 values tagged x8. Retained source-matched Vulkan reaches strict creation; the later probe instead crashes during phase parsing because generated `str_starts_with` recursively calls itself. The fixed-token probe parser no longer uses that helper and awaits one incremental rebuild. Three bounded incremental compiler cycles end with the retained generation terminating while lowering current `run_compile_bootstrap`. Probe rebuild, compiler deployment, then transport/CUDA/Vulkan confirmation remain. |
