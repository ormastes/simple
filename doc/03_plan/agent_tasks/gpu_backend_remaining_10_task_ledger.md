...
| 3 | Confucius | Offload break-even: `test/03_system/app/simpleos_gpu_host/processing_ir_offload_break_even_spec.spl` | Native receipt records CPU/device/transfer/total/RSS medians and measured decisions. | Linux CUDA GPU | POLICY ACTIVE, NATIVE PARTIAL: measured CPU wins at 64/65,536 and CUDA wins at 1,048,576/8,388,608. The default threshold is 1,048,576 (`0` disables); source contract passes 8/8 and native 8-element bypass publishes exact reason-18 CPU fallback before submit. Threshold-0/exact-or-larger device completion remains blocked by task 6's request wait. |
