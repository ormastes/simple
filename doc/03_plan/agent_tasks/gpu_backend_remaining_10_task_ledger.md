...
| 6 | Kierkegaard | Injection matrix: `doc/03_plan/sys_test/gpu_backend_failure_injection_matrix.md` | Maps unavailable, submit, readback, mismatch, and fallback cases to hooks/evidence. | Linux for CUDA/Vulkan; prepared macOS for Metal | PARTIAL: all executors implement guarded unavailable/init/submit/readback/mismatch; explicit CPU fallback policy and guest wire validation are implemented; end-to-end daemon fallback, Vulkan live phases, and prepared-macOS Metal phases remain open |
