...
| 8 | Ramanujan | macOS failure/fallback: `test/03_system/check/macos_gpu_backend_failure_fallback_spec.spl` | Darwin harness must emit typed fail-closed Metal/Vulkan receipts. | Prepared macOS Metal/Vulkan host only | CONTRACT REPAIRED: backend-create failures now include canonical backend/stage/exit fields; Metal children are bounded to 30 s/4 MiB and only an actual runtime timeout receives the timeout marker. The final Linux contract rerun is deferred after the three-cycle cap; prepared-host receipts remain unrun. |
