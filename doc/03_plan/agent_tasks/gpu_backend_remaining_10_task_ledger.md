...
| 6 | Kierkegaard | Injection matrix: `doc/03_plan/sys_test/gpu_backend_failure_injection_matrix.md` | Maps unavailable, submit, readback, mismatch, and fallback cases to hooks/evidence. | Linux for CUDA/Vulkan; prepared macOS for Metal | PARTIAL: writable mmap ABI smoke passes, current-source Vulkan default plus all five isolated fault phases pass, and a same-process submit failure recovers to 64 exact values with unchanged identity. Deterministic Linux CUDA submit fallback completes with exact reason `16` under separate HELLO/request deadlines and hard poll ceilings. Same-process CUDA recovery after synchronized readback failure and prepared-macOS Metal phases remain open. |
