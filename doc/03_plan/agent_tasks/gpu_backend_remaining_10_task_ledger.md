...
| 6 | Kierkegaard | Injection matrix: `doc/03_plan/sys_test/gpu_backend_failure_injection_matrix.md` | Maps unavailable, submit, readback, mismatch, and fallback cases to hooks/evidence. | Linux for CUDA/Vulkan; prepared macOS for Metal | PARTIAL: writable mmap ABI smoke passes and source-matched native Vulkan fault phases pass. Two intermediate wait variants produced the exact CUDA fallback receipt, but the final bounded cycle exhausted HELLO before admission; wait edits were withdrawn under the three-cycle cap. Deterministic Linux daemon-wire completion and prepared-macOS Metal phases remain open. |
