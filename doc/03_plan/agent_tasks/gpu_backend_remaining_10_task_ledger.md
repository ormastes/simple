...
|---:|---|---|---|---|---|
| 1 | McClintock | Linux CUDA/Vulkan parity: `test/03_system/app/simple_2d/native_processing_ir_cuda_vulkan_readback_parity_spec.spl` | Real device receipts, exact pixels/checksums, zero mismatches, no CPU fallback. | Linux CUDA/Vulkan GPU | PARTIAL: generated CUDA C/PTX device readback passes. The new direct CUDA probe fixes native PTX/name ABI transport and reaches device readback; direct CUDA and Vulkan both report 64 mismatches and an aggregate checksum multiplied by 8. CUDA native identity is also negative despite its runtime contract. Resolve `processing_ir_cuda_vulkan_fill64_parity_2026-07-26.md` before publishing parity |
