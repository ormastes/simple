...
| Vulkan | Real device is present. The SFFI owner routes interpreter arrays through typed upload/SPIR-V/push/readback externs and preserves raw core-C native ABI; the unsafe generic TLS shim remains rejected. Interpreter and native Rust runtime device tests pass nonzero-offset exact readback plus OOB rejection. A source-matched Simple native probe now passes eight exact values with positive handle/identity and all five isolated fault phases. | SOURCE-MATCHED SIMPLE NATIVE DEVICE + FAULT PASS |

## Ten Tasks

| ID | Agent | Task and file | Acceptance evidence | Host | Status |
|---:|---|---|---|---|---|
| 1 | McClintock | Linux CUDA/Vulkan parity: `test/03_system/app/simple_2d/native_processing_ir_cuda_vulkan_readback_parity_spec.spl` | Real device receipts, exact pixels/checksums, zero mismatches, no CPU fallback. | Linux CUDA/Vulkan GPU | PARTIAL: generated CUDA C/PTX device readback passes. A host-only native transport probe reproduces the 8x symptom: generic iteration untags correctly while direct indexing loads tagged array storage. CUDA identity tag-width overflow is fixed and proven by a positive native Simple identity plus two distinct live generated-C device identities. Current `bootstrap_main` closure compiles but cannot link because admitted runtime bundles omit compiler-host/Cranelift symbols; three bounded cycles are exhausted. An admitted compiler-host runtime route, then transport/CUDA/Vulkan confirmation, remains. |
...
| 3 | Confucius | Offload break-even: `test/03_system/app/simpleos_gpu_host/processing_ir_offload_break_even_spec.spl` | Native receipt records CPU/device/transfer/total/RSS medians and measured decisions. | Linux CUDA GPU | MEASURED, POLICY OPEN: current-emitter PTX provenance and exact device readback pass; CPU wins at 64/65,536 elements and CUDA wins at 1,048,576/8,388,608. The daemon still executes both paths; a calibrated pre-device policy passed 7/7 source checks but was withdrawn because the incrementally rebuilt native daemon exits 139 before CLI validation. |
...
| 6 | Kierkegaard | Injection matrix: `doc/03_plan/sys_test/gpu_backend_failure_injection_matrix.md` | Maps unavailable, submit, readback, mismatch, and fallback cases to hooks/evidence. | Linux for CUDA/Vulkan; prepared macOS for Metal | BLOCKED: CUDA and source-matched native Vulkan fault phases pass; explicit CPU fallback, guest validation, and the daemon-wire harness are implemented. The retained probe reaches HELLO but its poll-only request wait expires before fallback; recompiling the shared bridge reintroduces the compiler-gated `file_mmap`/MMIO ABI SIGSEGV. Prepared-macOS Metal phases remain open. |
...
| 8 | Ramanujan | macOS failure/fallback: `test/03_system/check/macos_gpu_backend_failure_fallback_spec.spl` | Darwin harness must emit typed fail-closed Metal/Vulkan receipts. | Prepared macOS Metal/Vulkan host only | HOST-INDEPENDENT PASS: Metal invalid-IR/identity checks pass 2/2 and shared fault/cleanup ownership checks pass 6/6 on Linux. PREPARED-HOST UNRUN: live Metal/Vulkan failure receipts still require macOS. |
...
runtime upload/readback checks pass. The source-matched Simple native probe now
