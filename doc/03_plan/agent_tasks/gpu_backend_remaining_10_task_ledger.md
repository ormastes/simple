...
| 6 | Kierkegaard | Injection matrix: `doc/03_plan/sys_test/gpu_backend_failure_injection_matrix.md` | Maps unavailable, submit, readback, mismatch, and fallback cases to hooks/evidence. | Linux for CUDA/Vulkan; prepared macOS for Metal | PARTIAL: all fault seams, explicit CPU fallback, guest validation, and the native daemon-wire harness are implemented; daemon/probe binaries compile, but live fallback is blocked by native probe SIGSEGV 139; Vulkan and prepared-macOS phases remain open |
