...
| 6 | Kierkegaard | Injection matrix: `doc/03_plan/sys_test/gpu_backend_failure_injection_matrix.md` | Maps unavailable, submit, readback, mismatch, and fallback cases to hooks/evidence. | Linux for CUDA/Vulkan; prepared macOS for Metal | PARTIAL: all executors implement guarded unavailable/init/submit/readback/mismatch; CUDA real dispatch/readback reasons and host wire codes are split; Vulkan live phases await the updated runtime artifact; Metal live phases await prepared macOS; explicit fallback remains open |
