...
| 6 | Kierkegaard | Injection matrix: `doc/03_plan/sys_test/gpu_backend_failure_injection_matrix.md` | Maps unavailable, submit, readback, mismatch, and fallback cases to hooks/evidence. | Linux for CUDA/Vulkan; prepared macOS for Metal | PARTIAL: CUDA and source-matched native Vulkan fault phases pass; explicit CPU fallback, guest validation, and the daemon-wire harness are implemented; current-runtime HELLO passes, while fallback completion needs one fresh run after correcting daemon/probe timeout ordering; prepared-macOS Metal phases remain open |
