...
| storage-fs | Partial | Missing | foundational | Only persistent FS is bare FAT32 (no journal/permissions/cache/LFN-write); `nvfs` is in-memory only; `BlockCache` is dead code; `fsync` no-op; VFS routes to `mounts[0]` only |
