...
58. Ratchet the live call-authority census alongside declarations. Current
    baseline: 21,371 raw calls, 12,903 production calls, 1,191 explicitly
    authorized, and 20,180 missing authority after the Torch backend slice. Use the per-symbol/family/scope
    reports to prioritize real production exposure, and keep report generation
    to one source scan plus linear aggregations.
59. Migrate the remaining 694 production `rt_torch` calls through minimal lexical FFI
...
60. Keep `scripts/audit/baselines/sffi-call-authority-v1.tsv` monotonic. Missing
    authority may decrease but must not rise above 20,180. Update the baseline
