...
| WP-20 access analysis | partial implementation | compiler MIR analysis now preserves constant partition ranges through record loads into field paths and derives conservative address-observation/unknown-access summaries; the production typed-storage producer fails closed on those facts; focused suites pass 6/6 and 5/5; typed completeness/advisory hashing, planner input, ABI aggregation, PGO, and authoritative CFG/noalias evidence remain |
...
| WP-22 host AoS/SoA lowering | partial implementation | compiler-private MIR allocation owner/fact, declaration conversion, canonical producer, and pre-optimization address rewrite are landed; CompileContext atomically installs rewritten MIR plus aligned full producer evidence, freezes a registration-order-independent identity before native cache scope creation, denies post-freeze mutation, and preserves frozen authority through MIR eviction; the owner→producer→install→freeze→native compile pilot passes 3/3; module-local frozen worker snapshots and observable receipts remain, as do public typed allocation, subword/other backends, and fresh non-stub W^X execution |
