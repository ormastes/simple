...
| WP-23 AoSoA/SIMD lowering | partial implementation | admitted full blocks emit typed MIR through OpenCL and, with an explicit 32-byte projection proof, aligned native x86 AVX2 f32x8 load/add/sub/mul/div/store; x86 selection, low-eight YMM assignment, scalar pointer allocation, SIB/high-base encoding, and exact bytes are covered; executable-host, CPU-feature receipt, liveness reuse/spills, partial-vector, and scalable routes remain |
