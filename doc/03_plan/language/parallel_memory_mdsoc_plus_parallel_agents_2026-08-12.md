...
| WP-22 host AoS/SoA lowering | partial implementation | compiler-session ownership stores module-qualified sites; a compiler-private MIR allocation owner atomically emits the exact raw allocation operation and immutable fact with stable identity/provenance/source revision, which MIR-opt converts to a declaration; the canonical producer cross-checks that owner operation and constant bounds before rewriting `GEP/record-load/GetField`, consumes it into `rt_alloc`, and preserves value loads/evidence; backend site rewriting precedes optimization; focused owner/declaration/producer/native suites pass; driver full-evidence registration/freeze and cache receipt identity, a real compiler pilot caller, public typed allocation, subword/other backends, and fresh non-stub W^X execution remain |
