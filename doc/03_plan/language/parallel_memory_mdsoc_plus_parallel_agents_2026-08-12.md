...
| WP-18 thread pool | in progress | internal `rt_pool_state_*_v1` groundwork bounds accepted unreleased scalar tasks, uses tagged generation handles with lifetime pins, rejects stale/wrong-kind handles, and reclaims task state on release; the public Simple facade, alternate-provider execution, legacy generic globals, cancellation, blocking admission, and heap transfer remain |
