...
| WP-22 host AoS/SoA lowering | partial implementation | compiler-private MIR allocation owner/fact, declaration conversion, canonical producer, and pre-optimization address rewrite are landed; CompileContext atomically installs rewritten MIR plus aligned full producer evidence, freezes a registration-order-independent identity before native cache scope creation, denies post-freeze mutation, and preserves frozen authority through MIR eviction; focused owner/declaration/producer/native/registry suites pass; a real compiler pilot must invoke owner→producer→install before freeze, module-local frozen worker snapshots and observable receipts remain, as do public typed allocation, subword/other backends, and fresh non-stub W^X execution |
