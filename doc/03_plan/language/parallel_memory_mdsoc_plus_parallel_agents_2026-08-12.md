...
| WP-23 AoSoA/SIMD lowering | partial implementation | admitted full blocks emit typed MIR SIMD load/binop/store through OpenCL; x86 now has canonical SIMD IDs, distinct AVX2 machine opcodes, and a fail-closed low-eight XMM/YMM allocator with no spill support, but MIR-to-native selection/encoding is not connected; partial blocks remain scalar and scalable routes remain deferred |
