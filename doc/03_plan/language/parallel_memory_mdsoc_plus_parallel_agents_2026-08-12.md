...
| WP-22 host AoS/SoA lowering | partial implementation | compiler-private MIR allocation owner/fact, declaration conversion, canonical producer, and pre-optimization address rewrite are landed; CompileContext atomically installs rewritten MIR plus aligned full producer evidence, deep-copies and validates module-qualified frozen rows with length-prefixed identity before native cache lookup, tracks zero-site modules across eviction, and denies post-freeze mutation; storage-bearing modules compile on the owner thread until an immutable MIR+storage worker capsule lands; focused registry evidence passes 11/11; observable receipts, public typed allocation, subword/other backends, and fresh non-stub W^X execution remain |
