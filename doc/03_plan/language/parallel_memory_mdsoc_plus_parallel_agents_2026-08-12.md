...
| WP-22 host AoS/SoA lowering | partial implementation | compiler-session ownership stores immutable module-qualified site bindings and cache identity; a compiler-private declaration admits exact raw allocations only; an automatic canonical-MIR producer requires an owned-raw marker and independently proven constant bounds before atomically rewriting `GEP/record-load/GetField` into logical address projection plus value load, retaining provenance/revision evidence; backend site rewriting occurs before generic optimization; focused producer and adjacent native projection suites pass; driver evidence registration, compiler emission of the allocation marker/declaration, public typed allocation, provenance receipts, subword/other backends, and fresh non-stub W^X execution remain |
