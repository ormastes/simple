...
| WP-23 AoSoA/SIMD lowering | partial implementation | admitted full blocks emit typed MIR through OpenCL and aligned native x86 AVX2 f32x8; native selection now requires a versioned target-capability receipt, and a compiled W^X executable-memory system spec runs emitted load/add/store bytes on a CPUID/XGETBV-admitted host and checks eight exact f32 results; liveness reuse/spills, production driver receipt propagation, partial-vector, and scalable routes remain |
