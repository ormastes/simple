...
| WP-22 host AoS/SoA lowering | partial implementation | compiler-session ownership stores immutable module-qualified site bindings, evicts them with MIR, hashes complete sorted semantics into native cache scope, and rewrites atomically after optimization; only x86_64 custom native and pointer-width fields are admitted; the loader's unsupported raw `*u8` dereference/double-copy path is replaced by canonical exact-width `rt_ptr_read_u8` across C, Rust runtime, interpreter, codegen, and symbol registry; a rebuilt runtime is still required to re-admit the W^X scenario; automatic `T[]` producer registration, typed subword backend operations, receipts, other backends, and admitted execution remain |
