...
| WP-22 host AoS/SoA lowering | partial implementation | a checked fixed-record byte oracle converts AoS, SoA, and tail-padded AoSoA; exact function/base-local sidecars rewrite logical field projections to overflow-checked AoS/SoA address recipes and reject missing, duplicate, dynamic-field, observed, or ABI-pinned inputs; custom x86 lowers the address intrinsic to multiply/add addressing; a W^X store/load parity scenario is present but blocked before execution by the deployed runner's existing `smf_mmap_native.ptr_read_u8` codegen failure; automatic `T[]` allocation/binding, driver registration, other host backends, and admitted native execution remain |
