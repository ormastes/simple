...
| WP-22 host AoS/SoA lowering | partial implementation | compiler-session ownership stores immutable module-qualified site bindings, evicts them with MIR, hashes complete sorted semantics into native cache scope, and rewrites atomically after optimization; a compiler-private declaration now admits only exact compiler-owned raw allocations with provenance, source revision, fixed schema/capacity, bounds evidence, and AoS/SoA plans while rejecting RuntimeValue/external storage; only x86_64 custom native and pointer-width fields are admitted; canonical exact-width `rt_ptr_read_u8` removes the mapped-byte loader blocker; automatic source-pattern production/registration, provenance in cache receipts, typed subword backend operations, other backends, and fresh non-stub W^X execution remain |
