...
| WP-23 AoSoA/SIMD lowering | partial implementation | admitted full blocks emit typed MIR through OpenCL and aligned native x86 AVX2 f32x8; native selection requires a versioned target-capability receipt, the pure-Simple driver now intersects its requested `SIMPLE_NATIVE_CPU` profile with the canonical host CPUID/XGETBV result, propagates that resolved context through the custom-native adapter, and keys its cache by the capability decision; cross-target inference remains denied; a compiled W^X executable-memory system spec checks eight exact f32 results; liveness reuse/spills, explicit cross-target receipts, partial-vector, scalable routes, and public custom-native CLI admission remain |
