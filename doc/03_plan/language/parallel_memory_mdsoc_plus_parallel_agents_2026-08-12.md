...
| WP-16 actor/channel migration | partial implementation | native safe paths carry route-validated inline packets, actor reply provenance is explicit, and heap actor context is rejected. Shared mailbox closure now prevents post-stop admission through copied handles; typed heap/owned payloads, native lifecycle execution, and public API migration remain |
| WP-17 process transport | partial implementation | common/native bounded encoded-copy frames share a complete golden vector and a real exec-child round trip. `ParentCommitPipedResultReaderV1` now reassembles bounded `SPRF1` ASCII-armored stdout into the parent frame inbox, but child request protocol, session/replay binding, schema registry, ObjectRef, rollback, and self-hosted native process evidence remain |
