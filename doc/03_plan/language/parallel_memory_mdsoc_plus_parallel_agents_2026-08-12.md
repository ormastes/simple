...
| WP-22 host AoS/SoA lowering | partial implementation | compiler-session ownership now stores immutable module-qualified site bindings in aligned arrays, evicts them with MIR, hashes their complete sorted semantics into native cache scope, and rewrites logical projections atomically after generic optimization; only x86_64 custom native and pointer-width fields are admitted, while every missing/stale/unsupported case fails before backend dispatch; large SoA offsets materialize an i64 register instead of truncating through `ADD_IMM`; the W^X scenario remains blocked by existing `smf_mmap_native.ptr_read_u8` codegen; automatic `T[]` allocation/producer registration, typed subword memory operations, receipts, other backends, and admitted native execution remain |
