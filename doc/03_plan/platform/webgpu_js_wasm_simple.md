...
| REQ-WGPU-007 | A browser page can load WASM glue and then use JS WebGPU APIs in the same session. | `webgpu_js_wasm_simple_spec.spl`, `browser_session_wasm_fetch_bridge_spec.spl`, and `browser_session_fetch_wasm_chain_spec.spl` cover `fetch` -> `arrayBuffer` -> `WebAssembly.instantiate`, deterministic module/instance metadata, WebGPU global metadata reached from that chain, `requestAdapter()` metadata in the same session, nested returned-Promise assimilation for WebGPU callbacks, `adapter.requestDevice` shape, fail-closed unsupported imports, and direct declared WASM-to-JS host import callbacks for adapter, dispatch-shaped, and buffer-upload-shaped bindings. Dispatch import coverage includes both `i32` return and WebGPU-like void return shapes with WASM-pushed `x/y/z` workgroup dimensions; writeBuffer coverage includes WASM `store8`, `store16`, and `store32` payload bytes, pointer/length handoff, a Simple2D rectangle payload `8,12,40,24` plus RGBA `51,102,153,255`, and host callback reads through `new Uint8Array(i.exports.memory.buffer)`. BrowserSession also has a bounded software `device.queue.writeBuffer` recorder, but WASM-memory-backed queue upload remains tracked as an SSpec harness issue until executable evidence is stable. The fetch bridge spec is the fast focused gate; the broad fetch-chain spec remains the wider WASM conformance lane. | Remaining bridge work is a full WebGPU binding surface and stable SSpec queue-upload evidence, not browser-side asset loading/instantiation, nested promise assimilation, or bounded host import invocation. |
