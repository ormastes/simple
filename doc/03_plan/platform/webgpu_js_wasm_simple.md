...
- Current WebGPU executor is deterministic software simulation, not a CTS-compliant renderer. `browser_webgpu_chrome_draw_evidence_spec.spl` and `browser_webgpu_chrome_compute_evidence_spec.spl` are the canonical Chrome/Electron probes for real browser WebGPU draw/compute evidence and must report non-success status when the local host cannot expose non-fallback WebGPU or cannot capture/read back expected data. The compute probe now accepts compiler-emitted WGSL source and entry metadata instead of relying only on helper-hardcoded WGSL.
