...
- M26 WGSL reflection has started: shader sources with `@group`/`@binding` declarations now expose stage-visible resource metadata for uniform buffers, textures, and samplers, covered by `test/web_platform/webgpu/webgpu_context_spec.spl`.
...
- M26 automatic render pipeline layout derivation has started: reflected WGSL resource bindings can now create deterministic bind group layouts, merge vertex/fragment visibility for shared bindings, and reject sparse groups or incompatible same-binding resource types, covered by `test/web_platform/webgpu/webgpu_context_spec.spl`.
