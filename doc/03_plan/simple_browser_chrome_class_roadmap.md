...
- M27 WebGL backend command coverage has started: `enable()`/`disable()`, `scissor()`, `activeTexture()`, `lineWidth()`, `depthFunc()`, `cullFace()`, `frontFace()`, `blendColor()`, `blendFuncSeparate()`, `blendEquationSeparate()`, `polygonOffset()`, `sampleCoverage()`, clear-depth/stencil, depth-range, color/depth masks, and stencil masks now record render commands after state validation so capability toggles, backend clipping, texture-unit replay, blend-state replay, and draw-state replay can follow browser WebGL state changes.
