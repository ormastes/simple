...
- M27 WebGL backend command coverage has started: `enable()`/`disable()`, `scissor()`, `activeTexture()`, `lineWidth()`, `depthFunc()`, `cullFace()`, and `frontFace()` now record render commands after state validation so capability toggles, backend clipping, texture-unit replay, and draw-state replay can follow browser WebGL state changes.
