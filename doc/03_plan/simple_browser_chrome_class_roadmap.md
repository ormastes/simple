...
- M26 WebGPU blend constant command recording has started: render pass encoders now record browser-style `setBlendConstant` color state, replay blend constants through the deterministic executor, and lower script calls, covered by `test/web_platform/webgpu/webgpu_commands_spec.spl`, `test/web_platform/webgpu/webgpu_software_executor_spec.spl`, and `test/unit/browser/script/js_transpiler_spec.spl`.
- M26 WebGPU depth/stencil format validation has started: `depth24plus-stencil8` texture handles are accepted for render attachments, plain depth textures reject stencil ops/read-only state, and color attachments reject all depth-stencil formats, covered by `test/web_platform/webgpu/webgpu_resources_spec.spl` and `test/web_platform/webgpu/webgpu_commands_spec.spl`.
