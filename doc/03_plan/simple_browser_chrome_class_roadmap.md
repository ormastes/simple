...
- M27 WebGL backend command coverage has started: `scissor()`, `activeTexture()`, `lineWidth()`, `depthFunc()`, `cullFace()`, and `frontFace()` now record render commands after state validation so backend clipping, texture-unit replay, and draw-state replay can follow browser WebGL state changes.
