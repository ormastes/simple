...
- Browser WebGPU deterministic software executor now replays queue writes, render passes, compute dispatches, transfer copies, and invalid sequencing into testable counters in `src/lib/gc_async_mut/gpu/browser_engine/webgpu_software_executor.spl`, covered by `test/web_platform/webgpu/webgpu_software_executor_spec.spl`.
