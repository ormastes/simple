...
- M27 WebGL backend command coverage has started: `enable()`/`disable()`, `scissor()`, `activeTexture()`, texture parameter/upload/update/mipmap operations, `lineWidth()`, `depthFunc()`, `cullFace()`, `frontFace()`, `blendColor()`, `blendFuncSeparate()`, `blendEquationSeparate()`, `polygonOffset()`, `sampleCoverage()`, clear-depth/stencil, depth-range, color/depth masks, stencil masks, and stencil test ops now record render commands after state validation so capability toggles, backend clipping, texture-unit replay, texture replay, blend-state replay, stencil replay, and draw-state replay can follow browser WebGL state changes.
