...
- M27 WebGL backend command coverage has started: `enable()`/`disable()`, `scissor()`, `activeTexture()`, `pixelStorei()`, uniform setters, buffer data and vertex-attribute setup, texture parameter/upload/update/mipmap operations, framebuffer/renderbuffer setup, `lineWidth()`, `depthFunc()`, `cullFace()`, `frontFace()`, `blendColor()`, `blendFuncSeparate()`, `blendEquationSeparate()`, `polygonOffset()`, `sampleCoverage()`, clear-depth/stencil, depth-range, color/depth masks, stencil masks, and stencil test ops now record render commands after state validation so capability toggles, backend clipping, geometry setup, texture-unit replay, texture/framebuffer replay, blend-state replay, stencil replay, uniform replay, and draw-state replay can follow browser WebGL state changes.
