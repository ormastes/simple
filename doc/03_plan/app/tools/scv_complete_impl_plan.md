...
| SCV-IMPL-D-08 | Merge corpus measuring **missed real conflicts** (not only spurious ones), incl. rename/edit through preprocessor-heavy sources | D-06 | `sh scripts/check/check-scv-merge-corpus.shs` → `PASS — 0 missed real conflicts…` | L | scv | done (2026-08-27) — corpus + gate landed (28 cases: 13 conflict-truth, 15 clean-truth, 6 preprocessor). Gate GREEN: `PASS — 28 case(s) checked, 0 missed real conflicts (0 undetected conflict, 0 silent mis-merge), 2 spurious (bound <=6)`. The 3 misses it originally reported (`22_cpp_ifdef_condition_vs_body`, `24_cpp_ifdef_else_split`, `26_cpp_rename_edit_preprocessor`) were FIXED in the merger, not baselined: `src/lib/scv/preprocessor_regions.spl` gives the merge path a conditional-region model and `merge.spl` refuses those merges with a new `preprocessor_region_divergent` conflict_v2 kind. See `doc/08_tracking/bug/scv_merge_silently_merges_across_divergent_preprocessor_branches_2026-08-26.md`. The 2 spurious commutative-list conflicts (`13`, `14`) remain, within the declared bound, tracked for D-05 follow-up. |
