...
1. `self` mutation loss regression coverage started.
   - Added focused assertions to
     `test/01_unit/compiler/interpreter/self_field_assign_spec.spl` for
...
5. Driver framework FR-DRIVER-0006 completed.
   ...
   - Verification:
     `SIMPLE_LIB=src bin/simple check src/lib/nogc_sync_mut/fs_driver/driver_adapter.spl test/01_unit/lib/driver/fat32_driver_adapter_test.spl`
     passed.
   - Verification:
     `SIMPLE_LIB=src bin/simple test test/01_unit/lib/driver/fat32_driver_adapter_test.spl --mode=interpreter --clean --format json`
     passed `3/3` in 432 ms.
   - Updated `doc/08_tracking/feature/driver_framework_requests.md`
...
8. Driver framework FR-DRIVER-0003 focused verification passed.
   ...
   - `SIMPLE_LIB=src bin/simple test test/01_unit/lib/driver/null_block_driver_test.spl --mode=interpreter --clean --format json`
     passed in 82 ms.
9. Driver framework remaining tracker checkbox resolved.
   ...
   - Original residual: `test/01_unit/lib/driver/fat32_file_io_spec.spl` reported
     `9 passed / 5 failed`; documented as FAT32 queue work rather than an open
     driver-framework porting checkbox.
   - `doc/08_tracking/feature/driver_framework_requests.md` now has no
...
15. SimpleOS in-guest toolchain kernel-side spawn preparation improved.
    - Focused checks passed:
      `SIMPLE_LIB=src bin/simple check src/os/kernel/loader/fs_exec_spawn.spl src/os/kernel/loader/x86_64_fs_exec_spawn.spl test/01_unit/os/kernel/loader/x86_64_fs_exec_spawn_spec.spl`.
    - Focused tests passed:
      `test/01_unit/os/kernel/loader/x86_64_fs_exec_spawn_spec.spl` `2/2` and
      `test/03_system/os/port/x86_64_elf_load_spec.spl` `1/1`.
...
16. FAT32 file I/O residual fixed.
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/lib/nogc_async_mut/fs_driver/fat32_core.spl src/lib/nogc_sync_mut/fs_driver/fat32_core.spl test/01_unit/lib/driver/fat32_file_io_spec.spl`
      and
      `SIMPLE_LIB=src bin/simple test test/01_unit/lib/driver/fat32_file_io_spec.spl --mode=interpreter --clean`
      (`14/14`).
17. JIT text extern / `.to_text()` segfault fixed.
    ...
    - Verification passed:
      ...
      `SIMPLE_LIB=src bin/simple test test/01_unit/app/simpleos_nvme_serial_check_spec.spl --mode=interpreter --clean`,
      and
...
18. Pure Simple collection parity/performance refreshed.
    ...
    - Verification passed:
      `SIMPLE_COLLECTION_BENCH_SAMPLES=1 SIMPLE_COLLECTION_BENCH_ENFORCE=0 bash test/05_perf/collections/run_collection_benchmarks.shs`
...
20. Pure Simple ctype performance rechecked; no pure-Simple repair remains.
    ...
    - Command:
      `SIMPLE_CTYPE_BENCH_SAMPLES=1 SIMPLE_CTYPE_BENCH_ENFORCE=0 bash test/05_perf/ctype/run_ctype_benchmarks.shs`.
    - Queue decision: keep
...
25. SimpleOS FR-SOS-025 x86_32 installed early syscall ABI live proof completed.
    ...
    - Narrowed `test/03_system/simpleos_x86_32_boot_probe_live_spec.spl` to the
    ...
    - Updated the x86_32 interrupt unit expectation so the hosted early bridge
      ...
      syscall ids.
    - Verification passed:
      `SIMPLE_LIB=src bin/simple test test/01_unit/os/kernel/ipc/syscall_reboot_spec.spl --mode=interpreter --clean`
      (`1/1`),
      `SIMPLE_LIB=src bin/simple test test/01_unit/os/kernel/arch/x86_32_interrupt_spec.spl --mode=interpreter --clean`
      (`5/5`),
      `SIMPLE_LIB=src bin/simple test test/01_unit/os/kernel/arch/x86_32_trap_model_spec.spl --mode=interpreter --clean`
...
32. Pure Simple ctype native performance partially mitigated; remains open.
    ...
    - Verification passed:
      ...
      `SIMPLE_LIB=src bin/simple check test/05_perf/ctype/bench_ctype.spl src/lib/common/ctype.spl`,
      and `git diff --check`.
    - Enforced perf check still fails the 0.50x native/C floor:
      `SIMPLE_CTYPE_BENCH_ENFORCE=1 SIMPLE_CTYPE_BENCH_CLEAN=1 SIMPLE_BIN=src/compiler_rust/target/debug/simple SIMPLE_LIB=src bash test/05_perf/ctype/run_ctype_benchmarks.shs`.
...
38. STUN SOFTWARE attribute bug completed.
    ...
    - Updated RFC 5769 KAT expectations in
      `test/01_unit/os/proxy/stun_spec.spl` so SOFTWARE no longer parses as
      `Unknown(0x8022)`.
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/os/proxy/stun.spl test/01_unit/os/proxy/stun_spec.spl`,
      `SIMPLE_LIB=src bin/simple test test/01_unit/os/proxy/stun_spec.spl --mode=interpreter`
...
39. Compiler loader shared-core stop condition completed.
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/compiler/99.loader/module_loader.spl src/compiler/99.loader/jit_instantiator.spl test/01_unit/compiler/loader/module_loader_spec.spl test/01_unit/compiler/interpreter/self_field_assign_spec.spl`,
      `SIMPLE_LIB=src bin/simple test test/01_unit/compiler/loader/module_loader_spec.spl --mode=interpreter --clean`
      (`22/22`),
...
44. Editor numbered controller split consolidation completed.
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/app/editor/editor_controller.spl src/app/editor/editor_ctrl_core.spl src/app/editor/editor_ctrl_lsp.spl src/app/editor/tui_main.spl src/app/editor/tui_shell.spl test/03_system/editor_palette_spec.spl`
      and
      `SIMPLE_LIB=src bin/simple test test/03_system/editor_palette_spec.spl --mode=interpreter --clean`
      (`11/11`).
    - Broader follow-up noted: `test/03_system/editor_controller_spec.spl`,
...
48. Editor core-C startup segfault fixed.
    ...
    - Added `rt_file_exists(root)` guards in both extension-host mirrors before
      calling `rt_dir_walk`.
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/lib/editor/extensions/host.spl src/lib/editor/extensions/host.spl src/app/editor/editor_controller.spl test/01_unit/lib/editor/extension_discovery_contract_spec.spl test/03_system/editor_extension_spec.spl`.
...
51. Editor controller system regression batch completed.
    ...
    - Verification passed:
      ...
      and
      `SIMPLE_LIB=src bin/simple test test/03_system/editor_controller_spec.spl --mode=interpreter --clean --format json`
      (`92/92`).
52. FR-COMPILER-005 lightweight outline/LSP domain-block surfacing completed.
    ...
    - Added focused fixtures/specs for LSP document-symbol metadata and outline
      metadata.
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/app/cli/query_visibility_part1.spl src/app/cli/query_visibility_part2.spl src/app/cli/query_outline.spl src/app/cli/query.spl test/01_unit/app/cli/query_visibility_domain_blocks_spec.spl test/01_unit/app/cli/query_outline_domain_blocks_spec.spl test/fixtures/query/domain_blocks.spl`
      and
      `SIMPLE_LIB=src bin/simple test test/01_unit/app/cli/query_visibility_domain_blocks_spec.spl --mode=interpreter --clean --format json`
      (`1/1`),
      `SIMPLE_LIB=src bin/simple test test/01_unit/app/cli/query_outline_domain_blocks_spec.spl --mode=interpreter --clean --format json`
      (`1/1`).
53. FR-SIMPLE-DB-0001 prefix-index core completed; cross-consumer integration remains.
    ...
    - Tracker status is partial, not fully implemented: DBFS namespace scans
      ...
      feature request.
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/lib/nogc_sync_mut/db/prefix_index.spl src/lib/nogc_sync_mut/db/db_persistence.spl src/lib/nogc_sync_mut/db/db_query.spl test/01_unit/lib/db/prefix_index_spec.spl`
      and
      `SIMPLE_LIB=src bin/simple test test/01_unit/lib/db/prefix_index_spec.spl --mode=interpreter --clean --format json`
      (`4/4`).
54. FR-SIMPLE-DB-0001 DBFS namespace integration completed.
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/lib/nogc_sync_mut/db/prefix_index.spl src/lib/nogc_sync_mut/db/dbfs_engine/schema.spl test/01_unit/lib/db/dbfs_dentry_prefix_index_spec.spl test/02_integration/storage/dbfs/dbfs_catalog_schema_spec.spl`
      and
      `SIMPLE_LIB=src bin/simple test test/01_unit/lib/db/dbfs_dentry_prefix_index_spec.spl --mode=interpreter --clean --format json`
      (`3/3`),
      `SIMPLE_LIB=src bin/simple test test/02_integration/storage/dbfs/dbfs_catalog_schema_spec.spl --mode=interpreter --clean --format json`
      (`15/15`).
55. FR-SIMPLE-DB-0001 SDN query-path prefix-index reuse completed.
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/lib/nogc_sync_mut/database/query.spl test/01_unit/lib/database/database_query_spec.spl`
      and
      `SIMPLE_LIB=src bin/simple test test/01_unit/lib/database/database_query_spec.spl --mode=interpreter --clean --format json`
      (`5/5`),
      `SIMPLE_LIB=src bin/simple test test/03_system/database/database_system_spec.spl --mode=interpreter --clean --format json`
      (`4/4`).
...
57. FR-SIMPLE-DB-0001 SDN repeated-prefix cache completed.
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/lib/nogc_sync_mut/database/query.spl src/lib/nogc_async_mut/database/query.spl test/01_unit/lib/database/database_query_spec.spl`
      and
      `SIMPLE_LIB=src bin/simple test test/01_unit/lib/database/database_query_spec.spl --mode=interpreter --clean --format json`
      (`6/6`),
      `SIMPLE_LIB=src bin/simple test test/03_system/database/database_system_spec.spl --mode=interpreter --clean --format json`
...
58. FR-SIMPLE-DB-0001 representative latency/RSS evidence completed.
    - Extended `test/05_perf/bench/simple_db_shared_accel.spl` with SDN repeated
    ...
    - Fresh interpreter evidence:
      ...
      elapsed `0:51.50`.
    - `doc/08_tracking/feature/simple_db_requests.md` now marks
      FR-SIMPLE-DB-0001 implemented.
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check test/05_perf/bench/simple_db_shared_accel.spl`,
      `SIMPLE_LIB=src bin/simple run test/05_perf/bench/simple_db_shared_accel.spl`,
      and
      `/usr/bin/time -f 'max_rss_kb=%M elapsed=%E' sh -c 'SIMPLE_LIB=src bin/simple run test/05_perf/bench/simple_db_shared_accel.spl >/tmp/simple_db_shared_accel.out'`.
59. FR-COMPILER-005 lightweight opening-range surfaces completed.
    ...
    - `doc/08_tracking/feature/compiler_requests.md` now marks
      FR-COMPILER-005 implemented.
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/app/cli/query_visibility_part1.spl src/app/cli/query_visibility_part2.spl test/01_unit/app/cli/query_visibility_domain_blocks_spec.spl test/fixtures/query/domain_blocks.spl`,
      `SIMPLE_LIB=src bin/simple test test/01_unit/app/cli/query_visibility_domain_blocks_spec.spl --mode=interpreter --clean --format json`
      (`1/1`),
      `SIMPLE_LIB=src bin/simple check src/app/cli/query_outline.spl test/01_unit/app/cli/query_outline_domain_blocks_spec.spl`,
      and
      `SIMPLE_LIB=src bin/simple test test/01_unit/app/cli/query_outline_domain_blocks_spec.spl --mode=interpreter --clean --format json`
      (`1/1`).
60. FR-SIMPLE-DB-0002 learned static-segment index completed.
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/lib/nogc_sync_mut/db/learned_index.spl test/01_unit/lib/db/learned_index_spec.spl test/05_perf/bench/simple_db_shared_accel.spl`,
      `SIMPLE_LIB=src bin/simple test test/01_unit/lib/db/learned_index_spec.spl --mode=interpreter --clean --format json`
      (`4/4`), and
      `SIMPLE_LIB=src bin/simple run test/05_perf/bench/simple_db_shared_accel.spl`.
61. FR-SIMPLE-DB-0003 learned cardinality-estimation experiment completed.
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/lib/nogc_sync_mut/db/cardinality_estimator.spl test/01_unit/lib/db/cardinality_estimator_spec.spl src/lib/nogc_sync_mut/db/__init__.spl`
      and
      `SIMPLE_LIB=src bin/simple test test/01_unit/lib/db/cardinality_estimator_spec.spl --mode=interpreter --clean --format json`
      (`4/4`).
62. FR-SIMPLE-DB-0004 posting-list bitmap execution completed.
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/lib/nogc_sync_mut/db/accel.spl test/01_unit/lib/db/posting_list_bitmap_spec.spl test/05_perf/bench/simple_db_shared_accel.spl`,
      `SIMPLE_LIB=src bin/simple test test/01_unit/lib/db/posting_list_bitmap_spec.spl --mode=interpreter --clean --format json`
      (`4/4`),
      `SIMPLE_LIB=src bin/simple run test/05_perf/bench/simple_db_shared_accel.spl`,
      and
      `/usr/bin/time -f 'max_rss_kb=%M elapsed=%E' sh -c 'SIMPLE_LIB=src bin/simple run test/05_perf/bench/simple_db_shared_accel.spl >/tmp/simple_db_shared_accel_fr0004.out'`.
63. FR-SIMPLE-DB-0005 WCO join research completed.
    - Added `doc/01_research/lib/simple_db/simple_db_wco_joins_2026-05-29.md` comparing
...
64. FR-COMPILER-006 schema contract model completed.
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/compiler/10.frontend/domain/schema_contract.spl test/01_unit/compiler/schema_contract_spec.spl`,
      `SIMPLE_LIB=src bin/simple test test/01_unit/compiler/schema_contract_spec.spl --mode=interpreter --clean --format json`
      (`5/5`),
...
65. FR-COMPILER-007 style/ui typed authoring surface completed.
    ...
    - Added `test/01_unit/common/ui/style_ui_authoring_spec.spl` covering typed
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/lib/common/ui/style_ui_authoring.spl src/lib/common/ui/simple_theme.spl test/01_unit/common/ui/style_ui_authoring_spec.spl`
      and
      `SIMPLE_LIB=src bin/simple test test/01_unit/common/ui/style_ui_authoring_spec.spl --mode=interpreter --clean --format json`
      (`5/5`).
66. FR-COMPILER-008 music IR and MusicXML subset interchange completed.
    ...
    - Added `test/01_unit/common/music_spec.spl` covering typed IR lowering,
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/lib/common/music.spl test/01_unit/common/music_spec.spl`
      and
      `SIMPLE_LIB=src bin/simple test test/01_unit/common/music_spec.spl --mode=interpreter --clean --format json`
      (`4/4`).
67. FR-COMPILER-009 BIM/city fixture-backed standards bindings completed.
    ...
    - Added `test/01_unit/common/bim_city_spec.spl` covering BIM bindings,
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/lib/common/bim_city.spl test/01_unit/common/bim_city_spec.spl`
      and
      `SIMPLE_LIB=src bin/simple test test/01_unit/common/bim_city_spec.spl --mode=interpreter --clean --format json`
      (`5/5`).
68. FR-COMPILER-010 CAD authoring and STEP AP242 fixture export completed.
    ...
    - Added `test/01_unit/common/cad_spec.spl` covering part authoring, assembly
    ...
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/lib/common/cad.spl test/01_unit/common/cad_spec.spl`
      and
      `SIMPLE_LIB=src bin/simple test test/01_unit/common/cad_spec.spl --mode=interpreter --clean --format json`
      (`4/4`).
69. FR-PLUG-0001 WFFI f64 calling-convention extern completed.
    ...
    - Updated `doc/04_architecture/platform/sffi_bidirectional_interop.md §9` and
      `doc/08_tracking/feature/plugin_surface_followups.md`.
    - Verification passed:
      `sh test/03_system/feature/plugin/fixtures/build_fixtures.shs`,
      ...
      `SIMPLE_LIB=src/compiler_rust/lib/std/src:src src/compiler_rust/target/debug/simple check src/compiler_rust/lib/std/src/plugin/__init__.spl test/03_system/feature/plugin/runtime_api_plugin_spec.spl test/03_system/feature/plugin/sugar_plugin_spec.spl`,
      and
      `SIMPLE_LIB=src/compiler_rust/lib/std/src:src src/compiler_rust/target/debug/simple test test/03_system/feature/plugin/runtime_api_plugin_spec.spl --mode=interpreter --clean --format json`
      (`8/8`).
...
71. C-runtime string-index and async-driver exclusion completed.
    ...
    - Worker follow-up noted that
      `SIMPLE_LIB=src bin/simple test test/02_integration/compiler/llvm_native_link_spec.spl --mode=interpreter --fail-fast`
...
73. HTTPS/TLS record AES-GCM provenance switched to pure Simple.
    ...
    - Fixed reserved worker record helpers to pass wire text to
      ...
      (`plaintext_len + 16`).
    - Added `test/01_unit/lib/io/tls_common_hooks_aes_gcm_spec.spl`.
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check src/lib/nogc_sync_mut/io/tls_common_hooks.spl src/lib/nogc_async_mut/io/tls_common.spl src/lib/nogc_async_mut/io/tls.spl src/lib/nogc_async_mut/http_server/worker.spl test/01_unit/lib/io/tls_common_hooks_aes_gcm_spec.spl`,
      `SIMPLE_LIB=src bin/simple test test/01_unit/lib/io/tls_common_hooks_aes_gcm_spec.spl --mode=interpreter --clean --format json`,
      and
      `SIMPLE_LIB=src bin/simple test test/01_unit/lib/crypto/aes128_gcm_nist_vectors_spec.spl --mode=interpreter --clean --format json`.
74. TLS live SHA/HMAC bisection refreshed; fix not complete.
    - Added `test/01_unit/os/crypto/sha256_pure_simple_spec.spl` for OS
...
76. TLS live x86_64 QEMU lane completed.
    ...
    - Added native regression coverage in
      `test/01_unit/compiler/unsigned_to_i64_spec.spl`.
    - Verification passed:
      `SIMPLE_LIB=src bin/simple test test/01_unit/compiler/unsigned_to_i64_spec.spl --mode=native --clean --force-rebuild`
      and
      `SIMPLE_ALLOW_FREESTANDING_STUBS=1 SIMPLEOS_QEMU_TLS_LIVE=1 bin/simple test test/03_system/os_tls_spec.spl --system --sequential --fail-fast`
      (`2/2`).
77. Native enum destructuring and direct `[u8]` equality follow-ups completed.
    ...
    - Added `test/01_unit/compiler/native_enum_if_val_probe.spl` and
      `test/01_unit/compiler/native_enum_u8_regression_spec.spl`.
    - Verification passed:
      `SIMPLE_LIB=src bin/simple check test/01_unit/compiler/native_enum_if_val_probe.spl test/01_unit/compiler/native_enum_u8_regression_spec.spl`,
      `src/compiler_rust/target/debug/simple compile test/01_unit/compiler/native_enum_if_val_probe.spl --native --backend=cranelift -o /tmp/native_enum_if_val_probe && /tmp/native_enum_if_val_probe`,
      and
      `SIMPLE_LIB=src bin/simple test test/01_unit/compiler/native_enum_u8_regression_spec.spl --mode=native --clean --force-rebuild`.
...
79. PERF-SUGAR-001 typed numeric allocation completed.
    ...
    - Added focused coverage in
      `test/01_unit/lib/science_math/typed_alloc_linalg_spec.spl` and corrected
      stale f32/i32 extern signatures in `test/05_perf/typed_array_alloc_spec.spl`.
    - Verification passed with rebuilt `src/compiler_rust/target/debug/simple`:
      `SIMPLE_LIB=src src/compiler_rust/target/debug/simple check src/lib/common/science_math/perf_sugar.spl src/lib/common/science_math/linalg.spl test/01_unit/lib/science_math/typed_alloc_linalg_spec.spl test/05_perf/typed_array_alloc_spec.spl`,
      `cargo check -p simple-compiler --manifest-path src/compiler_rust/Cargo.toml`,
      `SIMPLE_LIB=src src/compiler_rust/target/debug/simple test test/01_unit/lib/science_math/typed_alloc_linalg_spec.spl --mode=interpreter --clean --fail-fast`,
      `SIMPLE_LIB=src src/compiler_rust/target/debug/simple test test/05_perf/typed_array_alloc_spec.spl --mode=interpreter --clean --fail-fast`,
      and
      `SIMPLE_LIB=src src/compiler_rust/target/debug/simple test test/03_system/feature/scilib/perf_sugar_spec.spl --mode=interpreter --clean --fail-fast`.
...
81. FAT32 4K overwrite baseline gap closed with external VFAT blocker.
    ...
    - Reverified the direct-C gate:
      `SIMPLE_LIB=src bin/simple check test/05_perf/bench/fat32_4k_compare.spl`
      passed and
...
82. FR-PLUG-0004 backend fallback improved; true fusion remains open.
    ...
    - Verification passed:
      `SIMPLE_LIB=/tmp/simple-next-main/src bin/simple check src/compiler/70.backend/backend/cranelift_codegen_adapter.spl test/03_system/feature/plugin/sugar_plugin_spec.spl`
      and
      `SIMPLE_LIB=/tmp/simple-next-main/src bin/simple test test/03_system/feature/plugin/sugar_plugin_spec.spl --mode=interpreter --clean --fail-fast`
      (`14/14`).
...
84. FR-DRIVER-0001 function-level live path bridged.
    ...
    - Added live system proof and SPipe mirror:
      `test/03_system/compiler/driver_synthetic_registration_live_spec.spl` and
      `test/03_system/app/compiler/feature/driver_synthetic_registration_live_spec.spl`.
