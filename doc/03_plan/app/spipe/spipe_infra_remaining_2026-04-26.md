...
| 1.4 Build `test/01_unit/app/itf/adapter_outlook_curl_spec.spl` | (same agent) | argv shape, OAuth2 token-cache mocked roundtrip, list_messages URL builder, error mapping |
| 1.5 Build `test/03_system/adapter_outlook_curl_live_spec.spl` (skip-no-creds) | (same agent) | reads `GRAPH_TENANT_ID`, `GRAPH_CLIENT_ID`, `GRAPH_CLIENT_SECRET`, `GRAPH_MAILBOX_UPN`; smoke `list_folders`, `list_messages`, `get_message` |
...
| 2.2 Run existing read live spec | claude | `bin/simple test test/03_system/adapter_jira_curl_live_spec.spl` |
| 2.3 Build `test/03_system/adapter_jira_curl_writes_live_spec.spl` (gated `JIRA_ALLOW_WRITES=1`) | claude | covers `create_issue`, `get_attachment`; creates a throwaway issue in user's project |
...
| 3.3 Run existing read live spec | claude | `bin/simple test test/03_system/adapter_bitbucket_curl_live_spec.spl` |
| 3.4 Build `test/03_system/adapter_bitbucket_curl_writes_live_spec.spl` (gated `BB_ALLOW_WRITES=1`) | claude | covers `create_pr`, `post_pr_comment` (general + inline), `approve`, `unapprove`, `merge` (with `--dry-run` first option) |
