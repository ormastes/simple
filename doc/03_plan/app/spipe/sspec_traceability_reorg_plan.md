...
- Added local research for current `test/` folder organization and outliers:
  `doc/01_research/app/testing/misc/test_folder_organization.md`.
...
- Cleared root-level executable `.spl` tests:
  - removed duplicate `test/serpent_check_spec.spl` because
    `test/01_unit/os/crypto/serpent_kat_spec.spl` is identical,
  - moved `test/test_while_basic_spec.spl` to
    `test/01_unit/compiler/control_flow/while_basic_spec.spl`,
  - added mirrored doc
...
- Cleared legacy `test/specs/` by classifying the extracted language specs as
  `test/03_system/feature/language/`, moving their `summary.txt` evidence directories
...
- Moved the root syntax generated snapshot to
  ...
  `test/03_system/feature/usage/syntax_spec.spl`.
- Moved the web-dashboard app spec bucket from `test/app/web_dashboard/` to
  `test/03_system/feature/app/web_dashboard/`, moved its existing root generated docs to
  mirrored `doc/06_spec/feature/app/web_dashboard/` paths, and added scenario
  docs for the remaining moved web-dashboard specs.
- Moved the tmux stdlib unit spec from `test/01_unit/lib/std/tmux/` to
  `test/01_unit/lib/std/tmux/`, and moved its generated doc to
...
- Replaced the stale root tensor-dimensions generated snapshot with executable
  SPipe traceability coverage at `test/03_system/feature/ml/tensor_dimensions_spec.spl`
...
- Moved the remaining legacy stdlib unit spec bucket from `test/lib/std/unit/`
  to `test/01_unit/lib/std/`, preserving existing summary evidence directories.
- Cleared the remaining executable `test/lib/std/` tree:
  - `features/placeholder_lambda` moved to `test/03_system/feature/language/`,
  - `integration/*` moved to `test/02_integration/lib/std/`,
  - `spec/decorators` moved to `test/01_unit/lib/std/spec/`,
  - `type_checker/*` moved to `test/01_unit/lib/std/type_checker/`.
  Existing summary evidence directories were moved and their embedded spec paths
  updated.
- Removed the top-level `test/04_smoke/` alias by moving
  `compile_smoke_spec.spl` to `test/03_system/smoke/` and deleting its obsolete
...
- Removed top-level `test/scratch/` and `test/spike/`. Parser-safe generic
  ...
  `test/03_system/feature/language/generic_repro_spec.spl`; the parser-failing generic
...
- Removed the `test/sys/` alias by moving wm_compare system specs and goldens
  to `test/03_system/wm_compare/`, and SimpleOS driver/display specs to
  `test/03_system/simpleos/`. Added mirrored `doc/06_spec/system/...` docs for the
...
- Removed top-level `test/data/` by moving the agent roundtrip support fixture
  ...
  `test/02_integration/app/io_intensive_spec.spl` to the fixture namespace.
...
- Moved the non-shared contract testing spec out of `test/shared/` to
  `test/01_unit/lib/common/contracts/contract_testing_spec.spl`, preserved its
...
- Moved the top-level `test/kernel` boot/loader specs to
  `test/03_system/kernel`. All four moved kernel specs pass focused verification
  at their new paths.
...
- Moved top-level NVFS host-side storage specs to
  `test/02_integration/storage/nvfs`. The moved directory passes focused
  verification.
- Moved top-level tools specs into canonical `test/01_unit/tools` and
  `test/03_system/tools` buckets, with CI helpers and saved summaries preserved
  under `test/fixtures/tools`.
- Moved top-level DBFS storage specs to `test/02_integration/storage/dbfs` and
  support harnesses to `test/fixtures/storage/dbfs`.
- Moved top-level browser/rendering roots into canonical buckets:
  `test/browser_engine` to `test/01_unit/browser_engine`,
  `test/web_platform` to `test/03_system/feature/web_platform`, and `test/reftest` to
  `test/03_system/reftest`.
- Moved QEMU and FPGA hardware-gated roots into canonical system buckets:
  `test/qemu` to `test/03_system/qemu` and `test/riscv64_fpga` to
  `test/03_system/hardware/riscv64_fpga`.
- Moved top-level app specs into canonical `test/01_unit/app` and
  `test/02_integration/app` buckets, preserving summary evidence.
- Moved top-level OS specs into canonical `test/01_unit/os`,
  `test/02_integration/os`, `test/03_system/os`, and `test/fixtures/os` buckets.
- Moved the remaining top-level lib specs into canonical `test/01_unit/lib` and
  `test/fixtures/lib` buckets.

## Restart Checkpoint

- Current safe stopping point: `test/shared/` is canonical and runner-visible.
- Do not move `test/shared/` into `test/01_unit/shared/`; it is a documented
...
3. Reorganize remaining `test/` outliers after identifying source-area ownership:
   ...
   resolved under `test/03_system/kernel`; `test/fuzz` and `test/js` are resolved
   under feature/fixture buckets; `test/nvfs` is resolved under
   `test/02_integration/storage/nvfs`; `test/tools` is resolved under
...
- Focused unit: `SIMPLE_LIB=src bin/simple test --force-rebuild test/01_unit/app/tooling/spipe_docgen_scenario_body_spec.spl`
- Focused unit: `SIMPLE_LIB=src bin/simple test --force-rebuild test/01_unit/app/tooling/traceability_spec.spl`
- Moved stdlib feature: `SIMPLE_LIB=src bin/simple test --force-rebuild test/03_system/feature/language/placeholder_lambda_spec.spl`
- Moved stdlib unit: `SIMPLE_LIB=src bin/simple test --force-rebuild test/01_unit/lib/std/spec/decorators_spec.spl`
- Moved stdlib unit: `SIMPLE_LIB=src bin/simple test --force-rebuild test/01_unit/lib/std/type_checker/type_inference_spec.spl`
- Moved stdlib unit: `SIMPLE_LIB=src bin/simple test --force-rebuild test/01_unit/lib/std/type_checker/type_inference_v2_spec.spl`
- Moved stdlib integration: `SIMPLE_LIB=src bin/simple test --force-rebuild test/02_integration/lib/std/diagram/diagram_integration_spec.spl`
- Moved stdlib integration: `SIMPLE_LIB=src bin/simple test --force-rebuild test/02_integration/lib/std/doctest/discovery_spec.spl`
- Moved stdlib integration: `SIMPLE_LIB=src bin/simple test --force-rebuild test/02_integration/lib/std/failsafe/crash_prevention_spec.spl`
- Moved stdlib integration: `SIMPLE_LIB=src bin/simple test --force-rebuild test/02_integration/lib/std/improvements/stdlib_improvements_spec.spl`
- Moved stdlib integration: `SIMPLE_LIB=src bin/simple test --force-rebuild test/02_integration/lib/std/ml/simple_math_integration_spec.spl`
- Moved stdlib integration: `SIMPLE_LIB=src bin/simple test --force-rebuild test/02_integration/lib/std/screenshot/screenshot_ffi_spec.spl`
...
- System alias cleanup: `SIMPLE_LIB=src bin/simple test --force-rebuild test/03_system/wm_compare/emulated_capture_spec.spl`
- System alias cleanup: `SIMPLE_LIB=src bin/simple test --force-rebuild test/03_system/wm_compare/famous_site_engine2d_backend_spec.spl`
- Known FAIL after matcher/import cleanup: `test/03_system/wm_compare/backend_parity_spec.spl`, `golden_gate_spec.spl`, `browser_shell_parity_spec.spl`, `electron_reference_parity_spec.spl`.
- Data fixture cleanup: `SIMPLE_LIB=src bin/simple test --force-rebuild test/02_integration/app/io_intensive_spec.spl`
- Shared runner unit coverage: `SIMPLE_LIB=src bin/simple test --force-rebuild test/01_unit/app/test_runner/execution_strategy_reclassify_spec.spl`
- Shared runner unit coverage: `SIMPLE_LIB=src bin/simple test --force-rebuild test/01_unit/app/test_runner_new/test_categorization_spec.spl`
...
- Moved contract unit coverage: `SIMPLE_LIB=src bin/simple test --force-rebuild test/01_unit/lib/common/contracts/contract_testing_spec.spl`
...
- Low-risk unit move: `test/01_unit/hal/hal_traits_spec.spl` passed 30/30 in the
  worker. `test/01_unit/rtl/encode_riscv_spec.spl`,
  `test/01_unit/sffi/sffi_public_api_spec.spl`,
  `test/01_unit/runtime/simd_text/simd_text_test.spl`, and
  `test/01_unit/runtime/simd_text/simd_text_fuzz_test.spl` were discovered at the
  new paths but failed focused interpreter verification.
- Compiler slice move: `test/01_unit/compiler/semantics/auto_vec_string_test.spl`
  and the moved MIR optimization specs under `test/01_unit/compiler/mir_opt/`
  passed focused interpreter verification in the worker. The moved
  `test/01_unit/compiler/vhdl/hardware_spawn_lower_spec.spl` is discovered but
  fails 17 examples.
- Kernel system move:
  `SIMPLE_LIB=src bin/simple test --force-rebuild test/03_system/kernel/boot_fs_mount_spec.spl`
  passed 8/8;
  `SIMPLE_LIB=src bin/simple test --force-rebuild test/03_system/kernel/boot_fs_spec.spl`
  passed 11/11;
  `SIMPLE_LIB=src bin/simple test --force-rebuild test/03_system/kernel/elf_load_chain_spec.spl`
  passed 6/6;
  `SIMPLE_LIB=src bin/simple test --force-rebuild test/03_system/kernel/nvfs_elf_load_spec.spl`
  passed 3/3.
- Fuzz feature move:
...
- NVFS storage integration move:
  `SIMPLE_LIB=src bin/simple test --force-rebuild test/02_integration/storage/nvfs`
...
- DBFS storage integration move: representative moved specs passed:
  ...
  `test/02_integration/storage/dbfs/dbfs_engine_wal_spec.spl` 8/8. Existing
...
- Browser/web/reftest move: discovery passed for
  `test/01_unit/browser_engine` with 252 listed tests,
  `test/03_system/feature/web_platform` with 398 listed tests, and
  `test/03_system/reftest` with 35 listed tests.
...
- QEMU/FPGA system move:
  ...
  passed 9/9, `test/03_system/qemu/qmp_screendump_spec.spl` passed 5/5,
  `test/01_unit/qemu/qemu_harness_acquire_or_spawn_spec.spl` passed 8/8, and
  `test/03_system/hardware/riscv64_fpga` passed 91/91.
- App move: discovery passed for `test/01_unit/app/itf` with 219 listed tests,
  `test/01_unit/app/svim` with 72 listed tests, `test/02_integration/app/ui.web`
  with 158 listed tests, and `test/02_integration/app/diagnostics` with 13 listed
  tests. Focused checks passed for `test/01_unit/app/itf/itf_flags_spec.spl`
  10/10 and `test/02_integration/app/ui.web/random_hex_test.spl` 8/8. Existing
  behavioral failures remain in `test/01_unit/app/svim/core_spec.spl` and
...
- OS move: focused checks passed for
  ...
  `test/02_integration/os/port/disk_image_bake_spec.spl` 8/8. Discovery passed
  for `test/01_unit/os/kernel` with 1136 listed tests,
  `test/02_integration/os/port` with 73 listed tests, and `test/03_system/os/port`
  ...
  moved `test/01_unit/os/kernel/logging/marker_wire_format_spec.spl` exists but
  runner discovery reported 0 files.
- Lib move: discovery passed for `test/01_unit/lib/blink` with 182 listed tests
  and `test/01_unit/lib/transitive` with 1 listed test.
  ...
  `.spipe_matchers_*` files under `test/01_unit/lib/nogc_async_mut/...` are
