...
- `.claude/skills/lib/spipe_phases.md` + `spipe_notebook.md` (no `.claude/skills/spipe/` dir exists; the spipe skill lives in `skills/lib/`) / `.claude/templates/spipe_template.spl` — document
...
- [x] spipe skill lib + template document the manual-first authoring convention
      (design-the-manual-first phase text, `step("...")` calls, `"""` docstrings,
      `@manual`/`@manual-file` folding tags) — verified `.claude/skills/lib/spipe_phases.md:100`
      `design the manual first`, `.claude/skills/lib/spipe_notebook.md:32` `manual-first scenarios`,
      `.claude/templates/spipe_template.spl:86` `step("Verify the documented outcome")`
  - divergence: planned `@manual_section` + `@req` examples in the template; shipped
    `@manual`/`@manual-file` fold tags + `step()` only — no `@manual_section`/`@req` line
    in `.claude/templates/spipe_template.spl` (`/usr/bin/grep -n "@manual_section\|@req"` → 0 hits).

## Acceptance

Runnable oracles for the remaining open boxes: `test/03_system/plan_acceptance/sspec_modernization_plan_spec.spl`
