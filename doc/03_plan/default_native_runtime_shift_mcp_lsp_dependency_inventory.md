...
- `scripts/check/check-simple-core-runtime-smoke.shs` now provides a repeatable explicit `simple-core` gate for hello, the generated standalone TUI wrapper, and the real `run_tui("examples/ui/minimal.ui.sdn")` app path, with closure checks for hosted-runtime and unwind markers.
