...
- `failed=0; total=0; for f in test/system/editor_*_spec.spl test/system/dap_protocol_live_spec.spl; do SIMPLE_LIB=src bin/simple test "$f" --mode=interpreter --fail-fast; done`
  - Result: PASS, 35 files run, 0 failures.
- `SIMPLE_LIB=src bin/simple test test/system/dap_protocol_live_spec.spl --mode=interpreter --fail-fast`
