...
|---|---|---|---|
| Electron, Tauri, and pure Simple web renderer share the same web render API interface | A single Simple API/trait/module imported by all three adapters, with tests proving the same request/response contract for HTML/snapshot/patch/input/capabilities/host-window operations and optional pixel output | `src/lib/common/ui/web_render_api.spl` exists and `ui.web`, `ui.electron`, and `ui.tauri` import it. `test/unit/app/ui/web_render_backend_api_spec.spl` covers those backends plus pure Simple browser artifact participation and now proves Electron/Tauri standalone and async render IPC are built from common `WebRenderRequest` artifacts. `src/app/ui.ipc/async_handler.spl` preserves complete common render IPC JSON and wraps legacy raw HTML only for compatibility. Host window command JSON delegates to `WebRenderHostWindowCommand`; snapshot, patch, and input wire payloads are represented by `WebRenderSnapshotEnvelope`, `WebRenderPatchEnvelope`, and `WebRenderInputEnvelope`, covered by `test/unit/lib/common/ui/web_render_api_spec.spl`. Web, Electron, and Tauri expose adapter helpers for common snapshot/patch/input envelopes; the live Web runtime now sends snapshot and patch WebSocket payloads through the common envelopes, and the browser boundary unwraps them for the retained renderer. The legacy WebSocket render push in `src/app/ui.web/server.spl` now derives its body from `WebRenderRequest`/`web_render_body_html` and carries the `simple_web` target while preserving browser-compatible `type:"render"` frames. Pure Simple browser event queueing records the common input envelope. The surface registry now has a Tauri kind and focused Tauri binding coverage. Electron/Tauri live snapshot/patch transports are not yet exclusively routed through the new envelopes. | Partially complete |
...
| Host WM and SimpleOS WM share logic | Host and SimpleOS import/use the same WM service/core for create/focus/move/resize/minimize/restore/close/input routing; no local host service shim duplicates WM authority | `host_compositor_entry.spl` imports the real `os.services.wm.wm_service.WmService` for lifecycle state and translates host bridge requests into shared `WmAction` values before applying compositor mutations. Pointer-driven drag/resize also route through `WmAction` lifecycle helpers. `DesktopShell.apply_wm_action` now routes create, create-web, destroy, focus, resize, move, set-title, minimize, maximize, restore, and update-tree compositor mutation through `apply_wm_action_to_compositor(...)` while keeping shell-specific owner registration, launcher accounting, replies, and logs local. Shared update-tree materialization now lives in `wm_action_remote_tree(...)` instead of the desktop shell. Host still has a local `HostCompositor` adapter for host-native surfaces/input policy. | Partially complete |
...
| Pure Simple GUI/web-render path is memory and size optimized against an equivalent Qt GUI app | A measured Simple artifact and a measured equivalent Qt artifact, with the Simple path at or below Qt size and no avoidable allocation regressions in the pure Simple renderer | `src/app/ui.browser/backend.spl` uses pre-sized framebuffer/pixel arrays. `scripts/check-qt-gui-size-baseline.shs` records the smallest measured Simple Web artifact from `doc/09_report/web_baremetal_size_audit_2026-05-28.md`; current value is 14,336 bytes for `Simple web placeholder URL facade`. The script now also records Qt discovery state for pkg-config, qmake, cmake, and c++. Qt development files are missing on this host, so no Qt artifact exists and equal-or-better remains unproven. | Partially complete |
...
3. Implementation evidence:
   ...
   - broaden CUDA 2D hardware coverage beyond the current clear, filled-rect, and outline readback self-test
   ...
   - Electron/Tauri live snapshot and patch transport paths fully migrated to common envelopes, not only render IPC, async render IPC, and adapter helpers
