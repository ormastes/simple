...
| C3 A9 rung 4, full kernel | **blocked upstream** | The full Limine kernel (`limine_entry.spl`) fails to link with **ld.lld too**. With `--gc-sections`, 3 symbols are undefined: `rt_arm64_mrs_mpidr_el1`, `rt_value_u64`, and `mmio_disable_test_mode`, which merge `e274cd33719` dropped. Without GC, 23 are undefined. On the same 17 objects, `elf_boot_link` returns `Err` with the identical 23-symbol set. It hits no unsupported relocation or script construct, so there is no linker gap. No ELF, no sha256 and no gate verdict. Details: `doc/08_tracking/bug/boot_layout_a9_board_blocked_2026-09-19.md` § "Rung 4 with the full kernel" |
