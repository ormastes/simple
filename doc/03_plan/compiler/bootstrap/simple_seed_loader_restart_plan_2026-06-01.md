...
- 2026-07-04 focused restart evidence:
  ...
  - 2026-07-25 parser split evidence: splitting `parse_global_flags` into core/backend/limit helper groups moved `bin/simple check src/app/cli/_CliMain/args_and_os_commands.spl` from timeout to file-level `OK` in 14s. The command still exits 1 in the repo hygiene gate with `simple: seed sibling not found, skipping delegation: /usr/bin/simple_seed`; fix that delegation/hygiene path before treating the focused check as fully green.
  - 2026-07-25 delegation source fix: `src/app/io/cli_ops.spl` now falls back to repo-local `bin/simple_seed` when the current executable resolves outside the repo and lacks a sibling seed. `test/01_unit/app/io/cli_argv0_resolution_spec.spl` records this as a source regression. The deployed binary still has the old path, so the regression cannot execute until the pure-Simple CLI is rebuilt/redeployed.
  - 2026-07-25 clean redeploy probe: in `/home/ormastes/dev/pub/simple-redeploy-clean` at `debc189...`, the cache-backed full CLI native-build probe timed out at 240s with zero log lines and zero cached objects. The isolated `bootstrap_main.spl` mini shard timed out at 180s with zero log lines and zero cached objects. The active blocker has moved past parser/delegation source fixes back to pre-object native-build progress; continue from `doc/08_tracking/bug/bootstrap_main_entry_closure_no_object_progress_2026-07-23.md`.
