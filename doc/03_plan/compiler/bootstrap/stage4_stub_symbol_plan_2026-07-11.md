...
`walk_dir`, `glob_find`, `glob_matches`, `path_filename`, `path_extension`, and
`path_stem` now stay in pure Simple. They reuse the existing `rt_dir_walk`,
shared `std.glob` matcher, and `std.path` lexical helpers; no new runtime ABI
was added. The core-C `rt_dir_walk` owner now returns the canonical `rt_array_*` /
...
three path helpers and `file_metadata` (4 total); the three dead path declarations
...
`Path.stem`, filename, and extension coverage;
