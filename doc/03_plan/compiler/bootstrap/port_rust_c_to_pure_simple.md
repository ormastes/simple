...
## Status: Phases 1-5 DONE; Phase 6A/6B DONE; Phase 6C DONE; Phase 6D REMAINS
...
- A 2026-05-14 single-sample smoke using `SIMPLE_BIN=bin/release/x86_64-unknown-linux-gnu/simple SIMPLE_BENCH_SAMPLES=1 SIMPLE_LLVM_PROBE=0 SIMPLE_DISASM_PROBE=0 test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs` kept checksum parity but did not meet equal-or-better speed: XXHash64 `0.50x` C / `0.49x` Rust, CRC32 `0.89x` C / `1.29x` Rust, Adler-32 `0.78x` C / `0.74x` Rust, and ChaCha20 `0.70x` C / `0.56x` Rust. This confirms Phase 6D is still open in the current active release artifact.
- A later 2026-05-14 core-runtime single-sample smoke, using a temporary C runtime archive that includes `runtime_simd_utf8.c`, keeps checksum parity and proves the CRC cache no longer segfaults natively. It improves XXHash64 to `1.33x` C / `1.40x` Rust and CRC32 to `1.02x` C / `1.07x` Rust, while Adler-32 remains `0.70x` C / `0.75x` Rust and ChaCha20 remains `0.68x` C / `0.56x` Rust. Phase 6D is still open.
...
2. **Hotspot ASM diff baseline against Rust and C**
   - Captured C, Rust, Simple native, and Simple LLVM disassembly summaries from `SIMPLE_LLVM_PROBE=1 SIMPLE_DISASM_PROBE=1 test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`.
...
9. **Adler reduction-window deferral**
- `src/os/crypto/adler32.spl` now uses a 355328-byte reduction window for its `u64` accumulators instead of the classic 5552-byte window chosen for 32-bit accumulator safety.
- `test/01_unit/os/crypto/adler32_spec.spl` adds a 6000-byte KAT so the larger reduction window is covered across the previous boundary.
...
10. **General small-constant MIR multiply decomposition**
   ...
   - Stop condition met for this optimizer slice: `SIMPLE_LIB=src bin/release/x86_64-unknown-linux-gnu/simple check src/compiler/60.mir_opt/mir_opt/loop_strength.spl` passes, and `SIMPLE_LIB=src bin/release/x86_64-unknown-linux-gnu/simple test test/01_unit/compiler/mir_opt/strength_reduction_spec.spl --mode=interpreter --clean` passes 14 examples with 0 failures.
...
5. **SIMD import/runtime-symbol hygiene**
   ...
   - Current evidence: `bin/simple test test/01_unit/os/crypto/chacha20_simd_parity_spec.spl --mode=interpreter --no-cache` passes all 6 examples after replacing the OS scalar block with the unrolled RFC implementation and making the local i32 vector wrappers native-safe.
...
- `test/05_perf/run_comparison.shs` must continue showing self-hosted Simple no slower than the Rust bootstrap.
...
algorithms with C/Rust/Simple parity data in `test/05_perf/port_algorithms`.
...
|--------|-----------|--------|-----------|-----------------------|------------------|-------------|----------------|-----------|--------------|--------|--------|
| Hash | XXHash64 | 14653 median | 8578 median | 8339 median | not sampled | 0.57x median | 0.97x median | n/a | n/a | PASS | Below C/Rust in latest longer-window 3-sample median |
| Checksum | CRC32 | 457 median | 456 median | 403 median | not sampled | 0.88x median | 0.88x median | n/a | n/a | PASS | Below C/Rust; native global table cache and literal table attempts fail native validation |
| Checksum | Adler-32 | 2598 median | 2623 median | 2451 median | not sampled | 0.94x median | 0.93x median | n/a | n/a | PASS | Below C/Rust after longer-window median measurement |
| Cipher | ChaCha20 | 308 median | 347 median | 361 median | not sampled | 1.17x median | 1.04x median | n/a | n/a | PASS | Faster than both C and Rust in latest longer-window 3-sample median |

Current command:
- `SIMPLE_BIN=src/compiler_rust/target/release/simple SIMPLE_LLVM_PROBE=0 SIMPLE_DISASM_PROBE=0 test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
...
- The harness defaults `SIMPLE_NATIVE_RUNTIME_BUNDLE=auto` so non-compiler pure Simple benchmark entries prefer the core runtime lane when its ABI archive is complete; hosted Rust runtime must now be selected explicitly. It still fails on native compile failure unless `SIMPLE_ALLOW_INTERPRETER_FALLBACK=1` is explicitly set, so speed rows cannot silently come from the interpreter fallback.
- 2026-05-14 core-runtime smoke: building `runtime.c`, `runtime_native.c`, `runtime_simd_utf8.c`, `runtime_thread.c`, `runtime_fork.c`, `runtime_process.c`, and `runtime_memtrack.c` into a temporary `libsimple_runtime.a`, then running the harness with `SIMPLE_RUNTIME_PATH=<temp>` and `SIMPLE_NATIVE_RUNTIME_BUNDLE=auto`, links and runs the pure Simple benchmark without the Rust-hosted runtime. Checksum parity passes for XXHash64, CRC32, Adler-32, and ChaCha20. One-sample speed now beats both C and Rust for XXHash64 (`1.33x` C / `1.40x` Rust) and CRC32 (`1.02x` C / `1.07x` Rust), but Adler-32 (`0.70x` C / `0.75x` Rust) and ChaCha20 (`0.68x` C / `0.56x` Rust) remain below the equal-or-better target.
- 2026-05-14 follow-up: non-empty `[u8]` literals now lower through packed byte arrays (`rt_byte_array_new` plus `rt_typed_bytes_u8_push`) instead of generic boxed `ArrayLit`, closing the native literal-packing blocker recorded for direct byte table materialization. CRC32 and CRC32C now use lazy pure Simple table caches guarded by scalar integer globals so table generation is paid once per process without relying on native tagged-bool global initialization. `simple check src/os/crypto/crc32.spl`, the interpreter CRC KATs, and the core-runtime port benchmark pass.
...
|-------|---------------------|-------------------------|----------|------------|-------------|---------------|----------------|---------------------------|
| Measured non-crypto hash | XXHash64 | C/Rust/Simple parity benchmark PASS | 0.57x Cranelift median | 43% slower than C | 0.97x Cranelift median | 3% slower than Rust | MB/s one-shot hash | Keep byte-load, rotate, bounds-proof, and helper-inline regressions locked |
| Measured checksum | CRC32 | C/Rust/Simple parity benchmark PASS | 0.88x Cranelift median | 12% slower than C | 0.88x Cranelift median | 12% slower than Rust | MB/s checksum | Fix static-table/cache crash, native `[u8]` literal packing, generalize table loop, unchecked typed byte load, typed byte-array capacity, and byte-push inlining |
| Measured checksum | Adler-32 | C/Rust/Simple parity benchmark PASS | 0.94x Cranelift median | 6% slower than C | 0.93x Cranelift median | 7% slower than Rust | MB/s checksum | Finish compiler-driven weighted byte-reduction and small-multiply decomposition |
| Measured stream cipher | ChaCha20 | C/Rust/Simple parity benchmark PASS | 1.17x Cranelift median | 17% faster than C | 1.04x Cranelift median | 4% faster than Rust | MB/s encryption/checksum | Preserve rotate idiom, fixed-block byte stores, helper inlining, and SIMD-safe lowering |
...
|--------|-----------|----------|------------|-------------|---------------|--------|
| Non-crypto hash | XXHash64 | 0.57x | 43% slower | 0.97x | 3% slower | Measured PASS; below C/Rust |
| Checksum | CRC32 | 0.88x | 12% slower | 0.88x | 12% slower | Measured PASS; below C/Rust |
| Checksum | Adler-32 | 0.94x | 6% slower | 0.93x | 7% slower | Measured PASS; below C/Rust |
| Stream cipher | ChaCha20 | 1.17x | 17% faster | 1.04x | 4% faster | Measured PASS; faster than C/Rust |
...
- `bin/simple test test/01_unit/os/crypto/adler32_spec.spl --mode=interpreter --no-cache`
- `bin/simple check test/05_perf/port_algorithms/port_algorithms_simple.spl`
...
- `src/compiler_rust/target/release/simple test test/01_unit/os/crypto/adler32_spec.spl --mode=interpreter --no-cache`
- `SIMPLE_BIN=src/compiler_rust/target/release/simple SIMPLE_LLVM_PROBE=0 SIMPLE_DISASM_PROBE=1 test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
...
- `SIMPLE_RUNTIME_PATH=<temp-core-c-runtime-with-runtime_simd_utf8.c> SIMPLE_BIN=bin/release/x86_64-unknown-linux-gnu/simple SIMPLE_LLVM_PROBE=0 SIMPLE_DISASM_PROBE=1 SIMPLE_BENCH_SAMPLES=1 test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
...
- The remaining likely CRC32 gain is table lifetime: one-shot `crc32(data)` still builds the IEEE table for each call. A static or cached table should wait for reliable native top-level array initialization, correct native packed `[u8]` literal lowering, or an explicit runtime cache pattern, because direct cache and literal-table attempts both failed native validation.
...
- `cargo run --manifest-path src/compiler_rust/Cargo.toml -p simple-driver --bin simple -- test test/01_unit/os/crypto/crc32_kat_spec.spl --mode=interpreter --no-cache`
- `SIMPLE_LLVM_PROBE=1 SIMPLE_DISASM_PROBE=1 test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
...
Commands:
- `SIMPLE_LLVM_PROBE=0 test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
- `SIMPLE_LLVM_PROBE=1 SIMPLE_DISASM_PROBE=0 test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
...
- `SIMPLE_LLVM_PROBE=1 SIMPLE_DISASM_PROBE=1 test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
- `test/05_perf/run_comparison.shs`
...
Additional evidence:
- `test/05_perf/run_comparison.shs` remains green: Rust bootstrap wall clock `1390ms`, self-hosted Simple wall clock `730ms`; sum of average benchmark times `12121us` vs `6052us`, ratio `0.4x`.
...
Additional evidence:
- `test/05_perf/run_comparison.shs` remains green: Rust bootstrap wall clock `1475ms`, self-hosted Simple wall clock `796ms`; sum of average benchmark times `12872us` vs `6631us`, ratio `0.5x`.
...
- `cargo fmt --manifest-path src/compiler_rust/Cargo.toml --all`
- `cargo check --manifest-path src/compiler_rust/Cargo.toml -p simple-compiler --features llvm`
- `bin/simple check src/os/crypto/xxhash.spl test/05_perf/port_algorithms/port_algorithms_simple.spl`
- `cargo test --manifest-path src/compiler_rust/Cargo.toml -p simple-compiler codegen_inline_typed_bytes_u64_le_unchecked_does_not_emit_runtime_symbol -- --nocapture`
...
Commands:
- `bin/simple test test/01_unit/os/crypto/chacha20_simd_parity_spec.spl --mode=interpreter --no-cache`
...
- Existing compiler/runtime benchmark `test/05_perf/run_comparison.shs` remains green with self-hosted Simple faster than the Rust bootstrap: wall clock 798 ms vs 1446 ms, sum of average benchmark times 6657 us vs 12581 us, ratio 0.5x.
...
Commands:
- `cargo build --manifest-path src/compiler_rust/Cargo.toml -p simple-driver --release --features llvm --bin simple`
- `SIMPLE_BIN=src/compiler_rust/target/release/simple SIMPLE_LLVM_PROBE=1 SIMPLE_DISASM_PROBE=1 test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
...
- `bin/simple test test/01_unit/compiler/backend/llvm_opt_pipeline_spec.spl --mode=interpreter --no-cache` executed all 8 examples successfully but the file still failed at runner/semantic finalization with `function compiler_driver_create not found`; keep this as a test-runner validation gap, not a failing assertion in the new LLVM pipeline helpers.
...
- Verification passed for `src/compiler/60.mir_opt/mir_opt`, `src/compiler/70.backend/feature_caps.spl`, `src/lib/common/compress`, `src/lib/common/crypto/sha256_core.spl`, `src/lib/common/crypto/chacha20.spl`, `src/lib/common/crypto/poly1305.spl`, `src/lib/common/crypto/chacha20_poly1305.spl`, `test/01_unit/lib/crypto/chacha20_spec.spl --clean`, `test/01_unit/lib/crypto/chacha20_poly1305_spec.spl`, `test/01_unit/lib/crypto/chacha20_poly1305_wycheproof_spec.spl --clean`, `test/01_unit/lib/crypto/sha256_x4_spec.spl`, `test/01_unit/lib/common/compress/deflate_spec.spl`, and `test/01_unit/lib/simd/rotl_rotr_i32x4_spec.spl`.
- Existing compiler/runtime benchmark `test/05_perf/run_comparison.shs` passed with self-hosted Simple faster than the Rust bootstrap: wall clock 792 ms vs 1373 ms, sum of average benchmark times 6740 us vs 11971 us, ratio 0.5x.
...
- Algorithm-level C/Rust parity harness now exists at `test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs` and validates checksum parity across C, Rust, and Simple for dependency-free XXHash64 and ChaCha20.
...
- `test/05_perf/port_algorithms/port_algorithms_simple.spl` no longer carries unused `QRWords`, `quarter`, `push_word`, `load32`, or `rotl32` helpers in the benchmark hot code; ChaCha loads and rotates are direct expressions.
...
Notes:
- `test/05_perf/port_algorithms/port_algorithms_simple.spl` now mirrors the C/Rust harness more closely by reusing one ChaCha output buffer across benchmark iterations.
...
Commands:
- `src/compiler_rust/target/debug/simple check src/compiler/60.mir_opt/mir_opt/mod.spl src/compiler/60.mir_opt/mir_opt/inline_part2.spl test/05_perf/port_algorithms/port_algorithms_simple.spl src/os/crypto/xxhash.spl`
- `test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
...
Commands:
- `src/compiler_rust/target/debug/simple check src/compiler/60.mir_opt/mir_opt/inline_part1.spl src/compiler/60.mir_opt/mir_opt/inline_part2.spl src/compiler/60.mir_opt/mir_opt/mod.spl test/05_perf/port_algorithms/port_algorithms_simple.spl src/os/crypto/xxhash.spl`
- `bin/simple test test/01_unit/compiler/mir_opt/inlining_spec.spl --mode=interpreter --no-cache`
- `test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
...
- `src/compiler_rust/target/debug/simple check test/05_perf/port_algorithms/port_algorithms_simple.spl`
- `src/compiler_rust/target/debug/simple compile test/05_perf/port_algorithms/port_algorithms_simple.spl --native --cpu native --opt-level aggressive -o build/perf/port_algorithms/port_algorithms_simple_dbg && build/perf/port_algorithms/port_algorithms_simple_dbg`
...
- `SIMPLE_BIN=bin/simple test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
...
Commands:
- `bin/simple check src/compiler/70.backend/backend/llvm_passes.spl src/compiler/70.backend/backend/llvm_lib_backend.spl test/01_unit/compiler/backend/llvm_opt_pipeline_spec.spl test/05_perf/port_algorithms/port_algorithms_simple.spl`
- `bin/simple test test/01_unit/compiler/backend/llvm_opt_pipeline_spec.spl --mode=interpreter`
- `SIMPLE_LLVM_PROBE=1 test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
...
- `test/01_unit/compiler/backend/llvm_opt_pipeline_spec.spl` covers the default-pipeline mapping and verifies the explicit pass diagnostics remain available.
...
- `SIMPLE_BIN=src/compiler_rust/target/release/simple SIMPLE_LLVM_PROBE=0 test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
- `SIMPLE_BIN=src/compiler_rust/target/release-opt/simple SIMPLE_LLVM_PROBE=0 test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
- `SIMPLE_BIN=src/compiler_rust/target/bootstrap/simple SIMPLE_LLVM_PROBE=0 test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
...
- `SIMPLE_BIN=src/compiler_rust/target/release/simple test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
- `SIMPLE_LIB=src bin/simple test test/01_unit/lib/crypto/aes_gcm_rfc_vectors_spec.spl --mode=interpreter --clean`
...
- **Create:** `test/05_perf/port_algorithms/`
...
- Keep the existing `test/05_perf/run_comparison.shs` as the compiler/runtime regression guard.
...
- Compiler/runtime: self-hosted Simple remains no slower than Rust bootstrap on `test/05_perf/run_comparison.shs`; latest run is `730ms` Simple vs `1390ms` Rust bootstrap.
...
- Regression evidence: MIR lowering tests prove typed push calls replace `rt_array_push`, runtime tests cover grow semantics and masking, `cargo check` passes for `simple-runtime`, `simple-common`, and `simple-compiler`, and the cipher/compress gate reports `passed=13 skipped=0 failed=0`.
...
Latest 3-sample median for context: XXHash64 `0.57x` C / `0.97x` Rust, CRC32
`0.88x` C / `0.88x` Rust, Adler-32 `0.94x` C / `0.93x` Rust, and ChaCha20
`1.17x` C / `1.04x` Rust. The sample confirms that the measured optimization
gate remains open outside ChaCha20.
...
- Evidence: `cargo test -p simple-compiler single_file_runtime_bundle`, CRC32
  ...
  `test/05_perf/port_algorithms/port_algorithms_simple.spl`, and one-sample
...
| `test/05_perf/port_algorithms/` | 6D | Keep C/Rust/Simple checksum parity and throughput deltas as the acceptance gate |
