...
The backend target-context provider now scopes both normalized target reads
without caching mutable policy or changing read cardinality. The wiring spec
passes 5/5, optimizer findings remain 7, and isolated analyzer evidence is
...
- 263 symbols with source-signature-hash variants requiring resolved-type
  triage, not 263 presumed ABI conflicts;
...
1. Reconcile the 263 source-signature-hash variants against compiler-owned
   canonical type identity. Treat source-only differences as candidates, not
   proof; reject confirmed unresolved conflicts rather than choosing an arity
   or lowering unsupported values to zero.
