...
broader closure is not yet claimed complete. Windows still lacks
`rt_windows_build_command_line`, and the strict flow retains its fail-closed
projection barrier. The selected archives are not yet projected or linked;
executable proof remains pending.
