...
- Added focused coverage in
  `test/01_unit/lib/gpu/engine2d/backend_session_contract_spec.spl`.
- Added `CudaSession` contract helpers for backend kind, availability,
  ...
  covered by `test/01_unit/lib/gpu/engine2d/cuda_session_contract_spec.spl`.
...
1. Retrofit `CpuSimdSession` to implement `ComputeSession`
   ...
   - Covered by `test/01_unit/lib/gpu/engine2d/cpu_simd_session_contract_spec.spl`.
...
1. Create `RocmSession` implementing `ComputeSession`
   ...
   - Covered by `test/01_unit/lib/gpu/engine2d/rocm_session_contract_spec.spl`.
...
2. Create `OpenClSession` implementing `ComputeSession`
   ...
   - Covered by `test/01_unit/lib/gpu/engine2d/opencl_session_contract_spec.spl`
     and `test/01_unit/lib/gpu/engine2d/ffi_opencl_spec.spl`.
...
2. End-to-end benchmark: 1920×1080 fill+blit+scroll frame at 60fps target
   ...
   - In progress: benchmark evidence is recorded in
     ...
     Current runnable evidence covers the existing native/JIT smoke-mode scalar
     runner and report structure. Full 1920x1080 evidence still requires the C
     reference run, full-mode Simple runner, and real hardware-backed GPU rows
     where local probes initialize those backends.
3. Update `BackendProber.probe_all_summary` with runtime evidence from real session init
   ...
   - Covered by `test/01_unit/lib/gpu/engine2d/backend_probe_strict_spec.spl`.
