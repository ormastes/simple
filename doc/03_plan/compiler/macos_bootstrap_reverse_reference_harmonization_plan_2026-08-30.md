...
| **M2 — projection receipts** | **STRUCTURAL PASS; SPipe/NATIVE BLOCKED.** Ten typed registry families, lifecycle isolation, transitive/SCC closure, fail-closed fallback, and immutable receipts are present. The mutation matrix rejected 12/12 weakenings; no native incremental run is claimed. | `src/compiler/00.common/cache/reverse_reference_facts.spl`; `src/compiler/80.driver/cache/reverse_reference_receipt.spl`; `test/02_integration/compiler/cache/reverse_reference_projection_receipt_spec.spl`; `scripts/check/check-m3-phase2-phase3-mutations.shs`; `build/review/final_m2_m3_macos_audit_2026-09-02.md` |
| **M3 — Phase2→3 compatible reuse** | **STRUCTURAL PASS; SPipe/NATIVE BLOCKED.** Exact immutable M2 receipt digest, owner/root generations, key frame, consumer, canonical no-follow reads, and exclusive publication are enforced. No admitted native Phase2→3 reuse receipt or clean/reused output comparison exists. | `src/compiler/00.common/cache/phase_compatibility_manifest.spl`; `src/compiler/80.driver/cache/phase_compatibility_admission.spl`; `test/02_integration/compiler/cache/phase2_phase3_compatibility_manifest_spec.spl`; `doc/06_spec/03_system/compiler/macos_phase2_phase3_compatibility_spec.md`; `build/review/final_m2_m3_macos_audit_2026-09-02.md` |
...
| Incremental no-op | Zero parsed/lowered/emitted modules where producer-neutral caches apply; zero link when ordered link inputs are unchanged. Until implemented, mark RED rather than relaxing it. | **IMPLEMENTED; NATIVE EVIDENCE BLOCKED:** bounded pre-frontend admission binds compiler, target, options, provider, SCV, authority, environment, ordered inputs, and output fingerprints; immutable generation publication uses file/directory fsync and atomic `CURRENT`. Project-aware parser/HIR acceptance passes; no retained native production receipt exists yet. |
...
| Concurrency | No shared writable cache until atomic P4 publication exists; later tests cover identical/conflicting writers, pinned readers, crash recovery, and GC. | **IMPLEMENTED; NATIVE EVIDENCE BLOCKED:** one authority lock serializes immutable generation publication, reader lease pinning, and deterministic bounded GC; crash/rename/fsync failpoints and an OS-thread writer-race probe cover the required lifecycle. Project-aware parser/HIR acceptance passes; native SPipe execution still requires a compatible self-hosted runtime. |
