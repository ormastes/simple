...
## Status correction — 2026-08-24 (measured, seed binary `f6521b60…`)

The Phase 6 and Phase 9 status text above is stale. Measured audit and evidence:
`doc/01_research/compiler/hardening/simple_hardening_plan_2026-08-21.md` §28.

- **Phase 6 — `partial` → `REGRESSED`.** `check-aspect-seal.shs` exits **2**
  (`ERROR — nothing was checked (selftest failed: …)`); the census
  stack-overflows on the deployed binary. Its recorded PASS was minted against
  a different artifact and does not reproduce.
  Record: `doc/08_tracking/bug/aspect_seal_census_stack_overflow_gate_dead_2026-08-24.md`.
- **Phase 9 (a) is DONE** — `check-native-seed-parity` and
  `check-engine-differential` are both wired to `emit_receipt`.
- **Phase 9 (b) is DONE** — `scripts/check/check-reachable-unsupported.shs`
  exists and is wired (`PASS — 752 transition row(s) checked,
  reachable-unsupported=298 (baseline 298)`). Note the baseline is 298, so
  §22.1's "reachable unsupported 0" is still UNMET; the gate ratchets, it does
  not assert the criterion.
- **Phase 9 (c) half done** — `check-generated-visitors-fresh` is green
  (`selftest: 4/4`). `check-critical-wildcard-ban` is still RED with one newly
  introduced bucket at `src/compiler/40.mono/monomorphize_integration.spl:380`,
  owned by the mono lane.
- **New Phase 9 deliverable, landed:** `scripts/check/refresh-critical-release-receipts.shs`.
  The aggregate's "backlog" was mostly receipt staleness — one `bin/simple`
  redeploy invalidates every receipt at once (measured: 10 of 12
  `artifact-mismatch`, only 2 truly missing) and nothing existed to re-mint
  them. Run it before reading `check-critical-release-seal.shs` as a backlog.
- **Phase 7 remains `not started`,** and it is now the critical-path blocker:
  `check-seed-selfhost-parity.shs` does not exist, which is Phase 8's
  `E-PIN-006`, so no package is pinnable `critical`.
