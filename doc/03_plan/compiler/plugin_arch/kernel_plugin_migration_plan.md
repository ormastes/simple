...
implementation is broad, but the admitted
self-hosted binary lacks the required SPipe command surface, the latest Stage3
attempt failed without a candidate, and no cross-host bootstrap evidence exists.
...
| **1 — ABI digest** | **STRUCTURAL PASS; SPipe RUNTIME BLOCKED.** Typed-HIR field-ordinal digesting is compute-only and reached by both production HIR paths. | `src/compiler/20.hir/abi_interface.spl`; `src/compiler/80.driver/driver_hir_pipeline_lowering.spl`; `test/01_unit/compiler/interface_compat/compile_interface_spec.spl`; `doc/09_report/compiler/kernel_plugin_migration_phase_1_4_6_followup_review_2026-09-02.md` |
| **2 — param objects and lint** | **STRUCTURAL PASS; SPipe RUNTIME BLOCKED.** App-boundary environment decoding, typed parameters, AST-backed linting, and ordinal mutation checks exist. | `src/lib/common/plugin/aspect_params.spl`; `src/app/io/aspect_params_env.spl`; `scripts/check/check-param-object-evolution.shs`; `test/01_unit/compiler/plugin_arch/param_object_lint_spec.spl`; `build/review/accepted_phase_0_4_m0_m4_phase8_verification_audit.md` |
...
| **4 — lint table seam** | **STRUCTURAL PASS; SPipe/BOOTSTRAP BLOCKED.** Canonical per-rule provider identities are recomputed before production table dispatch. | `src/compiler/90.tools/lint/lint_rule_api.spl`; `src/compiler/90.tools/lint/static_rules.spl`; `test/01_unit/compiler/lint/lint_rule_table_spec.spl`; `doc/09_report/compiler/kernel_plugin_migration_phase_1_4_6_followup_review_2026-09-02.md` |
...
| **6 — APK/SFFI negotiation** | **STRUCTURAL PASS; SPipe/NATIVE BLOCKED; ABI v1 SELECTED.** Mandatory fail-closed negotiation and compiler-bound ABI v1 admission are wired; no real native dynamic-library result is retained. | `src/lib/common/plugin/negotiation.spl`; `src/compiler/70.backend/backend/runtime_compiler.spl`; `test/01_unit/lib/sffi/dynamic_versioned_negotiate_spec.spl`; `doc/09_report/compiler/kernel_plugin_migration_phase_1_4_6_followup_review_2026-09-02.md` |
...
| **8 — package ranges** | **STRUCTURAL PASS; SPipe RUNTIME BLOCKED; ORDERING BLOCKED BY PHASE 7.** Root CLI dispatch, parsed fail-closed classification, deterministic range resolution, policy binding, and atomic lock/update writes are implemented. | `src/app/pkg/requires_range.spl`; `src/app/lock/main.spl`; `src/app/update/main.spl`; `test/03_system/app/pkg/feature/requires_range_spec.spl`; `doc/06_spec/03_system/app/pkg/feature/requires_range_spec.md` |
