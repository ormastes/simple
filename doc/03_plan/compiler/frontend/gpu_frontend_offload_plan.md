...
|---|---|---|---|
| GFO-001 | Pure resolver + profile/decision/receipt helpers | `structural_contracts/frontend_offload_switch.spl`, `__init__.spl` export | DONE — `frontend_offload_switch_spec` 22/22; sabotage bites (env-beats-cli → 1 failed; pre-fix contract → 2 failed) |
...
| GFO-008 | Put `simple-gpu` in the cargo workspace so its CUDA/ROCm backends are type-checked | `src/compiler_rust/Cargo.toml` | DONE 2026-09-06 — adding it surfaced **4 more E0308s** invisible to every gate (`CString::as_ptr()` → `*const i8` externs); all five extern declarations retyped `c_char`; `cargo check -p simple-gpu --features cuda,rocm` now passes, `cargo check --workspace` rc=0 |
| GFO-009 | Wire `frontend_offload_profile` so it is not unused code | `80.driver/driver_source_pipeline_parsing.spl` | DONE 2026-09-06 — the driver now builds the profile and fails the compile on `validate_offload_profile` error (`frontend_offload_invalid_profile: …`) |
...
| AC-1 | `simple.sdn frontend.offload*` as an actual source (resolver slot is tested with injected text) | GFO-005 — **not blocked on `parser_framework`**; see below |
...
|---|---|---|
| 1 | contracts v2 + one typed mode enum (GFPU-100..106 / GPU-PARSE-01xx); `gpu_parse_available` becomes real admission | peer landing |
...
- **GFO-005 — `simple.sdn frontend.offload*` is never read.** Correction
  (2026-09-06): this is **not** blocked on `parser_framework`. `DriverContext.config`
  is built from `CompilerConfig.from_env()` plus CLI options and never loads
  `simple.sdn`; `ProjectContext` (`80.driver/project.spl`) has no caller in the
  compile path at all. So the blocker is a design choice about where to pay a
  config read, and it has a cost: loading `ProjectContext` inside the parse
  pipeline puts a file open + SDN parse on every compile that does not set the
  env var — i.e. almost all of them. The cheaper option is to resolve it in the
  CLI layer (which already folds both flags into the env vars and is off the hot
  path) and leave the driver's single env read alone. Decide before implementing;
  do not add the read to the pipeline by default.
