...
**Date:** 2026-02-19
**Status:** Draft
**Related:** `doc/03_plan/src_refactor_lib_rename_2026-02-19.md`
...
- **`test/feature/`** — end-to-end feature/system tests; **generates `doc/02_requirements/feature/`** docs on every run
...
|----------------|---------|
| `doc/02_requirements/feature/feature.md` | Every `bin/simple test` run |
| `doc/02_requirements/feature/pending_feature.md` | Every `bin/simple test` run |

The existing `doc/02_requirements/feature/` is already the target — no path changes needed, only the source
...
test/
  ...
  feature/                          # ← generates doc/02_requirements/feature/ on every test run
...
After T4 (rename): `doc/02_requirements/feature/feature.md` still generates correctly.
