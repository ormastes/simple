...
- Windows `bin/simple.cmd` source launches now skip zero-byte runtime candidates
  ...
  present. `scripts/check/check-windows-simple-cmd-launcher-health.ps1` now
  proves this path by launching the Win32 MDI probe through `bin/simple.cmd`,
  validating the proof env, and checking `--version`. The 2026-07-16 report
  records `windows_simple_cmd_launcher_health_status=pass`,
...
BrowserSession Uint8Array nonzero-offset mutating/search helper continuation:

Detailed completion checklist:

- Confirm the fresh worktree starts from synchronized `main`/`origin/main`.
- Confirm existing BrowserSession coverage already exercises `fill`,
  `includes`, `indexOf`, `lastIndexOf`, `copyWithin`, and `reverse` on
  zero-offset typed arrays and prototype-dispatched receivers.
- Confirm existing BrowserSession coverage already exercises nonzero-offset
  view callback helpers, `set`, `slice`, constructor copy, and shared subarray
  storage.
- Add one BrowserSession browser-script scenario using `base.subarray(2, 7)` as
  the receiver for non-callback helper methods.
- Verify `Uint8Array.prototype.fill.call(view, ...)` writes only the logical
  view range and returns the receiver.
- Verify `includes`, `indexOf`, and `lastIndexOf` search the logical view
  window with positive and negative from-index handling.
- Verify `copyWithin.apply(view, ...)` copies from the view window and returns
  the receiver.
- Verify `reverse.call(view)` reverses only the view window and returns the
  receiver.
- Verify base-buffer bytes outside the view window remain unchanged while the
  shared base buffer reflects all view-window mutations.
- Regenerate the mirrored SPipe scenario manual and move old-path docgen output
  onto `doc/06_spec/unit/...`.
- Restore generated index, tracking, and adjacent old-path manual noise.
- Record focused and manual scenario counts after docgen.
...
BrowserSession scripts now prove non-callback `Uint8Array` helpers operate on
the logical window of a nonzero-offset `subarray` receiver. The focused
assertion verifies `fill.call(view, ...)` writes only the view range and returns
the receiver, `includes`/`indexOf`/`lastIndexOf` search the view with positive
and negative from-index handling, `copyWithin.apply(view, ...)` copies from the
view window and returns the receiver, and `reverse.call(view)` reverses only the
view window while returning the receiver. The assertion also verifies base
buffer bytes outside the view stay unchanged while the shared base buffer
reflects every view-window mutation. The focused fetch/WASM chain spec now
passes `231/231`, and the generated manual records `Total scenarios | 231 |`.
The native WASM host spec remained `107/107`, the WebGPU JS/WASM system spec
remained `106/106`, Node API conformance remained `275/275`, and `src/lib`
completed with the current `405 warning(s)` across `5936` files. Broader
typed-array prototype parity and production GUI pixel parity remain open.
...
BrowserSession Uint8Array toSorted continuation:

Detailed completion checklist:

- [x] Start from clean temp worktree `/tmp/simple-gui-hardening-next30` on
  `main` at `origin/main`.
- [x] Leave the primary detached dirty checkout untouched.
- [x] Audit BrowserSession `Uint8Array` coverage and confirm mutating `sort`
  and copy-by-change `toReversed` are covered while `toSorted` is absent.
- [x] Add `NATIVE_UINT8_ARRAY_TO_SORTED` with an unused native id.
- [x] Register `toSorted` on constructed `Uint8Array` instances,
  byte-array-created typed arrays, and `Uint8Array.prototype`.
- [x] Route direct member calls without a comparator through a copied numeric
  sort helper.
- [x] Route direct member calls with a comparator through a copied comparator
  sort path.
- [x] Route prototype `call` and `apply` dispatch through the copied sort path.
...
- [x] Preserve byte normalization in copied output: `260 -> 4` and `-1 -> 255`.
...
- [x] `SIMPLE_LIB=src SIMPLE_BIN=/home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple test test/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.spl --mode=interpreter --timeout-ms=180000 --clean --force-rebuild --format json` - `240/240` passed.
- [x] `SIMPLE_LIB=src /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple spipe-docgen test/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.spl --output doc/06_spec` - generated `240`-scenario manual with existing docgen stub warning.
...
BrowserSession scripts are being extended with bounded
`Uint8Array.prototype.toSorted` for direct calls plus prototype `call`/`apply`
dispatch. The implementation returns fresh zero-offset copied storage for
numeric and comparator ordering, preserves the source typed array after returned
copy mutation, and preserves normalized byte output. The focused fetch/WASM
chain spec now passes `240/240`, and the generated manual records
...
- Confirm the fresh worktree starts from synchronized `main`/`origin/main`.
- Confirm the prior direct `Uint8Array.prototype.slice` copied-buffer isolation
  scenario and `225`-scenario generated manual are present.
- Add one BrowserSession browser-script scenario that invokes
  `Uint8Array.prototype.slice.call(...)` and
  `Uint8Array.prototype.slice.apply(...)` against the same source typed array.
- Verify both prototype-dispatched slices allocate independent buffers with
  `slice.buffer !== source.buffer`.
- Verify both copied slices start at byte offset `0`, preserve their requested
  lengths, coerce writes through Uint8 byte wrapping, and remain isolated from
  later source writes.
- Verify the source typed array remains independently mutable after both copied
  slices are mutated.
- Regenerate the mirrored SPipe scenario manual and move old-path docgen output
  onto `doc/06_spec/unit/...`.
- Restore generated index, tracking, and adjacent old-path manual noise.
...
BrowserSession scripts now prove prototype-dispatched `Uint8Array` slice copies
through both `Uint8Array.prototype.slice.call(...)` and
`Uint8Array.prototype.slice.apply(...)`. The focused assertion verifies both
returned typed arrays use independent buffers, start at byte offset `0`, preserve
the requested length, coerce writes through Uint8 byte wrapping, and remain
isolated from later source writes while the source remains independently
mutable. The focused fetch/WASM chain spec now passes `226/226`, and the
generated manual records `Total scenarios | 226 |`. The native WASM host spec
remained `107/107`, the WebGPU JS/WASM system spec remained `106/106`, Node API
conformance remained `275/275`, and `src/lib` completed with the current
`405 warning(s)` across `5936` files. Broader browser/WASM semantics remain
open.
...
evaluation. The focused assertion verifies the raw compileStreaming path
reports byte length `11`, section count `1`, and `hasTypeSection=true`, then
verifies the compileStreaming-instantiated path resolves with
`status=instantiated`, byte length `11`, section count `1`,
`hasTypeSection=true`, and exported instance object metadata. The focused
fetch/WASM chain spec now passes `222/222`, and the generated manual records
`Total scenarios | 222 |`. The native WASM host spec remained `107/107`, the
WebGPU JS/WASM system spec remained `106/106`, Node API conformance remained
`275/275`, and `src/lib` completed with the current `405 warning(s)` across
`5936` files. Broader browser/WASM semantics remain open.

BrowserSession scripts now compare raw compileStreaming module import
...
compileStreaming path reports import count `1`, byte length `27`, one
descriptor, and `env:foo:function`, then verifies the
compileStreaming-instantiated path resolves with `status=instantiated`, import
count `1`, byte length `27`, one descriptor, `env:foo:function`, and exported
instance object metadata. The focused fetch/WASM chain spec now passes
`221/221`, and the generated manual records `Total scenarios | 221 |`. The
native WASM host spec remained `107/107`, the WebGPU JS/WASM system spec
remained `106/106`, Node API conformance remained `275/275`, and `src/lib`
completed with the current `405 warning(s)` across `5936` files. Broader
browser/WASM semantics remain open.

BrowserSession WebAssembly compileStreaming module/instantiated base metadata
parity continuation:

Detailed completion checklist:

- Confirm the fresh worktree starts from synchronized `main`/`origin/main`.
- Confirm the prior compileStreaming export/import descriptor parity slices are
  present before extending base metadata parity.
- Add one same-script BrowserSession scenario that compares raw base metadata
  from `WebAssembly.compileStreaming(...)` with base metadata from a separate
  `compileStreaming(...).then(module => WebAssembly.instantiate(module))` path
  using the same module bytes.
- Verify the raw compiled-module side reports byte length `11`, section count
  `1`, and `hasTypeSection=true`.
- Verify the compileStreaming-instantiated side reports `status=instantiated`,
  byte length `11`, section count `1`, `hasTypeSection=true`, and exported
  instance object metadata.
...
BrowserSession combined streaming invalid-header rejection continuation:
...
- Add a BrowserSession browser-script scenario that queues both
  ...
  `WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm'), {})` for
  the same invalid-header WASM body in one script.
- Verify the initial script result is `queued` and the shared `out` variable is
  empty before any network response is committed.
- Verify the first pending network request is a fetch for
  `https://example.com/compile.wasm`.
- Commit the invalid-header response for `/compile.wasm` and verify the
  compile-streaming path rejects through `.catch(...)` with
  `invalid-wasm-header`, without writing any unexpected success marker.
- Verify the second pending network request is a fetch for
  `https://example.com/instantiate.wasm`.
- Commit the same invalid-header response for `/instantiate.wasm` and verify the
  final output keeps the compile-streaming rejection evidence and appends the
  instantiate-streaming rejection status `invalid` plus error
  `invalid-wasm-header`, again without writing any unexpected success marker.
- Regenerate the mirrored scenario manual for the changed SPipe spec and move
  docgen's old `01_unit` output onto the tracked `unit` manual path.
- Remove generated docgen noise from adjacent specs, restore generated tracking
  index churn, and keep only the intended source/manual/plan files in the commit.
...
BrowserSession scripts now compare both streaming entry points against the same
invalid-header WASM body in one queued script. The scenario proves the first
network commit completes only the compile-streaming rejection chain, then the
second network commit appends the instantiate-streaming rejection chain while
preserving the shared `invalid-wasm-header` evidence and the instantiate
`invalid` status. The focused fetch/WASM chain spec is now passing `203/203`;
the native WASM host spec remained `107/107`, the WebGPU JS/WASM system spec
remained `106/106`, Node API conformance remained `275/275`, and `src/lib`
completed with the current `399 warning(s)` across `5910` files. Docgen
...
BrowserSession fetched arrayBuffer direct/compiled multiple-function parity continuation:
...
- Verify the first pending network request is a fetch for
  `https://example.com/direct.wasm`.
- Commit the multi-function-export module response for `/direct.wasm` and
  verify the direct arrayBuffer instantiate path reports fetched byte length
  `47`, status `instantiated`, module byte length `47`, function export count
  `2`, both exported types as `function`, and both zero-argument call results as
  `undefined`.
- Verify the second pending network request is a fetch for
  `https://example.com/compiled.wasm`.
- Commit the same multi-function-export module response for `/compiled.wasm`
  and verify the final output keeps the direct arrayBuffer evidence and appends
  compiled arrayBuffer fetched byte length `47`, compiled module byte length
  `47`, instantiated status `instantiated`, result module byte length `47`,
  function export count `2`, both exported types as `function`, and both
  zero-argument call results as `undefined`.
- Regenerate the mirrored scenario manual for the changed SPipe spec and move
  docgen's old `01_unit` output onto the tracked `unit` manual path.
- Remove generated docgen noise from adjacent specs, restore generated tracking
...
BrowserSession scripts now compare fetched-arrayBuffer direct instantiate and
compiled instantiate multi-function-export paths in one queued script. The first
network commit completes only the direct `WebAssembly.instantiate(bytes)` path
and records fetched byte length `47`, status `instantiated`, module byte length
`47`, function export count `2`, exported `init`/`render` types as `function`,
and both zero-argument call results as `undefined`; the second commit appends
the `WebAssembly.compile(bytes)` plus `WebAssembly.instantiate(module)` path
with matching byte lengths, status, export count, exported types, and call
results. The focused fetch/WASM chain spec is now passing `206/206`; the native
WASM host spec remained `107/107`, the WebGPU JS/WASM system spec remained
`106/106`, Node API conformance remained `275/275`, targeted payment helper
width checks passed for the six touched payment files, and `src/lib` completed
with the current `404 warning(s)` across `5931` files after current-main
payment `int` helper literals were made explicit. Docgen regenerated the
mirrored manual with `Total scenarios | 206 |`; diff hygiene and the
`doc/06_spec` layout gate are checked before push.

BrowserSession fetched arrayBuffer direct/compiled table-global parity continuation:
...
- Verify the first pending network request is a fetch for
  `https://example.com/direct.wasm`.
- Commit the table/global module response for `/direct.wasm` and verify the
  direct arrayBuffer instantiate path reports fetched byte length `40`, status
  `instantiated`, module byte length `40`, table kind `table`, table element
  `funcref`, initial table slot `null`, grow result `1`, grown table length
  `2`, global kind `global`, value type `i32`, mutability `false`, and value
  `42`.
- Verify the second pending network request is a fetch for
  `https://example.com/compiled.wasm`.
- Commit the same table/global module response for `/compiled.wasm` and verify
  the final output keeps the direct arrayBuffer evidence and appends compiled
  arrayBuffer fetched byte length `40`, compiled module byte length `40`,
  instantiated status `instantiated`, result module byte length `40`, matching
  table kind/element/grow evidence, and matching global descriptor/value
  evidence.
- Regenerate the mirrored scenario manual for the changed SPipe spec and move
  docgen's old `01_unit` output onto the tracked `unit` manual path.
- Remove generated docgen noise from adjacent specs, restore generated tracking
...
BrowserSession scripts now compare fetched-arrayBuffer direct instantiate and
compiled instantiate table/global export paths in one queued script. The first
network commit completes only the direct `WebAssembly.instantiate(bytes)` path
and records fetched byte length `40`, status `instantiated`, module byte length
`40`, table kind `table`, table element `funcref`, initial table slot `null`,
grow result `1`, grown length `2`, global kind `global`, global value type
`i32`, mutability `false`, and value `42`; the second commit appends the
`WebAssembly.compile(bytes)` plus `WebAssembly.instantiate(module)` path with
matching byte lengths, status, table evidence, and global evidence. The focused
fetch/WASM chain spec is now passing `207/207`; the native WASM host spec
remained `107/107`, the WebGPU JS/WASM system spec remained `106/106`, Node API
conformance remained `275/275`, and `src/lib` completed with the current
...
BrowserSession fetched arrayBuffer direct/compiled missing-import parity continuation:
...
- Verify the first pending network request is a fetch for
  `https://example.com/direct.wasm`.
- Commit the imported-function module response for `/direct.wasm` and verify
  the direct arrayBuffer instantiate path routes through `catch` with fetched
  byte length `52`, rejected status `invalid`, and error
  `unsupported-wasm-imports`.
- Verify the second pending network request is a fetch for
  `https://example.com/compiled.wasm`.
- Commit the same imported-function module response for `/compiled.wasm` and
  verify the final output keeps the direct rejection evidence and appends
  compiled arrayBuffer fetched byte length `52`, compiled module byte length
  `52`, rejected status `invalid`, and error `unsupported-wasm-imports`.
- Regenerate the mirrored scenario manual for the changed SPipe spec and move
  docgen's old `01_unit` output onto the tracked `unit` manual path.
- Remove generated docgen noise from adjacent specs, restore generated tracking
  index churn, and keep only the intended source/manual/plan files in the
...
BrowserSession scripts now compare fetched-arrayBuffer direct instantiate and
compiled instantiate missing-import rejection paths in one queued script. The
first network commit completes only the direct
`WebAssembly.instantiate(bytes, {})` path and records fetched byte length `52`,
rejected status `invalid`, and error `unsupported-wasm-imports`; the second
commit appends the `WebAssembly.compile(bytes)` plus
`WebAssembly.instantiate(module, {})` path with matching fetched byte length
`52`, compiled module byte length `52`, rejected status `invalid`, and error
`unsupported-wasm-imports`. The focused fetch/WASM chain spec is now passing
`210/210`; the native WASM host spec remained `107/107`, the WebGPU JS/WASM
system spec remained `106/106`, Node API conformance remained `275/275`, and
`src/lib` completed with the current `405 warning(s)` across `5936` files.
Docgen regenerated the mirrored manual with `Total scenarios | 210 |`; diff
hygiene and the `doc/06_spec` layout gate are checked before push.

BrowserSession fetched arrayBuffer direct/compiled export-descriptor parity
continuation:

Completion checklist:

- Add one same-script BrowserSession scenario that queues both
  `window.fetch('/direct.wasm')` and `window.fetch('/compiled.wasm')`, so direct
  instantiate and compiled-module export-descriptor behavior run through the same
  event/promise scheduler.
...
- Verify the first pending network request is a fetch for
  `https://example.com/direct.wasm`.
- Commit the table/global export module response for `/direct.wasm` and verify
  the direct `arrayBuffer()` byte length is `40`.
- Verify the direct path uses `WebAssembly.instantiate(bytes)`, returns
  `status=instantiated`, preserves `result.module.byteLength=40`, and exposes
  exactly two `WebAssembly.Module.exports(result.module)` descriptors.
- Verify the direct descriptor order and kinds remain deterministic:
  `tbl:table` followed by `answer:global`.
- Verify the second pending network request is a fetch for
  `https://example.com/compiled.wasm`.
- Commit the same table/global export module response for `/compiled.wasm` and
  verify the compiled path records fetched `arrayBuffer()` byte length `40`.
- Verify `WebAssembly.compile(bytes)` preserves compiled module byte length
  `40`, and `WebAssembly.Module.exports(module)` exposes the same two
  descriptors in the same order: `tbl:table` then `answer:global`.
- Verify `WebAssembly.instantiate(module)` also succeeds from that compiled
  module and appends `compiledInstantiate:instantiated:40`, proving descriptor
  inspection does not consume or mutate the module.
- Regenerate the mirrored scenario manual and confirm its total scenario count
  increments from `210` to `211`.
...
BrowserSession scripts now compare fetched-arrayBuffer direct instantiate and
compiled-module export descriptor paths in one queued script. The first network
commit completes only `WebAssembly.instantiate(bytes)` and records byte length
`40`, instantiated status, module byte length `40`, and deterministic
`tbl:table` plus `answer:global` descriptors. The second network commit appends
the `WebAssembly.compile(bytes)` descriptor path, compiled module byte length
`40`, the same descriptor ordering, and successful
`WebAssembly.instantiate(module)` evidence. The focused fetch/WASM chain spec is
now passing `211/211`; adjacent WASM host, WebGPU JS/WASM, and Node API specs
remain passing at `107/107`, `106/106`, and `275/275`; `src/lib` remains passing
with `405 warning(s)` across `5936` files. Docgen regenerated the mirrored
manual with `Total scenarios | 211 |`; diff hygiene and the `doc/06_spec`
layout gate are checked before push.

BrowserSession fetched arrayBuffer direct/compiled import-descriptor parity
continuation:
...
- Add one same-script BrowserSession scenario that queues both
  `window.fetch('/direct.wasm')` and `window.fetch('/compiled.wasm')`, so direct
  instantiated-module imports and compiled-module imports run through the same
  event/promise scheduler.
- Verify the pre-commit script result is exactly `queued`, and verify `out`
  remains empty before any network response is committed.
- Verify the first pending network request is a fetch for
  `https://example.com/direct.wasm`.
- Commit the imported-function module response for `/direct.wasm` and verify the
  direct `arrayBuffer()` byte length is `27`.
- Verify the direct path uses `WebAssembly.instantiate(bytes, imports)`, returns
  `status=instantiated`, preserves `result.module.importCount=1`,
  `result.module.byteLength=27`, and exposes exactly one
  `WebAssembly.Module.imports(result.module)` descriptor.
- Verify the direct descriptor fields remain deterministic:
  module `env`, name `foo`, kind `function`, and verify the instance exports
  object is still available.
- Verify the second pending network request is a fetch for
  `https://example.com/compiled.wasm`.
- Commit the same imported-function module response for `/compiled.wasm` and
  verify the compiled path records fetched `arrayBuffer()` byte length `27`.
- Verify `WebAssembly.compile(bytes)` preserves `module.importCount=1`,
  compiled module byte length `27`, and the same `env:foo:function` descriptor
  from `WebAssembly.Module.imports(module)`.
- Verify `WebAssembly.instantiate(module, imports)` also succeeds from that
  compiled module and appends instantiated status, import count `1`, module byte
  length `27`, and exports object evidence.
- Regenerate the mirrored scenario manual and confirm its total scenario count
  increments from `211` to `212`.
- Move docgen's old `01_unit` output onto the tracked `unit` manual path, remove
  generated adjacent-spec noise, restore generated tracking-index churn, and keep
  only the intended spec/manual/plan files in the commit.
- Confirm no executable `*_spec.spl` files remain under `doc/06_spec` before
...
BrowserSession scripts now compare fetched-arrayBuffer direct instantiated-module
import descriptors and compiled-module import descriptors in one queued script.
The first network commit completes only
`WebAssembly.instantiate(bytes, imports)` and records byte length `27`,
instantiated status, import count `1`, module byte length `27`, descriptor
`env:foo:function`, and the instance exports object. The second network commit
appends `WebAssembly.compile(bytes)` descriptor evidence with byte length `27`,
import count `1`, the same descriptor, and successful
`WebAssembly.instantiate(module, imports)` evidence. The focused fetch/WASM chain
spec is now passing `212/212`; adjacent WASM host, WebGPU JS/WASM, and Node API
specs remain passing at `107/107`, `106/106`, and `275/275`; `src/lib` remains
passing with `405 warning(s)` across `5936` files. Docgen regenerated the
mirrored manual with `Total scenarios | 212 |`; diff hygiene and the
`doc/06_spec` layout gate are checked before push.

BrowserSession fetched arrayBuffer direct/compiled invalid-header catch parity
continuation:

Completion checklist:

- Add one same-script BrowserSession scenario that queues both
  `window.fetch('/direct-bad.wasm')` and
  `window.fetch('/compiled-bad.wasm')`, so direct invalid instantiate and
  compiled invalid compile rejection paths run through the same event/promise
  scheduler.
- Verify the pre-commit script result is exactly `queued`, and verify `out`
  remains empty before any network response is committed.
- Verify the first pending network request is a fetch for
  `https://example.com/direct-bad.wasm`.
- Commit the invalid-header module response for `/direct-bad.wasm` and verify
  the direct `arrayBuffer()` byte length is `16`.
- Verify `WebAssembly.instantiate(bytes)` does not write an unexpected success
  result and instead routes through `catch` with status `invalid` and error
  `invalid-wasm-header`.
- Verify the second pending network request is a fetch for
  `https://example.com/compiled-bad.wasm`.
- Commit the same invalid-header module response for `/compiled-bad.wasm` and
  verify the compiled `arrayBuffer()` byte length is `16`.
- Verify `WebAssembly.compile(bytes)` does not write an unexpected compiled
  module result and instead routes through `catch` with status `invalid` and
  error `invalid-wasm-header`.
- Verify the final output preserves the direct invalid-header rejection evidence
  and appends the compiled invalid-header rejection evidence in deterministic
  order.
- Regenerate the mirrored scenario manual and confirm its total scenario count
  increments from `212` to `213`.
- Move docgen's old `01_unit` output onto the tracked `unit` manual path, remove
  generated adjacent-spec noise, restore generated tracking-index churn, and keep
  only the intended spec/manual/plan files in the commit.
- Confirm no executable `*_spec.spl` files remain under `doc/06_spec` before
...
BrowserSession scripts now compare fetched-arrayBuffer direct invalid
instantiate and compiled invalid compile rejection paths in one queued script.
The first network commit completes only the direct
`WebAssembly.instantiate(bytes)` invalid-header path and records fetched byte
length `16`, rejected status `invalid`, and error `invalid-wasm-header`. The
second network commit appends `WebAssembly.compile(bytes)` invalid-header
evidence with fetched byte length `16`, rejected status `invalid`, and the same
`invalid-wasm-header` error. The focused fetch/WASM chain spec is now passing
`213/213`; adjacent WASM host, WebGPU JS/WASM, and Node API specs remain passing
at `107/107`, `106/106`, and `275/275`; `src/lib` remains passing with
`405 warning(s)` across `5936` files. Docgen regenerated the mirrored manual
with `Total scenarios | 213 |`; diff hygiene and the `doc/06_spec` layout gate
are checked before push.

BrowserSession fetched arrayBuffer direct/compiled fetch-error catch parity
continuation:

Completion checklist:

- Add one same-script BrowserSession scenario that queues both
  `window.fetch('/direct-down.wasm')` and
  `window.fetch('/compiled-down.wasm')`, so direct instantiate fetch errors and
  compiled fetch errors run through the same event/promise scheduler.
- Verify the pre-commit script result is exactly `queued`, and verify `out`
  remains empty before any network response is committed.
- Verify the first pending network request is a fetch for
  `https://example.com/direct-down.wasm`.
- Commit a failed network response for `/direct-down.wasm` with status `0` and
  error `network-down`.
- Verify the response commit reports the expected `network-down` error while
  still driving the queued JavaScript promise rejection.
- Verify the direct path does not write an unexpected instantiate success and
  instead routes through `catch` with `directArrayBufferFetchError:network-down`.
- Verify the second pending network request is a fetch for
  `https://example.com/compiled-down.wasm`.
- Commit the same failed network response for `/compiled-down.wasm` with status
  `0` and error `network-down`.
- Verify the second response commit reports the expected `network-down` error
  while still driving the queued JavaScript promise rejection.
- Verify the compiled path does not write an unexpected compiled module result
  and instead routes through `catch` with
  `compiledArrayBufferFetchError:network-down`.
- Verify the final output preserves the direct fetch-error rejection evidence and
  appends the compiled fetch-error rejection evidence in deterministic order.
- Regenerate the mirrored scenario manual and confirm its total scenario count
  increments from `213` to `214`.
- Move docgen's old `01_unit` output onto the tracked `unit` manual path, remove
  generated adjacent-spec noise, restore generated tracking-index churn, and keep
  only the intended spec/manual/plan files in the commit.
- Confirm no executable `*_spec.spl` files remain under `doc/06_spec` before
...
BrowserSession scripts now compare fetched-arrayBuffer direct instantiate base
module metadata and compiled module base metadata in one queued script. The
first network commit completes only `WebAssembly.instantiate(bytes)` and records
byte length `11`, instantiated status, module byte length `11`, section count
`1`, and an instance exports object. The second network commit appends
`WebAssembly.compile(bytes)` metadata with byte length `11`, section count `1`,
`hasTypeSection=true`, and successful `WebAssembly.instantiate(module)`
evidence. The focused fetch/WASM chain spec is now passing `215/215`; adjacent
WASM host, WebGPU JS/WASM, and Node API specs remain passing at `107/107`,
`106/106`, and `275/275`; `src/lib` remains passing with `405 warning(s)`
across `5936` files. Docgen regenerated the mirrored manual with
`Total scenarios | 215 |`; diff hygiene and the `doc/06_spec` layout gate are
checked before push.

BrowserSession compileStreaming/instantiateStreaming base module metadata parity
continuation:
...
- Add one same-script BrowserSession scenario that queues both
  `WebAssembly.compileStreaming(window.fetch('/compile.wasm'))` and
  `WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm'))`, so the
  streaming compile and streaming instantiate base metadata paths run through
  the same event/promise scheduler.
- Verify the pre-commit script result is exactly `queued`, and verify `out`
  remains empty before any network response is committed.
- Verify the first pending network request is a fetch for
  `https://example.com/compile.wasm`.
- Commit the minimal valid module response for `/compile.wasm` and verify
  `WebAssembly.compileStreaming(...)` records module byte length `11`,
  `sectionCount=1`, and `hasTypeSection=true`.
- Verify the second pending network request is a fetch for
  `https://example.com/instantiate.wasm`.
- Commit the same minimal valid module response for `/instantiate.wasm` and
  verify `WebAssembly.instantiateStreaming(...)` returns
  `status=instantiated`, preserves `result.module.byteLength=11`, reports
  `sectionCount=1`, and exposes an instance exports object.
- Verify the final output preserves the compileStreaming metadata evidence and
  appends the instantiateStreaming metadata evidence in deterministic order.
- Regenerate the mirrored scenario manual and confirm its total scenario count
  increments from `215` to `216`.
- Move docgen's old `01_unit` output onto the tracked `unit` manual path, remove
  generated adjacent-spec noise, restore generated tracking-index churn, and keep
  only the intended spec/manual/plan files in the commit.
- Confirm no executable `*_spec.spl` files remain under `doc/06_spec` before
...
BrowserSession scripts now compare compileStreaming base module metadata and
instantiateStreaming base module metadata in one queued script. The first
network commit completes only `WebAssembly.compileStreaming(...)` and records
module byte length `11`, section count `1`, and `hasTypeSection=true`. The
second network commit appends `WebAssembly.instantiateStreaming(...)` evidence
with instantiated status, module byte length `11`, section count `1`, and an
instance exports object. The focused fetch/WASM chain spec is now passing
`216/216`; adjacent WASM host, WebGPU JS/WASM, and Node API specs remain passing
at `107/107`, `106/106`, and `275/275`; `src/lib` remains passing with
`405 warning(s)` across `5936` files. Docgen regenerated the mirrored manual
with `Total scenarios | 216 |`; diff hygiene and the `doc/06_spec` layout gate
are checked before push.

BrowserSession fetched arrayBuffer direct/compiled memory-maximum parity
continuation:
...
- Add one same-script BrowserSession scenario that queues both
  `window.fetch('/direct.wasm')` and `window.fetch('/compiled.wasm')`, so the
  direct fetched-arrayBuffer memory maximum path and compiled fetched-arrayBuffer
  memory maximum path run through the same event/promise scheduler.
- Verify the pre-commit script result is exactly `queued`, and verify `out`
  remains empty before any network response is committed.
- Verify the first pending network request is a fetch for
  `https://example.com/direct.wasm`.
- Commit the one-page maximum memory module response for `/direct.wasm` and
  verify the direct `arrayBuffer()` byte length is `26`.
- Verify `WebAssembly.instantiate(bytes)` returns `status=instantiated`,
  preserves module byte length `26`, exposes memory byte length and page size
  `65536`, returns `1` for `memory.grow(0)`, returns `-1` for
  `memory.grow(1)`, and preserves the clamped byte evidence `18`.
- Verify the second pending network request is a fetch for
  `https://example.com/compiled.wasm`.
- Commit the same one-page maximum memory module response for `/compiled.wasm`
  and verify the compiled `arrayBuffer()` byte length and compiled module byte
  length are both `26`.
- Verify `WebAssembly.compile(bytes)` plus `WebAssembly.instantiate(module)`
  preserves instantiated status, module byte length `26`, memory byte length and
  page size `65536`, grow-zero result `1`, grow-failure result `-1`, and clamped
  byte evidence `20`.
- Verify the final output preserves the direct memory-maximum evidence and
  appends the compiled memory-maximum evidence in deterministic order.
- Regenerate the mirrored scenario manual and confirm its total scenario count
  increments from `216` to `217`.
- Move docgen's old `01_unit` output onto the tracked `unit` manual path, remove
  generated adjacent-spec noise, restore generated tracking-index churn, and keep
  only the intended spec/manual/plan files in the commit.
- Confirm no executable `*_spec.spl` files remain under `doc/06_spec` before
...
BrowserSession scripts now compare fetched-arrayBuffer direct and compiled
memory-maximum behavior in one queued script. The first network commit completes
only `WebAssembly.instantiate(bytes)` and records byte length `26`, instantiated
status, module byte length `26`, memory size/page metadata, `memory.grow(0) =
1`, `memory.grow(1) = -1`, and clamped byte evidence `18`. The second network
commit appends `WebAssembly.compile(bytes)` plus
`WebAssembly.instantiate(module)` evidence with matching lengths and growth
limits plus clamped byte evidence `20`. The focused fetch/WASM chain spec is now
passing `217/217`; adjacent WASM host, WebGPU JS/WASM, and Node API specs remain
passing at `107/107`, `106/106`, and `275/275`; `src/lib` remains passing with
`405 warning(s)` across `5936` files. Docgen regenerated the mirrored manual
with `Total scenarios | 217 |`; diff hygiene and the `doc/06_spec` layout gate
are checked before push.

BrowserSession compileStreaming module and instantiateStreaming export
descriptor parity continuation:

Completion checklist:

- Add one same-script BrowserSession scenario that queues both
  `WebAssembly.compileStreaming(window.fetch('/compile.wasm'))` and
  `WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm'))`, so raw
  compiled-module export descriptor inspection and instantiated streaming export
  descriptor inspection run through the same event/promise scheduler.
- Verify the pre-commit script result is exactly `queued`, and verify `out`
  remains empty before any network response is committed.
- Verify the first pending network request is a fetch for
  `https://example.com/compile.wasm`.
- Commit the table/global export module response for `/compile.wasm` and verify
  `WebAssembly.Module.exports(module)` on the raw compileStreaming module records
  module byte length `40`, descriptor count `2`, and deterministic descriptors
  `tbl:table` then `answer:global`.
- Verify the second pending network request is a fetch for
  `https://example.com/instantiate.wasm`.
- Commit the same table/global export module response for `/instantiate.wasm`
  and verify `WebAssembly.Module.exports(result.module)` from
  `WebAssembly.instantiateStreaming(...)` records instantiated status, module
  byte length `40`, descriptor count `2`, and the same `tbl:table` then
  `answer:global` ordering.
- Verify the final output preserves the raw compileStreaming descriptor evidence
  and appends the instantiateStreaming descriptor evidence in deterministic
  order.
- Regenerate the mirrored scenario manual and confirm its total scenario count
  increments from `217` to `218`.
- Move docgen's old `01_unit` output onto the tracked `unit` manual path, remove
  generated adjacent-spec noise, restore generated tracking-index churn, and keep
  only the intended spec/manual/plan files in the commit.
- Confirm no executable `*_spec.spl` files remain under `doc/06_spec` before
...
descriptors and instantiateStreaming export descriptors in one queued script.
The first network commit completes only `WebAssembly.compileStreaming(...)` and
records module byte length `40`, descriptor count `2`, and deterministic
`tbl:table` plus `answer:global` descriptors. The second network commit appends
`WebAssembly.instantiateStreaming(...)` export descriptor evidence with
instantiated status, module byte length `40`, descriptor count `2`, and the same
descriptor ordering. The focused fetch/WASM chain spec is now passing `218/218`;
adjacent WASM host, WebGPU JS/WASM, and Node API specs remain passing at
`107/107`, `106/106`, and `275/275`; `src/lib` remains passing with
`405 warning(s)` across `5936` files. Docgen regenerated the mirrored manual
with `Total scenarios | 218 |`; diff hygiene and the `doc/06_spec` layout gate
are checked before push.

BrowserSession compileStreaming module and instantiateStreaming import
descriptor parity continuation:
...
- Add one same-script BrowserSession scenario that queues both
  `WebAssembly.compileStreaming(window.fetch('/compile.wasm'))` and
  `WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm'), imports)`,
  so raw compiled-module import descriptor inspection and instantiated streaming
  import descriptor inspection run through the same event/promise scheduler.
- Verify the pre-commit script result is exactly `queued`, and verify `out`
  remains empty before any network response is committed.
- Verify the first pending network request is a fetch for
  `https://example.com/compile.wasm`.
- Commit the imported-function module response for `/compile.wasm` and verify
  `WebAssembly.Module.imports(module)` on the raw compileStreaming module records
  import count `1`, module byte length `27`, descriptor count `1`, and
  descriptor `env:foo:function`.
- Verify the second pending network request is a fetch for
  `https://example.com/instantiate.wasm`.
- Commit the same imported-function module response for `/instantiate.wasm` and
  verify `WebAssembly.Module.imports(result.module)` from
  `WebAssembly.instantiateStreaming(...)` records instantiated status, import
  count `1`, module byte length `27`, descriptor count `1`, descriptor
  `env:foo:function`, and an instance exports object.
- Verify the final output preserves the raw compileStreaming import descriptor
  evidence and appends the instantiateStreaming import descriptor evidence in
  deterministic order.
- Regenerate the mirrored scenario manual and confirm its total scenario count
  increments from `218` to `219`.
- Move docgen's old `01_unit` output onto the tracked `unit` manual path, remove
  generated adjacent-spec noise, restore generated tracking-index churn, and keep
  only the intended spec/manual/plan files in the commit.
- Confirm no executable `*_spec.spl` files remain under `doc/06_spec` before
...
BrowserSession WebAssembly compileStreaming module/instantiated import descriptor
parity continuation:

Detailed completion checklist:

- Confirm the fresh worktree starts from synchronized `main`/`origin/main`.
- Confirm the previous export-descriptor slice is present before extending the
  mirror import-descriptor coverage.
- Add one same-script BrowserSession scenario that compares raw import
  descriptors from `WebAssembly.compileStreaming(...)` with import descriptors
  from a separate `compileStreaming(...).then(module =>
  WebAssembly.instantiate(module, imports))` path using the same module bytes.
- Verify the raw compiled-module side reports import count `1`, byte length
  `27`, one descriptor, and descriptor fields `env:foo:function`.
- Verify the compileStreaming-instantiated side reports `status=instantiated`,
  import count `1`, byte length `27`, one descriptor, descriptor fields
  `env:foo:function`, and `typeof result.instance.exports == object`.
- Verify the scenario has two real asynchronous fetch boundaries, a pre-commit
  empty `out` assertion, committed `application/wasm` responses, ordered
  intermediate output after the first commit, and deterministic final output.
- Regenerate the mirrored SPipe scenario manual and move old-path docgen output
...
BrowserSession Uint8Array toLocaleString continuation:

Detailed completion checklist:

- [x] Start from clean temp worktree `/tmp/simple-gui-hardening-next28` on
  `main` at `origin/main`.
- [x] Leave the primary detached dirty checkout untouched.
- [x] Audit BrowserSession `Uint8Array` coverage and confirm
  `toLocaleString` is absent while `toString`, `join`, and prototype string
  dispatch are already covered.
- [x] Add `NATIVE_UINT8_ARRAY_TO_LOCALE_STRING` with an unused native id.
- [x] Register `toLocaleString` on constructed `Uint8Array` instances,
  byte-array-created typed arrays, and `Uint8Array.prototype`.
- [x] Route direct member calls through the same bounded numeric byte string path
  as `toString`.
- [x] Route prototype `call` and `apply` dispatch through the same native helper.
- [x] Extend direct BrowserSession string-helper evidence to compare
  `toString`, `toLocaleString`, and custom-separator `join`.
- [x] Extend prototype-dispatch evidence to cover
  `Uint8Array.prototype.toLocaleString.call(...)` and `.apply(...)`.
- [x] Confirm byte normalization remains visible in string output:
  `260 -> 4` and `-1 -> 255`.
- [x] Run focused changed-file compiler checks.
- [x] Run the focused BrowserSession fetch/WASM chain interpreter spec.
- [x] Regenerate the mirrored SPipe scenario manual and move old-path docgen
...
- [x] `SIMPLE_LIB=src /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple check src/lib/nogc_sync_mut/js/engine/interpreter_eval.spl src/lib/nogc_sync_mut/js/engine/interpreter_native.spl src/lib/nogc_sync_mut/js/engine/runtime.spl test/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.spl` - passed.
- [x] `SIMPLE_LIB=src SIMPLE_BIN=/home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple test test/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.spl --mode=interpreter --timeout-ms=180000 --clean --force-rebuild --format json` - `235/235` passed.
- [x] `SIMPLE_LIB=src /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple spipe-docgen test/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.spl --output doc/06_spec` - generated `235`-scenario manual with existing docgen stub warning.
- [x] `SIMPLE_LIB=src SIMPLE_BIN=/home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple test test/01_unit/lib/common/web/browser_session_wasm_host_spec.spl --mode=interpreter --timeout-ms=180000 --clean --format json` - `107/107` passed.
- [x] `SIMPLE_LIB=src SIMPLE_BIN=/home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple test test/03_system/app/browser/feature/webgpu_js_wasm_simple_spec.spl --mode=interpreter --timeout-ms=240000 --clean --format json` - `106/106` passed.
- [x] `SIMPLE_LIB=src SIMPLE_BIN=/home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple test test/03_system/feature/js/node_api_conformance_spec.spl --mode=interpreter --timeout-ms=240000 --clean --format json` - `275/275` passed.
- [x] `SIMPLE_LIB=src /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple check src/compiler` - passed with `14 warning(s)` across `2627` files.
- [x] `SIMPLE_LIB=src /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple check src/lib` - passed with `405 warning(s)` across `5936` files.
- [x] `SIMPLE_LIB=src /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple check src/app/mcp` - passed with `2 warning(s)` across `27` files.
- [x] `SIMPLE_LIB=src /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple check src/app/simple_lsp_mcp` - passed for `5` files.
- [x] `SIMPLE_LIB=src /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple test test/02_integration/app/mcp_stdio_integration_spec.spl --mode=interpreter` - `5/5` passed.
- [x] `git diff --check` - passed.
- [x] `find doc/06_spec -name '*_spec.spl' | wc -l` - `0`.

BrowserSession scripts now expose bounded `Uint8Array.prototype.toLocaleString`
for direct calls plus prototype `call`/`apply` dispatch. The implementation uses
the same numeric byte string path as `toString`, preserving normalized byte
output for the currently supported numeric `Uint8Array` subset. The focused
fetch/WASM chain spec remains `235/235`, and the generated manual remains at
`Total scenarios | 235 |`. Adjacent native WASM host, WebGPU JS/WASM, and Node
API conformance regressions passed at `107/107`, `106/106`, and `275/275`.
...
BrowserSession Uint8Array toReversed continuation:

Detailed completion checklist:

- [x] Start from clean temp worktree `/tmp/simple-gui-hardening-next29` on
  `main` at `origin/main`.
- [x] Leave the primary detached dirty checkout untouched.
- [x] Audit BrowserSession `Uint8Array` coverage and confirm mutating
  `reverse`/`sort` are covered while copy-by-change `toReversed` is absent.
- [x] Add `NATIVE_UINT8_ARRAY_TO_REVERSED` with an unused native id.
- [x] Register `toReversed` on constructed `Uint8Array` instances,
  byte-array-created typed arrays, and `Uint8Array.prototype`.
- [x] Route direct member calls through a copied reverse helper.
- [x] Route prototype `call` and `apply` dispatch through the same helper.
- [x] Return a new zero-offset `Uint8Array` with independent backing storage.
- [x] Preserve source `Uint8Array` bytes after mutating returned copies.
- [x] Preserve byte normalization in copied output: `260 -> 4` and `-1 -> 255`.
...
- [x] `SIMPLE_LIB=src /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple check src/lib/nogc_sync_mut/js/engine/interpreter_eval.spl src/lib/nogc_sync_mut/js/engine/interpreter_native.spl src/lib/nogc_sync_mut/js/engine/runtime.spl test/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.spl` - passed.
- [x] `SIMPLE_LIB=src SIMPLE_BIN=/home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple test test/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.spl --mode=interpreter --timeout-ms=180000 --clean --force-rebuild --format json` - `237/237` passed.
- [x] `SIMPLE_LIB=src /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple spipe-docgen test/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.spl --output doc/06_spec` - generated `237`-scenario manual with existing docgen stub warning.
- [x] `SIMPLE_LIB=src SIMPLE_BIN=/home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple test test/01_unit/lib/common/web/browser_session_wasm_host_spec.spl --mode=interpreter --timeout-ms=180000 --clean --format json` - `107/107` passed.
- [x] `SIMPLE_LIB=src SIMPLE_BIN=/home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple test test/03_system/app/browser/feature/webgpu_js_wasm_simple_spec.spl --mode=interpreter --timeout-ms=240000 --clean --format json` - `106/106` passed.
- [x] `SIMPLE_LIB=src SIMPLE_BIN=/home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple test test/03_system/feature/js/node_api_conformance_spec.spl --mode=interpreter --timeout-ms=240000 --clean --format json` - `275/275` passed.
- [x] `SIMPLE_LIB=src /home/ormastes/dev/pub/simple/src/compiler_rust/target/release/simple check src/compiler` - passed with `14 warning(s)` across `2627 file(s)`.
...
BrowserSession scripts now expose bounded `Uint8Array.prototype.toReversed` for
direct calls plus prototype `call`/`apply` dispatch. The implementation returns
fresh zero-offset copied storage and leaves the source typed array unchanged
after returned-copy mutation. The focused fetch/WASM chain spec now passes
