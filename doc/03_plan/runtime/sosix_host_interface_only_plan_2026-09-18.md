...
| AC acceptance `sosix_runtime_unification_acceptance_spec` | 8/8 | **7/8 on a clean checkout** (8/8 only with the two files present) | **broken since landing**. The AC-5 example reads `doc/10_metrics/runtime/sosix_unification_{perf_report,baseline}_2026-09-05.md`. Lane commit `4bd22ad051f` never committed either file. They existed only as untracked files in the main worktree and in the unmerged commit `2f5261a96a4` (branch `codex/opt-g0-20260910`). They are restored as **untracked** files in this worktree and are NOT committed. Needs a decision: commit them as spec fixtures (against the "no reports in git" default), or change the spec to not read `doc/10_metrics`. |
