...
BrowserSession combined streaming import-call continuation:

Completion checklist:

- Add a BrowserSession browser-script scenario that queues both
  `WebAssembly.compileStreaming(window.fetch('/compile.wasm'))` followed by
  `WebAssembly.instantiate(module, imports)` and
  `WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm'), imports)`
  for the same imported-function module in one script.
- Verify the initial script result is `queued` and the shared `out` variable is
  empty before any network response is committed.
- Verify the first pending network request is a fetch for
  `https://example.com/compile.wasm`.
- Commit the imported-function module response for `/compile.wasm` and verify
  the compile-streaming path reports import count `1`, module byte length `52`,
  exported `run(38)` result `42`, and shared import-call count `1`.
- Verify the second pending network request is a fetch for
  `https://example.com/instantiate.wasm`.
- Commit the same imported-function module response for `/instantiate.wasm` and
  verify the final output keeps the compile-streaming evidence and appends
  instantiate-streaming import count `1`, module byte length `52`, exported
  `run(38)` result `42`, and shared import-call count `2`.
- Regenerate the mirrored scenario manual for the changed SPipe spec and move
  docgen's old `01_unit` output onto the tracked `unit` manual path.
- Remove generated docgen noise from adjacent specs, restore generated tracking
  index churn, and keep only the intended source/manual/plan files in the commit.
...
BrowserSession scripts now compare both streaming entry points against the same
imported-function module and shared imports object in one queued script. The
scenario proves the first network commit completes only the compile-streaming
chain, then the second network commit appends the instantiate-streaming chain
while preserving shared import-call count evidence. The focused fetch/WASM chain
spec is now passing `198/198`; the native WASM host spec remained `107/107`,
the WebGPU JS/WASM system spec remained `106/106`, Node API conformance remained
`275/275`, and `src/lib` completed with the current `399 warning(s)` across
`5910` files. Diff hygiene and the `doc/06_spec` layout gate passed before
push.

BrowserSession combined streaming export-descriptor continuation:
...
- Add a BrowserSession browser-script scenario that queues both
  ...
  `WebAssembly.instantiate(module)` and
  `WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm'))` for the
  same table/global export descriptor module in one script.
- Verify the initial script result is `queued` and the shared `out` variable is
  empty before any network response is committed.
- Verify the first pending network request is a fetch for
  `https://example.com/compile.wasm`.
- Commit the table/global module response for `/compile.wasm` and verify the
  compile-streaming instantiated descriptor path reports status `instantiated`,
  module byte length `40`, descriptor count `2`, and descriptor pairs
  `tbl:table` and `answer:global`.
- Verify the second pending network request is a fetch for
  `https://example.com/instantiate.wasm`.
- Commit the same table/global module response for `/instantiate.wasm` and
  verify the final output keeps the compile-streaming evidence and appends
  instantiate-streaming status `instantiated`, byte length `40`, descriptor
  count `2`, and descriptor pairs `tbl:table` and `answer:global`.
- Regenerate the mirrored scenario manual for the changed SPipe spec and move
  docgen's old `01_unit` output onto the tracked `unit` manual path.
- Remove generated docgen noise from adjacent specs, restore generated tracking
  index churn, and keep only the intended source/manual/plan files in the commit.
...
BrowserSession scripts now compare both streaming entry points against the same
table/global export descriptor module in one queued script. The scenario proves
the first network commit completes only the compile-streaming instantiated
descriptor chain, then the second network commit appends the instantiate-
streaming descriptor chain while preserving the descriptor evidence. The focused
fetch/WASM chain spec is now passing `199/199`; the native WASM host spec
remained `107/107`, the WebGPU JS/WASM system spec remained `106/106`, Node API
conformance remained `275/275`, and `src/lib` completed with the current
`399 warning(s)` across `5910` files. Diff hygiene and the `doc/06_spec` layout
gate passed before push.

BrowserSession combined streaming import-descriptor continuation:
...
- Add a BrowserSession browser-script scenario that queues both
  ...
  `WebAssembly.instantiate(module, imports)` and
  `WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm'), imports)`
  for the same imported-function descriptor module in one script.
- Verify the initial script result is `queued` and the shared `out` variable is
  empty before any network response is committed.
- Verify the first pending network request is a fetch for
  `https://example.com/compile.wasm`.
- Commit the imported-function module response for `/compile.wasm` and verify
  the compile-streaming instantiated descriptor path reports status
  `instantiated`, import count `1`, module byte length `27`, descriptor count
  `1`, descriptor tuple `env:foo:function`, and instance exports type `object`.
- Verify the second pending network request is a fetch for
  `https://example.com/instantiate.wasm`.
- Commit the same imported-function module response for `/instantiate.wasm` and
  verify the final output keeps the compile-streaming evidence and appends
  instantiate-streaming status `instantiated`, import count `1`, byte length
  `27`, descriptor count `1`, descriptor tuple `env:foo:function`, and instance
  exports type `object`.
- Regenerate the mirrored scenario manual for the changed SPipe spec and move
  docgen's old `01_unit` output onto the tracked `unit` manual path.
- Remove generated docgen noise from adjacent specs, restore generated tracking
...
BrowserSession scripts now compare both streaming entry points against the same
imported-function descriptor module and shared imports object in one queued
script. The scenario proves the first network commit completes only the
compile-streaming instantiated descriptor chain, then the second network commit
appends the instantiate-streaming descriptor chain while preserving the
descriptor evidence. The focused fetch/WASM chain spec is now passing
`200/200`; the native WASM host spec remained `107/107`, the WebGPU JS/WASM
system spec remained `106/106`, Node API conformance remained `275/275`, and
`src/lib` completed with the current `399 warning(s)` across `5910` files. Diff
hygiene and the `doc/06_spec` layout gate passed before push.

BrowserSession combined streaming memory export continuation:
...
- Add a BrowserSession browser-script scenario that queues both
  ...
  `WebAssembly.instantiate(module)` and
  `WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm'))` for the
  same exported-memory module in one script.
- Verify the initial script result is `queued` and the shared `out` variable is
  empty before any network response is committed.
- Verify the first pending network request is a fetch for
  `https://example.com/compile.wasm`.
- Commit the memory module response for `/compile.wasm` and verify the
  compile-streaming path reports status `instantiated`, module byte length `25`,
  grown memory byte length `131072`, page size `65536`, wrapped byte value `4`,
  grow return `1`, grown view length `131072`, and preserved byte value `4`.
- Verify the second pending network request is a fetch for
  `https://example.com/instantiate.wasm`.
- Commit the same memory module response for `/instantiate.wasm` and verify the
  final output keeps the compile-streaming evidence and appends instantiate-
  streaming status `instantiated`, byte length `25`, grown memory byte length
  `131072`, page size `65536`, wrapped byte value `6`, grow return `1`, grown
  view length `131072`, and preserved byte value `6`.
- Regenerate the mirrored scenario manual for the changed SPipe spec and move
  docgen's old `01_unit` output onto the tracked `unit` manual path.
- Remove generated docgen noise from adjacent specs, restore generated tracking
  index churn, and keep only the intended source/manual/plan files in the commit.
...
BrowserSession scripts now compare both streaming entry points against the same
exported-memory module in one queued script. The scenario proves the first
network commit completes only the compile-streaming memory chain, then the
second network commit appends the instantiate-streaming memory chain while
preserving independent grow and byte-coercion evidence. The focused fetch/WASM
chain spec is now passing `201/201`; the native WASM host spec remained
`107/107`, the WebGPU JS/WASM system spec remained `106/106`, Node API
conformance remained `275/275`, and `src/lib` completed with the current
`399 warning(s)` across `5910` files. Diff hygiene and the `doc/06_spec` layout
gate passed before push.
