...
3. No config to target tui/gui/both with one body. → Add a target config + a
   parity check for `both`.
4. Headless GUI needs a heavy `Compositor` fixture (per `gtti_spec.spl`).
   → Add an ergonomic snapshot provider to amortize fixture cost.
...
`/api/test/draw-ir`, `?id=`, `/diff`, `/layout`) and **Phase 6** (`expect_draw`)
read the in-process snapshot substrate. They are **blocked on Phases 1–2 here**:

- `expect_draw` is the SGTTI driver's assertion vocabulary extended with
  layout/geometry fields (Protocol v2, capability-gated).
...
The Draw IR model/SDN/Draw.io work (its Phases 1–3) is independent and can run in
parallel.
...
5. Architecture (full + TLDR) and this plan in sync; Draw IR plan cross-link
   recorded; pushed.
