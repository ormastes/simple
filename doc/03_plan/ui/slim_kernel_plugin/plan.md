...
|---|---|---|---|
| 0 | A00 ledger + A01 harness | `.spipe/ui_slim_kernel_plugin/`, `test/helpers/ui_slim/`, `test/05_perf/ui_slim/`, `scripts/check/check-ui-slim-closure.shs` | harness rejects blank GUI, TUI/GUI mislabel, stale binary, concurrent build; closure gate reads link map/`deps`, not log absence |
| 1 (smallest useful slice) | A03 screen batching + frame builder; P08 thin TUI route adapter; old/new differential specs | `src/app/ui.tui/screen.spl`, new `src/app/ui.tui/route_adapter.spl`, `test/01_unit/app/ui/screen_batching_spec.spl` | one-cell parity corpus (ASCII/Korean/combining/wide/ANSI/negative/zero/right-edge); row-copy + alloc counts down; `async_app` closure excludes `os/drivers`,`os/kernel`,`lib/skia` in ordinary mode |
| 1 | A02 composition adapter | `src/lib/nogc_sync_mut/ui/composition_adapter.spl`, `test/01_unit/lib/ui/composition_adapter_spec.spl` | static and dynamic routes equivalent; sabotage: duplicate id, wrong ABI, missing interface, release-while-live |
