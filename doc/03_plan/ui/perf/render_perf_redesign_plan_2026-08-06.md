...
- [x] **A1 — physical adapter attribution:** the durable physical-window report
  names the NVIDIA RTX A6000, records device type plus
  vendor/device/driver/API identity, and binds that exact fingerprint to stable
  selected-device identity hash `666008366`. This closes attribution only;
...
| **F1** class/reference semantics | **PARTIAL — focused seed-interpreter receipt (2026-08-12)** | Plan `a4eb22fa77d` → `doc/03_plan/ui/perf/f1_class_identity_kind_propagation_plan_2026-08-09.md` (**not** under `doc/03_plan/compiler/`). Corpus + JIT struct-alias defect `47a162e079a`. The seed interpreter now has a distinct `Value::ClassInstance` shared identity carrier and routes construction, field reads/writes, receiver dispatch, optional/array/parameter/return paths through it; structs retain `Value::Object` copy-on-write semantics. The actor lifetime blocker was resolved by its owner and `interpreter_method` now imports the canonical identity-preserving self helper. `cargo test -p simple-compiler --test class_instance_interpreter_semantics` passes **1/1**, covering direct/trait/optional/array/parameter/return aliases plus a struct-COW control. This is seed compiler correctness evidence only: JIT/native struct-copy validation, the full F1 corpus/matrix, and provenance-matched self-hosted validation remain open. |
...
| **C2** typed forwarding | **PARTIAL — typed model/test-only** | `HirForwardDecl` and its reconstruction oracle exist, but production `ParserModule`/`ParserClass` discard `alias fn/me` declarations and no HIR/driver pass receives them. The next safe slice is parser-owned `ParserForwardDecl` capture, then structured HIR lowering; raw-source rescanning must not become the production contract. |
...
| **C4** effect verifier | **PARTIAL — production HIR wiring landed** | `src/compiler/35.semantics/effect_validation.spl` drains real parsed effect claims, extracts loop/copy/direct-call facts from lowered HIR, and the canonical post-HIR driver hook turns violations into compile errors. `effect_attribute_end_to_end_spec.spl` covers source parse → HIR → validation controls. Indirect-call and non-literal induction proofs remain fail-closed/open work; an admitted self-hosted compiler execution receipt is still required. |
...
| **O2** damage / raster skip | **PARTIAL (raster-skip plus conservative retained CPU execution)** | `paint_chunks_raster` → `RasterStats{rastered,skipped,bytes_painted}`; quiescent frame rasters 0. `paint_chunk_draw_ir_lowerer.spl` additionally creates a fail-closed, full-frame DrawIR list from every valid chunk using a producer-owned stable ID; it preserves source order and deliberately consumes no visibility result. A separate stable-ID DrawIR transaction now stages a complete target order and validates revision continuity, duplicate/missing identities, required upserts, and tombstones before publishing; invalid work leaves the prior command list unchanged. `chunk_occlusion` fails open on column/capacity mismatch rather than truncating input. These are prerequisites only: no production PaintChunk producer/caller invokes the transaction or emits exact opaque proofs, so visibility culling remains disabled. An ownership audit found that live Web/GUI/WM producers emit DrawIR directly and have no retained PaintChunk/proof frame; post-DrawIR reconstruction would be unsound. `draw_ir_patch.spl` can atomically apply stable insert/remove/reorder updates while preserving target batch boundaries; composition damage now classifies compatible multi-batch updates exactly. `engine2d_draw_ir_render_composition_damage_with_images` clears and replays exact non-overlapping LOCAL regions for direct opaque CPU/bare batches, returning the authoritative updated Engine2D; device/offscreen/translucent/parent-sampling batches fail closed. Hosted WM forwards its dirty region to a persistent compositor-owned 256/64/32 scheduler after a same-session, same-viewport p95 calibration from successful full frames; all unmeasured/deferred/unsafe plans retain canonical full replay. Web production entry points route through a persistent compositor-owned retained cache: exact revisions are NONE, and LOCAL requires an identical producer generation, backend, transfer profile, prior complete result, and resources; every mismatch fails closed FULL. The accepted child receipt now crosses `WmContentFrame`: outer WM validates revision/resource continuity, rejects overlap, translates LOCAL damage in i64 and clips before narrowing; a validated NONE frame remains a no-present path while all invalid input is full damage. GUI has a bounded per-window Engine2D target owner with exact-revision reuse, LOCAL replay, and shutdown-on-invalidate. Web and GUI focused runners are blocked before verdict by bootstrap-runner defects, so they remain source-reviewed mechanisms only. Gates: same spec, describe (c), 3/3, plus `paint_chunk_rasterizer_spec.spl` 9/9, `draw_ir_delta_spec.spl` 6/6, lowerer 4/4, stable-patch 14/14, composition damage 6/6, retained-damage 4/4, and compositor schedule parity 8/8 under the bootstrap seed. **Production occlusion/visibility (§4 pass 5), a retained PaintChunk producer/caller, producer-owned exact opaque proofs, device/offscreen LOCAL execution, live outer WM partial-present evidence, and admitted native 8K evidence remain open.** Do not read O2 as complete or as performance evidence. |
...
| **O3–O4**, **P0–P5** | **NOT STARTED** | Correctly gated behind §0's milestone. |
| **G1** Vulkan present bridge | **PARTIAL (mechanism only)** | Hosted Winit now opens the Vulkan presentation owner before the Engine2D Vulkan surface and stages only fresh device-readback candidates. `present_damage_with_receipt` is reached only after exact non-overlapping LOCAL/canonical FULL damage validation, increasing frame revision, exact Engine2D-buffer ownership, and stable framebuffer/device/physical-adapter identity; every rejection uses the existing pure-pixel Winit fallback. This is not `VulkanBackend.present()` (which is still storage-buffer → host cache). The `spl_winit` sibling cdylib now exports a fail-closed live-slot Linux Xlib/XCB/Wayland platform/display/surface triple, and the Simple facade resolves it dynamically before requesting an external presenter; invalid/unsupported slots return zero and no caller owns the raw values. Its focused Rust unit test passes 2/2. The canonical `vulkan_graphics_runtime::STATE` owner now validates those triples, enables exact display extensions, creates an owned surface, selects a present-capable device, creates a swapchain, and adopts them before Engine2D allocation; its presenter-unit suite passes 4/4 (invalid triples, exact rectangles, failed-acquire poisoning, forged/registered damage-array validation). The actual transaction validates device-buffer ownership/rectangles, seeds stale images with a full copy, performs acquire → transfer-copy → fence → present, and retains per-image revisions. The public high-level `[i64]` façade now crosses as a registered managed array handle, validated for heap type/layout/integer tuples before Vulkan work; typed interpreter dispatch and the compiler runtime-SFFI table check cleanly. Native archive retention now compiles offline with Vulkan enabled; a completed archive link, typed live receipts, and admitted native evidence remain open. |
| **G0, G2–G4** | **NOT STARTED** | Correctly gated behind §0's milestone. |

### G1 lifecycle addendum — 2026-08-12

The G1 mechanism now additionally has an exact Winit-window lease token, a
portable producer-ready/render-finished semaphore chain, and an
`VK_EXT_swapchain_maintenance1` retirement-fence path. A successful
replacement uses Vulkan's required live `oldSwapchain`; failures after creation
either destroy the new raw swapchain or terminally poison the retired owner, so
it cannot be reused. The runtime Vulkan library compiles offline and this
lifecycle repair received a higher-model source review. Its focused no-WSI
retirement-ordering unit test passes 1/1.

This is still **mechanism evidence only**. The native
`rt_vk_engine2d_presenter_recreate` entry is not yet re-exported through the
runtime facade or wrapped by the Simple presenter, so an out-of-date presenter
has no supported recovery route. The legacy public
`rt_vk_swapchain_recreate` also remains nontransactional and must not be used
by the presenter path. Facade/host resize execution,
focused lifecycle-test verdicts, a native archive link, an admitted
self-hosted runtime, a physical adapter, and a live external WSI receipt remain
required before G1 can be called accepted or used for any 8K/80 claim.

### O2 producer-proof addendum — 2026-08-12

The canonical HTML DrawIR producer now carries a final-composition-aligned
proof record for each command. An `EXACT_RECT` record is possible only from an
explicit primary-node rectangle that remains fully opaque, unclipped, and free
of image/gradient/radius/border/shadow/filter effects across its bounded
ancestor chain and final full-opacity embedding. All canvas, synthetic,
text/image, placeholder, and embedded commands are explicitly `UNKNOWN`.

The sidecar validates its command/batch/surface/embedding alignment but still
does not invoke occlusion or alter execution: culling remains disabled pending
the canonical retained full-frame transaction/consumer. This is source-reviewed
mechanism work, not an O2 execution or performance admission.
