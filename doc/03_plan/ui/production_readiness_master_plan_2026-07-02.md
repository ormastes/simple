...
| G1.1 real window via Vulkan | PARTIAL / FAILING CONTENT ORACLE: private Xvfb + gui-feature driver captures a live window and Vulkan-rendered frame at 96x72 in 96s, but widget readability fails (`ink_coverage=0.00217 < 0.005`); 200x150 still times out before frame under current Vulkan text fallback | `SIMPLE_BIN=src/compiler_rust/target/debug/simple SIMPLE_GUI_VULKAN_W=96 SIMPLE_GUI_VULKAN_H=72 SIMPLE_GUI_VULKAN_APP_TIMEOUT=150 sh scripts/check/check-gui-vulkan-window.shs` | `build/gui-window-evidence/evidence.env`, `showcase_vulkan_window.png`, `showcase_vulkan_offscreen.ppm`, `oracle.log` |
...
| G1.1 showcase Vulkan render | PARTIAL: `assert_vulkan_backend=pass`, `assert_vulkan_frame=pass`, `assert_window_capture=pass`; `assert_widget_content=fail` because the low-res readable-content oracle is below threshold. This replaces the stale pass claim until Vulkan text/readability performance allows an oracle-passing size. | `scripts/check/check-gui-vulkan-window.shs` + `test/03_system/check/gui_vulkan_window_spec.spl` | `build/gui-window-evidence/*` |
