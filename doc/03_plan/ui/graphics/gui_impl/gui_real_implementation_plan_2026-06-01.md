...
- `src/app/gui_perf/linux_smf_dynlib_e2e_gate.spl` and
  `test/03_system/gui/linux_smf_dynlib_e2e_gate_system_spec.spl` provide the
...
perf comparison is stale until `test/05_perf/tauri_equiv/tauri_reference/Cargo.toml`
...
- `SIMPLE_LIB=src bin/simple test test/01_unit/lib/gui/pure_smf_dynlib_perf_spec.spl --mode=interpreter`
- `SIMPLE_LIB=src bin/simple run src/app/gui_perf/smf_dynlib_probe.spl`
- `SIMPLE_LIB=src bin/simple test test/03_system/gui/linux_smf_dynlib_e2e_gate_system_spec.spl --mode=interpreter`
...
- `SIMPLE_LIB=src bin/simple test test/01_unit/os/posix/dynlib_spec.spl --mode=interpreter`
- `SIMPLE_LIB=src bin/simple test test/01_unit/os/smf_runtime_spec.spl --mode=interpreter`
...
- Tauri shell comparison:
  restore `test/05_perf/tauri_equiv/tauri_reference/Cargo.toml`, fix
  `workflow_driver.spl`/`simple_app.spl` argument access for the current CLI,
  then re-run `test/05_perf/tauri_equiv/README.md` workflows or the matching perf
...
- Update `doc/04_architecture/ui/simple_gui_stack.md` to separate pure GUI library,
