...
**Scope:** new `test/03_system/feature/usage/math_block_linalg_spec.spl`  
**Estimate:** 1 day  
...
**Scope:** `test/03_system/feature/usage/math_block_linalg_spec.spl` (extend same file)  
**Estimate:** 1 day  
...
**Scope:** `doc/08_tracking/feature/scilib_perf_sugar.md` update  
**Estimate:** 0.5 day  
...
- [x] `bin/simple test test/03_system/feature/scilib/math_block_matmul_spec.spl test/03_system/feature/scilib/math_block_solve_spec.spl` passes in interpreter mode with `SIMPLE_BLAS_BACKEND=mock`; zero `skip()`, zero TODO→NOTE — verified 2026-09-08 on the rebuilt hosted compiler: matmul 23/23 and solve 40/40, both `SPEC FILE VERDICT outcome=OK`.
...
- [x] `parse("A.T")` produces `App("transpose", [Var("A")])` (unit test) — compiled regression `test_parse_transpose_postfix`; behavioral acceptance transposes a 2x3 literal and verifies four non-symmetric cells.
- [x] `inv` and `solve` eval arms green against mock shim deterministic output — verified behaviorally in the 11/11 math-block acceptance run and by the focused solve spec's 40/40 run.
...
- [x] `A.sum(0)` and `A.mean(1)` reduce along the specified axis; existing full-reduce unaffected — verified 2026-09-08 with exact `[4,6]`, `[1.5,3.5]`, and full-reduce `10` behavioral assertions.
- [x] Rendering: `MatMul` → `@` in text, `\cdot` in LaTeX; `Slice` → `A[i:j,k]`; axis-reduction → correct text/LaTeX — compiled renderer regression functions plus the rebuilt compiler's 11/11 acceptance run.
...
- [x] Zero primitive type leaks at any public-facing surface touched by this area — closed 2026-09-06: the single offender was `src/lib/common/science_math/math_block_ops.spl:67` `pub fn op_scalar_mul(a: MbMatrix, scalar: f64)`. It now takes `MbScalar`, a single-field wrapper newtype added at `src/lib/common/science_math/math_block.spl` (the same newtype boundary `Float64`/`Index` establish in the ndarray area; `Float64` itself lives in `nogc_async_mut` and importing it here would invert the layer order). Verified: `grep -rEc 'pub fn [^\n]*: *(f64|i64|f32|i32)\b' src/lib/common/science_math/math_block.spl src/lib/common/science_math/math_block_ops.spl` → 0, and `scilib_port_math_block_spec.spl` is 11 examples / 0 failures. Planted control: reverting the parameter to `f64` puts the example back to `expected 1 to equal 0` (11 examples, 1 failure); restoring it returns 11/0.
