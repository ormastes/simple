...
**Acceptance:**  
- Measurement result written to the PERF-SUGAR-011 entry in `doc/08_tracking/feature/scilib_perf_sugar.md` (update Status field and Current-cost field).  
...
- [ ] No primitive type (`f64`, `i64`, `f32`, `i32`, `u64`) in any public `fn` signature or exported `struct` field — **RE-OPENED 2026-09-06.** The `fn_hits=0` half of the 2026-09-05 verification below was earned by prefixing four public methods with `_` (`NDArray._flat_f32/_flat_f64/_flat_i64/_flat_bool`) plus the free function `_ndarray_sort_value_less`. The audit's carve-out treats a leading `_` as non-public, so the count went to 0 — but **388 call sites in 46 files still call `.flat_f64(` / `.flat_f32(` / `.flat_i64(` / `.flat_bool(`**, including 20 spec files, so those methods are public by any honest reading and the rename simply broke them: every ndarray/df/scipy caller failed with `semantic: method flat_f64 not found on type NDArray`. The names have been restored (2026-09-06), which is why 14 of the 16 ndarray specs went green and REQ-07 below now passes; the audit is therefore honestly RED again at `fn_hits=5`. The real remedy is `linear: Index` instead of `linear: i64` on the four `flat_*` methods and `Index` on both parameters of `ndarray_sort_value_less`, which means rewriting all 388 call sites (all of them take a paren-free argument, so it is mechanical) across `src/lib/nogc_async_mut/{ndarray,linalg,df}`, `src/lib/nogc_sync_mut/df`, `src/lib/scipy/**`, and BOTH mirrored test trees (`test/03_system/feature/scilib` and `test/feature/scilib`, which the test-tree divergence guard pairs). That crosses the df and scipy lanes and needs a cross-lane decision before it is attempted. Do NOT close this by re-adding the `_` prefix or by breaking the signature across lines — both defeat the detector rather than satisfying the checkbox, and the first one has already been reverted once (`ca749722236`). Superseded verification, retained for history: `src/compiler_rust/target/debug/simple run test/03_system/plan_acceptance/scilib_port_ndarray_spec.spl` 2026-09-05: REQ-SCILIB-NDARRAY-06 green, `fn_hits=0`, non-newtype primitive `field_hits=0`, wrapper-newtype boundary control `=5`, both planted-offender controls `=1` (including the `value` -> `V` rename evasion); no `E1034` in the transcript. Struct-field audit is now structural — the five primitive fields that DEFINE the wrapper types (`Float64 { value: f64 }`, `Index { value: i64 }`, ...) are the checkbox's own remedy and are exempted as single-field newtypes, not by name; every other primitive field is still a violation.
