...
- [x] Mock structural invariants verified (non-negative singular values, orthogonal mock U/Vt shapes, 0-based Pivot.at assertions) — verified 2026-09-08 by behavioral `gesvd(diag(3,-2))` assertions over both U columns and Vt rows plus the direct 1-based-to-0-based pivot conversion; no file-absence proxy remains.
- [x] `NotConverged` and `Singular` error paths covered by specs — verified 2026-09-08: the compiler now resolves the explicit `LapackError` enum alias through its `EnumType` binding even beside the conflicting `std.linalg.*` glob, the rank-deficient mock `gesv` matches `Singular`, and `syevd_with_sweep_limit(..., Index.new(0))` deterministically matches `NotConverged`. Focused Rust alias regression passed and the strengthened LAPACK acceptance spec returned exit 0.
