...
2. Each discovered device negotiates modern VirtIO (`VIRTIO_F_VERSION_1`),
   owns queue 0, posts 32 writable eight-byte evdev records, drains used
   entries, acknowledges interrupts, and requeues the consumed descriptor.
3. `src/os/kernel/arch/arm64/virtio_input.spl` exposes only raw events.
   ...
   device, pointer state, and post-render markers.
