...
- 2026-05-26: Added proof-gated `CLibParityHotspot` rewrite eligibility in
  ...
  coverage in `test/01_unit/compiler/mir_opt/clib_parity_hotspot_spec.spl`. Command:
  `src/compiler_rust/target/debug/simple test test/01_unit/compiler/mir_opt/clib_parity_hotspot_spec.spl --mode=interpreter`.
...
- 2026-05-26: Ran safe board-validation lane without flashing hardware.
  ...
  - `src/compiler_rust/target/debug/simple test test/01_unit/os/simpleos_board_hardening_spec.spl --mode=interpreter --clean`;
    result: pass, 4 tests.
...
- Focused specs:
  - `test/01_unit/os/qemu_runner_spec.spl`
  - `test/01_unit/os/port/simpleos_board_hardening_spec.spl` if present.
...
- Optimization-plugin changes for this track must preserve volatile ordering,
  ...
  be covered by `test/01_unit/compiler/mir_opt/clib_parity_hotspot_spec.spl`.
