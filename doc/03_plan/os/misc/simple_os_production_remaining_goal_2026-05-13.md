...
- L4 IPC unit coverage exists in `test/01_unit/os/kernel/ipc/l4_fast_ipc_spec.spl`.
- L4 IPC has a discovered regression gate in
  `test/05_perf/ipc_l4_logic_perf_spec.spl`; latest evidence showed queued IPC at
  about 11.5s for 20k iterations and L4 inline paths at about 1.1s.
- Multiarch appscan specs exist under `test/03_system/qemu/os/appscan/` for ARM,
  RISC-V, and x86.
...
- Fix primitive API lint regressions:
  - `test/03_system/code_quality/primitive_api_lint_spec.spl` currently fails because the
...
- Re-run primitive API lint gates after the scanner fix:
  ...
  - `test/03_system/code_quality/primitive_api_canary_spec.spl`
