...
- `src/lib/nogc_sync_mut/js/engine/runtime.spl`,
  ...
  drained automatically on `resume()`. Current focused checks pass
  `node_api_conformance_spec.spl` `226/226` and
...
- CommonJS/Node APIs: extend beyond builtin module cache, deterministic
  ...
  drain on `resume()`. Full flow control, `for await` syntax support, broader
