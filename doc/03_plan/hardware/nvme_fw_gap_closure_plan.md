...
| P9 | `fw_rv32/entry.spl` | bare-metal rv32 ISA reference (re-expresses RAIN, ECC, fixed scheduler, fixed power/thermal, fixed map-cache, fixed band, fixed journal-ring, fixed HIL, fixed admin, fixed reactor, fixed policy/target, and fixed DRAM/durability floors array-free; `check`-clean + host-verified) | reference wired through rv32 boot hook; full 22-module no-alloc firmware port remains the ceiling |
...
> reconstruction, SECDED ECC floor, fixed scheduler floor, fixed power/thermal floor, fixed map-cache floor, fixed band floor, fixed journal-ring floor, fixed HIL command/queue floor, fixed admin floor, fixed reactor floor, fixed policy/target floor, and fixed DRAM/durability floor, wired through the optional rv32 boot hook. That is useful P9 evidence, not the full firmware port. The full 22-module
