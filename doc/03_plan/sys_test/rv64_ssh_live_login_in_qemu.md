...
| SSH capture | `bin/simple test test/01_unit/os/kernel/scheduler/process_stdout_capture_spec.spl --mode=interpreter --clean --sequential` then `bin/simple test test/01_unit/os/apps/sshd/ssh_production_exec_contract_spec.spl --mode=interpreter --clean --sequential` | success, empty/nonzero, truncation, stale-reset, typed SSH response |
