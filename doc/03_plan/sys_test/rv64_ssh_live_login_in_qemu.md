...
| AC-5 | Live process-owned WM plus correlated presented frame | SOURCE INTEGRATED; execution BLOCKED | PID/liveness receipt, compositor receipt, QMP capture metadata | PM/scheduler + compositor owners | root |
| AC-6 | Shared interfaces and exact/adjacent fail-closed SSpec coverage | SOURCE IMPLEMENTED; execution BLOCKED | focused unit and SSpec results | RV64 gate sidecar | root |
| AC-7 | Manual-first steps, zero stubs, seven-score maintain review, mirror and requirement traceability | SOURCE IMPLEMENTED; generation/review BLOCKED | scan output, preview/rollback record, generated manual | docs/SPipe sidecar | root manual reviewer |
