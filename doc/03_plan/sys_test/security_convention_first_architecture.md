...
- REQ-016: Simple unit tests `keeps explicit task security contexts isolated` and `restores previous task security context after scoped block` in `test/01_unit/lib/security/security_support_spec.spl`.
- REQ-017: Simple unit test `test/01_unit/lib/http_server/security_context_dispatch_spec.spl` plus remote header tests in `security_support_spec.spl`.
