...
| current `<label>` lane | Selected native inline-flow behavior with authored block override | Four exact semantic/style/layout/DrawIR/Engine2D steps, explicit-inline parity, two block controls, and current handwritten mirror | Source/spec/manual evidence only; label activation is covered separately and qualified execution/docgen remain unavailable |
...
`details`/`summary`, `figure`, `menu`, `small`, `abbr`, `time`, and `label` lanes
reclassify those twenty-one named rows into their own fallback rows, leaving 59 in the
...
| Partial remaining | 59 | Tag appears in text/grouped render coverage but lacks isolated end-to-end proof; excludes `code`,`kbd`,`samp`,`var`,`hr`,`fieldset`,`legend`,`dl`,`dt`,`dd`,`article`,`blockquote`,`header`,`details`,`summary`,`figure`,`menu`,`small`,`abbr`,`time`,`label` |
| Bounded selected-profile fallback | 21 | `code`,`kbd`,`samp`,`var`,`hr`,`fieldset`,`legend`,`dl`,`dt`,`dd`,`article`,`blockquote`,`header`,`details`,`summary`,`figure`,`menu`,`small`,`abbr`,`time`,`label`; basic UA/cascade/DrawIR/pixel paths covered, while special formatting and aggregate conformance remain Partial/RED |
...
| REQ-002/004/019/021 | HTML Partial 59 remaining; twenty-one bounded rows split below | Partial | grouped text/tag and bitmap matrices; exact scenario mapping missing | same canonical owners | per-group identity, UA defaults, geometry, DrawIR, pixels | new grouped specs/manuals required | RED |
...
| REQ-002/003/004/021 | HTML `label` selected profile | Bounded native inline-flow behavior in current lane | `label_element_rendering_spec.spl`: `should lower the label UA inline flow through Draw IR to pixels` | tokenizer/tree, canonical UA defaults/cascade, Web layout, DrawIR text owner, Engine2D | semantic parentage, inline default, authored block override, exact geometry/advances, inline-span full-frame parity, block inequalities, and exact red pixel | handwritten current mirror; label activation covered separately; qualified execution/docgen unavailable | Evidence-blocked |
...
| REQ-003/004/021 | CSS invalid `display` cascade value | Bounded invalid-value correction in current lane | `test/03_system/feature/web_platform/css/display_invalid_cascade_spec.spl`: `should retain the last valid display through WebIR DrawIR and Engine2D` | shared declaration table plus dispatch/full owners, Style, Web layout, DrawIR, Engine2D | last supported valid winner, hidden command absence, visible controls, exact 32-pixel framebuffer | handwritten mirror; unsupported display/CSS-wide keywords and qualified execution/docgen unavailable | Evidence-blocked |
