...
`details`/`summary`, `figure`, `menu`, `small`, and `abbr` lanes reclassify
those fifteen named rows into their own fallback rows, leaving 65 in the
...
| Partial remaining | 65 | Tag appears in text/grouped render coverage but lacks isolated end-to-end proof; excludes `hr`,`fieldset`,`legend`,`dl`,`dt`,`dd`,`article`,`blockquote`,`header`,`details`,`summary`,`figure`,`menu`,`small`,`abbr` |
| Bounded selected-profile fallback | 15 | `hr`,`fieldset`,`legend`,`dl`,`dt`,`dd`,`article`,`blockquote`,`header`,`details`,`summary`,`figure`,`menu`,`small`,`abbr`; basic UA/cascade/DrawIR/pixel paths covered, while special formatting and aggregate conformance remain Partial/RED |
...
| REQ-002/004/019/021 | HTML Partial 66 remaining; fourteen bounded rows split below | Partial | grouped text/tag and bitmap matrices; exact scenario mapping missing | same canonical owners | per-group identity, UA defaults, geometry, DrawIR, pixels | new grouped specs/manuals required | RED |
...
| REQ-003/004/005/006/007/017/021 | CSS/JavaScript animation frame trace | Bounded initial/intermediate/completed, pause/resume, SimpleScript style-frame, completed-frame reuse, and duplicate-offset source-order cascade behavior implemented | `browser_session_script_css_animation_spec.spl`: deterministic JavaScript trace, `applies CSS from a SimpleScript animation frame through Draw IR`, and `should cascade duplicate keyframe offsets into Draw IR pixels`; `animations_wpt_spec.spl`: `should reuse the completed animation Draw IR after its final frame` | BrowserSession monotonic clock/DOM bridge, keyframe parser/CSS animation instances, render-session cache, canonical HTML layout/DrawIR, Engine2D | exact stage geometry/colors/pixels, scheduler state, same-offset declaration union/winner, and unchanged paint count/checksum after completion | complete manuals; duplicate-offset row integrated `2078c3dfab4`; qualified execution/docgen unavailable | Evidence-blocked |
