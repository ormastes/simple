...
| REQ-002/004/019/021 | HTML `iframe` | Bounded source tranche, not yet qualified | `test/02_integration/rendering/simple_web_iframe_draw_ir_embedding_spec.spl` | canonical child Web composition to DrawIR embedding | order, clip, offsets, materials, exact pixels, deadline/rules | handwritten mirror; legacy pixel parity and qualified execution unavailable | RED |
...
| REQ-003/004/005/006/007/017/021 | CSS/JavaScript animation frame trace | Bounded initial/intermediate/completed, pause/resume, SimpleScript style-frame, completed-frame reuse, duplicate-offset source-order cascade, and implicit per-property endpoint behavior implemented | `browser_session_script_css_animation_spec.spl`: deterministic JavaScript trace, `applies CSS from a SimpleScript animation frame through Draw IR`, and `should cascade duplicate keyframe offsets into Draw IR pixels`; `animations_wpt_spec.spl`: completed-frame reuse and `should synthesize implicit underlying endpoints for one midpoint keyframe` | BrowserSession monotonic clock/DOM bridge, keyframe parser/CSS animation instances, render-session cache, canonical HTML layout/DrawIR, Engine2D | exact stage geometry/colors/pixels, scheduler state, same-offset declaration union/winner, per-property underlying endpoints, discrete incompatible values, and unchanged paint count/checksum after completion | complete manuals; duplicate-offset `2078c3dfab4` and implicit-endpoint `0e4b75b167b` integrated; qualified execution/docgen unavailable | Evidence-blocked |
