...
| `test/01_unit/compiler/mir/mir_exception_cfg_verifier_spec.spl`, `mir_exception_production_gate_source_spec.spl` | Exception-token linearity and production release gate | **Static implemented:** landing pads are unwind-only, tokens have one linear `Resume`, normal/ordinary uses fail, optimizer entry/exit validates every function, and auxiliary visitors/mappers handle the new variants explicitly; executable gate evidence remains pending |
