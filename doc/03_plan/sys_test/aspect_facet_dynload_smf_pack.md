...
| `test/01_unit/compiler/mir/mir_exception_cfg_verifier_spec.spl`, `mir_exception_production_gate_source_spec.spl` | Exception-token linearity and production release gate | **Static implemented:** landing pads are unwind-only; tokens cannot be forwarded; every unwind-region path must terminate in `Resume`; external entries, cycles, non-resuming paths, normal/ordinary uses, unknown opcodes, swapped cleanup arguments, and wrong-typed cleanup locals fail. Optimizer, public `CompilerBackend`, shared post-transform adapter, `CodegenPipeline`, and prepared/final VHDL handoffs validate every function. The MIR body still has no cleanup-entry manifest, so exact cleanup count and reverse registration order remain an explicit open gap; executable evidence remains pending. |
