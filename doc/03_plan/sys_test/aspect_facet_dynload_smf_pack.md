...
| `test/01_unit/compiler/mir/mir_exception_cfg_verifier_spec.spl`, `mir_exception_cleanup_manifest_spec.spl`, `mir_exception_production_gate_source_spec.spl`, `test/01_unit/compiler/backend/mir_cleanup_manifest_adapter_source_spec.spl` | Exception-token linearity, exact cleanup manifest, and production release gate | **Static implemented:** landing pads are unwind-only; tokens cannot be forwarded; every unwind-region path must terminate in `Resume`; external entries, cycles, non-resuming paths, normal/ordinary uses, unknown opcodes, and wrong-typed cleanup locals fail. The typed manifest records owner/lease/guard locals, exact release sites, and reverse execution order; guarded cursor dataflow rejects missing, duplicate, extra, reordered, stale, wrong-argument, and inconsistent-join cleanup. Optimizer, public `CompilerBackend`, shared post-transform adapter, `CodegenPipeline`, VHDL catalog, CUDA/Vulkan/VHDL body adapters, and prepared/final VHDL handoffs preserve or validate every function. Compile-pipeline JSON is covered; production SMF persistence/deserialization is not claimed. Executable evidence remains pending. |
