...
| `test/01_unit/compiler/mir/mir_exception_cfg_verifier_spec.spl`, `mir_exception_cleanup_manifest_spec.spl`, `mir_cleanup_manifest_completeness_source_spec.spl`, `mir_exception_production_gate_source_spec.spl`, `test/01_unit/compiler/backend/mir_cleanup_manifest_adapter_source_spec.spl` | Exception-token linearity, exact cleanup manifest, and production release gate | **Static implemented:** every current unwind landing pad requires exactly one manifest. Landing pads are unwind-only; tokens cannot be forwarded; every unwind-region path terminates in `Resume`; external entries, cycles, non-resuming paths, normal/ordinary uses, and unknown opcodes fail. Manifest owner/lease/presence locals require exact types, one definition, and dominance of each targeting `MayUnwind` call; the owner additionally requires the canonical context bitcast. Guarded cursor dataflow rejects missing, duplicate, extra, reordered, stale, wrong-argument, and inconsistent-join cleanup. All optimizer entrypoints identity-preserve manifested functions; backend adapters and final gates preserve or validate them. Compile-pipeline JSON is covered; production SMF persistence/deserialization is not claimed. Static typing and dominance do not prove that a lease came from the matching real acquisition call/generation, and the real HIR acquisition-to-`MayUnwind` lowering path remains executable evidence pending. |
