...
| `test/01_unit/compiler/mir/hir_mir_unwind_admission_spec.spl` | HIR-effect to MIR-call admission | **Static implemented:** `NoUnwind` maps to an ordinary call; `MayUnwind` with live cleanup builds a typed successor, while no cleanup owner rejects it; missing/conflicting metadata remains fail-closed with live-scope `E-AF007` precedence |
...
| `test/01_unit/compiler/mir/mir_exception_payload_resume_spec.spl` | Landing-pad implementation token and future language exception packet | **Static implementation-token path exists:** the original non-forwarded `ExceptionToken` reaches terminal `Resume` on every cleanup path. Executable source exception-packet identity and backend runtime decoding remain pending |
| `test/01_unit/compiler/mir/mir_exception_cfg_verifier_spec.spl`, `mir_exception_cleanup_manifest_spec.spl`, `mir_cleanup_manifest_completeness_source_spec.spl`, `mir_cleanup_manifest_verifier_spec.spl`, `mir_exception_production_gate_source_spec.spl`, `test/01_unit/compiler/backend/mir_cleanup_manifest_adapter_source_spec.spl` | Exception-token linearity, exact cleanup manifest, and production release gate | **Static implemented:** every current unwind landing pad requires exactly one typed manifest. Facet cleanup manifests require at least one release; only explicitly ordinary unwind manifests may be empty. Landing pads are unwind-only; tokens cannot be forwarded; every unwind-region path terminates in `Resume`; external entries, cycles, non-resuming paths, normal/ordinary uses, and unknown opcodes fail. Manifest owner/lease/presence locals require exact types, one definition, and dominance of every targeting `MayUnwind` call, including shared unwind targets. The owner requires the canonical context bitcast, and the module gate resolves its source Struct symbol to the nominal `AspectExecutionContext` type. Guarded cursor dataflow rejects missing, duplicate, extra, reordered, stale, wrong-argument, and inconsistent-join cleanup. Public optimizer entrypoints identity-preserve manifested functions; raw block mutators are private. Backend adapters and final gates preserve or validate manifests. Compile-pipeline JSON is covered; production SMF persistence/deserialization is not claimed. Static typing and dominance do not prove that a lease came from the matching real acquisition call/generation, and the real HIR acquisition-to-`MayUnwind` lowering path remains executable evidence pending. |
