...
| `W4-SRCH-31` SHA block and quantum parity | lengths 0, 1, 55, 56, 63, 64, 65, 4,095, 4,096, 4,097, and 1,048,576 match the canonical SHA-256 owner for every single split and deterministic randomized partition sequence; frozen domain-separated receipt, candidate, and payload vectors match exactly; negative offset, negative count, and `offset > bytes.len` reject without digest-state advance; padding across one/two final blocks is exact; injected charge failure prevents its compression, while checkpoint failure terminalizes the stream and publishes no digest without requiring rollback of already-compressed internal state; stop is observed no later than the next 4,096-byte quantum and before digest publication; only an executed post-fix PASS qualifies | REQ-SPKC-011, 013–014; NFR-SPKC-001, 011–012, 020–021 |
...
pushed. UTF-8 source review is complete, but `W4-SRCH-29` execution evidence is
incomplete. SHA source static review is complete, but the last focused
`W4-SRCH-31` execution was 4/5. These are open cells: a reviewed source change,
static correction, or earlier partial run cannot substitute for an executed
post-fix PASS of the full oracle above.
