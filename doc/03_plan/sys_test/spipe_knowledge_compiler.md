...
earlier pre-fix `W4-SRCH-31` execution was 4/5 and the latest post-fix cycle 2
was terminated at approximately 3:09 with zero output. Its current status is
`FAIL`. Wave 4S-C remains open: focused component evidence cannot substitute
...
ceiling and was terminated at approximately 3:09 with no output. Status is
`FAIL`; static review, the earlier 4/5 result, and smaller helper vectors do not
admit the 1 MiB row. Two cycles are consumed and no third run is authorized in
this session. Reproduction and acceptance criteria:
