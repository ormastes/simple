...
Status: **RED — the exact SSpecs below do not yet exist or pass.**
...
- `step("Measure the warmed JavaScript VM before repeated dispatch")`
- `step("Dispatch non-retaining DOM listeners through the production browser runtime")`
- `step("Verify escaped Events and captured closures remain live")`
- `step("Dispose navigation and close page-owned JavaScript state")`

Shared helpers:

- `_vm_stats(runtime)`
- `_expect_bounded_after_warmup(before, after)`
- `_dispatch_clicks(session, count)`
- `_expect_event_identity_and_fields(first, second)`
- `_expect_page_vm_disposed(receipt)`

Until implemented, helpers must call `fail("RED: JS VM reclamation not implemented")`.
...
| NFR-WEB-BROWSER-006 | focused 1,000-cycle integration precursor and required 10,000-cycle production gate | After bounded quiescence, live heap and retained browser resources are within 10% of post-warmup baseline; focused deterministic store counts remain exactly bounded. |
...
- every interpreter root-table family is traversed, including bound/native,
  WASM, require cache, Promise, async, timer, host DOM, pending event, return,
  thrown, and temporary host-return values;
...
- stable IDs/generations reject stale-handle aliasing;
- all five counters are exact for objects, functions, environments, globals,
  and properties, and parallel object-property arrays stay aligned.
