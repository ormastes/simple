...
1. Run `cargo clippy --workspace --all-targets -- -D warnings` from
   `src/compiler_rust/`.
2. Run `bin/simple lint test/03_system/code_quality/allow_suppressions_spec.spl test/03_system/code_quality/bare_bool_lint_spec.spl test/03_system/code_quality/primitive_api_lint_spec.spl test/03_system/code_quality/warning_allow_root_cause_cleanup_spec.spl --deny-all`.
3. Verify `test/03_system/code_quality/warning_allow_root_cause_cleanup_spec.spl` stays
