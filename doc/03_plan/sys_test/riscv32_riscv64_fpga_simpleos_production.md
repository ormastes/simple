...
|---|---|---|---|
| Generate RTL | pure-Simple compiler, tracked RV32/RV64 `.spl` roots | non-empty clocked/bus VHDL, source maps, hashes, changing RVFI; RV32 hierarchy includes `soc32_transition`, `soc32_sim_tick`, and `k26_soc32_clocked` with explicit AXI state feedback | stub/fallback, empty architecture, constant RVFI, emitted-string/copied CPU, handwritten product datapath |
...
5. Product-seam simulation through `soc32_sim_tick`, including one-shot DDR
   acceptance, byte strobes, mailbox UART, and Linux-visible RAM bounds.
6. Vivado feedback/flattening shell around `k26_soc32_clocked`, then
   Vivado/KV260/JTAG, Linux-sized DDR path, and proved bidirectional terminal.
...
- The focused test and diagram renderer remain unexecuted because they share
  ...
  acceptance evidence.
- The compiler source-of-truth system case now names the RV32 product hierarchy
  and K26 AXI adapter/root, but it remains unexecuted. Its generated manual is
  stale and must be regenerated by a later uncapped pure-Simple CLI session.
- Open blocker:
