...
- REQ-RFL-011 is satisfied for the same helper set. Backend specs now pin exact VHDL guard structure, slice usage, concat/update expressions, and source-map records, including execute-datapath and branch-datapath selection lowering, instead of relying on partial substring evidence.
