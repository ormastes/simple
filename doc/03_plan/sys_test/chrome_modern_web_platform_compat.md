...
- Given BDD coverage, when renderer SSpec is run, then the `:is()`, `:where()`, `:not()`, simple descendant `:has()`, simple CSS `@layer`, attribute selector/operator, `:empty`, `:first-child`, `:last-child`, and `:only-child` examples should pass.
