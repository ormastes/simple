...
| REQ-014 | eleven executable/manual pairs | zero-stub manuals; guide/notice freshness; evidence-recipe audit | 9/11 canonical manuals now regenerate with zero stubs and preserve module scope prose, but their executable specs remain unadmitted, so 0/11 pairs are accepted; `shared_font_surfaces` and `gpu_font_emission` remain dirty noncanonical drafts pending source settlement and canonical regeneration |
