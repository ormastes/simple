...
| REQ-010 | `gpu_font_emission_spec.spl`, `cuda_generated_font_handoff_spec.spl`, plus portable toolchain checker | five source targets; Vulkan contract; deterministic failures/hashes; native artifact exports the versioned font entry; source-tracked CUDA PTX binds immutable source/version/artifact hashes and program version; canonical construction installs it; tampering rejects before mutation; device readback matches the CPU oracle | stable pure-Simple emitter command, source-tracked CUDA trust tuple, exact pinned-Vulkan identity, install/tamper gates, and conditional device scenario are source-complete; retained canonical runner and CUDA device-readback PASS remain pending |
