...
| LDR-01/02, PRV-01, BYT-01, XLG-01, CMP-01 — Build11 prerequisite | Build the planner with compiler sources and runtime authority from one revision; the retained r2 compiler is incompatible because it still emits removed `rt_array_data_ptr_u8`. Produce a typed Stage-3 receipt, then use a Stage-3-only current-source bootstrap path and run one source-frozen debugger-backed diagnostic cycle. | Do not repeat the capped r3 command. In a fresh session, first prove the planner executable contains the real `rt_mem_snapshot_open/record/close` providers and has zero undefined `rt_array_data_ptr_u8`; emit a `//bootstrap:stage3` receipt. Then use the planned `--stop-after-stage3` flow (or an independently reviewed equivalent) so no Stage 4 runs. | `build-fresh-cycle1.log` (timeout), `build-fresh-cycle2.log` (snapshot undefineds), `build-fresh-cycle3.log` (removed-pointer undefined), coherent-authority hashes, planner receipt, Stage 2/3 logs/provenance, candidate hash | compiler/runtime authority owner | highest-capability reviewer |
