...
| LDR-01/02, PRV-01, BYT-01, XLG-01, CMP-01 — Build11 prerequisite | Link-verify the restored `defer_unsupported_marker` header with a focused guard and resume the retained 856-entry planner cache; produce the mandatory receipt; finally run one source-frozen debugger-backed Stage-3 diagnostic cycle | First rebuild `build/restart12-build11-a-r3/planner/simple` from the retained cache and run it as `<current capable pure-Simple CLI> build bootstrap --bootstrap-reason=self-host-convergence-check --bootstrap-receipt=$PWD/build/restart12-build11-a-r3/reason.receipt`; only after that succeeds run `env BOOTSTRAP_NATIVE_CACHE_TTL_DAYS=0 SIMPLE_NO_STUB_FALLBACK=1 sh scripts/bootstrap/bootstrap-from-scratch.sh --bootstrap-receipt="$PWD/build/restart12-build11-a-r3/reason.receipt" --full-bootstrap --deploy --backend=cranelift --output=build/restart12-build11-a-r3/output` | Full 856-file compile/link log, focused restored-marker guard, planner-produced receipt, Stage 2/3 logs and provenance manifests, candidate hash | pure-Simple compiler-driver owner | highest-capability reviewer |
