...
`origin/main`. The later argv runtime-closure blocker receipt is integrated at
`a137d7ff2459a728e8891321bdde9b155ed32378`, also reachable from refreshed
`origin/main`. The accepted argv-owner implementation and planner-link evidence
are integrated at `bfac2de350d5138ac62390c9afffbc13233bcfc6`, also reachable
...
Current continuation receipt: a coherent current-revision Rust seed/runtime
pair built successfully, exports all three memory-snapshot providers, and has
zero `rt_array_data_ptr_u8`; a real planner emitted the typed Stage-3 receipt.
The new fail-closed `--stop-after-stage3` route is structurally accepted and
exits after provenance verification, before capability or Stage-4 work. On
the pre-`IfChain` revision, current Stage 2 admitted (859 compiled, zero
failed), but two Stage-3 diagnostics exhausted the 24 GiB guard and exited 139.
The retained phase profile localizes the active file to the 28-arm dispatch in
`pattern_dispatch.spl`, not the coarse `ast_stmt.spl` completion label. Current
`origin/main` now carries the systemic flat `IfChain` repair; its focused spec
attempt timed out in the broad seed test daemon and is not counted as PASS.
The final allowed Stage-3 cycle must rebuild Stage 2 from this new revision and
verify the systemic repair without a leaf rewrite or any Stage-4 substitution.
...
another Stage-3 localization run, the already-proved argv-owner repair must be
carried by a receipt-capable planner. The bounded follow-up planner cycles
proved two successive authority gaps: the Rust bootstrap archive lacked the
real memory-snapshot provider, then the retained Stage-2 compiler lowered the
current linker byte-write path to the intentionally removed
`rt_array_data_ptr_u8`. No planner or receipt exists. The three-cycle cap is
exhausted; the next session must use a compiler/runtime authority from one
coherent revision, prove the planner link, and emit the receipt before
localizing the remaining Stage-3 allocation loop.
...
Fresh bootstrap preflight receipt: committed source `395349e0852` attempted
the canonical `--full-bootstrap --deploy` command once and stopped before any
compiler stage with `bootstrap-policy-error:
malformed-or-untrusted-planner-admission-v2`. The supplied historical v1
receipt is intentionally rejected. The public verifier in
`scripts/check/lib/bootstrap-planner-admission-bound.shs` structurally checks a
29-field v2 envelope and then unconditionally reports
`planner-admission-v2-producer-unavailable`; therefore no full bootstrap,
Stage-3 resume, or Stage-4 continuation can currently start without bypassing
the trust contract. This preflight rejection consumed no compiler fix cycle.
The iterative parser is reviewed and committed but remains unexecuted.
...
| VCS-01 | all | Only intentional lane-A files are committed; locked integration reaches refreshed `origin/main`; tree and lane marker are truthful | Git receipts | PROVED through `bfac2de350d5138ac62390c9afffbc13233bcfc6`; final bookkeeping follows the same locked sequence and unrelated untracked GUI reports are excluded |
...
| BOOT-ADM-01, LDR-01/02, PRV-01, BYT-01, XLG-01, CMP-01 | Implement and admit the non-circular planner-admission-v2 producer specified by `doc/08_tracking/bug/bootstrap_planner_v1_unbound_authorization_2026-08-14.md`; it must run an independently admitted Stage-2-built planner under an owned pre-exec lock and bind the exact 29-field evidence envelope | After that producer emits `/tmp/restart12-r6-stage4-bootstrap-receipt-v2.env`, run `env BOOTSTRAP_NATIVE_CACHE_TTL_DAYS=0 SIMPLE_NO_STUB_FALLBACK=1 sh scripts/bootstrap/bootstrap-from-scratch.sh --bootstrap-receipt=/tmp/restart12-r6-stage4-bootstrap-receipt-v2.env --full-bootstrap --deploy --backend=cranelift --jobs=1 --output=build/restart12-build11-a-r6/output` | Producer source/tests, admitted planner binary/hash, lock/argv/environment/stdout/exit/smoke receipts, v2 envelope, Stage 2/3/4 logs and provenance | bootstrap trust/planner owner | security + highest-capability reviewer |
...
| LDR-01/02, PRV-01, BYT-01, XLG-01, CMP-01 — Build11 prerequisite | Upstream admission v2 invalidates the historical v1 planner receipt. Add a canonical Stage-2 admission manifest and independently verify the actual Stage-2 build output plus the complete immutable source/runtime/helper closure before producing or replaying a Stage-3 planner receipt. The `--stop-after-stage3` ordering and fail-closed guards are structurally reviewed; positive v2 execution remains unproved. The one final current flat-IfChain Stage-3 cycle remains unused. | Do not hand-forge or bypass v2 evidence and do not repeat either pre-IfChain run. Emit and verify a dedicated Stage-2 manifest immediately after sanity; build/replay the planner from that admitted artifact under one owned lock with before/after hashes; only then run the structurally reviewed `--stop-after-stage3` flow once. | coherent-authority hashes; pre-IfChain Stage-2/3 logs and phase profile; admission-v2 prototype focused PASS plus independent security `REJECT`; future Stage-2 manifest, planner receipt, Stage-3 logs/provenance and candidate hash | compiler/runtime authority owner | highest-capability security reviewer |
