...
| Cached Caret live-PTY qualification | Checker/spec/manual implemented; `--case prerequisites` fails closed with `cached_caret_artifact_missing` | Current executable-test blocker; no live PASS or skipped prerequisite |
...
| REQ-LLM-CARET-CLI-HARDEN-006 | Production CLI/provider/session/tool declarations | Direct production unit specs plus CLI process and contract specs | CLI / static-complete, execution blocked | Execute on the qualified self-hosted runtime and cached wrapper |
...
| REQ-LLM-CARET-HIDDEN-008 | Production hidden command admission | `llm_caret_tui_hidden_feature_spec.spl` | Hidden / component coverage, execution blocked | Execute default/enabled/disabled cases without credentials |
...
- Live-terminal manuals link the complete `script(1)` typescripts, driver logs,
  ...
  A missing cached artifact, `script(1)`, `stty`, marker, or restoration row is
  a failure, never a skip. Each child has one fixed 20-second watchdog and no
...
- live PTY qualification proves forced/automatic routing, ANSI-free piped
  ...
  geometry, and failure before terminal mutation when raw entry is unavailable;
...
scenarios, the direct production chat/type and injected-runtime scenarios, and
five live-terminal scenarios, this expanded focused tranche contains 386
modern `should` examples
