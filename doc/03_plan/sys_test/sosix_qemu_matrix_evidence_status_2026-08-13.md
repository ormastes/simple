...
| Windows | all six | blocked on target-host and four guest contracts | Every guest has a distinct bounded `/SOSIXNON.TXT` echo. Only x86_64 and ARM32 have the complete ordered workload/listing/program/reap source contract; x86_32, ARM64, RV32, and RV64 fail closed with `guest-run-contract-not-implemented`. No path has run on Windows, and RV64 also requires explicit OpenSBI path/id/version. |
