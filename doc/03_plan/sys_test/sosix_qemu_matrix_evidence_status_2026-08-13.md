...
| FreeBSD | all six | blocked, target-host execution required | Obtain the checksum-pinned FreeBSD image, run Linux bootstrap smoke, then on FreeBSD run `sh scripts/check/check-sosix-qemu-matrix.shs --host freebsd --all-guests --run --parallel`. |
