...
| Linux | x86_32 | blocked | CPL3 entry/GDT/TSS/token/trap/scheduler owners are source-present and `esp0` is task/generation-bound; the retained ELF predates that correction and fails the strong linked-symbol gate. Rebuild once with an admitted Stage-4 CLI before a canonical run. |
| Linux | arm32 | blocked | EL0 entry/vector/SVC/token/reap owners are source-present and the retained ELF passes the linked gate; no fresh canonical target listing/program/exit-37/reap bundle exists. |
...
| Windows | all six | blocked, target-host and guest work required | `scripts/check/check-sosix-qemu-matrix.ps1` has six descriptors and a producer-backed x86_64 `-Run` path. The other five rows fail closed because their guests lack the distinct `/SOSIXNON.TXT` echo; no path has been run on Windows. Resume serially with `-AllGuests -Run` to retain explicit blockers, then add guest readers and canonical bundles. RISC-V64 also requires explicit OpenSBI path/id/version. |
