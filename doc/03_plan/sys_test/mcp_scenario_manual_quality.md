...
|---|---|---|
| `test/integration/app/mcp_stdio_integration_spec.spl` | Primary operator flow for stdio MCP server behavior | `show` |
| `test/feature/app/mcp_protocol_runtime_spec.spl` | Protocol capability matrix in one session | `folded` until split into user flows |
| `test/feature/app/mcp_cli_passthrough_diag_spec.spl` | Diagnostic regression flow | `show` for one primary diagnostic, `folded` for variants |
| `test/feature/app/mcp_protocol_gap_matrix_spec.spl` | Compliance inventory and plan locks | `folded` / `detail` |
| `test/integration/app/app_mcp_intensive_spec.spl` | Stress/intensive loops and matrix checks | `folded` by default |
...
`doc/08_tracking/feature_request/sspec_scenario_manual_requests.md`, especially:
