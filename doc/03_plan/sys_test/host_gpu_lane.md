...
| HGL-013 | HIR and MIR lowering preserve `target.later(max_packet: N) gpu \:` lane boundaries as explicit metadata and marker instructions | `test/01_unit/compiler/hir/host_gpu_lane_hir_lowering_spec.spl`; `test/01_unit/compiler/mir/host_gpu_lane_mir_marker_spec.spl` | preserves target.later gpu lane metadata in HIR; emits begin and end markers around target.later gpu lane body | Full for Pure Simple HIR/MIR lane metadata carriers; runtime queue-packet codegen pending |
