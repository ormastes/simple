...
- `SIMPLE_BINARY` names the existing Simple runtime used by the app gate.
...
- Stale evidence, persisted-store act, and source fallback fail. A full
  deployment rebuild is not required for an app-development system gate.
...
| `formula-avg` | show | REQ-003 | compatibility edge | tui+protocol |
