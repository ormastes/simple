...
- `SIMPLE_LIB=src bin/simple spipe-docgen test/feature/usage/math_blocks_spec.spl --output /tmp/docgen-check`
  ...
  `val result = m{ 2 + 3 }`.
- `src/compiler_rust/target/debug/simple test test/feature/usage/math_blocks_spec.spl --mode=interpreter --no-cache`
