...
```bash
# Files to update:
# - doc/07_guide/gpu_configuration.md (change GPU → CUDA(0) in examples)
...
bin/simple run test_gpu_config.spl 2>&1 | grep -i "parse"
...
```bash
bin/simple run examples/torch/basics/01_tensor_creation.spl 2>&1 | tee test_01.log
bin/simple run examples/torch/basics/02_tensor_operations.spl 2>&1 | tee test_02.log
bin/simple run examples/torch/basics/03_device_selection.spl 2>&1 | tee test_03.log
bin/simple run examples/torch/training/xor_pytorch.spl 2>&1 | tee test_04.log
bin/simple run examples/torch/training/mnist_classifier.spl 2>&1 | tee test_05.log
```
...
# 3. Run examples
bin/simple run examples/torch/basics/01_tensor_creation.spl
