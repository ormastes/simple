...
The traceability admission row owns only `MCI-DOC-003`, the focused negative
quality contract. `MCI-DOC-001/002` remain blocked until real docgen evidence
proves the zero-stub manual and displayed-helper visibility; the traceability
producer and aggregate must not convert that blocked metadata into a PASS claim.
