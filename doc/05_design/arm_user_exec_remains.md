...
- ARM64 uses the same high-level user-entry bridge shape as x86: arch code asks the scheduler for a handoff-ready user task and calls one runtime helper with entry, stack, status, and address-space root. The current ARM64 fs-exec closure now performs a live `eret` into the loaded proof ELF and handles its `svc #0` exit through the lower-EL AArch64 synchronous vector.
...
- Generalize the ARM64 SVC/trap runtime beyond the proof ELF's `svc #0` exit path so normal syscall dispatch can replace the current proof-specific exit handler.
