...
- ARM64 QEMU must show the virtual-entry preflight passed before live direct entry.
- ARM64 QEMU must show the user-entry handoff probe succeeded.
- ARM64 QEMU must show live EL0 `eret` entry and SVC exit handling succeeded.
...
- Activate the verified ARM64 user page table while SCTLR_EL1.M is enabled after kernel/user TTBR split, TCR/MAIR/SCTLR setup, rollback, and fault behavior are proven, then replace the direct physical task entry with normal ELF virtual entry.
