# Simple DB Engine Design

- **Feature:** `simple-db-nvfs-storage` (SStack `.sstack/simple-db-nvfs-storage/`)
- **Phase:** 3-arch
- **Status:** v0 — Phase 3 design deliverable
- **Last-updated:** 2026-04-18
- **Author:** Simple DB track (Claude solo; Codex unavailable per state file)
- **Related:**
...
**Simple DB** is a PostgreSQL-compatible relational engine for Simple that:
...
- Uses the 7 forks: `main / fsm / vm / pmap / wal / temp / blob` (4 PG-native, 3 Simple DB-native).
...
- Consumes NVFS through the S1–S7 API contract in `./nvfs/from_simple_db.md`.
...
Per CLAUDE.md "MDSOC+ by default" and the 1-dev state's MDSOC+ placement, Simple DB lives in these namespaces:
...
| Depends on Simple DB | Why |
|---|---|
| `src/app/simple_db` (symlink) | CLI binary |
| `test/` specs | Phase 4 SPipe harness |

**No circular dependencies.** Verified: NVFS has no dependency on Simple DB; ECS has no dependency on Simple DB; `simple_db_if` has no dependency on the `engine` impl (traits-only).
...
| `cap_simple_db_admin` | CLI only | admin operations (vacuum, checkpoint-now) |

The Simple DB capsule never forwards `cap_nvme_ns_open` or any raw NVFS driver capability — all I/O is mediated by the NVFS capsule.
...
Per MDSOC+ for userland, Simple DB uses the ECS primitives in `src/lib/nogc_sync_mut/ecs/`.
...
Outer calls are the MDSOC module boundary — any caller crossing the Simple DB capsule uses these. Inner helpers are private to the ECS business layer and never escape.
...
- Simple DB writes a FULL_PAGE_IMAGE to WAL on first modification after checkpoint.
...
6. Drop all DB_TEMP arenas (NVFS already does this; Simple DB confirms).
...
**Simple DB HOT-logical:** all tuples in a HOT chain live on the same **logical** page group (same `logical_page_no`, possibly different `page_generation`).
...
BRIN summaries fit naturally with arenas (no rebalancing). Simple DB auto-creates BRIN when:
...
- Low-correlation columns → B-tree at M4. Until M4, Simple DB uses sequential scan for equality lookups on non-BRIN-qualifying columns.
...
5. Drop DB_TEMP (NVFS does this; Simple DB confirms).
6. Open for queries.
...
| Mid-arena_discard | NVFS state atomic; either discarded or not | NVFS handles; Simple DB sees consistent state |

---

## 11. NVFS interface assumptions

This design depends on the upfront fs-contribution in `./nvfs/from_simple_db.md`. The following table maps each engine storage-API call to its NVFS S-entry:
...
- Redo-only recovery.
- CLI: `simple_db init`, `simple_db wal-replay`, `simple_db catalog-dump`.
...
- **OQ-3.** WAL record framing: PG-compat vs simple_db-native CRC32C. **Design decision: simple_db-native.** PG-compat framing sacrifices alignment flexibility; we prefer aligned-append discipline.
...
- **OQ-12.** Catalog bootstrap sequence. Bootstrap relations (pg_class, pg_attribute) live in a dedicated META_DURABLE arena created at `simple_db init`.
