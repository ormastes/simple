# slang Filesystem Requirements for nvfs

- **Audience:** the parallel nvfs (NVMe-aware filesystem) design agent/track
- **Author:** slang track
- **Status:** v0 — initial upfront contribution, derived from the slang master plan (`../slang/slang_master_plan.md`) and the attached research doc
- **Last-updated:** 2026-04-18
- **Companion docs:** `./README.md` (index for this directory), `../slang/fs_requests/` (secondary reactive feature-requests captured during implementation)

## Why this doc exists

slang is being built in parallel with nvfs. The goal of this file is to state — **upfront, before slang hits FAT32 walls** — exactly what filesystem API and semantics the serving engine needs, so nvfs design choices are made with a real client in mind. The existing FAT32 stack is only a bring-up substrate; slang's real performance target is on nvfs.

This doc does **not** specify how nvfs should be implemented (that is nvfs's job). It specifies what slang will call, with what expected behavior and rough perf envelope, and which requirements are load-bearing vs. nice-to-have.

## Workload profile (what slang actually does to storage)
...
## Object class taxonomy slang will request

slang will create objects via `fs_create(class, flags)` with these class values (terms from research doc "Exported APIs" table):

| Class | Semantics slang requires | Example object |
|---|---|---|
...
| `append_only` | Log-style append; read back for resume | `/slang/telemetry/2026-04-18.log` |
| `temp` | Short-lived, best-effort durability, preferred for scratch/sort spill | `/slang/tmp/<request-id>` |
| `kv_spill` (optional) | Large, append-only segments with explicit `fs_trim` at eviction time; reads from multiple concurrent request handlers | `/slang/kv/prefix-<hash>.blob` |
| `mutable` | Small configs and runtime state that can be overwritten; fallback class only | `/slang/state.json` |
...
slang's loader and engine will call the following. **All calls must be safe from async-runtime contexts** (slang lives in `gc_async_mut`; the async HTTP bridge and monoio reactor must not be blocked by any of these).
...
- `fs_trim(obj)` / `fs_trim(obj, ranges)` — explicit deallocate / TRIM. slang calls this at model uninstall and at KV-spill eviction. Must not block.
...
- Must honor a **preferred read granule** returned by `fs_query_caps` (§R5). slang will align all tensor-pack reads to that granule.
...
Either of these must be provided (slang does not care which, but exactly one must be atomic w.r.t. crash and w.r.t. concurrent readers):
...
slang will publish new model versions by (a) writing tensor_pack chunks, (b) sealing them, (c) writing a temp manifest, (d) calling the atomic publish. If neither pattern is supported, slang has to stop the serving engine to swap models — which defeats the `sleep(level=0)` / warm-switch design.

### R4. Durability scopes (load-bearing, P0)

slang needs three distinguishable durability levels, per the research doc's "durability scopes":
...
- `fs_sync(scope=root_durable, epoch)` — media-durable AND the filesystem root/checkpoint publication is also durable. slang uses this only when an atomic manifest publish must survive a crash.

slang commits **never** call the middle or high scopes on the hot decode path. They are only used at publish boundaries.
...
slang will downgrade gracefully (e.g., fall back to copy-based model switch if `supports_atomic_publish=false`) but will emit a structured warning and record a Phase-B1 feature-request.
...
- `fs_unregister_buffer(BufHandle)` — called at slang shutdown / model unload.
- Interop with `src/lib/gc_async_mut/gpu/pinned_pool.spl` (new): slang's pinned pool allocates via OS + `cuMemHostRegister`-equivalent, then registers with nvfs. nvfs must tolerate buffers that are already pinned for GPU transfer (double-registration is legal).
...
slang will tag tensor-pack reads as `(immutable, model_prefetch, prefer_resident)` and KV spill as `(short, background, prefer_evict)`. If nvfs ignores hints entirely, slang still works; if it honors them, model cold-start under mixed load should improve.
...
If nvfs computes per-extent or per-chunk checksums and exposes them, slang will skip its own checksum on the hot path. If nvfs doesn't, slang computes its own over the manifest's checksum tree.

## Non-requirements (explicit)

- slang does **not** need POSIX semantics (no mmap, no seek, no hierarchical permissions beyond owner/class).
- slang does **not** need write-back page cache (its pinned-pool staging is the cache). A filesystem page cache that duplicates this would hurt, not help.
- slang does **not** depend on fsync-at-every-write. Models publish in large epochs.
- slang does **not** depend on zoned namespaces or FDP. Both are accelerators nvfs can adopt later; slang must work on conventional NVMe (`supports_sealed_pin=true` emulated in software is fine).
...
- slang's callers are in `src/lib/gc_async_mut/slang/…` (GC + async + mut).
- The nvfs client shim slang uses should live in `src/lib/gc_async_mut/nvfs_client/` and adapt to whatever the canonical nvfs API is. Keeping it in `gc_async_mut` means slang can use GC-managed handles and awaitable futures. If nvfs's native API is `nogc_async_mut` or `nogc_async_mut_noalloc`, the client shim bridges.
...
1. Will nvfs expose `sealed + pin_extents` semantics on conventional NVMe from day one, or only on ZNS? slang's A1 loader needs it on conventional.
2. What is the buffer-registration model (shared with an io_uring-like ring, or a separate API)? This affects how slang wires its pinned pool.
3. Is `fs_publish` supported or do we use `fs_replace` + `fs_sync(root_durable)`?
4. What's the expected `preferred_read_granule` range? slang currently plans 2 MiB tensor chunks.
5. Will nvfs ship a user-space library binding slang links against, or a syscall ABI through a capability handle?

Please reply in this file (inline) or by creating `./nvfs_design.md` and linking responses there; slang will update R1–R9 based on the answers.

---

## R1 Append-Only Object Class — Concrete Spec

- **Status:** appended 2026-04-18 by slang A2-packer Phase 3 architect. Scope: make R1's `append_only` class precise enough that the slang packer and the future nvfs adapter can implement/verify it without further clarification.
...
### Adapter conformance test (slang-maintained)

The slang track will ship a spec under `test/01_unit/lib/gc_async_mut/slang/nvfs_client/` that exercises the full state machine against whatever `NvfsClient` implementation is provided. Nvfs implementations pass by making this spec green. FAT32 bring-up adapter is expected to fail `pin_extents` + `hash_on_seal` paths and MUST return `Unsupported` (not silently mis-behave).

---

## R3 Atomic Manifest Publish — Ordering Rules

- **Status:** appended 2026-04-18 by slang A2-packer Phase 3 architect.
- **Relates to:** R3 above, which stated the requirement at the black-box level. This section fixes the **exact sequence** that slang's packer will execute and that nvfs MUST preserve the visibility semantics of.
...
- **nvfs MUST provide both primitives.** The current runtime lacks them — tracked as `FS-REQ-001-fsync-primitive` under `doc/05_design/slang/fs_requests/`.
...
- POSIX `rename(2)` within a single directory on ext4/xfs/btrfs satisfies this. FAT32 does **not** — it is a delete-then-create sequence with a visible gap. slang's FAT32 bring-up accepts this gap and documents it as a known degraded-mode risk (`FS-REQ-002-fat32-rename-atomicity`).
...
The packer (and any future `slang_pack verify` / cleanup) MUST tolerate the following crash points and recover to a consistent state:
