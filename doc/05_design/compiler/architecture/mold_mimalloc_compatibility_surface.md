...
- `test/01_unit/lib/alloc/mimalloc_spec.spl` covers size classes, allocator trait
...
- `test/01_unit/os/kernel/memory/heap_mimalloc_spec.spl` covers the SimpleOS
  kernel heap pre-init contract and locks the `heap_alloc` to `mi_malloc_raw`
  type boundary plus sized-free null safety.
- `test/01_unit/os/memory/mold_linker_spec.spl` covers current linker discovery,
