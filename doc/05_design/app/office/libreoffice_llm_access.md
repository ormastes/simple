...
- Draw rows advertise SDD-backed read/edit actions including
  ...
  `sdd-node-parent-read`, `sdd-node-child-bounds-read`, `sdd-node-child-count-read`, `sdd-node-shape-read`, `sdd-node-layer-read`,
  ...
  `sdd-style-target-read`, `sdd-style-value-read`, `sdd-style-resolved-value-read`, `inspect-sdd-node`,
