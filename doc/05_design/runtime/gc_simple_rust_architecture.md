...
│ gc_collect(my_gc, "cleanup")               │
└─────────────────┬───────────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────────┐
│ Tier 2: Simple Wrapper (src/app/gc/mod.spl) │
│ - gc_new(), gc_with_limit_mb()             │
│ - gc_collect(), gc_stats()                 │
│ - gc_enable_logging()                      │
│ ✅ Simple code - user-friendly API          │
└─────────────────┬───────────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────────┐
│ Tier 1: FFI Spec (ffi_gen/specs/gc.spl)    │
│ - extern fn rt_gc_new() -> i64             │
│ - extern fn rt_gc_collect(gc, reason) -> i64│
│ ✅ Simple spec - defines interface          │
└─────────────────┬───────────────────────────┘
                  │
                  ▼ (generate with: simple ffi-gen)
┌─────────────────────────────────────────────┐
│ Generated Rust FFI (build/rust/ffi_gen/gc.rs)│
│ #[no_mangle] pub extern "C" fn rt_gc_new()│
│ ✅ Auto-generated - no manual editing      │
└─────────────────┬───────────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────────┐
│ Rust GC Implementation (runtime/memory/gc.rs)│
│ - GcRuntime::new()                          │
│ - Uses Abfall library                       │
│ ✅ Rust lib - performance-critical code     │
└─────────────────────────────────────────────┘
```
...
src/app/
├── gc/
│   └── mod.spl          # Tier 2: Simple wrapper (THIS IS THE API)
└── ffi_gen/
    └── specs/
        └── gc.spl       # Tier 1: FFI spec (defines bridge)

build/rust/ffi_gen/src/  # Generated by: simple ffi-gen
└── gc.rs                # Auto-generated FFI bridge

rust/runtime/src/memory/
└── gc.rs                # Rust GC implementation (Abfall wrapper)
```
...
```rust
// ❌ DON'T: Manual Rust FFI wrappers
#[no_mangle]
pub extern "C" fn rt_gc_collect(...) {
...
```simple
# test/02_integration/gc_integration_spec.spl
import gc
...
✅ **Simple code:** Defines interface (`src/app/gc/mod.spl`)
✅ **FFI spec:** Bridges to Rust (`ffi_gen/specs/gc.spl`)
✅ **Auto-generate:** No manual .rs files
✅ **Rust lib:** Performance implementation (`runtime/memory/gc.rs`)
