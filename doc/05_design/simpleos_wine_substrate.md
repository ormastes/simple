...
preflight; missing CPU evidence is rejected with no mapped image and no mapped
region.
...
modeled import-thunk VMA write window; TLS preflight or record-planning
rejection returns no patched image, no mapped region, and no VMA thunk writes.
...
dispatch plan. `wine_process_execute_known_console_with_peb_teb_vm_writes(...)`
requires the VM-readback-gated mapped-image path before running the same bounded
modeled NT bridge plan.
...
addresses. The loader-lock variant composes the bounded KERNEL32 critical
