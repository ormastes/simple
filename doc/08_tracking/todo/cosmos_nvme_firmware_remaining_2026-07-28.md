...
# TODO: [nvme][P0] In the next bounded verification session, run `check-rv32-nvme-nand-recovery.shs --ghdl` once against the restored source. This session's source-matched image passed the direct soft-core after CDW10 was corrected from unsupported RV32 `i64` width arithmetic to `u32`, and reached the final PASS marker through the K26 DDR top; the AXI trace gate then exposed that consolidation had deleted the `.nandram` execution path. The restored path compiles, but the repository three-cycle guard intentionally deferred its combined AXI/exact-BRAM rerun.
...
1. Run the one bounded source-matched NAND AXI/exact-BRAM GHDL gate.
