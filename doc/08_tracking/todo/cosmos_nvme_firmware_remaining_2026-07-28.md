...
# TODO: [bootstrap][P0] Run one bounded Retry 13 from pushed repair `9919ae21d0a7` plus the follow-up helper restoration; only an admitted/deployed full CLI may execute the NVMe SSpec/docgen gate. Retry 12 used pushed lifecycle authority `cac26454bc5b`, rebuilt the Rust authority, and stopped in Stage 2 after 23m28s. Consolidation had deleted `HirLowering`'s typed imported trait/enum registries and the shared `_driver_physical_source_key`/`enum_variant_disc` definitions; set lowering constructed nonexistent `MirTypeKind.Named`, and if/while coverage read absent `HirExpr.coverage_excluded`. Peak RSS was 2,588,804 KiB with zero swap; the prior Stage 4 state-loss path was not reached. The focused source-shape guard now covers all six contracts. Retry 11 remains the last Stage 4 evidence: it passed Stage 2/3 but failed after 1,278 surfaces with 10,292 OOB reads, 5,146 missing tags, `n_modules=0`, and missing streaming surfaces.
