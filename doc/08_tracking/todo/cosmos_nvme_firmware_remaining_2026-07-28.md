...
# TODO: [bootstrap][P0] Run one bounded strict bootstrap after the declaration-visibility recursion and Stage-4 arena-admission repair, admit/deploy Stage 4, and execute the NVMe SSpec/docgen gate. Retry 9 rebuilt current Rust authority, passed Stage 2/3, passed the prior first-`ModuleSurface` failure, and released 598 Stage-4 surfaces before stale legacy statement/declaration indices culminated in a recursive visibility stack overflow. The production launcher now binds native arenas before process initialization; the shared fallback and focused contracts are repaired.
