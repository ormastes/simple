...
# TODO: [bootstrap][P0] Run one bounded Retry 15 after the Retry 14 linker repair; only an admitted/deployed full CLI may execute the NVMe SSpec/docgen gate. Retry 14 used current pushed authority `3962be916c9d`, rebuilt the Rust authority, compiled all Stage 2 objects, and stopped at the native link after 24m01s on three stale symbol groups: an undeclared trait-lowering helper, coverage-inventory calls whose implementation was removed by consolidation, and CUDA type sizing lowered to unavailable bare `sum`. Peak RSS was 2,591,760 KiB with zero swap. An imported trait now materializes through the existing `lower_trait` owner only when a local impl uses it, overriding a stale shared short-name entry once; orphaned coverage calls are removed while compiler-owned inventory remains tracked separately as TODO594, and CUDA struct/tuple sizes use explicit local accumulation. The focused contract rejects all three regressions. Retry 11 remains the last Stage 4 evidence: it passed Stage 2/3 but failed after 1,278 surfaces with 10,292 OOB reads, 5,146 missing tags, `n_modules=0`, and missing streaming surfaces.
