...
# TODO: [bootstrap][P0] Run one bounded strict bootstrap after wiring the paired `SIMPLE_BOOTSTRAP_LOW_MEMORY=1` and `SIMPLE_STAGE4_STREAMING_SURFACES=1` production admission flags, admit/deploy Stage 4, and execute the NVMe SSpec/docgen gate. The latest attempt proved Stage 2/3 and source attestation, entered Stage 4, peaked at 2,923,468 KiB RSS with zero swaps, then exposed the now-fixed false low-memory predicate.
