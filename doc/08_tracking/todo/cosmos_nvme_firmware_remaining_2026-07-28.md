...
# TODO: [bootstrap][P0] Finish review and remaining method/lambda frame-lifecycle repairs before one bounded Retry 12, then admit/deploy Stage 4 and execute the NVMe SSpec/docgen gate. Retry 11 rebuilt pushed authority `a7b53d603fc0` and passed Stage 2/3 sanity, provenance, and native capability, but Stage 4 still failed after 1,278 surfaces with 10,292 OOB reads, 5,146 missing tags, `n_modules=0`, and missing streaming surfaces. Peak RSS was 2,650,944 KiB with zero swap, proving semantic state loss rather than memory exhaustion. The focused root repair now preserves defining-owner metadata for imported aliases, publishes caller writes before all five core entries, refreshes foreign aliases, and relays shadowed packets; AST-shaped, callback, block-shadow, parameter-shadow, and serialized 25-test regressions pass. Do not run Retry 12 until this patch is reviewed and the direct method/lambda lifecycle gaps are resolved or explicitly excluded by focused evidence.
