...
# TODO: [bootstrap][P0] Run one bounded strict bootstrap after the raw-aggregate promotion repair, admit/deploy Stage 4, and execute the NVMe SSpec/docgen gate. Retry 7 proved Stage 2/3, source attestation, and the paired streaming/low-memory path, reducing peak RSS to 1,643,920 KiB with zero swaps; Stage 4 then failed on the first `ModuleSurface` because the ownership walker could not cross raw `rt_alloc` structs. The focused C regression for that repaired graph edge passes; a fresh strict retry is still required.
