...
| C4 Linux io_uring provider | runtime lane | runtime wiring now compiles vendored liburing into the Linux seed, explicit `io_uring` selection fails closed, the interpreter ABI is reconciled, and `SosixLinuxUringFileDriver` owns the raw facade; the focused runtime test proves exact backend, positioned write/read payload, one cancellation CQE, and close. The remaining close gate is the admitted self-hosted interpreter/native SOSIX spec. | `SIMPLE_SOSIX_PROVIDER=io_uring bin/simple test test/01_unit/lib/nogc_async_mut/sosix/io_uring_file_driver_spec.spl --no-session-daemon` in interpreter and native modes | `cargo test -p simple-runtime --features runtime-symbol-table async_driver_sffi::tests::io_uring_provider_completes_positioned_io_and_cancellation_once -- --exact --nocapture` PASS; pure-Simple runner unavailable in this isolated worktree |
