...
**Status:** FIXED 2026-07-12 — `_nb_line_end`/char-by-char `substring` scans
replaced with native single-pass `split_lines()` / `index_of()`.
...
- `_nb_line_end` deleted; `_native_build_entry_closure`'s per-file scan now
  uses `for raw in content.split_lines():` — a single native O(n) pass
  (Rust `str::lines()`), matching the pattern already used elsewhere in the
  interpreter's string builtins (`"split"` / `"split_lines"` both call the
  native Rust iterator directly, with no per-position `substring` calls).
