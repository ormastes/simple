...
|---|---|---|---|
| `src/lib/nogc_sync_mut/service/lease_manager.spl:126,152` | `LeaseManager` | `try_acquire_{exclusive,shared}` → `leases.push` | **LIVE, interp-only** — caller chain `src/app/sj/client.spl:38` → `src/app/sj/fallback.spl:9` → `src/app/sj_daemon/request_handler.spl:50,104`, all by-value; `SjRequestHandler` (`request_handler.spl:22`) is itself a struct holding `LeaseManager` by value |
| `src/lib/nogc_sync_mut/service/lease_manager.spl:47` | `LeaseManager` | `_next_lease_id` → `next_id` (**scalar**) | **LIVE, ALL engines** — every lease id is `lease-1`. Not covered by (B); a true defect either way |
| `src/lib/nogc_sync_mut/service/lease_manager.spl:76,168` | `LeaseManager` | `_reclaim_ghosts`, `release_lease` → `leases = kept` | LIVE, interp-only; `release_lease` always reports `found=false` |
| `src/app/sj_daemon/request_handler.spl:66-72` | — | consequence | Under the interpreter `mgr.leases` is always empty, `lease.ok` unconditionally true, so the **BUSY / `exit_code: 75i64` branch is unreachable** and the daemon offers no mutual exclusion while appearing to |
| `src/lib/nogc_sync_mut/service/request_queue.spl:43,45,58,67,98,99` | `RequestQueue` | `enqueue`, `dequeue`, `queue_drain` | **DEAD** — no production caller outside the module; `src/app/sj_daemon/ecs/systems.spl:10,13` reimplements the same counts over plain arrays and bypasses it |
