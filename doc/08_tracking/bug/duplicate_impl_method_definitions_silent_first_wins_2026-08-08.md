...
## 14. `MirInterpreter._call_function` — investigated, NOT ported (2026-08-08, seventh pass)

Followed up on §11's `_call_function` row (flagged alongside `ObjectProvider.add_library`
and `IncrementalState.mark_dirty` — the latter two now resolved in §12/§13). Unlike those,
this pass concludes the winner does **not** need the loser's fix, based on direct
verification rather than blind porting.

**Reachability, reconfirmed:** same mechanism as every other `MirInterpreter` pair
(§9/§10/§11) — no importer constructs `MirInterpreter(` outside `mir_interpreter.spl`
itself, so `mir_interp_ops.spl`'s copy is never loaded by any consumer found in
`src/compiler`. `mir_interpreter.spl`'s by-reference copy (`locals: self.locals`,
`blocks: self.blocks`, no per-key copy loop) is the sole reachable implementation.

**The cited bug doc does not exist.** The loser's comment
(`src/compiler/95.interp/mir_interp_ops.spl:157`, unchanged) points at
`doc/bug/bug_report_module_var_array_init.md` — that path was not found in this repo at
any commit reachable from a normal search. The closest-named real docs
(`doc/08_tracking/bug/interp_module_var_array_get_method_2026-07-04.md`,
`doc/08_tracking/bug/interpreter_module_array_stale_read_via_free_fn_helper_2026-07-29.md`)
describe a **different** interpreter (the source-level tree-walk interpreter that
executes actual Simple programs, e.g. `CancellationToken`'s free-fn-vs-method aliasing
bug) and a **different** failure mode (a stale read through a free-function indirection,
or an "unknown extern function" dispatch error on `.get(i)`) — neither is the
"`self.locals = {}` invalidates a previously saved reference" mechanism the
`_call_function` comment describes, and neither is `MirInterpreter` (a separate,
self-hosted MIR-level interpreter under `95.interp/`, not the source-level one those
docs cover).

**Mechanism verified directly, three targeted repros, all under `bin/simple test`
(the same engine every one of `MirInterpreter`'s live consumer specs runs under):**

1. Field-reassignment aliasing: save `self.d` by reference into a local, reassign
   `self.d = {}`, mutate the new dict, then read back through the saved reference.
   Result: the saved reference's original values are untouched — `self.d = {}` binds
   a genuinely new dict object, it does not clear/alias the old one.
2. Exact `_call_function` shape: save-by-reference into a `Frame`, reset to `{}`,
   populate the "callee" scope, call `.len()` on it (the specific trigger named in the
   loser's comment), mutate further, then restore-by-reference and check the caller's
   original keys. Result: caller's locals (`0 -> 10`, `1 -> 20`) survive intact.
3. Two levels of nested save/reset/`.len()`/restore (mimicking recursive
   `_call_function` calls). Result: both levels' locals survive a full unwind
   uncorrupted.

All three passed cleanly (`1 example, 0 failures` each); none reproduce the corruption
the loser's comment warns about for a `Dict`-typed field under this engine.

**Marker-proof of the live consumer specs (§10's five: `mir_ssa_phi_intrinsic_spec.spl`,
`mir_interp_bounds_check_spec.spl`, `strict_interp_spec.spl`,
`resource_interp_drop_spec.spl`, `optimization_plugin_jit_hotspot_system_spec.spl`):
temporary unconditional `eprint` markers were added to both copies' `_call_function`
and each spec run individually (`SIMPLE_MODULE_LIMIT=0 bin/simple test <spec>`). Result:
**both markers fired zero times across all five specs.** None of `MirInterpreter`'s
known live consumers ever exercise a MIR `Call` instruction that reaches
`_call_function` at all — the cross-function-call path is currently untested by any
spec this doc has identified. (The markers were transient and did not persist to the
committed tree — confirmed by `git diff HEAD origin/main` on both files being empty
after the probe, and `scripts/check/check-no-sabotage-residue.shs` → `PASS`.)

**Disposition: not ported.** Porting the deep-copy loop would be a purely speculative,
performance-costing change (an O(n) per-key copy of `locals` and `blocks` on every
call) against a bug this pass could not locate, reproduce, or confirm applies to a
`Dict`-typed field under the engine that actually runs `MirInterpreter`. Per this
project's no-over-engineering rule, an unproven defensive copy is not added blind. This
differs from `_execute_intrinsic` (§10, ported: the loser had a case the winner
completely lacked — an unambiguous behavioral gap) and from `IncrementalState.mark_dirty`
(§13, ported: a real transitive-vs-direct correctness difference, independently
diffable). `_call_function`'s two copies differ in an internal representation choice
(copy vs. alias) for which no live test exercises either behavior and no reproducible
divergence was found. Flagged for a follow-up only if/when a spec is added that actually
drives `MirInterpreter` through a real cross-function call — at that point, re-run the
three repros above against that spec's actual MIR program before deciding.
