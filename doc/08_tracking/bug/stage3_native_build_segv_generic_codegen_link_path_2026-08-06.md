...
### 2026-08-07 addendum — attribution UPGRADED from inference to proof

Two corrections/upgrades to the section above, both material.

**1. The pure-Simple codegen emits the identical wrong index — proven, not
inferred.** The section above pinned the mechanism on a *seed-built* stage2
(FIX8). Objection: `build_stage2.sh` sets `SIMPLE_NATIVE_BUILD_RUST=1`, so that
binary might reflect seed codegen and say nothing about `src/compiler/**`.
Settled by reading the offset directly out of the **stripped stage3**
(`bootstrap/stage3/x86_64-unknown-linux-gnu/simple`), which was emitted by
stage2's *pure-Simple* codegen:

```
517941:  mov  $0x80009,%ecx
517946:  bt   %rax,%rcx
51794a:  jae  5179bd
51794c:  and  $0xfffffffffffffff8,%r15
517950:  mov  0x58(%r15),%rdi      # <-- SAME wrong slot 11
517954:  mov  %rdi,(%rsp)
517958:  call <rt_for_iterable>
51795d:  mov  %rax,%r15
517960:  and  $0xfffffffffffffff8,%rax
517964:  je   5179ad
517966:  mov  0x8(%rax),%r14       # <-- 0x517966, THE reported faulting address
```

This window is **byte-for-byte identical** to FIX8's `check_function` at the same
function offsets (+0x41 .. +0x66). So:

- **`0x517966` is `BorrowChecker.check_function+102`.** The "is this the same
  site?" question is now closed on *identical instruction bytes*, not on a shape
  match. The original report's `test rax; je; mov 0x8(%rax)` was reading this
  `and $~7,%rax; je; mov 0x8(%rax)` — the `and` is what sets ZF.
- The `0x58` field index is produced by the pure-Simple compiler, so
  `resolve_field_index` in `src/compiler/50.mir/_MirLowering/function_lowering.spl`
  is the correct fix target. **This is a `src/compiler/**` defect on the
  critical path, not a disposable-seed artifact.**
- No contradiction with FIX8: the seed's `native-build` runs
  `src/app/cli/native_build_worker.spl`, i.e. the **pure-Simple** pipeline, so
  the same `.spl` `resolve_field_index` ran for both. `src/compiler_rust/**`
  contains no `resolve_field_index` at all. Byte-identical output between the
  seed-driven stage2 and the stage2-driven stage3 is the expected self-hosting
  fixed point.
- Corollary: `bootstrap/stage3/.../simple` is dated 2026-08-06 04:14, i.e.
  **pre-`1ea6599e8fb`** — yet it carries this exact site. That is precisely why
  the five post-fix rebuilds still SIGSEGV: `1ea6599e8fb` is a MIR-lowering fix
  and never touched the borrow-check path. The fix was never a candidate cure
  for this crash.

**2. The verification recipe's success criterion was wrong.** Step 4 above said
"expect rc=0". Correct it to: **expect no SIGSEGV**, then inspect the output.

The wrong-offset read is reachable *only* on the `Errors` branch — i.e. only
because `nll.check()` returned falsey. Inside `nll.spl` that value comes from
`self.errors.is_empty()` read at the **correct** 0x20, so the array being tested
is the real one: `errors` is genuinely **non-empty** for a program with no
borrows. The borrow checker is reporting **spurious** errors on trivial code.
`mod.spl:83-86` is a live lead — it records that `Return` was never matched as a
`MirTerminator` variant — and `check_terminator`'s `Return` arm pushes
"borrow of `X` may still be active at return" for every active borrow.

So fixing the field index alone likely converts a SIGSEGV into a **borrow-check
diagnostic failure**, not a green hello-world. The "two stacked failures" note
above understated this: it said `Errors([])`, but the evidence says the list is
not empty. Treat the spurious-borrow-error question as a **second defect on the
same path**, to be confirmed as soon as a binary exists that survives the field
read.
