...
- **Verification 2026-08-21 (bug-status-consistency audit): PARTIAL, not fully fixed.** fix present (`lowering_expr_method.rs:1613` receiver_is_array gate) with both specs, but the sibling `codegen_user_method_stolen_by_builtin_name_suffix_2026-08-17` is open and the `_p0` workaround is still in `ftl_journal.spl`. `bug_db.sdn` row is `fix-implemented-verification-pending`.
