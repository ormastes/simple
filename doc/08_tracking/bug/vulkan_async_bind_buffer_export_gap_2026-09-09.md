...
Status: OPEN; source-level blocker at
...
The called import has no canonical implementation. Import-only or syntax-only
success cannot prove an executable managed resource path. This blocks the
new device port even before its separate context/presenter authority gaps.
...
- Every exported/called name resolves through the canonical owner.
