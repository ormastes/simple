...
compiler, QEMU, or GPU execution evidence. `os_build_run.spl` and
`runner_targets.spl` now use shared I/O/time facades; TODO 573 retains the three
`scenario_*` runner modules plus native child-env, unique-temp, and
cross-platform timeout process ownership. TODO 574 retains monotonic safety.
