...
- Native BDD `test` still fails on broad root-facade trie construction through `use std.gc_async_immut.{PersistentTrie}`. The latest GDB backtrace for `tmp/root_persistent_trie_native_probe_spec.spl` enters `rt_native_eq`, then `nogc_async_immut.persistent_map.hamt_node.hamt_set`, then `PersistentMap.set`, before reaching trie frames. This points to root facade import breadth and HAMT native equality rather than the package-level trie backend.
...
3. Promote map/set overwrite/removal, set dedupe/removal, and broad root-facade trie specs only after the native BDD specs pass from tracked `test/unit/lib/...` paths.
