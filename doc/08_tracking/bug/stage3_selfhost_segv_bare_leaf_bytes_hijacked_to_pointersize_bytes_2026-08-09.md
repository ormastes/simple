...
Date: 2026-08-09
Status: **FIXED 2026-08-10, commit `17a55168a11`.** The single-candidate branch
of the `functions.rs` suffix scan (`matches.len() == 1`) now applies the same
receiver-owner-type check the `matches.len() > 1` branch already had; when the
receiver's static type is known and disagrees with the sole candidate's owner
(e.g. `text.bytes()` vs. `PointerSize.bytes`), the candidate is rejected and
resolution falls through to the runtime-intrinsic table instead of binding
blindly. Independently reproduced under gdb before the fix (real backtrace,
disassembly, and register-state cross-check) in
`stage3_selfhost_segv_bytes_hijack_confirmed_reproduction_2026-08-10.md`.
Re-run a full bootstrap to confirm Stage 3 now progresses past source-loading.
Previously: **ROOT-CAUSED, NOT FIXED.** Genuine, new, pre-existing defect. **Not** a
