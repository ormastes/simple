...
- `bin/simple test test/03_system/wm_compare/famous_site_corpus_spec.spl --mode=interpreter --no-cache --timeout 80`
  passes 45/45.
...
`test/09_baselines/famous_site_corpus/site_0_google/chrome_metrics.json` records
...
`test/03_system/wm_compare/famous_site_corpus_spec.spl` requires these fields for
...
analyzer. `test/01_unit/app/ui.chromium/text_metrics_spec.spl` has a focused
...
- `bin/simple test test/03_system/wm_compare/famous_site_corpus_spec.spl --mode=interpreter --no-cache`
  passes all 45 corpus checks, including the exact completion gate and focused
  production glyph-compositing diagnostics.
- `bin/simple test test/01_unit/lib/gc_async_mut/gpu/browser_engine/browser_renderer_spec.spl --mode=interpreter --no-cache`
