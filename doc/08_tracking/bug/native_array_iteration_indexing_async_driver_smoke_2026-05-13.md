...
While validating `test/perf/webserver/async_driver_native_smoke.spl`, the native async-driver FFI path successfully created a driver, submitted a timeout, flushed, and returned one completion from `rt_driver_poll`.
...
```bash
src/compiler_rust/target/debug/simple compile test/perf/webserver/async_driver_native_smoke.spl --native --cpu native --opt-level aggressive -o build/perf/webserver/async_driver_native_smoke
