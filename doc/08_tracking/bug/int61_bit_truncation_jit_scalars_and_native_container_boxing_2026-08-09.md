...
## Re-verification (2026-08-09, later same day)

Re-ran the exact fixture via JIT (`bin/simple run
test/fixtures/engine_differential/i64_boundary_values.spl`, no
`SIMPLE_EXECUTION_MODE` override, JIT-compiled lane) and reproduced Defect A
identically:
```
small=42
...
boxed_imax=-1
```
Still matches the sign-extended 61-bit-field pattern documented above. The
native (LLVM AOT) lane for Defect B was **not** re-run this pass — it needs a
full `native-build` closure, which this lane's environment constraints
exclude (no `bootstrap`/multi-minute native builds). Root-causing the exact
lowering site (Cranelift tagged-immediate encode/decode, likely in the
pure-Simple JIT backend under `src/compiler/70.backend/` or the boxed-value
runtime shared with native) is nontrivial compiler work, not a `.spl`
one-file fix, and is left OPEN per the architectural characterization above.
Status confirmed unchanged: **OPEN**, JIT-scalar defect re-verified directly;
native-boxing defect re-asserted from the existing table (not re-measured
this pass).
