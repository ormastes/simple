## Closed 2026-09-16 — fix landed twice over; root cause removed

Closed during the 2026-09-16 macOS bug/todo db sweep. The fix landed twice
over: the CI brew-install step is in origin/main (6dfbf2cae86, PR #253 MERGED
2026-09-01; `.github/workflows/rust-bootstrap-multiplatform.yml` Install step
present), and the root cause was removed by PR #883 MERGED 2026-09-13
(2cc924ff877) — the unconditional `#[link(name="freetype")]` is gone;
freetype is now a default-off cargo feature with a pure-Rust fontdue fallback,
so default-feature builds can no longer emit `ld: library 'freetype' not found`.
No code/config work remains; the file's unmet condition is passive CI
confirmation only. The in-file status body remains the authoritative record
and the repro was not re-run this pass. Re-open with a fresh dated repro if
the symptom returns.
