...
**Status:** FIXED-IN-SOURCE 2026-08-10 (unresolved-variable lane; see "2026-08-10 root cause + fix" below). Root cause was NOT the exec driver: `src/compiler_rust/compiler/src/codegen/instr/mod.rs`, `MirInst::GlobalLoad` handler, final else branch — a truly unresolved name was lowered to `iconst 0` ("Use zero to allow compilation to proceed"). Now returns `Err("GlobalLoad: unresolved identifier ...")`, so the default JIT lane falls back to the interpreter (real diagnostic, rc=1) and `SIMPLE_JIT_STRICT=1` hard-errors. Verified on a fresh `cargo build --release -p simple-driver`: undefvar probe rc=1 with diagnostic, control rc=0; A/B over the feature corpus for blast radius. Spec: `test/01_unit/app/cli/run_semantic_error_exit_code_spec.spl`. Residual OPEN: unresolved `use` (module import) still exits 0 with only a `[WARN]` — separate lane (`import_loader` treats unresolved imports as warnings by design for cross-module lowering); deliberately not changed here. NOTE: the spec's two JIT rows stay RED against the currently DEPLOYED seed (built 2026-08-09, pre-fix) until the seed is rebuilt from a clean post-land tree and redeployed; the fixed binary was verified directly (fresh `-p simple-driver` release build) but NOT deployed, because the shared working copy carries other sessions' uncommitted compiler_rust edits that a deploy would ship.
**Found:** 2026-08-04
