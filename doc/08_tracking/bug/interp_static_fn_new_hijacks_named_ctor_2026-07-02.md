...
## Re-derivation 2026-08-17 — the REOPEN is misdirected; no interpreter defect remains

This doc has now been wrong twice in opposite directions (a RESOLVED verdict
overturned 2026-08-08, and this REOPEN). Re-derived from scratch against the
deployed seed rather than from the doc's history.

Measured, `SIMPLE_EXECUTION_MODE=interpreter` vs `=jit`, same source:

| construction | interpreter | JIT | agree? |
|---|---|---|---|
| `Font(id: 1, size: 8)`   | `id=1 size=8` | `id=1 size=8` | yes |
| `Font.new("x", 8)`       | `id=77`       | `id=77`       | yes |
| `Font(path: "x", size: 8)` | `id=77`     | `id=77` (JIT declines to lower, falls back) | yes |
| `Font(bogus: 1, size: 8)` | `error: class Font has no field named bogus` | same error | yes |

Every divergence this doc was filed on is gone. The garbage `id=3` and the
silently-accepted `bogus` were **JIT** results; the JIT now emits
`Cannot infer field type: struct 'Font' field 'path'` and drops that module to
the interpreter, so both engines produce one answer.

The remaining claim — that `Font(path: "x", size: 8)` reaching `new` and
yielding `id=77` is a "hijack" — does not survive scrutiny. `path` is **not a
field of `Font`**, so that call cannot be a field literal; routing it to
`static fn new(path: text, size: i64)`, whose parameters it matches exactly, is
the only coherent reading. `id=77` is `new`'s own body doing what it was
written to do. Treating this as a defect would make the call an error with no
valid interpretation at all.

The genuine hazard the doc was circling — a fully-named literal whose names
happen to collide with a static `new`'s parameters being routed away from
field construction — is already fenced by `all_named_struct_literal` at
`src/compiler_rust/compiler/src/interpreter_call/core/class_instantiation.rs:148-155`,
which excludes a `new` that `is_static` (added 2026-08-17 for
`enum_field_in_nested_call_arg_join_not_found_2026-08-17.md`).

Note the cited root-cause file is also stale: `arg_binding.rs` contains no
`new` handling at all (`grep '"new"'` returns nothing). The dispatch decision
lives in `class_instantiation.rs`.

**Suggested status: CLOSED — not a defect on the interpreter lane.** Not
proven: `all_named_struct_literal` requires EVERY argument to be named and to
match a declared field, so a MIXED positional/named construction against a
same-arity static `new` still routes to `new`. That is untested here and is the
one shape that could still surprise; it deserves its own entry rather than
