...
1. **Direct-C write gap:** Fixed for the benchmark gate. `_set_sec` in
   `test/05_perf/bench/fat32_4k_compare.spl` no longer writes 512 individual bytes
...
`_set_sec` in `test/05_perf/bench/fat32_4k_compare.spl` (lines 26-31).
...
The hot path is in the bench file itself (`test/05_perf/bench/fat32_4k_compare.spl`
...
```text
SIMPLE_LIB=src bin/simple check test/05_perf/bench/fat32_4k_compare.spl
SIMPLE_LIB=src bin/simple run test/05_perf/bench/fat32_4k_compare.spl
FAT32_4K_RUNS=3 scripts/perf/run-fat32-4k-cfat-baseline.shs
```
