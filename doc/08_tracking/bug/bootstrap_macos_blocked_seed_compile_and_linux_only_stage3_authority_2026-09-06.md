...
## What "trim the CLI entry closure" actually requires (traced)

The six symbols are not one problem. Traced to their declaring Simple modules
and from there to what drags those modules in:

| symbol | declared in | pulled in by |
|---|---|---|
| `rt_sdl2_present_rgba` | `src/lib/nogc_sync_mut/io/window_sffi.spl` | **`src/lib/nogc_sync_mut/io/__init__.spl`** |
| `rt_hosted_select_surface` | `src/lib/nogc_sync_mut/sffi/host.spl` | **`src/lib/nogc_sync_mut/sffi/__init__.spl`** |
| `rt_webgpu_shutdown`, `rt_webgpu_destroy_surface` | `src/lib/nogc_sync_mut/gpu/engine2d/webgpu_sffi.spl` | `webgpu_surface.spl`, `webgpu_session.spl`, `backend_webgpu.spl` |
| `rt_mmap_raw` | `src/compiler/99.loader/smf_mmap_native.spl` | **not trimmable — see below** |
| `rt_numeric.f64` | — | malformed name, separate defect |

**The GUI leak is through package `__init__` hubs.** `io/__init__.spl` re-exports
`window_sffi`, and `sffi/__init__.spl` re-exports `host`. So *any* `use std.io…`
or `use std.sffi…` anywhere in the CLI closure drags the SDL2 and hosted-surface
extern declarations in with it, whether or not a single one is ever called. This
is the same shape as the previously recorded `app.io.mod` hub blow-up. Trimming
therefore means changing what those two hubs re-export — a stdlib-package change
with a wide blast radius (every consumer that resolves `Window`-family names
through the hub), not an edit to `main.spl`.

**`rt_mmap_raw` cannot be trimmed and is a different bug.** It is the SMF
loader's mmap (`src/compiler/99.loader/smf_mmap_native.spl`), core compiler
functionality that belongs in the CLI. Its C definition is
`platform/unix_common.h:365`, reached only through `platform/platform.h`, which
is included by exactly one translation unit — `src/runtime/runtime.c:35` — and
`runtime.o` is **not** among the 23 objects in the `core-c-bootstrap` archive.
So this symbol needs the bundle to carry its provider; no closure change can
fix it.

**Plan, in dependency order:**

1. Add the `rt_mmap_raw` provider to the core-c-bootstrap bundle (or split the
   raw-syscall wrappers out of `runtime.c` into a bundle-included unit). Must be
   done regardless of the trim, and updates `runtime-source-list-parity`.
2. Stop `io/__init__.spl` and `sffi/__init__.spl` re-exporting the GUI SFFI
   modules; fix the consumers that relied on hub resolution.
3. Confirm nothing in the CLI closure reaches `webgpu_surface` / `webgpu_session`
   / `backend_webgpu` once (2) lands.
4. Chase `rt_numeric.f64` independently — a `.` cannot name a C provider.
