...
Spec: `test/02_integration/gpu/engine2d_vulkan_damage_scoped_mirror_spec.spl` (4/4).
...
Sabotage check: forcing `consume_damage_hint` to always widen to full turns the
counters into `damage_rect_px=684000, mirror_copy_iterations=0` and the steady
frame back to 4-6 ms — the spec's first two examples go red on the counters.
Reverted.
...
## The page path's "unreconciled 1.44x" is GONE, and was the readback term
...
|---|---|---|
| default (`cargo-r2`) | 2453 ms | — |
| `SIMPLE_VK_READBACK=native` | **18 ms** | **28 ms** |

The 2026-09-11 page probe measured 7.2 s @900x760 against the fixture probe's
5.0 s. On the default lane the page path is now 2453 ms — almost exactly ONE
684k interpreted unpack — i.e. the extra term was another pixel-linear unpack
and F8's present dedup removed it. There is no residual per-node or per-pixel
term left to name: at 18 ms / 28 ms the page path is no longer pixel-dominated.
The profile doc's three remaining candidates (the `draw_image` blit in
`simple_web_html_engine2d_presenter.spl`, the degraded-retry second layout pass,
a deep `[u32]` copy across the browser_renderer boundary) are therefore NOT the
explanation and need no fix.
