...
`tools/pixel_compare/argb_json_to_ppm.js` conversion path. Fixture 18 still
cannot produce checked-in `chrome.ppm` / `simple.ppm` pixel baselines on the
current Linux host because the harness hangs after a successful 76,800-pixel
source-A capture, including with `--skip-simple`; the full source-B path also
does not complete the Simple browser-engine capture.
...
- Current focused pixel attempt:
  ...
  now reaches fixture 18 and source A reports `ok pixels=76800` through the
  maintained Electron live-bitmap path. The run then hangs before producing a
  committed baseline row. A `--skip-simple` attempt also reaches `ok
  pixels=76800` and hangs before completion, so the next blocker is the
  post-capture PPM write/report path, not the old Electron screenshot app.
...
baseline update should finish the post-capture write/report path, then add
exact `chrome.ppm`, `simple.ppm`, and `report.sdn` rows for 18-24, or
explicitly document why flex rows are structural-only until text/raster parity
improves.
