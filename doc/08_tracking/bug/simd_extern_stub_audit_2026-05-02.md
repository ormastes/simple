...
`test/01_unit/lib/simd/public_extern_interpreter_spec.spl`.
...
| 2026-05-27 | Mechanical re-audit found 48 public `rt_simd_*` declarations in `simd.spl`; seven were still missing interpreter dispatch/implementation (`u32x4` add/sub/and/or/xor and `i64x4` add/sub). Added scalar interpreter implementations, dispatch entries, and `test/01_unit/lib/simd/public_extern_interpreter_spec.spl`. Marked `rt_simd_str_*` helpers interpreter-internal. Tracker resolved; native lane-record SIMD remains separate ABI work. |
