...
```text
external-suite-status=suite_steps|40
...
suite readiness remain missing. It also now consumes the generated reference
comparator env check report, so reference comparator env readiness remains a
separate missing fastest-comparator item until the Simple/uWebSockets/Seastar
URLs and uWebSockets/Seastar provenance validate.

Reference comparator env validation is generated by:

```sh
sh scripts/check/check-gpu-web-db-offload-reference-comparator-handoff.shs --write-env-check-report
```
