...
`bin/simple run test/05_perf/bench/fat32_microbench.spl` does not complete after
...
test/05_perf/bench/fat32_4k_compare.spl` segfaults with rc=139 immediately after
...
test/05_perf/bench/fat32_4k_compare.spl` no longer segfaults. The benchmark now
...
`src/compiler_rust/target/debug/simple run
test/05_perf/bench/fat32_4k_compare.spl` completes the Simple FAT read and write
...
```bash
src/compiler_rust/target/debug/simple test/05_perf/bench/fat32_microbench.spl
```

## Current Status

- `bin/simple check` passes for the sync and async FAT fs-driver files.
- `bin/simple check test/05_perf/bench/fat32_microbench.spl --mode=interpreter`
  passes.
...
- Hosted `src/compiler_rust/target/debug/simple run
  test/05_perf/bench/fat32_4k_compare.spl` completes Simple FAT 4K random read and
  write in the focused in-memory harness; VFAT/C-FAT same-device proof remains
  missing.
- Rechecked 2026-05-29 by spawned agent:
  `src/compiler_rust/target/debug/simple run test/05_perf/bench/fat32_microbench.spl`
  exits 0, and
  `src/compiler_rust/target/debug/simple run test/05_perf/bench/fat32_4k_compare.spl`
