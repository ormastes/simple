# Bug: GUI/web/2D Simple native Vulkan evidence crashes
...
On the linked worktree, `scripts/setup/setup-gui-web-2d-vulkan-env.shs --run`
falls back from missing `bin/simple` to `bin/simple_native`, then the Simple
readback and Simple ARGB lanes crash:
...
Completion requires the same `--run` gate to produce:
...
compiler/runtime/backend owner path or select a current non-crashing Simple
binary through the existing `SIMPLE_BIN` override.
