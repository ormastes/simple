...
| `cuda_vm` | `interpreter(remote(cuda(sm80)))` | host-aware | same as `cuda_jit` (cuda backend probe) | `src/lib/gc_async_mut/gpu_lane/cuda_vm_executor.spl` (`CudaVmExecutor`) | none in-tree yet — B3 verified the kernel out-of-tree (standalone CUDA-driver-API C harness, 3 hand-encoded vectors) but the full D3 61-vector conformance table has never been run against `CudaVmExecutor` in a checked-in system spec | Not-yet-verified in-tree (see `svmg_device_arena_code_coresidency_diverges_from_ref_vm_2026-08-07` — flags this exact gap: "B3's CUDA kernel has the identical exposure and has never actually been run against the D3 table on real hardware") |
