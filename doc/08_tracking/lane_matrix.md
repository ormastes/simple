...
| Lane ID | Arch | Adapter | Proof | Status | Primary Channel | Fallback Channel | Authoritative Spec |
|---------|------|---------|-------|--------|-----------------|------------------|--------------------|
| qemu_rv32_semihost | riscv32 | qemu_gdb | compiled | **stable** | semihost_text | exit_code | `test/integration/remote_jit/qemu_rv32_library_semihost_spec.spl` |
| qemu_arm_semihost | arm32 | qemu_gdb | compiled | **stable** | semihost_text | exit_code | `test/integration/remote_jit/qemu_arm_composite_runner_spec.spl` |
| x86_64_direct_boot | x86_64 | direct_boot | compiled | **stable** | exit_code | — | `test/integration/baremetal/remote_riscv32_spec.spl` |
| ch32v307_wlink | riscv32 | wlink_cli | compiled | **host_aware** | register_readback | ram_sentinel | `test/integration/remote_jit/ch32v307_composite_runner_spec.spl` |
| stm32h7_openocd | arm32 | openocd_gdb | compiled | **host_aware** | register_readback | ram_sentinel | `test/integration/remote_jit/stm32h7_composite_runner_spec.spl` |
| stm32h7_trace32 | arm32 | trace32 | compiled | **host_aware** | debugger_console | register_readback | `test/system/t32_terminal_power_remote_spec.spl` |
| ghdl_rv32_sim | riscv32 | ghdl_sim | compiled | **in_progress** | semihost_text | ram_sentinel | `test/integration/remote_jit/ghdl_rv32_composite_runner_spec.spl` |
| fpga_jtag_zedboard | arm32 | openocd_gdb | structural | **in_progress** | register_readback | — | *(none — quarantined)* |
| rv32_raw_injected | riscv32 | qemu_gdb | structural | transport_only | register_readback | — | `test/integration/remote_jit/qemu_rv32_raw_injected_regression_spec.spl` |
| baremetal_runtime_check | riscv32 | qemu_gdb | interpreter | transport_only | exit_code | — | `test/feature/app/remote_baremetal/remote_baremetal_runtime_spec.spl` |

## Classification Summary

- **Stable (3):** QEMU RV32 semihost, QEMU ARM semihost, x86_64 direct boot
- **Host-aware (3):** CH32V307 via wlink, STM32H7 via OpenOCD, STM32H7 via TRACE32
- **Transport-only (2):** RV32 raw injected regression, baremetal runtime check
- **In-progress (2):** GHDL RV32 simulation, ZedBoard/FPGA JTAG
...
### Host-Aware Hardware Lanes

Real hardware lanes that skip cleanly when prerequisites are missing:
...
- **ghdl_rv32_sim**: GHDL VHDL simulation of RV32. Needs mailbox protocol completion.
...
| ram_sentinel | CH32 (fallback), STM32 (fallback) | Read known RAM address for result |
...
- **Supported simulation:** GHDL RV32 (after mailbox completion)
