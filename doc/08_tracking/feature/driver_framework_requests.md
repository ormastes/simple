...
- **Notes:** The procedural registration path works today; this item is
  ...
  `test/01_unit/compiler/mir/mir_exported_types_spec.spl`,
  `test/01_unit/compiler/semantics/sffi_lint_spec.spl`, and
  `test/01_unit/lib/driver/null_block_driver_test.spl`. Remaining work is the
  ...
  `test/01_unit/compiler/mir/synthetic_driver_registration_spec.spl`.
  ...
  increments. Coverage:
  `test/03_system/compiler/driver_synthetic_registration_live_spec.spl`,
  mirrored at
...
- **Notes:** Division workaround stays valid until this lands.
  ...
  Completion verified 2026-04-22: `test/01_unit/compiler/shr_signedness_test.spl`
...
- **Status:** **Implemented** (Rust seed, 2026-05-10; self-hosted parity code written 2026-05-11; pure Simple parser/example evidence completed 2026-05-29).
  ...
  `test/01_unit/compiler/packed_struct_sugar_test.spl` (2 tests: round-trip field
...
- **Notes:** Pair this with FR-DRIVER-0001 — the writer depends on
  ...
  `test/01_unit/compiler/linker/smf_driver_manifest_section_spec.spl` and
  `test/01_unit/lib/driver/driver_manifest_test.spl`. AOT SMF build update
  ...
  `test/01_unit/compiler/linker/smf_driver_manifest_build_spec.spl`.
  ...
  `test/03_system/compiler/dynamic_driver_lsm_packaging_spec.spl`.
...
- **Notes:** User directive (project memory `project_driver_framework.md`)
  ...
  `test/01_unit/lib/io/dma_test.spl`.
...
- **Notes:** Effort is several days. Until it lands, the tolerant
  ...
  `test/01_unit/lib/driver/fat32_file_io_spec.spl` passes `14/14`; the old
...
- **Notes:** Effort: 2–3 days (estimate from Track-4 scope-mismatch
  ...
  test/03_system/feature/usage/bitfield_runtime_compat_spec.spl` now covers the
...
- **Acceptance-criteria:**
  ...
  - [x] Network, block/file, and VirtIO-GPU tests each exercise the same
        descriptor shape.
- **Related-upfront:** `doc/04_architecture/os/hardware_driver_safety_and_performance_2026-04-15.md`
...
- **Notes:** Existing `alloc_dma`/`free_dma` syscalls and
  ...
  `test/03_system/driver_shared_dma_contract_spec.spl`.
...
- **Notes:** Implemented as an explicit fs-driver direct-I/O request/result
  ...
  `test/03_system/driver_dma_direct_io_spec.spl`.
