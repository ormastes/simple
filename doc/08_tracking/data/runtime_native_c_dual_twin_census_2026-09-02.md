...
Measured BEFORE this change: 32 matching lines (30 pair annotations + 2 doc lines) across five
specs under test/01_unit/lib/common/spec/, which is exactly what
check-dual-run-shadow.shs reported as "30 pairs" at that time. THIS CHANGE ADDS 2,
so after it the same commands yield 34 lines and 32 pairs. Verify with:
```sh
grep -rn '@dual_pair' test/ --include='*.spl' | wc -l   # 32 before / 34 after this change
grep -rn --include="*_spec.spl" -E "^# @dual_pair: " test/ | wc -l  # 30 before / 32 after (this is what the gate counts)
```
