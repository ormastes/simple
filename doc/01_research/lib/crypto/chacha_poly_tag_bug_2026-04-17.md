...
Running `test/03_system/chacha_poly_roundtrip_spec.spl` (new):
...
`test/03_system/os_crypto_ref_primitives_spec.spl` asserts only:
...
- `test/03_system/chacha_poly_roundtrip_spec.spl` — reproduces both bugs and
  passes the surrounding cross-vendor lanes (11/13 green).
- `test/03_system/os_crypto_ref_primitives_spec.spl` — existing spec; assertion
