...
Findings:
- `examples/11_advanced/simple_db/src/engine/nvfs_shim.spl` locally declared `STORAGE_CLASS_DB_WAL`,
  `STORAGE_CLASS_META_DURABLE`, and `DURABILITY_DATA_DURABLE`.
- `examples/11_advanced/simple_db/src/engine/tier_cache.spl` locally declared `STORAGE_CLASS_DB_TEMP`.
- `examples/11_advanced/nvfs/src/core/arena.spl` accepts opaque `class_tag: i32` values and preserves
...
- Add `examples/11_advanced/nvfs/src/core/constants.spl`.
