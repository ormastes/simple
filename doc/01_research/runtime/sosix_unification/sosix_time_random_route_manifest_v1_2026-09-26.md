...
| SimpleOS realtime clock | `src/os/kernel/ipc/syscall_log_time_sysinfo.spl`: `_handle_clock_get_time(clock_id=0)` and `src/os/drivers/rtc/{rtc,rtc_calendar_v1}.spl` | Candidate x86 RTC path reads date, time, mode, and century twice, validates UTC calendar fields, then publishes Unix seconds or EIO. It replaces the seconds-since-midnight result. The UTC/century profile, shared CMOS index serialization, all-architecture providers, guest execution, and native parity remain **open**; do not promote this route as a working realtime SOSIX service. |
