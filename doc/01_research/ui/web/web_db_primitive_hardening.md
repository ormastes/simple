...
- `app.ui.web` uses `BoundedChannel` for event/render backpressure. Its `close()` was a no-op, and `test/02_integration/app/ui.web/backpressure_test.spl` documented that sends after close still succeeded.
...
- Primitive coverage already exists in `test/03_system/feature/usage/primitive_types_spec.spl`, `test/02_integration/app/primitive_api_lint_spec.spl`, and `test/shared/core/primitives_spec.spl`; interpreted checks pass.
