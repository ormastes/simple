...
- `doc/04_architecture/platform/cross_platform_wm.md` and `doc/07_guide/ui_stack_guide.md` define `UISession` as the shared UI state entry point. Graphics backend session handles should remain backend resources referenced by UI/WM backends, not fields cached on `UISession`.
