...
## 10. SUI + Builder Pattern: Multi-Platform Support

**Question:** Can SUI + Builder Pattern support Browser, Electron, VSCode, TUI, and Vulkan?

**Answer:** Yes, both approaches use the same `RenderBackend` trait abstraction:

```
┌─────────────────────────────────────────────────────────┐
│                    UI Definition                         │
│  ┌─────────────────┐     ┌─────────────────────────┐    │
│  │  SUI Templates  │     │    Builder Pattern      │    │
│  │  (.sui files)   │     │    (programmatic)       │    │
│  └────────┬────────┘     └───────────┬─────────────┘    │
│           │                          │                   │
│           └──────────┬───────────────┘                   │
│                      ▼                                   │
...
└──────────────────────┼───────────────────────────────────┘
     ...
     ▼         ▼               ▼         ▼         ▼
┌─────────┐ ┌─────────┐ ┌──────────┐ ┌─────────┐ ┌─────────┐
│ Browser │ │Electron │ │  VSCode  │ │   TUI   │ │ Vulkan  │
│  (DOM)  │ │ (DOM+   │ │(Webview  │ │(Terminal│ │  (GPU)  │
│         │ │ Node.js)│ │ Toolkit) │ │  cells) │ │         │
└─────────┘ └─────────┘ └──────────┘ └─────────┘ └─────────┘
```

### Platform Mapping (Extended)

| Widget | Browser | Electron | VSCode | TUI | Vulkan |
|--------|---------|----------|--------|-----|--------|
| Button | `<button>` | `<button>` | `<vscode-button>` | `[ Label ]` | Rect + Text |
| Input | `<input>` | `<input>` | `<vscode-text-field>` | `[_____]` | Rect + Cursor |
| Select | `<select>` | `<select>` | `<vscode-dropdown>` | `[v menu]` | Dropdown mesh |
| Panel | `<div>` | `<div>` | `<vscode-panel-view>` | `┌───┐` | Rounded rect |
| Text | `<span>` | `<span>` | `<span>` | ASCII | GPU text atlas |
| Image | `<img>` | `<img>` | `<img>` | ASCII art | Texture quad |

### Key Insight

SUI templates compile to the same Widget tree as Builder Pattern code. Both then render through platform-specific backends. This architecture enables:

1. **Write once, render anywhere** - Same widget code for all platforms
2. **Progressive enhancement** - TUI for servers, Vulkan for games
3. **Consistent API** - Same event handling across platforms
4. **Platform-specific optimization** - Each renderer optimizes for its target

---

## 11. Vulkan Rendering Target

### 11.1 Vulkan Renderer Architecture

```simple
# simple/std_lib/src/ui/render/vulkan.spl

class VulkanRenderer impl RenderBackend:
    device: VulkanDevice
    pipeline: GraphicsPipeline
    vertex_buffer: Buffer
    text_atlas: TextAtlas

    fn render_button(self, btn: Button) -> VulkanNode:
        # Button = rounded rect + text
        let rect = RoundedRect.new(btn.bounds, btn.corner_radius)
        ...
        let mesh = rect.to_mesh(color)
        let text = self.text_atlas.render(btn.label, btn.text_style)

        return VulkanNode.composite([mesh, text])

    fn render_column(self, col: Column) -> VulkanNode:
        let nodes = []
        let y = 0.0
        for child in col.children:
            let node = self.render(child)
            node.translate(0.0, y)
            nodes.append(node)
            y += node.height + col.spacing
        return VulkanNode.group(nodes)
```

### 11.2 Why Vulkan is Hard (and How Simple Helps)

See **[vulkan_dsl.md](vulkan_dsl.md)** for complete analysis of Vulkan difficulties and Simple's solutions.
...
4. **Unified event handling** across Browser, Electron, VSCode, TUI, and Vulkan
5. **Shared theming** with platform-aware output generation
6. **Vulkan support** for high-performance GPU rendering (games, visualization)
