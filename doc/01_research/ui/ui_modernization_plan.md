...
- Golden-byte snapshot fixture under `test/01_unit/app/ui/wire_golden/` capturing today's `UiAccessSnapshot` JSON for a representative widget tree (panel + row + button + dialog + toast + tabs); used as regression oracle by Phases 2–6

Investigation items (must resolve before Phase 1 starts):
- Audit `test/01_unit/app/ui/gui_widgets_spec.spl` and `backend_matrix_spec.spl` for raw string `kind`/`layout`/variant assertions that go *beyond* the wire boundary. **If they do, Phase 2 scope expands** to include co-located `to_wire()` use in test helpers.
...
- `doc/04_architecture/ui/shared_ui_contract.md` — add a "Wire vs Internal Types" subsection clarifying the codec boundary at `to_wire()/from_wire()`
- `doc/04_architecture/compiler/mdsoc_architecture_tobe.md` (MDSOC+ section) — add UI worked example showing `WidgetNode`-as-component, `UISession`-as-world, lifecycle-as-system mapping
...
- `test/01_unit/app/ui/wire_golden/` golden snapshots (Phase 0)

## Verification

End-to-end testing per phase:

1. **Wire stability** — `bin/simple test test/01_unit/app/ui/wire_golden/` — golden-byte diff must be zero through Phases 1–8
2. **Backend matrix** — `bin/simple test test/01_unit/app/ui/backend_matrix_spec.spl` — web and TUI must render identical snapshots
