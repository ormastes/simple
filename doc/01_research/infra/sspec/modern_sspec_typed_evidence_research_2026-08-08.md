...
### 2.3 Independent verification ledger (parallel read-only agents, 2026-08-08)

**Manual-example integrity — PARTIAL/CONFIRMED.**
- `doc/07_guide/app/spipe/manual_examples/gui_web_manual_example.md` → `test/03_system/app/notes_web/notes_web_spec.spl` **missing**; `statistics_manual_example.md` → `test/03_system/app/compiler_perf/warm_start_throughput_spec.spl` **missing**; `baremetal_network_manual_example.md` → `test/03_system/app/nvme_firmware/nvme_nand_capture_spec.spl` exists.
- NVMe manual mismatch CONFIRMED: the manual describes CQE phase-bit, CC register bit-fields, and NVMe/TCP capsule PDU scenarios, while its named spec covers emulated NAND write/read + FTL migration (the manual itself discloses this at lines 8–10). It must be labeled a design mockup until runnable wire-level source exists.
- All 7 target update files (requirements/design/plan/guide/agents/skills/template) exist.

**Evidence-record and comparator claims** and **PTE/SMF layout claims** — verification lanes were dispatched the same day; record their verdicts here before the owning lanes (E0/E3/E4) start.
