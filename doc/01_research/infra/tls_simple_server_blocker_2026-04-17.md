...
| Lane | Status |
|---|---|
| **Simple client ↔ rustls server** | ✅ `test/03_system/os_tls_system_spec.spl` (live handshake via `tools/tls_test_server/`, rustls-based) |
...
Per `doc/01_research/lib/tls13_phase2_backlog.md` §"Server-side TLS 1.3":
...
- **Intensive encode/decode at the TLS record layer**: our pure-Simple
  ...
  (`doc/01_research/lib/chacha_poly_tag_bug_2026-04-17.md`). The TLS record
...
4. A test spec (`test/03_system/os_tls_server_system_spec.spl`) spawns the Simple
