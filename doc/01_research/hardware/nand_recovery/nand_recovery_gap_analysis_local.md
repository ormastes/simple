...
| Read-retry / vref sweep | **actuator wired, policy absent** | chain `chip.spl:743-748` (`set_vref_offset`, vref=128+offset) → `fil_nand_emu.spl:354-358` → `fil_fmc.spl:225-228` → `fil.spl:140-141` → `ftl.spl:174-175`; zero production callers — only `fil_nand_emu_check.spl:119-121`, `fil_nand_emu_e2e_check.spl:130-133`; on `NAND_ECC_FAIL` the read fails immediately, no retry loop |
| Read-disturb scrub (`scrub_once`) | **implemented-UNWIRED** | `ftl.spl:617-631`, threshold `READ_DISTURB_THRESHOLD = 50` (`nvme_types.spl:43`); counter `rd_disturb: [i64]` (`ftl.spl:29`), bumped `ftl.spl:109,117,347-352`, cleared on erase/reclaim `ftl.spl:354-359,544,548`; no production caller (only `gc_once` is wired: `nvme_controller.spl:485,491`, `hil.spl:113-114`) |
| Static wear leveling (`wear_level_once`) | **implemented-UNWIRED** | `ftl.spl:605-615` (fires when `max_erase - erase_count(cold) > WL_THRESHOLD`), threshold `WL_THRESHOLD = 2` (`nvme_types.spl:42`); victim picker `ftl_gc.spl:15-27`; no production caller |
...
| Spare-block remapping (`alloc_spare`) | **implemented-UNWIRED** | `fil_badblock.spl:52-61` (spare pool = OP region blocks 56..63); no FTL caller — retired blocks drop from rotation but are never remapped to a reserved spare |
...
| Age / retention-time tracking | **absent in FW; present in emu only** | FTL tracks only a monotonic `seq` (`ftl.spl` `next_seq`), no wall-clock/last-program-time per page or block; emu's `NePageMeta.last_prog_time` (`nand_types.spl:301`) and `NeBlockMeta.last_erase_time` (`nand_types.spl:311-322`) exist but feed only the emulator's own drift model, never read by firmware |
