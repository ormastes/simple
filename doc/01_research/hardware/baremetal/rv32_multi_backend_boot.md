...
- `examples/09_embedded/mllvm_qemu_rtl` hybrid/RTL simulation
...
- The current smoke test, `test/03_system/qemu/os/boot/riscv32_boot_qemu_spec.spl`, checks for banner and boot markers.
- The current proof is early boot, not full OS feature coverage.

### GHDL RV32 is a remote execution lane, not an OS boot lane

- `src/lib/nogc_sync_mut/debug/remote/exec/adapter_ghdl_rv32.spl` and `test/02_integration/remote_jit/ghdl_rv32_composite_runner_spec.spl` prove that RV32 payloads can run under GHDL-backed execution plumbing.
...
### `examples/09_embedded/mllvm_qemu_rtl` is model-level RV32 work

- `examples/09_embedded/mllvm_qemu_rtl/src/timing/hybrid_sim.spl` loads raw bytes at `0x80000000`; it does not implement a real ELF loader or board boot flow.
- `examples/09_embedded/mllvm_qemu_rtl/src/rtl/sim_engine.spl` mirrors architectural state into a signal store; it is not an executable RV32 CPU core.
- `examples/09_embedded/mllvm_qemu_rtl/tools/rtk_verilator_runner.sh` special-cases the stub RTL and falls back to lint-only behavior for the non-stub path.
