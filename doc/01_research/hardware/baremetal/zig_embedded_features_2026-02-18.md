...
- `introspection_phantom_types_2026-02-17.md` — phantom types / introspection
...
### 1.1 Current Status — SYNTAX_ONLY (verified)

**Syntax:** ✅ Parser accepts both syntaxes (parser.spl:2293, 2157-2163)
```simple
const fn fibonacci(n: i64) -> i64:   # parsed as normal function — identical to fn
    if n <= 1: n
...
**Runtime behavior:** ❌ No actual compile-time evaluation. `comptime val` evaluates at **module load time** (runtime), not compile time. `const fn` is called at runtime like any other function. No CT execution engine exists — verified by examining `eval.spl` (no comptime-phase handling).

**Evidence:** `test/01_unit/parser/comptime_spec.spl` shows they work exactly like normal `val`/`fn` — evaluated when module loads, not at compile time.
...
**Grammar:** No change needed — `comptime`/`const` already parsed. This is a **semantic analysis pass**.
...
**Phase A — Semantic analysis (medium effort):**
...
### 2.1 Current Status — SYNTAX_ONLY (verified)

Verification revealed more than the earlier doc stated:
- `@repr(C)`, `@repr("packed")`, `@repr("transparent")`, `@packed`, `@align(N)` — **fully parsed** into `LayoutAttr` structures (found in `src/compiler_core/attributes.spl`)
...
```simple
# USART peripheral registers — must be exactly 32 bytes, C-layout
@repr(C)
@align(4)
struct UsartRegs:
...
| Aspect | Policy |
|--------|--------|
| `@repr(C)` | Required for all FFI/MMIO structs — explicitly declarative |
...
### 3.1 Current Status — NOT_IMPL (verified)

❌ Zero references to `link_section`, `addr_space`, or `linksection` anywhere in the codebase. The linker SDN format (`doc/06_spec/layout.sdn`) describes section placement but there's no language-level annotation to tag functions or variables to specific sections.
...
# Already fits existing @IDENT(expr) annotation pattern — O(1), no new keywords
```
...
### 4.1 Current Status — NOT_IMPL (verified)

❌ Zero references to `callconv`, `naked`, or calling convention control anywhere in parser.spl, tokens.spl, or eval.spl. Inline assembly stub exists (`src/compiler_core/inline_asm.spl`) but without calling convention integration.
...
### 5.1 Current Status — PARTIAL (verified)

- **Volatile annotations:** ✅ **PARSED AND TYPED** — `src/compiler_shared/volatile.spl` (80 lines) defines `VolatileAccess`, `VolatileContext`, `VolatileKind` (`Read`/`Write`/`ReadWrite`/`None_`). `@volatile`, `@volatile_read`, `@volatile_write`, `@nonvolatile` annotations are recognized.
...
**Option A — Volatile pointer type (cleanest, Zig-inspired):**
```simple
# Volatile pointer type
val uart_dr: volatile ptr<u8> = rawptr(0x40011004)
uart_dr.* = 'A'        # guaranteed write — not optimized away
...
**Option B — SFFI volatile builtins (current workaround, improved wrapper):**
```simple
# Already possible via SFFI — improve with generic wrappers
...
### 5.4 Volatile vs Atomic — The Critical Distinction
...
### 6.1 Current Status — PARTIAL (verified)
...
Proposed improvement — **`@wffi` annotation** that auto-generates the wrapper:
```simple
...
| No global `@cImport` (Zig style) — curated only | Large header imports can bring in unsafe patterns |
...
### 7.1 Current Status — PARTIAL (verified)

- `--target=embedded` flag: ✅ Sets immutable defaults, disables GC
...
- Baremetal modules exist: `src/baremetal/arm/`, `src/baremetal/riscv*/` — confirmed
- **Backend routing:** ❌ Compiler backend doesn't fully consume the target flag for code generation
- **ARM Thumb2 / RISC-V codegen:** ❌ Not confirmed working end-to-end
...
| Tests are part of the safety case — traceable to requirements | Make test ↔ requirement linking explicit |
...
Zig captures an **error return trace** — a compact stack of source locations where an error was returned — distinct from stack unwinding:
```zig
...
This is NOT stack unwinding — the trace is stored as a fixed-size array of frame pointers, no heap allocation needed.

### 9.3 Simple Proposed Design (Sketch for the Planned Task)

```simple
# Error trace: compact array of (file, line) pairs
# No heap allocation — fixed max depth
...
| Rule | Rationale |
|------|-----------|
| Fixed-size trace array — no heap allocation | Determinism |
...
| Error trace ≠ exception — no unwinding, no cleanup | Consistent with no-exceptions rule |
...
❌ Not implemented. Needs deeper research.
...
3. **Slice vs pointer:** Zig distinguishes `[*:0]u8` (unknown-length, sentinel-terminated) from `[:0]u8` (known-length, also sentinel-terminated). Does Simple need both? The `[:0]u8` form is safer — can verify sentinel in O(N) at creation.
...
5. **Interaction with Option:** A null-terminated C string is arguably `Option<CStr>` — the `\0` encodes "no more data" (None). Is sentinel-termination a special case of `Option<T>` encoding, or a separate concept?
...
**Recommendation:** Research sentinel types separately in `doc/01_research/sentinel_types_2026-02-XX.md` after comptime (feature #1) is designed, since CT sentinel values depend on CT evaluation.
...
|------------|----------------|----------------------|--------|
| comptime (CT eval) | SYNTAX_ONLY — runtime eval | ⚠️ Total/bounded subset only | Phase A: semantic checker; Phase B: CT eval engine |
| extern/packed struct | SYNTAX_ONLY — parsed, not enforced in layout | ✅ Required for MMIO | Wire `@repr(C)`/`@packed` through to codegen |
| `@link_section` | NOT_IMPL | ✅ Required for vector tables | LOW effort annotation wiring |
| callconv Naked | NOT_IMPL | ⚠️ Tiny audited surface only | Annotation-based, ISR/boot only |
| volatile | PARTIAL — parsed/typed; runtime ignores; SFFI builtins missing | ✅ Required for MMIO correctness | Add SFFI builtins; wire compiler backend |
| C import / bindgen | PARTIAL — runtime WFFI exists; no compile-time bindgen | ⚠️ Curated API only | `simple-bindgen` tool + `@wffi` annotation |
| Cross-compilation | PARTIAL — triples documented; backend incomplete | ✅ Required for embedded | Wire backend target selection; add presets |
| Inline test{} | NOT_IMPL | ✅ Required — tests must not ship in ROM | LOW effort new keywords |
| Error return trace | PARTIAL — panic traces exist; error-prop trace missing | ✅ Fixed-size, no heap | Separate task |
| Sentinel types | NOT_IMPL | ⚠️ Safe if type-system enforced | Research needed first |

---

## Implementation Priority

### Tier 1 — Near-Term (High ROI, Low-Medium Effort)
...
| CT semantic checker (warn when comptime ≠ RT) | MEDIUM | Makes existing `comptime` syntax meaningful |

### Tier 2 — Medium Term (High Embedded ROI, Medium Effort)
...
| Feature | Status |
|---------|--------|
| Error return trace | Separate task — planned |
...
|-----------|-------|--------------|--------------|
| Volatile MMIO access | SFFI workaround | SFFI builtins ✅ | `volatile ptr<T>` ✅ |
| C-layout structs for registers | ❌ | `@repr(C)` ✅ | + `bitfield` ✅ |
| CRC/lookup tables in ROM | Runtime only | CT checker warns | CT eval engine ✅ |
| ISR handler in Simple | SFFI only | `@callconv("Interrupt")` ✅ | + `@link_section` ✅ |
| Boot/startup in Simple | SFFI only | `@callconv("Naked")` ✅ | + naked asm ✅ |
| Tests excluded from ROM | Manual / separate files | `test{}` blocks ✅ | — |
| Cross-compile target | `--target=embedded` | Target triples ✅ | + presets ✅ |
...
| 10 | sentinel types | 📄 RESEARCH | `doc/01_research/sentinel_types_design_2026-02-18.md` |

**Tests:** 13 new spec files added across `test/01_unit/compiler/`, `test/01_unit/app/`, `test/01_unit/std/`, and `test/02_integration/`
