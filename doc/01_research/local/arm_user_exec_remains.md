...
- ARM64 now performs live EL0 entry for the filesystem-loaded proof ELF: the runtime helper programs `SP_EL0`, `ELR_EL1`, and `SPSR_EL1`, executes `eret`, catches the proof ELF's `svc #0` in the lower-EL AArch64 synchronous vector, prints the success markers, and exits QEMU.
...
- ARM64 lower-EL SVC handling is proof-specific for `svc #0` exit. General syscall dispatch still needs a real trap frame and syscall ABI path.
- ARM32 still needs ABI/output stabilization before replacing numeric traces with readable user-task markers.

## Implementation Decision

The implemented slice keeps the QEMU filesystem-load acceptance green, removes the old synthetic VMM sentinel from ARM64, verifies one PT_LOAD record at the scalar image level, stages PT_LOAD contents into the direct-load RAM arena, builds and verifies a user page-table translation for the staged entry, probes TTBR0_EL1 with restore, resolves and byte-checks the staged entry for the task context, validates an x86-shaped first-user handoff frame, and completes a live EL0 `eret` plus proof `svc #0` exit. Live ARM TTBR0 activation under MMU-on conditions, general ARM64 SVC/syscall handling, aggregate segment-array repair, and scheduler-integrated task context restore remain explicit follow-up work.
