...
- ARM64 now has an x86-shaped user-entry bridge (`dispatch_enter_user_blocking_probe`) that validates the scheduler handoff frame through one runtime helper. The bridge accepts the current direct physical entry only when it matches the page-table translation of the ELF virtual entry, then records the ELF virtual entry for the live handoff.
- ARM64 now performs live EL0 entry for the filesystem-loaded proof ELF at the ELF virtual entry under the user TTBR0 root: the runtime helper programs MAIR/TCR/TTBR0, enables SCTLR_EL1.M, programs `SP_EL0`, `ELR_EL1`, and `SPSR_EL1`, executes `eret`, catches the proof ELF's `svc #0` in the lower-EL AArch64 synchronous vector, prints the success markers, disables the MMU for the semihosting exit, and exits QEMU. The lower-EL SVC path saves/restores a GPR frame, dispatches non-exit SVC IDs through `userlib__syscall_raw__syscall`, returns the result in user `x0`, advances `ELR_EL1`, and `eret`s back to EL0.
- ARM64 now runs a non-live virtual-entry preflight before live entry. The preflight maps/proves a compact EL1 support window in the user root from linker `_start` through `_sbss`, plus the current EL1 stack page and UART MMIO. It verifies ELF virtual entry translation, the initial `SP_EL0` page, vectors/lower-EL SVC handler/syscall handler/live handoff code, and UART MMIO translation. The support window no longer maps the reserved boot stack range or heap reservation wholesale. User executable PTEs now set PXN while leaving UXN clear, so EL0 can execute text without allowing EL1 execution from user pages.
...
- ARM64 now builds a user page-table image for the loaded PT_LOAD pages, performs a controlled TTBR0_EL1 write/read/restore probe while SCTLR_EL1.M is off, passes the virtual-entry preflight, and enters the filesystem-loaded ELF through its virtual entry with TTBR0/MMU enabled. Remaining MMU work is hardening rather than first launch: kernel/user TTBR split, rollback, fault handling, and replacing the compact early identity support window with a proper kernel mapping half.
...
The implemented slice keeps the QEMU filesystem-load acceptance green, removes the old synthetic VMM sentinel from ARM64, verifies one PT_LOAD record at the scalar image level, stages PT_LOAD contents into the direct-load RAM arena, builds and verifies a user page-table translation for the staged entry, probes TTBR0_EL1 with restore, resolves and byte-checks the staged entry for the task context, validates an x86-shaped first-user handoff frame, proves the compact virtual-entry support mappings, and completes a live EL0 `eret` to the ELF virtual entry with TTBR0/MMU enabled plus lower-EL SVC dispatch/return path. Aggregate segment-array repair, full trap-frame/scheduler integration, kernel/user TTBR split hardening, and scheduler-integrated task context restore remain explicit follow-up work.
