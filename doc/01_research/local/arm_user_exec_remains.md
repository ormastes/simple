...
- ARM64 now builds a user page-table image for the loaded PT_LOAD pages, performs a controlled TTBR0_EL1 write/read/restore probe while SCTLR_EL1.M is off, passes the virtual-entry preflight, and enters the filesystem-loaded ELF through its virtual entry with TTBR0/MMU enabled. Remaining MMU work is hardening rather than first launch: kernel/user TTBR split, rollback, fault handling, and removing the broad early identity support window.
