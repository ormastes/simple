...
- `test/03_system/feature/usage/cooperative_green_compiled_handle_array_blocker_spec.spl` keeps the remaining standalone native cooperative-green function-spawn crash explicit: interpreter and SMF pass, but current native workloads that store multiple `GreenThreadHandle` values returned from `cooperative_green_spawn(worker)` still segfault, so the profile lane remains on `cooperative_green_spawn_value`.
