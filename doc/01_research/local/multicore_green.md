...
- `test/03_system/feature/usage/cooperative_green_smf_function_global_blocker_spec.spl` keeps the remaining SMF function-valued global/global-array crash path explicit: both minimal SMF fixtures still compile, then currently crash at runtime, so delayed `green_spawn(fn)` storage remains blocked even though native/direct callback coverage is fixed.
