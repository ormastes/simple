...
- A full scheduler-aware multicore green runtime still needs the dispatch result wired into real task/context-switch execution, blocking syscall boundaries, and QEMU proof that work runs across multiple SimpleOS CPUs/APs.
...
- `test/01_unit/os/kernel/scheduler/green_carrier_spec.spl` verifies the SimpleOS bridge contract between logical green tasks, bounded carrier queues, SimpleOS reschedule IPI state, and per-CPU green dispatch selection.
