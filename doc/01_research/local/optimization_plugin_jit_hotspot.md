...
- `doc/04_architecture/simple_optimization_plugin.md` defines provider metadata, built-in vs dynamic providers, optimization plans, and lookup responsibilities.
...
- `src/compiler/60.mir_opt/mir_opt/pattern/rule_engine.spl` provides the current in-source provider schema:
  ...
  - hot path flag
  - fact gating helpers
- `doc/04_architecture/jit_interpreter_integration.md` and `doc/04_architecture/jit_index.md` describe the JIT interpreter path and execution manager, but they were not tied into the optimization plugin provider taxonomy.
