...
- `doc/01_research/local/shared_wm_renderer_unification.md` and
  ...
  contract.
- `test/system/wm_compare/backend_measurement_capture_spec.spl` and
...
- `test/system/wm_compare/comparison_failure_report_spec.spl` already separates
  capture failure, metadata mismatch, structural/layout mismatch, exact pixel
  mismatch, and backend unavailability.
- `test/integration/rendering/backend_screenshot_compare_spec.spl` and
  `test/integration/rendering/screenshot_compare_helpers.spl` already cover
...
- `test/integration/rendering/screenshot_compare_helpers.spl` references
  ...
  capture harness.
- `test/integration/rendering/backend_screenshot_compare_spec.spl` validates
  comparison behavior with fixture buffers, not real Engine2D captures.
- `test/integration/rendering/engine2d_cpu_vulkan_parity_spec.spl` is named as
...
- `test/integration/rendering/engine2d_cpu_metal_parity_run.spl` is currently
...
- Extend `test/system/wm_compare/` with system-level contract specs after
