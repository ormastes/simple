...
Create `test/01_unit/app/io/process_limits_spec.spl`:
```simple
...
test_config {
    ...
    file_profiles [
        { pattern "test/02_integration/qemu/**" profile "intensive" }
        { pattern "test/01_unit/**" profile "fast" }
    ]
}
```
...
Create `test/01_unit/app/test_runner_new/resource_profiles_spec.spl`:
```simple
...
Update `test/01_unit/app/test_runner_new/test_runner_spec.spl`:
```simple
...
docker run --rm \
    ...
    test test/01_unit/std/string_spec.spl
...
Create `test/01_unit/app/test_runner_new/container_backend_spec.spl`:
```simple
...
describe "execution_strategy":
    it "selects container mode for QEMU tests":
        val strategy = strategy_from_test_file(
            "test/02_integration/qemu/boot_spec.spl",
            default_test_config()
        )
...
Create `test/01_unit/app/test_runner_new/resource_monitor_spec.spl`:
```simple
...
|-----------|-----------|-----------|---------|
| test/02_integration/qemu/boot_spec.spl | 195.3% | 87.2% | intensive |
...
| test/02_integration/compiler/large_file_spec.spl | 1,536 | 768 | slow |
| ... | ... | ... | ... |
      ...
      - name: Run integration tests (container)
        run: |
          docker run --rm \
            ...
            --cpus=4.0 \
            simple-test-runner:latest \
            test test/02_integration/ --mode=container
...
test-unit:
  ...
  image: simple-test-runner:latest
  script:
    - simple test test/01_unit/ --mode=native --profile=fast
  artifacts:
    reports:
...
test-integration:
  ...
  services:
    - docker:dind
  script:
    - simple test test/02_integration/ --mode=container --profile=slow
  artifacts:
    reports:
