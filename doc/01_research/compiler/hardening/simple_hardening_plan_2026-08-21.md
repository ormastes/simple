...
| 2026-08-24 | stage 3 self-host: the 405-error `HIR lowering error` blocker is STALE evidence | (this commit) | The lane evidence log (207 unresolved name / 152 field-not-visible / 42 unresolved type / 4 ambiguous callable) predates `e0e308c8681` ("explicit imports for glob-only dependencies — stage 3 HIR reach 600 -> 692/692, fatals 1400 -> 300"), which IS an ancestor of the lane HEAD `94564e9030e`. Static analysis of all **41 distinct (file, name) pairs** behind the 207 `unresolved name` errors confirmed ONE mechanism — **41/41 had `imported_in_use=0`**, i.e. every one referenced a symbol defined in a SIBLING file of the same package with no explicit `use` (9 names from `_Items/lowering_helpers.spl`, `hir_expr_env_get` from `_Expressions/expression_support.spl`, `X86_OP_CALL`/`op_imm` from `native/mach_inst.spl`, `local_count_increment` from `mir_opt/var_reassign_analysis.spl`, `is_lld_link_available` from `linker/msvc.spl`) — the class the in-tree comment at `70.backend/linker/link.spl:25` already documents and `e0e308c8681` already fixed. The 152 field errors are covered by the guards at `_Expressions/expression_support.spl:332-385`, whose recorded measurements show `branch=retained-false` (the only genuine private-field verdict) fired **0 times** across two full 692/692 runs. Re-measured at HEAD: **0** of each error class. **Two leads killed so they are not re-chased:** `src/compiler/{hir,backend,frontend,loader,mir_opt,types}` are SYMLINKS to the numbered dirs (so "duplicate module trees" is false, and the loader's `logical=832 physical=614` gap is that aliasing); and 336 of the 376 `[hir-callable-dep-origin-unresolved]` warnings name `Option`/`Result`/`Dict` — prelude types — and never became errors. | §27 | `doc/08_tracking/bug/stage3_selfhost_blocker_reclassified_2026-08-24.md` |
| 2026-08-24 | the admitted Stage-2 compiler cannot compile a TWO-LINE hello world — every Stage-3 verdict measured against it is unfalsifiable | (this commit) | **Found by shrinking the input, not by reading the 16k-line Stage-3 log.** Binary: `build/bootstrap/goal-stage2/stage2/x86_64-unknown-linux-gnu/simple`, 132,944,880 B, mtime 2026-08-24 01:34, `simple-bootstrap 1.0.0-RC`, admitted with a provenance receipt. Compiling the compiler with it gave 0 HIR lowering errors and `rc=139` (SIGSEGV) at `[build] hir 0/692 ... app.cli.bootstrap_main`; a line-buffered rerun under `ulimit -s unlimited` reproduced it identically, ruling out both the block-buffered log's lost tail and stack overflow. The shrink settled it: a two-line hello world fails on **BOTH** commands the bootstrap CLI exposes — `compile --format=smf` rc=1 and `native-build` rc=1 — each printing ``error: hir codec: no `Visibility` arm for tag -1; regenerate src/compiler/20.hir/generated/hir_codec.spl``. An UNSET (-1) Visibility tag reaching the codec is the same malformed-tagged-value family as `7c453e7b076` and `e52f3e4de26`, both of which POST-DATE that binary. **So the artifact, not the sources, is the blocker, and nothing measured with it can clear or condemn `src/compiler` at HEAD** — the SIGSEGV included. **Gate defect exposed:** Stage 2 was admitted and Stage 3 run against it without anything ever asking the artifact to compile a hello world — the exact shape `scripts/check/check-stage-binaries-runnable.shs` exists for (a stage binary whose `--version` answers cleanly while both real commands fail), and that guard is ADVISORY and honestly RED. The Stage-2 ADMISSION path needs that smoke test before the receipt is issued. **Correct next action: rebuild Stage 2 from HEAD** (which carries `7c453e7b076`, `e52f3e4de26`, `63f4b5d1362`, `e0e308c8681`) and re-run Stage 3. **Not done here:** the rebuild, and no source change — documentation only, zero `rt_*` call sites added. | §27 | Verbatim, exit status read into a variable never through a pipe, `SIMPLE_TIMEOUT_SECONDS=0`: hello-world `compile --format=smf` -> `rc=1` + the codec line; hello-world `native-build` -> `rc=1` + the same codec line; full `compile --format=smf src/app/cli/bootstrap_main.spl` -> `Segmentation fault`, `rc=139`, `grep -c 'HIR lowering error'` = 0, `grep -c 'not visible from this module'` = 0. |
| 2026-08-24 | stage3 self-host / module resolution (lane E4, cross-cutting hypothesis) | (this commit) | **The duplicate-compiler-module-tree hypothesis is REFUTED, and the spurious `ambiguous explicit callable dependency` is a re-export hop `item_name` defect.** All 17 unnumbered dirs under `src/compiler/` (`backend`, `hir`, `loader`, `frontend`, …) are **symlinks** to their numbered counterparts (`70.backend`, `20.hir`, `99.loader`, `10.frontend`, …) — one physical tree, 608 distinct realpaths behind the 609 distinct paths in `/mnt/data/goal-logs/stage3-failure.log` — and both spellings normalize to ONE module ID (compiling the unnumbered `src/compiler/backend/linker/link.spl` yields owner `compiler.backend.linker.link`; the numbered `70.backend/backend/llvm_backend.spl` yields `compiler.backend.backend.env`). No dual registration exists, so duplicate trees explain **0 of the 405** HIR lowering fatals; lanes E/E2/E3 must stop attributing their clusters to it. **What the ambiguity really is:** a cheap single-file reproducer (`SIMPLE_AMBIGDBG=1 stage2 compile --format=smf -o /tmp/link.smf src/compiler/backend/linker/link.spl`, rc=139 — the SEGV is the separate known stage-binary class, not chased) shows both `DiContainer` candidates naming the **same** target `compiler.common.di` (which has exactly ONE declaration tree-wide, `src/compiler/00.common/di.spl`), differing only in the ITEM slot where one carries the *target module's own name*; the sweep's `selected_item != candidate_item` test then declares ambiguity. **Universal, not anecdotal: of the 35 multi-candidate dependencies in that run, 35/35 have all candidates on one target and 35/35 carry an `item == module-name` candidate — zero counterexamples.** Distinct from the genuine two-owner `Backend` collision fixed by explicit-over-glob precedence (`ambiguous_explicit_callable_dependency_backend_env_2026-08-22.md`). The corrupt item originates in `find_reexport_source` (`chase … wanted=DiContainer found=true target=compiler.common.di item=compiler.common.di`), whose `found: true` sites return `wanted` / `source_item` / `exp_source` / `origin_lookup.source_name` — so a surface array is populated with a module name upstream, most likely for the brace-less dotted import form (`use compiler.common.di.DiContainer`), exactly how both failing deps are spelled. **Fix NOT landed, deliberately:** the stage2 binary is prebuilt, so editing `src/compiler/**` cannot change its behaviour and confirming the upstream site needs a stage2 rebuild (hours); and a sweep-side dedupe is **rejected** because the same hop feeds `register_imported_symbol_inner`'s first-match-wins facade chase, where a corrupt `item_name` binds the wrong symbol *silently* — a plausible mechanism for part of the 207 `unresolved name` fatals, so the upstream fix may clear far more than the 4 ambiguity errors. Heed `expression_support.spl:392-402`: a prior permissive change here drove this diagnostic 4 -> 447 and total fatals 312 -> ~6517, so measure only on a whole-tree run. **Evidence of:** `doc/08_tracking/bug/duplicate_module_trees_refuted_reexport_hop_item_name_2026-08-24.md`; no product code changed. |
| 2026-08-24 | phase-1 test-failure sweep, root-caused by reason bucket rather than per-spec (1,293 failures / 466 specs) | (this commit) | **Two root causes account for 327 of the 1,293 failures; the largest was not a code defect at all.** (1) `semantic: unknown extern function: rt_heap_ref_wellformed` — 373 error lines across **89** specs, the single biggest bucket — is **entirely a stale deployed binary**. The adapter landed at `fa4ca4aa7f9` (2026-08-23 09:29) with the record `seed_interpreter_extern_missing_rt_heap_ref_wellformed_2026-08-23.md`; the binary the sweep ran on was built 2026-08-23 04:47 and `strings <binary> \| grep -c rt_heap_ref_wellformed` = **0**. Zero source changes were needed and none of the 89 specs was touched. (2) A real, previously unfixed seed defect: **a group import of a self-named module shadows the class it contains**, so `use a.b.Foo.{Foo}` binds the module *namespace dict* to `Foo` and every static call through that name dies with ``method `X` not found on type `dict` ``. The receiver debug text is the proof — the constructor `Foo` AND the mangled static `Foo__X` are both sitting in the dict the call was made on. Fixed in `interpreter_method/mod.rs` by extending the existing Dict-receiver arm: scan for `Value::Constructor { class_name }` entries whose `{class_name}__{method}` is also a function in the same dict and dispatch it; two matches stay ambiguous and fall through to the existing error. Deliberately narrow — it only fires where dispatch **already** errored, so it cannot change a call that previously resolved, and import binding order (the broader, riskier design question) was left alone. (3) Smaller: `TreeSitter.new` had been desugared away in `outline_lexer.spl` into the free `treesitter_new` while 6 specs still call the documented `TreeSitter.new(src)`; a `static fn new` delegating to that single implementation was added to the existing `impl TreeSitter:` block in `outline.spl` rather than duplicating logic or repointing the specs. **Deliberately NOT done, and stated rather than implied:** the ~91-spec source-text-pinning class (specs asserting on literal banner comments, `use` lines and debug-log strings inside compiler source) was sampled, not batch-edited — `callable_dependency_route_precedence_contract_spec.spl` pins `selected_route_is_named`, which exists **nowhere** in `src/`, i.e. the mechanism was reimplemented, not moved, so "repoint the assertion" would launder a product question into a green test. That class needs per-spec judgement and is left open. | §27 | **Measured on this tree, 8-way max, `--no-cover-check --no-self-protect`, `SIMPLE_TIMEOUT_SECONDS=0`, exit status read into a variable never through a pipe.** Extern bucket, all 89 specs re-run on a `cargo build --release --bin simple` of this tree: **590 examples, 141 passed / 449 failed -> 432 passed / 158 failed**, 48 specs going fully green (291 failures cleared, zero source change). Dict-shadowing fix, verbatim before -> after: `test/01_unit/compiler/mdsoc/transform_adapters_spec.spl` `Results: 67 total, 35 passed, 32 failed` -> `Results: 67 total, 67 passed, 0 failed`; `module_resolver/group_import_self_named_module_spec.spl` `2 total, 1 passed, 1 failed` -> `2 total, 2 passed, 0 failed`; `module_resolver/group_import_shadowing_generalization_spec.spl` `3 total, 2 passed, 1 failed` -> `3 total, 3 passed, 0 failed` (those two specs were already in the tree as the reproduce cases). `TreeSitter.new`: `parser/treesitter_spec.spl` `11 total, 0 passed, 11 failed` -> `11 total, 1 passed, 10 failed` — construction now resolves and the residual is a **deeper product defect** (`parse_outline` returns an empty module under the interpreter lane), reported as such, not claimed fixed. 50-spec regression batch: 86 -> 50 failures, **one** apparent regression, `codegen/packed_array_copy_roundtrip_spec.spl` `5/5/0` -> `5/4/1`, empirically **excluded** as mine by rebuilding the seed with the patch reverted and re-running: `NOPATCH-RESULT: Results: 5 total, 4 passed, 1 failed`, identical — it is a stale-binary-vs-HEAD difference. The 5 baseline HUNG specs were untouched by this work and remain hung. **Limit, stated not hidden:** every number here is on the **Rust seed**; per `.claude/rules/bootstrap.md` they must be re-measured on a self-hosted `bin/simple`. No suite-wide improvement is claimed — only the 139 specs actually re-run. |
| 2026-08-24 | hir lowering / composite field index (lane E4) | (this commit) | **The composite field index was keyed by BARE type name; it is now additionally keyed per declaring module.** Two modules may declare the same composite name, and when both registrations land in ONE lowerer's map — which happens as soon as a module declares a composite and also imports a same-named one — the second write silently overwrote the first, so every field read against the loser resolved through the WINNER's field set and the loser's WHOLE field set read as invisible. Measured in the stage-3 self-host run: `JitInstantiator` is declared by both `src/compiler/99.loader/jit_instantiator.spl` (470 ln, `class`) and `src/compiler/99.loader/loader/jit_instantiator.spl` (541 ln, `struct`) — drifted copies, same field NAMES, different field TYPES (`in_progress: [text]` vs `Set<text>`, `jit_cache: Dict<text, JitCacheRecord>` vs `Dict<text, ([u8], i64)>`) — accounting for **83 of the 405** HIR lowering fatals. **Fixed the KEYING, not the duplicate:** a bare-name key is a latent defect for ANY two same-named types, so renaming a copy would clear 83 errors and leave the landmine armed. **The consumer side already prefers a module-qualified key** — `composite_name_for_owner_raw` (`_Expressions/expression_support.spl`) builds `defining_module + "." + name` and only falls back to the bare name when absent — and the IMPORT path already registered one (`module_import_registration.spl`, `canonical_composite_name`); **local declarations were the only writer that never did**, so the lookup always fell back to the colliding bare key. `prescan_composite_field_types` and `register_struct_field_types` (`_Items/module_callable_types.spl`) now also write `self.module_filename + "." + <name>` into all four maps. Safe by construction: **additive** (the bare key is still written, so a consumer with no owner symbol resolves exactly as before), **spelling matched to the reader** (`declare_module_symbols` passes `self.module_filename` as `defining_module`; a spec case pins the correspondence so the sides cannot drift), **guarded on empty** (an empty filename would produce the bogus key `.Name` and re-collide everything), and **no COW alias** (each write mutates the owning map directly). **Evidence of:** `test/01_unit/compiler/hir/composite_field_index_module_qualified_key_spec.spl` — pre-fix `Results: 5 total, 2 passed, 3 failed` rc=1 (the two core cases fail `method 'has' not found on type 'nil'` — the qualified key does not exist), post-fix `Results: 5 total, 5 passed, 0 failed` rc=0; the 2 cases green in BOTH states are deliberate no-regression pins. Neighbouring specs byte-identical with and without the fix (`compiler_module_scoped_hir_lowering_spec` 1 passed/1 failed both ways; `hir_method_call_receiver_payload_spec` 0/2 both ways) — those failures are pre-existing. **NOT yet proven:** the 83 stage-3 errors are not measured as cleared; that needs a stage-2 rebuild + whole-tree run, and per `expression_support.spl:392-402` (a prior permissive change here drove fatals 312 -> ~6517) the next run must be diffed against a recorded baseline. Analysis: `doc/08_tracking/bug/duplicate_module_trees_refuted_reexport_hop_item_name_2026-08-24.md`. |
| 2026-08-24 | Fabricated guest success markers: five `rt_riscv_*` probes were unconditional `return 1;` | (this commit) | **The riscv64 SMF transcript was a constant, not evidence.** `full_networking_runtime.c:24-41` defined `rt_riscv_nvfs_probe`, `rt_riscv_smf_cli_probe`, `rt_riscv_smf_cli_load`, `rt_riscv_smf_gui_probe` and `rt_riscv_native_gui_process_render` with bodies of exactly `return 1;`, so `[riscv-nvfs] image read ok`, `FS_MOUNT_OK`, `SMF_DISCOVERY_OK`, `ELF_LOAD_OK`, `SMF_CLI_LAUNCH_OK`, `SMF_WM_GUI_LAUNCH_OK` and `NATIVE_GUI_PROCESS_RENDER_OK` printed on every boot regardless of capability — the exact five rows baselined at `config/simpleos_fabricated_rt_baseline.sdn:219-223`. **They were NOT auto-stubbed:** the riscv64 link path links no `auto_stubs.c` (`simpleos_native_linkers.spl:325`) and the Rust stub generator emits `return 0;` (`stubs.rs:346`), which takes the failure branch and cannot launder anything. A tree-wide scan of every `*.c` under `examples/09_embedded/simple_os` and `src/os/kernel/arch` finds **exactly these five and nothing else**, so the success-emitting fabrication population is bounded at 5, not 55 — the other 50 baselined symbols are `simpleos_wm_production_desktop.elf` infrastructure with no marker attached. Fixed by announcing, not deleting: each now writes `STUBBED <name>` to serial via the non-static `rt_riscv_uart_put` and returns 0, so the symbols still link but a transcript can never again read as proof. Second defect found and partly fixed: 33 sites across 8 guest entries print `TEST FAILED` then call `rt_qemu_exit_success()` within 3 lines; 9 riscv64 sites flipped to `rt_qemu_exit_failure()`, 24 frozen shrink-only because `rt_qemu_exit_failure` is defined in exactly ONE place tree-wide (`baremetal_runtime_core.inc.c:95`) and arm32/arm64/riscv32 have no definition, so flipping them would break the link or auto-stub the symbol. **Nothing was weakened:** two consumers (`check_simpleos_riscv_telnet_serial.shs`, `riscv_smf_appscan_qemu_spec.spl:66-72`) grade on the transcript and will now honestly go RED — that is the intended outcome, and their prior passes are void. New gate `scripts/check/check-no-fabricated-success-markers.shs`. Record: `doc/08_tracking/bug/fabricated_guest_success_markers_riscv64_smf_2026-08-24.md`. | §27 | Every rc read directly into a variable on the line after the invocation, never through a pipe. `sh scripts/check/check-no-fabricated-success-markers.shs --selftest` -> `PASS — 6 selftest fixture(s) checked, 0 failures` SELFTEST_RC=0. `sh scripts/check/check-no-fabricated-success-markers.shs` -> `PASS — 403 file(s) scanned, 0 fabricated-success site(s) (0 unconditional-success rt_* stubs; 24 failure-exits-success site(s) frozen in scripts/check/fabricated_success_exit_baseline.txt, 0 new, 0 stale)` SCAN_RC=0. **Real-tree incident replay** (restore `return 1;` on `rt_riscv_nvfs_probe`) -> `FAIL — 403 file(s) scanned, unconditional-success-stub:./examples/09_embedded/simple_os/arch/riscv64/boot/full_networking_runtime.c:58` REPLAY_RC=1; restored -> RESTORED_RC=0. `clang --target=riscv64-unknown-none-elf -march=rv64gc -mabi=lp64d -ffreestanding -fno-pic -nostdinc -fsyntax-only .../full_networking_runtime.c` CLANG_RC=0. **Still open, stated not glossed:** `isa-debug-exit` at `scripts/os/run_x86_64_fs_exec_ovmf.shs:74` on a lane whose `:70-71` OVMF pflash is otherwise compliant — removing the device makes the guest's 0xf4 write a no-op so the runner must own termination via a teed serial transcript + timeout, not attempted rather than done half-way. `-kernel` at `qemu_systest_contract.spl:140` boots exactly the ELF this row proves fabricated, so migrating it is moot until the capabilities exist; `-kernel` at `:227` (arm64) stays blocked on a rebuilt kernel with no self-hosted `bin/simple` (Stage 3 SEGVs). |
| 2026-08-24 | `fn main(args)` entrypoint parameter is never bound — deterministic type confusion, not an ergonomics gap | (this commit) | **Sharpened the sibling lane's finding on both axes and swept every remaining declaration.** The typed form `fn main(args: [text])` with NO argv runs rc=0 and prints `args.len()` = `8246223157400007265` — that is `0x72707845736d7261`, the little-endian ASCII bytes `armsExpr`, a fragment of an interned compiler identifier string, and it reproduced **byte-for-byte across 3 consecutive runs**. So this is not uninitialised noise (ASLR would have varied it): a non-array heap value is being read as the array header, and anything indexing with that length is an out-of-bounds read. With argv present the same form does **not** "fail semantic analysis" as previously recorded — it **SEGVs, rc=139, core dumped**, which is strictly worse than a diagnostic. The untyped form `fn main(args)` is a *different* defect: `args` binds to nil and `args.len()` is `-1` with and without argv, which meant every `if args.len() > N` dispatch in the three `package/main.spl` tools was dead code that always fell through to `show_help()`. `fn main()` + `get_args()` is correct (`get_args()[0]` is the script path). **Fix chosen: migrate all call sites now, recommend a hard compile error, do not implement it here.** Wiring argv properly is a real feature in `src/compiler/**` / `src/compiler_rust/**`, both under active contention by four Stage-3 lanes, so it is reported for routing rather than edited. Rejecting the form at semantics is fail-closed and has a migration path (`get_args()`) that ships in this very commit — but a silent wrong answer is the worst possible behaviour, so until either lands the ratchet below is the enforcement. **5 declarations migrated** (all that the census found live): `src/lib/{nogc_sync_mut,nogc_async_mut,gc_async_mut}/package/main.spl` (byte-identical triplet, untyped form), `src/os/port/e2e_verify.spl` (typed), `src/os/tools/simplebox/simplebox_main.spl` (typed). simplebox is busybox-style — `simplebox_requested_applet` reads `argv[0]` to select the applet — so it passes the whole `get_args()` vector through unmodified instead of using the `_user_args()` helper the others share. Deliberately NOT migrated and stated as such: `src/os/runtime/baremetal/runtime_minimal.spl:67` (`fn main(argc: i32, argv: u64) -> i32`, a C-ABI baremetal entry — a different contract) and `src/compiler_rust/lib/std/src/tooling/html_utils.spl:361` (a module-local helper that merely happens to be named `main`). The doc comment at `src/app/__init__.spl:51` taught the broken form to every reader; it now shows `fn main():` + `get_args()` and carries the measured numbers so nobody re-derives them. **Unmasked, not caused, and not papered over:** with argv now real, `package/main.spl` fails loudly with `error: semantic: variable 'PackageList' not found` — it never imported its sibling classes and those files carry no `export` line, so the module has never worked as a CLI; the nil binding was hiding that behind a help screen. Fail-closed beats silently-wrong, so it is left loud and filed rather than patched over. | §27 | Seed `bin/release/x86_64-unknown-linux-gnu/simple`, 60650360 bytes, mtime 2026-08-23 04:47:05Z; `SIMPLE_TIMEOUT_SECONDS=0`; every rc read directly into a variable, never through a pipe. **Reproducers, verbatim:** `fn main(args: [text])` no argv -> rc=0 `typed n=8246223157400007265` (x3 identical); with argv -> `Segmentation fault (core dumped)` rc=139. `fn main(args)` -> rc=0 `untyped n=-1` both with and without argv. `fn main()` -> rc=0 `noargs ok`. `get_args()` with `p q` -> `arg[0]=scratch/e.spl` `arg[1]=p` `arg[2]=q`. **Stdlib-resolution precondition settled before trusting any verdict** (the lane worktree has no `bin/simple`): a probe `print` inserted at `src/lib/nogc_sync_mut/io_runtime.spl:329` is NOT reached for a script under `/tmp`, but IS reached (`LANE_H_PROBE_MARKER`) for a script inside the worktree — so worktree-local spec runs exercise this lane's stdlib, and the probe was reverted. **Ratchet spec** `test/01_unit/language/entrypoint/main_args_parameter_unsound_spec.spl` walks the entrypoint directories and fails on any `fn main(<param>)`; it carries 5 detector self-checks (typed / untyped / renamed param must flag; `fn main():` and `fn main() -> i32:` must not; `fn main_helper(x)` must not) plus a `scanned > 0` non-vacuity assert, so an empty or broken scan cannot read as a pass. BEFORE (fix stashed): `outcome=ERROR declared>=6 executed=6 passed=5 failed=1`, `Results: 6 total, 5 passed, 1 failed`, rc=1, naming all five offenders. AFTER: `outcome=OK declared>=6 executed=6 passed=6 failed=0 skipped=0 dropped=0`, `Results: 6 total, 6 passed, 0 failed`, rc=0. **Functional proof, not just the ratchet:** `bin/simple run src/os/port/e2e_verify.spl -- --dry-run` now rc=0 and prints the full six-check plan and QEMU command line — the same invocation SEGVs on the pre-fix source. No mirror copy under `test/unit/language/` was made: that directory does not exist. | 

| 2026-08-24 | Stage 3 self-host — backend/frontend/mir_opt/types unresolved-name+type class (lane E3) | (this commit) | **Root cause: genuine missing/incorrect module imports, not a Stage-2 resolver bug.** The Rust seed resolves a callee that is merely *reachable* (sibling file in the same package, a `use` written inside a function body, a runtime entry with no `extern` declaration); Stage-2 pure-Simple HIR lowering resolves **strictly from the module's own explicit import set**, so every such site becomes `unresolved name`/`unresolved type`. Diagnostics are reported at the *import block's* column (`:31:24`, `:32:18`, `:14:8`), never the call site, which is why one broken edge reads as dozens of errors and why the class looked like a resolver defect. **Two hypotheses were tested and DISPROVEN before any edit**, and are recorded so they are not re-chased: (a) *duplicate module trees* — `src/compiler/{backend,frontend,mir_opt,types,…}` are 17 git-tracked **symlinks** onto the numbered `{70.backend,10.frontend,60.mir_opt,30.types,…}` dirs, so the enumerator does see each file under two spellings, but HIR dedup is sound: 692 modules, **0 duplicate module ids**; (b) *spelling-split package scope* — every failing file and its defining sibling registered under the **same** spelling and same module prefix, so no package was split. **13 files fixed, +19/-6 lines, each the smallest semantics-preserving edit:** names added to an existing explicit `use` group (`var_reassign_local_id_value`, `mode_default_optimization`, `X86_OP_CALL`/`X86_OP_CALL_INDIRECT`/`op_imm`, `translate_call`); a new sibling import (`methods_push`/`fields_push`, `parser_validate_unit_suffix`, `format_shape`, `TreeSitter`, `CodegenTarget`, `syntaxfeatures_default`/`blockoutlineinfo_opaque`); a function-body `use` hoisted to module level (`is_lld_link_available`/`is_msvc_available` in `link.spl` — the same file already carried a comment from an earlier session documenting exactly this seed-vs-stage2 divergence for two other symbols, so the pattern is established, not invented here); a wrong import source corrected (`BackendCompileOptions` was imported from `compiler.common.driver_core_types`, which **does not define or re-export it** — the only definition is `backend_types.spl:251`); and two `extern fn` declarations added for runtime entries already being called (`rt_file_read_bytes`, `rt_env_get`). **A move of `format_shape` into `dim_constraints_types.spl` was attempted and REVERTED** — it calls `d.format()` from `impl DimExpr`, which lives in `dim_constraints.spl`, so the move traded an import edge for an impl-level cycle; a plain explicit import is both smaller and safer. **Verified by real recompile with the admitted Stage-2 binary, before/after, not by inspection:** the harness was validated with a control — reverting only the `outline_members` import reproduces **8 `methods_push` + 2 `fields_push`**, byte-matching the counts in `stage3-failure.log`, and restoring it yields **0**. Each fixed file now reports `[bootstrap-error-count] point=post-lowering count=0` with zero `hir-fatal` attributed to itself. **Incidental Stage-2 defects found and NOT fixed here (filed for the owning lanes, not papered over):** standalone `compile` on any compiler source SEGVs *after* lowering (rc=139), so single-file verification reads the lowering counters and ignores the crash; `compile` on a *numbered* path (`src/compiler/10.frontend/...`) fails entry collection with "collected zero source files" while the symlink spelling works; `parser_decls_use.spl:58` reports `unresolved name: __p-1` for `env_get`, which **is** explicitly imported at line 38 — a real resolver/diagnostic defect left to lane E; and `ambiguous explicit callable dependency DiContainer/AopWeaver in compiler.backend.linker.linker_context` is a DI-registration ambiguity, not an import gap. | **This row also CORRECTS the "the 405-error blocker is STALE evidence" row above, which was withdrawn by its author.** That row rested on a re-measure reporting 0 lowering errors at HEAD; the run that produced it SEGVd at `[build] hir 0/692`, so **zero of 692 modules lowered** and the phase never executed — the vacuous-run trap `.claude/rules/testing.md` pins ("a run that executed nothing is UNKNOWN, never a pass"), applied to a "0 errors" claim rather than a "0 tests failed" one. The positive control settles it: a detached worktree at `origin/main 8f4a17a6fc6`, compiled with that same admitted Stage-2 binary, reproduces **8 `unresolved name: methods_push` + 2 `fields_push`** in `outline_decls.spl`, byte-matching `stage3-failure.log`; and `e0e308c8681`, the commit cited as having already covered this class, touches 6 files (3 under `15.blocks/builtin_blocks_*`, 3 under `20.hir/`) with **zero overlap** with the 13 here, its own title reporting fatals 1400 -> 300 rather than 0. Because lane E's chosen approach is to fix sources and keep the resolver strict, these imports are **still required after the Stage-2 rebuild**, not obsoleted by it. Independent defects found while verifying are filed at `doc/08_tracking/bug/stage2_import_resolution_and_compile_harness_defects_2026-08-24.md`.
| 2026-08-24 | SimpleOS link line FIXED — aarch64 now builds and links; the "genuinely missing symbols" were never missing | (this commit) | **`aarch64-unknown-simpleos` goes FAIL -> PASS, and undefined `rt_*` goes 20 -> 0 on BOTH non-x86 arches.** The sibling lane's diagnosis ("the archive is resolved, reported, and then not linked") was exactly right; the mechanism is `NativeProjectBuilder::simpleos_user_runtime_paths()` in `70`-equivalent seed code (`src/compiler_rust/compiler/src/pipeline/native_project/linker.rs`), the ONLY thing that puts a runtime on the freestanding link line, failing **four** ways at once: (1) its arch guard was `X86_64 \| Aarch64`, excluding **Riscv64** entirely, so riscv64 got no crt0, no libc and no runtime at all; (2) it looked for the runtime only at `<sysroot>/lib/libsimple_runtime.a`, a path **no sysroot producer ever writes**, and never consulted `SIMPLE_SIMPLE_CORE_PATH` — which is exactly how the separately-built core archive is communicated, and why the archive the CI echoed as `runtime_archive=` was resolved and then dropped; (3) the all-or-nothing `crt0 && runtime && libc` test returned `None` when the runtime was missing, silently dropping **crt0 and libc too**; (4) it linked `libsimpleos_c.a` (libc ALONE) instead of `libsimpleos_all.a` (libc + the cross-compiled `src/runtime` C runtime). **Defect (4) retires the bug record's "1-3 genuinely missing symbols" claim as measurably FALSE, and the record has been corrected in this commit rather than left to mislead the next lane:** `rt_string_new_literal`, `rt_native_cmp` and `rt_unwrap_or_trap` are all defined in `src/runtime/runtime_native.c` and land in `libsimpleos_all.a`, which defines **761** `rt_*` symbols vs the core archive's 316. They read as missing only because the cross-check was made against the core archive alone while the link line carried NEITHER archive. `simpleos_guest_arch_id` is a different thing again — an unbacked `extern fn` in `src/app/simpleos_tool/guest_target.spl`, not a runtime symbol, and absent from the build. Also fixed: riscv64 mapped to the x86_64 unsuffixed `build/os/sysroot`, and `resolve_freestanding_linker_script` carried the same `X86_64 \| Aarch64` guard so riscv64 never got its `simpleos.ld`. **A SimpleOS link whose inputs cannot be resolved now fails CLOSED** with an actionable message naming the exact artifacts and producers, instead of silently linking with no runtime — the silent drop is what made this present as ~1000 undefined symbols instead of one clear error. **The one genuinely absent symbol**, exposed only once everything else resolved, was `__extenddftf2`: `long double` is IEEE binary128 on both ABIs and `strtold()` is `(long double)strtod(...)`, while these freestanding targets have no compiler-rt (`cc -print-libgcc-file-name` returns nothing for `aarch64-none-elf`/`riscv64-unknown-elf`). Implemented as a real exact conversion in `src/os/libc/simpleos_softfloat_builtins.c` — a **definition added to the C libc, not an `rt_*` call site**, so the commit is neutral against the `no-direct-rt` ratchet. **Nothing was stubbed, skipped, or silenced; no gate was weakened.** riscv64 now resolves every symbol and stops on a genuinely different, pre-existing defect that the broken link line had been masking, filed rather than papered over. | §27 / SimpleOS bootstrap | Seed rebuilt from this tree (`src/compiler_rust/target/release/simple`, the exact path `scripts/ci/build-simpleos-toolchain.shs` uses as `SEED`), `SIMPLE_TIMEOUT_SECONDS=0`, every rc read directly into a variable, never through a pipe. **aarch64 GREEN, twice from a clean sysroot rebuild:** `sh scripts/ci/build-simpleos-toolchain.shs --only aarch64` rc=**0**, `aarch64-unknown-simpleos: OK bin/release/aarch64-unknown-simpleos/simple (4393120 bytes, first PT_LOAD 0x0000000050000000)`, `RESULT: PASS`. **riscv64:** `grep -c "undefined symbol:"` on its build log = **0** (was 20 distinct, "referenced 978 more times"); now blocked solely on `ld.lld: error: crt0.o:(function _start: .text+0x9c): relocation R_RISCV_PCREL_HI20 out of range: -1048576 is not in [-524288, 524287]; references '__simple_startup_before_main'` — the weak-undefined-symbol address idiom cannot be formed PC-relatively, filed as `doc/08_tracking/bug/simpleos_riscv64_crt0_weak_undef_pcrel_out_of_range_2026-08-24.md`. Ruled out there by measurement rather than assumed: `-mcmodel=medany -mno-relax` added to all three riscv64 `.S` steps (crt0 was indeed the only riscv64 object built without the `medany` every other object gets) produced a **byte-identical** error, so that change was **reverted** rather than left in as an unevidenced codegen change. **`__extenddftf2` validated independently of the link:** bit-identical to the compiler's own `(__float128)` conversion across signed zeros, the minimum subnormal, subnormal-to-normal promotion, max/min normals, inf and NaN, plus **300,000 random doubles** — `bad=0`. Confirmed `DEFINED` by `nm --defined-only` in both `build/os/sysroot-{aarch64,riscv64}/lib/libsimpleos_all.a`. **Three new seed regression tests**, all reproduce-shaped (each fails pre-fix, because the fixture deliberately omits the `sysroot/lib/libsimple_runtime.a` the old code exclusively looked for): `test_simpleos_link_uses_core_archive_from_env_and_all_libc_archive` (all three arches; pins that the core archive comes from the env var and that libc resolves to `libsimpleos_all.a`), `test_simpleos_link_accepts_core_archive_file_spelling_and_falls_back_to_libc_only` (both env spellings, file and directory, both measured in use by the bug record), `test_simpleos_riscv64_gets_its_own_sysroot_and_linker_script`. `cargo test --release -p simple-compiler --lib simpleos_` -> **9 passed, 0 failed**, including the pre-existing `simpleos_freestanding_linker_script_defaults_and_overrides` and `freestanding_target_triple_keeps_none_and_simpleos_mappings`, so the riscv64 widening broke no existing pin. x86_64 unchanged and still has no sysroot producer. |
| 2026-08-24 | nvfs and dbfs: images now MOUNT and return seeded content; QEMU guest mount still blocked, with the wall named | (this commit) | **RETRACTION FIRST: this lane's earlier FAT32 verdict ("mounts and reads, real transcript", commit `038f1278541`) is WITHDRAWN.** All four markers it rested on — `FS_MOUNT_OK`, `SMF_DISCOVERY_OK`, `ELF_LOAD_OK`, `SMF_CLI_LAUNCH_OK` — are on Lane I's fabricated list: `return 1;` constants in `full_networking_runtime.c`. FAT32's honest status is unproven in QEMU, same as dbfs and nvfs. Retraction written into `doc/08_tracking/bug/fabricated_guest_success_markers_riscv64_smf_2026-08-24.md`, not just claimed in a report. **What actually advanced:** the previous row could only say the two filesystems *format*, which is a write-only claim — an image nobody can open is not a filesystem. Root cause found: `MemBlockDevice` had `write_to_file` and **no inverse**, so no code path in the tree could load an image off disk. Every existing image spec therefore re-derived the device in memory and asserted against that (`test/02_integration/storage/nvfs/nvfs_image_builder_spec.spl:105` reopens `_nvfs_image_mem_device(...)`, never a file), which proves the builder self-consistent and proves nothing about the bytes on disk; the DBFS side had no readback spec at all. Added `MemBlockDevice.from_file(path, sector_size)` — the mirror of `write_to_file`, deriving sector count from file length and rejecting a ragged image. Both filesystems now mount from a written image file and return their seeded content verbatim. **QEMU guest mount is NOT achieved, and the wall is now named precisely rather than guessed:** (a) `boot_fs_sequence()` (`src/os/kernel/boot/boot_fs.spl:106`) is the only code that mounts NVFS/DBFS and it has exactly ONE call site, `nvfs_positioned_entry.spl:34` — x86_64 only; **riscv64 is FAT32-only and has no dbfs/nvfs mount code at all**, so the suggested riscv64 route cannot carry these two filesystems; (b) `bin/simple os build` rejects the seed **by name** at `os_build_run.spl:432` (`version.contains("bootstrap seed only")`), on every arch — correct policy, left untouched; (c) the lower-level `native-build` path is separately blocked by a NEW defect: `--entry-closure` discovery parses partial-class fragment files standalone and cannot handle the `_Fat32Filesystem/` split (7 continuation files under one `class`/`impl` header in `filesystem_state.spl:1,25`), so the one entry that reaches `boot_fs_sequence()` will not build. Also found: **the DBFS QEMU gate could never have passed** — it boots `simpleos_x86_64.elf`, built from `os_entry.spl`, which never calls `boot_fs_sequence()`; and every marker it grades on is boolean-shaped, with nothing asserting mkfs-seeded bytes. Filed, not worked around. Nothing was weakened: no gate, discovery, or admission code was edited. | §27 | Seed `bin/release/x86_64-unknown-linux-gnu/simple`, `SIMPLE_TIMEOUT_SECONDS=0`, every rc read directly into a variable, never through a pipe. **New spec `test/02_integration/storage/fs_image_mount_roundtrip_spec.spl`: `Results: 5 total, 5 passed, 0 failed` rc=0.** It writes an image with mkfs, drops the in-memory device, reloads the image with `from_file`, mounts through the production driver and compares content: DBFS `/etc/motd` -> `"SimpleOS DBFS root\n"`, NVFS `/etc/motd` -> `"SimpleOS NVFS root\n"` (provider `nvfs-dbfs-backed-v1`), plus 3 negative cases (missing image, zero sector size, ragged 700-byte image) so the loader cannot pass by inventing a device. **Mutation-proved, and the first attempt FAILED this check:** asserting against the same constant used to seed was tautological — mutating it left the spec green (`5 passed` rc=0). Rewritten to assert independent literals; now mutating the seed constant alone gives `Results: 5 total, 4 passed, 1 failed` rc=1, so the expected text genuinely travels through the image file. Clean re-run rc=0. Independent corroboration: `strings` on the real 33 MiB mkfs outputs finds `SimpleOS DBFS root`, `/etc/motd`, `README` and `DBFS-rooted SimpleOS image` present as bytes. **QEMU attempts, both honest failures:** `bin/simple native-build --entry-closure --entry .../nvfs_positioned_entry.spl --backend cranelift --target x86_64-unknown-none` -> `Build failed: failed to parse .../_Fat32Filesystem/allocation_and_write.spl at 3:1 during discovery: Unexpected token: expected expression, found Indent` rc=1, no artifact. `bin/simple --version | grep -c 'bootstrap seed only'` -> `1`, i.e. the arch-independent rejection at `os_build_run.spl:432` is live. `check-simpleos-dbfs-root-qemu.shs` remains `ERROR: kernel ELF not found: build/os/simpleos_x86_64.elf` rc=3 — and per the new record it would grep the wrong kernel even if that ELF existed. Pre-existing specs still green: `dbfs_fs_driver_spec.spl` `Results: 13 total, 13 passed, 0 failed` rc=0; `positioned_binary_backend_parity_spec.spl` `Results: 9 total, 9 passed, 0 failed` rc=0. Records: `native_build_discovery_cannot_parse_partial_class_fragments_2026-08-24.md`, `simpleos_dbfs_qemu_gate_boots_a_kernel_that_never_mounts_dbfs_2026-08-24.md`. |
| 2026-08-24 | nvfs/dbfs over NVMe on an SBC: mount seam pinned to the BlockDevice trait; no SBC target declares NVMe | (this commit) | **The FS half is ready and now proven; the platform half does not exist yet, and saying so is the deliverable.** Seam: `DbFsDriver.open_on_device` (`dbfs_driver.spl:307`), `NvfsPosixDriver.new_on_device` (`nvfs_posix_driver.spl:49`) and `dbfs_superblock_set_device` (`dbfs_engine/superblock.spl:48`) all take `dev: BlockDevice` — the trait. Every pre-existing scenario passed a `MemBlockDevice`, which cannot tell "generic over the trait" from "accidentally concrete", so genericity was unverified. Two new scenarios mount both filesystems through `CachedRawImageBlockDevice` (`src/os/port/cached_raw_image_block_device.spl:61`), an unrelated impl. **Consequence: mounting over NVMe needs NO filesystem change** — `BootNvmeBlockDevice` (`nvme_boot_runtime_owner.spl:81`) is already a `BlockDevice` impl, so it is wiring, and no FS code was generalised because none needed to be. **The NVMe driver is real bring-up, not a model:** traced `read_sector` -> `_vfs_boot_nvme_read_sector_bytes:191` -> `sector_io.spl:337` -> `nvme_queue.spl:94-112` SQ writes and `:120` doorbell `mmio_write32`, CQ phase poll `:142`; BAR0 from the PCI grant (`driver_operations.spl:181/192/315`); PCIe enumeration real (`pcimgr.spl:53/78/183/209`). `nvme_storage_model.spl` is NOT a simulation backend — it is lease/namespace policy structs with no I/O. Only 5 of ~41 nvme files touch registers; `nvme_freestanding_controller.spl` does none and says so itself. Structural hazard checked: `_mmio_test_mode` (`boot/mmio.spl:15`) can silently replace all MMIO with an in-memory journal — same shape as this week's fabricated-marker class — but it IS mitigated on the boot paths, `mmio_disable_test_mode()` being called at `x86_64/arch_init.spl:47,84`, `arm64/console.spl:97`, `arm64/ramfb.spl:154`, `limine_boot_aarch64.spl:588`, `riscv64/fw_cfg_named_file_v1.spl:166,180`; any NEW guest entry reaching NVMe must call it or its transcript is journal output. **The blocker is the platform, and it is honest: no SBC target declares NVMe.** Of 8 catalog targets, 7 are QEMU machines; the only real SBC is `riscv64-starfive-jh7110` (VisionFive 2, `:436`, no `qemu_machine`) and it declares no NVMe — and riscv64 has no dbfs/nvfs mount code at all, since `boot_fs_sequence()` (`boot/boot_fs.spl:106`) has one call site, x86_64's `nvfs_positioned_entry.spl:34`. No Raspberry Pi / Rock Pi / Orange Pi / Jetson / BeagleBone target exists; every `board_lane` is `BoardCompileSmoke`, compile-only with no execution step. No committed transcript of SimpleOS on any physical machine exists: `build-simpleos-x86_64-board-usb.shs` is a complete image builder whose own header says "UNVERIFIED WITHOUT HARDWARE … does NOT prove the image boots real firmware", and `run_simpleos_physical_nvme_perf.shs` only validates a captured log (`--validate-log-only`). **Board-runnable is therefore NOT claimed** per `.claude/rules/board-runnable.md`; the record states the four things a board would have to show. No gate, boot, or firmware wiring was edited (boot/firmware is Lane K's). | §27 | Seed `bin/release/x86_64-unknown-linux-gnu/simple`, `SIMPLE_TIMEOUT_SECONDS=0`, every rc read directly into a variable, never through a pipe. `test/02_integration/storage/fs_image_mount_roundtrip_spec.spl`: `outcome=OK declared>=7 executed=7 passed=7 failed=0 skipped=0 dropped=0`, `Results: 7 total, 7 passed, 0 failed` rc=0 — 5 pre-existing plus the 2 new non-MemBlockDevice mounts, DBFS `/etc/motd` -> `"SimpleOS DBFS root\n"` and NVFS `/etc/motd` -> `"SimpleOS NVFS root\n"` through `CachedRawImageBlockDevice`. **Mutation-proved:** mutating the DBFS seed constant alone gives `Results: 7 total, 5 passed, 2 failed` rc=1 — both the file round-trip AND the new seam case go red, so content travels through the device rather than being asserted against itself; clean re-run rc=0. Honest cell-by-cell status — nvfs and dbfs are identical in every cell: host image file **PROVEN** (mount + byte-exact seeded-content readback); non-Mem `BlockDevice` **PROVEN** (same, via `CachedRawImageBlockDevice`); QEMU guest **NOT PROVEN** (no kernel ELF: `--entry-closure` discovery cannot parse the `_Fat32Filesystem/` partial-class split, and `os build` rejects the seed by name at `os_build_run.spl:432`); real NVMe device **NOT PROVEN** (driver is real MMIO but nothing has mounted a FS over it — reaching it requires the same missing kernel); SBC hardware **NOT PROVEN and not reachable from this host** (no SBC attached; no SBC target declares NVMe). Test-tree divergence delta for this range: `PASS — 3 pre-existing offender(s), 0 introduced by this range` rc=0; the pre-existing offender list is recorded in the commit message per the step-over rule. Record: `doc/08_tracking/bug/nvfs_dbfs_over_nvme_on_sbc_no_board_declares_nvme_2026-08-24.md`. |
| 2026-08-24 | Stage-2 `compile` dead on hello world — `hir codec: no `Visibility` arm for tag -1` (lane J) | (this commit) | **Producer found: 25 `SymbolTable.define()` call sites pass a BOOL where slot 5 is `visibility: Visibility`.** `define` (`hir_types.spl:316`) used to take `is_public: bool` in that slot; the signature changed and 25 sites in `src/compiler/20.hir/hir_lowering/**` were never migrated — they still passed the literal `false`, so `HirSymbol.visibility` held a boolean. A bool is not nil, so every nil-guard in the generated codec (`if node.visibility == nil: … else: hc_enc_visibility(…)`) took the ENCODE branch, the value matched none of the six `Visibility` variants, and `hc_enc_visibility`'s supposedly unreachable `case _:` (`generated/hir_codec.spl:6823`) fired `hc_bad_tag` -> `exit(1)`. The `-1` is a hardcoded sentinel in that fallthrough, not a decoded tag — do not hunt for where -1 is written. **Located by narrowing, not guessing:** the abort sits in `hir_cache_store` (`driver_hir_pipeline_lowering.spl:840` -> `driver_hir_cache.spl:187` -> `hir_module_encode`), which is why it landed immediately after `post-diagnostics count=0` and why `native-build` — which never takes that path — was rc=0 on the same binary; the kill switch `SIMPLE_HIR_CACHE=0` made the error vanish entirely, proving the codec was the messenger and lowering the producer. **Fix: all 25 sites now pass `Visibility.Private`**, exactly what `is_public: false` meant (`define` derives `is_public: visibility == Visibility.Public`, so the recorded flag is byte-identical); 6 files gained the `Visibility` import. **The ENCODER is deliberately untouched** — mapping an unknown value to `Public` or emitting a default would convert a loud, correct failure into silent corruption of a serialized artifact; the fallthrough is behaving correctly, and `hir_codec.spl` is generated anyway. Files: `statements.spl` (12), `_Expressions/match_desugaring.spl` (5), `_Expressions/expression_core.spl` (2), `_Expressions/expression_components.spl` (2), `_Expressions/expression_support.spl`, `_Expressions/union_narrow_arms.spl`, `_Items/module_build.spl`, `hir_lowering/types.spl`. Record: `doc/08_tracking/bug/hir_codec_visibility_bool_in_define_slot_2026-08-24.md`. | §27 | Stage-2 binary `/mnt/data/worktrees/goal-main-1/build/bootstrap/goal-r3/stage2/x86_64-unknown-linux-gnu/simple`, 132,945,096 bytes, built 2026-08-24 02:50, executed from a copy and never edited in place; `SIMPLE_TIMEOUT_SECONDS=0`; every rc read directly into a variable, never through a pipe. **CAUSAL PROOF, one run of one binary, two `HirSymbol`s differing in exactly one field handed to the same `hc_enc_hir_symbol`:** `GOOD visibility=Visibility.Private -> encoded, bytes=39` / `BAD visibility=false -> encoding now (expect the codec abort)` / `error: hir codec: no \`Visibility\` arm for tag -1; regenerate src/compiler/20.hir/generated/hir_codec.spl` rc=1. A bool in that field reproduces the production error byte-for-byte; a `Visibility` encodes. (Scratch harness, untracked; needed `SIMPLE_ALLOW_UNRESOLVED_RUNTIME=1 SIMPLE_ALLOW_INTERNAL_STUBS=1` because its closure drags in 9 mmap/file runtime entries plus `bytes_to_u16_le`/`bytes_to_u32_le` absent from the C bootstrap runtime — none on the path it exercises.) **Reproduce spec / guard** `scripts/check/check-symbol-define-visibility-arg.shs` parses every `.define(` call in `src/**/*.spl` (multi-line calls joined) and fails on any 7-argument call whose 5th argument is a bool literal — hard zero-bar, not a ratchet; `--selftest` runs first and is fatal (4 fixtures: clean must pass, the incident shape must be flagged, the unrelated 2-arg `Env.define` must NOT be flagged, an empty tree must report 0 checked so the caller is forced to ERROR); 0 call sites examined is ERROR, never a pass. BEFORE, pre-fix tree materialised with `git archive HEAD`: `FAIL — 55 call site(s) checked, 25 with a bool in the visibility slot: …` rc=1. AFTER, whole `src/`: `PASS — 137 call site(s) checked, 0 with a bool in the visibility slot` rc=0. **Stage-3 build from the fixed tree, stated honestly:** `stage2 native-build src/app/cli/bootstrap_main.spl` reached all 692 surfaces and ran the entire HIR phase, then failed rc=1 with **68 `hir-fatal`s and `grep -c 'hir codec' = 0`** — every one is the documented Stage-3 class other lanes own (`unresolved name`/`unresolved type`, `ambiguous explicit callable dependency DiContainer/AopWeaver`, `field 'kind' is not visible from this module`), and the 7 naming files this change touched are all unresolved-name/type, none mentioning `Visibility` or an ambiguous import — so this change introduced no new lowering error but produced no Stage-3 binary, which is why the harness above stands in for a re-run of `compile --format=smf` on a compiler built from the fix. **Unmasked, not caused:** with the abort gone `compile --format=smf` reaches the SMF backend and the pre-existing `cranelift-direct` SEGV becomes visible — measured on the UNMODIFIED Stage-2 binary under `SIMPLE_HIR_CACHE=0` BEFORE this fix existed (rc=139, last lines `[cranelift-direct] start / target / module`), so it is a separate defect this change exposes rather than introduces. |
| 2026-08-24 | SimpleOS aarch64 real-firmware boot (lane K) | (this commit) | **SimpleOS now BOOTS under real firmware on aarch64, end to end, from a clean build.** The only thing blocking `scripts/check/check-simpleos-arm64-efi-real-firmware-boot.shs` (previously `ERROR — nothing was checked: ESP build failed … missing kernel ELF build/os/aarch64_limine/kernel.elf`) was that the kernel ELF could not be produced: the documented 2026-08-07 recipe now fails to link with `ld.lld: error: undefined symbol: rt_raw_i64_to_string` referenced from `os__kernel__boot__limine_boot_aarch64__limine_aarch64_boot_main`. The aarch64 freestanding runtime had `rt_raw_u64_to_string` but never the signed mirror. Added `rt_raw_i64_to_string` to `examples/09_embedded/simple_os/arch/aarch64/boot/freestanding_runtime.c` (sign handled explicitly — no libc/snprintf here; magnitude taken in unsigned space so INT64_MIN is exact). The other 19 names the strict precheck listed are per-module reference-table entries that `--gc-sections` drops; `_bss_start`/`_kernel_end` come from the linker script. Result: `native-build --backend cranelift --entry-closure --entry examples/09_embedded/simple_os/arch/aarch64/limine_entry.spl --target aarch64-unknown-none-elf --linker-script .../linker_limine.ld` -> rc=0, 124 KB ELF, entry `0xffffffff80103b0c`, real `.text` (0x86f0 bytes), **0 weak stubs** (`SIMPLE_ALLOW_FREESTANDING_STUBS` deliberately NOT set). `check-simpleos-arm64-efi-real-firmware-boot.shs` -> `PASS — 4 boot-stage marker(s) checked … via BOOTAA64.EFI on a FAT ESP (no -kernel, no isa-debug-exit), 82 serial line(s) captured` rc=0. `check-simpleos-aarch64-limine-framebuffer.shs`, previously `ERROR … missing AAVMF_CODE.fd`, -> `PASS — … obtained a framebuffer: addr=… 800x600 bpp=32 pitch=3200; 3 refusal paths checked and none fired` rc=0. **Evidence is not marker-shaped**: the guest prints 46 distinct firmware-supplied memory-map region tuples, then a PMM round trip (`total_pages=393216 free_pages=120564`, `allocated pfn=262156 free_pages_after=120563`, `freed pfn=262156 free_pages_after=120564`) — content no `return 1;` can manufacture. **Sabotage-verified** per `fabricated_guest_success_markers_riscv64_smf_2026-08-24.md`: no-op'ing the PL011 `DR` store at `freestanding_runtime.c:1390`, rebuilding and rebooting gives `FAIL — aarch64 kernel never printed '[BOOT] Memory map:'`; restoring gives PASS again. `check-no-fabricated-success-markers.shs` -> `PASS — 403 file(s) scanned, 0 fabricated-success site(s)`. Still open: every `0x{...}` in the boot log renders DECIMAL (`HHDM offset: 0x18446462598732840960` is `0xFFFF800000000000`) — recorded as `doc/08_tracking/bug/aarch64_limine_boot_hex_interpolation_prints_decimal_2026-08-24.md`, deliberately not fixed in the same pass. Also unchanged: the main arm64 desktop lane `check-simpleos-arm64-unified-live.shs` still uses `-kernel` and stays filed. |
