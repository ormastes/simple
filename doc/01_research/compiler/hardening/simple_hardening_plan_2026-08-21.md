...
| 2026-08-23 | in-development tag sweep (slice 3: `test/02_integration`, `test/integration`, `test/feature`) | (this commit) | swept `test/feature` per-file with `--no-cover-check`; **0 specs tagged** — every one of the 21 failures classified as a real defect, an environmental gap, or a contract violation, none as unfinished feature work | §20 (test sweep), §27 | `doc/09_report/in_development_sweep_integration_2026-08-23.md` — 117 run / 96 pass / **21 fail**, ~12 distinct root causes incl. one SIMD runtime bug (`rt_simd_*_f32xN: field x must be a float, got Float32(...)`) behind 6 specs, wrong nested-parenthesis arithmetic (`expected 8 to equal 6`), context-method dispatch returning 0 instead of 42, and `src/lib/gc_sync_mut/` existing against the no-GC-first invariant. `test/integration` proven a strict subset mirror of `test/02_integration` (592 common, 0 unique), so tags apply to twins together |
