...
- 2026-05-14 core-runtime smoke: building `runtime.c`, `runtime_native.c`, `runtime_thread.c`, `runtime_fork.c`, `runtime_process.c`, and `runtime_memtrack.c` into a temporary `libsimple_runtime.a`, then running the harness with `SIMPLE_RUNTIME_PATH=<temp>` and `SIMPLE_NATIVE_RUNTIME_BUNDLE=auto`, links and runs the pure Simple benchmark without the Rust-hosted runtime. Checksum parity passes for XXHash64, CRC32, Adler-32, and ChaCha20. One-sample speed remains below the C/Rust target: XXHash64 `0.59x` C / `0.80x` Rust, CRC32 `0.92x` C / `0.90x` Rust, Adler-32 `0.74x` C / `0.78x` Rust, ChaCha20 `0.60x` C / `0.55x` Rust.
- 2026-05-14 follow-up: non-empty `[u8]` literals now lower through packed byte arrays (`rt_byte_array_new` plus `rt_typed_bytes_u8_push`) instead of generic boxed `ArrayLit`, closing the native literal-packing blocker recorded for direct byte table materialization. CRC32 and CRC32C now use lazy pure Simple table caches guarded by scalar bool globals so table generation is paid once per process in interpreter mode. `simple check src/os/crypto/crc32.spl` and the interpreter CRC KATs pass. Native benchmark validation is blocked in the current dirty tree before the CRC probe runs: the full port benchmark fails native compile with `undefined symbol: simd_text_init`, and a clean native CRC spec build fails in unrelated std/core collection inference. Do not count the CRC cache as native perf-complete until those native build blockers are cleared and the port benchmark reruns.
