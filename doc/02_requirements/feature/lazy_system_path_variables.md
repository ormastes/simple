...
- **REQ-LSPV-005:** Logical output uses UTF-8 `/` on Windows and Unix. Windows drive roots use `C:/...`; MinGW `/c/...` is not canonical.
...
- **REQ-LSPV-010:** Canonical values remain `/`-separated, but file access and executable launch convert at the final host boundary. Windows accepts `C:/x`, `/c/x`, and `c/x`, and passes `C:\\x` to the OS.
