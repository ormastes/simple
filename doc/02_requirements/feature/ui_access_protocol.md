...
- REQ-UAP-013: The MCP OS tool surface shall register `ui_access_snapshot`,
  ...
  `ui_access_query`, `ui_access_ensure`, `ui_access_value`,
  `ui_access_adapter_snapshot`, and `ui_access_visual_probe`.
...
### F7: Typed value access

- REQ-UAP-020: The system shall expose an additive `ui_access_value` read/write
  layer for canonical nodes whose kinds are `input`, `textfield`, or
  `textarea`.
- REQ-UAP-021: The `ui_access_value` layer shall reject non-value-bearing
  canonical nodes and shall not change the behavior of snapshot, query, state,
  or ensure flows.
- REQ-UAP-022: The system shall expose an additive
  `ui_access_adapter_snapshot` read layer that wraps the canonical snapshot in
  source and target metadata without changing the base snapshot contract.
- REQ-UAP-023: The system shall expose an additive `ui_access_visual_probe`
  sidecar that returns structured semantic marks and issues for a selected
  surface even when image-backed pixel capture is not configured.
...
- AC-UAP-03: `/api/test/ui/snapshot`, `/api/test/ui/surface`,
  ...
  `/api/test/ui/state`, `/api/test/ui/query`, `/api/test/ui/ensure`,
  `/api/test/ui/adapter_snapshot`, and `/api/test/ui/visual_probe`
  respond with stable additive behavior.
- AC-UAP-04: The MCP tool registry lists the twelve UI access tools with
  matching schemas, and the declarative observe/state/query/ensure/value tools
...
- AC-UAP-08: `ui_access_value` can read and write typed values for
  `input`/`textfield`/`textarea` canonical nodes, and rejects buttons and other
  non-value-bearing nodes.
- AC-UAP-09: `ui_access_adapter_snapshot` returns a source/target envelope for
  a surface and `ui_access_visual_probe` returns structured marks/issues rather
  than pretending to perform pixel capture.
