...
SPipe must provide bounded generated read-only files under `.spipe/view/` for file-only agents. It should provide a read-only editor virtual-filesystem adapter when client demand justifies it. Direct writes to either view must fail closed and approved canonical changes must route through refactor operations. Any supported operation that creates, replaces, removes, or cleans materialized filesystem entries must execute only through an admitted `SafeFilesystem` adapter/helper that provides descriptor-relative containment, no-follow traversal, and post-open identity validation for that platform. If the adapter cannot prove those capabilities, materializer mutation capability must be reported unavailable and no mutation may be attempted.
...
One canonical skill/rule source must deterministically generate supported Claude, Codex, Gemini, and agent-facing surfaces. Only explicitly approved policy-scope sources may generate executable agent instructions; ordinary indexed artifacts and provider output remain untrusted data and must be quoted/delimited so they cannot become ambient prompts. Generated artifacts must identify their source UID, trust scope, generator version, and content hash; verification must reject stale, unauthorized-source, secret-bearing, or hand-diverged generated surfaces.
