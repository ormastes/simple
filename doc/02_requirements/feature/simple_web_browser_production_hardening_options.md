...
| `REQ-WEB-HARD-007` | Live `/ui/ws` upgrades require allowed origin plus valid origin-bound bearer before HTTP 101, while legacy `/ws` and arbitrary WebSocket upgrade paths are hidden before upgrade. | B |
...
| `REQ-WEB-HARD-012` | Query-string bearer compatibility is non-authorizing in production, including when the deprecated `SIMPLE_UI_WEB_ALLOW_QUERY_TOKEN=1` environment flag is present. | C |
...
for `/ui/login`, canonical `/ui/ws`, hidden legacy `/ws`, `/ui/resume`,
sensitive `/api/*`, malformed tokens, wrong origins, and oversized
unauthenticated bodies.
...
query-token fallback retirement with deprecated env non-authorization, and
passing production GUI/Web renderer parity evidence.
