...
- WINCC-004: The canonical lint registry contains an executable deny rule for Windows compiler selections, with focused SSpec coverage. Acceptance requires an official LLVM 23.1.x distribution marker, a root-bound C driver, and a fail-closed exact version-family predicate; a bare `--version` token is insufficient. A missing admitted self-hosted runner is reported as pending, never replaced with Rust-seed evidence.
