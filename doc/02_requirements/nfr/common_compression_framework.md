...
- NFR-011 Verification Gate (current state, updated 2026-05-04): the
  ...
  `test/01_unit/lib/common/lz4_spec.spl`,
  `test/01_unit/lib/common/lzma2_range_coded_spec.spl`,
  `test/01_unit/lib/common/xz_lzma2_spec.spl`, and
  `test/01_unit/lib/common/zstd_fse_huffman_weight_encode_spec.spl`.
  This is useful evidence, but it is not full framework closure. Residual
  broader gates remain explicitly open:
  - (a) fresh re-verification of `test/01_unit/lib/common/compress_framework_spec.spl`
  ...
  - (c) explicit re-verification, merge, or retirement of overlapping older
    Zstd weight coverage such as `test/01_unit/lib/common/zstd_fse_weights_spec.spl`.
