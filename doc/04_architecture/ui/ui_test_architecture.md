...
*render/Draw IR stack*). This doc is about the *test access* seam between them.

## Two test layers (and the gap)

There are two distinct ways UI state is read for assertions today. They are not
competitors — they are different transports for the same semantic truth — but
they are **not yet unified behind one interface**.
...
(1) stands up a full HTTP server, or (2) hand-builds an SGTTI snapshot and walks
nodes manually (see `test/01_unit/os/compositor/gtti_spec.spl`). TUI tests take a
third, separate route entirely (widget/state model, below). This doc's target
...
Key property: `UiAccessNode` **already carries every assertable field** the
shared UI contract allows (id = `canonical_id`, text = `text_value`, plus
...
renderer. SGTTI gives them an in-process semantic tree **without a display
backend or HTTP server** via the compositor source in **hidden WM mode**.
...
1. **Semantic tier (SGTTI).** Assert the *structure and state* of the surface:
   the window exists, is focused, has the expected title/kind/children, and
   geometry props. Fast, headless, deterministic, no pixels.
2. **Pixel tier (bitmap evidence).** Assert the *rendered pixels* via the
   existing bitmap gates (`scripts/check-*-bitmap-evidence.shs`, Engine2D
   `draw_ir_adv.spl` CPU-fallback readback). This stays the pixel oracle; SGTTI
   does not replace it.

For 2D specifically: the Draw IR executor (`engine2d/draw_ir_adv.spl`) already
exposes pixel readback; SGTTI adds the *semantic* assertion that the Draw IR
batch produced the expected node tree before raster. See the Draw IR inspection
extension (`draw_io_sdn_draw_ir.md`), which is **built on SGTTI** — its gated
`/api/test/draw-ir` endpoint reads the same snapshot substrate.

**Honest cost note.** `gtti_snapshot_from_compositor` takes a real `Compositor`.
A GUI test must build a headless compositor fixture (backend + cursor +
`Compositor`), as `gtti_spec.spl` does today. This is not free; the improvement
...
`common.ui.state.{update_state}`) — see `test/01_unit/app/ui/async_tui_spec.spl`.
Per `shared_ui_contract.md`, TUI is an **S1 target**: it is not yet on the
semantic/SGTTI path. So tui/gui interface parity is a **target state, not
current reality.**
...
Design constraints (carried from `shared_ui_contract.md` and the no-fork rule):
...
- Reuse the pub `win_text_find_nodes` / `win_text_route_action` core; the driver
  is a thin façade, not a reimplementation.
- Keep this **additive**: `UITestClient` (HTTP) is unchanged; the SGTTI driver
  is a sibling. Surfaces that are S4 keep their HTTP tests.
...
- **`simple_gui_stack.md` (render stack).** SGTTI sits beside the render path:
  ...
  tier). The Draw IR (`draw_ir.spl`) is the render IR; SGTTI is the *test
  access* over the surfaces that consume it.
