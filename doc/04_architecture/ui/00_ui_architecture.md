...
| `... cpu simd (x86, riscv, arm/aarch64)` | Partial — real NEON + SSE2 row kernels exist for arm64/x86_64; RISC-V (RVV) and AVX2/AVX512 are not implemented; the Simple-level SIMD probe (`backend_probe.spl`, `probe_backend()`/`detect_simd_level()`) gates `cpu_simd_arm`/`cpu_simd_x86`/`cpu_simd_riscv` on runtime-detected NEON/SSE4.2/AVX2/RVV instead of hardcoding an arch (the old orphaned `backend_cpu_simd_opt.spl` simulator was deleted) | `src/compiler_rust/runtime/src/value/engine2d_simd_ops.rs` (real NEON/SSE2); `src/lib/gc_async_mut/gpu/engine2d/backend_probe.spl` (runtime SIMD probe) |
