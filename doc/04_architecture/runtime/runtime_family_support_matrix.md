...
- `nogc_async_mut` may expose facade modules over `nogc_sync_mut` for
  ...
  runtime-hook owners; `scripts/audit/runtime_backend_boundaries.spl` keeps the
...
|--------|---------------------------|-------------------|----------------------|---------------------|----------------|-------------|
| `common` | None (inherits project default) | Yes (auto-resolved) | Yes (neutral root manifest; pure shared helpers) | Yes (5th priority) | Partial | Neutral shared family; no `@gc`/`@no_gc` attribute; direct-import boundary checks present |
| `nogc_sync_mut` | `@no_gc` on root `__init__.spl`; manifest-backed semantic warnings and target-strict hard errors | Yes (4th priority in search, boundary warnings) | Yes (rich exports) | Yes (4th priority) | Good | Root attribute, direct-import boundary checks, and target-strict runtime-family enforcement present |
| `nogc_async_mut` | `@no_gc` on root `__init__.spl`; manifest-backed semantic warnings and target-strict hard errors | Yes (1st priority in search, boundary warnings) | Yes (rich exports) | Yes (1st priority, default) | Good | Root attribute, direct-import boundary checks, and target-strict runtime-family enforcement present |
| `gc_async_mut` | `@gc` on root `__init__.spl`; family-prefix/runtime checks present | Yes (6th priority in search) | Yes (GC family root manifest; concrete submodules own exports) | Yes (6th priority) | GPU-specific plus facade smoke | Direct-import boundary checks and target-strict runtime-family enforcement present |
| `nogc_async_immut` | `@no_gc` on root `__init__.spl`; manifest-backed semantic warnings and target-strict hard errors | Yes (2nd priority in search, boundary warnings) | Yes (persistent structures, Atom/Ref, combinators) | Yes (2nd priority) | Direct native | Resolution and root exports fixed; persistent-structure facade coverage is broad and direct coordination/CAS native coverage passes |
| `nogc_async_mut_noalloc` | `@no_gc` on root `__init__.spl`; direct and reachable unsafe imports plus explicit `@alloc` markers blocked by target-strict checker and regression audits | Yes (10th priority in search, boundary warnings) | Yes (baremetal/noalloc exports) | Yes (10th priority) | Partial (QEMU); check-clean under full-family `simple check` | Root export surface exists; resolver manifest metadata marks the family as `AllocationCapability.NoAlloc` |
| `gc_sync_mut` | `@gc` on root `__init__.spl`; facade-only modules mirror `gc_async_mut` | Recognized by interpreter family extraction (8th priority) | Yes (root facade over `gc_async_mut`) | Yes (8th priority) | Focused facade | Facade over `gc_async_mut`; real blocking wrappers are added only when API-specific behavior tests prove facade export is insufficient |
...
- The Rust interpreter module loader uses the same family set, ranks, and reason strings as Rust `simple check`. `SIMPLE_STRICT_RUNTIME_FAMILY=1` still works as an explicit override, and `simple test ... --target <baremetal/simpleos>` plus direct `simple file.spl --target <baremetal/simpleos>` now set the same strict mode automatically around interpreter execution.
- `test/01_unit/compiler/interpreter/gc_parity_spec.spl` covers family extraction and warning rules.
...
- `src/app/simple_lsp_mcp/main.spl` native-build now completes with the rebuilt Rust bootstrap compiler:
  ...
  - GC async runtime-owner wildcard update: GC async runtime-hook-owner facades now route through no-GC async facades where matching no-GC async surfaces exist. The remaining GPU 2D/3D and device I/O FFI API-shape gaps now have no-GC async facades over explicit no-GC sync backend export lists, and the GC async facades export those no-GC async surfaces instead of wildcard-exporting no-GC sync runtime-hook owners directly. `scripts/audit/runtime_backend_boundaries.spl` now reports `gc_async_runtime_owner_wildcard_facade_count=0`.
  ...
  - simple-core renderer ABI update: `core_values.spl`, `core_array_ops.spl`, and
    `core_string.spl` now provide `rt_value_as_int`, the single-thread bootstrap
    `rt_pool_safepoint`, `rt_for_iterable`, `rt_string_bytes`,
    `rt_string_char_code_at`, and `rt_any_add`. A focused C probe linked against
    the generated pure-Simple archive and passed signed untagging, ASCII/UTF-8
    character lookup, byte-array extraction, iterable identity, and dynamic
    integer addition. Automatic `simple-core` selection remains blocked by the
    pre-existing missing `rt_string_builder_*` required-symbol set.
  - native warning closure update: HIR import loading now treats empty grouped/glob root imports as non-addressable facade roots instead of resolving an empty module path. The core C runtime now exports handle-based `rt_atomic_int_*` and `rt_atomic_bool_*` atomics plus minimal enum/BDD helper ABI used by focused native BDD smoke. A rebuilt `src/compiler_rust/target/debug/simple` native run of `test/01_unit/lib/{gc_async_immut,gc_sync_immut,nogc_sync_immut}/trie_root_facade_native_spec.spl` passed all three focused specs with no empty-module type-loader warnings, no internal Simple symbol warnings, and no generated runtime stubs.
...
- **Maintenance rule**: Keep new compiler entrypoints on one of the covered paths: Rust check/test/file/native-build strict wrappers for target-restricted execution, Rust/native_all native-build strict wrappers, or Simple-side target-preset resolution through `CompileOptions.allowed_families`. Any future entrypoint that performs fresh module loading under a restricted target outside those paths must wire strict family enforcement before promotion.

### Gap 3: `nogc_async_immut` runtime coverage (Agent 3/4) -- **RESOLVED**
- **Status**: Fixed. Persistent-structure facade coverage is broad, and `test/01_unit/lib/nogc_async_immut/coordination_native_spec.spl` now covers direct no-GC async `Atom`, `Ref`, `VersionedSnapshot`, and exported CAS helper behavior in interpreter and native modes.
- **Maintenance rule**: Keep this spec in native smoke and preserve the manifest-backed strict-family gates for public promotion.
...
- **Maintenance rule**: Direct imports from allocating runtime families, direct `app.*` imports, unsafe reachable helper imports, explicit noalloc `@alloc` markers, and host allocation API calls are blocked by `dependency_boundary_spec`, target-strict `simple check`, and the baremetal verifier. Resolver manifests expose allocation capability metadata; future restricted-target module-loading entrypoints should consume it instead of bypassing the strict check path.

### Gate 5: Compiler-owned allocation capability metadata (Agent 2 -- Compiler)
- **Status**: Fixed for current restricted-target gates. `src/compiler/99.loader/module_resolver/types.spl` defines `AllocationCapability` and stores it on `DirectoryManifest` and `ChildModule`; `manifest.spl` infers noalloc runtime-family roots and honors explicit `@no_alloc` / `@alloc` attributes. `test/01_unit/compiler/module_resolver/allocation_capability_manifest_spec.spl` locks the source path.
- **Impact**: Module resolution has a first-class allocation-capability metadata surface. Target-strict Rust checks and audits still perform the hard reachable-closure enforcement.
- **Maintenance rule**: Any future restricted-target module-loading path that bypasses the existing strict checker/audit gates must consume manifest allocation metadata directly.
...
- **Guard**: `test/01_unit/compiler/semantics/gc_strategy_naming_spec.spl` prevents a second compiler `enum GcMode:` from being introduced.
...
- **Resolution**: `src/compiler/70.backend/target_presets.spl` carries runtime-family restrictions, `src/compiler/00.common/driver_core_types.spl` stores them on `CompileOptions`, and `src/compiler/99.loader/module_resolver/resolution.spl` filters `lib/*/` search when restrictions are non-empty. Covered by `test/01_unit/compiler/driver/compile_options_normalization_spec.spl`, `test/01_unit/compiler/target_presets_spec.spl`, and `test/01_unit/compiler/module_resolver/allowed_families_spec.spl`.
...
- **Maintenance rule**: Keep target-aware interpreter launch tests in smoke and apply the same policy to any future entrypoints that load modules under restricted targets.
...
- `SIMPLE_STRICT_RUNTIME_FAMILY=1`, `simple test ... --target <baremetal/simpleos>`, and direct `simple file.spl --target <baremetal/simpleos>` promote those interpreter diagnostics to errors for target-restricted gates.
...
| `test/01_unit/compiler/target_presets_spec.spl` | Added family mapping tests |
| `test/01_unit/compiler/driver/compile_options_normalization_spec.spl` | Verifies presets apply `gc_off` and `allowed_families` to `CompileOptions` |
| `test/01_unit/compiler/module_resolver/allowed_families_spec.spl` | Verifies resolver source keeps the `allowed_families` filter path |
...
|-----------|-------------|------|
| `test/01_unit/compiler/common/gc_config_spec.spl` | GcMode/GcConfig types | Unit |
| `test/01_unit/compiler/semantics/gc_safety_spec.spl` | GC safety analysis | Unit |
| `test/03_system/feature/usage/gc_managed_default_spec.spl` | GC managed default behavior | Feature |
| `test/03_system/feature/lib/gc_parity/gc_module_loader_spec.spl` | GC module loader parity | Feature |
| `test/03_system/feature/lib/gc_parity/nogc_sync_mut_contract_spec.spl` | nogc_sync_mut sync GC-adjacent contracts | Feature |
| `test/03_system/features/gc_system_spec.spl` | GC system-level | System |
| `test/lib/std/unit/gc_spec.spl` | GC stdlib unit | Unit |
| `test/01_unit/lib/gc_async_mut/gpu_runtime/gc_borrowed_view_spec.spl` | GPU borrowed views | Unit |
| `test/01_unit/lib/gc_async_mut/cuda/gc_cuda_ownership_spec.spl` | CUDA ownership | Unit |
| `test/01_unit/lib/gc_async_mut/torch/gc_*_spec.spl` (3 files) | Torch GC integration | Unit |
| `test/01_unit/lib/nogc_sync_mut/gc_runtime_contract_spec.spl` | NoGC sync GC metadata and pointer helper contracts | Unit |
| `test/01_unit/compiler/target_presets_spec.spl` | Target presets | Unit |
| `test/03_system/baremetal_test_runner_spec.spl` | Baremetal test runner | System |
