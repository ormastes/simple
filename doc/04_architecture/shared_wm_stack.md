...
6. **Add tests** — create `test/01_unit/os/compositor/<name>_backend_spec.spl`. At minimum: render a solid rect and verify `read_pixel` returns the expected `Argb32`; confirm input polling returns `nil` when the queue is empty. Add the new variant to `wm_compare` to get cross-backend pixel parity checked automatically.
