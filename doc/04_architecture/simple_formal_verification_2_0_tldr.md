...
Formal Verification 2.0 proposes a fail-closed evidence chain from canonical Simple
...
artifact-identity edges. It is normative proposed architecture, not a claim
that its V2 types, collectors, reducers, runners, or scripts exist.

## Core Shape

- Ten versioned V1 display names map to exact planned Simple identifiers; all
  ten are absent on current main.
...
- The only implemented foundation is the MIR `DecisionProbe`/`ConditionProbe`
  bridge. It retains and rejects unlowered evidence but does not verify it.
