...
- `src/lib/nogc_sync_mut/fs_driver/mount_table.spl`: path mount resolution via
  indexed char-copy.
- `examples/11_advanced/nvfs/src/core/compression.spl`: class-aware compression using a
  small RLE block frame and raw fallback for incompressible input.
- `examples/11_advanced/nvfs/src/tool/nvfs_upgrade.spl`: record-level offline pmap upgrade
  helper.
- `examples/11_advanced/nvfs/src/core/dedup.spl`: in-memory hot DDT cache, DEDUP tree
