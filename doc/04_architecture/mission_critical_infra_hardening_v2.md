...
| REQ-MCI-009 | `nogc_sync_mut.mission_critical.bounded_process_policy`: generation-bound slot reservations; opaque owner leases binding run/execution/start identity, PID, PGID, and slot token; sequenced termination/reap states; checked incremental capture receipts. Unit controls reject stale races, replay, PID reuse, forged groups, last-slot competition, and chunk overflow. **BLOCKED integration:** actual atomic synchronization and spawn/signal/wait syscalls remain canonical process-facade work and are not claimed by this pure policy layer. |
