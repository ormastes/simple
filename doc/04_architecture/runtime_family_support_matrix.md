...
- [x] Fix committed-path native `PersistentMap`/`PersistentSet` facade probe blocker with root `PersistentMap` set/get and `PersistentSet` add/contains one-entry, two-entry, and branch-shaped insert smoke across GC async, GC sync immutable, and no-GC sync immutable facades
- [ ] Resolve native BDD blockers for map branch overwrite/removal, set dedupe/removal, and untyped chained root-facade trie mutation, then clear remaining native type-loader/minimal-runtime-stub warnings
