...
- `src/app/simple_lsp_mcp/main.spl` native-build now completes with the rebuilt Rust bootstrap compiler:
  ...
  - I/O facade closure update: no-GC async `io/debug_state`, `io/debug_stubs`, `io/file_discovery`, `io/file_shell`, and `io/signal_handlers` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=120`.
  - Test-runner facade closure update: no-GC async `test_runner/resource_governor`, `test_runner/test_classification`, `test_runner/test_runner_args`, `test_runner/test_runner_async`, and `test_runner/test_runner_files` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=115`.
  - Security facade closure update: no-GC async `security/audit_chain`, `security/audit_log`, and `security/types` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=112`.
  - LSP facade closure update: no-GC async `lsp/lsp_query` and `lsp/parser_adapter` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=110`.
  - Experiment facade closure update: no-GC async `src/exp/artifact`, `src/exp/config`, and `src/exp/sweep` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=107`.
  - OIDC facade closure update: no-GC async `oidc/discovery` and `oidc/id_token` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=105`.
  - Spec facade closure update: no-GC async `spec/decorators` and `spec/env_detect` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=103`.
  - Hooks facade closure update: no-GC async `hooks/mod` and `hooks/stop` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=101`.
  - FFI dynamic facade closure update: no-GC async `ffi/dynamic` and `ffi/dynamic_versioned` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=99`.
  - Benchmark/GPU-runtime facade closure update: no-GC async `benchmark/string_bench` and `gpu_runtime/mod` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=97`.
  - Platform/play facade closure update: no-GC async `platform/linker` and `play/expect` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=95`.
  - Debug/shader facade closure update: no-GC async `debug/remote/protocol/stlink_tools`, `debug/remote/protocol/t32_window_capture`, `debug/remote/test/qemu_runner`, and `engine/render/shader_compile` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=91`.
  - Terminal/time/daemon facade closure update: no-GC async `terminal/credential`, `daemon_sdk/types`, `io/serial_proxy`, and `io/time_ops` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=87`.
  - SSH/JS/coverage/fuzz facade closure update: no-GC async `io/ssh_serial`, `js/engine/interpreter`, `coverage`, and `fuzz` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=83`.
  - Daemon/debug/coverage facade closure update: no-GC async `daemon_sdk/daemon`, `debug/formats/dsym_resolver`, `debug/remote/protocol/openocd`, and `ffi/coverage` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=79`.
  - I/O profiler/sysinfo/thread facade closure update: no-GC async `io/profiler_simple`, `io/sysinfo_ops`, and `io/thread` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=76`.
  - LSP/MCDC facade closure update: no-GC async `lsp/handlers/diagnostics`, `lsp/main_wasi`, and `mcdc` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=73`.
  - Platform root facade closure update: no-GC async `platform` and `platform/__init__` now use explicit public export lists over the no-GC sync platform owner, reducing the no-GC async wildcard scan to `remaining=71`.
  - Play runtime facade closure update: no-GC async `play/electron_application`, `play/page`, `play/sdl2_backend`, and `play/wm/mod` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=67`.
  - Shell/test-runner facade closure update: no-GC async `shell/env`, `test_runner/system_monitor`, and `test_runner/test_runner_coverage` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=64`.
  - Daemon/I/O runtime-hook facade closure update: no-GC async `daemon_sdk/client`, `io/dh_ffi`, `io/dir_ops`, and `io/signal_stubs` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=60`.
  - Log/LSP/Play/test-manifest facade closure update: no-GC async `log`, `lsp/lsp_handlers`, `play/xvfb`, and `test_runner/test_manifest_scanner` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=56`.
  - Env/VHDL/LSP/package facade closure update: no-GC async `io/env_ops`, `io/vhdl_ffi`, `lsp/main`, and `package/dist` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=52`.
  - QMP/src/test-manifest facade closure update: no-GC async `qemu/qmp_client`, `src/config`, `src/db`, and `test_runner/test_manifest` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=48`.
  - Image/play/storage/fork facade closure update: no-GC async `io/image_ffi`, `play/trace`, `src/exp/storage`, and `test_runner/test_runner_fork` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=44`.
  - Serial/SIMD/daemon/debug facade closure update: no-GC async `io/serial_ffi`, `simd/profile`, `daemon_sdk/lock`, and `debug/smf_agent` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=40`.
  - Env/coverage/FFI facade closure update: no-GC async `env/types`, `io/coverage_simple`, `ffi/error`, and `ffi/llvm_loader` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=36`.
  - Font/protocol/launcher facade closure update: no-GC async `io/font_ffi`, `daemon_sdk/protocol`, and `play/launcher` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=33`; `io/pipe` was deferred until the final facade closure pass.
  - Process/math/package facade closure update: no-GC async `io/process_ops`, `io/math`, and `package_ffi` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=30`.
  - Debug backend facade closure update: no-GC async `debug/native_agent` and `debug/interpreter_backend` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=28`.
  - Regex/signature/runtime facade closure update: no-GC async `io/regex_ffi`, `io/signature_ffi`, and `io_runtime` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=25`.
  - Crypto facade closure update: no-GC async `io/crypto_ffi` now uses an explicit public export list over its no-GC sync owner, reducing the no-GC async wildcard scan to `remaining=24`.
  - Render/package/GPU-driver facade closure update: no-GC async `engine/render/vulkan_backend3d`, `engine/render/webgpu_backend3d`, `ffi/package`, and `gpu_driver/mod` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=20`.
  - I/O device/archive facade closure update: no-GC async `io/gamepad_ffi`, `io/compress_ffi`, `io/ftp_ffi`, and `io/audio_ffi` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=16`.
  - CUDA/AST/SSH facade closure update: no-GC async `cuda/ffi`, `ffi/ast`, and `io/ssh_ffi` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=13`.
  - Runtime/I/O/TLS/HTTP facade closure update: no-GC async `ffi/runtime`, `ffi/io`, `io/tls_ffi`, and `io/http_ffi` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=9`.
  - CLI/system/debug facade closure update: no-GC async `ffi/cli`, `ffi/system`, and `ffi/debug` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=6`.
  - Graphics/window/codegen/Torch facade closure update: no-GC async `io/graphics2d_ffi`, `io/rapier2d_ffi`, `io/window_ffi`, `ffi/codegen`, and `torch/ffi` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=1` before the final facade closure pass.
  - Final runtime-owner wildcard facade closure update: no-GC async `io/pipe`, `conf`, `debug/remote/dwarf`, `debug/remote/ptrace`, `io/process_limit_enforcer`, `lsp/lsp_protocol`, `mem_tracker/mod`, and test-runner process/lifecycle/parsing facades now use explicit public export lists over their no-GC sync owners; `ffi/mod` no longer wildcard-exports the documentation-only sync root. The runtime-hook owner wildcard scan now reports `remaining=0`.
