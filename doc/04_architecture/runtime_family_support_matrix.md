...
- `src/app/simple_lsp_mcp/main.spl` native-build now completes with the rebuilt Rust bootstrap compiler:
  ...
  - Runtime facade execution update: GC async TLS, TCP, WebSocket, web-framework, hooks/detectors, HTTP/3, MCP SDK server/transport, play/CDP, sanitizer, security, storage, replay, and `src` compatibility facade modules now route to `nogc_async_mut` backends instead of self-re-exporting. The no-GC sync MCP SDK JSON owner now provides a local `escape_json` helper so imported symbols are not re-exported through facade chains; the no-GC sync replay checkpoint owner now provides local printable-ASCII decode support so replay facade chains do not depend on an imported text helper at runtime. No-GC TLS hex helpers avoid interpreter-only slice lowering, no-GC sync CUDA FFI normalizes runtime status codes into the public boolean/handle contract, and the GC async 2D WebGPU fallback text path no longer depends on optional dynamic font-renderer construction.
