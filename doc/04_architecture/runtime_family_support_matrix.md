...
- `src/app/simple_lsp_mcp/main.spl` native-build now completes with the rebuilt Rust bootstrap compiler:
  ...
  - facade closure update: Redis clients, `io/file_ops`, GC async `io/file`, no-GC async TLS setup, WebSocket close, and HTTP worker socket setup now route through no-GC backend modules. `gc_async_mut` and `nogc_async_mut` utility modules for `io_runtime`, `log`, `mcdc`, `process_ops`, `env_ops`, `time_ops`, `dir_ops`, `debug_state`, `file_discovery`, `file_shell`, `pipe`, `process_limit_enforcer`, `sysinfo_ops`, `thread`, `hooks/*`, selected `lsp/*`, `mem_tracker`, `qemu/qmp_client`, package distribution, platform linker, security audit helpers, SIMD profile helpers, spec decorators, experiment storage/config/artifact/sweep helpers, and simple I/O helpers (`coverage_simple`, `math`, `profiler_simple`, `signal_stubs`, `volatile_ops`) are compatibility facades over `nogc_sync_mut`; no-GC async `debug_stubs`, `ssh_serial`, `cache/*`, `daemon_sdk/*`, `database/atomic`, `database/requirement`, `benchmark/string_bench`, `shell/env`, and `spec/env_detect` also route to the no-GC sync backend. A source scan over `src/lib/gc_async_mut` and `src/lib/nogc_async_mut` finds no `rt_io_tcp_*`, `rt_async_tcp_*`, `rt_io_udp_*`, `rt_async_udp_*`, `rt_socket_set_nonblocking`, `rt_socket_set_reuse*`, `rt_epoll_*`, `rt_driver_*`, `rt_file_fsync*`, or `rt_file_stat` references; the raw socket hooks are concentrated in `nogc_sync_mut.io.tcp`.
  ...
  - SimpleOS POSIX boundary update: shared ABI errno values are now owned by `src/os/kernel/errno.spl`, and `src/os/posix/errno.spl` is a POSIX compatibility facade. A targeted scan finds no `os.posix.errno` imports under `src/os/kernel`, `src/os/services`, or `src/os/sosix`; those lower layers now depend on `os.kernel.errno` instead.
