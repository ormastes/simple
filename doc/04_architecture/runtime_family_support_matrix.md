...
- `src/app/simple_lsp_mcp/main.spl` native-build now completes with the rebuilt Rust bootstrap compiler:
  ...
  - SimpleOS POSIX boundary update: shared ABI errno values, FD table/I/O, async read/write adapters, pipe/socket compatibility, and process async/compatibility wrappers are now owned under `src/os/kernel/`. The matching `src/os/posix/` modules are POSIX compatibility facades. A targeted scan finds no `os.posix` imports under `src/os/kernel`, `src/os/services`, or `src/os/sosix`; those lower layers now depend on `os.kernel` ownership instead.
