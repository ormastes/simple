...
- **public**: Directory exists, is referenced in module loader search path, has test coverage, and either has an explicit root `__init__.spl` export surface or a documented root-manifest gap in Section 3.
- **advanced-scoped**: Directory exists with code and, where present, a root export surface, but still has limited test coverage or incomplete runtime/compiler enforcement for public-family guarantees.
...
- `gc_async_mut` facades prefer matching `nogc_async_mut` surfaces first. A
  ...
  file-read, TCP/UDP socket facade exports, mimalloc TLS thread-local hooks,
  CUDA GPU FFI, shared GPU math/presentation hooks, and 2D GPU backend FFI/hook
  facades have been moved to the no-GC async surface first. A source scan now
  finds no GC async import that directly targets a no-GC sync module when a
  matching no-GC async surface exists.
...
- **Alloc checking**: target family filtering, `dependency_boundary_spec`, and target-strict `simple check` prevent direct and reachable noalloc imports from allocating runtime families, explicit noalloc `@alloc` markers, host allocation APIs, and unsafe helper-module imports. `DirectoryManifest` and `ChildModule` now carry `AllocationCapability` metadata inferred for runtime-family roots and explicit `@no_alloc` / `@alloc` attributes, so module resolution has a compiler-owned capability surface while the reachable-import audit remains regression evidence.
...
**Current enforcement status**: the manifest-backed hard violation API, Rust lint parity, Rust `simple check` target strict mode, target-strict noalloc reachable-import closure check, and Rust interpreter strict mode exist. `simple check --target <baremetal/simpleos>` promotes runtime-family violations automatically, and the Rust test runner interpreted/native-cache paths, direct file interpreter launch path, and Rust/native_all native-build paths now enable strict runtime-family imports for baremetal/SimpleOS targets while hosted targets keep warnings by default. The Simple-side AOT output path compiles modules already admitted through `CompileOptions.allowed_families`, so its enforcement point is target-preset module resolution rather than an extra `SIMPLE_STRICT_RUNTIME_FAMILY` wrapper. Default unrestricted CLI/interpreter runs intentionally keep compatibility warning output.
...
### Gap 1: Target-restricted family hard errors are wired for current entrypoints (Agent 2 -- Compiler)
- **Status**: Fixed for current restricted-target entrypoints. Public runtime-family root `__init__.spl` files carry `@no_gc` or `@gc`, the Simple semantic checker exposes manifest-backed hard violations, and the Rust `simple check` lint path mirrors the manifest family set/ranks/reason strings.
- **Impact**: Target-restricted checker/CI, interpreter test, direct file, native-cache, Rust/native_all native-build, and Simple-side AOT target-preset paths can fail on runtime-family boundary violations; hosted checker runs and compiler pipelines that do not pass a restricted target remain warning-compatible.
- **Remaining work**: New entrypoints that load modules under a restricted target must wire into one of the covered strict-family paths before promotion.

### Gap 2: Interpreter strict family errors are target-aware in current launch paths (Agent 3 -- Interpreter)
- **Status**: Fixed for current restricted-target launch paths. The Rust interpreter can promote family-boundary diagnostics to errors with `SIMPLE_STRICT_RUNTIME_FAMILY=1`; `simple test ... --target <baremetal/simpleos>` interpreted execution and native-cache compilation, direct `simple file.spl --target <baremetal/simpleos>`, and Rust/native_all native-build paths now set that mode automatically for the duration of target-restricted execution. Hosted targets keep warning-compatible behavior.
...
- **Status**: Fixed for current restricted-target gates. `src/compiler/99.loader/module_resolver/types.spl` defines `AllocationCapability` and stores it on `DirectoryManifest` and `ChildModule`; `manifest.spl` infers noalloc runtime-family roots and honors explicit `@no_alloc` / `@alloc` attributes. `test/unit/compiler/module_resolver/allocation_capability_manifest_spec.spl` locks the source path.
...
- Compiler-owned reachable import closure enforcement now exists in target-strict `simple check`; resolver manifests now expose allocation capability metadata for runtime-family roots and explicit allocation attributes.
...
| `src/compiler/99.loader/module_resolver/manifest.spl` | L99 | Parses `@no_gc`/`@gc` from `__init__.spl` attributes, propagates to `ChildModule.gc_config`, and records `AllocationCapability` for family roots plus `@no_alloc` / `@alloc` attributes | **Partial**: runtime-family roots now use attributes and allocation metadata, Rust check/test/file/native-build paths can promote restricted-target violations, but full manifest-to-hard-error enforcement remains scoped to covered paths |
...
| `test/unit/lib/dependency_boundary_spec.spl` | Test | Blocks direct noalloc imports from allocating runtime families, direct `app.*` imports, noalloc `@alloc` markers, host allocation APIs, and unsafe reachable helper imports | Yes for direct imports/markers/API calls/reachable helper imports; `test/unit/compiler/module_resolver/allocation_capability_manifest_spec.spl` covers compiler-owned allocation metadata |
