...
- `src/app/simple_lsp_mcp/main.spl` native-build now completes with the rebuilt Rust bootstrap compiler:
  ...
  - driver source update: `std.io.driver.IoDriver` now defaults to a pure Simple no-GC fallback (`simple-fallback`) that supports timeout completion smoke behavior and reports kernel I/O as unsupported (`-38`) until a platform driver is selected. No `src/lib` source file imports `rt_driver_*` after this change, and the traced LSP build's internal Simple stub warning dropped from 85 to 61 symbols. Remaining `rt_driver_*` stubs are emitted by native linker/runtime metadata and must be cleaned up separately before package closure is considered driver-stub-free.
