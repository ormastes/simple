...
| `gc_async_mut` | `@gc` on root `__init__.spl`; family-prefix/runtime checks present | Yes (6th priority in search) | Yes (GC family root manifest; concrete submodules own exports) | Yes (6th priority) | Partial (GPU-specific) | Direct-import boundary checks present; full manifest-to-GcMode enforcement remains partial |
...
| `gc_sync_mut` | `@gc` on root `__init__.spl`; facade-only modules mirror `gc_async_mut` | Recognized by interpreter family extraction (8th priority) | Yes (root facade over `gc_async_mut`) | Yes (8th priority) | Minimal | Facade over `gc_async_mut`; independent sync runtime semantics remain a behavior-promotion gap |
...
- **Remaining work**: Move future restricted-target module-loading checks toward consuming manifest allocation metadata directly, and retain the reachable-import audit until every restricted entrypoint is covered.
...
- **Remaining work**: Keep target-aware interpreter launch tests in smoke and apply the same policy to any remaining entrypoints that load modules under restricted targets.
