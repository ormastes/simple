...
- `src/app/simple_lsp_mcp/main.spl` native-build now completes with the rebuilt Rust bootstrap compiler:
  ...
  - Runtime facade execution update: GC async TLS and WebSocket facade modules now route to `nogc_async_mut` backends instead of self-re-exporting, no-GC TLS hex helpers avoid interpreter-only slice lowering, no-GC sync CUDA FFI normalizes runtime status codes into the public boolean/handle contract, and the GC async 2D WebGPU fallback text path no longer depends on optional dynamic font-renderer construction.
