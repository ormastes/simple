...
- `src/app/simple_lsp_mcp/main.spl` native-build now completes with the rebuilt Rust bootstrap compiler:
  ...
  - Runtime facade execution update: GC async TLS, TCP, WebSocket, web-framework, hooks/detectors, HTTP/3, MCP SDK server/transport, play/CDP, and sanitizer facade modules now route to `nogc_async_mut` backends instead of self-re-exporting. The no-GC sync MCP SDK JSON owner now provides a local `escape_json` helper so imported symbols are not re-exported through facade chains. No-GC TLS hex helpers avoid interpreter-only slice lowering, no-GC sync CUDA FFI normalizes runtime status codes into the public boolean/handle contract, and the GC async 2D WebGPU fallback text path no longer depends on optional dynamic font-renderer construction.
