...
- `src/app/simple_lsp_mcp/main.spl` native-build now completes with the rebuilt Rust bootstrap compiler:
  ...
  - Font/protocol/launcher facade closure update: no-GC async `io/font_ffi`, `daemon_sdk/protocol`, and `play/launcher` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=33`; `io/pipe` remains a documented wildcard until the facade can explicitly export `out`/`err` without parser ambiguity.
  ...
  - Graphics/window/codegen/Torch facade closure update: no-GC async `io/graphics2d_ffi`, `io/rapier2d_ffi`, `io/window_ffi`, `ffi/codegen`, and `torch/ffi` now use explicit public export lists over their no-GC sync owners, reducing the no-GC async wildcard scan to `remaining=1`; the remaining `io/pipe` wildcard is blocked by parser ambiguity around explicit `out`/`err` exports.
