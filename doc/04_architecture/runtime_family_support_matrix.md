...
- `gc_async_mut` facades prefer matching `nogc_async_mut` surfaces first. A
  ...
  file-read, TCP/UDP socket facade exports, CUDA GPU FFI, shared GPU
  math/presentation hooks, and 2D GPU backend FFI/hook facades have been moved
  to the no-GC async surface first; the remaining direct sync import is the
  blocking runtime TLS boundary in the mimalloc TLS adapter.
