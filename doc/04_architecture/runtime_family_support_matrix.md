...
- `gc_async_mut` facades prefer matching `nogc_async_mut` surfaces first. A
  ...
  file-read, CUDA GPU FFI, shared GPU math/presentation hooks, and 2D GPU
  backend FFI/hook facades have been moved to the no-GC async surface first;
  remaining direct sync imports require per-API review before rerouting because
  they include sync socket exports or blocking runtime TLS boundaries.
