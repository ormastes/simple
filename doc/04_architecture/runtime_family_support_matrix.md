...
- [x] Fix committed-path native `PersistentMap`/`PersistentSet` facade probe blocker with root `PersistentMap` set/get and `PersistentSet` add/contains smoke across GC async, GC sync immutable, and no-GC sync immutable facades
- [ ] Add broader compiled behavior suites for map overwrite/removal, set removal/membership variants, trie mutation/lookup, and clear remaining native type-loader/minimal-runtime-stub warnings
