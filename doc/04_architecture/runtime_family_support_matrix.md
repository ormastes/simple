...
| `gc_async_immut` | `@gc` on root `__init__.spl`; facade-only | Recognized by interpreter family extraction (7th priority) | Yes | Yes (7th priority) | Minimal | Facade over `nogc_async_immut`; GC immutable runtime coverage still light |
