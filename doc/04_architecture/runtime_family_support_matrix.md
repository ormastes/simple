...
- [x] Fix committed-path native `PersistentMap`/`PersistentSet` facade probe blocker with root `PersistentMap` set/get and `PersistentSet` add/contains one-entry and two-entry smoke across GC async, GC sync immutable, and no-GC sync immutable facades
- [ ] Resolve native BDD blockers for map/set branch overwrite/removal, set dedupe/removal, and trie mutation/lookup, then clear remaining native type-loader/minimal-runtime-stub warnings
