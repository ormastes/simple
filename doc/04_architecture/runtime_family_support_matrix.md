...
|--------|---------------------------|-------------------|----------------------|---------------------|----------------|-------------|
| `common` | None (inherits project default) | Yes (auto-resolved) | No root `__init__.spl`; exported through parent `src/lib/__init__.spl` | Yes (5th priority) | Partial | Neutral shared family; no `@gc`/`@no_gc` attribute; root-manifest gap is tracked by the backend-boundary audit |
...
| `gc_async_mut` | No root `__init__.spl` attribute yet; GC semantics are family-prefix/runtime driven | Yes (6th priority in search) | No root `__init__.spl`; module-level GC/GPU exports only | Yes (6th priority) | Partial (GPU-specific) | Direct-import boundary checks present; root manifest and full manifest-to-GcMode enforcement remain partial |
...
| `gc_sync_mut` | No root `__init__.spl` attribute yet; facade-only modules mirror `gc_async_mut` | Recognized by interpreter family extraction (8th priority) | No root `__init__.spl`; per-module facades only | Yes (8th priority) | Minimal | Facade over `gc_async_mut`; root package facade and independent sync runtime semantics remain gaps |
