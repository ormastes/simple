...
- `SIMPLE_LIB=src src/compiler_rust/target/bootstrap/simple test test/unit/lib/{gc_async_immut,gc_sync_immut,nogc_sync_immut}/{map_facade_native_spec.spl,set_facade_native_spec.spl} --mode=native --clean --force-rebuild --format json` (run as six focused commands)
  ...
  - validates one-entry, two-entry, branch-shaped `app`/`apple` root-facade `PersistentMap` set/get, branch-shaped map overwrite/removal, and root-facade `PersistentSet` add/contains through GC async, GC sync immutable, and no-GC sync immutable compatibility families
  - set dedupe/removal and untyped chained root-facade trie mutation remain blocked by native BDD failures or timeouts; see `doc/08_tracking/bug/native_immut_facade_collections_bdd_blockers_2026-05-14.md`
