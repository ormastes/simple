...
| New facade-only runtime families need broader runtime coverage before public promotion | `gc_sync_mut`, `gc_async_immut`, `gc_sync_immut`, and `nogc_sync_immut` now have source facades, interpreter recognition, and focused interpreter behavior specs. `gc_sync_mut` has a passing native facade smoke; `gc_async_immut` links but times out in native execution with atomic stubs. | Fix immutable-family native atomic/runtime behavior, then add broader compiled behavior suites before promoting them from advanced-scoped to public. |
