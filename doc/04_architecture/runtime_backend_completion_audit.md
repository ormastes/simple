...
| Runtime/native execution coverage is still incomplete | Representative actor, TLS, WebSocket, web-framework facade, CUDA/engine FFI dispatch, and WebGPU interpreter specs now pass. Fresh local evidence: `SIMPLE_OS_BUILD_BACKEND=cranelift src/compiler_rust/target/debug/simple os test --scenario=riscv64-virtio-fat32-smf` builds via the cached Cranelift native artifact, boots QEMU, mounts the FAT32 image, launches SMF CLI/GUI packages, and reports `TEST PASSED`; `src/compiler_rust/target/debug/simple test test/feature/usage/cuda_spec.spl --mode=interpreter --clean --force-rebuild` passes 5 CUDA runtime-surface examples on a host with NVIDIA RTX A6000 and TITAN RTX devices; `src/compiler_rust/target/debug/simple test test/feature/scilib/cuda_device_buffer_spec.spl --mode=interpreter --clean --force-rebuild` passes 20 explicit CUDA buffer, CUDA-owned NDArray storage, device-side arithmetic, shape, combine, reduction, and slicing examples on the same CUDA host. Fresh x86_64 evidence: `SIMPLE_OS_BUILD_BACKEND=cranelift src/compiler_rust/target/debug/simple os test --arch=x86_64` restores the vendored Ring fiat headers needed by `curve25519_ring_helper.c`, links the freestanding typed-word u64 wrappers, boots QEMU, reaches `[stage1] PASS: Kernel boot + PCI scan`, and reports `TEST PASSED`. The default riscv64 OS test remains blocked without an LLVM-enabled driver, and no full OS boot matrix was run. | Run the full multi-arch OS boot matrix with an LLVM-enabled driver before declaring production completion; keep the x86_64 Ring helper and freestanding typed-word wrappers in the matrix because default x86_64 boot now depends on them. |
