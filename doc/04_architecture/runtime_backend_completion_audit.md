...
| Runtime-family root manifest claims match the tree | `scripts/audit/runtime_backend_boundaries.py` reports `documented_root_manifest_mismatch_count=0`, so Section 3 does not overclaim root `__init__.spl` files for `common`, `gc_async_mut`, or `gc_sync_mut`. | Closed for documentation/tree consistency; actual root manifests for those families remain tracked gaps |
...
| Sync/GC and sync/immutable family names exist as backend-backed compatibility surfaces | `src/lib/gc_sync_mut`, `src/lib/gc_async_immut`, `src/lib/gc_sync_immut`, and `src/lib/nogc_sync_immut` now exist as facade-only compatibility families. `gc_sync_mut` exports tracked `gc_async_mut` modules; immutable variants export `nogc_async_immut` / `gc_async_immut` surfaces. `scripts/audit/runtime_backend_boundaries.py` reports `sync_compat_direct_runtime_hook_count=0` and `sync_compat_facade_mirror_violation_count=0`, so these compatibility families do not own local runtime hooks, every tracked facade has a matching backing module/export, and every tracked non-root backing module has a corresponding compatibility facade. Interpreter fallback search and family extraction recognize all four names. Rust `simple check --deny-gc-boundary` / `--strict-runtime-family` promotes runtime-family violations to errors for target-restricted checks while default checks keep warnings, and target-strict noalloc checks now follow reachable imports before allowing a noalloc closure. The Rust test runner and direct file interpreter launch now enable the same strict mode automatically for baremetal/SimpleOS `--target` values. | Advanced-scoped; broad immutable facade and direct no-GC async coordination coverage exist, but sync-wrapper behavior decisions, root package manifests, and remaining compiler entrypoint policy propagation remain |
