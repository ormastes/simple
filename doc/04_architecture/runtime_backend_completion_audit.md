...
| --- | --- | --- |
| No-GC async backend modules still carry behavior-diff runtime hooks with sync counterparts | Current scan reports 6 files / 183 hook declarations: `nogc_async_mut/torch/ffi.spl`, `cuda/ffi.spl`, `cuda/mod.spl`, `gpu/memory.spl`, `http_server/response.spl`, `io/file.spl`. `nogc_async_mut/io/buffer.spl` and `nogc_sync_mut/io/buffer.spl` now use pure text/byte conversion helpers instead of runtime hooks. | Treat remaining files as backend modules unless review proves they should delegate to another backend. |
