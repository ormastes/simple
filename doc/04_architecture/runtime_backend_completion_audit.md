...
- `SIMPLE_LIB=src src/compiler_rust/target/bootstrap/simple test test/unit/lib/{gc_async_immut,gc_sync_immut,nogc_sync_immut}/trie_root_facade_native_spec.spl --mode=native --clean --force-rebuild --format json` (run as three focused commands)
  - exit code `0` for all three focused native runs
  - `3` native spec files passed
  - validates typed root-facade `PersistentTrie` shared-prefix set/get and overwrite behavior through GC async, GC sync immutable, and no-GC sync immutable compatibility facades
  - untyped chained root import `PersistentTrie.empty().set("app", 1).set("apple", 2)` remains blocked by native method dispatch ambiguity, reduced to a `PersistentMap.set` frame before trie frames
- `SIMPLE_LIB=src src/compiler_rust/target/bootstrap/simple test test/unit/lib/{gc_async_immut,gc_sync_immut,nogc_sync_immut}/{map_facade_native_spec.spl,set_facade_native_spec.spl} --mode=native --clean --force-rebuild --format json` (run as six focused commands)
  ...
  - untyped chained root-facade trie mutation remains blocked by native BDD failures or timeouts; see `doc/08_tracking/bug/native_immut_facade_collections_bdd_blockers_2026-05-14.md`
  ...
  - known non-fatal warnings remain for empty-module type loading and minimal-runtime atomic stubs
