...
| No-GC async files with no-GC sync counterparts do not redeclare backend hooks after review | The reviewed behavior-diff set (`torch/ffi`, `cuda/ffi`, `cuda/mod`, `gpu/memory`, `http_server/response`, `io/buffer`, `io/file`, `package_ffi`, `platform`, `simd`, `mimalloc_tls`, `coverage`, `atomic`, `conf`, `fuzz`) now has no local `extern fn rt_*` declarations. Remaining runtime hooks were moved to no-GC sync backend owners such as `cuda/ffi.spl`, `ptr/raw.spl`, `io/dir_entry_ops.spl`, and `runtime/thread_local.spl`, replaced with pure Simple conversion helpers, represented as facades over no-GC sync roots, or routed through no-GC sync file/dir wrappers for MCP and HTTP file helpers. A tracked scan now finds no `nogc_async_mut` runtime-hook file with a same-path `nogc_sync_mut` counterpart. | Closed for same-path counterpart class |
...
| --- | --- | --- |
| GC async still has GC-only runtime-hook files | Current tracked-only scan reports 45 GC async hook files / 475 hook declarations after package FFI, platform root, SIMD, mimalloc TLS hook-owner, and browser-session file-read closure; remaining files are GPU/vendor/browser-renderer surfaces. | Review by domain: GPU/vendor backends and browser renderer surfaces. |
| No-GC async still has runtime-hook files without sync counterparts | Current tracked-only scan reports 58 no-GC async hook files / 158 hook declarations, with no remaining same-path no-GC sync counterpart files. | Review by domain and either introduce no-GC sync backend owners or document async-specific ownership. |
