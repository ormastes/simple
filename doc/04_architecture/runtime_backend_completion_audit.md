...
- `SIMPLE_LIB=src cargo run --manifest-path src/compiler_rust/Cargo.toml -p simple-driver --bin simple -- test test/unit/lib/{gc_async_immut,gc_sync_immut,nogc_sync_immut}/persistent_vec_native_spec.spl --mode=native --clean --force-rebuild --format json` (run as three focused commands)
  ...
  - validates repeated tail pushes and tail `set()` through the GC async, GC sync immutable, and no-GC sync immutable facades
  - `from_array` still fails natively because the compiled caller discards the static constructor result, so vector remains below full promotion threshold
