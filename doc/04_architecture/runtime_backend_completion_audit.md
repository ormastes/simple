...
| No-GC async still has runtime-hook files without sync counterparts | Current tracked-only scan reports 51 no-GC async hook files / 151 hook declarations, with no remaining same-path no-GC sync counterpart files. | Review by domain and either introduce no-GC sync backend owners or document async-specific ownership. |
