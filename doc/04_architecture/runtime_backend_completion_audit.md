...
| GC async pure facades prefer no-GC async surfaces where available | Redis client, service audit/daemon/extern/lease/lifecycle/request-queue facades, file operations, hosted I/O stubs, `net.udp`, browser file-read imports, and the CUDA GPU FFI facade now route through matching `nogc_async_mut` surfaces instead of directly importing `nogc_sync_mut`; targeted `simple check` passes for the changed facades, with pre-existing direct-check unresolved import warnings in the browser session files unrelated to the rerouted file-read import. | Partial; remaining direct sync imports are TCP/UDP sync socket exports, GPU backend FFI/math/present hooks, or behavior-sensitive runtime TLS paths requiring per-API review |
