...
- `SIMPLE_LIB=src src/compiler_rust/target/bootstrap/simple test test/unit/lib/{gc_async_immut,gc_sync_immut,nogc_sync_immut}/trie_facade_native_spec.spl --mode=native --clean --force-rebuild --format json` (run as three focused commands)
  ...
  - broad root import `use std.gc_async_immut.{PersistentTrie}` remains blocked by a native HAMT equality crash in `PersistentMap.set` during root facade import/execution
- `SIMPLE_LIB=src src/compiler_rust/target/bootstrap/simple test test/unit/lib/{gc_async_immut,gc_sync_immut,nogc_sync_immut}/{map_facade_native_spec.spl,set_facade_native_spec.spl} --mode=native --clean --force-rebuild --format json` (run as six focused commands)
  ...
  - map/set branch overwrite/removal, set dedupe/removal, and broad root-facade trie import coverage remain blocked by native BDD failures or timeouts; see `doc/08_tracking/bug/native_immut_facade_collections_bdd_blockers_2026-05-14.md`
