...
| GC async one-line facades do not self-reexport when no-GC async owners exist | A precise scan for tracked one-line `export use (std.)?gc_async_mut.*` files now reports `194` remaining after TLS, TCP, WebSocket, web-framework, hooks/detectors, HTTP/3, and MCP SDK server/transport closures. | In progress |
...
| GC async one-line self-facade audit remains incomplete | A precise tracked scan still reports 194 one-line `export use (std.)?gc_async_mut.*` files, concentrated in `test_runner`, `src`, `replay`, `terminal`, `sanitizer`, `security`, `web_ui`, `storage`, and `tui`. | Continue tested slice-by-slice routing to `nogc_async_mut` or document GC-specific exceptions. |
