...
| No-GC async still has runtime-hook files without sync counterparts | Current tracked-only scan reports 22 no-GC async hook files / 77 hook declarations, with no remaining same-path no-GC sync counterpart files. MCP entrypoint GC collection cadence now calls the no-GC sync runtime wrapper instead of owning local GC hooks, and DNS resolver packet text conversion now uses pure text byte helpers. | Review by domain and either introduce no-GC sync backend owners or document async-specific ownership. |
