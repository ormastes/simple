...
| No-GC async still has runtime-hook files without sync counterparts | Current tracked-only scan reports 21 no-GC async hook files / 75 hook declarations, with no remaining same-path no-GC sync counterpart files. MCP entrypoint GC collection cadence now calls the no-GC sync runtime wrapper instead of owning local GC hooks, DNS resolver packet text conversion now uses pure text byte helpers, and async byte file operations use the no-GC sync FFI byte-file owner. | Review by domain and either introduce no-GC sync backend owners or document async-specific ownership. |
