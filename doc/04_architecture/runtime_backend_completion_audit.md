...
| --- | --- | --- |
| GC async still has GC-only runtime-hook files | Current tracked-only scan reports 48 GC async hook files / 481 hook declarations after package FFI, platform root, and SIMD facade closure; sampled largest files are GPU/vendor/browser-renderer surfaces with no no-GC counterpart. | Review by domain: GPU/vendor backends, browser renderer surfaces, and any portable non-GPU leftovers. |
