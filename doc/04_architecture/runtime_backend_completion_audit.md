...
| Sync/GC and sync/immutable family names exist as backend-backed compatibility surfaces | `src/lib/gc_sync_mut`, `src/lib/gc_async_immut`, `src/lib/gc_sync_immut`, and `src/lib/nogc_sync_immut` now exist as facade-only compatibility families. `gc_sync_mut` exports tracked `gc_async_mut` modules; immutable variants export `nogc_async_immut` / `gc_async_immut` surfaces. Interpreter fallback search and family extraction recognize all four names. | Advanced-scoped; runtime coverage still needed |
| New facade-only runtime families have focused import coverage | `test/unit/lib/{gc_sync_mut,gc_async_immut,gc_sync_immut,nogc_sync_immut}/facade_resolution_spec.spl` each pass in interpreter mode and exercise representative pure/persistent helpers through the new family names. | Closed for focused import/type-resolution coverage |
...
- No-GC async wildcard facade review scan:
  - `remaining=135` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the database checker/requirement/todo facades.
- No-GC async desktop wildcard facade review scan:
  - `remaining=125` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the desktop dialog/display/global_shortcut/lifecycle/notification/power/protocol/shell_open/tray/updater facades.
- No-GC async I/O wildcard facade review scan:
  - `remaining=120` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the I/O debug_state/debug_stubs/file_discovery/file_shell/signal_handlers facades.
- No-GC async test-runner wildcard facade review scan:
  - `remaining=115` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the test-runner resource_governor/test_classification/test_runner_args/test_runner_async/test_runner_files facades.
- No-GC async security wildcard facade review scan:
  - `remaining=112` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the security audit_chain/audit_log/types facades.
- No-GC async LSP wildcard facade review scan:
  - `remaining=110` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the LSP lsp_query/parser_adapter facades.
- No-GC async experiment wildcard facade review scan:
  - `remaining=107` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the experiment artifact/config/sweep facades.
- No-GC async OIDC wildcard facade review scan:
  - `remaining=105` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the OIDC discovery/id_token facades.
- No-GC async spec wildcard facade review scan:
  - `remaining=103` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the spec decorators/env_detect facades.
- No-GC async hooks wildcard facade review scan:
  - `remaining=101` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the hooks mod/stop facades.
- No-GC async FFI dynamic wildcard facade review scan:
  - `remaining=99` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the FFI dynamic/dynamic_versioned facades.
- No-GC async benchmark/GPU-runtime wildcard facade review scan:
  - `remaining=97` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the benchmark string_bench and gpu_runtime/mod facades.
- No-GC async platform/play wildcard facade review scan:
  - `remaining=95` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the platform linker and play expect facades.
- No-GC async debug/shader wildcard facade review scan:
  - `remaining=91` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the ST-Link tools, T32 window capture, QEMU runner, and shader_compile facades.
- No-GC async terminal/time/daemon wildcard facade review scan:
  - `remaining=87` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the terminal credential config, daemon SDK types, serial_proxy, and time_ops facades.
- No-GC async SSH/JS/coverage/fuzz wildcard facade review scan:
  - `remaining=83` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the SSH serial, JS interpreter, coverage, and fuzz facades.
- No-GC async daemon/debug/coverage wildcard facade review scan:
  - `remaining=79` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the daemon runner, dSYM resolver, OpenOCD protocol, and FFI coverage facades.
- No-GC async I/O profiler/sysinfo/thread wildcard facade review scan:
  - `remaining=76` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the profiler_simple, sysinfo_ops, and thread facades.
- No-GC async LSP/MCDC wildcard facade review scan:
  - `remaining=73` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the LSP diagnostics, LSP WASI main, and MCDC facades.
- No-GC async platform root wildcard facade review scan:
  - `remaining=71` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the no-GC async platform file and directory root facades.
- No-GC async Play runtime wildcard facade review scan:
  - `remaining=67` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the Play Electron application, page, SDL2 backend, and window-manager facades.
- No-GC async shell/test-runner wildcard facade review scan:
  - `remaining=64` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the shell env, system monitor, and test-runner coverage facades.
- No-GC async daemon/I/O runtime-hook wildcard facade review scan:
  - `remaining=60` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the daemon client, DH FFI, directory operations, and signal stubs facades.
- No-GC async log/LSP/Play/test-manifest wildcard facade review scan:
...
- No-GC async serial/SIMD/daemon/debug wildcard facade review scan:
  - `remaining=40` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the serial FFI, SIMD profile, daemon lock, and SMF debug agent facades.
- No-GC async env/coverage/FFI wildcard facade review scan:
...
- No-GC async font/protocol/launcher wildcard facade review scan:
  - `remaining=33` wildcard facades still point at no-GC sync owners with runtime hooks after narrowing the font FFI, daemon protocol, and Play launcher facades; the pipe facade was deferred until the final closure pass.
...
- No-GC async I/O device/archive wildcard facade review scan:
...
- No-GC async graphics/window/codegen/Torch wildcard facade review scan:
  - `remaining=1` after narrowing the graphics2d, Rapier2D, window, codegen, and Torch FFI facades; this was the last pre-closure scan before the final pipe/config/debug/LSP/mem-tracker/test-runner facade pass.
...
| New facade-only runtime families need broader runtime coverage | `gc_sync_mut`, `gc_async_immut`, `gc_sync_immut`, and `nogc_sync_immut` now have source facades, interpreter recognition, and focused import/type-resolution specs. They do not yet have broad behavior suites. | Add selected behavior specs before promoting them from advanced-scoped to public. |
