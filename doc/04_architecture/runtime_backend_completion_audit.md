...
| GC async one-line facades do not self-reexport when no-GC async owners exist | A precise scan for tracked one-line `export use (std.)?gc_async_mut.*` files now reports `191` remaining after TLS, TCP, WebSocket, web-framework, hooks/detectors, HTTP/3, MCP SDK server/transport, and play/CDP closures. | In progress |
...
| GC async one-line self-facade audit remains incomplete | A precise tracked scan still reports 191 one-line `export use (std.)?gc_async_mut.*` files, concentrated in `test_runner`, `src`, `replay`, `terminal`, `sanitizer`, `security`, `web_ui`, `storage`, and `tui`. | Continue tested slice-by-slice routing to `nogc_async_mut` or document GC-specific exceptions. |
