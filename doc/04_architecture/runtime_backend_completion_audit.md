...
| --- | --- | --- |
| Repeatable backend-boundary audit exists | `scripts/audit/runtime_backend_boundaries.py` checks tracked async compatibility families and sync/immutable compatibility families for direct `extern fn rt_*` ownership, sync/immutable compatibility families for matching backing modules plus facade exports, no-GC async wildcard facades over no-GC sync runtime-hook owners, SimpleOS native lower layers for POSIX/libc imports, and portable library roots for forbidden POSIX/Linux imports outside explicit platform/compatibility paths. | Closed for static boundary gate |
