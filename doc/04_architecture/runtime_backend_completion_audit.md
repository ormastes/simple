...
| --- | --- | --- |
| No-GC async backend modules still carry behavior-diff runtime hooks with sync counterparts | Current scan reports 7 files / 185 hook declarations: `nogc_async_mut/torch/ffi.spl`, `cuda/ffi.spl`, `cuda/mod.spl`, `gpu/memory.spl`, `http_server/response.spl`, `io/buffer.spl`, `io/file.spl`. | Treat as backend modules unless review proves they should delegate to another backend. |
