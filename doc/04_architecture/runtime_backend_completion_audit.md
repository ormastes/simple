...
| --- | --- | --- |
| No-GC async backend modules still carry behavior-diff runtime hooks with sync counterparts | Current scan reports 1 file / 6 hook declarations: `nogc_async_mut/io/file.spl`. CUDA FFI hooks are owned by `nogc_sync_mut/cuda/ffi.spl`; async CUDA FFI is a facade and async CUDA/GPU memory use the no-GC sync raw pointer backend plus Torch FFI imports instead of local declarations. `nogc_async_mut/torch/ffi.spl` now delegates to `nogc_sync_mut/torch/ffi.spl`. `nogc_async_mut/io/file.spl` now routes file existence and size through the no-GC sync file-ops backend, while `nogc_sync_mut/io/file.spl` uses pure text/byte conversion helpers instead of conversion hooks. `nogc_async_mut/io/buffer.spl`, `nogc_sync_mut/io/buffer.spl`, and `nogc_async_mut/http_server/response.spl` also use pure text/byte conversion helpers instead of runtime hooks. | Treat remaining async file directory hooks as backend operations unless review proves they should delegate to another backend. |
