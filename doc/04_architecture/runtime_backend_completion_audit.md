...
| GC async pure facades prefer no-GC async surfaces where available | Redis client, service audit/daemon/extern/lease/lifecycle/request-queue facades, and file operations now route through matching `nogc_async_mut` surfaces instead of directly importing `nogc_sync_mut`; targeted `simple check` passes for the changed facades. | Partial; remaining direct sync imports are GPU/FFI or behavior-sensitive paths requiring per-API review |
