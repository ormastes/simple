...
| GC async pure facades prefer no-GC async surfaces where available | Redis client, service audit/daemon/extern/lease/lifecycle/request-queue facades, file operations, hosted I/O stubs, `net.udp`, browser file-read imports, TCP/UDP socket facade exports, the CUDA GPU FFI facade, shared GPU math/presentation hook imports, and 2D GPU backend FFI/hook imports now route through matching `nogc_async_mut` surfaces instead of directly importing `nogc_sync_mut`; targeted `simple check` passes for the changed facades, with pre-existing direct-check unresolved import warnings in the browser session files unrelated to the rerouted file-read import. | Partial; remaining direct sync import is the behavior-sensitive runtime TLS path in `gc_async_mut/mimalloc_tls.spl` |
