...
| --- | --- | --- |
| Repeatable backend-boundary audit exists | `scripts/audit/runtime_backend_boundaries.py` checks tracked async compatibility families and sync/immutable compatibility families for direct `extern fn rt_*` ownership, sync/immutable compatibility families for matching backing modules plus facade exports, GC async one-line self-facades, no-GC async wildcard facades over no-GC sync runtime-hook owners, SimpleOS native lower layers for POSIX/libc imports, and portable library roots for forbidden POSIX/Linux imports outside explicit platform/compatibility paths. | Closed for static boundary gate |
| Async/GC exact-copy runtime hooks are not duplicated across families | `scripts/audit/runtime_backend_boundaries.py` reports `exact_duplicate_runtime_hook_count=0` for `src/lib/gc_async_mut` and `src/lib/nogc_async_mut` files that match `nogc_sync_mut` byte-for-byte and declare `extern fn rt_*`. | Closed for exact-copy class |
...
| Portable stdlib/library roots do not import SimpleOS lower layers | `scripts/audit/runtime_backend_boundaries.py` reports `portable_lib_forbidden_os_import_count=0`, blocking direct imports of `os`, `simple_os`, `sosix`, `kernel`, `drivers`, and `services` modules from portable library roots outside explicit platform/compat paths. | Closed for static import gate |
| No-GC runtime families do not depend upward on GC compatibility families | `scripts/audit/runtime_backend_boundaries.py` reports `nogc_family_forbidden_gc_import_count=0`. The no-GC async GPU package root now exports its own no-GC async `device`, `memory`, and `context` modules instead of importing `gc_async_mut.gpu.*`; async intrinsic modules are facades over no-GC backends rather than duplicate placeholder bodies. | Closed for static import direction gate |
| GC async pure facades prefer no-GC async surfaces where available | Redis client, service audit/daemon/extern/lease/lifecycle/request-queue facades, file operations, hosted I/O stubs, `net.udp`, browser file-read imports, TCP/UDP socket facade exports, mimalloc TLS thread-local hooks, the CUDA GPU FFI facade, shared GPU math/presentation hook imports, and 2D GPU backend FFI/hook imports now route through matching `nogc_async_mut` surfaces instead of directly importing `nogc_sync_mut`; targeted `simple check` passes for the changed facades, with pre-existing direct-check unresolved import warnings in the browser session files unrelated to the rerouted file-read import. | Closed for GC async direct-to-no-GC sync imports when a matching no-GC async surface exists |
...
| Runtime-family root manifest claims match the tree | `common`, `gc_async_mut`, and `gc_sync_mut` now have tracked root `__init__.spl` manifests, and `scripts/audit/runtime_backend_boundaries.py` reports `documented_root_manifest_mismatch_count=0`. | Closed for documentation/tree consistency and tracked root manifests |
| GC async one-line facades do not self-reexport when no-GC async owners exist | `scripts/audit/runtime_backend_boundaries.py` reports `gc_async_one_line_self_facade_count=0`, covering tracked one-line `export use (std.)?gc_async_mut.*` files, including top-level `src/lib/gc_async_mut/*.spl`. | Closed for tracked one-line self-facade class |
| Async compatibility families do not carry TODO bodies | `scripts/audit/runtime_backend_boundaries.py` reports `async_compat_pass_todo_count=0`. Compile-time intrinsic helpers now route through no-GC facades, and GPU builtin intrinsics have deterministic no-GC async interpreter fallbacks with GC async as a facade. | Closed for tracked `pass_todo` bodies in `gc_async_mut` and `nogc_async_mut` |
...
| Sync/GC and sync/immutable family names exist as backend-backed compatibility surfaces | `src/lib/gc_sync_mut`, `src/lib/gc_async_immut`, `src/lib/gc_sync_immut`, and `src/lib/nogc_sync_immut` now exist as facade-only compatibility families. `gc_sync_mut` exports tracked `gc_async_mut` modules, including the root package facade; immutable variants export `nogc_async_immut` / `gc_async_immut` surfaces. `scripts/audit/runtime_backend_boundaries.py` reports `sync_compat_direct_runtime_hook_count=0` and `sync_compat_facade_mirror_violation_count=0`, so these compatibility families do not own local runtime hooks, every tracked facade has a matching backing module/export, and every tracked backing module has a corresponding compatibility facade. Interpreter fallback search and family extraction recognize all four names. Rust `simple check --deny-gc-boundary` / `--strict-runtime-family` promotes runtime-family violations to errors for target-restricted checks while default checks keep warnings, and target-strict noalloc checks now follow reachable imports before allowing a noalloc closure. The Rust test runner and direct file interpreter launch now enable the same strict mode automatically for baremetal/SimpleOS `--target` values. | Advanced-scoped; broad immutable facade and direct no-GC async coordination coverage exist, but sync-wrapper behavior decisions and remaining compiler entrypoint policy propagation remain |
...
- `python3 scripts/audit/runtime_backend_boundaries.py`
  ...
  - `exact_duplicate_runtime_hook_count=0`
  - `sync_compat_direct_runtime_hook_count=0`
  - `sync_compat_facade_mirror_violation_count=0`
  - `async_compat_pass_todo_count=0`
  - `gc_async_one_line_self_facade_count=0`
  ...
  - `portable_lib_forbidden_os_import_count=0`
  - `nogc_family_forbidden_gc_import_count=0`
  - `documented_root_manifest_mismatch_count=0`
...
| Runtime/native execution coverage is still incomplete | Representative actor, TLS, WebSocket, web-framework facade, CUDA/engine FFI dispatch, and WebGPU interpreter specs now pass. Fresh local evidence: `SIMPLE_OS_BUILD_BACKEND=cranelift src/compiler_rust/target/debug/simple os test --scenario=riscv64-virtio-fat32-smf` builds via the cached Cranelift native artifact, boots QEMU, mounts the FAT32 image, launches SMF CLI/GUI packages, and reports `TEST PASSED`; `src/compiler_rust/target/debug/simple test test/feature/usage/cuda_spec.spl --mode=interpreter --clean --force-rebuild` passes 5 CUDA runtime-surface examples on a host with NVIDIA RTX A6000 and TITAN RTX devices; `src/compiler_rust/target/debug/simple test test/feature/scilib/cuda_device_buffer_spec.spl --mode=interpreter --clean --force-rebuild` passes 20 explicit CUDA buffer, CUDA-owned NDArray storage, device-side arithmetic, shape, combine, reduction, and slicing examples on the same CUDA host. Fresh x86_64 evidence: `SIMPLE_OS_BUILD_BACKEND=cranelift src/compiler_rust/target/debug/simple os test --arch=x86_64` restores the vendored Ring fiat headers needed by `curve25519_ring_helper.c`, links the freestanding typed-word u64 wrappers, boots QEMU, reaches `[stage1] PASS: Kernel boot + PCI scan`, and reports `TEST PASSED`. Fresh ARM64 evidence: `SIMPLE_OS_BUILD_BACKEND=cranelift src/compiler_rust/target/debug/simple os test --scenario=arm64-virtio-fat32-smf` now builds and boots the VirtIO FAT32 SMF lane, reads real ELF-backed SMF bytes for hello/simple_compiler/simple_interpreter/simple_loader/clang, registers synthetic process pids for those apps, and reaches guest `TEST PASSED`; the catalog still fails because LLVM and Rust fixture reads return only 34 bytes from the ARM64 known-path path, preventing Rust from normalizing to a valid SMF/ELF image and leaving `[desktop-e2e] process-backed:ok app=rust pid=` missing. The default riscv64 OS test remains blocked without an LLVM-enabled driver, ARM32/RV32 lanes were not rerun in this pass, and no full OS boot matrix was run. | Run the full multi-arch OS boot matrix with an LLVM-enabled driver before declaring production completion; keep the x86_64 Ring helper, freestanding typed-word wrappers, ARM64 freestanding helper set, ARM64 VFS cluster map, and Rust SMF fixture blocker in the matrix. |
