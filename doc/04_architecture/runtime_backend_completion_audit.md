...
| Runtime/native execution coverage is still incomplete | Representative actor, TLS, WebSocket, web-framework facade, CUDA/engine FFI dispatch, and WebGPU interpreter specs now pass. Fresh local evidence: `SIMPLE_OS_BUILD_BACKEND=cranelift src/compiler_rust/target/debug/simple os test --scenario=riscv64-virtio-fat32-smf` builds via the cached Cranelift native artifact, boots QEMU, mounts the FAT32 image, launches SMF CLI/GUI packages, and reports `TEST PASSED`; `src/compiler_rust/target/debug/simple test test/feature/usage/cuda_spec.spl --mode=interpreter --clean --force-rebuild` passes 5 CUDA runtime-surface examples on a host with NVIDIA RTX A6000 and TITAN RTX devices; `src/compiler_rust/target/debug/simple test test/feature/scilib/cuda_device_buffer_spec.spl --mode=interpreter --clean --force-rebuild` passes 20 explicit CUDA buffer, CUDA-owned NDArray storage, device-side arithmetic, shape, combine, reduction, and slicing examples on the same CUDA host. Fresh x86_64 matrix attempt: `src/compiler_rust/target/debug/simple os test --help` is not a help-only path and attempted the default x86_64 Cranelift OS build; it failed before QEMU because `examples/simple_os/arch/x86_64/boot/curve25519_ring_helper.c` includes vendored Ring `curve25519.c`, which requires missing `third_party/fiat/curve25519_64.h`, and the link still had unresolved `rt_typed_words_u64_push` / `rt_typed_words_u64_set`. The default riscv64 OS test remains blocked without an LLVM-enabled driver, and no full OS boot matrix was run. | Restore or replace the missing Ring fiat headers or gate that helper, close the typed-words runtime-symbol link gap for x86_64 OS builds, then run the full multi-arch OS boot matrix with an LLVM-enabled driver before declaring production completion. |
