...
| --- | --- | --- |
| No-GC async backend modules still carry behavior-diff runtime hooks with sync counterparts | Current scan reports 4 files / 48 hook declarations: `nogc_async_mut/cuda/ffi.spl`, `cuda/mod.spl`, `gpu/memory.spl`, `io/file.spl`. `nogc_async_mut/torch/ffi.spl` now delegates to `nogc_sync_mut/torch/ffi.spl`. `nogc_async_mut/io/file.spl` now routes file existence and size through the no-GC sync file-ops backend, while `nogc_sync_mut/io/file.spl` uses pure text/byte conversion helpers instead of conversion hooks. `nogc_async_mut/io/buffer.spl`, `nogc_sync_mut/io/buffer.spl`, and `nogc_async_mut/http_server/response.spl` also use pure text/byte conversion helpers instead of runtime hooks. | Treat remaining files as backend modules unless review proves they should delegate to another backend. |
