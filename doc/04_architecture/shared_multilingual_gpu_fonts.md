...
| `src/lib/skia/feature/shaper/ot_layout_{apply,context,gpos,gpos_data,shaper}.spl` and `src/lib/skia/feature/glyph/ot_parser_layout.spl` | Existing Pure Simple OpenType owners. `LayoutContext*`/`layout_context_*` is the context facade and `GposData*`/`gpos_data_*` is the positioning-data facade; sibling modules do not import underscore-private symbols. REQ-016 completes GSUB 1–8, GPOS 1–9, lookup flags/GDEF, device/variation data, FeatureVariations, and Script/LangSys selection transactionally inside these owners. Pinned Hindi, Arabic, and Urdu vectors remain independent oracles rather than an executor allow-list. Full BiDi and Emoji sequence/color shaping remain separate gated work. |
