...
| `src/lib/common/gpu/font_owner_fault_receipt.spl` | Native-safe scalar fault enum and exact reason classifier shared by the Vulkan font owners. Engine2D/Engine3D retain lifetime-monotonic masks and scalar sequences locally; classifier output alone is not evidence. |
| `src/lib/nogc_sync_mut/text_layout/font_types.spl` | Canonical shared values; owns `FontGlyphRun`, `FontRenderQuad`, `FontRenderBatch`, `FontRenderConfig`, `FontExecutionPolicy`, and the pure execution-plan functions. Native-sensitive consumers use `font_execution_plan_into` with caller-owned storage rather than returning a plan aggregate. `FontGlyphRun` carries the exact revocable face handle/generation pair plus logical codepoint clusters; consumers validate liveness and never dereference the handle directly. |
...
| `src/lib/skia/feature/shaper/ot_layout_{apply,context,gpos,gpos_data,shaper}.spl` and `src/lib/skia/feature/glyph/ot_parser_layout.spl` | Existing Pure Simple OpenType owners. `LayoutContext*`/`layout_context_*` is the context facade and `GposData*`/`gpos_data_*` is the positioning-data facade; sibling modules do not import underscore-private symbols. REQ-016 completes GSUB 1–8, GPOS 1–9, lookup flags/GDEF, device/variation data, FeatureVariations, and Script/LangSys selection transactionally inside these owners. Contextual GSUB composes old-to-new index maps; GPOS shares one ppem/coordinate/work context and keeps post-scale Device pixels separate from design-unit VariationIndex values. Pinned Hindi, Arabic, and Urdu vectors remain executor oracles, while the selected high-level complex-script preprocessor fails closed outside its supported inputs. Full BiDi and Emoji sequence/color shaping remain separate gated work. |
...
Engine3D retains its renderer through the existing one-slot
`Engine2DFontOwner`, stages text/glyph material in that slot, and transfers
batch fields inbound; renderer, plan, batch, and world-projection aggregates do
not cross native-sensitive return boundaries.
