...
## Results — 39 specs compared (10 no-oracle, excluded), **30 divergent rows**

Curated set: 60 stride-sampled pure-logic specs from `test/01_unit/lib/common` and
`test/01_unit/compiler`; the run was cut at spec ~45 by the session budget, so this
is 39 compared, not 60. The manifest is complete for what ran.

| class | rows | specs | smallest reproducer |
|---|---|---|---|
| `other` | 13 | `ui/render_opt/occlusion_spec.spl` (8), `crypto/hmac_sha1_spec.spl` (5) | `crypto/hmac_sha1_spec.spl` — all RFC-2202 test vectors pass interpreted, fail natively |
| `link` | 7 | `encoding/{utf32_byte_guard,protobuf_wire_bounds_guard,codec_decode_byte_guard}`, `search/explain_contract`, `engine/math3d_trig_precision_repro`, `web/{browser_renderer_frame_reuse_protocol,browser_session_html_text_level_tags}` | `encoding/utf32_byte_guard_spec.spl` — unresolved `rt_utf8_validate` / `rt_utf8_find_invalid` / `rt_utf8_count_codepoints` |
| `int-width-bitops` | 5 | `bytes/ints_spec.spl` | `U32le.of(0xDEADBEEF).to_span()` then `U32le.load(sp,0).value()` — `ints_spec.spl:38-42` |
| `crash` | 3 | `text_advanced_return_types_spec.spl`, `wine_process_session_{import_resolution,first_import_module}_spec.spl` | `text_advanced_return_types_spec.spl` — SEGV at `StringBuilder_dot_to_text+16`, `ldr x28,[x8]`, x8 null |
| `truncated-run` | 1 | `token_budget_spec.spl` | native run prints one example name then exits rc=65 with no summary line |
| `build-failed` | 1 | `imaging/find_diff_regions_spec.spl` | — |

Every `crash` row was triaged to a faulting pc before being called codegen: all
