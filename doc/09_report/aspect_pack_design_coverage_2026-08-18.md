...
| 12.1 | SMF header compatibility | PARTIAL | `APK_FLAG_ASPECT_PACK` is set and an unflagged file is REJECTED (`APK_NOT_ASPECT_PACK`, REQ-APK-DC-04). UPDATED 2026-08-24: the Pure Simple producer/writer and loader-side reader exist for `SectionType.AspectPackDirectory`, registered as wire byte 16 (`.aspect_pack`). `SMF_FLAG_ASPECT_PACK` uses extension bit `0x40`; bit `0x20` remains the existing note.sdn feature bit. Standalone packs clear `SMF_FLAG_EXECUTABLE`, and the reader parses the disjoint aspect flag. Automatic `--mode dynload` packaging, `ModuleLoader.load` registration, and resident facet routing APIs remain OPEN. `AspectPackSignature`/`AspectPackDictionary` are still NOT registered: no signature verifier and no shared dictionary codec exist in-tree. |
...
| 23 | Integration with existing architecture | PARTIAL (2026-08-24) | `src/compiler/99.loader/aspect_pack_section.spl` can find the `.aspect_pack` section in a parsed `SmfReaderMemory` and register its payload with the aspect-pack provider when called directly. Automatic `ModuleLoader.load` routing and the `aspect_facet`/`resident_aspect_facet` APIs are OPEN and their examples remain explicitly pending in `test/01_unit/compiler/loader/aspect_pack_smf_section_wiring_spec.spl`. |
