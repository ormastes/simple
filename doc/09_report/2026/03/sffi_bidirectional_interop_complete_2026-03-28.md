...
|------|------------|-------------|
| `test/03_system/sffi_bidirectional_interop_spec.spl` | 30 | End-to-end system tests across all three directions |
| `test/01_unit/compiler/common/export_attr_spec.spl` | 9 | `ExportAttr` parsing and validation |
| `test/01_unit/compiler/semantics/sffi_lint_spec.spl` | 17 | All 5 SFFI lint rules with positive and negative cases |
| `test/01_unit/compiler/types/layout_verification_spec.spl` | 28 | `StructLayout`, `verify_c_layout`, `_Static_assert` generation |
| `test/01_unit/compiler/tools/header_gen_spec.spl` | 22 | C and C++ header generation, platform flag selection |
