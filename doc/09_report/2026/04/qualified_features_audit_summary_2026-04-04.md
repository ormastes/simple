...
|---------|-----------------|--------------|------------------|---------------------------|
| **Math blocks** (`m{}`, `loss{}`, `nograd{}`) | Parser (Rust + Simple), block registry, HIR resolve, MIR lowering, rendering library (5 formats), VSCode extension (concealment + hover + panel), Neovim plugin (extmarks + float), treesitter highlighting | `loss{}`/`nograd{}` autograd runtime stubs (no `rt_backward`); codegen returns unit; no derivative/limit LaTeX nodes; interpreter-mode tests verify load only | 15.blocks (3 defs), 35.semantics (resolve + safety), 50.mir (lowering), math_repr.spl, VSCode math/ (4 files), nvim math.lua, 6 spec files (28+ tests) | Math blocks (`m{}`, `loss{}`, `nograd{}`) -- inline DSL with `^` power, `'` transpose, implicit multiplication, broadcast operators, and `@` matrix multiply; renders to LaTeX and Unicode in VSCode and Neovim; `loss{}`/`nograd{}` parse and evaluate but autograd runtime wiring is not yet connected |
...
1. **Math blocks**: Syntax, parsing, rendering, and editor tooling are solid. Autograd semantics (`loss{}`/`nograd{}`) are parse-only stubs -- qualify with "autograd not connected."
