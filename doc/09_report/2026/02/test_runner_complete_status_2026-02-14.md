...
```
test/01_unit/         → Native (fast)
test/02_integration/  → Safe (standard)
test/03_system/       → Safe (slow)
test/03_system/qemu/         → Container (intensive)
...
docker run --rm \
  ...
  --network=none \
  -v $(pwd):/workspace:rw \
  simple-test-isolation:latest test test/01_unit/spec.spl
```
...
```simple
# Path-based
discover_test_files("test/01_unit/", options)  # Only unit tests
...
```simple
# In test file
# @DependsOn test/01_unit/database_spec.spl
...
2. `doc/09_report/parallel_execution_design_2026-02-14.md` (design doc)
...
1. `test/02_integration/parallel_execution_spec.spl` (parallel mode)
2. `test/02_integration/container_tracking_spec.spl` (container cleanup)
3. `test/02_integration/metadata_override_spec.spl` (per-test limits)
4. `test/02_integration/signal_handling_spec.spl` (Ctrl+C cleanup)

### Manual Test Scenarios:
```bash
# Test 1: Parallel execution with limits
bin/simple test --parallel --max-workers=4 --self-protect test/01_unit/

# Test 2: Container tracking
bin/simple test --execution-mode=container-seq --container-track test/02_integration/

# Test 3: Per-test overrides
# Create test with: # @Timeout 600, # @Memory 2048
bin/simple test test/02_integration/heavy_spec.spl
