...
# Run in containers
bin/simple test --container-seq test/02_integration/
```
...
```
test/01_unit/app/test_runner_new/
...
```bash
$ bin/simple test test/01_unit/app/test_runner_new/process_tracker_spec.spl
PASS (8 tests, 8ms)

$ bin/simple test test/01_unit/app/test_runner_new/checkpoint_spec.spl
PASS (11 tests, 6ms)
```
...
```bash
$ bin/simple test --self-protect --cpu-limit=50 test/01_unit/
# Result: Monitoring works, triggers shutdown when CPU > 50%
```

**Test 2: Checkpoint/Resume**
```bash
$ bin/simple test --self-protect test/01_unit/ &
...
```bash
$ bin/simple test --container-seq test/01_unit/std/
...
bin/simple test --parallel --max-workers=4 test/01_unit/
```
...
**Testing:**
- Unit tests: `test/01_unit/app/test_runner_new/`
