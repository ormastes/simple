...
Create `test/01_unit/app/io/process_limits_enforcement_spec.spl`:
...
- `test/01_unit/app/io/process_limits_enforcement_spec.spl` - Tests (NEW, ~100 lines)
...
Add to `test/01_unit/app/test_runner_new/test_runner_execute_spec.spl`:
...
- `test/01_unit/app/test_runner_new/test_runner_execute_spec.spl` - Tests (~60 lines)
...
1. `doc/07_guide/container_testing.md` - Full guide (11KB)
2. `doc/07_guide/resource_limits.md` - Resource profiles (14KB)
3. `doc/07_guide/README_CONTAINER_TESTING.md` - Index (11KB)
...
**Testing:**
- Container backend: `test/01_unit/app/test_runner_new/container_backend_spec.spl`
- All tests pass in container environment

**See:** `doc/09_report/container_testing_integration_2026-02-14.md`
...
1. `test/01_unit/std/process_monitor_spec.spl` - 6 tests
2. `test/01_unit/std/resource_tracker_spec.spl` - 11 tests
3. `test/01_unit/app/test_runner_new/test_runner_resources_spec.spl` - 10 tests
...
**See:** `doc/09_report/phase3_resource_monitoring_implementation_2026-02-14.md`
...
test_config {
    ...
    execution_rules [
        { pattern "test/02_integration/qemu/**" mode "container" profile "intensive" }
        { pattern "test/02_integration/baremetal/**" mode "container" profile "intensive" }
        { pattern "test/lib/database/**" mode "process" profile "slow" }
        { pattern "test/02_integration/**" mode "safe" profile "standard" }
        { pattern "test/01_unit/**" mode "native" profile "fast" }
    ]
...
Create `test/01_unit/app/test_runner_new/execution_strategy_spec.spl`:
...
describe "execution_strategy":
    it "categorizes QEMU tests":
        val cat = categorize_test_file("test/02_integration/qemu/boot_spec.spl")
        expect(cat).to_equal("qemu")

    it "selects container mode for QEMU":
        val opts = TestOptions(mode: "")
        val strategy = strategy_auto_select("test/02_integration/qemu/boot_spec.spl", opts)
    ...
    it "selects native mode for unit tests":
        val opts = TestOptions(mode: "")
        val strategy = strategy_auto_select("test/01_unit/std/string_spec.spl", opts)
...
- `test/01_unit/app/test_runner_new/execution_strategy_spec.spl` - Tests (NEW, ~120 lines)
...
Create `test/01_unit/app/test_runner_new/sequential_container_spec.spl`:
...
describe "sequential_container":
    it "runs test in isolated container":
        ...
        val (result, container_id) = sequential_container_run_test(
            "test/01_unit/std/string_spec.spl",
            config
        )
    ...
    slow_it "runs multiple tests sequentially":
        ...
        val test_files = [
            "test/01_unit/std/string_spec.spl",
            "test/01_unit/std/array_spec.spl"
        ]
...
- `test/01_unit/app/test_runner_new/sequential_container_spec.spl` - Tests (NEW, ~80 lines)
...
test_config {
    ...
    critical_tests [
        "test/02_integration/qemu/**"
        "test/02_integration/baremetal/**"
        "test/compiler/bootstrap/**"
        "test/03_system/self_hosting/**"
    ]
}
```
