...
1. **test/01_unit/compiler/backend/native_ffi_spec.spl** (15 slow_it)
...
2. **test/01_unit/compiler/loader/jit_instantiator_spec.spl** (3 slow_it)
...
3. **test/01_unit/compiler/parser_attribute_spec.spl** (0 slow_it, but has `@slow` comment)
   - **Issue:** Runtime hangs on load
   - **Root Cause:** Unknown module loading issue
   - **Expected time after fix:** <100ms (all trivial tests)

4. **test/01_unit/compiler/codegen/static_method_spec.spl** (3 slow_it)
   - **Issue:** Runtime hangs on load
...
5. **test/01_unit/app/mcp/failure_analysis_spec.spl** (0 slow_it, has `# @slow` comment)
...
6. **test/01_unit/app/test_analysis_spec.spl** (0 slow_it, has `# @slow` comment)
   - **Issue:** Import hangs (2+ min before kill)
   - **Root Cause:** Dict<text, i64> runtime bug blocks imports
   - **Expected time after fix:** <1 second (pure logic tests)

### Stdlib Tests (Loader Issues)
7. **test/01_unit/std/resource_limited_spec.spl** (0 slow_it)
...
8. **test/01_unit/std/test_meta_spec.spl** (0 slow_it)
...
9. **test/01_unit/app/test_runner/benchmark_spec.spl**
...
4. **Move 9 stub files** to `doc/05_design/*.pending.spl`
