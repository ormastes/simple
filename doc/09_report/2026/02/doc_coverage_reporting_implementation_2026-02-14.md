...
2. **Analysis Module** (`app.doc_coverage.analysis.sdoctest_coverage`)
   - `load_sdoctest_blocks()` - Load from README.md and doc/07_guide/*.md
...
1. **JSON Exporter Tests** (`test/01_unit/app/doc_coverage/json_exporter_spec.spl`)
...
2. **CSV Exporter Tests** (`test/01_unit/app/doc_coverage/csv_exporter_spec.spl`)
   - Test CSV escaping (commas, quotes, newlines)
   - Test tag pipe-separation
   - Test header row format
   - Test empty items array

3. **Markdown Generator Tests** (`test/01_unit/app/doc_coverage/markdown_generator_spec.spl`)
   - Test status emoji selection (≥90%, 60-89%, <60%)
   - Test scope grouping (src/lib, src/core, etc.)
   - Test sorting by missing_docs
   - Test table formatting

4. **Terminal Renderer Tests** (`test/01_unit/app/doc_coverage/terminal_renderer_spec.spl`)
...
5. **CLI Integration Tests** (`test/01_unit/app/cli/doc_coverage_command_spec.spl`)
...
- Create `doc/07_guide/doc_coverage_user_guide.md`
- Add examples to README
- Design document: `doc/05_design/doc_coverage_complete_design.md`
