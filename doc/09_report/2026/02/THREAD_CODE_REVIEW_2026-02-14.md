...
> `fn identity<T>(x: T) -> T:`). Verified by `test/01_unit/compiler_core/generic_syntax_spec.spl` (30/30 tests passing).
...
**Files to modify:**
- `test/01_unit/std/thread_pool_spec.spl` (~50 lines added)
...
**Action:** Add `test/01_unit/std/thread_safe_queue_spec.spl` (30 tests)

### Integration Tests (Missing)
**Recommended:** `test/02_integration/thread_pool_async_spec.spl`
...
2. ✅ **Verify tests run** - `bin/simple test test/01_unit/std/thread_sffi_spec.spl`
3. ⚠️ **Add missing test file** - `test/01_unit/std/thread_safe_queue_spec.spl`
...
- A focused reproduction accidentally triggered recursive `simple test` spawning for:
  - `test/01_unit/os/kernel/boot/riscv_noalloc_handoff_vexriscv_spec.spl`
  - `test/01_unit/os/kernel/arch/riscv64/platform/board_memory_map_spec.spl`
  - `test/01_unit/os/kernel/arch/riscv64/platform/litex_fpga_spec.spl`
...
- Bounded repro: `timeout 60 bin/simple test test/01_unit/os/kernel/arch/riscv64/platform/litex_fpga_spec.spl --mode=interpreter --clean --timeout 15`
...
- These bounded repros passed and left no active/zombie `simple` processes:
  ...
  - `test/01_unit/lib/test_runner` with `--parallel -j 2`
- Final process check: `active_simple=0`, `zombie_simple=0`.

**Broader bounded verification:**
- Passed without process leftovers:
  - `test/01_unit/app/test_runner_coverage_spec.spl`
  - `test/01_unit/app/cli_parser_spec.spl`
- Failed normally, without process kill or process leftovers:
  - `test/01_unit/app/test_incremental_state_shared_spec.spl` (`discover_tests`, `TestDaemon` unresolved)
  - `test/01_unit/app/lifecycle_spec.spl` (`nogc_sync_mut.src.app.config` unresolved)
  - `test/01_unit/app --tag cli --parallel -j 2` (35 files completed; 15 CLI-related files failed normally)
...
- Completed without process leaks:
  ...
  - `test/02_integration/app/loader_run_function_spec.spl`
...
- The old report paths for `test/01_unit/std/thread_sffi_spec.spl`, `test/01_unit/std/thread_pool_spec.spl`, `test/01_unit/lib/process_manager_spec.spl`, and `test/01_unit/lib/process_governor_spec.spl` are stale; each failed immediately as file-not-found and was not treated as runtime evidence.
- Current equivalents completed without process kill or leaks:
  - `test/01_unit/os/kernel/wine/thread_sffi_extensions_spec.spl`: passed 19 examples.
  - `test/02_integration/lib/thread_pool_async_spec.spl`: failed normally with 4 `Option::Some(...)` assertion mismatches.
...
- Historical crash-regression probes completed without process kill or leaks:
  ...
  - `test/01_unit/browser_engine/js_integration_spec.spl`: failed normally on unresolved module `std.gc_async_mut.gpu.browser_engine.script.script_host`.
...
- `bin/simple check src/lib/common/science_math/perf_sugar.spl` passes and `test/03_system/feature/scilib/perf_sugar_spec.spl` passes.
...
- Fixed `test/01_unit/lib/hardware/soc_rtl/soc_top_64_spec.spl` to use `SOC64_TEST_DRAM_SIZE = 0x1000` for SoC fixture tests while preserving QEMU virt memory-map constant checks.
...
- Native/subprocess bounded probes completed without process leaks:
  ...
  - `test/01_unit/compiler/backend/native` failed normally in `isel_aarch64_spec.spl`
- Target-filtered runner cleanup probes passed:
  ...
  - `test/01_unit/compiler/r2_probe_fail_spec.spl` deliberate targeted parallel failure left no matching child processes.
- Final process check: `active_simple=0`, `zombie_simple=0`.
- Final crashed-run check remains unchanged: 17 May 20 zero-count crashed runs, no new crashed run.
