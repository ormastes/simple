...
#### 1. **doc/07_guide/container_testing.md** (11KB)
...
#### 2. **doc/07_guide/resource_limits.md** (14KB)
...
#### 3. **doc/07_guide/README_CONTAINER_TESTING.md** (11KB)
...
TEST_PROFILE=standard scripts/ci-test.sh test/01_unit/

# Slow tests with custom image
CONTAINER_IMAGE=simple-test:dev TEST_PROFILE=slow scripts/ci-test.sh test/03_system/
...
```bash
# Single test
scripts/local-container-test.sh quick test/01_unit/std/string_spec.spl
...
2. Quick test (native): `bin/simple test test/01_unit/std/string_spec.spl`
3. Verify isolated: `scripts/local-container-test.sh quick test/01_unit/std/string_spec.spl`
...
**Steps:**
1. Run exact CI command: `scripts/ci-test.sh test/01_unit/`
...
test:
  stage: test
  script:
    - scripts/ci-test.sh test/01_unit/
  artifacts:
    reports:
...
stage('Test') {
  steps {
    sh 'scripts/ci-test.sh test/01_unit/'
    junit 'test-results-fast.json'
  }
}
```
...
2. **Update team workflows:**
   - Share `doc/07_guide/container_testing.md` with team
