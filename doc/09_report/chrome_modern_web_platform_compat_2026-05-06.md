...
CSS custom property support now includes bounded `var(--name, fallback)` color fallback resolution for `background-color` and `background` shorthand when a `:root` custom property is missing; nested `var()`, scoped custom-property inheritance, cyclic variables, typed registration, and cascade parity are still not claimed.
