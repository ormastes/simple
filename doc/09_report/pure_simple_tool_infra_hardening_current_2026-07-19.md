...
|---|---|---|---|
| Test runner | Structured-evidence and shared sdoctest config dispatch fixed; indexed-u8 red fixture added; deployment blocked | current pure-Simple source launch floods parser recovery diagnostics and does not reach the MCP spec; a live unrelated PID is also accepted as daemon readiness | stop recovery diagnostics from escaping successful module loads; use a bounded daemon ping and reject non-numeric/unresponsive locks before enqueueing |
...
| Formatter/fix | `FixApplicator` failed-write false-green fixed and directly exercised; provider regressions pass; Stage 4 integration pending | formatter `--write` still uses checked but delete-first `file_write`; current deployed runner blocks the required integration gate | route formatter through the canonical atomic writer; qualify both consumers on the admitted Stage 4 runtime |
