...
| Duplicate checker | missing-path false-green confirmed; bounded fix blocked by flags-only bypass | bare invocation returns usage/0; flags-only input falls into the hidden full-src default; implicit semantic scan is hang-prone | require a positional target for bare and flags-only shapes, keep explicit help at 0, test argument extraction without scanning, then qualify bounded fixtures on admitted Stage 4 |
...
| `check` | canonical warn probe unblocked; CLI false-green remains | both workers stop at parse/policy; HM finds the negative mismatch but also rejects valid expression-bodied non-Unit functions, so the bounded fatal experiment was removed | fix the HM positive false-positive, prove paired reject/positive exits, then make HM fatal only in canonical Check mode and route both workers through its internal owner |
