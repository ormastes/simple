...
| Test daemon | timeout/orphan bug confirmed; ownership/lifecycle evidence pending | light client deadline is not sent to the daemon, whose fixed 600s child blocks later requests; production CLI and session client use inconsistent owners/protocols | forward one bounded request deadline to the daemon child, then reconcile CLI owner and add start/status/run/stop plus stale-state recovery evidence |
