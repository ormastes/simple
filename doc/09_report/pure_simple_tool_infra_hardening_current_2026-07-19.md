...
|---|---|---|---|
| Test runner | Structured evidence, indexed-u8 failure gate, opt-in-only SPipe docgen, responsive daemon routing, and fail-closed atomic request publication fixed in source; seed/source MCP stdio spec exits 0 | self-hosted Stage 4 qualification pending; source launch emits excessive parser recovery diagnostics | admit an incremental Stage 4, then suppress recovery noise and prove stale-daemon direct fallback |
...
| Duplicate checker | missing-target, malformed-option, CLI/config invalid mode/format false-greens, and valid CLI-over-config precedence fixed in source; focused repair probes pass | admitted Stage 4 runtime and authoritative scan qualification remain pending | keep explicit help at 0 and qualify the bounded clean/found/invalid/config fixtures on admitted Stage 4 |
