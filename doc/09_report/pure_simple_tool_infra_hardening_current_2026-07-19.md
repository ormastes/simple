...
| Native build/cache | focused Rust source checks passed; rebuilt provider pending | timeout discarded safe partial work; cache key omitted effective backend/optimization; method-owner changes retained stale dependents | atomically retain only dependency-independent `no_mangle` objects; key by backend/opt; use isolated shards after owner changes; cold/fail/retry equivalence |
