...
- memory_model_drf_formal_scope: multithread executions with no conflicts, read-only same-location sharing, or with all conflicting operations ordered by happens-before, program-order, or synchronizes-with, are data-race free; fences are not conflicting memory operations; same-location write/write, read/write, lock acquire/release, and lock release/release pairs are formally classified as conflicts; lock acquire/acquire pairs are non-conflicting; unordered same-location write/write and read/write pairs are formally data races
- memory_model_drf_formal_evidence: src/verification/memory_model_drf/src/MemoryModelDRF.lean drf_when_no_conflicts + drf_two_reads_same_location + drf_when_conflicts_ordered + drf_when_conflicts_program_or_sync_ordered + drf_two_ops_synchronized + fence_left_no_conflict + fence_right_no_conflict + write_write_same_location_conflicts + read_write_same_location_conflicts + write_read_same_location_conflicts + lock_acquire_release_same_location_conflicts + lock_release_acquire_same_location_conflicts + lock_release_release_same_location_conflicts + lock_acquire_acquire_same_location_no_conflict + two_unordered_writes_same_location_race + two_unordered_read_write_same_location_race + two_unordered_write_read_same_location_race; `cd src/verification/memory_model_drf && lake build`
