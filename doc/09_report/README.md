...
| Report | Status | Notes |
|--------|--------|-------|
| [linux_renderdoc_simpleos_hardening_evidence_current_2026-07-02.md](linux_renderdoc_simpleos_hardening_evidence_current_2026-07-02.md) | Pass | Current Linux RenderDoc/Vulkan, Engine2D Vulkan readback, CPU SIMD Engine2D acceleration, QEMU GPU capture access, SimpleOS QEMU/WM, and LLVM port hardening evidence. Points at the passing Linux Vulkan render-log compare with Simple/Chrome/Electron `RDOC` artifacts, zero-mismatch Vulkan/CPU-SIMD Engine2D reports, live x86_64 and RV64 QMP screendump/readback parity, the current `10/10` SimpleOS hardening matrix including RV64 display-smoke QMP, and the LLVM-to-SimpleOS cross-toolchain report with x86_64 Clang/LLD smokes and aarch64 stage-2 cross build. |
