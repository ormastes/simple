...
| Windows native MDI | Host-gated contract | `test/03_system/gui/windows_native_mdi_evidence_spec.spl` | On Windows, the spec requires Win32 backend, live window title, screenshot path, proof env, drag movement, focus cycle, titlebar/body controls, titlebar CSS source, rendered titlebar CSS, and restore from minimize. On non-Windows, it requires explicit `requires-windows` skip. | No current checked-in Windows pass report with screenshot/proof artifacts was found in `doc/09_report`. |
...
On non-Windows it must skip explicitly with `requires-windows`. A current
Windows pass report with screenshot/proof artifacts is still missing.

## Next Evidence Work

1. Generate a fresh macOS live-window report that satisfies the current GUI SMF
   artifact contract.
2. Generate a Windows native MDI pass report with screenshot and proof env
   artifacts.
