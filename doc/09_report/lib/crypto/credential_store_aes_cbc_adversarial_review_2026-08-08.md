...
| F3 | `store.spl:196` | `rt_file_write_text(path, key_hex)` writes the master AES key with **no `chmod`**, on **both** engines. Interpreter: `interpreter_extern/file_io.rs:217` `fs::write(&path, &content)` → `0666 & ~umask`. Native: `runtime_native.c:8425` `open(path, O_WRONLY \| O_CREAT, 0644)` → `0644 & ~umask`. Neither is `0600`. Under this machine's `umask 0002` the key file lands **0664 — group-readable**; any local user in the group reads every stored credential. (Positive evidence from reading both implementations, not an absence-of-grep.) Secondary: the C path opens `O_CREAT` without `O_TRUNC`, so rewriting a shorter key over a longer file can leave stale trailing bytes. | MEDIUM | No — pre-existing | High |
...
| F5 | `store.spl:342-347` | **No upgrade-on-read/write.** A v1 record decrypts correctly forever but is never re-encrypted to v2, so legacy secrets linger under CTR-with-plaintext-derived-IV (the original XOR-leak defect) until some caller happens to rewrite them. No enumeration/migration routine exists. Silent-destruction check: swept every `credential_resolve`/`credential_decrypt`/`credential_encrypt` call site outside the module (`terminal/__init__.spl:43`, `ssh_terminal.spl:94,100`, `telnet_terminal.spl:99`) — all are **read-only auth consumers**; none does read-modify-write of a config file, so the `""`-on-failure return cannot be persisted back over a credential. The "no path destroys a credential file" claim holds repo-wide, not just inside `store.spl`. | LOW-MEDIUM | Yes (the migration design) | High |
...
| F6b | `store.spl:146-176` | **`credential_key_generate` never completes on the interpreter path.** Measured twice: it first tripped `kill_simple_monitor`'s 60s CPU guard, then — with the guard disabled (`SIMPLE_TIMEOUT_SECONDS=0`) — ran to a **900s wall-clock `timeout` and was Terminated with no key file created at all**. Since this is the only way to create the key that `credential_encrypt`/`credential_decrypt` require, the credential store cannot be bootstrapped end-to-end on this engine; the store tests in this review only ran because a key file was hand-written to disk as a workaround. A deliberately slow KDF is correct design, but >15 min with no completion is a functional break, not cost tuning — `BCRYPT_COST = 10` is 1024 iterations × 64 Blowfish rounds × 3 blocks interpreted. Suspect the interpreted Blowfish inner loop rather than the cost constant. Also the reason F1's zero-tail rests on structure (24 pushes into 32 slots with an `else: push(0)` arm) rather than on reading a generated file. | MEDIUM (functional) | No | High (two independent runs) |
...
**Key-material leakage: CLEAN.** Swept every `print`/log/`eprint`/`to_text(` site in
`store.spl`. The only `print` is inside a docstring example (`:138`); `key`,
`key_hex`, and the expanded schedule are never printed, logged, embedded in an
error string, or exposed through a `to_text`/debug path. `bytes_to_text` at
`:347`/`:353` returns decrypted plaintext to the caller, which is the function's
contract, not a leak. No key is returned by any exported symbol.
