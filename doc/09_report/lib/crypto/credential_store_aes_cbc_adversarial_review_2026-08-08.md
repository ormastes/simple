...
| F6b | `store.spl:146-176` | `credential_key_generate` at `BCRYPT_COST = 10` does not complete within 60s under the interpreter — it tripped `kill_simple_monitor`'s CPU guard, and still had not produced a key file after 8+ minutes with the guard disabled. F1's zero-tail is therefore established structurally (24 pushes into 32 slots with an `else: push(0)` arm), not by reading a generated file. The cost is not a crypto defect (a slow KDF is the point) but it makes key generation effectively unusable on the interpreter path and blocks empirical key-file testing. | LOW | No | High (observed twice) |
