...
| SimpleOS-WM × QEMU | native-build+boot | **FAIL (build-timeout; parser regressions fixed)** | `cli` soft-keyword (`d5a6312d`) and pipe-enum (`5b04d2f5`) parser regressions FIXED; harness now reaches the kernel cranelift build, which exceeds every budget tried (2700s run 5, 7200s run 7) with `--cache-dir` writing NOTHING (0-byte cache after 2h — reruns never warm) and zero progress output under `--log off`. Run 7 2026-07-26: `fail reason=wm-simple-web-build-timeout`. Run 9 (2026-07-26, linked worktree, first OBSERVABLE build via `SIMPLE_COMPILER_TRACE=1`) burned the full 10800s still in phase2 PARSE: measured ~100 chars/sec, degrading with heap growth (48 files in 2.5h; 27KB entry file alone took 436s) — root defect is a native-build-lane parser perf collapse (`native_build_parser_100cps_regression_2026-07-26.md`), NOT budget size. Cell blocked on that fix. Also fixed en route: harness seed-detection false positive (`62e79e2d`), admission linked-worktree requirement discovered, cache + observability fixes landed (`a90c1680`) |
