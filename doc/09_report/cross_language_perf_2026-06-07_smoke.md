...
| Simple (native)        |     873.3 KB |     876.8 KB |         none |              |
...
|------------------------|--------------|--------------|--------------|--------------|
| Simple (interpreter)   |       25.135 |    interpret |              |              |
| Simple (SMF loader)    |       23.089 |          smf |              |              |
| Simple (native)        |        4.793 |       native |              |              |
| C (gcc -O2)            |        4.493 |       native |              |              |
| Go (compiled)          |       55.797 |       native |              |              |
| Python                 |       31.370 |    interpret |              |              |
| Bun                    |      103.882 |          JIT |              |              |
| Java                   |      135.200 |    JIT (JVM) |              |              |
| Erlang                 |     1457.125 |      BEAM VM |              |              |

## Warm Throughput — fib(35) in process (10 warmup + 1 measured)

| Language               |     Avg (ms) |                                    Notes |
|------------------------|--------------|------------------------------------------|
| Simple (interpreter)   |       95.758 | tree-walk (outer-process, no in-proc timing) |
| Simple (SMF loader)    |       93.960 | bytecode (outer-process, no in-proc timing) |
| Simple (native)        |       62.942 |           AOT via Cranelift (in-process) |
| C (gcc -O2)            |       14.379 |                             baseline AOT |
| Go                     |       53.227 |                                  SSA AOT |
| Python                 |     1602.849 |                         CPython bytecode |
| Bun                    |       66.285 |        JavaScriptCore JIT (steady-state) |
| Java                   |       51.537 |            HotSpot C2 JIT (steady-state) |
| Erlang                 |      125.287 |                    BEAM (single-process) |
...
| Simple (native)        |      101.898 |        thread_spawn fork-join OS threads |
| Simple cooperative green (interp) |       54.628 | cooperative_green_spawn_value cooperative queue |
| Simple cooperative green (SMF) |         fail | cooperative_green_spawn_value cooperative queue (SMF mutable-global runtime blocker) |
| Simple cooperative green (native) |       30.356 | cooperative_green_spawn_value cooperative queue |
| Simple multicore green (SMF) |         fail | multicore_green runtime pool candidate (SMF runtime-pool closure lookup blocker) |
| Simple multicore green (native) |       99.622 | multicore_green runtime pool candidate (pool_used=100/100, parallelism=64/64) |
| C (pthreads)           |       12.742 |                               OS threads |
| Go                     |        9.687 |           goroutines + chan result (M:N) |
| Python                 |     3108.094 |                          threading (GIL) |
| Bun                    |      348.038 |                           worker_threads |
| Java                   |      163.621 |                               ThreadPool |
| Erlang                 |     1385.228 |                    lightweight processes |
...
| Simple (native)        |       58.576 |               OS-thread fork-join fanout |
| Simple cooperative green (interp) |       33.419 |                 cooperative queue fanout |
| Simple cooperative green (SMF) |         fail | cooperative queue fanout (SMF mutable-global runtime blocker) |
| Simple cooperative green (native) |        6.864 |                 cooperative queue fanout |
| Simple multicore green (SMF) |         fail | multicore_green runtime pool fanout (SMF runtime-pool closure lookup blocker) |
| Simple multicore green (native) |       15.631 | multicore_green runtime pool fanout (pool_used=1000/1000, parallelism=64/64) |
| C (pthreads)           |       56.315 |              one OS thread per tiny task |
| Go                     |        7.479 |        goroutine per tiny task + channel |
| Python                 |      174.944 |            threading per tiny task (GIL) |
| Bun                    |         fail |              worker_thread per tiny task |
| Java                   |      101.502 |   cached ThreadPool future per tiny task |
| Erlang                 |     1353.120 |               BEAM process per tiny task |

## Simple vs Go vs C Large Fanout Stress — spawn 512 tiny workers (1 runs avg)

| Language               |     Avg (ms) |                        Concurrency model |
|------------------------|--------------|------------------------------------------|
| C (pthreads)           |       36.726 |                  pthread per stress task |
| Simple multicore green (native) |       11.062 | multicore_green stress fanout (pool_used=512/512, parallelism=64/64) |
| Go                     |        5.739 |          goroutine per stress task (M:N) |

## Parallel Artifact Footprint

| Language               |         Binary |     Per-thread |                Notes |
|------------------------|----------------|----------------|----------------------|
| Simple (native)        |       937.0 KB | OS default (8MB) |        Cranelift AOT |
| Simple (SMF)           |         7.3 KB | OS default (8MB) |    + runtime 42.9 MB |
| Simple cooperative green (native) |       892.5 KB | current OS thread |    cooperative queue |
| Simple cooperative green (SMF) |        12.9 KB | current OS thread |    + runtime 42.9 MB |
| Simple multicore green (native) |       896.5 KB |   runtime pool |      multicore_green |
...
|------------------------|----------------|----------------|----------------------|
| Simple (native)        |         3.0 MB |         2.2 MB |                 ~7KB |
| Simple multicore green |         2.5 MB |         2.2 MB |                 ~2KB |
| C (pthreads)           |         2.0 MB |         2.0 MB |                 ~0KB |
| Go                     |         2.0 MB |         2.0 MB |                 ~0KB |
| Python                 |        10.2 MB |        10.2 MB |                 ~0KB |
| Java                   |        47.5 MB |        43.0 MB |                ~46KB |
