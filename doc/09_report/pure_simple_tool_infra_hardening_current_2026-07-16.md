...
| Test runner | SOURCE REPAIRED / DEPLOY BLOCKED | The retained release binary crashes in stale two-arg `rt_env_set`; current ABI is correct. Rust native/interpreter wait owners now return `-2` while live, retain the child, reap after kill, and inherit async output so unread pipes cannot hang chatty children. Bootstrap smoke has TERM-to-KILL grace; explicit Rust-runner opt-in is temporary bounded evidence only. Driver verification reached its three-attempt cap after exposing and removing a stale C8 debug hook | Rebuild/deploy current pure CLI, run the same green/red/empty fixture through the pure runner, then remove temporary Rust opt-in | P0 |
