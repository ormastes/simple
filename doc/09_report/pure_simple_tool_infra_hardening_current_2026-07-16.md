...
| Lint | SOURCE GUARDED | Production CLI delegates to the canonical file linter; dead duplicate paths are deleted; hot-loop BYTE names are file-scoped; MCP performance fails closed without an owner; the UI isolation ratchet has zero new violations after classifying 22 exact restricted owners; the hot-loop gate reports 34 new findings after bounded-scan classification, native row gathers, font-tail scan reuse, and canonical clipped-rectangle raster reuse | Add one repository-scanner owner for the four MCP rules, repair classified violations, then run focused fixtures | P1 |
...
3. The UI isolation ratchet has zero new violations after 22 exact bare-metal,
   ...
   gate still exposes 34 new findings. Focused lint no longer fails on unrelated
