...
3. The UI isolation ratchet has zero new violations after 22 exact bare-metal,
   ...
   gate still exposes 30 new findings. Focused lint no longer fails on unrelated
...
- **Bare-metal isolation ownership:** 19 exact architecture entry/probe paths
  are documented as restricted hardware/QEMU/runtime-ABI owners and added to
  the ratchet individually. No directory wildcard or hosted consumer is
  exempted; the gate therefore falls from 25 to 6 new findings while future
  architecture files still fail closed.
- **Restricted target/facade ownership:** the contract-pinned freestanding argv
  facade, the SimpleOS guest target pipeline, and the documented WM raw-buffer
  workaround are ratcheted as three exact paths. This preserves entry-closure,
  guest syscall/CPL, Stage3 aggregate, and QEMU performance contracts while
  leaving three migratable consumers visible; new findings fall from 6 to 3.
- **SimpleOS focused-tool entry:** the guest CLI entry now reuses the pinned
  minimal argv facade and exact environment facade. A source contract forbids
  raw externs in the consumer while preserving the restricted target pipeline;
  new isolation findings fall from 3 to 2.
- **SimpleOS image patcher I/O:** the hosted patch utility now uses the narrow
  binary-file SFFI facade and the real boolean write ABI. Failed writes return
  nonzero, and its caller removes stale output and propagates failure; the UI
  gate falls from 2 to 1 new finding.
- **Lightweight test fault policy:** the test entry now uses a pure-Simple SFFI
  leaf whose four wrappers directly preserve the runtime setters, seconds unit,
  and depth-before-enable ordering. Source contracts reject raw/no-op/broad
  facade regressions; the UI isolation ratchet reaches zero new findings.
- **Compositor readiness count:** the process-owned surface count is called only
  by desktop materialization/readiness logging, so its bounded live-table scan
  is explicitly classified instead of adding cache invalidation state. The
  hot-loop gate falls from 43 to 42 new findings.
- **Software SIMD row gathers:** contiguous source and framebuffer spans now use
...
- **GPU fill materialization:** the solid-command framebuffer path now reuses
  the renderer's canonical clipped-rectangle raster owner instead of duplicating
  its clipping and nested pixel loops. The hot-loop gate falls from 36 to 34 new
  findings without coupling an independent parity oracle.
- **Iframe child blit:** parent-bound clipping is computed once and the visible
  child rectangle is copied through the canonical native blit owner. Opaque
  replacement, source offsets, and strides are preserved while the hot-loop
  gate falls from 34 to 32 new findings.
- **Font-number scan:** size and family parsing now share one numeric-component
  scanner. Numeric weights, fractional sizes, line height, and quoted-family
  behavior remain pinned while the hot-loop gate falls from 32 to 30 new
  findings.
...
- LLVM imported function values now use the existing exact arity index;
  focused positive/zero-arity/unknown-negative verifier coverage PASS. One
...
- Implicit receiver discovery now sees `self` through `as` casts. Focused
  ...
  import discovery.
- Generic identifier lookahead now accepts an enabled `{` continuation and
  reuses the existing struct initializer without weakening comparison or
  control-flow parsing. Focused AST coverage PASS; one bounded bootstrap
  removed both `slice` diagnostics, reducing undeclared globals from 14 to 12.
  It exited 2 normally with no timeout, OOM, crash, hang, or orphan.
- `Tensor.ndim` now takes `self` and returns the actual `_shape` rank instead
  of treating generic type parameter `N` as a runtime value. The exact method
  contract is pinned; one bounded bootstrap removed both `N` diagnostics,
  reducing undeclared globals from 12 to 10. It exited 2 normally with no
