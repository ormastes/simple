...
| Browser-shaped WebGPU device queue | `src/lib/nogc_sync_mut/js/engine/interpreter_native.spl`; `webgpu_js_wasm_simple_spec.spl`; `doc/08_tracking/bug/browser_webgpu_queue_wasm_memory_sspec_promise_2026-06-14.md` | Runtime and SSpec prove bounded software `adapter.requestDevice()`, `device.createBuffer({size, usage})`, and `device.queue.writeBuffer(buffer, offset, Uint8Array, sourceOffset, size)` mutating observable buffer bytes/counts/checksums; WASM-memory-backed queue upload still needs stable SSpec evidence |
