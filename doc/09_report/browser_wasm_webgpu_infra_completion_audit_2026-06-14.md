...
| Simple browser WASM beside JavaScript | `test/01_unit/lib/common/web/browser_session_wasm_script_spec.spl`; `doc/06_spec/test/01_unit/lib/common/web/browser_session_wasm_script_spec.md` | Proven for inline/external `application/wasm` records and ordered JS continuation |
...
| Browser-shaped WebGPU device queue | `src/lib/nogc_sync_mut/js/engine/interpreter_native.spl`; `webgpu_js_wasm_simple_spec.spl`; `doc/08_tracking/bug/browser_webgpu_queue_wasm_memory_sspec_promise_2026-06-14.md` | Runtime has bounded software `adapter.requestDevice()` and `device.queue.writeBuffer(...)` recorder; executable proof currently covers `requestDevice` shape, while WASM-memory-backed queue upload needs stable SSpec evidence |
