...
- Optional language inventory is explicit: Python participates when available; Bun, Java, and Erlang rows are emitted when their toolchains exist. C and Go remain the required native baselines for release-blocking M:N comparisons.
...
- Supports `PROFILE_DOCKER_ISOLATION=1` to re-exec the same profile script in a Docker container with `--network=none`, a memory limit, a CPU limit, and the current UID/GID. Use this mode for crash-prone native and SMF profile runs. Build `simple-cross-language-perf:latest` with `docker build -t simple-cross-language-perf:latest -f tools/docker/Dockerfile.cross-language-perf tools/docker` for contract-gated C/Go comparison evidence. Unless `PROFILE_DOCKER_SIMPLE_BINARY` is explicitly set, the container uses the canonical `require-self-hosted.shs` identity gate rejects Rust bootstrap seeds; unavailable or stale release paths are untestable, never fallback evidence.
- Docker CPU quota is containment metadata, not the scheduler-width proof; release-blocking M:N comparisons use the recorded `CPU_WORKERS == GOMAXPROCS` scheduler-width contract.
...
- Host: `db41af7ab8b3`
- Shell: `unknown`
- Simple binary: `build/cargo-target-thread-fix/debug/simple`
...
|------------------------|--------------|--------------|--------------|--------------|
| Simple (interpreter)   |       38.0 B |      175.0 B |     438.6 MB |              |
| Simple (SMF loader)    |       3.0 KB |       3.7 KB |     438.6 MB |              |
...
|------------------------|--------------|--------------|--------------|--------------|
| Simple (interpreter)   |       53.206 |    interpret |              |              |
| Simple (SMF loader)    |       38.070 |          smf |              |              |
| Simple (native)        |        4.721 |       native |              |              |
| C (gcc -O2)            |        4.440 |       native |              |              |
| Go (compiled)          |       66.875 |       native |              |              |
| Python                 |       18.347 |    interpret |              |              |

## Warm Throughput — fib(35) in process (10 warmup + 1 measured)

| Language               |     Avg (ms) |                                    Notes |
|------------------------|--------------|------------------------------------------|
| Simple (interpreter)   |      130.436 | tree-walk (outer-process, no in-proc timing) |
| Simple (SMF loader)    |      110.324 | bytecode (outer-process, no in-proc timing) |
| Simple (native)        |       62.431 |           AOT via Cranelift (in-process) |
| C (gcc -O2)            |       14.362 |                             baseline AOT |
| Go                     |       58.230 |                                  SSA AOT |
| Python                 |     1903.527 |                         CPython bytecode |
...
| Simple (SMF)           |         7.0 KB | OS default (8MB) |   + runtime 438.6 MB |
| Simple cooperative green (native) |         6.3 MB | current OS thread |    cooperative queue |
| Simple cooperative green (SMF) |         7.3 KB | current OS thread |   + runtime 438.6 MB |
| Simple multicore green (native) |         6.3 MB |   runtime pool |      multicore_green |
| Simple multicore green (SMF) |        10.4 KB |   runtime pool |   + runtime 438.6 MB |
...
|------------------------|----------------|----------------|----------------------|
| Simple (native)        |         6.5 MB |         5.5 MB |                ~10KB |
| Simple multicore green |         6.2 MB |         5.5 MB |                 ~7KB |
| C (pthreads)           |         2.0 MB |         1.5 MB |                 ~5KB |
| Go                     |         2.8 MB |         2.5 MB |                 ~2KB |
