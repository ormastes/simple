...
**Report path:** `doc/09_report/cross_language_perf_2026-06-11_refresh.md`
...
|------------------------|--------------|--------------|--------------|--------------|
| Simple (interpreter)   |       24.637 |    interpret |              |              |
| Simple (SMF loader)    |       27.607 |          smf |              |              |
| Simple (native)        |        5.152 |       native |              |              |
| C (gcc -O2)            |        4.575 |       native |              |              |
| Go (compiled)          |       58.429 |       native |              |              |
| Python                 |       15.553 |    interpret |              |              |

## Warm Throughput — fib(35) in process (10 warmup + 1 measured)

| Language               |     Avg (ms) |                                    Notes |
|------------------------|--------------|------------------------------------------|
| Simple (interpreter)   |       93.607 | tree-walk (outer-process, no in-proc timing) |
| Simple (SMF loader)    |       81.642 | bytecode (outer-process, no in-proc timing) |
| Simple (native)        |       57.474 |           AOT via Cranelift (in-process) |
| C (gcc -O2)            |       13.374 |                             baseline AOT |
| Go                     |       51.766 |                                  SSA AOT |
| Python                 |     1528.454 |                         CPython bytecode |
...
| Simple (SMF loader)    |      163.416 |    std thread_spawn fork-join (bytecode) |
| Simple (native)        |         fail |        thread_spawn fork-join OS threads |
| Simple cooperative green (interp) |       56.481 | cooperative_green_spawn cooperative queue |
...
| Simple multicore green (SMF) |      187.259 | multicore_green runtime pool candidate (pool_used=100/100, parallelism=64/64, queue_model=work_stealing) |
| Simple multicore green (native) |       98.859 | multicore_green runtime pool candidate (pool_used=100/100, parallelism=64/64, queue_model=work_stealing) |
| C (pthreads)           |       12.976 |                               OS threads |
| Go                     |       16.139 |           goroutines + chan result (M:N) |
| Python                 |     2896.154 |                          threading (GIL) |
...
|------------------------|--------------|------------------------------------------|
| Simple (interpreter)   |          n/a |          extern thread FFI not supported |
| Simple (SMF loader)    |      100.099 |                  std thread_spawn fanout |
| Simple (native)        |         fail | OS-thread fork-join fanout (checksum mismatch) |
| Simple cooperative green (interp) |       41.648 |                 cooperative queue fanout |
...
| Simple multicore green (SMF) |       36.135 | multicore_green runtime pool fanout (pool_used=1000/1000, parallelism=64/64, queue_model=work_stealing) |
| Simple multicore green (native) |       15.969 | multicore_green runtime pool fanout (pool_used=1000/1000, parallelism=64/64, queue_model=work_stealing) |
| C (pthreads)           |       67.535 |              one OS thread per tiny task |
| Go                     |        6.856 |        goroutine per tiny task + channel |
| Python                 |      169.960 |            threading per tiny task (GIL) |

## Simple vs Go vs C Large Fanout Stress — spawn 512 tiny workers (1 runs avg)

| Language               |     Avg (ms) |                        Concurrency model |
|------------------------|--------------|------------------------------------------|
| C (pthreads)           |       32.358 |                  pthread per stress task |
| Simple multicore green (native) |       14.055 | multicore_green stress fanout (pool_used=512/512, parallelism=64/64, queue_model=work_stealing) |
| Go                     |        5.970 |          goroutine per stress task (M:N) |
...
| Simple multicore green |         3.5 MB |         2.8 MB |                 ~7KB |
