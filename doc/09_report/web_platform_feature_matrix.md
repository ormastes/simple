...
| CSS attribute selectors | partial | `src/lib/gc_async_mut/gpu/browser_engine/style_block.spl`; `src/lib/gc_async_mut/gpu/browser_engine/browser_renderer.spl`; `test/unit/lib/gc_async_mut/gpu/browser_engine/browser_renderer_spec.spl`; `test/web_platform/css/selector_color_subset_spec.spl` | Covers `[attr]` presence, `[attr=value]` exact matching, bounded `[attr^=value]`, `[attr*=value]`, `[attr~=value]`, `[attr|=value]` operators, bounded ASCII case-insensitive `[attr=value i]`, and explicit case-sensitive `[attr=value s]` syntax, including tag-qualified exact/prefix/dash selectors in the fallback renderer. Namespaces, escaping, quoted whitespace edge cases, non-ASCII folding, and full selector specificity parity are not claimed. |
...
| WPT selector/color subset | partial | `tools/wpt_to_sspec/README.md`; `doc/03_plan/sys_test/wpt_subset_migration.md`; `test/web_platform/css/selector_color_subset_spec.spl` | Adds the first executable 34-case WPT-shaped subset for deterministic selector and color/background pixel behavior. This is a curated subset, not a complete WPT importer or conformance claim. |
