...
| CSS attribute selectors | partial | `src/lib/gc_async_mut/gpu/browser_engine/style_block.spl`; `src/lib/gc_async_mut/gpu/browser_engine/browser_renderer.spl`; `test/unit/lib/gc_async_mut/gpu/browser_engine/browser_renderer_spec.spl`; `test/web_platform/css/selector_color_subset_spec.spl` | Covers basic `[attr]` presence and `[attr=value]` exact matching, including tag-qualified exact-value selectors in the fallback renderer. Substring operators, whitespace-token operators, dash-match, namespaces, case-sensitivity flags, escaping, and full selector specificity parity are not claimed. |
...
| WPT selector/color subset | partial | `tools/wpt_to_sspec/README.md`; `doc/03_plan/sys_test/wpt_subset_migration.md`; `test/web_platform/css/selector_color_subset_spec.spl` | Adds the first executable 28-case WPT-shaped subset for deterministic selector and color/background pixel behavior. This is a curated subset, not a complete WPT importer or conformance claim. |
