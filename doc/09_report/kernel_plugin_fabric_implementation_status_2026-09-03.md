...
| REQ-KPF-007 lifecycle safety | Pins, stale handles, quiescence/failure states and immediate-predecessor rollback exist | Focused terminal-race, request ABA, failed-candidate, pin-capacity, rollback, active-unload, retired collection, and stale-handle matrix passes 10/10 plus 3/3 mutation checks on the admitted pure-Simple runtime; cross-placement lifecycle parity and crash-loop policy remain open |
