...
- `bin/simple test test/01_unit/lib/db/accel_spec.spl`
...
- `/usr/bin/time -f "elapsed=%e rss_kib=%M" bin/simple run test/05_perf/bench/simple_db_shared_accel.spl`
...
- `test/01_unit/lib/db/accel_spec.spl` passed: `11` examples, `0` failures
- `test/05_perf/bench/simple_db_shared_accel.spl` now runs a preflight before
...
`test/05_perf/bench/simple_db_shared_accel.spl` as the before/after harness when
