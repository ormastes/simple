...
router -> response -> close, and TLS must reject missing/invalid material rather
than downgrade. Today `tls_server_accept` fails closed because GAP-TLS-3 still
blocks encrypted transport. The DB path includes an owned bounded listener, authenticated `OPEN`,
...
batch/range operations. None is credited until focused scenarios pass.
