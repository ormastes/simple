...
step("walks EMPTY->QUEUED->SUBMITTED->COMPLETED with exact depth after each step")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("does not enqueue when the decision is not accepted (lane rejected)")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("does not enqueue a host-committed event (queue not required)")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("rejects emit when the backend handle is missing")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("rejects emit when the packet exceeds the pure queue limit")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("submit on an EMPTY queue is a no-op that stays EMPTY")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("complete without a prior submit completes nothing")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("a second complete after everything is completed adds nothing")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("drain on an EMPTY queue drains nothing and reports empty")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("round-trips a Draw IR payload: hash, schema and backend handle all validated")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("rejects a corrupted payload-hash with no false receipt")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("rejects a payload that is not Draw IR v2 (wrong schema)")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("rejects a receipt whose backend handle does not match the queue")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("a GPU-forwarded coarse batch is NOT reported completed (it committed on GPU, not host)")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("a host-committed event IS reported completed via committed_on_host")
...
```simple
# @req REQ-SSPEC-INTEGRATION
step("reports failure (not silent success) when the runtime queue is unavailable")
...
- Canonical SPipe generation for source `a57163fed2a8078181bdf73949e33739777b247268014bcc793fb63e95f07132`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `a57163fed2a8078181bdf73949e33739777b247268014bcc793fb63e95f07132`.
...
source: test/02_integration/lib/gpu/host_gpu_queue_roundtrip_spec.spl
mirror: doc/06_spec/02_integration/lib/gpu/host_gpu_queue_roundtrip_spec.md (current)
findings: 6 blockers: 0
  narrative=100 structure=100 oracle=70
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/02_integration/lib/gpu/host_gpu_queue_roundtrip_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/02_integration/lib/gpu/host_gpu_queue_roundtrip_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/02_integration/lib/gpu/host_gpu_queue_roundtrip_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-30): 51 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/02_integration/lib/gpu/host_gpu_queue_roundtrip_spec.spl:118:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'walks EMPTY->QUEUED->SUBMITTED->COMPLETED with exact depth after each step' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/02_integration/lib/gpu/host_gpu_queue_roundtrip_spec.spl:167:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'does not enqueue when the decision is not accepted (lane rejected)' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/02_integration/lib/gpu/host_gpu_queue_roundtrip_spec.spl:183:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'does not enqueue a host-committed event (queue not required)' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
