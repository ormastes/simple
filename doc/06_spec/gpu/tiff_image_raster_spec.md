...
#### renders 16-bit tiled RGBA decoded TIFF pixels through the raster image path exactly
...
3. rasterizer raster draw image
   ...
   - Expected: pixel(surface, 1, 0) equals `[64, 32, 0, 16]`
...
```simple
var rasterizer = Rasterizer.new(Logger.new("tiff-raster-rgba16-tile-spec"))
var surface = GpuSurface.create(1, 2, 1, SurfaceFormat.RGBA8)
...
expect(pixel(surface, 1, 0)).to_equal([64, 32, 0, 16])
```
...
#### renders 16-bit planar decoded TIFF pixels through the raster image path exactly
...
3. rasterizer raster draw image
   - Expected: pixel(surface, 0, 0) equals `[0, 128, 64, 255]`
   - Expected: pixel(surface, 1, 0) equals `[255, 32, 1, 255]`
...
```simple
var rasterizer = Rasterizer.new(Logger.new("tiff-raster-planar-rgb16-spec"))
var surface = GpuSurface.create(1, 2, 1, SurfaceFormat.RGBA8)
val image = decode_tiff(_tiff_le_2x1_planar_rgb16())

rasterizer.raster_draw_image(surface, image, 0, 0, 2, 1)

expect(pixel(surface, 0, 0)).to_equal([0, 128, 64, 255])
expect(pixel(surface, 1, 0)).to_equal([255, 32, 1, 255])
```
...
#### renders sparse 8K TIFF metadata placeholder without full-frame allocation
...
```simple
var rasterizer = Rasterizer.new(Logger.new("tiff-raster-sparse-8k-placeholder-spec"))
var surface = GpuSurface.create(1, 2, 2, SurfaceFormat.RGBA8)
