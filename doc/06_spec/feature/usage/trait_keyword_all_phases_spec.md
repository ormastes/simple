...
step("trait declaration is recognized")
trait Display:
    fn to_string() -> text
...
```simple
# @req REQ-SSPEC-FEATURE
step("trait name is extracted correctly")
trait Container:
    fn size() -> i64
...
```simple
# @req REQ-SSPEC-FEATURE
step("trait without methods acts as marker")
trait Marker:
    fn is_marked() -> bool:
        true
...
```simple
# @req REQ-SSPEC-FEATURE
step("struct without trait impl has no trait methods")
trait Special:
...
```simple
# @req REQ-SSPEC-FEATURE
step("finds two traits and implements both")
trait Readable:
    fn read() -> text
...
```simple
# @req REQ-SSPEC-FEATURE
step("traits mixed with non-trait declarations work together")
trait Sizeable:
    fn size() -> i64
...
```simple
# @req REQ-SSPEC-FEATURE
step("correct trait impl is found for the right type")
trait Identifiable:
    fn get_id() -> i64
...
```simple
# @req REQ-SSPEC-FEATURE
step("type without matching trait impl uses own methods")
trait Logger:
    fn log_entry() -> text
...
```simple
# @req REQ-SSPEC-FEATURE
step("fn method is immutable and returns value")
trait Inspect:
    fn inspect() -> text
...
```simple
# @req REQ-SSPEC-FEATURE
step("fn method with return type works for multiple methods")
trait Measurable:
    fn length() -> i64
    fn weight() -> i64
...
```simple
# @req REQ-SSPEC-FEATURE
step("me method can mutate state")
trait Mutable:
...
```simple
# @req REQ-SSPEC-FEATURE
step("mixed fn and me methods in same trait")
trait Repository:
    fn get_count() -> i64
...
```simple
# @req REQ-SSPEC-FEATURE
step("single-param method works")
trait Processor:
...
```simple
# @req REQ-SSPEC-FEATURE
step("multi-param method extracts all params correctly")
trait Transformer:
...
```simple
# @req REQ-SSPEC-FEATURE
step("trait with only relevant methods works")
trait Iter:
    fn next_val() -> i64
...
```simple
# @req REQ-SSPEC-FEATURE
step("trait with associated type declaration works")
trait Collection:
    type Item
    fn get_item() -> i64
...
```simple
# @req REQ-SSPEC-FEATURE
step("abstract method must be implemented")
trait Eq:
...
```simple
# @req REQ-SSPEC-FEATURE
step("default method is used when not overridden")
trait Comparable:
...
```simple
# @req REQ-SSPEC-FEATURE
step("default method with inline body works")
...
```simple
# @req REQ-SSPEC-FEATURE
step("overriding default method replaces it")
trait Greetable:
    fn greet() -> text:
        "hello"
...
```simple
# @req REQ-SSPEC-FEATURE
step("default method calls abstract method correctly")
trait Calculator:
    fn compute() -> i64
    fn double() -> i64:
        self.compute() * 2
...
```simple
# @req REQ-SSPEC-FEATURE
step("all-abstract trait requires all methods implemented")
trait Printable:
    fn print_text() -> text
    fn print_count() -> i64
...
```simple
# @req REQ-SSPEC-FEATURE
step("delegates immutable method to field")
struct Inner:
    data: i64
    fn get_len() -> i64:
        self.data
...
```simple
# @req REQ-SSPEC-FEATURE
step("delegates fn with args to field")
struct InnerCalc:
    base: i64
...
```simple
# @req REQ-SSPEC-FEATURE
step("delegates mutable method to field")
struct InnerList:
    length: i64
    me push(item: i64):
...
```simple
# @req REQ-SSPEC-FEATURE
step("forwards fn methods from trait to field")
trait Sizeable:
    fn size() -> i64
    fn is_empty() -> bool
...
```simple
# @req REQ-SSPEC-FEATURE
step("forwards me methods from trait to field")
trait Writable:
...
```simple
# @req REQ-SSPEC-FEATURE
step("unknown trait generates no forwarding - wrapper has only own methods")
struct InnerData:
    value: i64
...
```simple
# @req REQ-SSPEC-FEATURE
step("define a trait, implement it, use methods")
trait EventHandler:
...
```simple
# @req REQ-SSPEC-FEATURE
step("complete define-implement-forward workflow")
trait Formattable:
...
```simple
# @req REQ-SSPEC-FEATURE
step("multiple traits in source: each generates correct forwarding")
trait ReadCapable:
    fn read_data() -> text
...
```simple
# @req REQ-SSPEC-FEATURE
step("trait with default methods: only abstract methods need implementation")
...
```simple
# @req REQ-SSPEC-FEATURE
step("trait scanner and forwarding agree on method count")
trait Pipeline:
    me run(input: text)
    fn status() -> text
...
```simple
# @req REQ-SSPEC-FEATURE
step("fn with trait-bounded param uses static dispatch")
trait PrintCapable:
    fn print_text() -> text
...
```simple
# @req REQ-SSPEC-FEATURE
step("multiple trait params dispatch correctly")
trait ReadOp:
    fn read_op() -> text
...
```simple
# @req REQ-SSPEC-FEATURE
step("dyn Trait param uses dynamic dispatch")
trait Displayable:
    fn display() -> text
...
```simple
# @req REQ-SSPEC-FEATURE
step("interface-typed param dispatches at runtime")
trait Drawable:
    fn draw() -> text
...
- Canonical SPipe generation for source `ae29bd73bb03bc01eb22fd401b95dce38c52dcbed0c3f32c7df30bd211ea147e`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `ae29bd73bb03bc01eb22fd401b95dce38c52dcbed0c3f32c7df30bd211ea147e`.
...
source: test/feature/usage/trait_keyword_all_phases_spec.spl
mirror: doc/06_spec/feature/usage/trait_keyword_all_phases_spec.md (current)
findings: 6 blockers: 0
  narrative=100 structure=100 oracle=70
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/feature/usage/trait_keyword_all_phases_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/feature/usage/trait_keyword_all_phases_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, evidence, unsupported/limitations, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/feature/usage/trait_keyword_all_phases_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-30): 31 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/feature/usage/trait_keyword_all_phases_spec.spl:44:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'trait declaration is recognized' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/feature/usage/trait_keyword_all_phases_spec.spl:61:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'trait name is extracted correctly' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/feature/usage/trait_keyword_all_phases_spec.spl:77:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'trait without methods acts as marker' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
