# webgpu_commands_spec

> Purpose: Verify Browser WebGPU command encoding.
...
| Category | Other |
| Status | Active |
| Source | `test/feature/web_platform/webgpu/webgpu_commands_spec.spl` |
| Updated | 2026-08-26 |
...
step("records a render pass and submits a command buffer")
step("records a render pass and submits a command buffer")
# @req: REQ-FEAT-WEBGPU-WEBGPU-COMMANDS-SPEC-001
...
```simple
# @req REQ-SSPEC-FEATURE
step("rejects draw outside a render pass")
step("rejects draw outside a render pass")
var encoder = WebGPUCommandEncoder.create("frame")
...
```simple
# @req REQ-SSPEC-FEATURE
step("rejects invalid indexed draw descriptors")
step("rejects invalid indexed draw descriptors")
var encoder = WebGPUCommandEncoder.create("frame")
expect(encoder.begin_render_pass()).to_equal(true)
...
```simple
# @req REQ-SSPEC-FEATURE
step("records dynamic bind group offsets")
step("records dynamic bind group offsets")
var encoder = WebGPUCommandEncoder.create("frame")
expect(encoder.begin_render_pass()).to_equal(true)
...
```simple
# @req REQ-SSPEC-FEATURE
step("rejects invalid dynamic bind group offsets")
step("rejects invalid dynamic bind group offsets")
var encoder = WebGPUCommandEncoder.create("frame")
expect(encoder.begin_render_pass()).to_equal(true)
...
```simple
# @req REQ-SSPEC-FEATURE
step("records vertex and index buffer bindings")
step("records vertex and index buffer bindings")
var encoder = WebGPUCommandEncoder.create("frame")
expect(encoder.begin_render_pass()).to_equal(true)
...
```simple
# @req REQ-SSPEC-FEATURE
step("validates vertex and index buffer bindings against resources")
step("validates vertex and index buffer bindings against resources")
...
```simple
# @req REQ-SSPEC-FEATURE
step("rejects invalid vertex and index buffer commands")
step("rejects invalid vertex and index buffer commands")
var encoder = WebGPUCommandEncoder.create("frame")
...
```simple
# @req REQ-SSPEC-FEATURE
step("records indirect draw commands")
step("records indirect draw commands")
var encoder = WebGPUCommandEncoder.create("frame")
expect(encoder.begin_render_pass()).to_equal(true)
...
```simple
# @req REQ-SSPEC-FEATURE
step("rejects invalid indirect draw descriptors")
step("rejects invalid indirect draw descriptors")
var encoder = WebGPUCommandEncoder.create("frame")
...
```simple
# @req REQ-SSPEC-FEATURE
step("validates render pass color and depth attachments before recording")
step("validates render pass color and depth attachments before recording")
var resources = WebGPUResourceTable.create()
...
```simple
# @req REQ-SSPEC-FEATURE
step("records depth-only render pass descriptors for shadow and prepass workloads")
step("records depth-only render pass descriptors for shadow and prepass workloads")
var resources = WebGPUResourceTable.create()
...
```simple
# @req REQ-SSPEC-FEATURE
step("validates render pass depth and stencil attachment operations")
step("validates render pass depth and stencil attachment operations")
var resources = WebGPUResourceTable.create()
val color = resources.create_texture("color", 640, 480, 1, GPU_TEXTURE_FORMAT_RGBA8_UNORM, WEBGPU_TEXTURE_USAGE_RENDER_ATTACHMENT)
val depth = resources.create_texture("depth", 640, 480, 1, GPU_TEXTURE_FORMAT_DEPTH24_PLUS, WEBGPU_TEXTURE_USAGE_RENDER_ATTACHMENT)
...
```simple
# @req REQ-SSPEC-FEATURE
step("rejects invalid render pass attachment descriptors")
step("rejects invalid render pass attachment descriptors")
var resources = WebGPUResourceTable.create()
val color = resources.create_texture("color", 640, 480, 1, GPU_TEXTURE_FORMAT_RGBA8_UNORM, WEBGPU_TEXTURE_USAGE_RENDER_ATTACHMENT)
...
```simple
# @req REQ-SSPEC-FEATURE
step("records dispatch workgroups")
step("records dispatch workgroups")
...
```simple
# @req REQ-SSPEC-FEATURE
step("rejects dispatch outside a compute pass")
step("rejects dispatch outside a compute pass")
var encoder = WebGPUCommandEncoder.create("compute")
...
```simple
# @req REQ-SSPEC-FEATURE
step("records buffer and texture copies outside passes")
step("records buffer and texture copies outside passes")
...
```simple
# @req REQ-SSPEC-FEATURE
step("rejects copies inside active passes")
step("rejects copies inside active passes")
var encoder = WebGPUCommandEncoder.create("transfers")
expect(encoder.begin_render_pass()).to_equal(true)
...
```simple
# @req REQ-SSPEC-FEATURE
step("rejects invalid copy descriptors")
step("rejects invalid copy descriptors")
var encoder = WebGPUCommandEncoder.create("transfers")
...
```simple
# @req REQ-SSPEC-FEATURE
step("records buffer and texture writes")
step("records buffer and texture writes")
var queue = WebGPUQueue.create()
...
```simple
# @req REQ-SSPEC-FEATURE
step("rejects invalid queue write descriptors")
step("rejects invalid queue write descriptors")
var queue = WebGPUQueue.create()
...
```simple
# @req REQ-SSPEC-FEATURE
step("validates buffer copy resource usage and ranges")
step("validates buffer copy resource usage and ranges")
var resources = WebGPUResourceTable.create()
...
```simple
# @req REQ-SSPEC-FEATURE
step("validates texture copy resource usage format and ranges")
step("validates texture copy resource usage format and ranges")
var resources = WebGPUResourceTable.create()
...
```simple
# @req REQ-SSPEC-FEATURE
step("validates queue writes against resources")
step("validates queue writes against resources")
var resources = WebGPUResourceTable.create()
...
```simple
# @req REQ-SSPEC-FEATURE
step("validates dynamic bind group offsets against layout and buffers")
step("validates dynamic bind group offsets against layout and buffers")
var resources = WebGPUResourceTable.create()
...
```simple
# @req REQ-SSPEC-FEATURE
step("records bind groups with resource-aware dynamic offset validation")
step("records bind groups with resource-aware dynamic offset validation")
var resources = WebGPUResourceTable.create()
val uniform = resources.create_buffer("uniforms", 1024, WEBGPU_BUFFER_USAGE_UNIFORM)
val layout = resources.create_bind_group_layout("dynamic", [
...
```simple
# @req REQ-SSPEC-FEATURE
step("validates indirect draw buffers against resources")
step("validates indirect draw buffers against resources")
var resources = WebGPUResourceTable.create()
...
```simple
# @req REQ-SSPEC-FEATURE
step("rejects finish while a pass is active")
step("rejects finish while a pass is active")
var encoder = WebGPUCommandEncoder.create("frame")
expect(encoder.begin_render_pass()).to_equal(true)
...
```simple
# @req REQ-SSPEC-FEATURE
step("rejects recording after finish")
step("rejects recording after finish")
var encoder = WebGPUCommandEncoder.create("frame")
...
```simple
# @req REQ-SSPEC-FEATURE
step("rejects empty queue submissions")
step("rejects empty queue submissions")
var queue = WebGPUQueue.create()
...
```simple
# @req REQ-SSPEC-FEATURE
step("rejects mixed invalid submissions atomically")
step("rejects mixed invalid submissions atomically")
var encoder = WebGPUCommandEncoder.create("frame")
...
- `REQ-SSPEC-FEATURE`
- `REQ-FEAT-WEBGPU-WEBGPU-COMMANDS-SPEC-001`
<!-- sspec-maintain:traceability:end -->

<!-- sspec-maintain:provenance:start -->
## Generation history

- Canonical SPipe generation for source `7e5518a63ba61ac9556718272998229d767f6ca380864798949cd07d7c85bbca`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `7e5518a63ba61ac9556718272998229d767f6ca380864798949cd07d7c85bbca`.
...
source: test/feature/web_platform/webgpu/webgpu_commands_spec.spl
mirror: doc/06_spec/feature/web_platform/webgpu/webgpu_commands_spec.md (current)
findings: 6 blockers: 0
  narrative=100 structure=100 oracle=70
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/feature/web_platform/webgpu/webgpu_commands_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/feature/web_platform/webgpu/webgpu_commands_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: scope, assumptions/preconditions, primary workflow, evidence, unsupported/limitations, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/feature/web_platform/webgpu/webgpu_commands_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-30): 65 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/feature/web_platform/webgpu/webgpu_commands_spec.spl:58:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'records a render pass and submits a command buffer' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/feature/web_platform/webgpu/webgpu_commands_spec.spl:91:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'rejects draw outside a render pass' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/feature/web_platform/webgpu/webgpu_commands_spec.spl:101:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'rejects invalid indexed draw descriptors' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
