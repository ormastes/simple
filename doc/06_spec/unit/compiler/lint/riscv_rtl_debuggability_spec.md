...
| Category | Compiler |
| Status | Active |
| Source | `test/unit/compiler/lint/riscv_rtl_debuggability_spec.spl` |
| Updated | 2026-08-26 |
...
- Verify: accepts clean generated RV64 debug sidecars
   - Expected: shell("rm -rf '/tmp/riscv_rtl_debug_lint_clean'") equals `0`
...
- Verify: emits RTLDBG003 when acceptanceMarkers drift from the generated boot products manifest
   - Expected: shell("rm -rf '/tmp/riscv_rtl_debug_lint_acceptance_manifest'") equals `0`
   - Expected: result.is_ok() is true
...
- Verify: emits RTLDBG003 when sidecar and board manifest drift together from the proof-lane contract
   - Expected: shell("rm -rf '/tmp/riscv_rtl_debug_lint_acceptance_contract'") equals `0`
   - Expected: result.is_ok() is true
...
- Verify: emits RTLDBG001 for malformed sidecars
   - Expected: shell("rm -rf '/tmp/riscv_rtl_debug_lint_001'") equals `0`
   - Expected: result.is_ok() is true
...
- Verify: emits RTLDBG002 for source-map mismatches
   - Expected: shell("rm -rf '/tmp/riscv_rtl_debug_lint_002'") equals `0`
   - Expected: result.is_ok() is true
   - Expected: rt_file_write_text(sidecar_path, broken) is true
...
- Verify: emits RTLDBG003 for manifest/sidecar debug output drift
   - Expected: shell("rm -rf '/tmp/riscv_rtl_debug_lint_003'") equals `0`
   - Expected: result.is_ok() is true
...
- Verify: accepts sidecars with non-canonical whitespace
   - Expected: shell("rm -rf '/tmp/riscv_rtl_debug_lint_ws'") equals `0`
   - Expected: result.is_ok() is true
...
- Verify: emits RTLDBG101 when observability coverage is incomplete
   - Expected: shell("rm -rf '/tmp/riscv_rtl_debug_lint_101'") equals `0`
   - Expected: result.is_ok() is true
   - Expected: rt_file_write_text(sidecar_path, broken) is true
...
- Verify: emits RTLDBG102 when proof markers are too thin
   - Expected: shell("rm -rf '/tmp/riscv_rtl_debug_lint_102'") equals `0`
   - Expected: result.is_ok() is true
   - Expected: rt_file_write_text(sidecar_path, broken) is true
...
- Verify: emits RTLDBG003 when runnerSuccessMarkers drifts from runner testbench metadata
   - Expected: shell("rm -rf '/tmp/riscv_rtl_debug_lint_runner_success_markers'") equals `0`
   - Expected: result.is_ok() is true
   - Expected: rt_file_write_text(sidecar_path, broken) is true
   - Expected: has_code(results, "RTLDBG003") is true
...
- Verify: emits RTLDBG003 when acceptanceMarkers drift from board boot products
   - Expected: shell("rm -rf '/tmp/riscv_rtl_debug_lint_acceptance_markers'") equals `0`
   - Expected: result.is_ok() is true
...
- Verify: accepts board boot products manifests with reordered acceptance markers and relaxed spacing
   - Expected: shell("rm -rf '/tmp/riscv_rtl_debug_lint_acceptance_reordered'") equals `0`
   - Expected: result.is_ok() is true
   - Expected: rt_file_write_text(board_products_path, broken) is true
   - Expected: has_code(results, "RTLDBG003") is false
...
- Verify: emits RTLDBG003 when a runner file listed in the sidecar is missing
   - Expected: shell("rm -rf '/tmp/riscv_rtl_debug_lint_runner_missing'") equals `0`
   - Expected: result.is_ok() is true
   - Expected: shell("rm -f '/tmp/riscv_rtl_debug_lint_runner_missing/rv64/rtl/tb_generated_rv64_fw_jump.vhd'") equals `0`
   - Expected: has_code(results, "RTLDBG003") is true
...
- Canonical SPipe generation for source `b43a63ba51bf0fe8f750c915383cebd8a644cd58e6d367b9e3fa353a05a4ae34`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `b43a63ba51bf0fe8f750c915383cebd8a644cd58e6d367b9e3fa353a05a4ae34`.
...
source: test/unit/compiler/lint/riscv_rtl_debuggability_spec.spl
mirror: doc/06_spec/unit/compiler/lint/riscv_rtl_debuggability_spec.md (current)
findings: 6 blockers: 0
  narrative=100 structure=100 oracle=70
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/unit/compiler/lint/riscv_rtl_debuggability_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/unit/compiler/lint/riscv_rtl_debuggability_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: scope, assumptions/preconditions, primary workflow, unsupported/limitations, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/unit/compiler/lint/riscv_rtl_debuggability_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-30): 14 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/unit/compiler/lint/riscv_rtl_debuggability_spec.spl:44:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'accepts clean generated RV64 debug sidecars' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/unit/compiler/lint/riscv_rtl_debuggability_spec.spl:59:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'emits RTLDBG003 when acceptanceMarkers drift from the generated boot products manifest' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/unit/compiler/lint/riscv_rtl_debuggability_spec.spl:73:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'emits RTLDBG003 when sidecar and board manifest drift together from the proof-lane contract' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
