...
#### preserves valid void returns during alloca admission
...
```simple
# @req REQ-SSPEC-UNIT
step("preserves valid void returns during alloca admission")
val first = MirBlock(
    id: BlockId.new(0),
    label: "entry",
    instructions: [
...
val result = ssa_alloca_transform_blocks([first, second], locals, BlockId.new(0))
...
#### accepts LocalId zero in a valid cross-block return operand
...
```simple
# @req REQ-SSPEC-UNIT
step("accepts LocalId zero in a valid cross-block return operand")
val first = MirBlock(
    id: BlockId.new(0),
    label: "entry",
...
val second = MirBlock(
    id: BlockId.new(1),
    label: "return",
    instructions: [inst(MirInstKind.Store(int_operand(1), copy_operand(0)))],
    terminator: MirTerminator.Ret(nil)
)
val locals = [MirLocal(id: local(0), name: nil, type_: MirType.i64(), kind: LocalKind.Var, is_ghost: false)]
val result = ssa_alloca_transform_blocks([first, second], locals, BlockId.new(0))
...
#### rejects a malformed Some nil return operand before live-local collection

- rejects a malformed Some nil return operand before live-local collection
   - Expected: malformed_term_is_rejected(MirTerminator.Ret(Some(nil))) is true
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects a malformed Some nil return operand before live-local collection")
expect(malformed_term_is_rejected(MirTerminator.Ret(Some(nil)))).to_equal(true)
```

</details>

#### rejects every present value return before alloca analysis

- rejects every present value return before alloca analysis
   - Expected: malformed_term_is_rejected(MirTerminator.Ret(Some(copy_operand(0)))) is true
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects every present value return before alloca analysis")
expect(malformed_term_is_rejected(MirTerminator.Ret(Some(copy_operand(0))))).to_equal(true)
```

</details>

#### rejects a return Copy with a nil local payload

- rejects a return Copy with a nil local payload
   - Expected: malformed_term_is_rejected(MirTerminator.Ret(Some(operand))) is true
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects a return Copy with a nil local payload")
val operand = MirOperand(kind: MirOperandKind.Copy(nil))
expect(malformed_term_is_rejected(MirTerminator.Ret(Some(operand)))).to_equal(true)
```

</details>

#### rejects a return Move with a nil local payload
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects a return Move with a nil local payload")
val operand = MirOperand(kind: MirOperandKind.Move(nil))
expect(malformed_term_is_rejected(MirTerminator.Ret(Some(operand)))).to_equal(true)
```

</details>

#### rejects a Copy instruction with a nil source local

- rejects a Copy instruction with a nil source local
   - Expected: malformed_source_instruction_is_rejected(instruction) is true
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects a Copy instruction with a nil source local")
val instruction = inst(MirInstKind.Copy(local(1), nil))
expect(malformed_source_instruction_is_rejected(instruction)).to_equal(true)
```

</details>

#### rejects a Move instruction with a nil source local
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects a Move instruction with a nil source local")
val instruction = inst(MirInstKind.Move(local(1), nil))
expect(malformed_source_instruction_is_rejected(instruction)).to_equal(true)
```
...
| Category | Compiler |
| Status | Active |
| Source | `test/unit/compiler/mir_opt/var_reassign_analysis_spec.spl` |
| Updated | 2026-08-26 |
...
- Canonical SPipe generation for source `5d6daa4ffd6c58c4516401cb13d56136fab3f5695f412ded6200f20c26a4e074`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `5d6daa4ffd6c58c4516401cb13d56136fab3f5695f412ded6200f20c26a4e074`.
...
source: test/unit/compiler/mir_opt/var_reassign_analysis_spec.spl
mirror: doc/06_spec/unit/compiler/mir_opt/var_reassign_analysis_spec.md (current)
findings: 6 blockers: 0
  narrative=100 structure=100 oracle=70
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/unit/compiler/mir_opt/var_reassign_analysis_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/unit/compiler/mir_opt/var_reassign_analysis_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/unit/compiler/mir_opt/var_reassign_analysis_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-30): 27 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/unit/compiler/mir_opt/var_reassign_analysis_spec.spl:113:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'preserves valid void returns during alloca admission' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/unit/compiler/mir_opt/var_reassign_analysis_spec.spl:136:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'accepts LocalId zero in a valid cross-block return operand' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/unit/compiler/mir_opt/var_reassign_analysis_spec.spl:156:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'rejects a malformed Some nil return operand before live-local collection' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
