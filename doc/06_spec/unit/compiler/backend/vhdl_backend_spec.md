...
| Category | Compiler |
| Status | Active |
| Source | `test/unit/compiler/backend/vhdl_backend_spec.spl` |
| Updated | 2026-08-26 |
...
- preserves named clock domain metadata in MIR domain helpers
   - Expected: domain.name equals `core-domain`
   - Expected: domain.clock_signal equals `clk_core`
   - Expected: domain.reset_signal.unwrap() equals `rst_n`
   - Expected: domain.reset_polarity equals `VhdlResetPolarity.ActiveLow`
   - Expected: domain.reset_synchrony equals `VhdlResetSynchrony.Sync`
...
```simple
# @req REQ-SSPEC-UNIT
step("preserves named clock domain metadata in MIR domain helpers")
...
- resolves vhdl by backend name
   - Expected: kind != nil is true
   - Expected: kind.unwrap() equals `BackendKind.Vhdl`
...
```simple
# @req REQ-SSPEC-UNIT
step("resolves vhdl by backend name")
val kind = backend_for_name("vhdl")
expect(kind != nil).to_equal(true)
...
```simple
# @req REQ-SSPEC-UNIT
step("lists VHDL as an available backend")
...
```simple
# @req REQ-SSPEC-UNIT
step("generates entity with input/output ports for add function")
...
```simple
# @req REQ-SSPEC-UNIT
step("generates package with record and enum types")
...
```simple
# @req REQ-SSPEC-UNIT
step("generates arithmetic signal assignments")
...
```simple
# @req REQ-SSPEC-UNIT
step("generates bitwise signal assignments")
...
```simple
# @req REQ-SSPEC-UNIT
step("maps all integer types for port declarations")
val mapper = VhdlTypeMapper.create()
...
```simple
# @req REQ-SSPEC-UNIT
step("maps resolved types for std_logic ports")
...
```simple
# @req REQ-SSPEC-UNIT
step("generates complete ALU module with package")
val mapper = VhdlTypeMapper.create()
...
```simple
# @req REQ-SSPEC-UNIT
step("compiles MIR locals to ports and result output")
...
```simple
# @req REQ-SSPEC-UNIT
step("matches checked backend-generated adder golden")
val backend = make_simple_backend()
val result = backend.compile(single_block_module("adder_mod", make_adder_function()))
check(result.is_ok())
...
```simple
# @req REQ-SSPEC-UNIT
step("declares fixed local arrays through constrained memory type templates")
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects array ports before emitting anonymous array VHDL")
...
```simple
# @req REQ-SSPEC-UNIT
step("compiles package declarations through the backend trait path")
...
```simple
# @req REQ-SSPEC-UNIT
step("compiles tuple aggregates through a generated tuple record type")
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects anonymous hardware tuple outputs before VHDL emission")
val func = MirFunction(
    ..make_adder_function(),
...
```simple
# @req REQ-SSPEC-UNIT
step("compiles payloadless enum aggregates to variant literals")
...
```simple
# @req REQ-SSPEC-UNIT
step("compiles process enum aggregates to sanitized variant literals")
val state_type = make_payloadless_enum_type_def(
    SymbolId(id: 59),
    "process_state_t",
...
```simple
# @req REQ-SSPEC-UNIT
step("emits payload enum package records with tag and payload fields")
...
```simple
# @req REQ-SSPEC-UNIT
step("lowers payload-free variants of payload enums with default payload fields")
...
```simple
# @req REQ-SSPEC-UNIT
step("lowers payload variants of payload enums with the payload operand")
...
```simple
# @req REQ-SSPEC-UNIT
step("projects payload enum tag and payload fields from tagged records")
...
```simple
# @req REQ-SSPEC-UNIT
step("switches on payload enum tags while projecting payload fields")
...
```simple
# @req REQ-SSPEC-UNIT
step("lowers unit-return hardware entities without result ports")
val block = MirBlock(
    id: BlockId(id: 0),
    label: Some("entry"),
    instructions: [],
...
```simple
# @req REQ-SSPEC-UNIT
step("switches on payload enum tags inside combinational processes")
...
```simple
# @req REQ-SSPEC-UNIT
step("lowers payload-free variants of payload enums inside processes")
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects enum literal VHDL name collisions after sanitization")
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects floating-point input ports before VHDL emission with a source span")
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects Unit local signal lowering before VHDL emission")
val ret_local = make_return_local(1, make_i32())
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects pointer constants before package VHDL emission")
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects pointer memory instructions with instruction spans")
val ret_local = make_return_local(1, make_i32())
val block = MirBlock(
    id: BlockId(id: 0),
    label: Some("entry"),
...
```simple
# @req REQ-SSPEC-UNIT
step("keeps generic call lowering and unsupported terminators as hard errors")
val ret_local = make_return_local(1, make_i32())
val block = MirBlock(
    id: BlockId(id: 0),
    label: Some("entry"),
...
```simple
# @req REQ-SSPEC-UNIT
step("compiles MIR VHDL combinational processes with real block contents")
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects ambiguous fixed-width integer mismatches for bitwise operations")
...
```simple
# @req REQ-SSPEC-UNIT
step("rejects ambiguous fixed-width integer mismatches for comparisons")
...
```simple
# @req REQ-SSPEC-UNIT
step("preserves explicit casts before fixed-width comparisons")
val left_local = make_arg_local(1, 0, "a", make_i32())
val right_local = make_arg_local(2, 1, "b", make_i64())
val ret_local = make_return_local(3, make_bool())
...
```simple
# @req REQ-SSPEC-UNIT
step("maps arbitrary unsigned fixed-width bit lanes to numeric_std unsigned")
...
```simple
# @req REQ-SSPEC-UNIT
step("maps arbitrary signed fixed-width bit lanes and sign-extends resize")
...
```simple
# @req REQ-SSPEC-UNIT
step("emits explicit unsigned resize for named truncation")
...
```simple
# @req REQ-SSPEC-UNIT
step("zero-extends VhdlSlice results into wider unsigned destinations")
...
```simple
# @req REQ-SSPEC-UNIT
step("casts VhdlSlice results into wider signed destinations")
...
```simple
# @req REQ-SSPEC-UNIT
step("lowers hardware bitfield reads through MIR VhdlSlice into typed VHDL slices")
val src = """
```
...
- Canonical SPipe generation for source `64ae30d4e56d107d029236bf452a8a013fc312b12f85e1a89539cf35d7acd704`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `64ae30d4e56d107d029236bf452a8a013fc312b12f85e1a89539cf35d7acd704`.
...
source: test/unit/compiler/backend/vhdl_backend_spec.spl
mirror: doc/06_spec/unit/compiler/backend/vhdl_backend_spec.md (current)
findings: 5 blockers: 0
  narrative=100 structure=100 oracle=100
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/unit/compiler/backend/vhdl_backend_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/unit/compiler/backend/vhdl_backend_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/unit/compiler/backend/vhdl_backend_spec.spl:214:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'preserves named clock domain metadata in MIR domain helpers' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/unit/compiler/backend/vhdl_backend_spec.spl:234:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'resolves vhdl by backend name' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/unit/compiler/backend/vhdl_backend_spec.spl:241:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'lists VHDL as an available backend' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
