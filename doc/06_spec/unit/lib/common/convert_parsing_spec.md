...
| Status | Active |
| Source | `test/unit/lib/common/convert_parsing_spec.spl` |
| Updated | 2026-08-26 |
...
- returns value for digit characters
- Verify: returns value for digit characters
   - Expected: digit_value("0") equals `0`
   - Expected: digit_value("1") equals `1`
...
```simple
# @req REQ-SSPEC-UNIT
step("returns value for digit characters")
step("Verify: returns value for digit characters")
...
- Verify: returns -1 for non-digits
   ...
   - Expected: digit_value("Z") equals `-1`
...
- Verify: trims whitespace
   - Expected: safe_parse_int("  42  ") equals `42`
   - Expected: safe_parse_int(" -7 ") equals `-7`
...
- Verify: parses valid u16 values
   ...
   - Expected: parse_u16("256") equals `256`
...
- returns 0 for invalid
- Verify: returns 0 for invalid
   - Expected: parse_u16("abc") equals `0`
...
```simple
# @req REQ-SSPEC-UNIT
step("returns 0 for invalid")
step("Verify: returns 0 for invalid")
...
- Verify: parses valid u32 values
   ...
   - Expected: parse_u32("100000") equals `100000`
...
- parses valid u64 values
- Verify: parses valid u64 values
   - Expected: parse_u64("0") equals `0`
   - Expected: parse_u64("999999") equals `999999`
...
```simple
# @req REQ-SSPEC-UNIT
step("parses valid u64 values")
step("Verify: parses valid u64 values")
...
- passes through non-negative values
- Verify: passes through non-negative values
   - Expected: i64_to_usize(0) equals `0`
   - Expected: i64_to_usize(42) equals `42`
   - Expected: i64_to_usize(1000) equals `1000`
...
```simple
# @req REQ-SSPEC-UNIT
step("passes through non-negative values")
step("Verify: passes through non-negative values")
...
expect(i64_to_usize(1000)).to_equal(1000)  # oracle: 1000 — named expected value from the requirement
```
...
- Verify: clamps negatives to 0
   ...
   - Expected: i64_to_usize(-100) equals `0`
...
expect(i64_to_usize(-100)).to_equal(0)  # oracle: 0 — named expected value from the requirement
```
...
- Verify: is identity operation
   ...
   - Expected: usize_to_i64(999) equals `999`
...
expect(usize_to_i64(999)).to_equal(999)  # oracle: 999 — named expected value from the requirement
```
...
- returns true for truthy strings
- Verify: returns true for truthy strings
...
```simple
# @req REQ-SSPEC-UNIT
step("returns true for truthy strings")
step("Verify: returns true for truthy strings")
...
- converts booleans to text
- Verify: converts booleans to text
...
```simple
# @req REQ-SSPEC-UNIT
step("converts booleans to text")
step("Verify: converts booleans to text")
...
- Verify: converts integers to text
   ...
   - Expected: i64_to_text(-7) equals `-7`
...
- Canonical SPipe generation for source `27891e579599889127d60d2e0e4f288a1eddded42d10d9a9f44eadaeefbe0efe`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `27891e579599889127d60d2e0e4f288a1eddded42d10d9a9f44eadaeefbe0efe`.
...
source: test/unit/lib/common/convert_parsing_spec.spl
mirror: doc/06_spec/unit/lib/common/convert_parsing_spec.md (current)
findings: 6 blockers: 0
  narrative=100 structure=100 oracle=70
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/unit/lib/common/convert_parsing_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/unit/lib/common/convert_parsing_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: scope, assumptions/preconditions, primary workflow, evidence, unsupported/limitations, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/unit/lib/common/convert_parsing_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-30): 29 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/unit/lib/common/convert_parsing_spec.spl:23:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'returns value for digit characters' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/unit/lib/common/convert_parsing_spec.spl:33:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'returns -1 for non-digits' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/unit/lib/common/convert_parsing_spec.spl:42:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'parses positive integers' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
