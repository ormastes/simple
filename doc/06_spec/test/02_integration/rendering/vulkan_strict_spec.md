...
#### keeps a shared-session renderer usable after the first surface shuts down

- var session = VulkanSession create
- var first = VulkanBackend create
- var second = VulkanBackend create
- second clear
- second present
   - Expected: before_shutdown.len() equals `256`
   - Expected: before_shutdown[0] equals `color`
- first shutdown
- second clear
- second present
   - Expected: pixels.len() equals `256`
   - Expected: pixels[0] equals `color`
- second shutdown
- first shutdown
- second shutdown
- session release
...
var first = VulkanBackend.create()
var second = VulkanBackend.create()
val first_ok = first.init_with_session(16, 16, session)
val second_ok = second.init_with_session(16, 16, session)
if first_ok and second_ok:
    val color = color_blue_u32()
    second.clear(color)
    second.present()
    val before_shutdown = second.read_pixels()
    expect(before_shutdown.len()).to_equal(256)
    expect(before_shutdown[0]).to_equal(color)
    first.shutdown()
    second.clear(color)
    second.present()
    val pixels = second.read_pixels()
    expect(pixels.len()).to_equal(256)
    expect(pixels[0]).to_equal(color)
    second.shutdown()
else:
    if first_ok:
        first.shutdown()
