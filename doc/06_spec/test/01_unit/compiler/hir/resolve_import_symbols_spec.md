...
1. var lowering2 = HirLowering with filename
...
1. var lowering = HirLowering with filename
   ...
   - Expected: driver_sym.unwrap().defining_module.unwrap() equals `path_driver`
   - Expected: backend_sym.unwrap().defining_module.unwrap() equals `path_backend`
...
val driver_sym = hir.symbols.get(driver_id.unwrap())
val backend_sym = hir.symbols.get(backend_id.unwrap())
...
expect(driver_id.unwrap().id).to_be_less_than(backend_id.unwrap().id)
```
