...
val unsafe = office_ui_design_parse("design: Unsafe\nnode bad id|Bad|button|0|0|40|20|primary|1|action\nnode child|Child|bad kind|0|0|40|20|primary bad&quot;onclick=1 accent_1|bad layer|bad component|bad parent")
...
val bad_id = office_ui_design_add_node_checked(design, OfficeUiNode(id: "bad id", label: "Bad Id", kind: "button", css: "primary", x: "10", y: "20", width: "80", height: "32", layer: "controls", component: "action", parent: "", layout_mode: "off", layout_gap: "0", layout_padding: "0,0,0,0", constraint_horizontal: "left", constraint_vertical: "top"))
expect(bad_id.accepted).to_be(false)
expect(bad_id.reason).to_equal("invalid-id")
expect(bad_id.design.nodes.len()).to_equal(0)
...
expect(bad_layout.design.nodes.len()).to_equal(0)
val bad_spacing = office_ui_design_add_node_checked(design, OfficeUiNode(id: "bad_spacing", label: "Bad Spacing", kind: "button", css: "primary", x: "10", y: "20", width: "80", height: "32", layer: "controls", component: "action", parent: "", layout_mode: "horizontal", layout_gap: "-1", layout_padding: "0,0,0,0", constraint_horizontal: "left", constraint_vertical: "top"))
expect(bad_spacing.accepted).to_be(false)
expect(bad_spacing.reason).to_equal("invalid-layout")
expect(bad_spacing.design.nodes.len()).to_equal(0)
val bad_token = office_ui_design_add_node_checked(design, OfficeUiNode(id: "bad_token", label: "Bad Token", kind: "button", css: "primary", x: "10", y: "20", width: "80", height: "32", layer: "controls", component: "bad component", parent: "", layout_mode: "off", layout_gap: "0", layout_padding: "0,0,0,0", constraint_horizontal: "left", constraint_vertical: "top"))
expect(bad_token.accepted).to_be(false)
expect(bad_token.reason).to_equal("invalid-token")
expect(bad_token.design.nodes.len()).to_equal(0)
```
...
#### guards kind updates with safe kind rejection
...
```simple
val design = office_ui_design_parse("design: Kind\nnode action|Action|button|10|10|80|32|primary|1|action")
val accepted = office_ui_design_update_kind_checked(design, "action", "button", "input")
expect(accepted.accepted).to_be(true)
expect(accepted.design.nodes[0].kind).to_equal("input")
expect(office_ui_design_render_html(accepted.design)).to_contain("office-ui-input")
val invalid = office_ui_design_update_kind_checked(design, "action", "button", "!!!")
expect(invalid.accepted).to_be(false)
