...
val padded = sdn_graph_render_html_with_selection(graph, " B ", -1)
...
expect(padded).to_contain("data-selected-node-id=\"B\"")
expect(padded).to_contain("data-node=\"B\" data-selected=\"true\" aria-selected=\"true\"")
...
expect(sdn_graph_add_node_checked(graph, "D", "Bad", "", "", "", "80", "80", "64", "48", "", "bad parent").reason).to_equal("invalid-parent-token")
...
expect(sdn_graph_update_node_parent_checked(graph, "Child", "bad parent").reason).to_equal("invalid-parent-token")
