...
- build a real x86_64 user process image
- seed a scheduler user task from that image
- validate syscall-14 handoff readiness without executing the arch bridge
   - Expected: validation.error equals ``
   - Expected: validation.pid equals `701u64`
   - Expected: validation.cr3 equals `1u64`
   - Expected: context_present equals `1`
   - Expected: ctx.rip equals `image.entry`
   - Expected: ctx.rsp equals `expected_sp`
   - Expected: ctx.cs equals `0x2B`
   - Expected: ctx.ss equals `0x23`
