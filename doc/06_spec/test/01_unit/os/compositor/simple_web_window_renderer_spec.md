...
Exercises the canonical Simple Web → Draw IR → Engine2D compositor adapter,
theme/material provenance gates, external-frame admission, retained animation
cache identity, final-frame quiescence, and content revision stability.

## Scenarios

### Simple Web compositor adapter

#### projects exact accepted snapshot CSS and identity into hosted HTML

The complete executable assertions are retained in the folded SSpec source below.
...
#### publishes no external frame for a mismatched theme receipt

The complete executable assertions are retained in the folded SSpec source below.

#### renders bounded CSS animation frames then becomes quiescent

- Prime a distinct static artifact.
- Start the finite animation and release stale HTML/pixels.
- Retain only the finite terminal artifact.
- Reuse that terminal artifact with an exact cache hit and no new store.

#### separates untimed and terminal animation cache identities

- Timed terminal → untimed static → terminal: each mode/revision mismatch is a miss.
- Untimed static → timed terminal: the stale entry is released and terminal is stored once.
- Exact terminal mode, HTML, and content revision are required for a hit.
...
| Updated | 2026-07-31 |
| Generator | Manual-complete executable mirror |
