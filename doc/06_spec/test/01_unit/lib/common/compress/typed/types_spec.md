...
#### normalized weights match expected table size
...
```simple
val table = FseTable.new(3, [1, 2, 5])
assert_equal(table.log(), 3)
assert_equal(table.symbol_count(), 3)
assert_equal(table.count(2), 5)
assert_equal(table.total_weight(), 8)
assert_equal(table.expected_table_size(), 8)
assert_true(table.is_normalized())
```

</details>

#### unnormalized weights fail closed
...
```simple
val table = FseTable.new(3, [1, 2, 4])
assert_equal(table.total_weight(), 7)
assert_equal(table.expected_table_size(), 8)
assert_false(table.is_normalized())
```

</details>

#### out-of-range symbols return zero
...
```simple
val table = FseTable.new(2, [1, 3])
assert_equal(table.count(-1), 0)
assert_equal(table.count(2), 0)
```

</details>

### Compression payload newtypes

#### Plaintext wraps uncompressed bytes
...
```simple
val payload = Plaintext.new([65u8, 66u8, 67u8])
assert_equal(payload.len(), 3)
assert_false(payload.is_empty())
assert_equal(payload.get(1).to_i64(), 66)
assert_equal(payload.get(99).to_i64(), 0)
```

</details>

#### Compressed wraps compressed bytes separately
...
```simple
val payload = Compressed.new([31u8, 139u8])
assert_equal(payload.len(), 2)
assert_false(payload.is_empty())
assert_equal(payload.get(0).to_i64(), 31)
assert_equal(payload.get(-1).to_i64(), 0)
```
