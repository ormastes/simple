...
```simple
val arr = json_array([json_number(1), json_number(2)])
expect(json_array_every(arr, json_to_number(_1) > 1)).to_equal(false)
```
...
```simple
val arr = json_array([json_number(1), json_number(2)])
expect(json_array_some(arr, json_to_number(_1) == 2)).to_equal(true)
```
...
val grouped = json_array_group_by(arr, json_to_string(json_object_get(_1, "kind")))
