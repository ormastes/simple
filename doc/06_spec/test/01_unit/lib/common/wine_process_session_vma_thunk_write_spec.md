# Wine Process Session Vma Thunk Write Specification

> <details>

<!-- sdn-diagram:id=wine_process_session_vma_thunk_write_spec.arch -->
<details class="sdn-source">
<summary>SDN source</summary>

```sdn id=wine_process_session_vma_thunk_write_spec.arch hash=sha256:auto render=ascii
@layout dag
@direction LR

wine_process_session_vma_thunk_write_spec -> common
```
...
```ascii generated-from=wine_process_session_vma_thunk_write_spec.arch hash=sha256:auto
...
### Wine process session VMA thunk writes

#### opens a bounded process VMA write window for known thunk patches
...
#### keeps VMA thunk writes behind record planning
...
#### opens VMA thunk writes only after PEB/TEB VM byte-write readback
...
#### blocks VMA thunk writes when PEB/TEB VM byte writes are not ready
...
#### blocks VM-gated VMA thunk writes without patched image when record planning rejects
...
| Status | Active |
| Source | `test/01_unit/lib/common/wine_process_session_vma_thunk_write_spec.spl` |
| Updated | 2026-06-01 |
