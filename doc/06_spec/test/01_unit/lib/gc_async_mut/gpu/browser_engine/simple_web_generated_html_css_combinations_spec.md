...
- generated form shell: `form`, `fieldset`, `legend`, `label`, `input`,
  `select`, `option`, and `progress` with grouped selectors;
...
val html = "<html><head><style>html,body{margin:0;padding:0;background-color:#ffffff}fieldset{display:block;width:96px;height:44px;padding:3px;border:1px solid #334155;background-color:#f8fafc}label{display:block;color:#111827;font-size:8px}input,select,progress{display:block;width:60px;height:8px;margin-top:2px;background-color:#fde68a;color:#111827}</style></head><body><form><fieldset><legend>Prefs</legend><label>Name<input value='Ada'></label><select><option>One</option></select><progress value='1' max='2'></progress></fieldset></form></body></html>"
