...
#### feeds rendered Draw IR command counts into host GPU event-flow evidence

- var engine = Engine2D create with backend
- engine clear
- draw ir rect
- draw ir rect
- result rendered command count to i64
- result rendered command count to i64
- result pixels[8 * 32 + 7] to i64
   - Expected: result.rendered_command_count equals `2`
   - Expected: evidence.ok is true
   - Expected: evidence.draw_ir_delta_count equals `2`
   - Expected: evidence.packet_bytes equals `256`
   - Expected: evidence.pixel_hash equals `GREEN.to_i64()`
...
val evidence = engine2d_host_gpu_event_flow_evidence(
    ...
    "draw_ir_delta",
    1,
    result.rendered_command_count.to_i64(),
    result.rendered_command_count.to_i64() * 128,
    4096,
    false,
    false,
    true,
...
expect(evidence.packet_bytes).to_equal(256)
expect(evidence.pixel_hash).to_equal(GREEN.to_i64())
expect(evidence.speedup_x1000).to_equal(2000)
expect(engine2d_host_gpu_event_flow_summary(evidence)).to_contain("draw_ir_deltas=2")
engine.shutdown()
```
