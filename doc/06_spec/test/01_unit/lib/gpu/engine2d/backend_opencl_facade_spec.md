...
> Verifies that the OpenCL Engine2D facade exposes the generated 2D render kernel set, fails closed without CPU backend substitution, and routes Engine2D probing through typed OpenCL backend diagnostics.
...
Verifies that the OpenCL Engine2D facade exposes the generated 2D render kernel
set, fails closed without CPU backend substitution, and routes Engine2D probing
through typed OpenCL backend diagnostics.

**Source:** `src/lib/gc_async_mut/gpu/engine2d/backend_opencl.spl`
...
Use `OpenClBackend.create()` for direct facade checks and
`Engine2D.probe_backend(width, height, "opencl")` for strict selection
diagnostics. Inspect `last_probe` instead of parsing error text.

## Examples

The scenarios verify generated kernel entry names, mirror-only fallback stale
marking, advanced facade method behavior, clip-state device-draw blocking, and
strict OpenCL probe metadata without accepting CPU fallback.
