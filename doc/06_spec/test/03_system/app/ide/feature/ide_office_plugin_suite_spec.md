...
draw: Diagram Draw [office-app] -> std.editor.services.sdn_graph (draw, diagram, sdd, sdn)
  check: draw: sdn_graph nodes=2 edges=1 html=true route=true select=true inspect=true edit=true edge_style=true reconnect=true layout=true canvas=true
...
db-admin: Database Admin [database] -> std.editor.core.session_db (embedded-db, simple-db, portal-db)
  ...
  llm-catalog: apps=9 features=82 actions=41
...
Runnable source: 336 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.
...
expect(office_llm_catalog_summary()).to_equal("llm-catalog: apps=9 features=82 actions=41")
