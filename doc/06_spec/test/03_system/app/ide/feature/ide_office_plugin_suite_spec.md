...
db-admin: Database Admin [database] -> std.editor.core.session_db (embedded-db, simple-db, portal-db)
  ...
  llm-catalog: apps=11 features=203 actions=124
...
expect(tui_report).to_contain("features=203")
...
Runnable source: 1127 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.
...
expect(office_llm_catalog_summary()).to_equal("llm-catalog: apps=11 features=203 actions=124")
val dispatch_probe = office_catalog_dispatch_probe()
expect(dispatch_probe.advertised_count).to_equal(124)
expect(dispatch_probe.recognized_count).to_equal(124)
