...
> This specification keeps the remaining hosted multicore-green Go-parity gap explicit. Hosted runtime-pool evidence is real, but the lane must stay `current` until blocking integration and fairness/preemption are tracked as unresolved host-runtime work rather than being implied closed by SimpleOS-only proofs.
...
until blocking integration and fairness/preemption are tracked as unresolved
...
- Hosted multicore-green pool evidence and work-stealing rows are current, but
  that does not close host fairness/preemption.
...
- Verify the requirement still names blocking integration and preemption before Go-like fairness claims
...
step("Verify the requirement still names blocking integration and preemption before Go-like fairness claims")
...
expect(research).to_contain("blocking integration and final")
...
expect(bug).to_contain("blocking integration")
