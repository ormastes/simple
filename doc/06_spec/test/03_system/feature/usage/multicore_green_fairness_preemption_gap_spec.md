...
```sdn id=multicore_green_fairness_preemption_gap_spec.arch hash=sha256:auto render=ascii
@layout dag
@direction LR

multicore_green_fairness_preemption_gap_spec -> std
```
...
```ascii generated-from=multicore_green_fairness_preemption_gap_spec.arch hash=sha256:auto
