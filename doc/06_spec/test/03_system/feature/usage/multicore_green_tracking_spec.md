...
expect(architecture).to_contain("The user-facing API is Pure Simple")
expect(architecture).to_contain("runtime-seed support")
expect(architecture).to_contain("user code does not call `rt_pool_*` directly")
expect(design).to_contain("Pure Simple user-facing facade")
expect(design).to_contain("runtime-seed `rt_pool_submit")
expect(design).to_contain("does not import or call `rt_pool_*` directly")
