...
gui_entry_engine2d_wm_simple_web_spec -> app
...
- Serial output proves WM, Engine2D, Simple Web, top command lane, taskbar,
  HTML renderability through the Simple Web QEMU panel renderer contract, and
  the canonical five shared MDI surfaces.
...
- stop guest
   - Expected: _pass_fail(saw_probe and saw_wm and saw_engine and saw_web and saw_web_contract and saw_mdi and saw_top and saw_taskbar and saw_html and saw_ready) equals `pass`
-  print guest diagnostics
- stop guest
   - Expected: _pass_fail(captured_input) equals `pass`
- stop guest
...
match spawn_guest_with_qmp(capture_target, qmp_socket, serial_log):
    Ok(handle):
        ...
        val saw_web_contract = serial.contains("[web-demo] renderer=simple-web-qemu-panel-contract") and serial.contains("html=about:engine2d-wm") and serial.contains("checksum=")
        val saw_html = serial.contains("[mdi-demo] html-renderable window=browser renderer=simple-web-qemu-panel-contract") and serial.contains("checksum=")
