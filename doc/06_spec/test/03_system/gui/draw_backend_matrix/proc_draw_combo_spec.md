...
> `draw_backend_matrix_spec.spl` (this dir) pins ONE drawing backend at a time. This spec instead runs full **processing × drawing COMBINATION** lanes on a headless host, for two combos — **cuda/vulkan** and **vulkan/vulkan** (processing / drawing) — across three surfaces plus event handling:
...
### Processing × Drawing combination runs — headless offscreen (cuda/vulkan & vulkan/vulkan)
