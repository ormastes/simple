...
> This SSpec keeps the final SimpleOS green-thread hardware handoff boundary executable. Earlier QEMU evidence proved AP startup, fixed-slot dispatch, preemption, scheduler-owned handoff through `SCHED_HANDOFF_PASS=true`, and prerequisite user-entry readiness markers. Current direct QEMU evidence now reaches `USER_CR3_READY=true`, `HW_HANDOFF_PASS=true`, `USER_ENTRY_PASS=true`, and `USER_SYSCALL_PASS=true` through the opt-in final live lane. The scheduler-only readiness lane remains separate so prerequisite markers cannot be mistaken for final hardware proof.
...
reaches `USER_CR3_READY=true`, `HW_HANDOFF_PASS=true`,
`USER_ENTRY_PASS=true`, and `USER_SYSCALL_PASS=true` through the opt-in final
live lane. The scheduler-only readiness lane remains separate so prerequisite
...
- The closure document must record the direct final-entry probe finding:
  legacy `cr3=1` was insufficient, the probe-local page-table root closed the
  CR3 prerequisite, and the 2026-06-14 final live QEMU refresh closed the final
  marker triplet.
...
expect(blocker).to_contain("probe-local page-table root")
expect(blocker).to_contain("2026-06-14 Final Live QEMU Refresh")
expect(blocker).to_contain("rt_pool_safepoint")
expect(blocker).to_contain("clears IF while keeping IOPL=3")
expect(blocker).to_contain("passed 3 scenarios in 74244ms")
...
- Verify the evidence report records the current opt-in final ring/user handoff proof
...
step("Verify the evidence report records the current opt-in final ring/user handoff proof")
...
expect(report).to_contain("| SimpleOS green-carrier QEMU final hardware handoff lane | PASS | 3 |")
...
expect(report).to_contain("Result: PASS")
expect(report).to_contain("rt_pool_safepoint")
expect(report).to_contain("clears IF while keeping IOPL=3")
...
expect(report).to_contain("passed 3 scenarios in")
expect(report).to_contain("`--backend llvm` remains unavailable")
expect(report).to_contain("fast contract below remains the release-visible guard")
