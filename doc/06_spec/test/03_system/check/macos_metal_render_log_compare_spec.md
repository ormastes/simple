...
11. Reject ARGB artifact fallbacks that borrow stale working-directory files
    instead of evidence beside the browser env file.
12. Reject missing Xcode GPU capture when strict capture mode is enabled.
13. Reject Xcode GPU capture rows whose artifact bytes do not match the native
   marker, even if the env row claims `XCODE-GPUTRACE`.
14. Reject Xcode GPU capture rows that omit the capture artifact or native
    artifact marker.
15. Preserve typed unavailable evidence when required Metal input env files are
    missing.
16. Reject strict Metal capture rows that use browser metadata as the capture
