...
6. Reject pairwise rows whose Simple, Chrome, or Electron ARGB evidence is
   blank or uses mismatched viewport geometry.
7. Reject pairwise rows whose ARGB checksums are missing or mismatched.
8. Reject missing Xcode GPU capture when strict capture mode is enabled.
9. Reject Xcode GPU capture rows whose artifact bytes do not match the native
   marker, even if the env row claims `XCODE-GPUTRACE`.
10. Reject status-only Xcode GPU capture rows that omit the capture artifact or
