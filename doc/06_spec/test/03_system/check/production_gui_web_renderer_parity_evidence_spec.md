...
- The wrapper skips host-incompatible self-hosted artifacts during fallback
  discovery so a Linux release binary cannot be selected on macOS.
...
- The wrapper checks candidate host compatibility before selecting a fallback.
...
- A missing resolved Simple binary must not silently fall back to a stale cargo
  target path.
- A stale cross-host release binary must not be selected when a native
  self-hosted Simple binary is available.
...
expect(script).to_contain("is_host_compatible_simple")
expect(script).to_contain("*ELF*|*PE32*) return 1")
expect(script).to_contain("*Mach-O*|*PE32*) return 1")
expect(script).to_contain("if is_host_compatible_simple \"$candidate\"")
expect(script).to_contain("SIMPLE_BIN_STATUS=forbidden")
expect(script).to_contain("SIMPLE_BIN_STATUS=incompatible")
expect(script).to_contain("host-incompatible")
