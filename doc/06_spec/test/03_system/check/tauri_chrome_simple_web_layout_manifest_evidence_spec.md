...
> Validates platform-specific evidence rows emitted by the Tauri/Chrome surface manifest wrapper used by the production GUI/Web renderer parity gate. The macOS branch must stay explicit: until a native WKWebView snapshot backend exists, Darwin hosts report the Tauri surface lane as unavailable while still identifying Chrome as the live-bitmap browser lane.
...
macOS branch must stay explicit: until a native WKWebView snapshot backend
exists, Darwin hosts report the Tauri surface lane as unavailable while still
identifying Chrome as the live-bitmap browser lane.
...
- A Darwin host reports the Tauri surface backend as
  `macos-wkwebview-snapshot-unimplemented`.
- The Darwin Tauri surface lane is unavailable with reason
  `macos-wkwebview-snapshot-backend-unimplemented` and does not claim live
  capture.
...
#### reports the macOS WKWebView snapshot backend as unavailable without claiming Tauri live capture
...
```simple
val root = "build/test-tauri-chrome-layout-manifest-darwin-unimplemented"
...
expect(evidence).to_contain("tauri_chrome_simple_web_layout_manifest_tauri_capture_status=unavailable")
expect(evidence).to_contain("tauri_chrome_simple_web_layout_manifest_tauri_capture_reason=macos-wkwebview-snapshot-backend-unimplemented")
expect(evidence).to_contain("tauri_chrome_simple_web_layout_manifest_tauri_capture_backend=macos-wkwebview-snapshot-unimplemented")
expect(evidence).to_contain("tauri_chrome_simple_web_layout_manifest_tauri_capture_required_commands=macos-wkwebview-snapshot-backend")
expect(evidence).to_contain("tauri_chrome_simple_web_layout_manifest_tauri_capture_missing_commands=macos-wkwebview-snapshot-backend")
