...
`artifact_magic=RDOC`; status-only rows are not capture proof. macOS must report
...
marker, and `windows_d3d12_render_log_compare_gpu_debugger_status=pass` with a
debugger artifact.
...
expect(evidence).to_contain("windows_d3d12_render_log_compare_reason=windows-d3d12-gpu-debugger-artifact-missing")
```
