...
For the current Linux host, Simple RenderDoc evidence can pass while Chrome and
Electron browser `.rdc` evidence remains missing or crashed under RenderDoc.
That state is incomplete, but it is actionable only when the blocked gate list
is visible at aggregate level.
...
Do not replace the missing `.rdc` proof with ARGB bitmap proof, browser backing
metadata, or cached log text.
