...
- All 130 implemented Simple Web CSS properties appear in actual rendered
  fixture CSS.
...
- Read the emitted evidence contract
   ...
   - Expected: css_count equals `130`
   - Expected: css_covered equals `130`
   - Expected: css_covered_names.split(",").len() equals `130`
...
expect(css_count).to_equal("130")
expect(css_covered).to_equal("130")
...
expect(css_covered_names.split(",").len()).to_equal(130)
...
expect(report).to_contain("- implemented CSS properties: 130/130")
```
