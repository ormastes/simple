...
- All 125 implemented Simple Web CSS properties appear in actual rendered
  fixture CSS.
...
- Read the emitted evidence contract
   ...
   - Expected: css_count equals `125`
   - Expected: css_covered equals `125`
   - Expected: css_covered_names.split(",").len() equals `125`
...
expect(css_count).to_equal("125")
expect(css_covered).to_equal("125")
...
expect(css_covered_names.split(",").len()).to_equal(125)
...
expect(report).to_contain("- implemented CSS properties: 125/125")
```
