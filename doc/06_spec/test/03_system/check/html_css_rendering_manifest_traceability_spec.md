...
- Read the emitted evidence contract
   ...
   - Expected: css_count equals `241`
   - Expected: css_covered equals `241`
   - Expected: css_covered_names.split(",").len() equals `241`
...
expect(evidence).to_contain("html_css_rendering_manifest_traceability_css_property_source_count=241")
...
expect(css_count).to_equal("241")
expect(css_covered).to_equal("241")
...
expect(css_covered_names.split(",").len()).to_equal(241)
...
expect(report).to_contain("- implemented CSS properties: 241/241")
```
