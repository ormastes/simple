...
- All 63 implemented Simple Web CSS properties appear in actual rendered
  fixture CSS.
...
- Read the emitted evidence contract
   ...
   - Expected: css_count equals `63`
   - Expected: css_covered equals `63`
   - Expected: css_covered_names.split(",").len() equals `63`
...
expect(css_count).to_equal("63")
expect(css_covered).to_equal("63")
...
expect(css_covered_names.split(",").len()).to_equal(63)
