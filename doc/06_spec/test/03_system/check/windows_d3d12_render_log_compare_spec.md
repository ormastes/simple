...
3. If PIX or another GPU debugger capture is required for the host, write
   ...
   status-only debugger rows and symlinked artifacts are diagnostic, not native
   proof.
...
Native debugger artifacts must be regular files, not symlinks to capture
outputs outside the evidence boundary.
