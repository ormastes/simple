...
expect(evidence).to_contain("gui_showcase_4k_200fps_reason=missing-4k-native-artifacts:alias_src=missing;native_bin=missing;native_bin_executable=missing;native_build_log=missing")
expect(report).to_contain("4K retained perf native artifacts: alias_src missing; native_bin missing; native_bin_executable missing; native_build_log missing")
```
...
expect(report).to_contain("4K retained perf native artifacts: alias_src pass; native_bin pass; native_bin_executable pass; native_build_log pass")
expect(report).to_contain("8K retained perf native artifacts: alias_src pass; native_bin pass; native_bin_executable pass; native_build_log pass")
```
