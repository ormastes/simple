...
RESOLUTION=8k MAX_RSS_KB=0 BUILD_DIR=build/widget-showcase-8k-perf \
sh scripts/check/check-widget-showcase-4k-200fps.shs
```
...
release-quality memory claim should set `MAX_RSS_KB` so
`gui_showcase_8k_perf_rss_status=pass` proves the budget was enforced. A bare
perf `status=pass` without FPS, checksum, and exact geometry is downgraded to
`fail`.
