...
expect(evidence).to_contain("blocked_completion_gate_count=2")
expect(evidence).to_contain("blocked_completion_gates=production GUI/web parity evidence with live Tauri and Chrome captures|full CSS specification rendering coverage beyond implemented Simple Web subset")
expect(evidence).to_contain("gui_renderdoc_feature_coverage_status=incomplete")
