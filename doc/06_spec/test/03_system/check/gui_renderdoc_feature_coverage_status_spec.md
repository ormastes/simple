...
- The Electron Simple Web layout manifest remains visible with its 32 cases.
...
- Assert the Electron layout manifest and RenderDoc gates remain visible
   - Expected: manifest_cases equals `32`
...
expect(manifest_cases).to_equal("32")
...
expect(report).to_contain("- Electron layout manifest cases: 32")
```
