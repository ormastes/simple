...
val command = "rm -rf build/test-gui-renderdoc-feature-coverage-status-8k-bare && mkdir -p build/test-gui-renderdoc-feature-coverage-status-8k-bare/source && printf 'gui_showcase_8k_perf_status=pass\\ngui_showcase_8k_perf_reason=met-target-fps\\n' > build/test-gui-renderdoc-feature-coverage-status-8k-bare/source/status.env && GUI_SHOWCASE_8K_PERF_ENV=build/test-gui-renderdoc-feature-coverage-status-8k-bare/source/status.env BUILD_DIR=build/test-gui-renderdoc-feature-coverage-status-8k-bare/out REPORT_PATH=build/test-gui-renderdoc-feature-coverage-status-8k-bare/report.md sh scripts/check/check-gui-renderdoc-feature-coverage-status.shs"
val (_stdout, _stderr, code) = process_run("/bin/sh", ["-c", command])
expect(code).to_equal(0)
