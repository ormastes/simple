...
- The Electron Simple Web layout manifest remains visible with its 46 cases.
...
- Assert the Electron layout manifest and RenderDoc gates remain visible
   - Expected: manifest_cases equals `48`
...
expect(manifest_cases).to_equal("48")
...
expect(report).to_contain("- Electron layout manifest cases: 48")
```
