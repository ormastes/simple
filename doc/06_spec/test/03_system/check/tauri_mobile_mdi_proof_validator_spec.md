...
- Explicitly requested mobile MDI proof source paths must exist and be
  ...
  empty, symlinked, or non-regular requested source artifact.
...
expect(event.contains("ios_mdi_event_body_click_routed=false")).to_equal(false)
expect(html).to_contain("android_mdi_proof_status=fail")
expect(html).to_contain("android_mdi_render_status=fail")
expect(html).to_contain("android_mdi_render_html_renderable=")
expect(html.contains("android_mdi_render_html_renderable=false")).to_equal(false)
...
expect(perf.contains("ios_mdi_performance_now_available=false")).to_equal(false)
...
expect(animation.contains("android_mdi_animation_frame_available=false")).to_equal(false)
expect(animation.contains("android_mdi_css_animation_probe=false")).to_equal(false)
```
