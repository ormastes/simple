...
#### gates multicore-green native rows and preserves explicit OS-thread failure evidence
...
- Verify the checked-in contract keeps the OS-thread native row as an explicit fail
   - Expected: metric_text(row_for_label(parallel, "Simple (native)")) equals `fail`
- Verify multicore-green, Go, and C worker rows have positive timing evidence
- Verify multicore-green stays inside the current native smoke threshold
...
- Verify the checked-in contract keeps the OS-thread fanout native row as an explicit fail
   - Expected: metric_text(row_for_label(fanout, "Simple (native)")) equals `fail`
