# RISC-V Gen2 Zca target-trap product closure
...
- Evidence class: typed-HWIR elaboration and deterministic VHDL text.
...
1. **Product ISA closure.** Build each concrete target profile and require 26
   RV32 rows and 32 RV64 rows. The RV64-only load/store and word-operation rows
   must be present only in the RV64 product.
2. **Row ambiguity guard.** Build the complete RV64 decoder, require a valid
   graph and exactly one row-level overlap guard, and retain origins for every
   RV64-only row.
3. **Deterministic specialized artifact.** Lower the RV64 stateful product
   twice, require identical 64-character HWIR digests, and inspect the emitted
...
These scenarios establish source-level typed composition, profile selection,
and deterministic VHDL serialization. They do not run generated VHDL in GHDL,
prove complete RTL behavior, or qualify an architectural retirement producer.
