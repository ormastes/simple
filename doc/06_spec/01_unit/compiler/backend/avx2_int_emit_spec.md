...
> Tests covering AVX2 emit_vpaddd_ymm ymm0 ymm1 ymm2 golden, AVX2 emit_vpaddd_ymm extended dst ymm8, AVX2 emit_vpaddd_ymm extended rn ymm9, AVX2 emit_vpaddd_ymm extended rm ymm8, AVX2 emit_vpsubd_ymm ymm0 ymm1 ymm2 golden, AVX2 emit_vpsubd_ymm ymm3 ymm4 ymm5, AVX2 emit_vpmulld_ymm ymm0 ymm1 ymm2 golden, AVX2 emit_vpmulld_ymm ymm0 ymm3 ymm4, AVX2 emit_vpand_ymm ymm0 ymm1 ymm2 golden, AVX2 emit_vpand_ymm ymm0 ymm2 ymm3, AVX2 emit_vpor_ymm ymm0 ymm1 ymm2 golden, AVX2 emit_vpor_ymm ymm0 ymm3 ymm4, AVX2 emit_vpxor_ymm ymm0 ymm1 ymm2 golden, AVX2 emit_vpxor_ymm ymm0 ymm4 ymm5, AVX2 emit_vpsrld_ymm_imm ymm0 ymm1 imm8=8 golden, AVX2 emit_vpsrld_ymm_imm ymm1 ymm2 imm8=16, AVX2 emit_vpsrld_ymm_imm extended rn ymm8, AVX2 emit_vpslld_ymm_imm ymm0 ymm1 imm8=3 golden, AVX2 emit_vpslld_ymm_imm ymm2 ymm3 imm8=5, AVX2 emit_vpslld_ymm_imm extended rn ymm8.
...
```simple
# @req REQ-SSPEC-UNIT
step("byte1 is 0x61 (~R=0 ~X=1 ~B=1 mmmmm=1)")
...
```simple
# @req REQ-SSPEC-UNIT
step("byte1 is 0xC1 (~R=1 ~X=1 ~B=0 mmmmm=1)")
...
```simple
# @req REQ-SSPEC-UNIT
step("byte4 is ModRM 0xC4 (mod=11 reg=0 rm=4)")
val result = emit_vpmulld_ymm(0, 3, 4)
expect result[4] == 196
```
...
```simple
# @req REQ-SSPEC-UNIT
step("byte4 is ModRM 0xC3 (mod=11 reg=0 rm=3)")
val result = emit_vpand_ymm(0, 2, 3)
expect result[4] == 195
```
...
```simple
# @req REQ-SSPEC-UNIT
step("byte4 is ModRM 0xC5 (mod=11 reg=0 rm=5)")
val result = emit_vpxor_ymm(0, 4, 5)
expect result[4] == 197
```
...
```simple
# @req REQ-SSPEC-UNIT
step("byte4 is ModRM 0xD1 (mod=11 reg=2 rm=1)")
val result = emit_vpsrld_ymm_imm(0, 1, 8)
expect result[4] == 209
```
...
```simple
# @req REQ-SSPEC-UNIT
step("byte2 is 0x75 (vvvv=rd=1 → ~vvvv=14)")
...
```simple
# @req REQ-SSPEC-UNIT
step("byte4 is ModRM 0xD2 (mod=11 reg=2 rm=2)")
val result = emit_vpsrld_ymm_imm(1, 2, 16)
expect result[4] == 210
```
...
```simple
# @req REQ-SSPEC-UNIT
step("byte4 is ModRM 0xD0 (mod=11 reg=2 rm=0)")
val result = emit_vpsrld_ymm_imm(0, 8, 4)
expect result[4] == 208
```
...
```simple
# @req REQ-SSPEC-UNIT
step("byte4 is ModRM 0xF1 (mod=11 reg=6 rm=1)")
val result = emit_vpslld_ymm_imm(0, 1, 3)
expect result[4] == 241
```
...
```simple
# @req REQ-SSPEC-UNIT
step("byte2 is 0x6D (vvvv=rd=2 → ~vvvv=13)")
...
```simple
# @req REQ-SSPEC-UNIT
step("byte4 is ModRM 0xF3 (mod=11 reg=6 rm=3)")
val result = emit_vpslld_ymm_imm(2, 3, 5)
expect result[4] == 243
```
...
```simple
# @req REQ-SSPEC-UNIT
step("byte4 is ModRM 0xF0 (mod=11 reg=6 rm=0)")
val result = emit_vpslld_ymm_imm(0, 8, 4)
expect result[4] == 240
```
...
- Canonical SPipe generation for source `707a68aa88e1b4a0b23092d49edf0caf20a7d35de9fc0eaa1657832664a6c15d`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `707a68aa88e1b4a0b23092d49edf0caf20a7d35de9fc0eaa1657832664a6c15d`.
...
source: test/01_unit/compiler/backend/avx2_int_emit_spec.spl
mirror: doc/06_spec/01_unit/compiler/backend/avx2_int_emit_spec.md (current)
findings: 5 blockers: 0
  narrative=100 structure=100 oracle=100
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/01_unit/compiler/backend/avx2_int_emit_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/01_unit/compiler/backend/avx2_int_emit_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, unsupported/limitations, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/01_unit/compiler/backend/avx2_int_emit_spec.spl:53:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'emits 5 bytes' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/01_unit/compiler/backend/avx2_int_emit_spec.spl:59:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'byte0 is VEX 3-byte escape 0xC4' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/01_unit/compiler/backend/avx2_int_emit_spec.spl:65:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'byte1 is 0xE1 (~R=1 ~X=1 ~B=1 mmmmm=1)' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
