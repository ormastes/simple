...
# Anonymous tuple returns are split into per-field output ports
# (out0/out1) rather than a single aggregate port -- see
# VhdlReturnAbi/vhdl_return_field_name, which is also relied on by
# "rejects anonymous hardware tuple outputs before VHDL emission".
check(compiled.vhdl.contains("out0 : out signed(31 downto 0)"))
check(compiled.vhdl.contains("out1 : out bit"))
check(compiled.vhdl.contains("signal tuple_sig : tuple_t_0;"))
check(compiled.vhdl.contains("tuple_sig <= (f0 => a, f1 => flag);"))
check(compiled.vhdl.contains("out0 <= return_value.f0;"))
check(compiled.vhdl.contains("out1 <= return_value.f1;"))
```
...
var func = make_named_hardware_adder_function(SymbolId(id: 42), "anonymous_hardware_outputs")
func.signature = MirSignature(params: [], return_type: make_tuple_i32_bool(), is_variadic: false)
...
val builder = VhdlBuilder.create("payload_agg_mod")
val result = backend.compile_function(builder, func)
check(result.is_ok())
val vhdl = builder.build()
check(vhdl.contains("state_sig <= (tag => None, Some_payload => to_signed(0, 32));"))
```
...
var backend = make_simple_backend()
backend.active_type_defs = backend.active_type_defs.set(payload_symbol.id, payload_enum)
val builder = VhdlBuilder.create("payload_some_mod")
val result = backend.compile_function(builder, func)
check(result.is_ok())
val vhdl = builder.build()
check(vhdl.contains("state_sig <= (tag => Some, Some_payload => to_signed(7, 32));"))
```
...
var backend = make_simple_backend()
backend.active_type_defs = backend.active_type_defs.set(payload_symbol.id, payload_enum)
val builder = VhdlBuilder.create("payload_project_mod")
val result = backend.compile_function(builder, func)
check(result.is_ok())
val vhdl = builder.build()
check(vhdl.contains("payload_sig <= state_sig.Some_payload;"))
check(vhdl.contains("result_out <= payload_sig;"))
```
...
var backend = make_simple_backend()
backend.active_type_defs = backend.active_type_defs.set(payload_symbol.id, payload_enum)
val builder = VhdlBuilder.create("payload_match_mod")
val result = backend.compile_function(builder, func)
check(result.is_ok())
val vhdl = builder.build()
check(vhdl.contains("payload_sig <= state.Some_payload;"))
check(vhdl.contains("state.tag = Some"))
```
...
val builder = VhdlBuilder.create("unit_sink_mod")
val result = make_simple_backend().compile_function(builder, func)
check(result.is_ok())
val vhdl = builder.build()
check(vhdl.contains("entity unit_sink is"))
check(vhdl.contains("port ("))
check(not vhdl.contains("result_out"))
```
...
val none_block = MirBlock(
    ...
    instructions: [
        MirInst(
            kind: MirInstKind.Const(LocalId(id: 2), MirConstValue.Int(0), make_i32()),
            span: nil
        )
    ],
    terminator: MirTerminator.Ret(nil)
...
var backend = make_simple_backend()
backend.active_type_defs = backend.active_type_defs.set(payload_symbol.id, payload_enum)
val builder = VhdlBuilder.create("payload_proc_match_mod")
val result = backend.compile_function(builder, func)
check(result.is_ok())
val vhdl = builder.build()
check(vhdl.contains("process(state)"))
check(vhdl.contains("if state.tag = Some then"))
check(vhdl.contains("payload_sig <= state.Some_payload;"))
```
...
var backend = make_simple_backend()
backend.active_type_defs = backend.active_type_defs.set(payload_symbol.id, payload_enum)
val builder = VhdlBuilder.create("payload_proc_mod")
val result = backend.compile_function(builder, func)
check(result.is_ok())
val vhdl = builder.build()
check(vhdl.contains("result_out <= (tag => None, Some_payload => to_signed(0, 32));"))
```
...
- Canonical SPipe generation for source `9da6d3894536ab6cd6b92c324f3274c31291af9437218b4d04b254046d8b2b60`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `9da6d3894536ab6cd6b92c324f3274c31291af9437218b4d04b254046d8b2b60`.
...
source: test/01_unit/compiler/backend/vhdl_backend_spec.spl
mirror: doc/06_spec/01_unit/compiler/backend/vhdl_backend_spec.md (current)
findings: 5 blockers: 0
  narrative=100 structure=100 oracle=100
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/01_unit/compiler/backend/vhdl_backend_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/01_unit/compiler/backend/vhdl_backend_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/01_unit/compiler/backend/vhdl_backend_spec.spl:289:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'preserves named clock domain metadata in MIR domain helpers' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/01_unit/compiler/backend/vhdl_backend_spec.spl:309:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'resolves vhdl by backend name' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/01_unit/compiler/backend/vhdl_backend_spec.spl:316:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'lists VHDL as an available backend' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
