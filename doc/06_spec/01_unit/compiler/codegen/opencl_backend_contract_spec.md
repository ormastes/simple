...
> Tests covering OpenCL backend contract.
...
- names the OpenCL backend and supports OpenCL artifact targets only
   - Expected: backend.backend_name() equals `opencl`
   - Expected: backend.supports_target(CodegenTarget.OpenClC) is true
   - Expected: backend.supports_target(CodegenTarget.OpenClSpirv) is true
...
step("names the OpenCL backend and supports OpenCL artifact targets only")
...
- builds generated Engine2D OpenCL C to SPIR-V artifact evidence
   ...
   - Expected: contract.plan.source_format equals `opencl-c`
   - Expected: contract.plan.binary_format equals `spirv`
   - Expected: contract.plan.artifact_path_suffix equals `simple_2d_optimization.spirv`
...
```simple
# @req REQ-SSPEC-COMPILER
step("builds generated Engine2D OpenCL C to SPIR-V artifact evidence")
...
```simple
# @req REQ-SSPEC-COMPILER
step("rejects incomplete OpenCL generated artifact evidence")
...
- plans MIR kernels using OpenCL target metadata
   - Expected: OpenClBackend.accepts_gpu_kernel(opencl_kernel) is true
   - Expected: OpenClBackend.accepts_gpu_kernel(auto_kernel) is true
   - Expected: OpenClBackend.accepts_gpu_kernel(cuda_kernel) is false
   - Expected: plan.accepted_kernel_count equals `2`
   - Expected: plan.rejected_kernel_count equals `1`
...
```simple
# @req REQ-SSPEC-COMPILER
step("plans MIR kernels using OpenCL target metadata")
...
```simple
# @req REQ-SSPEC-COMPILER
step("emits OpenCL C source for accepted MIR GPU kernel subset")
...
- compiles tagged OpenCL kernels through the shared Codegen factory interface
   ...
   - Expected: output.name equals `gpu_target_module`
   - Expected: output.kind equals `CodegenOutputKind.GpuCode`
   - Expected: output.has_text_output is true
   - Expected: output.has_object_code is false
...
```simple
# @req REQ-SSPEC-COMPILER
step("compiles tagged OpenCL kernels through the shared Codegen factory interface")
val module = make_kernel_module([make_opencl_id_kernel()])
...
```simple
# @req REQ-SSPEC-COMPILER
step("emits OpenCL C load store and pointer arithmetic for memory kernels")
...
```simple
# @req REQ-SSPEC-COMPILER
step("emits OpenCL C checked arithmetic as GPU arithmetic")
...
```simple
# @req REQ-SSPEC-COMPILER
step("emits OpenCL C casts and bitcasts distinctly")
...
```simple
# @req REQ-SSPEC-COMPILER
step("emits OpenCL C direct named device calls")
...
```simple
# @req REQ-SSPEC-COMPILER
step("rejects unsupported MIR instead of emitting invalid OpenCL comments")
...
```simple
# @req REQ-SSPEC-COMPILER
step("emits OpenCL C array aggregates and field access")
...
```simple
# @req REQ-SSPEC-COMPILER
step("emits OpenCL C private allocation for per-work-item locals")
...
- maps Simple SIMD vector types to OpenCL C vector types
   - Expected: OpenClBackend.opencl_type(vec4f_type(), false) equals `float4`
   - Expected: OpenClBackend.opencl_type(vec8f_type(), false) equals `float8`
   - Expected: OpenClBackend.opencl_type(vec4d_type(), false) equals `double4`
   - Expected: OpenClBackend.opencl_type(vec4i_type(), false) equals `int4`
   - Expected: OpenClBackend.opencl_type(vec8i_type(), false) equals `int8`
...
```simple
# @req REQ-SSPEC-COMPILER
step("maps Simple SIMD vector types to OpenCL C vector types")
...
```simple
# @req REQ-SSPEC-COMPILER
step("emits OpenCL C SIMD vector arithmetic and f32x4 reductions")
...
```simple
# @req REQ-SSPEC-COMPILER
step("emits OpenCL C generic MirSimd vector operations")
...
```simple
# @req REQ-SSPEC-COMPILER
step("emits OpenCL C labels gotos and conditional branches")
...
```simple
# @req REQ-SSPEC-COMPILER
step("emits OpenCL C switch terminators with case and default gotos")
...
```simple
# @req REQ-SSPEC-COMPILER
step("emits OpenCL C atomics fences and local memory for GPU kernels")
...
expect(source).to_contain("// unsupported OpenCL atomic operation")
...
- Canonical SPipe generation for source `ce620bfe26dcddc67b260ee97d720ad9c502ed67f422438be570c4908a0083f2`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `ce620bfe26dcddc67b260ee97d720ad9c502ed67f422438be570c4908a0083f2`.
...
source: test/01_unit/compiler/codegen/opencl_backend_contract_spec.spl
mirror: doc/06_spec/01_unit/compiler/codegen/opencl_backend_contract_spec.md (current)
findings: 6 blockers: 0
  narrative=100 structure=100 oracle=80
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/01_unit/compiler/codegen/opencl_backend_contract_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/01_unit/compiler/codegen/opencl_backend_contract_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/01_unit/compiler/codegen/opencl_backend_contract_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-20): 2 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/01_unit/compiler/codegen/opencl_backend_contract_spec.spl:793:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'names the OpenCL backend and supports OpenCL artifact targets only' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/01_unit/compiler/codegen/opencl_backend_contract_spec.spl:804:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'builds generated Engine2D OpenCL C to SPIR-V artifact evidence' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/01_unit/compiler/codegen/opencl_backend_contract_spec.spl:819:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'rejects incomplete OpenCL generated artifact evidence' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
