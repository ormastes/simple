...
```simple
# @req REQ-SSPEC-APP
step("renders a fixed 1kHz tone SDN to a WAV whose PCM16 sample bytes exactly match the precomputed reference")
...
```simple
# @req REQ-SSPEC-APP
step("produces byte-identical WAV output across two separate render invocations")
...
```simple
# @req REQ-SSPEC-APP
step("applies a lowpass effect such that high-frequency sample magnitude is reduced versus an unfiltered render")
...
#### applies a pitch effect that halves the frame count at pitch_factor=2.0 (resample-based shift)

- applies a pitch effect that halves the frame count at pitch_factor=2.0 (resample-based shift)
- Render the same 8-frame tone with a pitch=2.0 effect
   - Expected: _write_fixtures() is true
   - Expected: r.exit_code equals `0`
- Then the WAV has half the data frames (4 instead of 8): 44-byte header + 4*2 bytes
   - Expected: bytes.len() equals `52`
...
```simple
# @req REQ-SSPEC-APP
step("applies a pitch effect that halves the frame count at pitch_factor=2.0 (resample-based shift)")
...
#### exits 1 with a path-qualified error when a pitch effect's pitch is <= 0

- exits 1 with a path-qualified error when a pitch effect's pitch is <= 0
- Render an SDN whose pitch effect has pitch=0
   - Expected: _write_fixtures() is true
...
```simple
# @req REQ-SSPEC-APP
step("exits 1 with a path-qualified error when a pitch effect's pitch is <= 0")
...
```simple
# @req REQ-SSPEC-APP
step("produces byte-identical WAV output across two separate reverb render invocations")
...
```simple
# @req REQ-SSPEC-APP
step("exits 1 with a path-qualified error when a reverb effect's room_size is out of [0,1]")
...
```simple
# @req REQ-SSPEC-APP
step("exits non-zero with a parse error message when given a malformed SDN file")
...
```simple
# @req REQ-SSPEC-APP
step("exits 2 when render is missing -o/--out")
...
```simple
# @req REQ-SSPEC-APP
step("mixes 2 clip events onto the timeline sample-accurately with gain — clip A alone, then A+B overlap, then B alone")
...
```simple
# @req REQ-SSPEC-APP
step("produces byte-identical WAV output across two separate sequence render invocations")
...
```simple
# @req REQ-SSPEC-APP
step("exits 1 with a path-qualified error when an event references a missing clip")
...
```simple
# @req REQ-SSPEC-APP
step("prints an ok summary and exits 0 for a well-formed sound.sdn")
...
```simple
# @req REQ-SSPEC-APP
step("exits 1 with a clear error for a missing sound file")
...
```simple
# @req REQ-SSPEC-APP
step("exits 2 with usage text when no subcommand is given")
...
```simple
# @req REQ-SSPEC-APP
step("exits 2 on an unknown subcommand")
...
- Canonical SPipe generation for source `e3b12e17e5226231ef674ee5745a258b165159a2630459abb568ca2c0f9bf672`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `e3b12e17e5226231ef674ee5745a258b165159a2630459abb568ca2c0f9bf672`.
...
source: test/01_unit/app/cli/sound_cmd_render_spec.spl
mirror: doc/06_spec/01_unit/app/cli/sound_cmd_render_spec.md (current)
findings: 6 blockers: 0
  narrative=100 structure=100 oracle=70
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/01_unit/app/cli/sound_cmd_render_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/01_unit/app/cli/sound_cmd_render_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/01_unit/app/cli/sound_cmd_render_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-30): 15 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/01_unit/app/cli/sound_cmd_render_spec.spl:111:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'renders a fixed 1kHz tone SDN to a WAV whose PCM16 sample bytes exactly match the precomputed reference' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/01_unit/app/cli/sound_cmd_render_spec.spl:137:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'produces byte-identical WAV output across two separate render invocations' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/01_unit/app/cli/sound_cmd_render_spec.spl:155:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'applies a lowpass effect such that high-frequency sample magnitude is reduced versus an unfiltered render' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
