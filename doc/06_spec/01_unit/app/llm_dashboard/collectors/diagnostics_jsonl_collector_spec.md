...
- remove file if exists
- write file
   - Expected: text does not expose the internal absence marker
   - Expected: text contains `last_event=PostToolUse`
   - Expected: text contains `last_session=none`
- remove file if exists
...
#### keeps missing diagnostics fields absence-safe
...
#### keeps missing files absence-safe

- remove file if exists
   - Expected: text does not expose the internal absence marker
   - Expected: text contains `last_event=none`
   - Expected: text contains `last_session=none`
...
- write file
   ...
   - Expected: html does not expose the internal absence marker
- remove file if exists
...
| Status | Active |
| Source | `test/01_unit/app/llm_dashboard/collectors/diagnostics_jsonl_collector_spec.spl` |
| Updated | 2026-06-01 |
