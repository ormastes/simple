...
> Validates that every MCP tool advertised in `tools/list` has a corresponding dispatch handler, every dispatch handler has a schema, and the tool count matches the expected total (80).
...
| Design | N/A |
| Research | N/A |
| Source | `test/01_unit/app/mcp_unit/mcp_inventory_alignment_spec.spl` |
| Updated | 2026-06-01 |
...
#### schema list has exactly 80 tools
...
```simple
# All 80 tool names from make_tools_list() in main_lazy_protocol.spl
val schema_tools = [
...
expect(schema_tools.len()).to_equal(80)
```

</details>

#### dispatch handler has exactly 80 tool branches
...
```simple
# All 80 tool names from dispatch_tool() in main.spl
val dispatch_tools = [
    "simple_read", "simple_check", "simple_symbols",
    "simple_status", "simple_diagnostics", "simple_edit",
    "simple_multi_edit", "simple_run",
...
expect(dispatch_tools.len()).to_equal(80)
```
...
expect(analysis_tools.len()).to_equal(4)
```
...
```simple
val total = 19 + 6 + 6 + 8 + 8 + 12 + 6 + 4 + 3 + 3 + 4 + 1
# 19 debug + 6 hw + 6 log + 8 diag + 8 vcs + 12 ui-access +
# 6 cli + 4 analysis + 3 task + 3 query + 4 daemon + 1 api = 80
expect(total).to_equal(80)
```
