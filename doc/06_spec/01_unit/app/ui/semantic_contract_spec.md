...
#### validates dispatch evidence against access read-after-write snapshots

1. var session = UISession new
   - Expected: semantic_ui_command_to_access_event_kind(command) equals `input_change`
   - Expected: semantic_ui_command_to_access_payload(command) equals `Grace`
   - Expected: evidence.dispatch_ok is true
   - Expected: evidence.revision_advanced is true
   - Expected: evidence.recent_event_matches is true
   - Expected: evidence.read_after_write_observed is true
   - Expected: evidence.ready is true
   - Expected: evidence.reason equals `pass`
...
val before = semantic_ui_snapshot_from_access(session.access_snapshot(), SEMANTIC_UI_STAGE_STATE, SEMANTIC_UI_STATUS_AVAILABLE)
val command = SemanticUiCommand.type_text("main", "name_input", "Grace")
val result = session.dispatch_semantic_command(command)
val after = semantic_ui_snapshot_from_access(session.access_snapshot(), SEMANTIC_UI_STAGE_STATE, SEMANTIC_UI_STATUS_AVAILABLE)
val evidence = semantic_ui_command_dispatch_evidence("headless", "main", before, after, command, result)

expect(semantic_ui_command_to_access_event_kind(command)).to_equal("input_change")
expect(semantic_ui_command_to_access_payload(command)).to_equal("Grace")
expect(evidence.dispatch_ok).to_equal(true)
expect(evidence.revision_advanced).to_equal(true)
expect(evidence.recent_event_matches).to_equal(true)
expect(evidence.read_after_write_observed).to_equal(true)
