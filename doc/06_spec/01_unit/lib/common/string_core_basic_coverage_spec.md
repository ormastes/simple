...
#### returns empty when a non-empty range starts beyond the string

- returns empty when a non-empty range starts beyond the string
- Exercise the start-at-or-beyond-length guard independently
   - Expected: str_slice("hello", 8, 9) equals ``
...
```simple
# @req REQ-SSPEC-LIB
step("returns empty when a non-empty range starts beyond the string")
step("Exercise the start-at-or-beyond-length guard independently")
expect(str_slice("hello", 8, 9)).to_equal("")
```
...
#### returns false when suffix is longer than the string

- returns false when suffix is longer than the string
- Exercise the suffix-length early return
   - Expected: str_ends_with("hi", "hello") is false
...
```simple
# @req REQ-SSPEC-LIB
step("returns false when suffix is longer than the string")
step("Exercise the suffix-length early return")
expect(str_ends_with("hi", "hello")).to_equal(false)
```
...
#### returns the byte length for an empty needle

- returns the byte length for an empty needle
- Exercise the empty-needle early return
   - Expected: str_last_index_of("hello", "") equals `5`
...
```simple
# @req REQ-SSPEC-LIB
step("returns the byte length for an empty needle")
step("Exercise the empty-needle early return")
expect(str_last_index_of("hello", "")).to_equal(5)
```
...
#### identifies vertical tab and form feed

- identifies vertical tab and form feed
- Exercise every supported ASCII whitespace spelling
   - Expected: is_whitespace_char("\u000B") is true
   - Expected: is_whitespace_char("\u000C") is true
...
```simple
# @req REQ-SSPEC-LIB
step("identifies vertical tab and form feed")
step("Exercise every supported ASCII whitespace spelling")
expect(is_whitespace_char("\u000B")).to_equal(true)
expect(is_whitespace_char("\u000C")).to_equal(true)
```
...
#### splits into scalar elements for an empty separator

- splits into scalar elements for an empty separator
- Exercise the empty-separator loop and its zero-iteration exit
   - Expected: parts.len() equals `3`
   - Expected: parts[0] equals `a`
   - Expected: parts[1] equals `b`
   - Expected: parts[2] equals `c`
...
```simple
# @req REQ-SSPEC-LIB
step("splits into scalar elements for an empty separator")
step("Exercise the empty-separator loop and its zero-iteration exit")
val parts = str_split("abc", "")
...
#### returns 0 for a non-ASCII scalar

- returns 0 for a non-ASCII scalar
- Exercise the non-empty value outside the ASCII byte range
   - Expected: char_code_inline("é") equals `0`
...
```simple
# @req REQ-SSPEC-LIB
step("returns 0 for a non-ASCII scalar")
step("Exercise the non-empty value outside the ASCII byte range")
...
#### returns vertical tab and form feed for 11 and 12

- returns vertical tab and form feed for 11 and 12
- Exercise the remaining dedicated control-code branches
   - Expected: char_from_code_inline(11) equals `\u000B`
   - Expected: char_from_code_inline(12) equals `\u000C`
...
```simple
# @req REQ-SSPEC-LIB
step("returns vertical tab and form feed for 11 and 12")
step("Exercise the remaining dedicated control-code branches")
expect(char_from_code_inline(11)).to_equal("\u000B")
expect(char_from_code_inline(12)).to_equal("\u000C")
```
...
#### encodes three-byte and four-byte Unicode scalars

- encodes three-byte and four-byte Unicode scalars
- Exercise every UTF-8 output-width branch
   - Expected: char_from_code_inline(0x20AC) equals `€`
   - Expected: char_from_code_inline(0x1F600) equals `😀`
...
```simple
# @req REQ-SSPEC-LIB
step("encodes three-byte and four-byte Unicode scalars")
step("Exercise every UTF-8 output-width branch")
expect(char_from_code_inline(0x20AC)).to_equal("€")
expect(char_from_code_inline(0x1F600)).to_equal("😀")
```
...
### string_core - String Manipulation

#### str_reverse

#### reverses non-empty and empty ASCII text

- reverses non-empty and empty ASCII text
- Exercise loop entry, continuation, and zero-iteration exit
   - Expected: str_reverse("abc") equals `cba`
...
```simple
# @req REQ-SSPEC-LIB
step("reverses non-empty and empty ASCII text")
step("Exercise loop entry, continuation, and zero-iteration exit")
expect(str_reverse("abc")).to_equal("cba")
...
#### repeats positive counts and rejects non-positive counts

- repeats positive counts and rejects non-positive counts
- Exercise early return and builder loop outcomes
   - Expected: str_repeat("ab", 3) equals `ababab`
   - Expected: str_repeat("ab", 0) equals ``
   - Expected: str_repeat("ab", -1) equals ``
...
```simple
# @req REQ-SSPEC-LIB
step("repeats positive counts and rejects non-positive counts")
step("Exercise early return and builder loop outcomes")
expect(str_repeat("ab", 3)).to_equal("ababab")
expect(str_repeat("ab", 0)).to_equal("")
expect(str_repeat("ab", -1)).to_equal("")
```
...
#### handles empty, fitting, and oversized text

- handles empty, fitting, and oversized text
- Exercise both early returns and truncation
   - Expected: str_truncate("", 3) equals ``
   - Expected: str_truncate("abc", 3) equals `abc`
   - Expected: str_truncate("abcdef", 3) equals `abc...`
...
```simple
# @req REQ-SSPEC-LIB
step("handles empty, fitting, and oversized text")
step("Exercise both early returns and truncation")
expect(str_truncate("", 3)).to_equal("")
expect(str_truncate("abc", 3)).to_equal("abc")
expect(str_truncate("abcdef", 3)).to_equal("abc...")
```
...
#### covers fitting and expanding left padding

- covers fitting and expanding left padding
- Exercise the width guard in both directions
   - Expected: str_pad_left("abc", 2, "0") equals `abc`
   - Expected: str_pad_left("abc", 5, "0") equals `00abc`
...
```simple
# @req REQ-SSPEC-LIB
step("covers fitting and expanding left padding")
step("Exercise the width guard in both directions")
expect(str_pad_left("abc", 2, "0")).to_equal("abc")
expect(str_pad_left("abc", 5, "0")).to_equal("00abc")
```

</details>

#### covers fitting and expanding right padding

- covers fitting and expanding right padding
- Exercise the width guard in both directions
   - Expected: str_pad_right("abc", 2, "0") equals `abc`
   - Expected: str_pad_right("abc", 5, "0") equals `abc00`
...
```simple
# @req REQ-SSPEC-LIB
step("covers fitting and expanding right padding")
step("Exercise the width guard in both directions")
expect(str_pad_right("abc", 2, "0")).to_equal("abc")
expect(str_pad_right("abc", 5, "0")).to_equal("abc00")
```

</details>

#### covers fitting, even, and odd center padding

- covers fitting, even, and odd center padding
- Exercise the center guard and both padding distributions
   - Expected: str_center("abc", 2) equals `abc`
   - Expected: str_center("abc", 5) equals ` abc `
   - Expected: str_center("abc", 6) equals ` abc  `
...
```simple
# @req REQ-SSPEC-LIB
step("covers fitting, even, and odd center padding")
step("Exercise the center guard and both padding distributions")
expect(str_center("abc", 2)).to_equal("abc")
expect(str_center("abc", 5)).to_equal(" abc ")
expect(str_center("abc", 6)).to_equal(" abc  ")
```
...
- Canonical SPipe generation for source `079cf202c616acd222030d3a60e5bc3f3dba972c4a76820b9d037ee7f49941f9`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `079cf202c616acd222030d3a60e5bc3f3dba972c4a76820b9d037ee7f49941f9`.
...
source: test/01_unit/lib/common/string_core_basic_coverage_spec.spl
mirror: doc/06_spec/01_unit/lib/common/string_core_basic_coverage_spec.md (current)
findings: 6 blockers: 0
  narrative=100 structure=100 oracle=70
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/01_unit/lib/common/string_core_basic_coverage_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/01_unit/lib/common/string_core_basic_coverage_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: scope, assumptions/preconditions, primary workflow, evidence, unsupported/limitations, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/01_unit/lib/common/string_core_basic_coverage_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-30): 64 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/01_unit/lib/common/string_core_basic_coverage_spec.spl:81:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'returns length of normal string' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/01_unit/lib/common/string_core_basic_coverage_spec.spl:88:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'returns 0 for empty string' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/01_unit/lib/common/string_core_basic_coverage_spec.spl:94:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'returns 1 for single character' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
