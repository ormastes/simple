...
The browser serializes one parsed URL into an ASCII request target shared by
HTTP/1.1, HTTP/2, and WebSocket handshakes. The scenario protects request-line
framing while preserving URL component delimiters and valid percent escapes.

**Requirements:** REQ-WEB-BROWSER-010, REQ-WEB-BROWSER-011

## Scenario: Serialize one safe target across HTTP and WebSocket transports

1. Resolve redirect references and build producer controls.
   - Relative `next?q=1#fresh` resolves into path `/dir/next`, query `q=1`,
     and fragment `fresh`; its request target is `/dir/next?q=1`.
   - Fragment-only `#fresh` preserves path `/dir/page` and query `old=1`;
     its request target remains `/dir/page?old=1` with no encoded `%23`.
   - Root-relative, query-only, and relative-path locations containing
     `https://id.test/` as query data remain on authority `safe.test`.
   - Spaces become `%20`, NBSP becomes its UTF-8 bytes `%C2%A0`, CR/LF become
     `%0D%0A`, a valid `%2f` escape remains unchanged, and a stray `%` becomes
     `%25` exactly once.
   - Path `/` and query `&`, `=`, and `+` remain component delimiters.
   - Authority, user information, and fragment data are absent from the target.
2. Serialize the exact target into the HTTP/1.1 request line.
3. Serialize the exact target into the HTTP/2 `:path` pseudo-header.
4. Serialize the exact target into the WebSocket opening handshake.

The three transport outputs must equal:

```text
/folder%20name/%C2%A0/%2f/%25%0D%0A?q=a%20b&x=1+2&z=%C2%A0&raw=%25
```
