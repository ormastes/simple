...
### rejects CPU fallback, incomplete buffers, and missing device provenance

Pins exact 256-pixel and deterministic checksum checks before accepting the
16x16 Vulkan receipt.
...
Requires Windows to retain exact counts, canonical checksums, device identity,
and direct device-readback provenance rather than a post-present host cache.
