# simpleos_host_gpu_image_execution_contract_spec
...
### SimpleOS host GPU IMAGE and TEXT execution
...
- admits resources only through the fresh-device Draw IR path with strict provenance
   - Expected: daemon does not contain `if decoded_resources.resources.len() > 0`
   - Expected: exact_body does not contain `if not wholly_bounded:\n        return false`
   - Expected: draw_ir.split("for pixel in pixels:").len() equals `2`
...
step("admits resources only through the fresh-device Draw IR path with strict provenance")
