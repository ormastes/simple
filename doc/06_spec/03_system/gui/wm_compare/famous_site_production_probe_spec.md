...
#### fails closed when residual pixel diagnostics are hidden
...
#### summarizes the production text-compositing deficit for renderer tuning
...
expect(rt_file_exists(famous_site_sample_production_report_sdn_path("site_0_google"))).to_equal(true)
val result = rt_process_run_timeout("node", [tool_path, "--limit=1"], 10000)
expect(result.2).to_equal(0)
