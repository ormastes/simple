...
The matrix passes only when all eleven rows are `pass` and the required
...
expect(stdout).to_contain("simpleos_hardening_matrix_passed=11/11")
...
expect(stdout).to_contain("simpleos_hardening_matrix_passed=10/11")
expect(stdout).to_contain("simpleos_hardening_qemu_host_counterpart_status=pass")
