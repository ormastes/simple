...
checks that the semantic widget target has a hit rectangle and that widget and
WM Draw IR commands retain nonempty parent metadata. The WM lane additionally
requires hit metadata on every emitted command.

Run with the admitted pure-Simple test runner and asserted BDD execution:

```text
bin/release/simple test test/03_system/app/ui.browser/feature/draw_ir_web_renderer_event_ancestry_spec.spl --mode=interpreter --assert-ran --fail-fast
```
