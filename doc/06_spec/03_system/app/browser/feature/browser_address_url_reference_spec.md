...
> URL-reference forms resolve against the committed HTTPS document before a navigation command crosses the renderer boundary. Invalid absolute hosts fail closed without replacing the editable draft or committed page.
...
1. Resolve only URL-reference forms against the committed document.
2. Submit a root-relative address while retaining the committed page.
3. Send only the resolved absolute URL across the worker wire.
4. Reject an invalid absolute host without losing editable or committed state.

The shared resolver covers root-, query-, fragment-, current-directory-, and
parent-directory-relative inputs. BrowserSession textual UI and the hosted
semantic adapter retain the committed body and history until response commit.
The registry resolves before encoding its command. The canonical navigation,
document, and history identity retains its fragment while only the transport
fetch and response URL omit it. A successful transport response commits the
fragment-bearing pending identity; genuinely different responses remain stale.
Invalid commands, including commands carrying a nonempty parent snapshot,
leave draft, focus, DOM identity, document, and history unchanged.
...
```simple
describe "Browser address URL-reference resolution":

    # codex-system-test
    # @req REQ-WEB-BROWSER-007 REQ-WEB-BROWSER-008 REQ-WEB-BROWSER-009
    it "should resolve URL references before the worker boundary and reject invalid hosts atomically":
        ...
        expect(hosted_submit.callback_count).to_equal(1)
        ...
        expect(registry_submit.callback_count).to_equal(1)
        ...
        expect(registry.entries[0].renderer.history_index).to_equal(0)
...
|-------------|----------|
| REQ-WEB-BROWSER-007 | Address references resolve through textual and hosted controls |
| REQ-WEB-BROWSER-008 | Invalid input preserves focus, draft, page, and history |
| REQ-WEB-BROWSER-009 | Registry sends only an absolute canonical navigation URL |
