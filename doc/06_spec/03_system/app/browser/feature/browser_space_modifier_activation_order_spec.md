...
- Focus both buttons through the host Tab route
   ...
   - Expected: hosted_tab.callback_count equals `1`
...
- Hold Space while pressing and releasing Shift on both buttons
   ...
   - Expected: hosted_space_down.callback_count equals `1`
   - Expected: hosted_shift_down.callback_count equals `1`
   - Expected: hosted_shift_up.callback_count equals `1`
...
- Release Space and observe ordered activation in both renderers
   ...
   - Expected: hosted_space_up.callback_count equals `2`
   ...
   - Expected: hosted.browser.dom_callback_count equals `6`
   - Expected: worker.browser.dom_callback_count equals `6`
...
step("should retain Space activation while Shift is pressed and released")
...
expect(hosted_tab.callback_count).to_equal(1)
...
expect(hosted_space_down.callback_count).to_equal(1)
expect(hosted_shift_down.callback_count).to_equal(1)
expect(hosted_shift_up.callback_count).to_equal(1)
...
expect(hosted_space_up.callback_count).to_equal(2)
...
expect(hosted.browser.pending_space_activation_target).to_equal("")
expect(worker.browser.pending_space_activation_target).to_equal("")
expect(hosted.browser.dom_callback_count).to_equal(6)
expect(worker.browser.dom_callback_count).to_equal(6)
hosted.close()
```
...
- Canonical SPipe generation for source `011987fcec028d8f768588981eae6fd13ae688287ce2a72e69416610b347262d`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `011987fcec028d8f768588981eae6fd13ae688287ce2a72e69416610b347262d`.
...
doc/06_spec/03_system/app/browser/feature/browser_space_modifier_activation_order_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, unsupported/limitations, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/03_system/app/browser/feature/browser_space_modifier_activation_order_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-30): 7 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/03_system/app/browser/feature/browser_space_modifier_activation_order_spec.spl:51:1: advice SSDOC-BEH-002 [structure] (-5): scenario name 'should retain Space activation while Shift is pressed and released' describes the test rather than its outcome
  why: Outcome names describe product behavior rather than test mechanics.
  improve: Rename it to the observable product outcome.
test/03_system/app/browser/feature/browser_space_modifier_activation_order_spec.spl:51:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'should retain Space activation while Shift is pressed and released' has no retained capture or evidence
