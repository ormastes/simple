...
> Focusing the address bar selects its complete committed URL. One Backspace clears that selection without navigating or mutating the committed document.
...
1. Commit pages and capture their navigation and rendered state.
2. Focus each address bar and select its complete value.
3. Press Backspace once through every executable chrome route.
4. Keep focus while clearing the selection and preserving the page.

The hosted session, isolated renderer worker, and parent renderer registry are
the executable routes. They assert the cleared draft, repeated Backspace
remaining empty, retained focus, cleared replace flag, next input starting from
empty, unchanged committed URL/history/body, and byte-identical pixels. The
direct hosted entry has no narrow executable harness here; this scenario checks
only that its source branch is bound to the same validated helper.
...
|-------------|----------|
| REQ-WEB-BROWSER-007 | Selection-all Backspace clears the editable address |
| REQ-WEB-BROWSER-008 | Committed navigation, document, and pixels are preserved |
