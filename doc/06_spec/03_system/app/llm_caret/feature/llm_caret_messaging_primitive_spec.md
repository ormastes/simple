...
- Recover messaging state after restart
   ...
   - Expected: store.audit_count() equals `1`
   - Expected: store.close() is true
...
step("should persist rooms and monotonically ordered messages after restart")
...
expect(store.audit_count()).to_equal(1)
expect(store.close()).to_equal(true)
file_delete(path)
```
...
- Observe task and receipt transitions
   - Expected: page.len() equals `1`
   - Expected: page[0].message_id.value equals `p-2`
   - Expected: page[0].room_seq equals `2`
   - Expected: store.close() is true
...
```simple
# @req REQ-SSPEC-SYSTEM
step("should paginate history strictly after the supplied room sequence")
step("Create and bind a room")
...
expect(page.len()).to_equal(1)
expect(page[0].message_id.value).to_equal("p-2")
expect(page[0].room_seq).to_equal(2)
expect(store.close()).to_equal(true)
file_delete(path)
```
...
- Create and bind a room
   ...
   - Expected: store.message_history("missing", 0, 10).len() equals `0`
   - Expected: store.close() is true
...
```simple
# @req REQ-SSPEC-SYSTEM
step("should reject a message for a room that does not exist")
step("Create and bind a room")
...
expect(store.message_history("missing", 0, 10).len()).to_equal(0)
expect(store.close()).to_equal(true)
file_delete(path)
```
...
- Observe task and receipt transitions
   ...
   - Expected: duplicate.room_seq equals `1`
   - Expected: store.message_history("dedup", 0, 10).len() equals `1`
   - Expected: store.close() is true
...
```simple
# @req REQ-SSPEC-SYSTEM
step("should return the original sequence for a repeated idempotency key")
step("Observe task and receipt transitions")
...
expect(duplicate.room_seq).to_equal(1)
expect(store.message_history("dedup", 0, 10).len()).to_equal(1)
expect(store.close()).to_equal(true)
file_delete(path)
```
...
- Observe task and receipt transitions
   ...
   - Expected: store.close() is true
- Recover messaging state after restart
   - Expected: store.cursor("cursor-room", "reader-1") equals `1`
   - Expected: store.cursor("cursor-room", "reader-2") equals `0`
   - Expected: store.close() is true
...
```simple
# @req REQ-SSPEC-SYSTEM
step("should persist an independently advanced room cursor")
step("Observe task and receipt transitions")
...
step("Recover messaging state after restart")
store = PureSqlMessagingStore.open(path)
expect(store.cursor("cursor-room", "reader-1")).to_equal(1)
expect(store.cursor("cursor-room", "reader-2")).to_equal(0)
expect(store.close()).to_equal(true)
file_delete(path)
```
...
```simple
# @req REQ-SSPEC-SYSTEM
step("should accept one canonical inbound event for repeated transport delivery")
step("Observe task and receipt transitions")
...
- Recover messaging state after restart
   - Expected: store.close() is true
   - Expected: public_history.len() equals `1`
   - Expected: public_history[0].body equals `visible`
   - Expected: private_history.len() equals `1`
...
```simple
# @req REQ-SSPEC-SYSTEM
step("should keep direct-room content out of public-room history")
step("Create and bind a room")
...
expect(public_history.len()).to_equal(1)
expect(public_history[0].body).to_equal("visible")
expect(private_history.len()).to_equal(1)
...
```simple
# @req REQ-SSPEC-SYSTEM
step("should persist a queued delivery and classify a retry")
step("Observe task and receipt transitions")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("should recover queued outbox state after reopening the database")
step("Observe task and receipt transitions")
...
- Observe task and receipt transitions
   ...
   - Expected: store.dead_letter_count() equals `1`
...
```simple
# @req REQ-SSPEC-SYSTEM
step("should move a permanently exhausted delivery to dead-letter state")
step("Observe task and receipt transitions")
...
expect(store.dead_letter_count()).to_equal(1)
...
- Canonical SPipe generation for source `07ac434b7c26ed5b55bd7a1304d29279591a3e3fb6597c10de0e0f0f43c47824`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `07ac434b7c26ed5b55bd7a1304d29279591a3e3fb6597c10de0e0f0f43c47824`.
...
doc/06_spec/03_system/app/llm_caret/feature/llm_caret_messaging_primitive_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, unsupported/limitations
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/03_system/app/llm_caret/feature/llm_caret_messaging_primitive_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-30): 16 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/03_system/app/llm_caret/feature/llm_caret_messaging_primitive_spec.spl:53:1: advice SSDOC-BEH-002 [structure] (-5): scenario name 'should persist rooms and monotonically ordered messages after restart' describes the test rather than its outcome
  why: Outcome names describe product behavior rather than test mechanics.
  improve: Rename it to the observable product outcome.
test/03_system/app/llm_caret/feature/llm_caret_messaging_primitive_spec.spl:53:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'should persist rooms and monotonically ordered messages after restart' has no retained capture or evidence
...
test/03_system/app/llm_caret/feature/llm_caret_messaging_primitive_spec.spl:84:1: advice SSDOC-BEH-002 [structure] (-5): scenario name 'should paginate history strictly after the supplied room sequence' describes the test rather than its outcome
  why: Outcome names describe product behavior rather than test mechanics.
  improve: Rename it to the observable product outcome.
test/03_system/app/llm_caret/feature/llm_caret_messaging_primitive_spec.spl:84:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'should paginate history strictly after the supplied room sequence' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/03_system/app/llm_caret/feature/llm_caret_messaging_primitive_spec.spl:103:1: advice SSDOC-BEH-002 [structure] (-5): scenario name 'should reject a message for a room that does not exist' describes the test rather than its outcome
  why: Outcome names describe product behavior rather than test mechanics.
  improve: Rename it to the observable product outcome.
test/03_system/app/llm_caret/feature/llm_caret_messaging_primitive_spec.spl:103:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'should reject a message for a room that does not exist' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/03_system/app/llm_caret/feature/llm_caret_messaging_primitive_spec.spl:118:1: advice SSDOC-BEH-002 [structure] (-5): scenario name 'should return the original sequence for a repeated idempotency key' describes the test rather than its outcome
  why: Outcome names describe product behavior rather than test mechanics.
  improve: Rename it to the observable product outcome.
test/03_system/app/llm_caret/feature/llm_caret_messaging_primitive_spec.spl:136:1: advice SSDOC-BEH-002 [structure] (-5): scenario name 'should persist an independently advanced room cursor' describes the test rather than its outcome
  why: Outcome names describe product behavior rather than test mechanics.
  improve: Rename it to the observable product outcome.
test/03_system/app/llm_caret/feature/llm_caret_messaging_primitive_spec.spl:154:1: advice SSDOC-BEH-002 [structure] (-5): scenario name 'should accept one canonical inbound event for repeated transport delivery' describes the test rather than its outcome
  why: Outcome names describe product behavior rather than test mechanics.
  improve: Rename it to the observable product outcome.
