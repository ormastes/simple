...
step("cannot MUTATE tenant-A records from a tenant-B session")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("gets 403 on every back-office read and write it can name")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("refuses a revoked, an expired, and a cross-tenant bearer token")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("does not hand out a guessable or colliding token when the caller supplies CONSTANT entropy")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("rejects entropy too short to have come from a CSPRNG (W13-C boundary check)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("binds SQL payloads and escapes hostile names on every rendered surface")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("refuses a valid signature lifted onto a different provider_ref / kind / event")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("applies the request-limit and throttle rungs to the unauthenticated auth routes")
...
- The 4th request in the SAME window is rejected
   - Expected: throttle_count(store, "tenant-a|anon", throttle_window(30, 60)) equals `3`
- A second key in the same window is counted independently
   - Expected: throttle_count(store, "tenant-a|login:bob", throttle_window(30, 60)) equals `1`
   - Expected: throttle_count(store, "tenant-a|anon", throttle_window(30, 60)) equals `3`
- Crossing the window boundary re-admits, and the count is the NEW window's
   - Expected: throttle_count(store, "tenant-a|anon", throttle_window(60, 60)) equals `1`
- The elapsed window's rows are gone — they can never change a later decision
   - Expected: throttle_count(store, "tenant-a|anon", throttle_window(0, 60)) equals `0`
   - Expected: throttle_rows_retained(store) equals `1`
...
- ...and the live window still decides correctly on that bounded data
   - Expected: throttle_count(store, "tenant-a|anon", throttle_window(last, 60)) equals `2`
...
```simple
# @req REQ-SSPEC-SYSTEM
step("keeps the count correct across a window boundary AND keeps retained rows bounded")
...
expect(throttle_count(store, "tenant-a|anon", throttle_window(30, 60))).to_equal(3)
...
expect(throttle_count(store, "tenant-a|login:bob", throttle_window(30, 60))).to_equal(1)
expect(throttle_count(store, "tenant-a|anon", throttle_window(30, 60))).to_equal(3)
...
expect(throttle_count(store, "tenant-a|anon", throttle_window(60, 60))).to_equal(1)
step("The elapsed window's rows are gone — they can never change a later decision")
expect(throttle_count(store, "tenant-a|anon", throttle_window(0, 60))).to_equal(0)
expect(throttle_rows_retained(store)).to_equal(1)
...
val last = 301 * 60
expect(throttle_count(store, "tenant-a|anon", throttle_window(last, 60))).to_equal(2)
...
- charset recombination) — but never manufacture a '<', '>' or quote
   - Expected: two.1.len() equals `7`
   - Expected: three.1.len() equals `6`
   - Expected: mixed.len() equals `10`
...
```simple
# @req REQ-SSPEC-SYSTEM
step("decodes exactly once and fails closed on every malformed escape")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("does NOT weaken escaping: a decoded XSS payload still renders escaped")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("role-gates /booking/* and /restaurant/* reads with each family's own frozen action")
...
expect(http_status_code(r1.status)).to_equal(403)
...
expect(http_status_code(r2.status)).to_equal(403)
...
expect(http_status_code(r3.status)).to_equal(403)
...
expect(http_status_code(ok1.status)).to_equal(200)
...
expect(http_status_code(r4.status)).to_equal(403)
...
expect(http_status_code(a1.status)).to_equal(200)
...
expect(http_status_code(a2.status)).to_equal(404)
store_close(store)
```
...
- Canonical SPipe generation for source `44b2cefa45ffcb8932cd2d8c68c8aebb19c5f6a4a9572f8f67974599eed25148`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `44b2cefa45ffcb8932cd2d8c68c8aebb19c5f6a4a9572f8f67974599eed25148`.
...
doc/06_spec/03_system/app/enterprise/enterprise_security_audit_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, unsupported/limitations
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/03_system/app/enterprise/enterprise_security_audit_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-30): 44 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/03_system/app/enterprise/enterprise_security_audit_spec.spl:175:1: warning SSDOC-BEH-001 [structure] (-10): scenario 'leaks nothing across ANY of the catalog, hcm, proc, booking, dashboard routes' has no visible step flow
  why: Ordered visible actions make the manual operable.
  improve: Add ordered step("...") calls for meaningful actions.
test/03_system/app/enterprise/enterprise_security_audit_spec.spl:217:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'cannot MUTATE tenant-A records from a tenant-B session' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/03_system/app/enterprise/enterprise_security_audit_spec.spl:250:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'gets 403 on every back-office read and write it can name' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/03_system/app/enterprise/enterprise_security_audit_spec.spl:289:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'refuses a revoked, an expired, and a cross-tenant bearer token' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
