...
> Validates the lightweight inventory gate that ties the WHATWG HTML element inventory and implemented Simple Web CSS subset to the 50-case Electron/Simple layout fixture source. This is fixture-assignment evidence only. It cannot satisfy executable HTML/CSS behavior without the qualified runner, semantic, DrawIR, Engine2D, and current-manual evidence required by the canonical plan.
...
Validates the lightweight inventory gate that ties the WHATWG HTML element
inventory and implemented Simple Web CSS subset to the 50-case Electron/Simple
layout fixture source. This is fixture-assignment evidence only. It cannot
satisfy executable HTML/CSS behavior without the qualified runner, semantic,
DrawIR, Engine2D, and current-manual evidence required by the canonical plan.
...
This gate validates fixture-manifest accounting.

It does not execute Electron.

It does not execute the Simple Web renderer.

It does not execute Draw IR lowering.

It does not execute Engine2D.

It does not prove browser events.

It does not prove a CSS declaration changes computed style.

It does not prove an HTML element has correct semantics.

It does not prove an unsupported feature fails closed.

It does not promote fixture-source occurrence into behavioral coverage.

Its PASS means the inventory contract is internally consistent.

Behavioral acceptance remains owned by the executable traceability gate.

## Inputs

The WHATWG element index supplies the diagnostic HTML inventory.

The canonical traceability script supplies the claimed implemented CSS inventory.

The 50-case layout manifest supplies required scene identifiers.

The Electron fixture producer source supplies fixture HTML and CSS text.

The offline test mode prevents network dependence in this contract spec.

Every input path is emitted in normalized evidence.

Input counts are compared to their producing evidence rather than copied constants.

The CSS count is derived from the canonical source evidence.

The manifest case count remains exactly 50.

Every manifest scene must have a fixture-source assignment.

## Evidence classes

`html_tag_count` is the diagnostic inventory denominator.

`html_tag_covered_count` is fixture-source occurrence accounting.

`html_tag_fixture_covered_count` excludes manifest prose-only occurrence.

`html_tag_manifest_only_count` exposes names found only in the manifest.

`html_tag_missing` exposes names absent from fixture source.

`css_property_count` is the current claimed implemented-property inventory.

`css_property_covered_count` is fixture-source occurrence accounting.

`css_property_missing` exposes names absent from fixture source.

`manifest_case_count` is the parsed case inventory.

`manifest_missing_fixture` exposes scene identifiers without fixture source.

None of these fields is an execution count.

## Trust model

The checker reads source artifacts directly.

Manifest prose cannot satisfy fixture-element coverage.

A property name in the traceability script cannot alone prove rendering.

A fixture containing a declaration cannot prove the declaration was consumed.

The aggregate behavioral checker must retain that distinction.

This inventory gate may pass while behavioral status is evidence-blocked.

That split is intentional and must remain visible in generated manuals.

No runner path or runner SHA is accepted here.

No target execution receipt is synthesized here.

No screenshot is accepted as semantic or layout proof here.

## Frozen step

The primary displayed step is:

`Verify executable HTML and CSS traceability`

In this file the step introduces an inventory-accounting subgate.

It does not redefine the step as executable renderer acceptance.

The manual must preserve this qualification next to the scenario.

## Failure discrimination

`unexpected-manifest-case-count` means the required 50-case matrix was truncated.

`manifest-scenes-missing-fixture-html` means one or more scene IDs lack fixture text.

`html-tags-missing-rendering-fixture` means fixture occurrence is incomplete.

`css-properties-missing-rendering-fixture` means fixture occurrence is incomplete.

`css-property-source-not-pass` means the upstream inventory producer is unavailable.

A truncated manifest fails even when all remaining fixture files exist.

An added claimed property fails until fixture source names it.

Neither failure is reported as a runtime renderer mismatch.

## Examples

Example: the complete canonical manifest and fixture source are checked offline.

Expected result: inventory contract PASS with derived counts.

Example: one manifest row is removed.

Expected result: FAIL with `unexpected-manifest-case-count`.

Example: a synthetic property is added to the claimed source inventory.

Expected result: FAIL with `css-properties-missing-rendering-fixture`.

Example: all names occur but no qualified runner exists.

Expected result: this inventory gate may pass while behavioral status remains
`evidence-blocked`.

Example: an old report contains `248`.

Expected result: the executable spec derives the current count and does not
hardcode that historical number.

## Current evidence state

The checked manifest contains 50 cases.

The CSS count is derived, not frozen at 248 or 132.

The scenario includes positive and adversarial fixture-accounting checks.

No target HTML/CSS behavior execution is claimed.

No RenderDoc capture is claimed.

No Electron launch is claimed.

No bootstrap or doc-generation run is claimed by this authored prose.
...
- The gate writes stable `html_css_rendering_manifest_traceability_*` evidence
  keys.
- The diagnostic HTML inventory has fixture-source occurrence accounting;
  manifest prose alone is not sufficient.
- Claimed implemented Simple Web CSS properties appear in fixture source.
...
- A fixture-assignment PASS must never be reported as renderer execution PASS.
- The generated manual must retain the evidence-class and claim-boundary text.
...
- Read the emitted evidence contract
   ...
   - Expected: css_count equals `css_source_count`
   - Expected: css_covered equals `css_source_count`
...
val css_source_count = _value_of(evidence, "html_css_rendering_manifest_traceability_css_property_source_count")
expect(css_source_count.len()).to_be_greater_than(0)
...
expect(css_count).to_equal(css_source_count)
expect(css_covered).to_equal(css_source_count)
...
expect(report).to_contain("- implemented CSS properties: " + css_source_count + "/" + css_source_count)
```
