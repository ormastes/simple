...
- All 129 implemented Simple Web CSS properties appear in actual rendered
  fixture CSS.
...
- Read the emitted evidence contract
   ...
   - Expected: css_count equals `129`
   - Expected: css_covered equals `129`
   - Expected: css_covered_names.split(",").len() equals `129`
...
expect(css_count).to_equal("129")
expect(css_covered).to_equal("129")
...
expect(css_covered_names.split(",").len()).to_equal(129)
...
expect(report).to_contain("- implemented CSS properties: 129/129")
```
