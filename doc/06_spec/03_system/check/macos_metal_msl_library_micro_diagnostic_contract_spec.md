...
1. Admit only the canonical self-hosted compiler and Metal runtime providers
   ...
   manifest.
2. Require the exact Metal availability, initialization, device, library, and
   last-error provider symbols before building.
3. Link the diagnostic with `SIMPLE_NO_STUB_FALLBACK=1`.
4. Run native compilation and the diagnostic through the same bounded process
   ...
   checker.
5. Use dyld logging to prove the manifest-bound Metal provider was loaded.
6. Reject missing fields, a non-64-character source hash, any CPU fallback, or
