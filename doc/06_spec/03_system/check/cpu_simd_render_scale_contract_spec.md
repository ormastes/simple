...
# @req REQ-CPU-SIMD-SCALE-013
# @req REQ-CPU-SIMD-SCALE-002
# @req REQ-CPU-SIMD-SCALE-003
# @req REQ-CPU-SIMD-SCALE-004
# @req REQ-CPU-SIMD-SCALE-005
# @req REQ-CPU-SIMD-SCALE-006
# @req REQ-CPU-SIMD-SCALE-007
# @req REQ-CPU-SIMD-SCALE-008
# @req REQ-CPU-SIMD-SCALE-009
# @req REQ-CPU-SIMD-SCALE-010
# @req REQ-CPU-SIMD-SCALE-011
# @req REQ-CPU-SIMD-SCALE-012
# @req REQ-CPU-SIMD-SCALE-001
# @req REQ-CPU-SIMD-SCALE-013
# @req REQ-CPU-SIMD-SCALE-002
# @req REQ-CPU-SIMD-SCALE-003
...
step("exports software baseline timing comparison fields")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("external cpu drawing baseline comparison records benchmark scope")
...
- software exporter routes the configured backend through the shared Draw IR executor
   - Expected: exporter does not contain `web_render_backend`
   - Expected: exporter does not contain `parsed as i32`
   - Expected: exporter does not contain `simple_web_render_html_to_pixels_with_engine2d_backend`
   - Expected: renderer does not contain `CpuBackend`
   - Expected: renderer does not contain `_cpu_draw_ir_`
   - Expected: renderer does not contain `draw_gradient_rect`
   - Expected: renderer does not contain `draw_rounded_rect_outline`
   - Expected: renderer does not contain `set_clip(`
   - Expected: driver does not contain `err.span`
   - Expected: hir_types does not contain `span.file`
   - Expected: font_renderer does not contain `_resolved_font_metric_lock.unlock(0)`
   - Expected: font_renderer does not contain `var found: ResolvedFontMetrics?`
   - Expected: emu_adv does not contain `core.draw_image_blend(`
   - Expected: emu does not contain `fn emu_draw_image_blend(mut core: RenderBackend`
   - Expected: engine does not contain `vulkan.completion_unknown = true`
   - Expected: engine does not contain `evidence.fallback_pixels = []`
   - Expected: engine does not contain `self.backend.draw_image_blend(`
...
```simple
# @req REQ-SSPEC-SYSTEM
step("software exporter routes the configured backend through the shared Draw IR executor")
val exporter = file_read("src/app/wm_compare/backend_measurement_software_export.spl")
expect(exporter).to_contain("use std.nogc_sync_mut.gpu.engine2d.simd_provider.{reset_simd_hits, simd_hit_counts}")
expect(exporter).to_contain("use std.nogc_sync_mut.io_runtime.{get_args}")
...
expect(driver).to_contain("HIR lowering error in {name}{err.location}: {err.message}")
expect(driver.contains("err.span")).to_equal(false)
val hir_types = file_read("src/compiler/20.hir/hir_lowering/types.spl")
expect(hir_types).to_contain("location: text\n    span: Span")
expect(hir_types).to_contain("location: \"\",\n            span: span")
expect(hir_types.contains("span.file")).to_equal(false)
...
expect(font_renderer).to_contain("_resolved_font_metric_lock_release(active_lock)\n            return found")
expect(font_renderer.contains("_resolved_font_metric_lock.unlock(0)")).to_equal(false)
...
- typed software exporter reports true nearest rank percentiles
   - Expected: percentile_body does not contain `sorted[sorted.len() - 1]`
...
```simple
# @req REQ-SSPEC-SYSTEM
step("typed software exporter reports true nearest rank percentiles")
val exporter = file_read("src/app/wm_compare/backend_measurement_software_export.spl")
...
- native array repeat fills the framebuffer with memcpy doubling
   - Expected: repeat_body does not contain `rt_array_push`
   - Expected: simd_alloc_body does not contain `rt_array_new(n)`
   - Expected: rust_repeat_body does not contain `rt_array_push`
...
```simple
# @req REQ-SSPEC-SYSTEM
step("native array repeat fills the framebuffer with memcpy doubling")
...
val simd_source = file_read("src/runtime/runtime_simd_dispatch.c")
val simd_alloc_start = simd_source.find("static SplArray* engine2d_new_pixel_array")
val simd_alloc_body = simd_source.slice(simd_alloc_start, simd_alloc_start + 260)
expect(source).to_contain("SplArray* rt_array_new_uninit(int64_t cap)")
expect(source).to_contain("return rt_core_array_new_fill(cap, 0, 0)")
expect(simd_alloc_body).to_contain("rt_array_new_uninit(n)")
expect(simd_alloc_body.contains("rt_array_new(n)")).to_equal(false)
...
- browser layout framebuffers use the safe owner fill facade
   - Expected: source does not contain `rt_engine2d_simd_fill_u32`
   - Expected: source does not contain `engine2d_simd_fill_row_u32`
   - Expected: source does not contain `rt_u32_alloc_filled`
...
```simple
# @req REQ-SSPEC-SYSTEM
step("browser layout framebuffers use the safe owner fill facade")
val source = file_read("src/lib/gc_async_mut/gpu/browser_engine/simple_web_html_layout_renderer.spl") +
    file_read("src/lib/gc_async_mut/gpu/browser_engine/simple_web_html_layout_renderer_core.spl") +
    file_read("src/lib/gc_async_mut/gpu/browser_engine/simple_web_html_layout_renderer_paint_layout.spl")
expect(source).to_contain("fn browser_layout_framebuffer_filled")
expect(source).to_contain("use std.nogc_async_mut.gpu.engine2d.simd_kernels.{engine2d_simd_fill_rows_u32}")
expect(source).to_contain("if pixels >= 1048576: 8 else: 1")
expect(source).to_contain("engine2d_simd_fill_rows_u32(width.to_i64(), height.to_i64(), base, workers)")
expect(source).to_contain("browser_layout_framebuffer_filled_serial(base, width, height)")
...
val app = file_read("src/app/ui.browser/app.spl")
expect(app).to_contain("val BROWSER_WINDOW_SCALE: i32 = 1")
expect(app).to_contain("val width: i32 = 672")
expect(app).to_contain("val height: i32 = 448")
```
...
- Canonical SPipe generation for source `3c579227cf7237de88a1d05c47b0a64efe4a6d6b3227b1126c15a49ab1bae906`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `3c579227cf7237de88a1d05c47b0a64efe4a6d6b3227b1126c15a49ab1bae906`.
...
doc/06_spec/03_system/check/cpu_simd_render_scale_contract_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, assumptions/preconditions, primary workflow, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/03_system/check/cpu_simd_render_scale_contract_spec.spl:1:1: blocker SSDOC-ORA-002 [oracle] (-50): scenario relies on source-text inspection as system evidence
  why: Source presence or self-created arithmetic does not demonstrate production behavior.
  improve: Observe runtime behavior or a stable generated artifact instead.
test/03_system/check/cpu_simd_render_scale_contract_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-30): 7 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/03_system/check/cpu_simd_render_scale_contract_spec.spl:129:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'exports software baseline timing comparison fields' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/03_system/check/cpu_simd_render_scale_contract_spec.spl:216:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'external cpu drawing baseline comparison records benchmark scope' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/03_system/check/cpu_simd_render_scale_contract_spec.spl:235:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'software exporter routes the configured backend through the shared Draw IR executor' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
