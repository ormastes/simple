...
expect(evidence).to_contain("rdoc_simple_gate_reason=missing-capture-log-path")
...
#### classifies capture log failures before replay failures
...
```simple
val command = "RDOC_SIMPLE_GATE_CAPTURE_LOG_SELF_TEST=1 sh scripts/check/check-renderdoc-simple-gate.shs"
val (stdout, _stderr, code) = process_run("/bin/sh", ["-c", command])
expect(code).to_equal(0)
expect(stdout).to_contain("rdoc_simple_gate_capture_log_self_test_status=pass")
expect(stdout).to_contain("rdoc_simple_gate_capture_log_self_test_reason=missing-capture-log-path")
expect(stdout).to_contain("rdoc_simple_gate_capture_log_self_test_reason=capture-log-symlink")
expect(stdout).to_contain("rdoc_simple_gate_capture_log_self_test_reason=capture-log-missing")
expect(stdout).to_contain("rdoc_simple_gate_capture_log_self_test_reason=missing-capture-log-sha256")
expect(stdout).to_contain("rdoc_simple_gate_capture_log_self_test_reason=invalid-capture-log-sha256")
expect(stdout).to_contain("rdoc_simple_gate_capture_log_self_test_reason=capture-log-sha256-mismatch")
val gate = file_read("scripts/check/check-renderdoc-simple-gate.shs")
expect(gate).to_contain("elif [ \"$capture_log_reason\" != \"\" ]; then")
expect(gate).to_contain("elif [ \"$replay_status\" != \"pass\" ]")
```
...
expect(_count_key(passing, "rdoc_simple_gate_replay_xml_file_bytes")).to_equal(1)
expect(_count_key(passing, "rdoc_simple_gate_capture_log_path")).to_equal(1)
expect(_count_key(passing, "rdoc_simple_gate_capture_log_file_sha256")).to_equal(1)
expect(host_renderdoc_evidence_passes(passing)).to_be(true)
