...
- All 132 implemented Simple Web CSS properties are rendered in fixture CSS.
- The current full CSS inventory is tested as 394 properties, with 262 still
  unrendered and 269 held in unsupported-inventory ownership.
...
- Read the full rendering goal evidence
   ...
   - Expected: full_css_rendered equals `132`
   - Expected: full_css_unrendered equals `262`
   - Expected: unsupported_inventory equals `269`
   - Expected: full_css_unrendered_properties.split(",").len() equals `262`
...
expect(evidence).to_contain("html_css_full_rendering_goal_implemented_css_total_count=132")
expect(evidence).to_contain("html_css_full_rendering_goal_implemented_css_rendered_count=132")
...
expect(evidence).to_contain("html_css_full_rendering_goal_full_css_rendered_count=132")
...
expect(full_css_rendered).to_equal("132")
expect(full_css_unrendered).to_equal("262")
expect(unsupported_inventory).to_equal("269")
expect(full_css_unrendered_properties.split(",").len()).to_equal(262)
...
expect(report).to_contain("- implemented CSS rendered: 132/132")
