...
- All 130 implemented Simple Web CSS properties are rendered in fixture CSS.
- The current full CSS inventory is tested as 394 properties, with 264 still
  unrendered and 271 held in unsupported-inventory ownership.
...
- Read the full rendering goal evidence
   ...
   - Expected: full_css_rendered equals `130`
   - Expected: full_css_unrendered equals `277`
   - Expected: unsupported_inventory equals `271`
   - Expected: full_css_unrendered_properties.split(",").len() equals `277`
...
expect(evidence).to_contain("html_css_full_rendering_goal_implemented_css_total_count=130")
expect(evidence).to_contain("html_css_full_rendering_goal_implemented_css_rendered_count=130")
...
expect(evidence).to_contain("html_css_full_rendering_goal_full_css_rendered_count=130")
...
expect(full_css_rendered).to_equal("130")
expect(full_css_unrendered).to_equal("277")
expect(unsupported_inventory).to_equal("271")
expect(full_css_unrendered_properties.split(",").len()).to_equal(277)
...
expect(report).to_contain("- implemented CSS rendered: 130/130")
