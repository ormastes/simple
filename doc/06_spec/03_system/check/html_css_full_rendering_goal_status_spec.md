...
- All 125 implemented Simple Web CSS properties are rendered in fixture CSS.
- The current full CSS inventory is tested as 394 properties, with 269 still
  unrendered and 276 held in unsupported-inventory ownership.
...
- Read the full rendering goal evidence
   ...
   - Expected: full_css_rendered equals `125`
   - Expected: full_css_unrendered equals `277`
   - Expected: unsupported_inventory equals `276`
...
expect(evidence).to_contain("html_css_full_rendering_goal_implemented_css_total_count=125")
expect(evidence).to_contain("html_css_full_rendering_goal_implemented_css_rendered_count=125")
...
expect(evidence).to_contain("html_css_full_rendering_goal_full_css_rendered_count=125")
...
expect(full_css_rendered).to_equal("125")
expect(full_css_unrendered).to_equal("277")
expect(unsupported_inventory).to_equal("276")
...
expect(report).to_contain("- implemented CSS rendered: 125/125")
