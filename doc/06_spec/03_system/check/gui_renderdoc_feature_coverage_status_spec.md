...
expect(evidence).to_contain("gui_showcase_4k_200fps_warmup_frames=12")
expect(evidence).to_contain("gui_showcase_4k_200fps_frame_sample_count=200")
...
val command = "rm -rf build/test-gui-renderdoc-feature-coverage-status-4k-sample-window && mkdir -p build/test-gui-renderdoc-feature-coverage-status-4k-sample-window/source && " +
    ...
    "GUI_SHOWCASE_4K_PERF_ENV=build/test-gui-renderdoc-feature-coverage-status-4k-sample-window/source/status.env GUI_RENDERDOC_AGGREGATE_STATIC_CACHE_DIR=build/test-gui-renderdoc-feature-coverage-static-cache BUILD_DIR=build/test-gui-renderdoc-feature-coverage-status-4k-sample-window/out REPORT_PATH=build/test-gui-renderdoc-feature-coverage-status-4k-sample-window/report.md GUI_RENDERDOC_AGGREGATE_PRINT_ENV=0 sh scripts/check/check-gui-renderdoc-feature-coverage-status.shs"
val (_stdout, _stderr, code) = process_run("/bin/sh", ["-c", command])
expect(code).to_equal(0)

step("Require the aggregate to reject percentiles from an incomplete retained window")
val evidence = file_read("build/test-gui-renderdoc-feature-coverage-status-4k-sample-window/out/evidence.env")
expect(evidence).to_contain("gui_showcase_4k_200fps_status=fail")
expect(evidence).to_contain("gui_showcase_4k_200fps_reason=untrusted-4k-sample-window:warmup=12;samples=199;frames=200")
```
...
expect(evidence).to_contain("gui_showcase_8k_perf_warmup_frames=12")
expect(evidence).to_contain("gui_showcase_8k_perf_frame_sample_count=200")
