...
- Production parity subchecks are bounded by default so a direct wrapper run
  cannot spin indefinitely when a nested GUI/Electron probe stalls.
- The heavy wrapper requires an explicit current-source Stage4 binary and its
  canonical provenance receipt before any nested renderer check runs.
...
- Backend-executed parity must promote same-frame readback, positive backend
  handles, and matching software/CPU/Metal checksum rows before production
  parity can pass.
- A complete fixture pass must promote Chrome live-capture provenance,
  ...
  top level.
- Promoted Tauri iOS Metal render-log gate rows must appear exactly once so
  duplicated aggregation output cannot hide stale or conflicting evidence.
...
- The wrapper never searches release, Stage3, repo, or PATH fallbacks.
- The explicit binary must pass the canonical Stage4 provenance verifier.
- Rejected binary admission exits nonzero before nested checks run.
- The admitted binary, source, and status are exported to nested subchecks.
...
- A missing explicit Stage4 binary must fail without discovery or fallback.
- An unreceipted, stale, or source-mismatched Stage4 binary must fail before
  nested renderer checks execute.
...
- A timed-out nested check must not erase partial evidence already written by
  that check.
- A direct production parity run must not depend on the caller remembering to
  set a timeout environment variable.
...
- exports an admitted Stage4 Simple binary to nested production parity checks
   - Expected: script does not contain `for candidate in`
   - Expected: script does not contain `command -v simple`
...
step("exports an admitted Stage4 Simple binary to nested production parity checks")
val script = file_read("scripts/check/check-production-gui-web-renderer-parity-evidence.shs")
expect(script).to_contain("SIMPLE_BIN_SOURCE=\"explicit-stage4\"")
expect(script).to_contain("SIMPLE_BIN_STATUS=pass")
expect(script).to_contain("jj log -r @ --no-pager --no-graph -T 'commit_id.short(12)'")
expect(script.contains("for candidate in")).to_equal(false)
expect(script.contains("command -v simple")).to_equal(false)
expect(script).to_contain("stage4_verify_candidate_provenance")
expect(script).to_contain("SIMPLE_BIN_STATUS=unprovenanced")
...
expect(script).to_contain("ELECTRON_LAYOUT_MANIFEST_RESUME=\"$LAYOUT_MANIFEST_RESUME\"")
expect(script).to_contain("PRODUCTION_GUI_WEB_RENDERER_PARITY_EVENT_SCRIPT")
expect(script).to_contain("run_event_routing")
expect(script).to_contain("PRODUCTION_GUI_WEB_RENDERER_PARITY_METAL_RENDER_LOG_SCRIPT")
expect(script).to_contain("PRODUCTION_GUI_WEB_RENDERER_PARITY_METAL_BROWSER_BACKING_SCRIPT")
expect(script).to_contain("check-macos-metal-browser-backing-evidence.shs")
expect(script).to_contain("check-macos-metal-render-log-compare.shs")
expect(script).to_contain("run_metal_render_log_compare")
expect(script).to_contain("MACOS_METAL_BROWSER_ENV")
expect(script).to_contain("production_gui_web_renderer_parity_metal_render_log_status")
expect(script).to_contain("production_gui_web_renderer_parity_metal_render_log_required_api")
expect(script).to_contain("production_gui_web_renderer_parity_metal_render_log_pairwise_gate_status")
expect(script).to_contain("production_gui_web_renderer_parity_metal_render_log_argb_source_gate_status")
expect(script).to_contain("production_gui_web_renderer_parity_metal_render_log_tauri_ios_gate_status")
expect(script).to_contain("production_gui_web_renderer_parity_metal_render_log_tauri_ios_env_file_status")
expect(script).to_contain("production_gui_web_renderer_parity_metal_render_log_tauri_ios_env_artifact_status")
expect(script).to_contain("production_gui_web_renderer_parity_metal_render_log_require_tauri_ios")
expect(script).to_contain("production_gui_web_renderer_parity_metal_render_log_tauri_ios_render_log_validation_status")
expect(script).to_contain("metal-render-log-compare-failed")
expect(script).to_contain("production_gui_web_renderer_parity_event_routing_status")
expect(script).to_contain("production_gui_web_renderer_parity_event_routing_performance_now_available")
expect(script).to_contain("production_gui_web_renderer_parity_event_routing_input_to_paint_ms")
...
expect(script).to_contain("production_gui_web_renderer_parity_event_routing_proof_symlink_status")
...
expect(script).to_contain("production_gui_web_renderer_parity_event_routing_chrome_process_version")
...
expect(script).to_contain("num_positive_at_most \"$performance_now_delta_ms\" 1000")
...
expect(script).to_contain("production_gui_web_renderer_parity_backend_metal_command_queue_handle=$metal_command_queue_handle")
expect(script).to_contain("production_gui_web_renderer_parity_backend_software_checksum=$software_checksum")
expect(script).to_contain("production_gui_web_renderer_parity_backend_cpu_simd_checksum=$cpu_simd_checksum")
expect(script).to_contain("production_gui_web_renderer_parity_backend_metal_checksum=$metal_checksum")
expect(script).to_contain("production_gui_web_renderer_parity_backend_metal_gpu_readback_checksum=$metal_gpu_checksum")
expect(script).to_contain("production_gui_web_renderer_parity_backend_checksum_match=$backend_checksum_match")
expect(script).to_contain("production_gui_web_renderer_parity_backend_same_frame_readback=$same_frame_readback")
expect(script).to_contain("production_gui_web_renderer_parity_backend_readback_source=$backend_readback_source")
expect(script).to_contain("[ \"$backend_checksum_match\" = \"true\" ]")
expect(script).to_contain("[ \"$same_frame_readback\" = \"true\" ]")
expect(script).to_contain("[ \"$backend_readback_source\" = \"device_readback\" ]")
expect(script).to_contain("[ \"$software_checksum\" = \"$cpu_simd_checksum\" ]")
expect(script).to_contain("num_at_least \"$metal_command_queue_handle\" 1")
expect(script).to_contain("backend_checksum_match_final")
expect(script).to_contain("backend_same_frame_readback_final")
expect(script).to_contain("backend_readback_source_final")
expect(script).to_contain("backend_metal_command_queue_handle_final")
expect(script).to_contain("[ \"$backend_checksum_match_final\" != \"true\" ]")
expect(script).to_contain("[ \"$backend_readback_source_final\" != \"device_readback\" ]")
expect(script).to_contain("[ \"$backend_software_checksum_final\" != \"$backend_cpu_simd_checksum_final\" ]")
expect(script).to_contain("num_at_least \"$backend_metal_command_queue_handle_final\" 1")
expect(script).to_contain("num_at_least \"$animation_frame_count\" 2")
expect(script).to_contain("[ \"$browser_engine\" = \"chromium\" ]")
expect(script).to_contain("electron_chromium_user_agent \"$electron_user_agent\"")
expect(script).to_contain("version_text \"$electron_process_version\"")
...
expect(script).to_contain("metal_engine2d_framebuffer_readback_simple_bin")
expect(script).to_contain("production_gui_web_renderer_parity_metal_readback_simple_bin=$simple_bin")
expect(script).to_contain("production_gui_web_renderer_parity_metal_readback_simple_bin_source=$simple_bin_source")
expect(script).to_contain("production_gui_web_renderer_parity_metal_readback_simple_bin_status=$simple_bin_status")
```
...
```simple
# @req REQ-SSPEC-SYSTEM
step("rejects an unreceipted browser binary")
step("Reject an unreceipted browser binary")
val root = "build/test-production-gui-web-stage4-unreceipted"
val setup = make_stage4_receipt_fixture(root, false) +
    " && rm -f " + root + "/simple.provenance.env"
val (_setup_out, _setup_err, setup_code) =
    process_run("/bin/sh", ["-c", setup])
expect(setup_code).to_equal(0)
val (_stdout, _stderr, code) = run_renderer_evidence_wrapper(
    "scripts/check/check-production-gui-web-renderer-parity-evidence.shs",
    root
)
expect(code).to_equal(1)
expect(file_read(root + "/out/evidence.env")).to_contain(
...
```simple
# @req REQ-SSPEC-SYSTEM
step("rejects a stale Stage4 receipt")
step("Reject a stale Stage4 receipt")
val root = "build/test-production-gui-web-stage4-stale"
val (_setup_out, _setup_err, setup_code) = process_run(
    "/bin/sh", ["-c", make_stage4_receipt_fixture(root, true)]
)
expect(setup_code).to_equal(0)
val (_stdout, _stderr, code) = run_renderer_evidence_wrapper(
    "scripts/check/check-production-gui-web-renderer-parity-evidence.shs",
...
```simple
# @req REQ-SSPEC-SYSTEM
step("keeps every renderer evidence entrypoint on the canonical Stage4 gate")
val entrypoints = [
    "scripts/check/check-production-gui-web-renderer-parity-evidence.shs",
    "scripts/check/check-wm-browser-event-routing-evidence.shs",
    "scripts/check/produce-aetheric-host-web-gui-evidence.shs",
    "scripts/check/check-aetheric-host-web-gui-evidence.shs"
]
for entrypoint in entrypoints:
    val source = file_read(entrypoint)
    ...
    expect(source).to_contain(".provenance.env")
```
...
```simple
# @req REQ-SSPEC-SYSTEM
step("prints production parity blocker summary rows for operator triage")
val script = file_read("scripts/check/check-production-gui-web-renderer-parity-evidence.shs")
expect(script).to_contain("emit_summary_env()")
expect(script).to_contain("production_gui_web_renderer_parity_surface_manifest_tauri_live_capture")
expect(script).to_contain("production_gui_web_renderer_parity_surface_manifest_chrome_live_capture")
expect(script).to_contain("production_gui_web_renderer_parity_font_offload_vector_readback_status")
expect(script).to_contain("production_gui_web_renderer_parity_font_offload_bitmap_readback_status")
expect(script).to_contain("production_gui_web_renderer_parity_metal_render_log_status")
expect(script).to_contain("production_gui_web_renderer_parity_metal_render_log_blocked_gate_count")
expect(script).to_contain("production_gui_web_renderer_parity_event_routing_status")
```
...
```simple
# @req REQ-SSPEC-SYSTEM
step("rejects Rust seed Simple binary in Metal framebuffer readback evidence")
val script = file_read("scripts/check/check-metal-engine2d-framebuffer-readback-evidence.shs")
...
expect(script).to_contain("export SIMPLE_BIN SIMPLE_BIN_SOURCE SIMPLE_BIN_STATUS")
expect(script).to_contain("metal_engine2d_framebuffer_readback_simple_bin=$SIMPLE_BIN")
expect(script).to_contain("metal_engine2d_framebuffer_readback_simple_bin_source=$SIMPLE_BIN_SOURCE")
expect(script).to_contain("metal_engine2d_framebuffer_readback_simple_bin_status=$SIMPLE_BIN_STATUS")

val root = "build/test-metal-engine2d-framebuffer-readback-seed-forbidden"
val command = "rm -rf " + root + " && SIMPLE_BIN=src/compiler_rust/target/release/simple BUILD_DIR=" + root + "/out REPORT_PATH=" + root + "/report.md sh scripts/check/check-metal-engine2d-framebuffer-readback-evidence.shs || true"
val (_stdout, _stderr, code) = process_run("/bin/sh", ["-c", command])
expect(code).to_equal(0)

val evidence = file_read(root + "/out/evidence.env")
expect(evidence).to_contain("metal_engine2d_framebuffer_readback_status=unavailable")
expect(evidence).to_contain("metal_engine2d_framebuffer_readback_reason=simple-bin-forbidden")
expect(evidence).to_contain("metal_engine2d_framebuffer_readback_simple_bin=src/compiler_rust/target/release/simple")
expect(evidence).to_contain("metal_engine2d_framebuffer_readback_simple_bin_status=forbidden")
```
...
```simple
# @req REQ-SSPEC-SYSTEM
step("selects only self hosted backend simple candidates by default")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("preserves explicit backend simple bin overrides in missing-bin evidence")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("rejects explicit backend rust seed simple bin evidence")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("records bounded subcheck timeout evidence")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("prints bounded summary output while keeping full evidence on disk")
...
val command = "rm -rf " + root + " && mkdir -p " + root + "/fixture && " +
    ...
    "SIMPLE_WEB_FONT_RUN_ID=fixture-run PRODUCTION_GUI_WEB_RENDERER_PARITY_MATRIX_SCRIPT=" + root + "/fixture/matrix.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_LAYOUT_SCRIPT=" + root + "/fixture/layout.sh BUILD_ROOT=" + root + "/out REPORT_PATH=" + root + "/report.md sh scripts/check/check-production-gui-web-renderer-parity-evidence.shs || true"
...
expect(stdout.length()).to_be_less_than(1400)
```
...
val command = "rm -rf " + root + " && mkdir -p " + root + "/fixture && " +
    ...
    "SIMPLE_WEB_FONT_RUN_ID=fixture-run PRODUCTION_GUI_WEB_RENDERER_PARITY_MATRIX_SCRIPT=" + root + "/fixture/matrix.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_LAYOUT_SCRIPT=" + root + "/fixture/layout.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_SURFACE_SCRIPT=" + root + "/fixture/surface.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_BACKEND_SCRIPT=" + root + "/fixture/backend.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_FONT_OFFLOAD_SCRIPT=" + root + "/fixture/font.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_METAL_READBACK_SCRIPT=" + root + "/fixture/metal.sh BUILD_ROOT=" + root + "/out REPORT_PATH=" + root + "/report.md sh scripts/check/check-production-gui-web-renderer-parity-evidence.shs || true"
val (_stdout, _stderr, code) = process_run("/bin/sh", ["-c", command])
expect(code).to_equal(0)
...
expect(evidence).to_contain("production_gui_web_renderer_parity_layout_manifest_status=fail")
...
expect(evidence).to_contain("production_gui_web_renderer_parity_font_offload_reason=fixture-font-unavailable")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_readback_status=unavailable")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_readback_reason=metal-requires-macos")
```
...
```simple
# @req REQ-SSPEC-SYSTEM
step("derives partial layout manifest counts after timeout")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("records Metal readback evidence even when font offload is unavailable")
...
expect(evidence).to_contain("production_gui_web_renderer_parity_surface_manifest_tauri_capture_reason=macos-wkwebview-snapshot-backend-unimplemented")
expect(evidence).to_contain("production_gui_web_renderer_parity_surface_manifest_tauri_capture_backend=macos-wkwebview-snapshot-unimplemented")
expect(evidence).to_contain("production_gui_web_renderer_parity_surface_manifest_tauri_capture_required_commands=macos-wkwebview-snapshot-backend")
expect(evidence).to_contain("production_gui_web_renderer_parity_surface_manifest_tauri_capture_missing_commands=macos-wkwebview-snapshot-backend")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("emits surface manifest host and capture provenance without downstream fallbacks")
...
expect(evidence).to_contain("tauri_chrome_simple_web_layout_manifest_tauri_capture_backend=")
expect(evidence).to_contain("tauri_chrome_simple_web_layout_manifest_tauri_capture_required_commands=")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("runs macOS Metal render-log compare when Metal readback exists even if backend row is fallback")
val root = "build/test-production-gui-web-renderer-parity-metal-log-readback-present"
val command = "rm -rf " + root + " && mkdir -p " + root + "/fixture && " +
    "printf '#!/bin/sh\\nmkdir -p \"$BUILD_ROOT\"\\nprintf \"electron_generated_gui_web_matrix_status=pass\\\\nelectron_generated_gui_web_matrix_reason=pass\\\\n\" > \"$BUILD_ROOT/evidence.env\"\\n' > " + root + "/fixture/matrix.sh && " +
    ...
    "printf '#!/bin/sh\\nmkdir -p \"$BUILD_DIR\"\\nprintf \"tauri_chrome_simple_web_layout_manifest_status=fail\\\\ntauri_chrome_simple_web_layout_manifest_reason=tauri-macos-wkwebview-snapshot-backend-unimplemented\\\\ntauri_chrome_simple_web_layout_manifest_host_uname_s=Darwin\\\\ntauri_chrome_simple_web_layout_manifest_host_uname_m=arm64\\\\ntauri_chrome_simple_web_layout_manifest_electron_capture_status=pass\\\\ntauri_chrome_simple_web_layout_manifest_tauri_capture_status=unavailable\\\\ntauri_chrome_simple_web_layout_manifest_tauri_capture_reason=macos-wkwebview-snapshot-backend-unimplemented\\\\ntauri_chrome_simple_web_layout_manifest_tauri_capture_backend=macos-wkwebview-snapshot-unimplemented\\\\ntauri_chrome_simple_web_layout_manifest_tauri_capture_required_commands=macos-wkwebview-snapshot-backend\\\\ntauri_chrome_simple_web_layout_manifest_tauri_capture_missing_commands=macos-wkwebview-snapshot-backend\\\\ntauri_chrome_simple_web_layout_manifest_tauri_live_capture=false\\\\ntauri_chrome_simple_web_layout_manifest_chrome_capture_status=pass\\\\ntauri_chrome_simple_web_layout_manifest_chrome_capture_reason=pass\\\\ntauri_chrome_simple_web_layout_manifest_chrome_capture_backend=chrome-live-bitmap\\\\ntauri_chrome_simple_web_layout_manifest_chrome_live_capture=true\\\\ntauri_chrome_simple_web_layout_manifest_tauri_case_count=0\\\\ntauri_chrome_simple_web_layout_manifest_tauri_pass_count=0\\\\ntauri_chrome_simple_web_layout_manifest_tauri_tracked_count=0\\\\ntauri_chrome_simple_web_layout_manifest_tauri_fail_count=0\\\\ntauri_chrome_simple_web_layout_manifest_tauri_argb_artifact_pass_count=0\\\\ntauri_chrome_simple_web_layout_manifest_tauri_argb_artifact_fail_count=0\\\\ntauri_chrome_simple_web_layout_manifest_chrome_case_count=1\\\\ntauri_chrome_simple_web_layout_manifest_chrome_pass_count=1\\\\ntauri_chrome_simple_web_layout_manifest_chrome_tracked_count=0\\\\ntauri_chrome_simple_web_layout_manifest_chrome_fail_count=0\\\\ntauri_chrome_simple_web_layout_manifest_chrome_argb_artifact_pass_count=1\\\\ntauri_chrome_simple_web_layout_manifest_chrome_argb_artifact_fail_count=0\\\\ntauri_chrome_simple_web_layout_manifest_tauri_mismatch_count=0\\\\ntauri_chrome_simple_web_layout_manifest_chrome_mismatch_count=0\\\\ntauri_chrome_simple_web_layout_manifest_no_fake_capture=true\\\\ntauri_chrome_simple_web_layout_manifest_blur_or_tolerance_used=false\\\\n\" > \"$BUILD_DIR/evidence.env\"\\nexit 1\\n' > " + root + "/fixture/surface.sh && " +
    "printf '#!/bin/sh\\nmkdir -p \"$BUILD_DIR\"\\nprintf \"production_gui_backend_status=pass\\\\nproduction_gui_backend_reason=pass\\\\nproduction_gui_backend_exact_backend_parity=true\\\\nproduction_gui_backend_cpu_simd_different_pixels=0\\\\nproduction_gui_backend_metal_resolved=vulkan-unavailable\\\\nproduction_gui_backend_metal_different_pixels=0\\\\nproduction_gui_backend_metal_gpu_frame_complete=false\\\\nproduction_gui_backend_metal_gpu_readback_checksum=0\\\\nproduction_gui_backend_metal_command_queue_handle=0\\\\nproduction_gui_backend_software_checksum=900\\\\nproduction_gui_backend_cpu_simd_checksum=900\\\\nproduction_gui_backend_metal_checksum=900\\\\nproduction_gui_backend_backend_checksum_match=true\\\\nproduction_gui_backend_same_frame_backend_readback=true\\\\nproduction_gui_backend_readback_source=software_reference\\\\nproduction_gui_backend_blur_or_tolerance_used=false\\\\nproduction_gui_backend_sample_count=3\\\\nproduction_gui_backend_total_elapsed_us_min=90\\\\nproduction_gui_backend_total_elapsed_us_avg=100\\\\nproduction_gui_backend_total_elapsed_us_max=120\\\\nproduction_gui_backend_total_pixels_per_second_min=2000000\\\\nproduction_gui_backend_total_pixels_per_second_avg=2400000\\\\nproduction_gui_backend_total_pixels_per_second_max=2800000\\\\n\" > \"$BUILD_DIR/evidence.env\"\\n' > " + root + "/fixture/backend.sh && " +
    "printf '#!/bin/sh\\nmkdir -p \"$BUILD_DIR\"\\nprintf \"production_gui_font_offload_status=pass\\\\nproduction_gui_font_offload_reason=pass\\\\nproduction_gui_font_offload_runtime_evidence_status=pass\\\\nproduction_gui_font_offload_vector_readback_status=pass\\\\nproduction_gui_font_offload_bitmap_readback_status=pass\\\\n\" > \"$BUILD_DIR/evidence.env\"\\n' > " + root + "/fixture/font.sh && " +
    "printf '#!/bin/sh\\nmkdir -p \"$BUILD_DIR\"\\nprintf \"metal_engine2d_framebuffer_readback_status=pass\\\\nmetal_engine2d_framebuffer_readback_reason=raw-metal-framebuffer-download-proven\\\\nmetal_engine2d_framebuffer_readback_spec_status=pass\\\\nmetal_engine2d_framebuffer_gpu_readback_available=true\\\\nmetal_engine2d_framebuffer_blur_or_tolerance_used=false\\\\n\" > \"$BUILD_DIR/evidence.env\"\\n' > " + root + "/fixture/metal.sh && " +
    "printf '#!/bin/sh\\nmkdir -p \"$BUILD_DIR\"\\nprintf \"macos_metal_browser_backing_status=pass\\\\nmacos_metal_pixel_comparison_status=pass\\\\n\" > \"$BUILD_DIR/evidence.env\"\\n' > " + root + "/fixture/browser.sh && " +
    "printf '#!/bin/sh\\nmkdir -p \"$BUILD_DIR\"\\nprintf \"macos_metal_render_log_compare_status=unavailable\\\\nmacos_metal_render_log_compare_reason=missing-macos-metal-browser-env\\\\nmacos_metal_render_log_compare_required_api=metal\\\\nmacos_metal_render_log_compare_generated_readback_gate_status=pass\\\\nmacos_metal_render_log_compare_framebuffer_readback_gate_status=pass\\\\nmacos_metal_render_log_compare_browser_backing_gate_status=missing\\\\nmacos_metal_render_log_compare_pairwise_gate_status=missing\\\\nmacos_metal_render_log_compare_argb_source_gate_status=missing\\\\nmacos_metal_render_log_compare_gpu_capture_gate_status=not-required\\\\nmacos_metal_render_log_compare_blocked_gate_count=3\\\\nmacos_metal_render_log_compare_blocked_gates=browser-metal-backing,pairwise-argb-diff,argb-source-evidence\\\\nmacos_metal_render_log_compare_pairwise_status=unavailable\\\\nmacos_metal_render_log_compare_gpu_capture_status=unavailable\\\\nmacos_metal_render_log_compare_gpu_capture_tool=xcode-gpu-frame-capture\\\\n\" > \"$BUILD_DIR/evidence.env\"\\nexit 1\\n' > " + root + "/fixture/metal-log.sh && " +
    "SIMPLE_WEB_FONT_RUN_ID=fixture-run PRODUCTION_GUI_WEB_RENDERER_PARITY_MATRIX_SCRIPT=" + root + "/fixture/matrix.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_LAYOUT_SCRIPT=" + root + "/fixture/layout.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_SURFACE_SCRIPT=" + root + "/fixture/surface.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_BACKEND_SCRIPT=" + root + "/fixture/backend.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_FONT_OFFLOAD_SCRIPT=" + root + "/fixture/font.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_METAL_READBACK_SCRIPT=" + root + "/fixture/metal.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_METAL_BROWSER_BACKING_SCRIPT=" + root + "/fixture/browser.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_METAL_RENDER_LOG_SCRIPT=" + root + "/fixture/metal-log.sh BUILD_ROOT=" + root + "/out REPORT_PATH=" + root + "/report.md sh scripts/check/check-production-gui-web-renderer-parity-evidence.shs || true"
val (_stdout, _stderr, code) = process_run("/bin/sh", ["-c", command])
expect(code).to_equal(0)
...
expect(evidence).to_contain("production_gui_web_renderer_parity_status=fail")
expect(evidence).to_contain("production_gui_web_renderer_parity_reason=tauri-chrome-layout-manifest-failed")
expect(evidence).to_contain("production_gui_web_renderer_parity_backend_metal_resolved=vulkan-unavailable")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_readback_status=pass")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_status=unavailable")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_reason=missing-macos-metal-browser-env")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_backend_resolved=vulkan-unavailable")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_metal_readback_status=pass")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_browser_backing_gate_status=missing")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_pairwise_gate_status=missing")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_blocked_gate_count=3")
```
...
```simple
# @req REQ-SSPEC-SYSTEM
step("fails top-level parity when Electron event routing validator details are missing")
val root = "build/test-production-gui-web-renderer-parity-event-fail"
val command = "rm -rf " + root + " && mkdir -p " + root + "/fixture && " +
...
```simple
# @req REQ-SSPEC-SYSTEM
step("fails top-level parity when Electron event timing or animation rows are invalid")
...
val bad_animation_command = "perl -0pi -e 's/wm_browser_event_routing_performance_now_delta_ms=1001/wm_browser_event_routing_performance_now_delta_ms=16.7/g; s/wm_browser_event_routing_input_to_paint_ms=1001/wm_browser_event_routing_input_to_paint_ms=18.4/g; s/wm_browser_event_routing_animation_frame_count=2/wm_browser_event_routing_animation_frame_count=1/g; s/wm_browser_event_routing_css_animation_probe=true/wm_browser_event_routing_css_animation_probe=false/g' " + root + "/fixture/event.sh && " +
    "SIMPLE_WEB_FONT_RUN_ID=fixture-run PRODUCTION_GUI_WEB_RENDERER_PARITY_MATRIX_SCRIPT=" + root + "/fixture/matrix.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_LAYOUT_SCRIPT=" + root + "/fixture/layout.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_SURFACE_SCRIPT=" + root + "/fixture/surface.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_BACKEND_SCRIPT=" + root + "/fixture/backend.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_FONT_OFFLOAD_SCRIPT=" + root + "/fixture/font.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_METAL_READBACK_SCRIPT=" + root + "/fixture/metal.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_EVENT_SCRIPT=" + root + "/fixture/event.sh BUILD_ROOT=" + root + "/out REPORT_PATH=" + root + "/report.md sh scripts/check/check-production-gui-web-renderer-parity-evidence.shs || true"
val (_animation_stdout, _animation_stderr, animation_code) = process_run("/bin/sh", ["-c", bad_animation_command])
expect(animation_code).to_equal(0)

val animation_evidence = file_read(root + "/out/evidence.env")
expect(animation_evidence).to_contain("production_gui_web_renderer_parity_status=fail")
expect(animation_evidence).to_contain("production_gui_web_renderer_parity_reason=event-routing-evidence-failed")
expect(animation_evidence).to_contain("production_gui_web_renderer_parity_event_routing_status=pass")
...
expect(animation_evidence).to_contain("production_gui_web_renderer_parity_event_routing_css_animation_probe=false")
```
...
- passes with complete Electron Chrome backend Metal and event proof rows
   - Expected: code equals `0`
   - Expected: _line_count(evidence, "production_gui_web_renderer_parity_metal_render_log_tauri_ios_gate_status=not-required") equals `1`
   - Expected: hardlinked_proof_code equals `0`
...
```simple
# @req REQ-SSPEC-SYSTEM
step("passes with complete Electron Chrome backend Metal and event proof rows")
...
val command = "rm -rf " + root + " && mkdir -p " + root + "/fixture && " +
    ...
    "printf '#!/bin/sh\\nmkdir -p \"$BUILD_DIR\"\\nprintf \"tauri_chrome_simple_web_layout_manifest_status=pass\\\\ntauri_chrome_simple_web_layout_manifest_reason=pass\\\\ntauri_chrome_simple_web_layout_manifest_host_uname_s=Darwin\\\\ntauri_chrome_simple_web_layout_manifest_host_uname_m=arm64\\\\ntauri_chrome_simple_web_layout_manifest_electron_capture_status=pass\\\\ntauri_chrome_simple_web_layout_manifest_tauri_capture_status=pass\\\\ntauri_chrome_simple_web_layout_manifest_tauri_capture_reason=pass\\\\ntauri_chrome_simple_web_layout_manifest_tauri_capture_backend=macos-wkwebview-snapshot\\\\ntauri_chrome_simple_web_layout_manifest_tauri_capture_required_commands=swift,node\\\\ntauri_chrome_simple_web_layout_manifest_tauri_capture_missing_commands=\\\\ntauri_chrome_simple_web_layout_manifest_chrome_capture_status=pass\\\\ntauri_chrome_simple_web_layout_manifest_chrome_capture_reason=pass\\\\ntauri_chrome_simple_web_layout_manifest_chrome_capture_backend=chrome-live-bitmap\\\\ntauri_chrome_simple_web_layout_manifest_tauri_live_capture=true\\\\ntauri_chrome_simple_web_layout_manifest_chrome_live_capture=true\\\\ntauri_chrome_simple_web_layout_manifest_tauri_case_count=3\\\\ntauri_chrome_simple_web_layout_manifest_tauri_pass_count=3\\\\ntauri_chrome_simple_web_layout_manifest_tauri_tracked_count=0\\\\ntauri_chrome_simple_web_layout_manifest_tauri_fail_count=0\\\\ntauri_chrome_simple_web_layout_manifest_tauri_argb_artifact_pass_count=3\\\\ntauri_chrome_simple_web_layout_manifest_tauri_argb_artifact_fail_count=0\\\\ntauri_chrome_simple_web_layout_manifest_chrome_case_count=3\\\\ntauri_chrome_simple_web_layout_manifest_chrome_pass_count=3\\\\ntauri_chrome_simple_web_layout_manifest_chrome_tracked_count=0\\\\ntauri_chrome_simple_web_layout_manifest_chrome_fail_count=0\\\\ntauri_chrome_simple_web_layout_manifest_chrome_argb_artifact_pass_count=3\\\\ntauri_chrome_simple_web_layout_manifest_chrome_argb_artifact_fail_count=0\\\\ntauri_chrome_simple_web_layout_manifest_tauri_mismatch_count=0\\\\ntauri_chrome_simple_web_layout_manifest_chrome_mismatch_count=0\\\\ntauri_chrome_simple_web_layout_manifest_no_fake_capture=true\\\\ntauri_chrome_simple_web_layout_manifest_blur_or_tolerance_used=false\\\\n\" > \"$BUILD_DIR/evidence.env\"\\n' > " + root + "/fixture/surface.sh && " +
    "printf '#!/bin/sh\\nmkdir -p \"$BUILD_DIR\"\\nprintf \"production_gui_backend_status=pass\\\\nproduction_gui_backend_reason=pass\\\\nproduction_gui_backend_exact_backend_parity=true\\\\nproduction_gui_backend_cpu_simd_different_pixels=0\\\\nproduction_gui_backend_metal_resolved=metal\\\\nproduction_gui_backend_metal_different_pixels=0\\\\nproduction_gui_backend_metal_gpu_frame_complete=true\\\\nproduction_gui_backend_metal_gpu_readback_checksum=900\\\\nproduction_gui_backend_metal_command_queue_handle=42\\\\nproduction_gui_backend_software_checksum=900\\\\nproduction_gui_backend_cpu_simd_checksum=900\\\\nproduction_gui_backend_metal_checksum=900\\\\nproduction_gui_backend_backend_checksum_match=true\\\\nproduction_gui_backend_same_frame_backend_readback=true\\\\nproduction_gui_backend_backend_readback_source=device_readback\\\\nproduction_gui_backend_blur_or_tolerance_used=false\\\\nproduction_gui_backend_sample_count=3\\\\nproduction_gui_backend_total_elapsed_us_min=90\\\\nproduction_gui_backend_total_elapsed_us_avg=100\\\\nproduction_gui_backend_total_elapsed_us_max=120\\\\nproduction_gui_backend_total_pixels_per_second_min=2000000\\\\nproduction_gui_backend_total_pixels_per_second_avg=2400000\\\\nproduction_gui_backend_total_pixels_per_second_max=2800000\\\\nproduction_gui_backend_simple_bin=fixture-simple\\\\n\" > \"$BUILD_DIR/evidence.env\"\\n' > " + root + "/fixture/backend.sh && " +
    "printf '#!/bin/sh\\nmkdir -p \"$BUILD_DIR\"\\nprintf \"production_gui_font_offload_status=pass\\\\nproduction_gui_font_offload_reason=pass\\\\nproduction_gui_font_offload_runtime_evidence_status=pass\\\\nproduction_gui_font_offload_vector_backend=metal\\\\nproduction_gui_font_offload_vector_status=pass\\\\nproduction_gui_font_offload_vector_readback_status=pass\\\\nproduction_gui_font_offload_bitmap_backend=metal\\\\nproduction_gui_font_offload_bitmap_status=pass\\\\nproduction_gui_font_offload_bitmap_readback_status=pass\\\\n\" > \"$BUILD_DIR/evidence.env\"\\n' > " + root + "/fixture/font.sh && " +
    "printf '#!/bin/sh\\nmkdir -p \"$BUILD_DIR\"\\nprintf \"metal_engine2d_framebuffer_readback_status=pass\\\\nmetal_engine2d_framebuffer_readback_reason=raw-metal-framebuffer-download-proven\\\\nmetal_engine2d_framebuffer_readback_spec_status=pass\\\\nmetal_engine2d_framebuffer_gpu_readback_available=true\\\\nmetal_engine2d_framebuffer_blur_or_tolerance_used=false\\\\n\" > \"$BUILD_DIR/evidence.env\"\\n' > " + root + "/fixture/metal.sh && " +
    "printf '#!/bin/sh\\nmkdir -p \"$BUILD_DIR\"\\nprintf \"macos_metal_browser_backing_status=pass\\\\nmacos_metal_pixel_comparison_status=pass\\\\n\" > \"$BUILD_DIR/evidence.env\"\\n' > " + root + "/fixture/browser.sh && " +
    "printf '#!/bin/sh\\nmkdir -p \"$BUILD_DIR\"\\nprintf \"macos_metal_render_log_compare_status=pass\\\\nmacos_metal_render_log_compare_reason=pass\\\\nmacos_metal_render_log_compare_required_api=metal\\\\nmacos_metal_render_log_compare_generated_readback_gate_status=pass\\\\nmacos_metal_render_log_compare_framebuffer_readback_gate_status=pass\\\\nmacos_metal_render_log_compare_browser_backing_gate_status=pass\\\\nmacos_metal_render_log_compare_pairwise_gate_status=pass\\\\nmacos_metal_render_log_compare_argb_source_gate_status=pass\\\\nmacos_metal_render_log_compare_gpu_capture_gate_status=not-required\\\\nmacos_metal_render_log_compare_blocked_gate_count=0\\\\nmacos_metal_render_log_compare_blocked_gates=\\\\nmacos_metal_render_log_compare_pairwise_status=pass\\\\nmacos_metal_render_log_compare_gpu_capture_status=unavailable\\\\nmacos_metal_render_log_compare_gpu_capture_tool=xcode-gpu-frame-capture\\\\nmacos_metal_render_log_compare_tauri_ios_gate_status=not-required\\\\nmacos_metal_render_log_compare_tauri_ios_render_log_validation_status=missing\\\\nmacos_metal_render_log_compare_tauri_ios_render_log_validation_reason=missing-tauri-mobile-renderer-parity-evidence\\\\nmacos_metal_render_log_compare_tauri_ios_backend=tauri2-wkwebview\\\\nmacos_metal_render_log_compare_tauri_ios_expected_gpu_backend=metal\\\\nmacos_metal_render_log_compare_tauri_ios_metal_log_status=missing\\\\nmacos_metal_render_log_compare_tauri_ios_env_file_status=missing\\\\nmacos_metal_render_log_compare_tauri_ios_env_artifact_status=fail\\\\nmacos_metal_render_log_compare_require_tauri_ios=0\\\\n\" > \"$BUILD_DIR/evidence.env\"\\n' > " + root + "/fixture/metal-log.sh && " +
    "printf '#!/bin/sh\\nmkdir -p \"$BUILD_DIR\"\\nprintf \"wm_browser_event_routing_status=pass\\\\nwm_browser_event_routing_reason=pass\\\\nwm_browser_event_routing_validation_status=pass\\\\nwm_browser_event_routing_validation_reason=pass\\nwm_browser_event_routing_proof_symlink_status=pass\\nwm_browser_event_routing_proof_source=fixture-event-proof.js\\nwm_browser_event_routing_proof_source_file_status=pass\\nwm_browser_event_routing_proof_source_size_bytes=23\\nwm_browser_event_routing_browser_engine=chromium\\nwm_browser_event_routing_electron_user_agent=Mozilla/5.0 Chrome/142.0.0.0 Electron/42.5.0 Safari/537.36\\nwm_browser_event_routing_electron_process_version=42.5.0\\nwm_browser_event_routing_chrome_process_version=142.0.0.0\\nwm_browser_event_routing_ready=true\\\\nwm_browser_event_routing_wm_found=true\\\\nwm_browser_event_routing_focus_count=1\\\\nwm_browser_event_routing_move_count=1\\\\nwm_browser_event_routing_maximize_count=1\\\\nwm_browser_event_routing_title_command_count=1\\\\nwm_browser_event_routing_text_input_count=1\\\\nwm_browser_event_routing_pointer_down_count=1\\\\nwm_browser_event_routing_pointer_up_count=1\\\\nwm_browser_event_routing_performance_now_available=true\\\\nwm_browser_event_routing_performance_now_delta_ms=16.7\\\\nwm_browser_event_routing_input_to_paint_ms=18.4\\\\nwm_browser_event_routing_animation_frame_available=true\\\\nwm_browser_event_routing_animation_frame_count=2\\\\nwm_browser_event_routing_css_animation_probe=true\\\\nwm_browser_event_routing_event_sequence=host_wm_pointer:down,window_cmd:focus,window_cmd:move,window_cmd:title_command,window_cmd:maximize,input_event:text_input,input_event:pointer_down,input_event:pointer_up\\\\nwm_browser_event_routing_title_text=Terminal\\\\nwm_browser_event_routing_title_context_text=terminal\\\\nwm_browser_event_routing_traffic_button_count=3\\\\nwm_browser_event_routing_title_input_tag=input\\\\nwm_browser_event_routing_titlebar_height=34px\\\\nwm_browser_event_routing_titlebar_display=flex\\\\nwm_browser_event_routing_titlebar_cursor=grab\\\\nwm_browser_event_routing_titlebar_background=rgb(229, 231, 235)\\\\nwm_browser_event_routing_title_color=rgb(17, 24, 39)\\\\nwm_browser_event_routing_title_font_weight=700\\\\nwm_browser_event_routing_title_input_min_width=142px\\\\nwm_browser_event_routing_title_input_width=148px\\\\nwm_browser_event_routing_title_input_width_px=148\\\\nwm_browser_event_routing_title_input_height=24px\\\\nwm_browser_event_routing_title_input_cursor=text\\\\nwm_browser_event_routing_title_input_background=rgb(241, 245, 249)\\\\nwm_browser_event_routing_close_button_background=rgb(239, 68, 68)\\\\nwm_browser_event_routing_minimize_button_background=rgb(234, 179, 8)\\\\nwm_browser_event_routing_maximize_button_background=rgb(34, 197, 94)\\\\nwm_browser_event_routing_expected_move_x=86\\\\nwm_browser_event_routing_expected_move_y=86\\\\nwm_browser_event_routing_move_payload_x=86\\\\nwm_browser_event_routing_move_payload_y=86\\\\nwm_browser_event_routing_move_payload_source=native_event\\\\nwm_browser_event_routing_move_payload_window_id_hint=win1\\\\nwm_browser_event_routing_title_command_text=/tmp/project\\\\nwm_browser_event_routing_text_input_text=Hello Simple\\\\nwm_browser_event_routing_font_text=WEB\\\\nwm_browser_event_routing_font_composition_id=html-layout\\\\nwm_browser_event_routing_font_identity=sha256=2cb2adb378a8f574213e23df697050b83c54c27df465a2015552740b2769a081;axes=wght=400,wdth=100\\\\nwm_browser_event_routing_font_loaded=true\\\\nwm_browser_event_routing_font_frame_event_count=8\\\\nwm_browser_event_routing_font_frame_correlation_id=wm-browser-event-routing|fixture-run|html-layout|sha256=2cb2adb378a8f574213e23df697050b83c54c27df465a2015552740b2769a081;axes=wght=400,wdth=100|WEB|1|8\\\\nwm_browser_event_routing_font_frame_path=fixture-font-frame.bgra\\\\nwm_browser_event_routing_font_frame_pixel_checksum=1785\\\\nwm_browser_event_routing_font_frame_nonbackground_pixels=1\\\\nwm_browser_event_routing_font_frame_artifact_status=pass\\\\nwm_browser_event_routing_simple_composition_pixel_count=1\\\\nwm_browser_event_routing_simple_composition_run_id=fixture-run\\\\nwm_browser_event_routing_simple_composition_pixel_checksum=1\\\\nwm_browser_event_routing_simple_composition_artifact_size_bytes=1\\\\nwm_browser_event_routing_simple_composition_artifact_sha256=1111111111111111111111111111111111111111111111111111111111111111\\\\nwm_browser_event_routing_simple_composition_artifact_status=pass\\\\nwm_browser_event_routing_blur_or_tolerance_used=false\\\\n\" > \"$BUILD_DIR/evidence.env\"\\n' > " + root + "/fixture/event.sh && " +
    "SIMPLE_WEB_FONT_RUN_ID=fixture-run PRODUCTION_GUI_WEB_RENDERER_PARITY_MATRIX_SCRIPT=" + root + "/fixture/matrix.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_LAYOUT_SCRIPT=" + root + "/fixture/layout.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_SURFACE_SCRIPT=" + root + "/fixture/surface.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_BACKEND_SCRIPT=" + root + "/fixture/backend.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_FONT_OFFLOAD_SCRIPT=" + root + "/fixture/font.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_METAL_READBACK_SCRIPT=" + root + "/fixture/metal.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_METAL_BROWSER_BACKING_SCRIPT=" + root + "/fixture/browser.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_METAL_RENDER_LOG_SCRIPT=" + root + "/fixture/metal-log.sh PRODUCTION_GUI_WEB_RENDERER_PARITY_EVENT_SCRIPT=" + root + "/fixture/event.sh BUILD_ROOT=" + root + "/out REPORT_PATH=" + root + "/report.md sh scripts/check/check-production-gui-web-renderer-parity-evidence.shs"
val (_stdout, _stderr, code) = process_run("/bin/sh", ["-c", command])
expect(code).to_equal(0)

val evidence = file_read(root + "/out/evidence.env")
expect(evidence).to_contain("production_gui_web_renderer_parity_status=pass")
expect(evidence).to_contain("production_gui_web_renderer_parity_reason=pass")
expect(evidence).to_contain("production_gui_web_renderer_parity_surface_manifest_chrome_capture_backend=chrome-live-bitmap")
expect(evidence).to_contain("production_gui_web_renderer_parity_surface_manifest_chrome_live_capture=true")
expect(evidence).to_contain("production_gui_web_renderer_parity_surface_manifest_tauri_argb_artifact_pass_count=3")
expect(evidence).to_contain("production_gui_web_renderer_parity_surface_manifest_tauri_argb_artifact_fail_count=0")
expect(evidence).to_contain("production_gui_web_renderer_parity_surface_manifest_chrome_argb_artifact_pass_count=3")
expect(evidence).to_contain("production_gui_web_renderer_parity_surface_manifest_chrome_argb_artifact_fail_count=0")
expect(evidence).to_contain("production_gui_web_renderer_parity_surface_manifest_no_fake_capture=true")
expect(evidence).to_contain("production_gui_web_renderer_parity_backend_status=pass")
...
expect(evidence).to_contain("production_gui_web_renderer_parity_backend_metal_gpu_readback_checksum=900")
expect(evidence).to_contain("production_gui_web_renderer_parity_backend_metal_command_queue_handle=42")
expect(evidence).to_contain("production_gui_web_renderer_parity_backend_software_checksum=900")
expect(evidence).to_contain("production_gui_web_renderer_parity_backend_cpu_simd_checksum=900")
expect(evidence).to_contain("production_gui_web_renderer_parity_backend_metal_checksum=900")
...
expect(evidence).to_contain("production_gui_web_renderer_parity_backend_readback_source=device_readback")
...
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_readback_status=pass")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_readback_available=true")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_status=pass")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_required_api=metal")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_generated_readback_gate_status=pass")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_framebuffer_readback_gate_status=pass")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_browser_backing_gate_status=pass")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_pairwise_gate_status=pass")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_argb_source_gate_status=pass")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_gpu_capture_gate_status=not-required")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_blocked_gate_count=0")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_tauri_ios_gate_status=not-required")
expect(_line_count(evidence, "production_gui_web_renderer_parity_metal_render_log_tauri_ios_gate_status=not-required")).to_equal(1)
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_tauri_ios_render_log_validation_status=missing")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_tauri_ios_render_log_validation_reason=missing-tauri-mobile-renderer-parity-evidence")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_tauri_ios_backend=tauri2-wkwebview")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_tauri_ios_expected_gpu_backend=metal")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_tauri_ios_metal_log_status=missing")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_tauri_ios_env_file_status=missing")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_tauri_ios_env_artifact_status=fail")
expect(evidence).to_contain("production_gui_web_renderer_parity_metal_render_log_require_tauri_ios=0")
expect(evidence).to_contain("production_gui_web_renderer_parity_event_routing_status=pass")
expect(evidence).to_contain("production_gui_web_renderer_parity_event_routing_validation_status=pass")
expect(evidence).to_contain("production_gui_web_renderer_parity_event_routing_proof_symlink_status=pass")
...
expect(evidence).to_contain("production_gui_web_renderer_parity_event_routing_chrome_process_version=142.0.0.0")
expect(evidence).to_contain("production_gui_web_renderer_parity_event_routing_performance_now_available=true")
expect(evidence).to_contain("production_gui_web_renderer_parity_event_routing_performance_now_delta_ms=16.7")
expect(evidence).to_contain("production_gui_web_renderer_parity_event_routing_input_to_paint_ms=18.4")
expect(evidence).to_contain("production_gui_web_renderer_parity_event_routing_animation_frame_count=2")
expect(evidence).to_contain("production_gui_web_renderer_parity_event_routing_css_animation_probe=true")
expect(evidence).to_contain("production_gui_web_renderer_parity_event_routing_event_sequence=host_wm_pointer:down,window_cmd:focus,window_cmd:move,window_cmd:title_command,window_cmd:maximize,input_event:text_input,input_event:pointer_down,input_event:pointer_up")
expect(evidence).to_contain("production_gui_web_renderer_parity_event_routing_titlebar_background=rgb(229, 231, 235)")
...
expect(report).to_contain("- Surface ARGB artifacts: pass (tauri 3/0; chrome 3/0)")
expect(report).to_contain("- Metal render-log gates: generated=pass framebuffer=pass browser=pass pairwise=pass argb_source=pass gpu_capture=not-required blocked=0")
expect(report).to_contain("- Electron event proof source: pass / pass / pass")
expect(report).to_contain("- Electron performance/animation detail: perf=true delta_ms=16.7 input_to_paint_ms=18.4 animation=true frames=2 css=true")
...
val bad_checksum_command = command.replace("production_gui_backend_cpu_simd_checksum=900", "production_gui_backend_cpu_simd_checksum=901") + " || true"
...
val bad_checksum = file_read(root + "/out/evidence.env")
expect(bad_checksum).to_contain("production_gui_web_renderer_parity_status=fail")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("finalizes partial evidence when the wrapper is interrupted")
...
- Canonical SPipe generation for source `661abcd8b94f5b9803dd65703465ea4cab13651c2f8a2ae2775edbdce29b06d0`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `661abcd8b94f5b9803dd65703465ea4cab13651c2f8a2ae2775edbdce29b06d0`.
...
source: test/03_system/check/production_gui_web_renderer_parity_evidence_spec.spl
mirror: doc/06_spec/03_system/check/production_gui_web_renderer_parity_evidence_spec.md (current)
findings: 6 blockers: 0
  narrative=100 structure=100 oracle=70
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/03_system/check/production_gui_web_renderer_parity_evidence_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/03_system/check/production_gui_web_renderer_parity_evidence_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, unsupported/limitations, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/03_system/check/production_gui_web_renderer_parity_evidence_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-30): 25 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/03_system/check/production_gui_web_renderer_parity_evidence_spec.spl:185:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'exports an admitted Stage4 Simple binary to nested production parity checks' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/03_system/check/production_gui_web_renderer_parity_evidence_spec.spl:326:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'rejects an unreceipted browser binary' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/03_system/check/production_gui_web_renderer_parity_evidence_spec.spl:345:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'rejects a stale Stage4 receipt' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
