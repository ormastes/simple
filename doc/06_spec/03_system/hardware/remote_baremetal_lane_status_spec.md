...
## Related: GPU Remote Lanes

The `cuda`/`vulkan` remote interpreter test lanes
(`interpreter(remote(cuda(sm80)))` family) are a separate, GPU-specific
extension of the remote-lane concept this spec covers for baremetal/QEMU
targets — not yet one of the `LaneRegistry` rows exercised below. See:
...
- Feature-expert skill (current landed/open status):
  `doc/00_llm_process/feature_expert/gpu_remote_lanes/skill.md`
- User-facing guide: `doc/07_guide/testing/gpu_remote_lanes_testing.md`

_Note: this file is generated by `simple spipe-docgen` from
`test/03_system/hardware/remote_baremetal_lane_status_spec.spl`; this note
is a hand-added cross-link and may need to be re-applied (or moved into the
`.spl` source docstring) after the next docgen regeneration._
