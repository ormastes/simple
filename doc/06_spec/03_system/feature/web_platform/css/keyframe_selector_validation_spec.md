...
> A comma-separated keyframe selector list is accepted as one unit or its
> entire declaration block is discarded.
...
| Field | Value |
|-------|-------|
| Requirements | REQ-WEB-BROWSER-003, REQ-WEB-BROWSER-004, REQ-WEB-BROWSER-006 |
| Source | `test/03_system/feature/web_platform/css/keyframe_selector_validation_spec.spl` |
| Source SHA-256 | `b7f1f8d9e02d6d7d133303482ea8ca3d2fb29e7cfa468dee6cafd0dae54a8fb8` |
| Source lines | 233 |
| Scenarios / steps / assertion sites | 1 / 4 / 35 |
| Status | Active |
| Updated | 2026-07-31 |
| Generator | Exact manual mirror; qualified docgen pending |

## Contract

- Normalized `from,to`, `0%,100%`, and `.5%,1e2%` parse to exact
  `[0.0,1.0]`, `[0.0,1.0]`, and `[0.005,1.0]` offsets.
- Unknown, malformed, non-finite, overflowing, negative, greater-than-100,
  empty, leading-comma, trailing-comma, doubled-comma, and mixed-invalid lists
  insert no declarations.
- A 256-member selector list is accepted. A 257-member list, or a two-member
  list with only one unit of the shared quota left, is rejected atomically.
- Valid duplicate-zero cascade and implicit endpoint behavior remain intact.
- Scheduler state, canonical Draw IR, and exact Engine2D pixels preserve the
  same decision.

<details>
<summary>Executable SSpec — complete self-contained source</summary>
...
val BLUE: u32 = 0xFF2563EBu32

fn _animation_html(keyframes: text) -> text:
    (
        "<style>html,body{margin:0}" + keyframes +
        "#box{width:8px;height:8px;background-color:#dc2626;" +
        "animation-name:tint;animation-duration:1000ms;" +
        "animation-timing-function:linear;animation-fill-mode:both;" +
        "animation-play-state:paused}</style><div id='box'></div>"
    )

fn _mixed_invalid_html() -> text:
    _animation_html(
        "@keyframes tint{" +
        "from{background-color:#dc2626}" +
        "from,bogus{background-color:#2563eb}" +
        "to{background-color:#16a34a}}"
    )

fn _trailing_comma_html() -> text:
    _animation_html(
        "@keyframes tint{" +
        "from{background-color:#dc2626}" +
...
fn _implicit_endpoint_html() -> text:
    _animation_html(
        "@keyframes tint{50%{background-color:#2563eb}}"
    )

fn _repeated_selector_list(selector: text, count: i32) -> text:
    var out = ""
    var index = 0
    while index < count:
        if index > 0:
...
fn _quota_html(selector_count: i32) -> text:
    _animation_html(
        "@keyframes tint{" +
        _repeated_selector_list("from", selector_count) +
        "{background-color:#2563eb}}"
    )

fn _remaining_budget_html(final_selectors: text) -> text:
    _animation_html(
        "@keyframes tint{" +
        _repeated_selector_list("from", 255) +
        "{background-color:#dc2626}" +
        final_selectors + "{background-color:#2563eb}}"
    )

fn _instances(html: text) -> [BrowserCssAnimationInstance]:
    simple_web_layout_reconcile_animation_instances(
        html, WIDTH, 0, 0, false, []
    )

fn _draw_color(html: text) -> u32:
    val result = (
        simple_web_layout_render_html_draw_ir_result_at_time_with_animations(
            html, WIDTH, HEIGHT, 0, _instances(html)
        )
    )
    ...
    fail("missing Draw IR command: box")
    0u32

fn _pixels(html: text) -> [u32]:
    simple_web_layout_render_html_pixels_engine2d_at_time_with_animations(
        html, WIDTH, HEIGHT, "software", 0, _instances(html)
    )

describe "CSS keyframe selector validation":
    it "should discard an invalid selector list before inserting its block":
