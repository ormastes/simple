...
**Evidence:** `test/unit/parser/comptime_spec.spl` shows they work exactly like normal `val`/`fn` — evaluated when module loads, not at compile time.
...
❌ Zero references to `link_section`, `addr_space`, or `linksection` anywhere in the codebase. The linker SDN format (`doc/spec/layout.sdn`) describes section placement but there's no language-level annotation to tag functions or variables to specific sections.
