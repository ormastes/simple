...
**Date:** 2026-03-28
**Related Requirement:** [doc/plan/requirement/security_aop.md](../plan/requirement/security_aop.md)
**Related NFR:** [doc/plan/nfr/security_baseline.md](../plan/nfr/security_baseline.md)
...
1. **Parser Security** — SDN parser operates in data-only mode by default (prevents RCE, injection, DoS). Documented in `doc/research/data_format_parser_security_2026-01-31.md`.

2. **Sandbox Strategies** — Hybrid native OS (Linux namespaces) + bubblewrap approach documented in `doc/research/sandbox_implementation_strategies.md`.

3. **Memory Safety** — 3-phase plan (warning → refactor → strict mode) in `doc/plan/manual_memory_safety_plan.md`. Borrow checker at `src/compiler/55.borrow/` with NLL, borrow graph, lifetime analysis, GC analysis.
...
| No security NFRs | No measurable quality targets | `doc/plan/nfr/security_baseline.md` |
