...
That aggregate must report `simpleos_hardening_matrix_passed=22/22` and
...
`simpleos_hardening_formal_boundary_integrity_status=pass` plus
`simpleos_hardening_formal_compiler_language_status=pass` plus
`simpleos_hardening_formal_ui_policy_status=pass` plus
`simpleos_hardening_formal_coverage_status=pass` before claiming the
