...
```
  ...
  FTL ┌────────────────────────▼──────────┐  ftl_map (DFTL write-back cache) · ftl_band
      ...
      │ crash recovery                    │  (WAL + checkpoint + A/B superblock) · ftl_gc
      └────────────────────────┬──────────┘  (greedy victim) · ftl (write/read/trim/recover/gc)
...
```bash
B=bin/simple
$B run examples/09_embedded/simpleos_nvme_fw/fw/test_fw.spl    # -> ALL FIRMWARE SELF-TESTS PASS  (300 assertions)
...
- **`test_fw.spl`** — full self-test suite, 300 `PASS:` assertions across all modules.
...
- **`nvme_main.spl`** — admin-driven controller acceptance: host bring-up (Identify → Features →
  ...
  delete-bound-CQ rejected) → reverse-order teardown → SMART log → power-cycle survival.
...
> The firmware (`fw/`) itself has **no** Lean proofs — the proofs cover the emulator's algorithms.

---

## 5. rv32 bare-metal status (read this plainly)

The pure-Simple firmware was **NOT** booted bare-metal on rv32 this session. The boot recipe and
runtime are in place, but the native build is blocked (the firmware's `[i64]`/`.push` usage needs
a heap that `-bios none` does not provide, plus seed-binary/worktree drift). A separate,
self-contained **C** NAND/FTL demo *did* boot on `qemu-system-riscv32 -bios none`
(`ALL RV32 NAND CHECKS PASS`) — that is a **toolchain + boot-path proof only**. Do not read it as
the Simple firmware running on rv32. A no-alloc rv32 port is genuinely future work (plan phase
E5 / baremetal phase "rv32 boot").
