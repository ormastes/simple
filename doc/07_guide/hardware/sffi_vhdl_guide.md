...
Test coverage: 37 cases in `test/01_unit/compiler/backend/vhdl_abi_spec.spl`.
...
```bash
# All GHDL tests
bin/simple test test/03_system/feature/baremetal/

# Specific test
bin/simple test test/03_system/feature/baremetal/ghdl_generated_riscv32_smoke_spec.spl

# Slow tests only (GHDL simulation)
bin/simple test --only-slow test/03_system/feature/baremetal/
```
