...
The cache is deliberately semantic, not just timestamp-based. A verified unit's fingerprint includes source, generated Lean, imported proof modules, toolchain identity, public ABI shape, accessor forwarding, and field-wrapper dependencies. If module `A` relies on `B` through a forwarded field/getter/setter path and `B` changes that wrapped field in a way that changes `A`'s meaning, the stale fingerprint is rejected instead of reusing an old proof or loader/JIT artifact. The same rule is covered in the interpreter-facing incremental cache, the SMF loader cache, and the JIT instantiator cache: semantic dependency changes miss the cache and rebuild.

The related lint lane catches two LLM-prone mistakes before they become cache bugs: dummy accessor pairs that just hide an unimplemented field contract, and inherited method names that are suspiciously close to a parent API. Intentional near-name APIs can be marked with `@name_checked`, so the guard is strict without blocking documented compatibility shims.
