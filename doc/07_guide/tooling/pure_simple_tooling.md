...
- **MCP** — **Source status:** the full wrapper's repository-root CWD restore
  ...
  Active VCS, read-only query, diagnostics, and CLI-passthrough handlers now
  pass request fields through one bounded argv owner instead of interpolating
  them into a shell. Positional child-option injection is rejected at the
  diagnostics/CLI boundaries, and bounded runtime timeouts normalize to exit
  124 for handler reporting; see the
