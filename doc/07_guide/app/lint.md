...
| RAW-RT-001 (`raw_rt_access`) | Encapsulation | WARNING | Raw `extern fn rt_*` runtime intrinsic declared outside the privileged tiers (`src/lib`, `src/runtime`, `src/compiler`) — app/library code must use a std wrapper (e.g. `std.io_runtime.file_read`/`file_write`). Low-level modules (emulators, baremetal MMIO, crypto/protocol impls) may opt out with a file-level marker on line 1 — **prefer the bracket form `#[runtime_intrinsics]`** (the bare `@runtime_intrinsics` decorator triggers E0002 when the next item is a `use` statement). Both forms are lint-accepted |
