...
- DI context: explicit service/context records passed into Office actions when
  ...
  current `OfficePluginContext`, rejects mismatched action/source-format pairs,
  and uses no service locator.
...
`sdd-outline-read`, `sdd-style-rules-read`, `sdd-weave-summary`, `export-sdd-canonical`. The bridge delegates to the canonical
...
`sdd-outline-read` returns compact node and edge rows for Draw outline/navigator
panels.
`sdd-style-rules-read` lists reusable CSS rule definitions and style rows for
Draw style inspector panels.
...
`sdd-style-resolved-read` uses `css|target` and returns the inherited/cascaded declaration string for `node` or `edge`.
`sdd-style-resolved-value-read` uses `css|target|key` and returns the inherited/cascaded style value for `node` or `edge`.
...
`sdd-node-child-count-read` uses `node_id` and returns the direct child count.
...
`sdd-edge-segments-read` uses `edge_index` and returns `segments|segment_midpoints|segment_orientations`.
...
Mail uses the existing `app.office.mail.mail_app` in-memory mailbox state.
`mail-summary` is read-only and returns active folder, message count, unread
count, folder count, and filtered active-folder count.
...
- LLM catalog: Markdown has `render-markdown-preview-html` and `md-edit`; Writer has
  ...
  `writer-markdown-range`, `writer-markdown-blocks`, `writer-markdown-tables`,
  `writer-markdown-replace`, `writer-markdown-insert`, `writer-markdown-delete`, and
  ...
  `sdd-document-summary`, `sdd-outline-read`, `sdd-style-rules-read`, `sdd-weave-summary`, `render-sdd-html-with-selection`, `export-sdd-canonical`, `reroute-sdd-connector`, `edit-sdd-style-rule`,
...
- `office_catalog_dispatch_probe()` dispatches each advertised catalog action
  with `OfficePluginContext`, so wrong component/evidence metadata is rejected
  instead of counted as valid.
