...
alignment/distribution, visible selected-node resize handles, selected connector endpoint/waypoint/label handles, rendered shape CSS with safe `sdd-shape-*` classes, and rendered canvas/page background and grid metadata. `src/app/ide/draw_sanity.spl`
