...
and geometry `data-x`/`data-y`/`data-width`/`data-height` metadata for layer and inspector panels, exports nodes to SDD-compatible tables
...
alignment/distribution, visible selected-node resize handles, selected connector endpoint/waypoint/label handles, rendered shape CSS with safe `sdn-css-*` and `sdd-shape-*` classes, safe `sdd-kind-*` connector classes, and rendered canvas/page background and grid metadata. `src/app/ide/draw_sanity.spl`
