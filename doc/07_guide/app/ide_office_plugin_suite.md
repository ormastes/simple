...
checkbox rows inside the same WYSIWYG document wrapper. Markdown bullet lines
(`- item` / `* item`) render as HTML lists with escaped inline content.
...
count. Writer paper HTML renders Markdown headings, paragraphs, lists, tables,
images, and inline links as document HTML while escaping user text and
...
- Markdown: `task_list=true strike=true link=true list=true css_doc=true escaped=true metadata=true`
