...
6. **Chrome/Electron Compute Evidence** — `std.gc_async_mut.gpu.browser_engine.chrome_webgpu_compute_evidence` launches `tools/web-render-backend/chromium-webgpu-compute` for host-adaptive processing evidence. It accepts compiler-emitted WGSL for `simple_webgpu_add_u32` and `simple_2d_fill_u32`; the Simple2D fill profile uses the WebGPU `Simple2DParams` uniform contract, validates storage-buffer readback, and can carry WASM-shaped payload byte-count/checksum provenance through `chrome_webgpu_compute_wasm_simple2d_fill_payload_bytes_evidence(...)`.
