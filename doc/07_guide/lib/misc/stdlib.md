...
| Send inline values between concurrent work | `std.concurrent.channel.{channel_new, channel_from_id}` | `src/lib/nogc_sync_mut/concurrent/channel.spl` | Native compatibility channel uses validated transfer packets, a finite capacity of 256, and reports full/closed/non-transferable sends as failure. Heap graphs are rejected until typed frozen/owned/encoded payload support lands. |
