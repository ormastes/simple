...
- [x] AC-5: A spipe spec at `test/01_unit/app/build/repo_hygiene_gate_spec.spl` validates gate behavior.
...
### 2-research
Existing infrastructure: `scripts/check/check-*.shs` pattern (11 scripts), `src/app/build/cli_entry.spl` (stub returning 0), dispatch table routes `build` to `src/app/build/main.spl` (missing — needs creation). `src/app/build/os_harden_audit.spl` is the closest existing audit pattern. `test/01_unit/app/audit/audit_spec.spl` provides test template.

### 5-implement
Created `scripts/check/check-repo-hygiene.shs` (hygiene gate), `src/app/build/repo_hygiene_gate.spl` (standalone .spl entry), dedicated CI workflow `.github/workflows/repo-hygiene.yml` (no path filter — runs on all pushes/PRs), test spec at `test/01_unit/app/build/repo_hygiene_gate_spec.spl`.
