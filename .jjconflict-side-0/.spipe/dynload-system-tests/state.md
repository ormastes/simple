...
- `test/03_system/` contains subdirs: `jupyter/`, `daemon_sdk/`, `dap/`, `lint/`, `llm_caret/`, `mcp/`, `os/`, `smoke/`, plus many more. Each has `*_system_spec.spl` + `.spipe_matchers_*` companion files.
- `test/03_system/daemon_sdk/daemon_sdk_ipc_system_spec.spl` — canonical pattern for real-I/O system tests: uses `sys_setup()`/`sys_cleanup()` helpers, writes real files to `/tmp`, reads back, verifies content with `expect().to_contain()`. Uses `rt_file_exists()`, `rt_file_delete()`, `read_file_content()` extern functions.
- No existing `test/03_system/` tests for dynload/dylib. This is a gap.

#### ELF Fixture Building (Synthetic — In-Memory)
- `test/01_unit/os/kernel/loader/dylib_registry_spec.spl:1-60` — builds minimal ELF64 via `make_elf64_exec()` using `push_u16_le/push_u32_le/push_u64_le` helper functions that construct valid ELF headers byte-by-byte (magic `7F 45 4C 46`, class 2, little-endian, ET_EXEC, e_machine=62/x86_64, entry=0x400000).
- `test/01_unit/os/kernel/loader/elf64_dynsym_spec.spl` — builds ELF64 with `.dynsym` + `.strtab` sections via `_make_elf64_with_dynsym()`. Includes section header table, null symbol, and a "hello" symbol at address 0xBEEF. Uses `_pad_to()` for alignment.
...
## Requirements
- REQ-1 (from AC-1): Linux system test must build a valid ELF64 .so fixture (synthetic [u8] bytes), register via `dylib_registry_register`, open via `dylib_open` or `dylib_registry_open`, resolve symbol via `dylib_registry_symbol`, verify address > 0 — area: `test/03_system/dynload/`, uses `os.kernel.loader.dylib_registry`, `os.posix.dylib_async`
- REQ-2 (from AC-2): SimpleOS system test must exercise the full `dylib_async` pipeline (open -> symbol -> close) with an SMF-wrapped ELF fixture, verifying async request lifecycle — area: `test/03_system/dynload/`, uses `os.posix.dylib_async`, `os.kernel.loader.dylib_registry`
- REQ-3 (from AC-3): macOS system test must use the same ELF registry path (since `dylib_registry` is format-agnostic, not platform-native), gated by `if is_macos()` from `std.env.platform` — area: `test/03_system/dynload/`
- REQ-4 (from AC-4): Windows/Wine system test must build a PE64 fixture using the `_reloc_image`/`_export_image` pattern from `wine_pe_loader_spec.spl`, load via `wine_dll_map_pe_image`, resolve export via `pe_export_lookup_by_name`, gated by `if is_windows()` or Wine detection — area: `test/03_system/dynload/`, uses `common.wine_dll_image_loader`, `common.pe_coff_header`
- REQ-5 (from AC-5): Platform detection helper must use `std.env.platform.{detect_os, is_linux, is_macos, is_windows}` to conditionally execute platform-specific test blocks — area: `test/03_system/dynload/`
- REQ-6 (from AC-6): All tests must pass on Linux dev machine; macOS/Windows tests must be gated (print skip message, not fail) — area: `test/03_system/dynload/`
...
|--------|------|------|--------------|
| elf_fixture_helpers | `test/03_system/dynload/dynload_fixture_helpers.spl` | Shared synthetic ELF64/SMF/PE byte-array builders (push_u16_le, make_elf64_exec, make_smf_wrapped_elf64_exec, make_elf64_with_dynsym, _reloc_image, _export_image, _zero_bytes, _put_u16_le, _put_u32_le, _put_u64_le, _put_ascii) | New |
| linux_elf_system_spec | `test/03_system/dynload/dynload_linux_elf_system_spec.spl` | Linux ELF system test: register synthetic ELF64 .so fixture, open via dylib_registry, resolve symbol, verify address > 0 | New |
| linux_elf_matchers | `test/03_system/dynload/.spipe_matchers_dynload_linux_elf_system_spec.spl` | SPipe matchers companion for Linux ELF spec | New |
| simpleos_smf_system_spec | `test/03_system/dynload/dynload_simpleos_smf_system_spec.spl` | SimpleOS SMF system test: register SMF-wrapped-ELF fixture, open via dylib_registry, resolve entry symbol | New |
| simpleos_smf_matchers | `test/03_system/dynload/.spipe_matchers_dynload_simpleos_smf_system_spec.spl` | SPipe matchers companion for SimpleOS SMF spec | New |
| macos_system_spec | `test/03_system/dynload/dynload_macos_system_spec.spl` | macOS system test: same ELF registry path gated by is_macos(), prints skip on other platforms | New |
| macos_matchers | `test/03_system/dynload/.spipe_matchers_dynload_macos_system_spec.spl` | SPipe matchers companion for macOS spec | New |
| windows_pe_system_spec | `test/03_system/dynload/dynload_windows_pe_system_spec.spl` | Windows/Wine PE system test: build PE64 fixture, load via wine_dll_map_pe_image, resolve export via pe_export_lookup_by_name, gated by is_windows() | New |
| windows_pe_matchers | `test/03_system/dynload/.spipe_matchers_dynload_windows_pe_system_spec.spl` | SPipe matchers companion for Windows PE spec | New |
...
- `test/03_system/dynload/dynload_fixture_helpers.spl` -- shared ELF64/SMF/PE64 synthetic byte-array builders (14 public functions)
- `test/03_system/dynload/dynload_linux_elf_system_spec.spl` -- Linux ELF system tests using shared helpers + platform gating
- `test/03_system/dynload/dynload_simpleos_smf_system_spec.spl` -- SimpleOS SMF system tests
- `test/03_system/dynload/dynload_macos_system_spec.spl` -- macOS ELF cross-load tests (SKIP on non-macOS)
- `test/03_system/dynload/dynload_windows_pe_system_spec.spl` -- Windows PE system tests
...
- `bin/simple test test/03_system/dynload/` -- 26 examples, 0 failures, 7753ms total
