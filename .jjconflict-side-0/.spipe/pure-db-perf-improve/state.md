...
- **bench_now_ns pattern:** Already used in `test/05_perf/bench/simple_db_wal_append.spl` — reusable for PureDB benchmarks
...
| `test/05_perf/bench/simple_db_wal_append.spl` | Existing WAL benchmark pattern |
| `test/05_perf/bench/simple_db_shared_accel.spl` | Existing accel benchmark pattern |
...
- **No existing PureDatabase benchmarks:** `test/05_perf/bench/` contains WAL and shared-accel benchmarks but nothing for PureDatabase INSERT/SELECT/FTS. AC-1 (micro-benchmark spec) is entirely new work.
- **Timing helper available:** `test/05_perf/bench/lib/timing.spl` provides `BenchResult`, `bench_now_ns`, `bench_print`, `bench_csv` — reusable for PureDB benchmarks
...
| `test/05_perf/bench/pure_db_micro.spl` | NEW | Micro-benchmark spec (AC-1, AC-2) |
...
**File:** `test/05_perf/bench/pure_db_micro.spl`
...
1. `test/05_perf/bench/pure_db_micro_spec.spl` — Micro-benchmark spec (AC-1, AC-2, AC-5)
...
2. `test/05_perf/bench/pure_db_optimization_spec.spl` — Optimization verification spec (AC-4, AC-5)
...
2. **`test/05_perf/bench/pure_db_optimization_spec.spl`** — Fixed spec bug: `exec_sql` returns `i64`, not `[DbRow]`; changed SELECT queries to use `query()` method

3. **`test/05_perf/bench/pure_db_micro_spec.spl`** — Same fix: SELECT queries use `query()` instead of `exec_sql()`
