...
- implement: Extended the shared structural layout surface with `StructuralLayoutBox`, GUI/browser layout-box SDN output, and box-list geometry comparison before pixel acceptance.
- verify: Structural layout report source/spec typecheck and `structural_layout_report_spec` passes `7/7`; regenerated the scenario manual and restored purpose text for TUI cells, GUI boxes, backend evidence, pixel links, and corpus attachment.
- implement: Added `ComparisonFailureReport` to preserve capture, metadata, structural, pixel, and backend statuses separately for TUI/GUI and Chrome/Simple comparison diagnostics.
- verify: Comparison failure report source/spec typecheck and `comparison_failure_report_spec` passes `5/5`; generated the scenario manual and added a purpose section for the failure-triage contract.
- implement: Added backend measurement binary-size delta calculation, validation, and SDN output.
- verify: Backend measurement report source/spec typecheck and `backend_measurement_report_spec` passes `6/6`; regenerated the scenario manual and restored purpose text for timing, RSS, binary-size delta, scalar-baseline, unavailable-lane, and fallback-rejection evidence.
- verify: Reconciled stale selected-scope docs and ran the focused verification set: backend screenshot `9/9`, HTML compatibility `17/17`, emulated capture `5/5`, site corpus pair `1/1`, structural layout report `7/7`, comparison failure report `5/5`, backend measurement report `6/6`, backend measurement capture `3/3`, and backend probe strict `8/8`. `doc/06_spec` executable count is `0`, and the touched executable placeholder scan has no matches.
