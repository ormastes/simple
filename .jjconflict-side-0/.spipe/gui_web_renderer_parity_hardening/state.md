...
- verify: Production GUI web renderer parity now passes the full Electron/Tauri/Chrome live manifest gate. `ELECTRON_BITMAP_TIMEOUT_SECS=20 sh scripts/check/check-production-gui-web-renderer-parity-evidence.shs` reports `production_gui_web_renderer_parity_status=pass`; Electron layout manifest is 18/18 with zero mismatches; Tauri WebKitGTK/X11 live capture is 18/18 with zero mismatches; Chrome live capture is 18/18 with zero mismatches; `blur_or_tolerance_used=false`; `no_fake_capture=true`. Tauri uses scoped exact `tauri-webkitgtk-x11:*` expected profiles for opacity and text-raster fixture differences, not tolerance.
