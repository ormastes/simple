#!/usr/bin/env bash
# build.sh -- Validate and package the codex-research plugin
set -euo pipefail
...
echo "=== codex-research plugin build ==="
...
# 5. Package (tar.gz)
PACKAGE_NAME="codex-research-$(python3 -c "import json; print(json.load(open('$PLUGIN_JSON'))['version'])" 2>/dev/null || echo "0.1.0")"
