...
Cross-reference IDs in `doc/08_tracking/feature/scilib_perf_sugar.md`.
...
Write `test/05_perf/typed_array_alloc_spec.spl` (interpreter mode; no `--mode` flag). The spec must:
...
**Deliverable:** `test/05_perf/typed_array_alloc_spec.spl` passing under `bin/simple test`.
...
Update `doc/08_tracking/feature/scilib_perf_sugar.md` entry PERF-SUGAR-001:
...
Add a `## Append Protocol` section to `doc/08_tracking/feature/scilib_perf_sugar.md` immediately after `## Schema`:
...
The note should be appended to `doc/08_tracking/feature/scilib_perf_sugar.md` under PERF-SUGAR-002's `Proposed-sugar` section, or placed in a new file `doc/05_design/math_block_fusion_design.md` if length warrants.
