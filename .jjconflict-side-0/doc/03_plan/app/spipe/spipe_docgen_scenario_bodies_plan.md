...
[test/03_system/feature/usage/math_blocks_spec.spl](../../test/03_system/feature/usage/math_blocks_spec.spl)
...
1. **Add body capture + fenced-code rendering to the pure-Simple generator.** This
   ...
   `bin/simple spipe-docgen test/03_system/feature/usage/math_blocks_spec.spl --output doc/06_spec`.
...
`test/01_unit/app/tooling/spipe_docgen_scenario_body_spec.spl`. The pure-Simple
...
**Progress 2026-05-30:** Implemented for both current runner paths. The
self-hosted Simple runner batches successful non-cached `*_spec.spl` files and
delegates once to `bin/simple spipe-docgen ... --output doc/06_spec`. The Rust
bootstrap runner mirrors this by delegating to the current `simple` executable's
`spipe-docgen` command, so the generator remains the pure-Simple app rather than
the removed Rust docgen module.
...
Implemented as a batched delegation to `bin/simple spipe-docgen` from
`src/app/test_runner_new/test_runner_main.spl`, which keeps the generation path
pure Simple while avoiding one process spawn per spec file.
...
Implemented by removing the Rust `spipe-docgen` command path and mirroring the
test-run post-processing hook in
`src/compiler_rust/driver/src/cli/test_runner/runner.rs` as a delegation to the
pure-Simple `spipe-docgen` command.

## Verification

Golden file: `test/03_system/feature/usage/math_blocks_spec.spl` (contents already known).

1. `bin/simple spipe-docgen test/03_system/feature/usage/math_blocks_spec.spl --output /tmp/docgen-check`
...
2. Spot-check edge cases: a spec with `skip it`, a `pending`, a `"""` docstring body,
   ...
   correct dedent).
3. `bin/simple test test/03_system/feature/usage/math_blocks_spec.spl` → confirm
...
- `SIMPLE_LIB=src bin/simple spipe-docgen test/03_system/feature/usage/math_blocks_spec.spl --output /tmp/docgen-check`
  generated `/tmp/docgen-check/feature/usage/math_blocks_spec.md` with
  `Generator | simple spipe-docgen (Simple)` and a `simple` code block containing
  `val result = m{ 2 + 3 }`.
- `src/compiler_rust/target/debug/simple test test/03_system/feature/usage/math_blocks_spec.spl --mode=interpreter --no-cache`
  passed 28/28 and refreshed `doc/06_spec/feature/usage/math_blocks_spec.md`
  at `2026-05-30 15:38:48 +0000`, with the same Simple generator marker and
  scenario code block.
- `SIMPLE_LIB=src bin/simple check src/app/test_runner_new/test_runner_main.spl`
  passed.
- `cargo check -p simple-driver` passed with existing warnings.
