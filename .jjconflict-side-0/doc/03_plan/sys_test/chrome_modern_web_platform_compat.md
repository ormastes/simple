...
- Primary SPipe: `test/03_system/app/lib/feature/chrome_modern_web_platform_compat_spec.spl`
- Focused renderer SPipe: `test/01_unit/lib/gc_async_mut/gpu/browser_engine/browser_renderer_spec.spl`
...
- Given modern CSS selectors, when source is inspected, then bounded ancestor `> div/.class/#id` child matching should be implemented in the fallback block scanner.
...
- Given modern CSS selectors, when source is inspected, then bounded direct body-child general sibling matching should be implemented in the fallback block scanner.
