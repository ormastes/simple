...
- var cache = StaticCompressionCache new
   ...
   - Expected: enc equals `zstd`
   - Expected: vary equals `Accept-Encoding`
...
- var cache = StaticCompressionCache new
   ...
   - Expected: cache.entries.len() equals `cached_after_first`
   - Expected: enc2 equals `zstd`
...
#### gzip cache miss compresses, caches, and emits Content-Encoding
...
- var cache = StaticCompressionCache new
   - Expected: r1.body_bytes.len() > 0 is true
   - Expected: r2.body_bytes.len() > 0 is true
   - Expected: cache.entries.len() equals `2`
