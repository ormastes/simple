...
expect(bug).to_contain("multicore_green_spawn_sliced used_runtime_pool=true, quick_done=true, parallelism=1, total=9")
expect(profile_contract).to_contain("Hosted Fairness Evidence")
expect(profile_contract).to_contain("multicore_green_spawn_sliced used_runtime_pool=true, quick_done=true, parallelism=1, total=9")
expect(report).to_contain("Simple sliced (native)")
expect(report).to_contain("multicore_green_spawn_sliced used_runtime_pool=true, quick_done=true, parallelism=1, total=9")
