...
This document is the UI **identity + test** contract layer. It does not own
widget/event semantics or the WM/render stack, and must not fork them:

- `doc/04_architecture/ui/shared_ui_contract.md` — normative shared
  widget/event semantics. UiAccessNode/UIEvent assumptions here derive from it.
- `doc/04_architecture/os/shared_wm_stack.md` — WM ownership and renderer rules.
- `doc/04_architecture/tiny_ui_web_wm.md` (parallel session) — Tiny TUI/Web/GUI
  → TinyPane → TinyDrawStream → Tiny 2D → Tiny WM. Complementary, not
  competing: Tiny is a render/WM stack whose panes are a FUTURE driver target
  for the ActionPlan (a TinyWm adapter alongside TUI/GUI/Web/HTTP-remote).
- `src/lib/common/net/http_core.spl` — canonical HTTP request limits and
  static-path security (enterprise-owned; the web-host lane consumes it).
