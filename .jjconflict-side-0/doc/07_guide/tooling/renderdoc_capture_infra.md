...
- `scripts/setup/setup-gui-web-2d-vulkan-env.ps1 --browser-backing` runs the
  same direct Windows launch path, then forwards the shared
  `gui-web-2d-vulkan-browser-backing-status.js` validator rows. Treat a
  failing `gui_web_2d_vulkan_browser_backing_status` as authoritative even when
  Electron/Chrome ARGB capture passes.
