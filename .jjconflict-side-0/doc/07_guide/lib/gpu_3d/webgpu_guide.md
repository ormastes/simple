...
5. **Chrome/Electron Draw Evidence** — `std.gc_async_mut.gpu.browser_engine.chrome_webgpu_draw_evidence` launches a dedicated Electron app under `tools/web-render-backend/chromium-webgpu-draw` and records whether real Chrome WebGPU drawing produced pixels or whether the host explicitly lacks browser WebGPU support. The WASM Simple3D wrapper parses `simple3d:canvas:w,h:triangle:x1,y1,z1,x2,y2,z2,x3,y3,z3:rgba:r,g,b,a` payloads, draws a three-vertex WebGPU triangle when the host supports it, and preserves source/payload metadata through host-adaptive evidence.
6. **Chrome/Electron Compute Evidence** — `std.gc_async_mut.gpu.browser_engine.chrome_webgpu_compute_evidence` launches `tools/web-render-backend/chromium-webgpu-compute` for host-adaptive processing evidence. It accepts compiler-emitted WGSL for `simple_webgpu_add_u32` and `simple_2d_fill_u32`; the Simple2D fill profile uses the WebGPU `Simple2DParams` uniform contract, validates storage-buffer readback, and can carry WASM-shaped payload byte-count/checksum provenance through `chrome_webgpu_compute_wasm_simple2d_fill_payload_bytes_evidence(...)`. Compiler-side `portable_compute_browser_wasm_webgpu_bridge_plan(...)` records that this browser leg follows the native priority boundary but remains WGSL source-only with a browser host-import contract.
7. **Host/GPU Engine2D Lane Evidence** — `std.gpu.engine2d.backend_lane.Engine2dHostGpuEventFlowEvidence` records `target.later(...) gpu \:` event-flow data: event counts, Draw IR deltas, packet bounds, fallback state, event-to-present stages, p50/p95 baseline and candidate timings, pixel hash, and speedup. The canonical report is `doc/09_report/perf/host_gpu_lane_event_flow_perf_evidence_2026-06-14.md`; fallback probes and small software smokes are harness evidence only, not proof of real GPU less-ms behavior.
