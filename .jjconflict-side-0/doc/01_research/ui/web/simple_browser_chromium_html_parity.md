...
`src/app/wm_compare/html_compat.spl` owns HTML fixture capture and PPM/report output, `test/09_baselines/html_compat/<scene>/` stores Chromium/Simple/diff artifacts, and `src/lib/gc_async_mut/gpu/browser_engine/` owns DOM parsing, CSS style-block processing, layout, paint, and render-scene execution.
...
Implemented scope builds on `doc/04_architecture/ui/drawing_stack.md` and `doc/04_architecture/platform/cross_platform_wm.md` rather than replacing them.
