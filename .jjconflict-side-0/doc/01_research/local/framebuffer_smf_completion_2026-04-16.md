...
- `test/03_system/simpleos_desktop_with_apps_framebuffer_spec.spl` no longer crashes on `Option::None.len()` in baseline-read fallback logic.
...
- `test/03_system/simpleos_desktop_framebuffer_spec.spl`
- `test/03_system/simpleos_desktop_with_apps_framebuffer_spec.spl`
...
- `test/09_baselines/simpleos_desktop_framebuffer/README.md:83-84`
  documents that `--mode smf` still falls back to safe/static behavior.
- `test/03_system/multi_mode_test_runner_spec.spl`
...
1. First fix the shared QMP capture contract once, below the specs:
   - `test/03_system/qemu/os/common/qemu_os_harness.spl`
