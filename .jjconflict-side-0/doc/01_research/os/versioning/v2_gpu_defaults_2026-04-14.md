...
|---|---:|---|---|
| `backend_cpu` | 444 | **functional, tested** | `src/lib/gc_async_mut/gpu/engine2d/backend_cpu.spl:1`; pure-Simple Bresenham/scanline; always reachable via `Engine2D.detect_best_backend` fallback. Exercised by `test/02_integration/rendering/engine2d_primitives_spec.spl`, `test/03_system/engine2d_primitives_spec.spl`. |
...
2. **No CI spec runs vulkan against a real Mesa ICD** — only
   ...
   `test/02_integration/rendering/engine2d_backend_spec.spl` matrix entry
