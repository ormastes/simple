...
- **Acceptance-criteria:**
  ...
  - [x] `doc/04_architecture/platform/sffi_bidirectional_interop.md §9` carve-out note
        updated to reference this FR's resolution
- **Related-upfront:** `doc/02_requirements/feature/runtime_api_block_sugar_plugins.md`
  REQ-PLUG-003b
- **Related-design-doc:** `doc/04_architecture/platform/sffi_bidirectional_interop.md §9`
- **Related-issue:** none
...
- **Notes:** The extern still requires a Rust seed/runtime rebuild because no
  ...
  `plugin_call_f64(...)`; the fixture at `test/03_system/feature/plugin/fixtures/demo.c`
...
- **Verification (2026-05-29):** `DesugarRule` has `ast_rewrite_fn: i64` field in
  ...
  `test/03_system/feature/plugin/sugar_plugin_spec.spl` verify struct shape and sentinel round-trip.
...
- **Implementation:**
  ...
  - `test/03_system/feature/plugin/sugar_plugin_spec.spl` — `ast_rewrite_fn` field added
...
- **Verification (2026-05-30):** Added pure-Simple source-marker assertions to
  `test/03_system/feature/plugin/sugar_plugin_spec.spl` for the static insertion points in
  `sugar_registry.spl`, `collection_desugar.spl`, and
  `c_backend_translate_ops.spl`. Focused run passed:
  `SIMPLE_LIB=src bin/simple test test/03_system/feature/plugin/sugar_plugin_spec.spl --mode=interpreter --clean`
...
- **Notes:** Added `src/compiler/00.common/di_runtime_slots.spl` and
  `test/01_unit/compiler/di/di_runtime_slots_spec.spl`. The runtime-slot index is
...
- `desugar_collections` loop in `collection_desugar.spl` now calls
  `apply_sugar_rules_ast` after each built-in rewrite arm
- Two spec scenarios added to `test/03_system/feature/plugin/sugar_plugin_spec.spl`
...
Pure-Simple verification added to `test/03_system/feature/plugin/sugar_plugin_spec.spl`
