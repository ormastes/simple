...
| Baremetal TLS first encrypted handshake record fails authentication after CCS | 🔍 OPEN | High |
...
- Session 5: `doc/09_report/MODULE_SYSTEM_COMPLETE_FIX_2025-12-29.md`
- Session 6: `doc/09_report/BDD_FEATURE_DOC_SESSION6_COMPLETE_2025-12-29.md`
...
The `export foo as bar` and `use module.{foo as bar}` aliasing syntax causes parse errors in runtime/interpreter mode with error: **"expected expression, found As"**.

This affects:
1. **Export aliasing:** `export original_name as new_name`
2. **Import aliasing:** `use module.{original_name as alias}`

Both should be valid Simple syntax according to language design, but runtime parser doesn't recognize the `as` keyword in these contexts.

### Reproduction

**Test case:** `test/unit/bugs/export_as_runtime_bug_spec.spl`

**Module A (exporter.spl):**
```simple
fn compute_sum(arr: [i64]) -> i64:
    var total = 0
    for x in arr:
        total = total + x
    total

# Try to export with alias - FAILS
export compute_sum as sum  # Error: expected expression, found As
```

**Module B (importer.spl):**
```simple
# Try to import with alias - FAILS
use exporter.{compute_sum as sum}  # Error: expected expression, found As

fn main():
    sum([1, 2, 3])  # Should work but doesn't
```

### Expected Behavior

- `export foo as bar` exports function `foo` under name `bar`
- `use mod.{foo as bar}` imports `foo` from `mod` as local name `bar`
- Both compiled and runtime modes support this syntax

### Actual Behavior

- Runtime parser error: "expected expression, found As"
- Compiled mode may work (not tested)
- Parser doesn't recognize `as` as keyword in export/import context

### Current Workaround

Use thin wrapper functions for aliasing:

```simple
# Instead of: export compute_sum as sum
fn sum(arr: [i64]) -> i64:
    """Alias for compute_sum."""
    compute_sum(arr)
...
**Medium severity:**
- ✅ Workaround available (wrapper functions)
- ✅ Only affects runtime mode
- ❌ Verbose - adds boilerplate
- ❌ Makes refactoring harder (can't easily rename exports)
- ❌ Breaks language feature parity between modes

### Investigation Needed

1. Check if `as` keyword is in lexer token list
2. Verify parser grammar for export/use statements
3. Test if compiled mode supports this syntax
4. Add parser recovery for better error messages
5. Consider adding `alias` keyword as alternative syntax

### Related Issues

- Runtime limitations documented in MEMORY.md
- Part of larger runtime/compiled mode feature parity gaps
- May affect module compatibility if one mode supports it
