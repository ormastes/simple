...
**Report path:** `doc/09_report/cross_language_perf_2026-06-11_refresh_current.md`
...
|------------------------|--------------|--------------|--------------|--------------|
| Simple (interpreter)   |       27.337 |    interpret |              |              |
| Simple (SMF loader)    |       19.528 |          smf |              |              |
| Simple (native)        |        3.533 |       native |              |              |
| C (gcc -O2)            |        3.032 |       native |              |              |
| Go (compiled)          |       57.126 |       native |              |              |
| Python                 |       24.291 |    interpret |              |              |

## Warm Throughput — fib(35) in process (10 warmup + 1 measured)

| Language               |     Avg (ms) |                                    Notes |
|------------------------|--------------|------------------------------------------|
| Simple (interpreter)   |       89.427 | tree-walk (outer-process, no in-proc timing) |
| Simple (SMF loader)    |       82.290 | bytecode (outer-process, no in-proc timing) |
| Simple (native)        |       65.124 |           AOT via Cranelift (in-process) |
| C (gcc -O2)            |       15.208 |                             baseline AOT |
| Go                     |       55.343 |                                  SSA AOT |
| Python                 |     1638.606 |                         CPython bytecode |
...
| Simple (SMF loader)    |      148.453 |    std thread_spawn fork-join (bytecode) |
...
| Simple multicore green (SMF) |      146.492 | multicore_green runtime pool candidate (pool_used=100/100, parallelism=64/64, queue_model=work_stealing) |
| Simple multicore green (native) |      157.193 | multicore_green runtime pool candidate (pool_used=100/100, parallelism=64/64, queue_model=work_stealing) |
| C (pthreads)           |        9.414 |                               OS threads |
| Go                     |        8.248 |           goroutines + chan result (M:N) |
| Python                 |     2949.741 |                          threading (GIL) |
...
|------------------------|--------------|------------------------------------------|
| Simple (interpreter)   |          n/a |          extern thread FFI not supported |
| Simple (SMF loader)    |       94.044 |                  std thread_spawn fanout |
| Simple (native)        |         fail | OS-thread fork-join fanout (checksum mismatch) |
| Simple cooperative green (interp) |       32.565 |                 cooperative queue fanout |
...
| Simple multicore green (SMF) |       38.775 | multicore_green runtime pool fanout (pool_used=1000/1000, parallelism=64/64, queue_model=work_stealing) |
| Simple multicore green (native) |       14.143 | multicore_green runtime pool fanout (pool_used=1000/1000, parallelism=64/64, queue_model=work_stealing) |
| C (pthreads)           |       68.147 |              one OS thread per tiny task |
| Go                     |        8.036 |        goroutine per tiny task + channel |
| Python                 |      132.823 |            threading per tiny task (GIL) |

## Simple vs Go vs C Large Fanout Stress — spawn 512 tiny workers (1 runs avg)

| Language               |     Avg (ms) |                        Concurrency model |
|------------------------|--------------|------------------------------------------|
| C (pthreads)           |       42.542 |                  pthread per stress task |
| Simple multicore green (native) |       12.980 | multicore_green stress fanout (pool_used=512/512, parallelism=64/64, queue_model=work_stealing) |
| Go                     |        5.547 |          goroutine per stress task (M:N) |
