...
**Symptom**: `bin/simple test` returns PASS while `bin/simple test/01_unit/.../<spec>.spl` returns FAIL on the same spec. Caused by release binary's `--mode=native` no-op-stub generation for unresolved std calls + `--mode=smf` swallowing runtime errors (per `feedback_compile_mode_false_greens.md`).
