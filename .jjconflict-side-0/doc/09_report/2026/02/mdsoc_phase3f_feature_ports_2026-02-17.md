...
**Date:** 2026-02-17
**Status:** Complete
**Note:** A subsequent one-struct-per-file refactor (2026-03-02) split all `ports.spl` files into individual port struct files. See `doc/09_report/compiler_mdsoc_migration.md` for details.
...
`test/01_unit/compiler/mdsoc/doc_validation_spec.spl` had API calls that no longer matched the current MDSOC structs. Fixed:
...
- `test/01_unit/compiler/transform_adapters_spec.spl` — +9 tests for `TokenStreamView` (Phase 3f)
- `test/01_unit/compiler/feature_ports_spec.spl` — +18 tests for monomorphization, optimization, and linking ports
...
Tests added:
- `test/01_unit/compiler_core/entity/entity_span_spec.spl` — 16 tests for SourceLocation + Span
- `test/01_unit/compiler/transform_adapters_spec.spl` — +24 tests (8 each for MirOptView, ObjectFileView, LoadedModuleView)
