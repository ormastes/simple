...
class EditorMarkerManager {
    ...
    toggleBreakpoint(editor) {
        this.toggleLine(editor.document.uri.toString(), editor.selection.active.line, 'breakpoint');
        this.refresh(editor);
    }
    toggleBookmark(editor) {
        this.toggleLine(editor.document.uri.toString(), editor.selection.active.line, 'bookmark');
        this.refresh(editor);
    }
    togglePointer(editor) {
        const key = editor.document.uri.toString();
        const state = this.getOrCreateState(key);
        state.pointerLine = state.pointerLine === editor.selection.active.line ? null : editor.selection.active.line;
        this.refresh(editor);
    }
    clearPointer(editor) {
        this.getOrCreateState(editor.document.uri.toString()).pointerLine = null;
        this.refresh(editor);
    }
