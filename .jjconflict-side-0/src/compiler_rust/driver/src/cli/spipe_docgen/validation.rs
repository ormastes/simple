//! Validation of SPipe documentation quality
...
pub fn validate_spec(spipe_doc: &SspecDoc) -> ValidationResult {
    let mut warnings = Vec::new();
    let mut has_sections = HashSet::new();

    let has_docs = !spipe_doc.doc_blocks.is_empty();
    let doc_lines: usize = spipe_doc.doc_blocks.iter().map(|b| b.content.lines().count()).sum();
    ...
    ValidationResult {
        file_path: spipe_doc.file_path.clone(),
        has_docs,
        doc_lines,
        has_sections,
        warnings,
