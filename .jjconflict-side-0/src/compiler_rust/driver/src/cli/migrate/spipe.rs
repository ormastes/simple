//! SPipe docstring migration
//!
//! Converts print-based SPipe tests to intensive docstring format.
//! Example:
...
/// Migrate print-based SPipe tests to docstring format
pub fn migrate_spipe_docstrings(path: &Path, dry_run: bool) -> i32 {
    if !path.exists() {
    ...
    if dry_run {
        println!(
            "DRY RUN: Previewing SPipe docstring migration for {} file(s)...",
            files.len()
        );
    } else {
        println!(
            "Migrating {} SPipe test file(s) to intensive docstring format...",
            files.len()
        );
    }
    ...
    for file in &files {
        match migrate_file_spipe(file, dry_run) {
            Ok(true) => {
                modified_count += 1;
                if dry_run {
    ...
    if dry_run {
        ...
        if modified_count > 0 {
            ...
            println!("See doc/examples/spipe_conversion_example.md for migration guide");
        }
    } else {
        ...
        if modified_count > 0 {
            ...
            println!();
            println!("See doc/examples/spipe_conversion_example.md for best practices");
        }
    }
...
/// Migrate a single file's SPipe print-based tests to docstring format
/// Returns Ok(true) if file was/would be modified, Ok(false) if no changes needed
pub fn migrate_file_spipe(path: &Path, dry_run: bool) -> Result<bool, String> {
    eprintln!("[MIGRATE_SPIPE] Processing file: {}", path.display());
    let content = fs::read_to_string(path).map_err(|e| format!("failed to read file: {}", e))?;

    let new_content = migrate_spipe_to_docstrings(&content);
...
/// Convert print-based SPipe patterns to intensive docstring format
fn migrate_spipe_to_docstrings(source: &str) -> String {
...
/// Convert a single print line to SPipe syntax with docstring
...
/// Extract text from print statement and convert to SPipe syntax
...
if ex.code == "1 + 1":
    ...
    passed = passed + 1"#;
        ...
        let output = migrate_spipe_to_docstrings(input);

        // Should have proper SPipe structure
        assert!(output.contains(r#"describe "Docstring parsing":"#));
        assert!(output.contains(r#"context "extracts code blocks":"#));
        assert!(output.contains(r#"it "finds code in docstrings":"#));
