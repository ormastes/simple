...
impl CompilerPipeline {
    ...
    /// Enable coverage instrumentation (#674)
    ///
    /// When enabled, the compiler inserts coverage probes at:
    /// - Decision points (if, while, for, match conditions)
    /// - Condition points (individual parts of compound boolean expressions)
    /// - Path points (entry to each basic block)
