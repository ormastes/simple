//! BDD SFFI bridge for native/SMF test execution modes.
//!
//! Provides runtime SFFI functions that mirror the interpreter's BDD state
//! management (describe/it/expect), enabling compiled tests to use the same
//! BDD framework without the interpreter.
//!
//! These functions are called by Cranelift-compiled code via the runtime SFFI
...
pub extern "C" fn rt_bdd_expect_eq_rv(actual: i64, expected: i64) {
    let equal = crate::value::sffi::equality::rt_native_eq(actual, expected) == 1;
    if !equal {
