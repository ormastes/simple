...
impl BytecodeVM {
    ...
    /// Read a u16 from bytecode without bounds checking (hot path).
    ///
    /// # Safety
    /// Caller must ensure `self.ip + 2 <= self.code.len()`.
    #[inline(always)]
    unsafe fn read_u16_unchecked(&mut self) -> u16 {
        let ptr = self.code.as_ptr().add(self.ip);
        self.ip += 2;
        u16::from_le_bytes([*ptr, *ptr.add(1)])
    }

    /// Read an i32 from bytecode without bounds checking (hot path).
    ///
    /// # Safety
    /// Caller must ensure `self.ip + 4 <= self.code.len()`.
    #[inline(always)]
    ...
    /// Read an i64 from bytecode without bounds checking (hot path).
    ///
    /// # Safety
    /// Caller must ensure `self.ip + 8 <= self.code.len()`.
    #[inline(always)]
    ...
    pub fn execute(&mut self) -> VmResult<RuntimeValue> {
        ...
        loop {
            ...
            // Read opcode (safe - we just checked bounds)
            let opcode = if self.ip + 2 <= code_len {
                unsafe { self.read_u16_unchecked() }
            } else {
                return Err(VmError::UnexpectedEndOfCode);
            };
            ...
            match opcode {
                ...
                // =============================================================
                // Super-Instructions (fused for performance)
                // =============================================================
                CONST_I64_PUSH => {
                    let value = self.read_i64()?;
                    self.push(RuntimeValue::from_int(value))?;
                }
                ...
                ADD_I64_POP => {
                    ...
                    let b = self.pop()?;
                    let a = self.pop()?;
                    self.set_stack(dest, RuntimeValue::from_int(a.as_int() + b.as_int()))?;
                }
                ...
                LT_JMP_IF_NOT => {
                    ...
                    let b = self.pop()?;
                    let a = self.pop()?;
                    let is_lt = if a.is_int() && b.is_int() {
                        a.as_int() < b.as_int()
                    } else {
