...
pub fn export_ast_json<W: Write>(module: &AstModule, writer: &mut W) -> ExportResult {
    ...
    writeln!(writer, "{}", json)
        .map_err(|e| format!("Failed to write AST JSON: {}", e))
}
    ...
    match path {
        Some(p) => {
            let mut file = File::create(p)
                .map_err(|e| format!("Failed to create file {:?}: {}", p, e))?;
...
pub fn export_hir_json<W: Write>(module: &HirModule, writer: &mut W) -> ExportResult {
    ...
    writeln!(writer, "{}", json)
        .map_err(|e| format!("Failed to write HIR JSON: {}", e))
}
...
pub fn export_mir_json<W: Write>(module: &MirModule, writer: &mut W) -> ExportResult {
    ...
    writeln!(writer, "{}", json)
        .map_err(|e| format!("Failed to write MIR JSON: {}", e))
}
