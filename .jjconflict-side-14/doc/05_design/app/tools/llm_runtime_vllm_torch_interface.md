...
## 2026-07-01 OpenCode Kill and SGLang-Derived Serve Metadata

<!-- codex-design -->

Implementation slice:

- Add `src/app/llm_caret/opencode_cli.spl` with pure
  `build_opencode_args(...)`, response parsing, spawn, running-status, and
  pid-scoped kill helpers.
- Register `opencode_cli` in `src/app/llm_caret/provider.spl` and
  `src/app/llm_caret/config.spl` without changing existing defaults.
- Extend `LlmRuntimeManifest` with backend and SGLang-style parallel/memory
  intent fields while preserving the existing constructor as vLLM defaults.
- Extend `llm_runtime_static_serve_plan(...)` and lifecycle command generation
  so vLLM previews stay unchanged and SGLang previews use
  `python -m sglang.launch_server --model-path ...` with redacted model/path
  evidence.

Runtime-adjacent decision record:

- `runtime_need`: OpenCode-like CLI providers can hang or leave long-running
  agent processes; runtime serving needs backend metadata for SGLang parity
  planning.
- `facade_checked`: `app.io.mod.process_run`, `process_spawn_async`,
  `process_is_running`, and `process_kill`; existing vLLM serve lifecycle.
- `chosen_path`: reuse process facades and add backend metadata to the existing
  runtime manifest/serve-plan owner.
- `rejected_shortcuts`: shell `kill`, process-name killing, local
  `rt_process_*` imports in app providers, and claiming live SGLang endpoint
  readiness from command previews.
