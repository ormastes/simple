...
## 2026-07-01 OpenCode Kill and SGLang Metadata Continuation

Owner: Codex

Sidecar lanes: N/A. The continuation is a narrow owner-boundary change in
existing LLM provider/runtime files.

Merge owner: Codex. Final reviewer: normal/highest-capability Codex pass before
goal completion. Shared helper names: `build_opencode_args`,
`opencode_cli_kill`, `llm_runtime_manifest_with_backend`.

Tasks:

1. Add OpenCode CLI argument construction, response classification, spawn, poll,
   and pid kill helpers under `src/app/llm_caret`.
2. Register `opencode_cli` provider/config defaults without changing
   `claude_cli` as the default provider.
3. Extend runtime manifest/serve-plan evidence with backend metadata and
   SGLang-style parallel/memory intent while keeping vLLM as default.
4. Add focused unit coverage for OpenCode argument/kill semantics and
   backend-aware serve-plan evidence.
5. Run focused Simple checks once plus direct-env/runtime guard.

Runtime-adjacent decision record:

- `runtime_need`: provide pid-scoped kill evidence for OpenCode-like CLI agent
  runs and backend-neutral serve-plan metadata for SGLang-inspired deployment
  knobs.
- `facade_checked`: `app.io.mod` process facades and existing vLLM serve
  lifecycle.
- `chosen_path`: `reuse-facade`; add only provider/runtime owner helpers.
- `rejected_shortcuts`: shell kill commands, process-name matching, local raw
  `rt_process_*` imports, dashboard-owned side effects, and live SGLang
  readiness claims without host proof.
