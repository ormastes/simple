...
*Source: `simple/std_lib/test/features/ml/config_system_spec.spl`*
...
val config = Conf.load("config.sdn")
val validated = Conf.validate(config, TrainConfig)
# Automatic type coercion: "100" → 100, "0.001" → 0.001
```
...
error: circular reference in config
 ...
 3 |     b: "${a}"
   ...
   |
   = note: circular reference chain: a → b → a
```
...
| Type Safety | ✅ Schema | ✅ Structured | ❌ No | ❌ No |
| Interpolation | ✅ `${var}` | ✅ `${var}` | ✅ `${var}` | ❌ No |
| CLI Overrides | ✅ Dotlist | ✅ Dotlist | ✅ Dotlist | ❌ No |
| Resolvers | ✅ Custom | ✅ Custom | ✅ Custom | ❌ No |
| Immutability | ✅ Freeze | ✅ Freeze | ❌ No | ❌ No |
| Native Lang | ✅ Yes | ❌ Python | ❌ Python | N/A |
...
**Current Status:**
- ✅ SDN parser implemented
- ⏳ Conf API not yet implemented
- ⏳ Awaiting schema/validation system
- ⏳ Awaiting CLI argument parsing integration
...
## Command-Line Configuration Overrides
    ...
    **Type Coercion:**
    Automatically infer types from syntax:
        - Integers: `batch_size=32` → 32
    - Floats: `lr=0.001` → 0.001
    - Booleans: `use_gpu=true` → true
    - Strings: `name="experiment"` → "experiment"
    - Arrays: `dims=[128,256,512]` → [128, 256, 512]
    - Dicts: `config={a:1,b:2}` → {a: 1, b: 2}
    ...
    **Implementation:** Parse dotlist → create nested dict → merge with base config
...
**Given** dotlist arguments with various value formats
            ...
            **Type Inference Rules:**
            ...
            "false"        → false (bool)    # Boolean
            "hello"        → "hello" (text)  # String (no quotes)
            "\"hello\""    → "hello" (text)  # String (with quotes)
            "[1,2,3]"      → [1, 2, 3]       # Array
            ...
            **Quote Handling:**
            ```simple
            name="my experiment"  # With quotes → "my experiment"
            name=my_experiment    # No quotes → "my_experiment"
            ```
...
## Variable Interpolation and Custom Resolvers
    ...
    **Circular Reference Detection:**
    Detects and reports cycles: `a → b → c → a`
...
**Given** a config with variable references (`${var}`)
        ...
        **Nested References:**
        ```simple
        {
            root: "/data",
            base: "${root}/project",
            train: "${base}/train"  # → "/data/project/train"
        }
        ```
...
**Given** a custom resolver function registered
            ...
            **Implementation:** Resolver registry (dict of name → function)
...
## Type-Safe Schema Validation
    ...
    early. Automatic type coercion handles string→int conversions common in CLI overrides.
...
**Given** a config with string values (common from CLI/env vars)
        ...
        **Conversion Rules:**
        ...
        text → bool:  "true" → true, "false" → false
        i32 → f64:    100 → 100.0
        ```
