...
*Source: `simple/std_lib/test/features/data_structures/tuples_spec.spl`*
...
## Tuple Literal Syntax
    ...
    **Type Inference:**
    The tuple's type is inferred from its elements:
        - `(1, 2)` → Tuple<i64, i64>
    - `(42, 'text')` → Tuple<i64, text>
    - `(true, 1, 'x')` → Tuple<bool, i64, text>
...
**Given** a tuple literal with two integer values
        ...
        **Access Patterns:**
        - `pair[0]` → 1 (first element)
        - `pair[1]` → 2 (second element)
        - `pair[2]` → ERROR: Index out of bounds
...
**Given** a tuple containing other tuples
        ...
        **Chained Access:**
        - `nested[0]` → `(1, 2)` (first inner tuple)
        - `nested[0][0]` → `1` (first element of first tuple)
        - `nested[1]` → `(3, 4)` (second inner tuple)
        - `nested[1][1]` → `4` (second element of second tuple)
...
**Given** a tuple containing arrays
        ...
        **Access Patterns:**
        - `combo[0]` → `[1, 2]` (first array, type: Array<i64>)
        - `combo[0][0]` → `1` (first element of first array)
        - `combo[1]` → `[3, 4]` (second array)
        - `combo[1][1]` → `4` (second element of second array)
