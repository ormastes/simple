...
- Backend-qualified measurement validation now exists in `src/app/wm_compare/backend_measurement_report.spl` and `test/03_system/wm_compare/backend_measurement_report_spec.spl`, including binary-size delta reporting and scalar-baseline enforcement for initialized lanes.
- Host measurement capture now exists in `src/app/wm_compare/backend_measurement_capture.spl` and `test/03_system/wm_compare/backend_measurement_capture_spec.spl`.
