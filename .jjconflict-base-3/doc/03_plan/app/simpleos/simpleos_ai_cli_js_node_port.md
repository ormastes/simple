...
`test/03_system/feature/js/node_api_conformance_spec.spl`. Full QEMU runtime smoke remains
...
- Host FAT32 ingestion readiness:
  `test/01_unit/os/port/host_fat32_tree_populator_spec.spl` now proves the generic
...
- Harness image population: `--populate-fat32-image <img>` connects the staged
  ...
  mirror fails.
- VFS-manager file grants: `test/01_unit/os/services/vfs/vfs_spec.spl` now proves
