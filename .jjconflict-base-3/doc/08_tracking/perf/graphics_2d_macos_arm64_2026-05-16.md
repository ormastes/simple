...
The pre-built binary at `test/05_perf/graphics_2d/c_reference/bench_2d` is an
...
```
clang -O2 -o /tmp/bench_2d_arm64 test/05_perf/graphics_2d/c_reference/bench_2d.c
```
...
Command: `bin/simple run test/05_perf/graphics_2d/bench_2d_gpu.spl`
...
Command: `bin/simple run test/05_perf/graphics_2d/bench_2d_gpu_batch.spl`
...
## 3. Metal-targeting Benchmark Files in `test/05_perf/`

Three files reference Metal:

| File | Role |
|------|------|
| `test/05_perf/graphics_2d/metal_smoke_spec.spl` | Spec (Phase 5 pending) — correctness via sync_readback pixel hash |
| `test/05_perf/graphics_2d/backend_probe_spec.spl` | Probe result spec — includes Metal probe expectations |
| `test/05_perf/graphics_2d/no_duplication_spec.spl` | Structural test — checks backend name constant uniqueness |
