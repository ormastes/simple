...
- Only when that verifier mechanism is unavailable may owner ID `2378857` explicitly dispatch `authority_class=owner_attested_actions` from trusted `main` with `NO-VERIFY:OWNER-PROOF`. The dedicated `owner-convergence-admission` environment permits its own reviewer while release/npm protections remain unchanged. The ten-minute receipt declares `verification_performed=false` and `github_pr_approval_claimed=false`, binds exact run/workflow/config/environment/rulesets/PR/parents/candidate/manifest/ports/checks, retains unavailability proof, and is Actions-attested. Candidate admission downloads the exact run/artifact/digest and verifies trusted-main signer identity/ref/digest with self-hosted runners denied. Same author/merger is allowed only on this fallback; normal external-broker mode retains inequality.
