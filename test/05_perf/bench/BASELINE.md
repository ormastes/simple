...
## Bench: spostgre WAL append (`test/perf/bench/spostgre_wal_append.spl`)
...
The `test.perf.bench.lib.timing` library uses the corrected formula `(iters*1_000_000)/total_ns`.
...
## Bench: NVFS arena throughput (`test/perf/bench/nvfs_arena_throughput.spl`)
...
## Bench: fs_driver MountTable (`test/perf/bench/fs_driver_mount_table.spl`)
...
## Bench: run_all (`test/perf/bench/run_all.spl`)
...
`test/perf/bench/lib/timing.spl` updated:
