...
static int gpu_run(const struct api *api, CUfunction kernel, unsigned int count, uint32_t value,
    ...
    sample->total_us = elapsed_us(started, completed);
...
int main(int argc, char **argv) {
    ...
    if (argc < 6 || argc > 13 || !parse_int(argv[3], 1, 64, &warmups) || !parse_int(argv[4], 5, 64, &samples)) return 2;
    batch_count = argc - 5;
    if (batch_count < 2 || batch_count > 8) return 2;
    ...
    fprintf(raw, "# batch sample cpu_us allocation_us launch_sync_us readback_us conversion_cleanup_us total_us mismatch_count\n");
    for (row = 0; row < batch_count; row++) {
        ...
        for (sample = 0; sample < warmups + samples; sample++) {
            ...
            if (sample >= warmups) {
                ...
                fprintf(raw, "%u %d %lld %lld %lld %lld %lld %lld %lld\n", count, measured, current.cpu_us,
    ...
    fclose(raw);
    write_receipt(argv[2], device_name, uuid_text, identity, argv[1], warmups, samples, batches, medians, batch_count, first_fast);
    for (row = 0; row < batch_count; row++) {
