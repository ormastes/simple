...
| gc_ehframe_a64.o | `clang -c -O1 -fPIC -fexceptions -ffunction-sections -fdata-sections gc_ehframe_a64.c`. Carries `.eh_frame` plus a `never_called_fn` nothing reaches. Until 2026-09-19 `--gc-sections` REFUSED any input with `.eh_frame` ("does not model per-FDE liveness"), which made the flag unusable on real compiler output; `.eh_frame` is now an unconditional GC root, so this links, runs (exit 42) and still collects unreferenced data. `never_called_fn` survives where lld drops it — the recorded conservative divergence |
