/* Test-only extraction of the production C FSBL owner.
 *
 * The coverage producer and direct host runner reject this oracle unless its
 * text from the first include through EOF exactly matches production
 * cosmos_fsbl.c. Production firmware continues to link cosmos_fsbl.c.
 */
