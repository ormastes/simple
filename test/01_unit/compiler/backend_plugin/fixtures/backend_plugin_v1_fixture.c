...
#endif
#ifndef SBP_OBJECT_TEXT
#define SBP_OBJECT_TEXT "object-ok"
#endif
...
static int32_t open_(const simple_backend_request_v1 *r,uint64_t *s){if(SBP_FAIL_STAGE==1)return 31;if(!r||!s||r->abi_version!=1||r->struct_size<sizeof(*r))return 20;*s=0x51A7E;return 0;}
...
static int32_t finalize_(uint64_t s,simple_backend_compile_result_v1 *o){if(SBP_FAIL_STAGE==3)return 33;return s==0x51A7E?owned(SBP_OBJECT_TEXT,2,o):22;}
...
static int32_t close_(uint64_t s){if(s!=0x51A7E)return 25;closed=1;return 0;}
...
__attribute__((destructor)) static void unloaded(void){const char*p=getenv("SBP_FIXTURE_UNLOAD_MARKER");if(p){FILE*f=fopen(p,"w");if(f){fprintf(f,"closed=%d releases=%d\n",closed,releases);fclose(f);}}}
