...
| option_pattern_accepted_on_non_option_scrutinee_2026-07-27.md | 2026-07-27 | STILL-OPEN | ~~`eval_methods.spl:107` Option handling still gated on `kind == VAL_STRUCT`~~ — **evidence retracted 2026-08-01: that file was a dead duplicate, deleted `f97dfbbb8ee`; the live `eval_method_call` has no Option/Result built-in block at all** (see the target doc's NOW-WRONG note); `method_calls_literals.spl:388-396` still emits `rt_unwrap_or_self` — **that half was verified on live code and still carries the STILL-OPEN verdict.** | high |
