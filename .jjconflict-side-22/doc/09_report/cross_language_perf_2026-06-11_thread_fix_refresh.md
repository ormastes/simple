...
**Report path:** `doc/09_report/cross_language_perf_2026-06-11_thread_fix_refresh.md`
...
|------------------------|--------------|--------------|--------------|--------------|
| Simple (interpreter)   |       23.061 |    interpret |              |              |
| Simple (SMF loader)    |       23.001 |          smf |              |              |
| Simple (native)        |        4.062 |       native |              |              |
| C (gcc -O2)            |        3.521 |       native |              |              |
| Go (compiled)          |       75.766 |       native |              |              |
| Python                 |       20.207 |    interpret |              |              |

## Warm Throughput — fib(35) in process (10 warmup + 1 measured)

| Language               |     Avg (ms) |                                    Notes |
|------------------------|--------------|------------------------------------------|
| Simple (interpreter)   |       93.188 | tree-walk (outer-process, no in-proc timing) |
| Simple (SMF loader)    |       96.360 | bytecode (outer-process, no in-proc timing) |
| Simple (native)        |        65.30 |           AOT via Cranelift (in-process) |
| C (gcc -O2)            |       14.297 |                             baseline AOT |
| Go                     |       52.651 |                                  SSA AOT |
| Python                 |     1617.304 |                         CPython bytecode |
...
| Simple (SMF loader)    |      142.299 |    std thread_spawn fork-join (bytecode) |
| Simple (native)        |         fail |        thread_spawn fork-join OS threads |
| Simple cooperative green (interp) |       53.000 | cooperative_green_spawn cooperative queue |
| Simple cooperative green (SMF) |       39.057 | cooperative_green_spawn cooperative queue |
| Simple cooperative green (native) |       22.291 | cooperative_green_spawn cooperative queue |
| Simple multicore green (SMF) |      183.730 | multicore_green runtime pool candidate (pool_used=100/100, parallelism=64/64, queue_model=work_stealing) |
| Simple multicore green (native) |      173.734 | multicore_green runtime pool candidate (pool_used=100/100, parallelism=64/64, queue_model=work_stealing) |
| C (pthreads)           |       11.496 |                               OS threads |
| Go                     |        9.666 |           goroutines + chan result (M:N) |
| Python                 |     3147.089 |                          threading (GIL) |
...
|------------------------|--------------|------------------------------------------|
| Simple (interpreter)   |          n/a |          extern thread FFI not supported |
| Simple (SMF loader)    |       96.518 |                  std thread_spawn fanout |
| Simple (native)        |         fail | OS-thread fork-join fanout (checksum mismatch) |
| Simple cooperative green (interp) |       34.121 |                 cooperative queue fanout |
| Simple cooperative green (SMF) |       20.778 |                 cooperative queue fanout |
| Simple cooperative green (native) |        4.131 |                 cooperative queue fanout |
| Simple multicore green (SMF) |       30.750 | multicore_green runtime pool fanout (pool_used=1000/1000, parallelism=64/64, queue_model=work_stealing) |
| Simple multicore green (native) |       15.780 | multicore_green runtime pool fanout (pool_used=1000/1000, parallelism=64/64, queue_model=work_stealing) |
| C (pthreads)           |       61.783 |              one OS thread per tiny task |
| Go                     |        8.927 |        goroutine per tiny task + channel |
| Python                 |      151.244 |            threading per tiny task (GIL) |

## Simple vs Go vs C Large Fanout Stress — spawn 512 tiny workers (1 runs avg)

| Language               |     Avg (ms) |                        Concurrency model |
|------------------------|--------------|------------------------------------------|
| C (pthreads)           |       34.787 |                  pthread per stress task |
| Simple multicore green (native) |       13.798 | multicore_green stress fanout (pool_used=512/512, parallelism=64/64, queue_model=work_stealing) |
| Go                     |        5.799 |          goroutine per stress task (M:N) |
...
| Simple cooperative green (SMF) |         7.3 KB | current OS thread |    + runtime 51.9 MB |
...
|------------------------|----------------|----------------|----------------------|
| Simple (native)        |         4.0 MB |         2.8 MB |                ~12KB |
