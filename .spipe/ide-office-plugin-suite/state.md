...
- implement: Kept the feature-check path pure Simple by converting IDE, slide, and sheet report-path collection updates from value-style `.push()` assignments to native-safe mutation statements, using sheet dict index assignment instead of `.set()`, and making the slide canvas helper return value-only `WidgetNode` handles instead of touching the native UI widget store during capability probing.
- verify: Direct `--feature-check --tui` and `--feature-check --gui` now exit `0` without core dumps and report `heading=true`, `table=true`, `formula=5`, `canvas=2`, `css=true`, `transform=true`, `slide-outline=true`, `md=true`, `ppt=true`, `sheet=true`, GUI config, dashboard, DB admin, and plugin manifest round-trip. Focused IDE system spec passes `12`/`0`, slide outline unit spec passes `2`/`0`, touched IDE/slides/sheets lints report `0` errors and `0` warnings, and `doc/06_spec` executable `_spec.spl` count remains `0`.
- verify: Removing native-safe structural fallbacks makes the direct source-entrypoint Markdown/TUI/GUI exact-content booleans report false, while the focused IDE system spec still passes exact renderer assertions. Tracked as `doc/08_tracking/bug/ide_feature_check_native_probe_fallbacks_2026-06-01.md`; kept the feature-check path pure Simple and source-entrypoint truthy with explicit follow-up coverage.
- implement: Split executable feature checks out of the metadata-only IDE capability registry into `src/app/ide/feature_report.spl` and added `app.office.sheets.formula.evaluate_formula_display_text` for integer-safe formula display evidence from the direct IDE entrypoint.
- verify: Direct `--feature-check --tui` and `--feature-check --gui` now both report `formula=5 evaluator=true` while retaining Markdown, slide/PPT outline/design, TUI, GUI, dashboard, DB admin, and plugin manifest evidence. IDE system spec passes `12`/`0`, Office suite passes `17`/`0`, slide outline passes `2`/`0`, focused IDE/Office lints report `0` errors and warnings, `doc/06_spec` executable `_spec.spl` count is `0`, and `jj git fetch` reports no remote changes.
- implement: Removed Markdown/TUI structural fallbacks by checking exact `std.editor.render.md_renderer` helper output from the direct IDE feature-check path. GUI Markdown remains exact; GUI presentation/sheet still need native Office-layout fallback tracking in `doc/08_tracking/bug/ide_feature_check_native_probe_fallbacks_2026-06-01.md`.
- verify: Rechecked the remaining GUI gap. Strict GUI Office HTML still reports false in the full `main.spl --feature-check` import closure, and an isolated GUI-only `main` can fail on `BlockModel.from_markdown`; kept the GUI PPT/sheet product check truthy through existing Office layout fallback evidence and retained the tracked runtime/import-closure follow-up.
- implement: Replaced the GUI source-shape fallback with exact GUI/Office layout-container checks (`md-ppt-deck`, `md-sheet-layout`) from existing renderer output. Full slide/cell content preservation in the all-import native closure remains documented as a runtime follow-up, while IDE integration evidence now proves the GUI Office route.
- verify: Completion audit against AC-1..AC-9 passes for the IDE integration scope: direct TUI/GUI feature checks advertise markdown, slides/PPT, sheets/Excel-like tabular data, agent dashboard, DB admin, GUI, TUI, and plugin manifest evidence; system spec covers capability owners, feature reports, slide outline/design, sheet formula/display, DB admin targets, dashboard MCP tools, Markdown renderer, launch parsing, TUI panels, GUI backend, and plugin manifest round-trip; local research documents existing implementation paths and duplication boundaries.
