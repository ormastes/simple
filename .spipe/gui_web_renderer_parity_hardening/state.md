...
- impl: Added `scripts/check/check-gui-renderdoc-feature-coverage-status.shs` to summarize WidgetKind HTML renderer dispatch coverage, the 18-case Electron Simple Web layout manifest, production parity evidence status, and RenderDoc completion gates without relaunching heavy GUI/browser capture.
- verify: 2026-06-19 refresh fixed local `bin/simple` discovery for Electron layout and generated-GUI parity evidence. The 18-case Electron Simple Web layout manifest now passes with 16 exact cases, 2 tracked text divergences, and 0 hard failures; the opacity matrix is exact. The broader production generated-GUI matrix now runs but remains divergent at 80x64 (`mismatch_count=1204`, `text_normalization_pixels=640`, `other_mismatch_pixels=1204`), so it is tracked separately from the layout manifest pass.
