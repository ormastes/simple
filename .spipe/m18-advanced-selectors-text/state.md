...
- `test/08_web_platform/css/` (WPT tests)
...
- `test/08_web_platform/css/selector_color_subset_spec.spl` — canonical test pattern for pixel-rendering WPT tests. Uses `render_html_to_pixels_with_viewport(html, WIDTH, HEIGHT)`, `_count_color(pixels, color)`, and `_renders_color(style, body, color)` helpers. Tests verify that specific CSS rules produce expected pixel colors. This is the template for new pseudo-element and text-overflow tests.
- `test/08_web_platform/css/wpt_scorecard_spec.spl` — pure-function WPT tests (no rendering). Not the right template for M18 render-based tests.
- 9 existing WPT test files in `test/08_web_platform/css/`. No existing pseudo-element, text-overflow, or word-break tests.
...
- REQ-5 (from AC-5): Add 8+ WPT tests using `render_html_to_pixels_with_viewport` pattern covering `::before`, `::after`, `text-overflow: ellipsis`, `word-break: break-all`, `overflow-wrap: break-word` -- area: `test/08_web_platform/css/` (new spec file)
- REQ-6 (from AC-6): Run full WPT suite after changes, verify 104/104 still pass -- area: `test/08_web_platform/css/`
...
| pseudo_and_text_layout_wpt_spec | `test/08_web_platform/css/pseudo_and_text_layout_wpt_spec.spl` | 8+ WPT tests covering `::before`, `::after`, `text-overflow: ellipsis`, `word-break: break-all`, `overflow-wrap: break-word` using pixel-rendering verification | New |
...
### Spec Files
- `test/08_web_platform/css/pseudo_text_wpt_spec.spl` — 8 specs covering AC-1, AC-2, AC-3, AC-4

### AC Coverage Matrix
| AC | Spec File | it block | Status |
|----|-----------|----------|--------|
| AC-1 | `test/08_web_platform/css/pseudo_text_wpt_spec.spl` | "renders before pseudo-element content text on empty div" | Failing (no impl) |
| AC-1 | `test/08_web_platform/css/pseudo_text_wpt_spec.spl` | "renders before pseudo-element on empty element" | Failing (no impl) |
| AC-2 | `test/08_web_platform/css/pseudo_text_wpt_spec.spl` | "renders after pseudo-element content text on empty div" | Failing (no impl) |
| AC-2 | `test/08_web_platform/css/pseudo_text_wpt_spec.spl` | "renders after pseudo-element on empty element" | Failing (no impl) |
| AC-2 | `test/08_web_platform/css/pseudo_text_wpt_spec.spl` | "renders both before and after pseudo-elements on empty div" | Failing (no impl) |
| AC-3 | `test/08_web_platform/css/pseudo_text_wpt_spec.spl` | "truncates overflowing text with ellipsis" | Failing (no impl) |
| AC-4 | `test/08_web_platform/css/pseudo_text_wpt_spec.spl` | "break-all wraps long word onto second line" | Failing (no impl) |
| AC-4 | `test/08_web_platform/css/pseudo_text_wpt_spec.spl` | "overflow-wrap break-word wraps long word onto second line" | Failing (no impl) |
