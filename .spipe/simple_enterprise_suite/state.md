...
## W16-A — in-guest OVMF board-runnable gate UNBLOCKED (2026-08-17)

Lane W15-C found `check-enterprise-store-in-guest-ovmf.shs` blocked by a kernel
`can_release` ABI mismatch. Reproduced, root-caused, fixed, verified end-to-end.

- Reproduced (Rust seed native-build, the gate's exact kernel-build command):
  `callable ABI mismatch for ...SharedDmaMapping_dot_can_release: Instance target
  declares 1 parameter(s), call supplies 2 explicit argument(s) and 1 receiver
  slot(s)` at `memory_swap_coordinator.spl:234` (can_release_swapped) and
  `memory_swap_runtime.spl:131` (can_release_range_in). EXIT=1.
- Root cause: `MemoryLevelingManager` had NO `can_release`; name resolved to
  `SharedDmaMapping.can_release()` (1 param/self). Call sites correctly pass
  `(allocation_id, owner_id)` — the DECLARATION was missing, not the calls.
- Fix: added read-only `can_release(allocation_id, owner_id) ->
  MemoryLevelingOperation` mirroring `release`'s pre-mutation gating without
  mutating. No behavior change; no call/param deleted.
- Verify: clean rebuild EXIT=0, kernel elf 2703896 bytes. Toggle proof (revert ->
  EXIT=1, restore -> EXIT=0). Gate `PASS — 6 rung(s)` over REAL OVMF pflash ->
  GRUB-EFI multiboot boot (NOT -kernel, NOT isa-debug-exit): L1..L4 + L3.5a/b all
  OK; guest wrote and read back its own FAT32 marker. Transcript
  `build/os/entstore/ent_store_in_guest_ovmf.serial.log`.
- No new blocker — the board-runnable evidence chain is green.
- Files: `src/os/kernel/memory/memory_leveling_manager.spl`;
  evidence `doc/08_tracking/bug/memory_leveling_can_release_abi_mismatch_2026-08-17.md`.
- W17-C TESTING-INFRA CHECK (2026-08-17, lane W17-C). Proved the enterprise
  verification guards actually BITE (a guard that cannot FAIL is theater).
  NEW: scripts/check/check-enterprise-suite-enumeration.shs — enforces the
  canonical enterprise spec count. Canonical set is deterministic: every on-disk
  `.spl` under src/ test/ doc/ whose path contains "enterprise" MINUS the 8
  *_probe_main.spl app drivers = 79-8 = EXPECTED=71 (measured). Added a fatal
  --selftest (exact-count fixture PASSES; over-count and under-count fixtures
  both FAIL; empty tree is ERROR not PASS), mirroring the mature
  check-tree-size-push.shs --selftest pattern. REPRODUCED RED both directions on
  the real tree: dummy spec added -> "FAIL — 72 ... OVER EXPECTED=71 by 1";
  a real spec moved away -> "FAIL — 70 ... UNDER EXPECTED=71 by 1"; restored ->
  "PASS — 71 canonical spec(s) enumerated, matches EXPECTED=71". EXPECTED NOT
  weakened.
  VERIFIED: scripts/check/check-enterprise-cross-os.shs already ships a fatal
  --selftest with discrimination (well-formed both-OS probe PASSES; deliberately
  host-only slice-using probe is REJECTED with "cannot compile to standalone
  SMF") and is structurally non-vacuous (0 probes matched -> ERROR, lines
  130-133). Ran it against the seed bin/simple (size 59536728, Rust bootstrap
  seed): "PASS — 3 selftest fixture(s) checked, gate logic verified".
  NEW META-HARNESS: scripts/check/check-enterprise-guards-bite.shs — a cheap,
  re-runnable proof that BOTH guards still bite (enumeration over+under live
  reproduction on a throwaway mirror + both guards' own --selftest). Result:
  "PASS — 5 guard-bite check(s) verified". Discoverable by the check-*.shs
  naming convention; run `sh scripts/check/check-enterprise-guards-bite.shs`
  (add --fast to skip the cross-os compile selftest when bin/simple is absent).

## W18-A — Inventory / Stock-Control vertical (2026-08-17)
New enterprise vertical raising coverage: src/lib/nogc_sync_mut/enterprise_inventory/
(inventory.spl + __init__.spl). Guarded commands over the durable store follow the
FROZEN sequence (session -> rbac -> validation -> idempotency -> effects-in-one-UoW):
- stock_receive(sku, location, qty): +qty `inv-receive` row.
- stock_reserve(sku, location, qty): +qty `inv-reserve` row; denies
  `insufficient-stock` (closed-set reason) when qty > available.
- stock_adjust(sku, location, delta): admin-only audited signed correction;
  denies `insufficient-stock` if it would drive on-hand below reserved.
- pure reads stock_on_hand / stock_reserved / stock_available.
INVARIANTS: single insert-only `inv_moves` table; on_hand = sum(receive+adjust),
reserved = sum(reserve), available = on_hand - reserved. NO UPDATE (folds over
movements — durable claim rests on the file backend, interpreter sqlite is
non-ACID). Every mutation writes a sha256-chained audit row via
records.audit_append -> enterprise_store.audit_hash.audit_sha256_hex (SMF-safe
facade; std.common.crypto.sha256 NOT imported).

RBAC — NO edit to frozen foundation.role_allows and NO new rbac_registry: reuse
existing grants. receive/reserve guard on `proc.po.receive` (procurement + admin);
adjust guards on admin-only `inventory.stock.adjust` (blanket admin rule only).
Fine-grained inventory-operator grants (dedicated role or data-driven
enterprise_sale/rbac_registry.spl) are the ADR-gated migration, deferred.

SPEC: test/01_unit/lib/nogc_sync_mut/enterprise_inventory_spec.spl — interpreter
mode, SIMPLE_TIMEOUT_SECONDS=900, seed = Rust bootstrap seed
(release/x86_64-unknown-linux-gnu/simple, 59536728 bytes, built 2026-08-16).
VERDICT: declared>=5 executed=5 passed=5 failed=0. Scenarios: happy-path
receive->reserve->adjust on-hand/available math + cross-location independence +
audit chain; rbac denial (sales actor -> forbidden; keeper on adjust -> forbidden;
inactive session -> invalid-session); over-reservation denial
(reason=insufficient-stock, detail=over-reserve:SKU-1@L1:available=3:got=5, no
effect); idempotent replay (same key -> duplicate-key, on-hand+outbox unchanged);
tenant isolation (tenant-B sees 0, cross-tenant session -> invalid-session,
tenant-A ledger untouched).

CROSS-OS: src/app/enterprise/inventory_probe_main.spl added; compiles host +
x86_64-unknown-simpleos, both SMF\0 (534d4600). Full gate:
scripts/check/check-enterprise-cross-os.shs -> PASS 9 probe(s) checked.

## W18-B — Finance reporting: trial balance / general ledger (2026-08-17)
New module `src/lib/nogc_sync_mut/enterprise_finance/reporting.spl`:
session-gated `trial_balance(store, session, tenant, who) -> TrialBalanceReport`
folds every posted journal row for the session's tenant into per-account
debit/credit totals (integer minor units, exact i64 — no floats) and the
invariant TOTAL DEBITS == TOTAL CREDITS (`balanced:bool`). Pure read: no new
guarded write / RBAC action (foundation note: reports need no grant), but it
reads through the session (`session_valid`) and is tenant-isolated (only
`tenant.tenant_id`'s rows). Period-scoped variant `trial_balance_at_close`
returns the period-boundary totals snapshotted by `fin_period_close`.
Limitation: `journal` rows carry no posting date, so the period variant reads
the close snapshot (totals only, no per-account lines).

Spec `test/01_unit/lib/nogc_sync_mut/enterprise_finance_reporting_spec.spl`:
posts via the REAL guarded path `fin_post_journal_guarded` (no rows fabricated
behind the guard). VERDICT declared>=5 executed=5 passed=5 failed=0.
Proves: per-account math + debits==credits; tenant isolation (reproduce-first:
raw journal holds tenant B's rows, report excludes them); imbalance REPORTED as
balanced=false via a simulated single-sided corrupt row (reproduce-first);
invalid-session refusal; period-scoped snapshot read + no-close.

Cross-OS: `src/app/enterprise/finance_report_probe_main.spl` compiles to
standalone SMF (magic 534d4600) for BOTH host default and
--target=x86_64-unknown-simpleos (rc=0 both) — auto-covered by
check-enterprise-cross-os.shs.

Runner: Rust seed bin/release/x86_64-unknown-linux-gnu/simple, interpreter mode.

## W17-A — DEEPEN web-UI end-to-end tests: attribute-context XSS through real routes (2026-08-17)

Gap closed: escaping was proven end-to-end only for ELEMENT context
(enterprise_security_audit_spec drives `<script>` through store_app_handle and
asserts `&lt;script&gt;`). ATTRIBUTE-context breakout (`x" onmouseover=…`,
`x"><img…`) was proven ONLY at the esc() PRIMITIVE level
(enterprise_output_escaping_audit_spec, 0 dispatches). `<script>` has no `"`,
so it can never exercise the double-quoted-attribute breakout axis.

New spec: test/03_system/app/enterprise/enterprise_attribute_escaping_e2e_spec.spl
(4 examples). For each `<... data-*="…">` site rendering attacker-influenceable
data — /store/catalog (data-sku), /hcm/employees (data-employee), /proc/pos
(data-po), /booking/resources (data-resource) — it SEEDS a record whose id
carries `a" onmouseover="alert(1)` and `b"><img src=y onerror=z>`, drives the
REAL store_app_handle, and asserts the rendered body: (a) does NOT contain the
live breakout `" onmouseover=` or `<img`; (b) DOES contain only the escaped form
`&quot; onmouseover=` / `&lt;img`; (c) still renders the data-* attribute.

Runner: bin/simple = Rust seed (symlinked from simple-main build,
59536728 bytes, mtime 2026-08-16 22:59:37). Interpreter-mode, one spec at a
time, SIMPLE_TIMEOUT_SECONDS=900, verdict read from "SPEC FILE VERDICT:" line.

Verdicts:
- GREEN baseline: declared>=4 executed=4 passed=4 failed=0.
- BITE (neuter esc(): drop `out.replace("\"","&quot;")` in web_common.esc):
  passed=0 failed=4 — the attribute-context quote assertions fire on every
  route (`*-live-quote` raw breakout now present, `*-esc-quote` escaped form now
  absent). The element-context `<img`/`&lt;img` assertions still pass because
  `<`-escaping is untouched — proving the bite is on the ATTRIBUTE (quote) axis
  specifically, exactly what the `<script>` audit cannot cover.
- RESTORE esc(): back to passed=4 failed=0. esc() diff clean.

Real defect found: NONE. All four attribute sites already route their id/field
through esc() before interpolation into the double-quoted attribute; the
end-to-end spec confirms the primitive-level guarantee holds through the real
routes. No app code changed.

## W19-B — Logistics / Fulfillment (shipments) vertical — 2026-08-17
- New vertical `src/lib/nogc_sync_mut/enterprise_fulfillment/` (shipments.spl +
  __init__.spl) over the durable enterprise_store: insert-only rows, status as a
  pure fold over an append-only event stream (NOT UPDATE), sha256 audit only via
  `enterprise_store.audit_hash.audit_sha256_hex` (facade, cross-OS rule), tenant
  isolation, idempotency replay.
- Guarded commands run the FROZEN sequence (session -> rbac -> validation ->
  idempotency -> effects in one UoW): shipment_create (pending),
  shipment_dispatch (pending->dispatched), shipment_deliver (dispatched->
  delivered, with over-shipment denial `insufficient-stock`), shipment_cancel
  (pending->cancelled only; deny `invalid-transition` after dispatch).
- RBAC REUSES the frozen `procurement` role against existing `proc.*` action
  strings — foundation.spl NOT edited, rbac_registry.spl NOT touched.
- Spec `test/01_unit/lib/nogc_sync_mut/enterprise_fulfillment_spec.spl`:
  SPEC FILE VERDICT declared>=9 executed=9 passed=9 failed=0. Covers happy path
  create->dispatch->deliver; rbac denial (sales role -> forbidden); illegal
  transition (cancel after dispatch -> invalid-transition; deliver before
  dispatch -> invalid-transition); over-ship -> insufficient-stock; idempotent
  replay -> duplicate-key, one effect; tenant isolation.
- Cross-OS probe `src/app/enterprise/fulfillment_probe_main.spl`: host compile
  and `--target x86_64-unknown-simpleos` compile both OK, both emit SMF magic
  (53 4d 46 00).
- Runner: Rust seed bootstrap binary (bin/release/x86_64-unknown-linux-gnu/simple,
  59536728 bytes), interpreter-mode, SIMPLE_TIMEOUT_SECONDS=900, one spec at a time.

## W19-A — CRM / Sales-Pipeline vertical (2026-08-17)

New enterprise vertical `src/lib/nogc_sync_mut/enterprise_crm/` over the durable
enterprise store, following the frozen foundation contracts (no edit to
`enterprise_sale/foundation.spl`).

- Commands (frozen sequence session -> rbac -> validation -> idempotency ->
  effects-in-one-UoW): `crm_lead_create`, `crm_lead_qualify` (stage transition
  via validated `lead_transition_allowed` set; illegal move denied
  `invalid-transition`), `crm_opportunity_open` (only from a qualified lead,
  with a `Money` amount), `crm_opportunity_advance` (open -> won|lost, audited).
  Pure read `crm_pipeline_summary` (counts + total Money per stage).
- Insert-only stage rows (`crm_lead_stages`, `crm_opp_stages`); current stage is
  a pure fold (`crm_lead_stage`/`crm_opp_stage`), never UPDATE. Every mutation
  writes a sha256-chained audit row via `records.audit_append` (facade only —
  never imports `std.common.crypto.sha256`), an outbox event, and an idempotency
  key, all inside one unit of work. Tenant isolation enforced by the session
  tenant as authority.
- RBAC: REUSES the existing `sales` role via a LOCAL `crm_role_allows` gate for
  the four `crm.*` actions. Did NOT touch the frozen `role_allows` table, and
  deliberately did NOT add rows to `enterprise_sale/rbac_registry.spl` (its
  equivalence spec is byte-fenced to the frozen table; adding CRM grants would
  turn it red). The frozen session->rbac->validation->idempotency SEQUENCE is
  preserved exactly.
- Spec `test/01_unit/lib/nogc_sync_mut/enterprise_crm_spec.spl`: SPEC FILE
  VERDICT declared>=7 executed=7 passed=7 failed=0. Reproduce-first denials
  proven: rbac `forbidden` (viewer), `invalid-transition` (re-qualify, open from
  unqualified lead, bogus advance target), idempotent replay `duplicate-key`
  (one effect), tenant-B `invalid-record`/`invalid-session` isolation.
- Cross-OS probe `src/app/enterprise/crm_probe_main.spl`:
  `check-enterprise-cross-os.shs` -> `PASS — 10 probe(s) checked, each compiles
  host + x86_64-unknown-simpleos with SMF magic`.
- Runner: Rust seed `bin/simple` (bootstrap seed) x86_64-unknown-linux-gnu,
  59536728 bytes, sha256 40d348ef073bdbb0, interpreter mode, one spec at a time.

## W19-C — interface coherence audit + doc-freshness refresh (2026-08-17)

Lane W19-C: harden the FROZEN contract layer
(`enterprise_sale/foundation.spl` + `enterprise_store/`) by coherence-auditing
every vertical and refreshing the guide. No vertical `.spl` logic changed;
foundation stays frozen.

- Coherence matrix (vertical × invariant), audited by reading code at this HEAD:
  invariant A (denies only with `reason_set()` members) and invariant B
  (session→rbac→validation→idempotency→effects) are **PASS for all 10 verticals**
  (goods_sale, booking, restaurant, payment, hcm, procurement, finance, outbox,
  channel, session). Invariant C (route hashing through `audit_sha256_hex`, not
  `std.common.crypto.sha256` directly) is PASS for store + session.
- **Finding W19-C-1 (invariant C VIOLATION, owner: payment lane — NOT fixed
  here):** `enterprise_payment/payment.spl:59` imports
  `std.common.crypto.sha256.{sha256_text}` and `:78` calls it in `provider_sign`
  (used by `provider_verify`/`provider_verify_webhook`), bypassing the SMF
  facade. Consequence: payment vertical will not build standalone-SMF
  (SimpleOS). Audit chain unaffected (payment audit rows still route via the
  store facade). Fix: swap import to
  `enterprise_store.audit_hash.{audit_sha256_hex}` — proven digest-preserving by
  the new spec, so webhook signatures stay valid.
- Added executable interface-conformance spec:
  `test/01_unit/lib/nogc_sync_mut/enterprise_interface_conformance_spec.spl`.
  Mechanically asserts (1) closed reason set — 16 members, live denial swept out
  of every vertical, plus a **reproduce-first bite**: `reason_allowed` must
  reject `__spec_bogus_reason__`/`""`/`Accepted`/`duplicate_key`/`ok`, so
  widening the set goes red; (2) cross-OS facade — `audit_sha256_hex` pinned to
  the canonical SHA-256 vectors (`""`, `"abc"`) AND digest-equal to
  `sha256_text`, then the store's facade-hashed audit chain verified. Not
  executed in this worktree (no `bin/simple` here; running is optional per lane
  brief) — bite is structural in the negative assertions.
- Guide refreshed: NEW suite index `doc/07_guide/app/enterprise/README.md`
  (contract surface, five-rung sequence, vertical catalog incl. the "landing in
  parallel: CRM, fulfillment — absent at this HEAD" note, durable-store model,
  cross-OS facade rule, full coherence matrix + finding, verification guards).
  Feature-expert wiki `doc/00_llm_process/feature_expert/enterprise_suite/skill.md`
  updated to match (matrix summary, finding, new spec in the run list).
- Enumeration guard `scripts/check/check-enterprise-suite-enumeration.shs`:
  bumped `EXPECTED 71 -> 78`. It was ALREADY RED at 77 (pre-existing +6 on-disk
  drift from parallel lanes that never updated it); +1 is this lane's new spec.
  Guard now `PASS — 78`. Owning lanes must still add their 6 specs to the wiki
  list.

- SEED REDEPLOY (2026-08-17, BLOCKED, recorded): the run-alone seed rebuild+redeploy
  cannot proceed on any session — scripts/check/lib/bootstrap-planner-admission-bound.shs:99-107
  unconditionally returns "planner-admission-v2-producer-unavailable" (by design: no canonical
  producer executes an admitted planner yet), so bootstrap-from-scratch.sh:313 exits 64
  (reason-receipt-required) before Stage 1. Not a compiler bug (Stage-3 ByteOrder defect is fixed
  in this tree). No bypass; hand-rolling a cargo seed is forbidden (.claude/rules/bootstrap.md:38-50).
  Consequence: W13-A's native sqlite / rt_file_atomic_write cannot reach the DEPLOYED binary until
  the planner-admission producer is implemented or the gate opened (needs repo-owner action).
  Interpreter-mode certification is unaffected. Report: doc/09_report/ent_redeploy_seed_rebuild_20260817.md


- W20-A MANUFACTURING / BILL-OF-MATERIALS (2026-08-17): new vertical
  `src/lib/nogc_sync_mut/enterprise_manufacturing/` (manufacturing.spl + __init__.spl)
  on the durable store. Guarded commands, frozen sequence
  (session→rbac→validation→idempotency→effects-in-one-UoW):
  * bom_define(finished_sku, [(component_sku, qty)]) — insert-only BOM lines.
  * work_order_release(wo_id, finished_sku, qty) — denies invalid-record ("no-bom:<sku>")
    when no BOM exists; invalid-transition when wo_id already open.
  * work_order_complete(wo_id) — denies invalid-transition when WO not in `released`
    state (unknown or already completed); records finished-goods production.
  Two insert-only tables (mfg_bom_lines, mfg_wo_events); wo_status/bom_exists/
  produced_qty are pure-Simple folds (NO UPDATE). RBAC REUSES the frozen
  `procurement` role — bom_define/work_order_release on `proc.po.create`,
  work_order_complete on `proc.po.receive` — no new grant, foundation.spl untouched.
  sha256 audit ONLY via enterprise_store.audit_hash facade (cross-OS rule honored).
  Spec test/01_unit/lib/nogc_sync_mut/enterprise_manufacturing_spec.spl:
  SPEC FILE VERDICT declared>=6 executed=6 passed=6 failed=0 (interpreter, seed
  59536728 2026-08-16). Denials proven: forbidden (sales role), invalid-session
  (inactive + cross-tenant), invalid-record (no-bom), invalid-transition (double-
  complete + never-released), duplicate-key (idempotent replay, one effect),
  tenant isolation. Probe src/app/enterprise/manufacturing_probe_main.spl
  cross-compiles host (x86_64-unknown-linux-gnu) + x86_64-unknown-simpleos, both
  valid SMF magic 534d4600. In-guest run blocked on the same seed-redeploy gate above.

## W20-B — Tax engine vertical (2026-08-17)

New enterprise vertical `src/lib/nogc_sync_mut/enterprise_tax/` over the durable
enterprise store, following the FROZEN foundation contracts (no edit to
`enterprise_sale/foundation.spl`, no edit to `rbac_registry.spl`).

- Guarded command (frozen sequence session -> rbac -> validation -> idempotency
  -> effects-in-one-UoW): `tax_rate_set` — a finance/admin actor sets a rate in
  integer BASIS POINTS for a (jurisdiction, category). Insert-only `tax_rates`
  rows; the effective rate is the LATEST by insert order via a pure fold
  (`tax_effective_rate_bp`), never UPDATE. Every write chains a sha256 audit row
  (facade `records.audit_append` only — never imports `std.common.crypto.sha256`),
  an outbox event, and an idempotency key inside one unit of work.
- Pure reads (session-gated, tenant-isolated): `tax_for(store, session, tenant,
  who, jurisdiction, category, Money) -> Money tax`; `tax_breakdown(...,
  line_amounts, currency) -> TaxBreakdown` (per-line + total).
- COMPUTE CONTRACT + ROUNDING: `tax = round_half_up(amount_cents*rate_bp/10000)`
  computed on integers as `(amount_cents*rate_bp + 5000)/10000` (5000 = 10000/2).
  Round-HALF-UP on the minor unit; float-free end to end. `tax_breakdown` rounds
  PER LINE then sums the rounded per-line taxes (line-level rounding, the invoice
  convention): [1000,1000]@875bp = 88+88 = 176, NOT aggregate 2000@875 = 175.
  Absent/0% rate => 0 tax.
- RBAC: REUSES the existing `finance` role via a LOCAL `tax_role_allows` gate for
  the `tax.rate.set` action. Did NOT touch the frozen `role_allows`, and
  deliberately did NOT add rows to `rbac_registry.spl` (its equivalence spec is
  byte-fenced to the frozen table; a tax grant there would turn it red).
- Spec `test/01_unit/lib/nogc_sync_mut/enterprise_tax_spec.spl`: SPEC FILE VERDICT
  declared>=7 executed=7 passed=7 failed=0 dropped=0. Exact integer results:
  1000@875bp -> 88 (boundary 87.5 rounds UP), 2000@875bp -> 175 (exact),
  1000@874bp -> 87 (below-half rounds DOWN, reproduce-first), breakdown total 176
  (line-level) vs aggregate 175; rbac denial (sales -> forbidden, no rate
  recorded, reproduce-first); latest-rate-wins (875 then 500 -> compute 50);
  tenant isolation (A=875 -> 88, B=2000 -> 200, each reads only its own rate);
  idempotent replay (same key, different rate -> duplicate-key, rate stays 875,
  outbox unchanged); audit chain recomputes.
- Cross-OS probe `src/app/enterprise/tax_probe_main.spl`:
  `check-enterprise-cross-os.shs` -> `PASS — 14 probe(s) checked, each compiles
  host + x86_64-unknown-simpleos with SMF magic (534d4600)`.
- Runner: Rust seed `bin/simple` (bootstrap seed) x86_64-unknown-linux-gnu,
  59536728 bytes, sha256 40d348ef073bdbb0, interpreter mode,
  SIMPLE_TIMEOUT_SECONDS=900, one spec at a time.

## W20-C — Multi-Currency / FX vertical (2026-08-17)

New enterprise vertical `src/lib/nogc_sync_mut/enterprise_fx/` (fx.spl,
__init__.spl) over the durable enterprise store, following the FROZEN foundation
contracts (no edit to `enterprise_sale/foundation.spl`).

- Guarded command `fx_rate_set` (frozen sequence session -> rbac -> validation ->
  idempotency -> effects-in-one-UoW): records a rate A->B in MICRO-UNITS
  (scale 1e6) into insert-only `fx_rates`; effective rate = latest by pure fold
  `fx_latest_rate_micro`, per tenant, never UPDATE. Each accepted set writes an
  insert-only rate row + outbox event + sha256-chained audit row via
  `records.audit_append` (facade only — NEVER imports `std.common.crypto.sha256`,
  cross-OS rule honoured) + idempotency key, all in one unit of work.
- Pure read `fx_convert(store, session, Money amount, to_currency) -> Money`.
  CONTRACT: identity when amount.currency == to_currency (amount returned
  unchanged, no lookup); else converted_cents =
  round_half_up(amount_cents * rate_micro / 1_000_000) in i64 integer math, no
  floats. ROUNDING: `fx_round_half_up(n,d) = (n + d/2)/d` (integer div) — exact
  round-half-up for non-negative Money minor units (0.5 rounds up). OVERFLOW
  BOUND: only product is amount_cents*rate_micro (i64); exact iff
  amount_cents*rate_micro <= i64::MAX - 500_000 (~9.22e18); callers keep the
  product under that (e.g. amount_cents ~1e12 at rate ~9.2 is safe); i128
  widening is a future extension. Missing rate -> sentinel Money(-1, to_ccy);
  checked wrapper `fx_convert_result` denies closed-set `not-found`.
- RBAC: REUSES the `finance` role. The `fx.rate.set` grant is added ONLY to the
  finance row in `enterprise_sale/rbac_registry.spl` (with a note), and the FX
  module gates via `registry_role_allows` (the data-driven seam). Frozen
  `role_allows` untouched. Grant is intentionally OUTSIDE the equivalence fence's
  `every_action()` list, so `enterprise_rbac_registry_equivalence_spec.spl` stays
  green (re-run: declared>=4 executed=4 passed=4 failed=0). Adding it to
  every_action() would correctly turn the fence red (frozen table lacks it).
- Spec `test/01_unit/lib/nogc_sync_mut/enterprise_fx_spec.spl`: SPEC FILE VERDICT
  declared>=7 executed=7 passed=7 failed=0. EXACT integer proofs (USD->EUR 0.90):
  1000c->900, boundary 1055c=949.5->950 (round-half-up), 1054c=948.6->949; self-
  conversion identity (USD->USD 4242c->4242c, no rate); latest-rate-wins
  (0.90->0.95, 1000c->950); reproduce-first denials rbac `forbidden` (sales
  role), `invalid-session` (inactive/cross-tenant), `invalid-record` (same-ccy /
  zero rate), idempotent replay `duplicate-key` (one rate row, latest still 0.90
  after replay with a different value), missing-rate `not-found`, tenant-B
  isolation.
- Cross-OS probe `src/app/enterprise/fx_probe_main.spl`:
  `scripts/check/check-enterprise-cross-os.shs` -> `PASS — 14 probe(s) checked,
  each compiles host + x86_64-unknown-simpleos with SMF magic (534d4600)`.
  Probe output: open=true setup=true scale=1000000 round_101x1.5=152.
- Runner: Rust seed `bin/simple` (bootstrap seed) x86_64-unknown-linux-gnu,
  59536728 bytes, sha256 40d348ef073bdbb0, interpreter mode
  (SIMPLE_EXECUTION_MODE=interpret), SIMPLE_TIMEOUT_SECONDS=900, one spec at a
  time (SPEC FILE VERDICT read, never tail -1). Note: this worktree's deployed
  pure-Simple binary segfaults on any run/test in-env, so the certified Rust
  seed was used per lane precedent.


## W21-A — NOTIFICATIONS vertical (insert-only durable outbox ledger) — 2026-08-17

New enterprise vertical `enterprise_notifications` over the durable store,
mirroring the CRM/inventory house style exactly.

- Module `src/lib/nogc_sync_mut/enterprise_notifications/notifications.spl`
  + `__init__.spl`. Two guarded commands `notify_enqueue` /
  `notify_mark_sent` and a pure `notify_pending(tenant)` fold. Status is a
  pure fold over the insert-only `notify_status` stream (never UPDATE); only
  legal transition is `pending -> sent`, double-send denied `invalid-transition`.
- Frozen five-rung SEQUENCE preserved (session -> rbac -> validation ->
  idempotency -> effects in one UoW). RBAC reuses the existing `sales` role
  via a LOCAL `notify_role_allows` gate for `notify.enqueue`/`notify.mark_sent`
  — foundation.spl and rbac_registry.spl are byte-identical to base.
- Denials only from the closed 16-reason set (no reason added). Tenant is
  session-authoritative; payload tenant never trusted. NO `crypto.sha256`
  import — hashing only via `records.audit_append` facade (cross-OS safe).
- Probe `src/app/enterprise/notifications_probe_main.spl`: cross-os PASS
  (14 probes, host + x86_64-unknown-simpleos, SMF magic 534d4600).
- Spec `test/01_unit/lib/nogc_sync_mut/enterprise_notifications_spec.spl`:
  RED-FIRST denials (forbidden, invalid-session forged cross-tenant,
  invalid-transition double-send, invalid-record unknown/tenant-isolation,
  duplicate-key idempotent replay) + happy path.
  VERDICT: declared>=6 executed=6 passed=6 failed=0 dropped=0.

- W21-B STOREFRONT CUSTOMER JOURNEY (2026-08-17, GREEN): new customer-facing
  vertical `enterprise_storefront` — browse -> cart -> checkout -> receipt over
  the durable enterprise store. Cart is an insert-only event log (`sf_cart_events`,
  add-line / remove-line) folded to a current cart; checkout emits an immutable
  order + receipt with a folded total (integer minor units); `receipt_of` is a
  pure read. Frozen five-rung sequence (session -> rbac -> validation ->
  idempotency -> effects-in-one-UoW) on every write. RBAC reuses the frozen
  `sales` role via a LOCAL `storefront_role_allows` gate (foundation.spl and
  rbac_registry.spl byte-identical to base). Denials only from the closed
  16-reason set: empty-cart checkout -> `invalid-record`, already-checked-out ->
  `invalid-transition`, replayed key -> `duplicate-key`, non-sales ->
  `forbidden`, inactive/cross-tenant -> `invalid-session`. Audit via the store
  facade only (no `std.common.crypto.sha256` import). Files:
  `src/lib/nogc_sync_mut/enterprise_storefront/{storefront,__init__}.spl`,
  `src/app/enterprise/storefront_probe_main.spl`,
  `test/01_unit/lib/nogc_sync_mut/enterprise_storefront_spec.spl`.
  Spec: `executed=7 passed=7 failed=0 dropped=0`. Cross-os guard: PASS (14 probes).
  Runner: Rust seed v1.0.0-beta interpreter mode (59536728 bytes,
  sha256 40d348ef073bdbb0b6916b1fbe7294c3e3089cc5cabf9f890993ca610b176cc2).

## W21-C — Returns / RMA vertical (2026-08-17)

New enterprise vertical `enterprise_returns` (customer returns + refund
authorization) over the durable enterprise store. RMA is an insert-only event
log state machine: requested -> approved|rejected -> received -> refunded; the
current state is a pure fold over `ret_rma_events` (never UPDATE). Money in
integer minor units; refund = qty * captured unit price, recorded into the
insert-only `ret_refunds` ledger and folded by `rma_status`.

- Commands (frozen sequence session->rbac->validation->idempotency->UoW):
  `rma_request`, `rma_decide(approve)`, `rma_receive`, `rma_refund`, plus pure
  `rma_status`. Denials use ONLY the closed 16-reason set (no new reason):
  `invalid-transition` for illegal moves (decide twice / receive before approve
  / refund before receive), `not-found` for unknown RMA, `invalid-record` for
  malformed request, `forbidden`/`invalid-session`/`duplicate-key` on the
  standard rungs.
- RBAC: local `returns_role_allows` gate REUSING frozen roles — intake/receipt/
  refund map to `sales`, the approval decision is admin-tier (`admin` only).
  foundation.spl and rbac_registry.spl NOT edited (verified byte-identical to
  base bd5e57113cd).
- Cross-OS: audit only via `records.audit_append` facade; never imports
  `std.common.crypto.sha256`.
- Deliverables: `src/lib/nogc_sync_mut/enterprise_returns/{returns,__init__}.spl`,
  `src/app/enterprise/returns_probe_main.spl`,
  `test/01_unit/lib/nogc_sync_mut/enterprise_returns_spec.spl`.
- Verify (seed interpreter, bin/simple v1.0.0-beta, 59536728 bytes, sha256
  40d348ef073bdbb0b6916b1fbe7294c3e3089cc5cabf9f890993ca610b176cc2):
  `SPEC FILE VERDICT: ... executed=10 passed=10 failed=0 dropped=0`;
  `check-enterprise-cross-os.shs` = `PASS — 14 probe(s) checked`.

- W21-D (PRICING / DISCOUNTS): new enterprise vertical — deterministic, float-free
  price-and-discount engine on the durable enterprise store, modeled on enterprise_tax.
  Module `src/lib/nogc_sync_mut/enterprise_pricing/pricing.spl` (+ `__init__.spl`):
  insert-only `pricing_discounts` rows, latest-rule-wins pure fold `discount_effective`;
  kinds percent_bp (basis points) + fixed_cents. Rounding = round-HALF-UP on the minor
  unit `(base*value_bp + 5000)/10000` (documented in docstring, mirrors tax). Floor-at-zero
  clamp: `price_after_discount` never returns negative; unknown code -> base unchanged.
  Guarded write `discount_define` runs the FROZEN sequence session -> rbac -> validation ->
  idempotency -> effects-in-one-UoW; denies only with reason_set() members
  (invalid-session/forbidden/invalid-record/duplicate-key/store-error). RBAC reuses the
  frozen `sales` (+ blanket `admin`) role via LOCAL gate `pricing_role_allows` — foundation.spl
  and rbac_registry.spl untouched (byte-identical to base bd5e57113cd). Audit only via
  records.audit_append facade; never imports std.common.crypto.sha256. Cross-OS probe
  `src/app/enterprise/pricing_probe_main.spl`. Spec
  `test/01_unit/lib/nogc_sync_mut/enterprise_pricing_spec.spl` (RED-FIRST): rounding boundary
  875->88 / 874->87, percent+fixed, floor-at-zero clamp, latest-rule-wins, unknown-code
  passthrough, forbidden + invalid-session + invalid-record + duplicate-key + tenant isolation.
  VERDICT: declared>=11 executed=11 passed=11 failed=0 dropped=0. check-enterprise-cross-os PASS
  (14 probes). Runner = Rust seed bin/simple v1.0.0-beta (interpreter), 59536728 bytes,
  sha256 40d348ef073bdbb0b6916b1fbe7294c3e3089cc5cabf9f890993ca610b176cc2.

- W22-A (GENERAL LEDGER / double-entry accounting): new enterprise vertical — balanced
  journal posting on the durable enterprise store, complementing the enterprise_finance
  trial-balance (owns the WRITE side). Module `src/lib/nogc_sync_mut/enterprise_gl/gl.spl`
  (+ `__init__.spl`): insert-only `gl_lines` rows, pure folds `gl_account_balance` (sum
  debit-credit per account/tenant) and `gl_trial_balance` (total debits/credits per tenant).
  CORE invariant enforced pre-effect by `gl_entry_balanced`: sum(debits)==sum(credits) AND
  each line exactly one nonzero side, non-negative; an UNBALANCED entry -> deny `invalid-record`
  (a frozen reason_set() member — NO new reason added). Money = integer minor units, i64 folds,
  never floats. Guarded write `gl_post(entry_id, [GlLine])` runs the FROZEN sequence
  session -> rbac -> validation -> idempotency -> effects-in-one-UoW (all lines or nothing);
  denies only with reason_set() members (invalid-session/forbidden/invalid-record/duplicate-key/
  store-error). RBAC reuses the frozen `finance` (+ blanket `admin`) role via LOCAL gate
  `gl_role_allows` mapping `gl.journal.post` — foundation.spl and rbac_registry.spl untouched
  (byte-identical to base 622a213a080). Audit only via records.audit_append facade; never
  imports std.common.crypto.sha256. Cross-OS probe `src/app/enterprise/gl_probe_main.spl`.
  Spec `test/01_unit/lib/nogc_sync_mut/enterprise_gl_spec.spl` (RED-FIRST): unbalanced 1000 vs
  900 -> invalid-record, balanced accepted with account-fold numbers (cash +1250 / revenue
  -1250), trial-balance nets to zero (1700==1700), invalid-session + forbidden + duplicate-key +
  tenant isolation. VERDICT: declared>=7 executed=7 passed=7 failed=0 dropped=0.
  check-enterprise-cross-os PASS (21 probes). Runner = Rust seed bin/simple v1.0.0-beta
  (interpreter), 59536728 bytes,
  sha256 40d348ef073bdbb0b6916b1fbe7294c3e3089cc5cabf9f890993ca610b176cc2.

- W22-B (LOYALTY / REWARDS POINTS): new enterprise vertical — an insert-only
  points ledger on the durable enterprise store. Module
  `src/lib/nogc_sync_mut/enterprise_loyalty/loyalty.spl` (+ `__init__.spl`):
  single insert-only table `loyalty_events` (earn stored +, redeem stored −),
  balance = pure fold `loyalty_balance` (no UPDATE). Guarded writes
  `loyalty_earn` / `loyalty_redeem` run the FROZEN sequence session -> rbac ->
  validation -> idempotency -> effects-in-one-UoW. Deny ONLY with reason_set()
  members: over-redeem (points > balance) denies `insufficient-stock` — the
  closest closed-set reason for "not enough of a consumable resource"
  (documented in the module docstring); non-positive points -> invalid-record;
  standard rungs forbidden/invalid-session/duplicate-key. No new reason added.
  RBAC reuses the frozen `sales` (+ blanket `admin`) role via LOCAL gate
  `loyalty_role_allows` for `loyalty.points.earn`/`loyalty.points.redeem` —
  foundation.spl and rbac_registry.spl NOT edited (verified byte-identical to
  base 622a213a080: foundation 6242814ab5d1..., rbac_registry 0411dd39c9...).
  Cross-OS: audit only via `records.audit_append` facade; never imports
  `std.common.crypto.sha256`. Cross-OS probe
  `src/app/enterprise/loyalty_probe_main.spl`. Spec (RED-FIRST)
  `test/01_unit/lib/nogc_sync_mut/enterprise_loyalty_spec.spl`: earn-then-balance
  proven with numbers (0->100->140), redeem within balance (140-90=50), over-redeem
  balance=100 redeem=150 -> deny insufficient-stock, balance still 100; non-positive
  -> invalid-record; forbidden + invalid-session + duplicate-key + tenant isolation.
  VERDICT: declared>=6 executed=6 passed=6 failed=0 dropped=0.
  check-enterprise-cross-os PASS (21 probes). Runner = Rust seed bin/simple
  v1.0.0-beta (interpreter), 59536728 bytes, sha256
  40d348ef073bdbb0b6916b1fbe7294c3e3089cc5cabf9f890993ca610b176cc2.
- W22-C (INTER-LOCATION STOCK TRANSFERS): new enterprise vertical — move inventory
  between warehouses/locations on the durable enterprise store, complementing
  enterprise_inventory. Module `src/lib/nogc_sync_mut/enterprise_transfer/transfer.spl`
  (+ `__init__.spl`): insert-only signed movement ledger `xfer_moves` + immutable
  route rows `xfer_transfers`; per-location on-hand is a pure fold `location_on_hand`
  (no UPDATE). A transfer is a TWO-PHASE event log: `transfer_dispatch` decrements the
  SOURCE (−qty) and marks in-transit, `transfer_receive` increments the DESTINATION
  (+qty) and marks received; `stock_seed` is the guarded initial stock-in. Status folds
  "" -> in-transit -> received. Conservation proven: total on-hand across locations dips
  by the in-transit qty mid-flight and is restored exactly on receive. Denials use only
  reason_set() members: dispatch of more than source on-hand -> insufficient-stock
  (state-dependent, gated on not-replayed, exact numbers in detail); receive not
  in-transit (receive-before-dispatch OR double-receive) -> invalid-transition; plus
  invalid-session/forbidden/invalid-record/duplicate-key. FROZEN sequence session -> rbac
  -> validation -> idempotency -> effects-in-one-UoW preserved. RBAC reuses the frozen
  `procurement` (+ blanket `admin`) role via LOCAL gate `transfer_role_allows` —
  foundation.spl (6242814ab5d) and rbac_registry.spl (0411dd39c91) untouched,
  byte-identical to base 622a213a080. Audit only via records.audit_append facade; never
  imports std.common.crypto.sha256. Cross-OS probe
  `src/app/enterprise/transfer_probe_main.spl`. Spec
  `test/01_unit/lib/nogc_sync_mut/enterprise_transfer_spec.spl` (RED-FIRST): over-transfer
  15>10 -> insufficient-stock (over-transfer:TR-OV:SKU-1@L1:on_hand=10:got=15), dispatch
  decrements source, receive increments destination, receive-before-dispatch +
  double-receive -> invalid-transition, total-conservation proven, forbidden +
  invalid-session + duplicate-key + tenant isolation. VERDICT: declared>=6 executed=6
  passed=6 failed=0 dropped=0. check-enterprise-cross-os PASS (21 probes). Runner = Rust
  seed bin/simple v1.0.0-beta (interpreter), 59536728 bytes, sha256
  40d348ef073bdbb0b6916b1fbe7294c3e3089cc5cabf9f890993ca610b176cc2.

- W22-D (SUBSCRIPTIONS / RECURRING BILLING): new enterprise vertical — subscription
  lifecycle + recurring invoice generation on the durable enterprise store, modeled on
  enterprise_crm / enterprise_fulfillment. Module
  `src/lib/nogc_sync_mut/enterprise_subscription/subscription.spl` (+ `__init__.spl`):
  a subscription is an EVENT LOG — insert-only `sub_subscriptions` (plan, Money amount in
  integer minor units), append-only `sub_events` lifecycle stream folded by pure
  `sub_status` ("" -> active -> cancelled), append-only `sub_invoices` keyed by an explicit
  integer `period` the caller passes (NO wall-clock; deterministic), summed by pure fold
  `sub_invoiced_total`. Commands `sub_create` / `sub_bill(sub_id, period)` / `sub_cancel`
  each run the FROZEN sequence session -> rbac -> validation -> idempotency -> effects-in-one-UoW.
  Denies ONLY with reason_set() members: bill/cancel unknown sub -> not-found; bill/cancel an
  inactive (cancelled) sub -> invalid-transition; re-bill an already-billed period -> duplicate-key;
  invalid-session/forbidden/invalid-record; idempotent replay -> duplicate-key. No new reason.
  RBAC reuses the frozen `sales` (+ blanket `admin`) role via LOCAL gate `sub_role_allows`
  (sub.create/sub.bill/sub.cancel) — foundation.spl and rbac_registry.spl untouched
  (byte-identical to base 622a213a080). Audit only via records.audit_append facade; never imports
  std.common.crypto.sha256. Cross-OS probe `src/app/enterprise/subscription_probe_main.spl`. Spec
  `test/01_unit/lib/nogc_sync_mut/enterprise_subscription_spec.spl` (RED-FIRST): create ->
  bill period 1 -> bill period 2 -> invoiced-total proven with numbers (1500/3000), re-bill same
  period -> duplicate-key, bill-after-cancel -> invalid-transition, cancel-twice -> invalid-transition,
  unknown sub -> not-found, forbidden + invalid-session + idempotent replay + tenant isolation.
  VERDICT: declared>=10 executed=10 passed=10 failed=0 dropped=0. check-enterprise-cross-os PASS
  (21 probes). Runner = Rust seed bin/simple v1.0.0-beta (interpreter), 59536728 bytes,
  sha256 40d348ef073bdbb0b6916b1fbe7294c3e3089cc5cabf9f890993ca610b176cc2.
