...
- resume (2026-06-10): original session crashed (kill(-1) incident) before committing; restarted session re-verified the work in docker isolation (host_io 36, mcp_sdk 40, net_server 7, deps 31, assistant_surface 2 specs — all pass; src/app/mcp check green 28 files) and committed it, together with export-surface fixes required by the redeployed strict Rust seed.
- sync (2026-06-10): state doc refreshed after push. Lane remains verify-done on `main`; current unrelated working-copy edits in `src/compiler/15.blocks/**`, `src/compiler/99.loader/compiler_sffi.spl`, `src/compiler_rust/compiler/src/hir/types/module.rs`, and `src/lib/nogc_async_mut_noalloc/io/__init__.spl` were intentionally excluded from this plan-doc sync commit.
- verify (2026-06-11): re-audited the current shared worktree without absorbing unrelated dirty files. `find doc/06_spec -name '*_spec.spl' | wc -l` -> `0`; `SIMPLE_LIB=src bin/simple check src/app/mcp src/app/simple_lsp_mcp --mode=interpreter` -> all 34 files OK; `test/01_unit/compiler/interpreter/module_loader_lazy_spec.spl`, `test/01_unit/compiler/cache/shb_mtime_spec.spl`, and `test/02_integration/watcher/watcher_shb_integration_spec.spl` pass; `SIMPLE_LIB=src sh scripts/check/check-mcp-native-smoke.shs` passes with `mcp_startup_ms=1366`, `lsp_mcp_startup_ms=50`, `mcp_second_start_ok=true`, `mcp_stale_stamp_reprobe_ok=true`, `rt_forward_cache_valid=true`, and direct-rt guards still green across MCP-related apps/spec.
- docs (2026-06-11): refreshed `doc/07_guide/app/mcp/startup_performance.md` with the current warm-start wrapper/probe-cache behavior and latest local smoke numbers; updated `.codex/skills/sp_dev/SKILL.md` so future MCP/runtime-forwarding lanes refresh the startup guide and lane state together before handoff.
