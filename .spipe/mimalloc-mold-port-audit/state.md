...
|---|---|---|---|
| `test/01_unit/lib/alloc/mimalloc_spec.spl` (472 lines) | 41 | 0 | Size classes, allocator trait, zeroing/count, size-query, aligned validation, sized-free, heap shims, stats/options model, 10k stress loop, mixed-size accounting |
| `test/01_unit/os/kernel/memory/heap_mimalloc_spec.spl` (46 lines) | 7 | 0 | Pre-init state, zero-byte rejection, nil free safety, raw boundary |
| `test/01_unit/os/memory/mimalloc_os_spec.spl` (241 lines) | 22 | 0 | mimalloc_init, mi_malloc/mi_free for sizes 8/64/1024/0, successive-call distinctness, MimallocAllocator init guard, mi_malloc_raw stub mode (returns 0), injected provider tracking, mi_free_raw decrement |
| `test/01_unit/os/memory/mold_linker_spec.spl` (137 lines) | 12 | 0 | install script presence, linker preference chain, lld fallback, SIMPLE_LINKER aliases, bundled path priority, platform binary names |
| `test/01_unit/compiler/mono/mold_pure_spec.spl` (64 lines) | 2 | 1 | **ENTIRELY SKIPPED** — one `it "skipped"` placeholder; no real coverage |
...
- REQ-5 (from AC-3): Fix `mold_pure_spec.spl` — it is a skipped placeholder; either fill with real coverage or remove — area: `test/01_unit/compiler/mono/mold_pure_spec.spl`
- REQ-6 (from AC-3): Add end-to-end linking test (link real objects, verify binary runs) — area: `test/01_unit/compiler/mono/` or integration tests
...
| `mold_pure_spec` | `test/01_unit/compiler/mono/mold_pure_spec.spl` | Remove placeholder; add real ELF inspector tests | Modified |
...
Architecture lists the fix target as `test/01_unit/compiler/mono/mold_pure_spec.spl`, but
`elf_inspect.spl` lives at `src/compiler/70.backend/linker/`. The mirrored path would be
`test/01_unit/compiler/70.backend/linker/elf_inspect_spec.spl`. The implement phase must decide:
...
**Target spec file:** `test/01_unit/lib/alloc/mimalloc_tls_spec.spl`
...
**Target spec file:** `test/01_unit/lib/alloc/mimalloc_page_policy_spec.spl`
...
**Target spec file:** `test/01_unit/lib/alloc/mimalloc_secure_spec.spl`
...
**Target spec file:** `test/01_unit/compiler/70.backend/linker/elf_inspect_spec.spl`
**Path note:** implement phase should delete `test/01_unit/compiler/mono/mold_pure_spec.spl` placeholder
...
**Target spec file:** `test/01_unit/compiler/70.backend/linker/mold_wrapper_spec.spl`
...
|---|---|---|---|
| `test/01_unit/lib/alloc/mimalloc_tls_spec.spl` | 7 | mimalloc M1 | #1, L15 >> fix |
| `test/01_unit/lib/alloc/mimalloc_page_policy_spec.spl` | 8 | mimalloc M2 | #2 |
| `test/01_unit/lib/alloc/mimalloc_secure_spec.spl` | 11 | mimalloc M3 | #3, #4, #7 |
| `test/01_unit/compiler/70.backend/linker/elf_inspect_spec.spl` | 9 | mold A-M1 | #8 |
| `test/01_unit/compiler/70.backend/linker/mold_wrapper_spec.spl` | 7 | mold A-M2 | #5, #6 |
| **Total** | **42** | | **all 8 gaps** |

**Action for implement phase:** delete `test/01_unit/compiler/mono/mold_pure_spec.spl` (placeholder with skipped it-block) and create `elf_inspect_spec.spl` at the mirrored linker path above.
...
- All 36 existing mimalloc tests pass: `bin/simple test test/01_unit/lib/alloc/mimalloc_spec.spl`
  → 36 passed, 0 failed.
...
- Delete placeholder `test/01_unit/compiler/mono/mold_pure_spec.spl`
...
**What was done:**
- Ran `bin/simple test test/01_unit/lib/alloc/mimalloc_spec.spl` → 36 passed, 0 failed (5.8s).
