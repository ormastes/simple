...
- revive-web (2026-07-11): `qemu_rv64_http_test.shs --with-db --expect-http-only` — HTTP 4/4 PASS live in RV64 QEMU (AC-1 evidence current).
- revive-db (2026-07-11): DB gate now **5/5 PASS with `codex-41`** (AC-2 evidence current). Chain: stripped hoisted-locals restored → seed llvmlib fn-method arity bug worked around via `me`-method conversion (clean-cache rebuild 51/0/0 proved no stale link) → terminal blocker root-caused and FIXED in seed `compile_inline_len` (only recognized hosted STR1 magic; freestanding byte-tag 0x01 strings hit the -1 phi arm; fix ORs in byte-tag check, proven by disassembly at 61 sites). Bugs: `rv64_llvm_nested_len_arg_miscompile_2026-07-11.md`, `native_build_noncritical_skip_stale_cache_masking_2026-07-11.md`. Side fix: riscv64 `rt_invlpg` (sfence.vma).
- revive-clang (2026-07-11): `ssh_clang_hello_ring3.shs` REVIVED — exit 0, `hello from clang on simpleos`, `[user] exit rc=42` (AC-4 evidence current). Root causes: streaming rewrite dropped the boot-preload short-circuit (exec-time FAT reads return 0 — `x64_ssh_kernel_fat32_stream_open_zero`), reinstated at `x86_64_fs_exec_spawn.spl:78` with streaming kept as fallback; missing `mmio_disable_test_mode` defined at `mmio.spl:118` (weak-stub no-op left garbage .bss test-mode → all mmio reads nil).
- redeploy-diagnosis (2026-07-11): three distinct failures fully separated in `self_hosted_simpleos_target_native_build_crash_2026-07-11.md` — stale 2-arg `rt_env_set` in Jul-7 release binary; deterministic llvm-lib `DataLayout::setAlignment` SIGSEGV (`backend_core.rs:846`, even hello-world); bootstrap pipeline exit-0 masking. Viable paths: stage3 cranelift (verify not stub-collapsed) or seed `SIMPLE_BOOTSTRAP=1` external-llc. AC-5/AC-6 remain blocked on redeploy.
