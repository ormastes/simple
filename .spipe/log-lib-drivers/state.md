...
- [~] AC-3: Unified facade with runtime per-subsys level filter verified by executable proof (`test/02_integration/log_facade_runtime_check.spl`, 14/14 PASS); compile-time `STATIC_MAX_LEVEL` deferred to bug **B-LOG-CTL**.
...
**Unit specs (one per row; `.spl` under `test/01_unit/lib/log/`):**
...
**Integration specs (`test/02_integration/log/`):**
...
- `test/01_unit/lib/log_facade_level_filter_spec.spl` — 9 it blocks, covers AC-3, AC-6a (level enum sanity, global filter, per-subsys filter, EnvFilter parser)
- `test/01_unit/lib/log_facade_backend_swap_spec.spl` — 7 it blocks, covers AC-3, AC-6b (register, swap, MAX_BACKENDS=4, removal, slot reuse)
- `test/01_unit/lib/log_lite_ring_spec.spl` — 9 it blocks, covers AC-5, AC-6c (40-byte LogRecord layout, capacity power-of-two, producer/drainer order, head wrap, overflow drop, panic_flush, no-alloc placeholder)
- `test/01_unit/lib/audit_log_hash_chain_spec.spl` — 7 it blocks, covers AC-5, AC-6e (genesis, append, recompute, tamper detect, recovery without full replay, severity back-compat)
- `test/02_integration/log_facade_back_compat_spec.spl` — 7 it blocks, covers AC-4 back-compat (top-level imports resolve, legacy `info(scope, msg)` round-trips, scope→subsys mapping)
- `test/02_integration/simpleos_driver_log_smoke_spec.spl` — 2 it blocks + manual QEMU command, covers AC-4, AC-6d (null_block driver registration emits SUBSYS_DRIVER_BLOCK INFO record via facade)
...
`bin/simple test test/01_unit/lib/log_lite_ring_spec.spl` — runner reports PASSED in 0ms (interpreter no-ops because the imports `std.log.ring.{...}` and `std.log.panic.{...}` don't exist yet — runner treats as zero-test file). `bin/simple check test/01_unit/lib/log_lite_ring_spec.spl` confirms red phase:
...
- **Test run (NOT GREEN — same red-phase pattern as Phase 4 §C):**
  `bin/simple test test/01_unit/lib/audit_log_hash_chain_spec.spl`
...
- `test/01_unit/lib/log_facade_level_filter_spec.spl` (9 it-blocks) — copied from parent (was untracked).
- `test/01_unit/lib/log_facade_backend_swap_spec.spl` (6 it-blocks) — copied from parent.
- `test/01_unit/lib/log_lite_ring_spec.spl` (9 it-blocks) — copied from parent.
- `test/01_unit/lib/audit_log_hash_chain_spec.spl` (7 it-blocks) — copied from parent.
- `test/02_integration/log_facade_back_compat_spec.spl` (6 it-blocks) — copied from parent.
- `test/02_integration/simpleos_driver_log_smoke_spec.spl` (2 it-blocks) — copied from parent.
...
| `.spipe/log-lib-drivers/state.md` | this checkpoint |
...
2. **Phase 4 spec import paths `std.log.ring.{...}` and `std.log.panic.{...}` will not resolve.**
   `test/01_unit/lib/log_lite_ring_spec.spl` imports from `std.log.ring`
...
- `bin/simple test test/02_integration/log_facade_back_compat_spec.spl`:
...
#### Executable proof
- File: `test/02_integration/log_facade_runtime_check.spl`.
- Run command: `bin/simple test/02_integration/log_facade_runtime_check.spl`.
...
`bin/simple test/02_integration/log_facade_runtime_check.spl` and confirm
...
- **Verification (Phase 7)** — `test/02_integration/log_facade_runtime_check.spl`
...
- **B-LOG-BACKEND-INTERP** — backend swap spec
  (`test/01_unit/lib/log_facade_backend_swap_spec.spl`) is paper-only;
...
```bash
bin/simple test/02_integration/log_facade_runtime_check.spl
