...
- verify: `bin/release/x86_64-unknown-linux-gnu/simple test test/01_unit/compiler/frontend/aop_log_policy_spec.spl --mode=interpreter --no-cover-check` passed.
- verify: `bin/release/x86_64-unknown-linux-gnu/simple test test/03_system/os/baremetal/feature/log_policy_spec.spl --mode=interpreter --no-cover-check` passed.
...
- verify: `bin/release/x86_64-unknown-linux-gnu/simple test test/01_unit/compiler/frontend/aop_weaving_config_spec.spl --mode=interpreter --no-cover-check` passed after fixing `core/aop.spl` parser-incompatible `context:` named constructor arguments.
- verify: `bin/release/x86_64-unknown-linux-gnu/simple test test/01_unit/compiler/frontend/aop_log_policy_spec.spl --mode=interpreter --no-cover-check` passed with 10 examples, including call-only and assignment-only MIR join-point matches.
- verify: `test/01_unit/compiler/mir/aop_injection_spec.spl` remains blocked in this runner by an existing `MirTerminator.Return` enum parser incompatibility in `src/compiler/50.mir/mir_instructions.spl`; call/assignment AOP behavior is covered through the frontend AOP policy spec instead.
...
- verify: `bin/release/x86_64-unknown-linux-gnu/simple test test/01_unit/compiler/frontend/aop_log_policy_spec.spl --mode=interpreter --no-cover-check` passed again with 10 examples after the MIR parser fixes.
- verify: `bin/release/x86_64-unknown-linux-gnu/simple test test/01_unit/compiler/frontend/aop_weaving_config_spec.spl --mode=interpreter --no-cover-check` passed again with 16 examples.
- verify: `bin/release/x86_64-unknown-linux-gnu/simple test test/03_system/os/baremetal/feature/log_policy_spec.spl --mode=interpreter --no-cover-check` passed again with 3 examples.
- verify: `find doc/06_spec -name '*_spec.spl' | wc -l` returned `0`.
- verify: `test/01_unit/compiler/mir/aop_injection_spec.spl` now gets past the MIR terminator keyword blocker but remains blocked by an existing parse issue in `src/compiler/30.types/type_layout_part2.spl` (`Unexpected token: expected expression, found Newline`).
...
- verify: `bin/release/x86_64-unknown-linux-gnu/simple test test/01_unit/compiler/mir/aop_injection_spec.spl --mode=interpreter --no-cover-check` passed with 6 examples.
...
- verify: `SIMPLE_LIB=src bin/release/x86_64-unknown-linux-gnu/simple test test/02_integration/app/mcp_stdio_integration_spec.spl --mode=interpreter` still fails independently with 5 examples failing on missing `proc_slot_acquire`.
- verify: `find doc/06_spec -name '*_spec.spl' | wc -l` returned `0`; test-runner-created `doc/test/test_db.sdn.backup` was removed.
...
- verify: `SIMPLE_LIB=src bin/release/x86_64-unknown-linux-gnu/simple test test/02_integration/app/mcp_stdio_integration_spec.spl --mode=interpreter --no-cover-check` passed with 5 examples.
- verify: `find doc/06_spec -name '*_spec.spl' | wc -l` returned `0`; test-runner-created `doc/test/test_db.sdn.backup` was removed.
