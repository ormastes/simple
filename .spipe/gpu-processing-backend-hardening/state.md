...
- optimize: Default host-daemon ProcessingIR requests no longer allocate and execute a full CPU mirror before GPU dispatch. FillU32 output is validated directly, CPU fallback computes its oracle only when selected, and the QEMU evidence wrapper opts into the independent CPU oracle with `--processing-verify-cpu`. Unit 3/3, fallback contract 9/9 before the strengthened duplicate-call assertion, reason mapping 6/6, daemon check, shell syntax, and env guards pass. The corrected final assertion was not rerun after the third-cycle cap; fresh source-matched latency evidence remains open.
