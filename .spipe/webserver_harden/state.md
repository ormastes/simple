...
- W17-B (adversarial edge hardening, 2026-08-17): Added test/01_unit/lib/common/net/http_core_adversarial_spec.spl — 32 assertions, all GREEN first-try (interpreter mode, Rust seed runner). All are REGRESSION FENCES on already-correct behaviour; no real defect found (the unified http_core is genuinely hardened). Gaps closed (previously untested entry points):
  - hex_chunk_size: valid lower/upper hex, empty->-1, non-hex->-1, >2^27->-1.
  - decode_chunked_bounded: well-formed decode, bad size->400, truncated chunk->400, missing CRLF terminator->400, decoded-size>max_body->413.
  - chunked_body_end_scan: complete->offset(15), bad hex->-2, truncated->-1, embedded "0\r\n\r\n" in DATA not mistaken for terminal chunk.
  - Duplicate/conflicting singleton security headers (Host, X-Forwarded-Host, Authorization) via body_decision -> 400 smuggling reject; single Host accepted.
  - HttpParseError limit checks: over-long request line->414, malformed line->400, too-many-headers->400, over-long header line->400, over-sized body->413, in-limit body/line accepted.
  - path_is_safe / is_safe_static_path: backslash + encoded-backslash (%5c) traversal, raw NUL, relative (no leading slash) all rejected; normal absolute accepted.
  VERDICT: declared>=32 executed=32 passed=32 failed=0 dropped=0. No http_core.spl change required.
