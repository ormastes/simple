...
```
File: `test/02_integration/app/scv_wasm_executor_spec.spl`
- 6 `it` blocks × ≥1 child each = 6+ spawns of the full interpreter
- `SIMPLE_LIB="$REPO/src"` covers all of `src/` — 600+ .spl files

The same pattern is used in `test/02_integration/app/` other SCV specs (merge, diff, etc.):
...
- `test/02_integration/app/scv_wasm_executor_spec.spl`
...
| `test/02_integration/app/scv_wasm_executor_spec.spl` | PROD-001 spec — sets SIMPLE_LIB="$REPO/src" |
