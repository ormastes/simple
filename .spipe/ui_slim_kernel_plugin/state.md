...
- 2026-09-06 landing: PR #382 branch `ui-slim-wave1` had origin/main merged in by a peer (`496e7b8d`); Wave 2 (`95b2774b`), deps dotted-dir fix (`1482c862`), and the advisory entry-gate rows + plan entry map (`80461303`) were re-applied on top; range guards + guard-wiring PASS; push in progress. Product entry map: `cli_entry.spl` 118 files / `backend_entry_tui.spl` 114 files, both 0 forbidden; `main.spl` 875 files off-path.
- 2026-09-06: pushed `ui-slim-wave1` tip `64f7b1d3` (7aaf09b8 / cbaa2239 / 64f7b1d3 on merged base 8a177348); PR #382 comments updated. Wave 3 BLOCKED: needs deployed pure-Simple ui (bootstrap forbidden).
- 2026-09-06 (while PR #382 pending): A10 pack inventory `6ca7b6c1` (6 entries, 0 violations; shared-WM/web classified ui-full), A08 termbox2/ncursesw T1 fixtures `3ece3325` (PTY-driven, 26.9/27.1 ms spawn->greeting diagnostic), A09 microui/Nuklear widget-core fixtures `445edb17` (FLTK unsupported: not installed), A07b G1 real window `cf3a2fdd` (2026-07-25 stage4 binary + prebuilt libspl_winit; window_created 5.7 s interpreted; PPM non-blank). A01 startup harness in flight.
- 2026-09-06: backend_loader `_simple_binary()` defined `348dd773`; tui backend now runs IN-PROCESS from cli_entry `7b732fe0` (no tty-inheriting spawn facade exists: process_run pipes, rt_process_spawn_async nulls stdin); **first end-to-end T1 PASS** on the seed: median 6801 ms, diagnostic. cli_entry closure 149 files, 0 forbidden. Found `_host_backend_selector` undefined in host_compositor_entry (bug filed, out of lane scope). PR #382 tip after these: see landing agent report.
- 2026-09-06 (close): A11 independent review (opus, read-only) on `efcb78ed` → PARTIAL_WITH_BLOCKERS; all findings fixed and folded into the squashed tip `68c816d9` (absorbed peer `draw_ir` generation edit removed from `session.spl`; no python in the GUI harness; event-wait + backend-loader specs self-contained; pack-inventory wired as advisory push row, startup/gui-present harnesses opted out with reasons; manifest `ci_job, inputs` columns; todo_db rows 271-280). Range guards PASS. Wave 3 certification still BLOCKED (no deployed pure-Simple `ui`; bootstrap forbidden).
