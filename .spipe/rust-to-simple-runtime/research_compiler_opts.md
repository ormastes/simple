...
### Port algorithms harness (`test/05_perf/port_algorithms/`)
...
current microbenchmark for these operations** in `test/05_perf/`. The collections harness and
...
- **Benchmark command:** `SIMPLE_NATIVE_CPU=native SIMPLE_BIN=... test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`
  and `test/05_perf/collections/` (3–5 samples for reliable medians).
