...
- C.2 Cranelift >>: DONE — verified with `bin/simple test test/01_unit/compiler/u64_shift_param_spec.spl` (8/8 pass)
...
**Existing tests:**
- `test/01_unit/compiler/packed_struct_bitfield_spec.spl` — grep-based spec checking source code contains expected symbols (NOT runtime behavior)
- `test/03_system/feature/usage/packed_struct_bitfield_syntax_spec.spl` — documents syntax intent, status "parser diagnostic only"
- `test/03_system/feature/usage/bitfield_spec.spl` — standalone `bitfield` keyword tests
- `test/01_unit/compiler/bitfield_sugar_spec.spl` — compiler-level bitfield tests
- `test/03_system/bitfield_reorder_spec.spl` — bitfield reordering tests
- `test/01_unit/compiler/mir/bitfield_mir_spec.spl` — MIR-level bitfield tests
- `test/01_unit/compiler/native/bitfield_codegen_spec.spl` — native codegen bitfield tests
...
**Existing tests:**
- `test/01_unit/compiler/mir/synthetic_driver_registration_spec.spl` — 5 `it` blocks covering all planner statuses (AlreadyHandwritten, BlockedMissingDriverOps, ReadyToSynthesize, BlockedNativeLibOps, NoManifest)
- `test/03_system/feature/compiler/driver_native_spec.spl` — driver native compilation tests
- `test/03_system/feature/usage/serial_driver_spec.spl` — serial driver usage tests
- `test/01_unit/lib/driver/` — driver library-level tests
...
- Q1: Does `@packed struct { f: u16:4; g: u16:12 }` already work end-to-end in the Rust seed? Run `bin/simple test test/01_unit/compiler/packed_struct_bitfield_spec.spl` + write a small runtime test to verify.
...
- REQ-4 (from AC-4): Round-trip test: construct, write, read back — area: `test/03_system/feature/usage/` or `test/01_unit/compiler/` (new runtime test)
