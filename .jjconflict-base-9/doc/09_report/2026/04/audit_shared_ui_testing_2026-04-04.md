...
- `test/03_system/ui/unified_test_spec.spl` -- **Key file**: starts BOTH web and tui_web servers, uses same `UITestClient` against both, compares results (3 tests)
- `test/03_system/ui/web_test_api_spec.spl` -- Web-only coverage (8 tests)
- `test/03_system/ui/tui_web_render_spec.spl` -- TUI web proxy (5 tests)
- `test/03_system/ui/interaction_spec.spl` -- Interaction flows (5 tests)
- `test/03_system/ui/helpers/ui_test_helpers.spl` -- `start_ui_server()`, `stop_ui_server()`, `free_port()`
