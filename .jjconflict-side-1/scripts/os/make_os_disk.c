...
/* FAT32 requires every SUBdirectory to begin with "." (itself) and ".."
 * (its parent, recorded as cluster 0 when the parent is the root directory).
 * These were never emitted, so `fsck.fat` failed the image with
 *   "/EFI/BOOT Expected a valid '.' entry in this slot"
 *   "/EFI      Expected a valid '..' entry in the second slot"
 * and the SimpleOS-WM evidence harness rejected it as invalid-fat32-structure.
 * Call this immediately after a subdirectory's buffer is declared, BEFORE any
 * other entry goes into that buffer — the two dot entries must be first.
 * The root directory itself takes no dot entries.
 * See doc/08_tracking/bug/simpleos_wm_fat32_image_fsinfo_uninitialized_2026-07-25.md */
static void put_dot_entries(unsigned char *entries, int *count, int self_cluster, int parent_cluster)
{
    put_dir_entry(entries, count, ".          ", self_cluster, 0, 0x10);
    put_dir_entry(entries, count, "..         ",
                  parent_cluster == ROOT_CLUSTER ? 0 : parent_cluster, 0, 0x10);
}
...
static void finish_fat32_image(const char *img_path)
{
    ...
    le16(48, 1);
    le16(50, 6);
    /* Extended boot signature + volume label. The label field (offset 71, 11
     * bytes, space-padded) was left as zeros, which fsck rejects with
     * "Label '' stored in boot sector is not valid. Auto-removing label".
     * 0x29 at offset 66 marks vol-id/label/fs-type as present. */
    g_image[66] = 0x29;
    le32(67, 0x53494D50u); /* volume id (arbitrary but stable) */
    memcpy(g_image + 71, "SIMPLEOS   ", 11);
    memcpy(g_image + 82, "FAT32   ", 8);
    g_image[510] = 0x55;
    g_image[511] = 0xaa;

    /* Sector 1: FSInfo. The BPB above DECLARES fsinfo_sector=1, but this
     * sector was previously left as zeros, so `fsck.fat` failed the image with
     * "FSINFO sector has bad magic number(s)" and the SimpleOS-WM evidence
     * harness rejected it as invalid-fat32-structure. Offsets are FAT32-spec
     * fixed: lead 0x41615252 at +0, struct 0x61417272 at +484, trail
     * 0x00 0x00 0x55 0xAA at +508. free_count/next_free are 0xFFFFFFFF =
     * "unknown", which is spec-legal and lets the driver recompute.
     * See doc/08_tracking/bug/simpleos_wm_fat32_image_fsinfo_uninitialized_2026-07-25.md */
    {
        const size_t fsinfo = (size_t)1 * SECTOR_SIZE;
        le32(fsinfo + 0, 0x41615252u);
        le32(fsinfo + 484, 0x61417272u);
        /* Report the REAL free/next-free counts rather than 0xFFFFFFFF
         * ("unknown"). Unknown is spec-legal but fsck flags it as
         * "Free cluster summary uninitialized (should be N)". g_next_cluster is
         * the next cluster alloc_clusters would hand out, and data clusters are
         * numbered from 2, so used = g_next_cluster - 2. */
        {
            uint32_t data_clusters =
                (g_total_sectors - g_data_start_sector) / SECTORS_PER_CLUSTER;
            uint32_t used = (uint32_t)g_next_cluster - 2u;
            uint32_t free_count = data_clusters > used ? data_clusters - used : 0u;
            le32(fsinfo + 488, free_count);
            le32(fsinfo + 492, (uint32_t)g_next_cluster);
        }
        g_image[fsinfo + 508] = 0x00;
        g_image[fsinfo + 509] = 0x00;
        g_image[fsinfo + 510] = 0x55;
        g_image[fsinfo + 511] = 0xaa;
    }

    /* Sector 6: backup boot sector, also declared above (le16(50, 6)) and also
     * previously never written — fsck reported every BPB byte as differing from
     * its all-zero backup. A verbatim copy of sector 0 is what the spec wants. */
    ...
    struct bytes third_party_notices_payload)
{
    ...
    /* Dot entries first, before any content lands in these buffers. */
    put_dot_entries(sys, &sys_n, sys_cluster, ROOT_CLUSTER);
     ...
     * "..") this directory now emits. The guard is kept (it catches a missing
     * or extra font asset); only the expected total moved, by exactly 2. */
    if (fonts_n != 93)
...
int main(int argc, char **argv)
{
    ...
    /* TODO(simpleos-steam-staging): steam_manifest / steam_marker are built
     * above but were never published into any directory — the clusters were
     * allocated in the FAT and orphaned (fsck "Reclaimed unused clusters", and
     * the compiler's own unused-variable warnings pointed straight at it). The
     * manifest names /usr/share/simpleos/games/steam_2048/marker.txt as its
     * proof_marker, so the intended staging path exists but was not wired.
     * Allocation removed so the FAT stays consistent; the staging itself is
     * still owed. Do NOT re-add alloc_clusters here without also adding the
     * put_dir_entry that consumes it. */
    int steam_manifest_cluster = 0;
    int steam_port_cluster = alloc_clusters(steam_port.data, steam_port.len);
    int steam_marker_cluster = 0; /* see TODO(simpleos-steam-staging) above */
     ...
     * chain. Sharing made fsck report
     *   "/HELLO.C and /SYS/CLANGHEL.C share clusters. Truncating second to 0 bytes"
     * and then "cluster chain length is 0 bytes" for the loser — i.e. one of the
     * two paths would read as empty. This payload is deliberately published at
     * two paths, so give the second path its own independent copy.
     * See doc/08_tracking/bug/simpleos_wm_fat32_image_fsinfo_uninitialized_2026-07-25.md */
    int clang_c_cluster_alias = alloc_clusters(clang_c.data, clang_c.len);
    ...
    /* Same no-hard-links rule as clang_c_cluster_alias above: this payload is
     * published at both /SIMPLE.ELF and /BIN/SIMPLE, so the second entry needs
     * its own chain.
     * The guard must match the guard on the CONSUMING entry, which lives inside
     * `if (simple_usr_cluster) { ... }`. Allocating on `simple_payload.len`
     * alone leaked the entire multi-MB payload (~52 clusters) whenever
     * simple_usr_cluster was 0 — fsck: "Reclaimed 54 unused clusters". */
    int simple_bin_cluster_alias = (simple_payload.len && simple_usr_cluster)
        ? alloc_clusters(simple_payload.data, simple_payload.len) : 0;
    ...
    /* `tmp` was the one directory given a cluster and a root entry but NO
     ...
     * Every other directory here is memcpy'd below; TMP was the sole gap. */
    unsigned char root[4096] = {0}, efi[4096] = {0}, boot[4096] = {0}, sys[4096] = {0}, fonts[4096] = {0}, apps[4096] = {0}, perf[4096] = {0}, usr[4096] = {0}, usr_bin[4096] = {0}, usr_lib[4096] = {0}, bin[4096] = {0}, sysrt[4096] = {0}, tmp[4096] = {0};
    int root_n = 0, efi_n = 0, boot_n = 0, sys_n = 0, fonts_n = 0, apps_n = 0, perf_n = 0, usr_n = 0, usr_bin_n = 0, usr_lib_n = 0, bin_n = 0, sysrt_n = 0, tmp_n = 0;
    /* Dot entries first, before any content lands in these buffers.
     * Parents: efi/sys/usr/bin/sysrt <- root; boot <- efi;
     *          apps/perf/fonts <- sys; usr_bin/usr_lib <- usr. */
    put_dot_entries(efi, &efi_n, efi_cluster, ROOT_CLUSTER);
    put_dot_entries(sys, &sys_n, sys_cluster, ROOT_CLUSTER);
    put_dot_entries(usr, &usr_n, usr_cluster, ROOT_CLUSTER);
    put_dot_entries(bin, &bin_n, bin_cluster, ROOT_CLUSTER);
    put_dot_entries(sysrt, &sysrt_n, sysrt_cluster, ROOT_CLUSTER);
    ...
    put_dot_entries(tmp, &tmp_n, tmp_cluster, ROOT_CLUSTER);
    /* Volume label must exist BOTH in the boot sector (offset 71) and as a
     * root-directory entry with ATTR_VOLUME_ID (0x08), cluster 0, size 0.
     * With only the boot-sector half, fsck reports "Label in boot sector is
     * 'SIMPLEOS', but there is no volume label in root directory". The two
     * spellings must match exactly, including space padding. */
    ...
    put_dir_entry(sys, &sys_n, "CLANGHELC  ", clang_c_cluster_alias, clang_c.len, 0x20);
    ...
    if (simple_usr_cluster) {
        ...
        put_dir_entry(bin, &bin_n, "SIMPLE     ", simple_bin_cluster_alias, simple_payload.len, 0x20);
    ...
    memcpy(g_image + cluster_offset(tmp_cluster), tmp, (size_t)tmp_n * 32);
