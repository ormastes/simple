...
impl<T: UserEvent> Wry<T> {
  ...
  fn init(event_loop: EventLoop<Message<T>>) -> Result<Self> {
    ...
    let context = Context {
      ...
      webview_runtime_installed: android_safe_webview_runtime_installed(),
    };
...
#[cfg(target_os = "android")]
fn android_safe_webview_runtime_installed() -> bool {
  match std::panic::catch_unwind(|| wry::webview_version().is_ok()) {
    Ok(installed) => installed,
    Err(_) => {
      log::warn!(
        "Android WebView version probe panicked; continuing so WebView creation can provide the real runtime proof."
      );
      true
    }
  }
