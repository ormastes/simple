...
- REQ-UAP-013: The MCP OS tool surface shall register `ui_access_snapshot`,
  ...
  `ui_access_value`, `ui_access_query`, `ui_access_adapter_snapshot`, and
...
- AC-UAP-03: `/api/test/ui/snapshot`, `/api/test/ui/surface`,
  ...
  `/api/test/ui/state`, `/api/test/ui/adapter_snapshot`, and
  `/api/test/ui/visual_probe` respond with stable additive behavior.
- AC-UAP-04: The MCP tool registry lists the eleven UI access tools with
  matching schemas, and the declarative observe/state/value tools preserve
  canonical node IDs while routing through the shared protocol.
