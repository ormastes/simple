...
- forced Vulkan, CUDA, OpenCL, Metal, WebGPU, and CPU-only backend proof.
...
Simple GUI / web / direct 2D
  ...
  -> cpu-scalar | cpu-simd | cuda | opencl | vulkan | metal | webgpu
...
- OpenCL: ICD/runtime path with real platform, context, queue, program build,
  kernel creation, enqueue, finish, and readback evidence. It must not be
  reported as CUDA-compatible until the OpenCL runtime bridge is complete and
  generated-kernel artifacts load through `opencl_session.spl`.
...
## OpenCL Like CUDA Rule

CUDA currently has the stronger runtime precedent: driver API loading, PTX
module loading, kernel lookup/launch, memory operations, and unavailable stubs.
OpenCL must reach the same evidence standard before it can be selected by
strict Engine2D acceleration:

1. `src/runtime/runtime_simd_dispatch.c` or an equivalent Rust runtime bridge
   loads the OpenCL ICD functions beyond `clGetPlatformIDs`.
2. `src/lib/nogc_sync_mut/gpu/engine2d/sffi_opencl.spl` exposes real context,
   queue, program, kernel, enqueue, finish, and error status calls.
3. `src/lib/gc_async_mut/gpu/engine2d/opencl_session.spl` records typed
   unavailable, build-failed, submit-failed, sync-failed, and readback-failed
   states.
4. `generated_kernel_dispatch.spl` maps Simple generated 2D kernels to OpenCL C
   or OpenCL SPIR-V artifacts with target environment metadata.
5. Strict OpenCL creation reports CPU fallback as a failure, not as a pass.
...
- Report Simple backend identity: CPU scalar, CPU SIMD, Vulkan, CUDA, OpenCL,
  Metal, or WebGPU.
...
Existing startup, web-size, Tauri-equivalent, Electron/Node bitmap, and
graphics_2d probes may keep native outputs, but release-facing evidence must be
normalized into one backend comparison schema:
...
Claims compare only samples with the same fixture ID, viewport, sample count,
warmup count, compiler mode, hardware identity, and backend selection policy.
