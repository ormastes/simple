...
| E8-backlog Untyped-evidence corpus migration | migrate the remaining `untyped_capture` candidates | **OPEN, bounded and tracked** — `scripts/check/scan-untyped-evidence-candidates.shs` enumerates the full population: 1119 category-1 candidates across 414 files (`doc/08_tracking/audit/untyped_evidence_migration_candidates_2026-08-08.md`). 37 migrated ('yes') and 143 explicitly rejected with recorded reasons to date, across 9 worked batches (see `doc/08_tracking/todo/untyped_evidence_migration_backlog_2026-08-08.md` for the full migrated-file list). Yield rate is declining sharply as the easy front-loaded wins are exhausted (roughly 5/8 -> 1/24 -> 0/26 -> 1/41 accepted per batch) — most of the remaining ~939 unmarked rows skew toward the scanner's known false-positive class (static `file_read` source-text checks). This is real, correctly-scoped, incremental work with no natural single-session completion point; each future session should pull the next unmigrated block from the audit doc and apply the same per-candidate triage rule. |
...
**What remains open, precisely:** the E8-backlog row above (1082 of 1119 known untyped-
evidence candidates), and building real live-capture infrastructure for the E7 domain
profiles beyond the THREE live paths now landed (`exec_capture.spl`, `file_capture.spl`,
and — 2026-08-09 — `test/03_system/tools/spipe/examples/live_terminal_capture_spec.spl`,
the first real-subprocess-to-`TerminalSnapshot` slice, sabotage/revert proven). Both are
real, separately-scoped, multi-session efforts — not gaps in the lanes defined and
completed above. Task breakdown and per-domain design for both, plus the seed-binary
re-verification gate (T1): `modern_sspec_completion_plan_2026-08-09.md` (same directory).

**2026-08-09 goal audit:** an independent "really done? any gap or mocking?" audit
re-verified every LANDED claim above against origin/main bytes: all modules present,
docgen wiring real, `check-spipe-docgen-regeneration-live` PASS 4/4, and core proof specs
re-run green (`typed_evidence_oracle_spec` 28/28, `terminal_grid_spec` 21/21,
`exec_capture_spec` 6/6). No hidden mocking found — every fixture-driven artifact is
labeled as such. Caveat inherited by ALL green results to date: they ran on the disclosed
Rust seed binary; re-proving on a self-hosted deploy is T1 of the completion plan.
