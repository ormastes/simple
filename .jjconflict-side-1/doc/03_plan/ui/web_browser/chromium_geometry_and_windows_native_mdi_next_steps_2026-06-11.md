...
- The focused fixture-53 result records Chromium row flex-wrap cross-axis
  ...
  - the Simple row-wrap branch applies the same focused `flex-end` cross-axis
    ...
    pixels
- Live `54_flex_wrap_align_content_space_between` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-54 result records Chromium row flex-wrap cross-axis
  `align-content:space-between` distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=90`,
    `height=120`
  - the two wrapped lines have heights `20` and `30`, leaving 70px free in the
    cross axis; Chrome places the first line at `y=16` and the second at
    `y=106`
  - the Simple row-wrap branch applies the same focused inter-line gap without
    blur, tolerance, resolution scaling, or copied Chromium pixels
- Live `55_flex_wrap_align_content_space_around` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-55 result records Chromium row flex-wrap cross-axis
  `align-content:space-around` distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=90`,
    `height=130`
  - the two wrapped lines have heights `20` and `30`, leaving 80px free in the
...
- The focused fixture-56 result records Chromium row flex-wrap cross-axis
  ...
  - the Simple row-wrap branch applies the same focused start offset and
    inter-line gap without blur, tolerance, resolution scaling, or copied
    Chromium pixels
- Live `57_flex_wrap_gap_basic` evidence now passes with `layout_match` and
  `mismatch_count=0`.
- The focused fixture-57 result records Chromium row flex-wrap `gap:8px`
  distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=140`,
    `height=56`
  - Chrome places the first line at `y=16`, the second line at `y=48`, and
    the second item on the first line at `x=84`, proving the same 8px gap is
    used on the inline and cross axes
  - the Simple row-wrap branch applies the same gap without blur, tolerance,
    resolution scaling, or copied Chromium pixels
- Live `58_flex_wrap_axis_gap_basic` evidence now passes with `layout_match`
  and `mismatch_count=0`.
- The focused fixture-58 result records Chromium row flex-wrap axis-specific
  gap distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=150`,
    `height=56`
  - Chrome places the second first-line item at `x=86`, proving
    `column-gap:10px`, and the second line at `y=48`, proving `row-gap:8px`
  - the Simple row-wrap branch keeps row and column gap values separately
    without blur, tolerance, resolution scaling, or copied Chromium pixels
- Live `59_flex_column_axis_gap_basic` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-59 result records Chromium column flex axis-specific
  gap distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=120`,
    `height=90`
  - Chrome places the second item at `y=46` and third at `y=86`, proving
    `row-gap:10px` is the column flex main-axis gap while `column-gap:6px`
...
- The focused fixture-60 result records Chromium row flex per-item cross-axis
  ...
  - the Simple row flex branch applies the same per-item align-self offsets
    without blur, tolerance, resolution scaling, or copied Chromium pixels
- Live `61_flex_gap_space_around` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-61 result records Chromium row flex gap plus
  `justify-content:space-around` distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=180`,
    `height=40`
  - item widths `20 + 30 + 20` plus two `10px` base gaps leave `90px` free
    space; Chrome assigns `15px` before the first item and `30px` additional
    space between items, placing the three children at `x=31`, `x=91`, and
    `x=161`
...
- The focused fixture-62 result records Chromium column flex gap plus
  ...
  - the Simple column flex branch applies the same base-gap and space-around
    distribution without blur, tolerance, resolution scaling, or copied
    Chromium pixels
- Live `63_flex_wrap_gap_align_content_space_between` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-63 result records Chromium wrapped row flex line-gap plus
  `align-content:space-between` distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=90`,
    `height=140`
  - the first flex line has `height=20`; the second line has `height=30`; the
    explicit `row-gap:10px` makes base line content `60px`, leaving `80px`
    of extra space for `align-content:space-between`
  - Chrome therefore keeps the first item at `y=16` and places the second line
    at `y=126`, with the second-line children at `x=16` and `x=66`
  - the Simple wrap branch applies the same base row gap and align-content
    extra gap without blur, tolerance, resolution scaling, or copied Chromium
    pixels
- Live `64_flex_wrap_gap_align_content_space_around` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-64 result records Chromium wrapped row flex line-gap plus
  `align-content:space-around` distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=90`,
    `height=140`
  - the first flex line has `height=20`; the second line has `height=30`; the
    explicit `row-gap:10px` makes base line content `60px`, leaving `80px`
    of extra space for `align-content:space-around`
  - Chrome assigns `20px` before the first line and `40px` additional space
    between the lines, placing line starts at `y=36` and `y=106`
...
- The focused fixture-65 result records Chromium wrapped row flex line-gap plus
  ...
  - the Simple wrap branch applies the same base row gap and equal outer/inner
    ...
    pixels
- Live `66_flex_wrap_gap_align_content_flex_end` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-66 result records Chromium wrapped row flex line-gap plus
  `align-content:flex-end` distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=90`,
    `height=140`
  - the first flex line has `height=20`; the second line has `height=30`; the
    explicit `row-gap:10px` makes base line content `60px`, leaving `80px`
    of free cross-axis space
  - Chrome offsets the line block by that `80px`, placing line starts at
    `y=96` and `y=126`
  - the Simple wrap branch applies the same flex-end offset and base row gap
    without blur, tolerance, resolution scaling, or copied Chromium pixels
- Live `67_flex_wrap_gap_align_content_center` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-67 result records Chromium wrapped row flex line-gap plus
  `align-content:center` distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=90`,
    `height=140`
  - the first flex line has `height=20`; the second line has `height=30`; the
    explicit `row-gap:10px` makes base line content `60px`, leaving `80px`
    of free cross-axis space
  - Chrome centers the line block by offsetting it `40px`, placing line starts
    at `y=56` and `y=86`
  - the Simple wrap branch applies the same center offset and base row gap
    without blur, tolerance, resolution scaling, or copied Chromium pixels
- Live `68_flex_wrap_gap_align_content_stretch` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-68 result records Chromium wrapped row flex line-gap plus
  `align-content:stretch` distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=90`,
    `height=140`
  - the first flex line has base `height=20`; the second line has base
    `height=30`; the explicit `row-gap:10px` makes base line content `60px`,
    leaving `80px` of free cross-axis space
  - Chrome stretches the two flex line boxes by `40px` each, so the first line
    starts at `y=16` and the second line starts at `y=86`
  - the Simple wrap branch applies the same stretched line-box spacing without
    blur, tolerance, resolution scaling, or copied Chromium pixels
- Live `69_flex_wrap_gap_align_content_flex_start` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-69 result records Chromium wrapped row flex line-gap plus
  explicit `align-content:flex-start` distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=90`,
    `height=140`
  - the first flex line has `height=20`; the second line has `height=30`; the
    explicit `row-gap:10px` makes base line content `60px`, leaving `80px`
    of unused cross-axis space after the packed line block
  - Chrome places line starts at `y=16` and `y=46`, contrasting the stretched
    fixture where the second line starts at `y=86`
  - the Simple wrap branch keeps the same flex-start line packing without blur,
    tolerance, resolution scaling, or copied Chromium pixels
- Live `70_flex_wrap_gap_align_content_normal` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-70 result records Chromium wrapped row flex line-gap plus
  explicit `align-content:normal` distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=90`,
    `height=140`
  - the first flex line has base `height=20`; the second line has base
    `height=30`; the explicit `row-gap:10px` makes base line content `60px`,
    leaving `80px` of free cross-axis space
  - Chrome treats `normal` like stretch in this flex context, expanding the
    two flex line boxes by `40px` each and placing line starts at `y=16` and
    `y=86`
  - the Simple wrap branch now applies the same line-box expansion for
    `normal` and `stretch` without blur, tolerance, resolution scaling, or
    copied Chromium pixels
- Live `71_flex_wrap_gap_align_content_end` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-71 result records Chromium wrapped row flex line-gap plus
  logical `align-content:end` distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=90`,
    `height=140`
  - the first flex line has `height=20`; the second line has `height=30`; the
    explicit `row-gap:10px` makes base line content `60px`, leaving `80px`
    of free cross-axis space before the packed line block
  - Chrome maps logical `end` to the same cross-end placement as flex-end here,
    placing line starts at `y=96` and `y=126`
  - the Simple wrap branch applies the same end offset without blur,
    tolerance, resolution scaling, or copied Chromium pixels
- Live `72_flex_wrap_gap_align_content_start` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-72 result records Chromium wrapped row flex line-gap plus
  logical `align-content:start` distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=90`,
    `height=140`
  - the first flex line has `height=20`; the second line has `height=30`; the
    explicit `row-gap:10px` keeps the packed line starts at `y=16` and `y=46`
  - Chrome maps logical `start` to the same cross-start placement as flex-start
    here
  - the Simple wrap branch already applies the same start offset without blur,
    tolerance, resolution scaling, or copied Chromium pixels
- Live `73_flex_wrap_gap_align_content_unsafe_center` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-73 result records Chromium wrapped row flex line-gap plus
  `align-content:unsafe center` distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=90`,
    `height=140`
  - the first flex line has `height=20`; the second line has `height=30`; the
    explicit `row-gap:10px` makes base line content `60px`, leaving `80px`
  ...
  - Chrome ignores the overflow-position prefix for this in-bounds case and
    centers the packed line block at starts `y=56` and `y=86`
...
- The focused fixture-74 result records Chromium row flex gap plus
  ...
  - the Simple parser preserves `unsafe-center` and applies the same positive
    main-axis offset without blur, tolerance, resolution scaling, or copied
    Chromium pixels
- Live `75_flex_gap_justify_unsafe_center_overflow` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-75 result records Chromium row flex gap plus nominally
  overflowing `justify-content:unsafe center` distribution:
  - the explicit flex container border box is `x=16`, `y=16`, `width=100`,
    `height=60`
  - child widths `60`, `50`, and `30` plus two `12px` gaps create a `164px`
    nominal item group, then default `flex-shrink:1` resolves item widths to
    `33`, `27`, and `16`
  - Chrome aligns the shrink-resolved item block at `x=16`, `x=61`, and
    `x=100`
  - the Simple parser now preserves `unsafe-center` and avoids applying a
    pre-shrink negative main-axis offset, matching Chromium without blur,
    tolerance, resolution scaling, or copied Chromium pixels
- Live `76_flex_gap_justify_unsafe_center_no_shrink_overflow` evidence now
  passes with `layout_match` and `mismatch_count=0`.
- The focused fixture-76 result records Chromium row flex gap plus
  `flex-shrink:0` and overflowing `justify-content:unsafe center`:
  - the explicit flex container border box is `x=16`, `y=16`, `width=100`,
    `height=60`
  - child widths `60`, `50`, and `30` plus two `12px` gaps create a `164px`
    item group and `-64px` of main-axis free space
  - Chrome honors the unsafe overflow-position prefix when shrinking is
    disabled, placing children at `x=-16`, `x=56`, and `x=118`
...
- The focused fixture-77 result records Chromium row flex gap plus
  ...
  - the Simple renderer preserves `safe-center` and applies the same start-edge
    ...
    pixels
- Live `78_flex_column_gap_justify_unsafe_center_no_shrink_overflow` evidence
  now passes with `layout_match` and `mismatch_count=0`.
- The focused fixture-78 result records Chromium column flex gap plus
  `flex-shrink:0` and overflowing `justify-content:unsafe center`:
  - the explicit flex container border box is `x=16`, `y=16`, `width=80`,
    `height=100`
  - child heights `60`, `50`, and `30` plus two `12px` gaps create a `164px`
    item group and `-64px` of main-axis free space
  - Chrome honors the unsafe overflow-position prefix, placing children at
    `y=-16`, `y=56`, and `y=118`
  - the Simple renderer applies the same negative vertical unsafe-center offset
    without blur, tolerance, resolution scaling, or copied Chromium pixels
- Live `79_flex_column_gap_justify_safe_center_no_shrink_overflow` evidence
  now passes with `layout_match` and `mismatch_count=0`.
- The focused fixture-79 result records Chromium column flex gap plus
  `flex-shrink:0` and overflowing `justify-content:safe center`:
  - the explicit flex container border box is `x=16`, `y=16`, `width=80`,
    `height=100`
  - child heights `60`, `50`, and `30` plus two `12px` gaps create a `164px`
    item group and `-64px` of main-axis free space
  - Chrome falls back to start-edge alignment for the safe overflow-position,
    placing children at `y=16`, `y=88`, and `y=150`
  - the Simple renderer preserves `safe-center` and applies the same vertical
    start-edge fallback without blur, tolerance, resolution scaling, or copied
    Chromium pixels
- Live `80_flex_gap_justify_end` evidence now passes with `layout_match` and
  `mismatch_count=0`.
- The focused fixture-80 result records Chromium row flex gap plus logical
  `justify-content:end`:
  - the explicit flex container border box is `x=16`, `y=16`, `width=220`,
    `height=60`
  - child widths `40`, `50`, and `30` plus two `12px` gaps leave `76px` of
    free main-axis space
  - Chrome maps logical `end` to the main-end offset in this horizontal writing
    mode, placing children at `x=92`, `x=144`, and `x=206`
  - the Simple renderer applies the same end offset without blur, tolerance,
    resolution scaling, or copied Chromium pixels
- Live `81_flex_column_gap_justify_end` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-81 result records Chromium column flex gap plus logical
  `justify-content:end`:
  - the explicit flex container border box is `x=16`, `y=16`, `width=80`,
    `height=140`
  - child heights `20`, `30`, and `20` plus two `12px` gaps leave `46px` of
    free main-axis space
  - Chrome maps logical `end` to the main-end offset in this horizontal writing
    mode, placing children at `y=62`, `y=94`, and `y=136`
  - the Simple renderer applies the same vertical end offset without blur,
    tolerance, resolution scaling, or copied Chromium pixels
- Live `82_flex_gap_align_items_end` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-82 result records Chromium row flex gap plus logical
  `align-items:end`:
  - the explicit flex container border box is `x=16`, `y=16`, `width=140`,
    `height=80`
  - child heights `20`, `30`, and `40` leave `60`, `50`, and `40` pixels of
    cross-axis offset from the container top
  - Chrome maps logical `end` to the cross-axis end offset in this horizontal
    writing mode, placing children at `y=76`, `y=66`, and `y=56`
  - the Simple renderer applies the same cross-axis end offset without blur,
    tolerance, resolution scaling, or copied Chromium pixels
- Live `83_flex_auto_margin_align_center` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-83 result records Chromium row flex auto-margin behavior
  plus `align-items:center`:
  - the explicit flex container border box is `x=0`, `y=0`, `width=210`,
    `height=110`, including `10px` padding on each side
  - fixed child widths plus the third child's `10px` left margin leave `60px`
    of positive main-axis free space
  - Chrome assigns that free space to the second child's `margin-left:auto`,
    placing children at `x=10`, `x=110`, and `x=160`
  - the Simple renderer now preserves left/right auto-margin state and assigns
    row main-axis positive free space to auto margins before `justify-content`,
    without blur, tolerance, resolution scaling, or copied Chromium pixels
- Live `84_flex_column_auto_margin_align_center` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-84 result records Chromium column flex auto-margin
  behavior plus `align-items:center`:
  - the explicit flex container border box is `x=0`, `y=0`, `width=110`,
    `height=210`, including `10px` padding on each side
  - fixed child heights plus the third child's `10px` top margin leave `60px`
    of positive main-axis free space
  - Chrome assigns that free space to the second child's `margin-top:auto`,
    placing children at `y=10`, `y=110`, and `y=160`
  - the Simple renderer now preserves top/bottom auto-margin state and assigns
    column main-axis positive free space to auto margins before
    `justify-content`, without blur, tolerance, resolution scaling, or copied
    Chromium pixels
- Live `85_flex_cross_auto_margin_top` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-85 result records Chromium row flex cross-axis
  auto-margin behavior plus `align-items:center`:
  - the explicit flex container border box is `x=0`, `y=0`, `width=160`,
    `height=110`, including `10px` padding on each side
  - Chrome keeps the first and third children center-aligned on the cross axis
    at `y=45` and `y=35`
  - Chrome assigns the second child's cross-axis free space to
    `margin-top:auto`, placing it at `y=70`
  - the Simple renderer now lets top/bottom auto margins absorb positive row
    cross-axis free space before `align-items`, without blur, tolerance,
    resolution scaling, or copied Chromium pixels
- Live `86_flex_row_cross_auto_margin_align_center` evidence now passes with
  `layout_match` and `mismatch_count=0`.
- The focused fixture-86 result records Chromium row flex cross-axis
  auto-margin behavior with separate top and bottom auto margins:
  - the explicit flex container border box is `x=0`, `y=0`, `width=210`,
    `height=110`, including `10px` padding on each side
  - Chrome keeps the first child center-aligned on the cross axis at `y=45`
  - Chrome assigns the second child's positive cross-axis free space to
    `margin-top:auto`, placing it at `y=50`
  - Chrome assigns the third child's positive cross-axis free space to
...
- Chrome headless manifest evidence now includes
  ...
  `66_flex_wrap_gap_align_content_flex_end`,
  `67_flex_wrap_gap_align_content_center`,
  `68_flex_wrap_gap_align_content_stretch`, and
  `69_flex_wrap_gap_align_content_flex_start`, and
  ...
  `77_flex_gap_justify_safe_center_no_shrink_overflow`, and
  `78_flex_column_gap_justify_unsafe_center_no_shrink_overflow`, and
  `79_flex_column_gap_justify_safe_center_no_shrink_overflow`, and
  `80_flex_gap_justify_end`, and
  `81_flex_column_gap_justify_end`, and
  `82_flex_gap_align_items_end`, and
