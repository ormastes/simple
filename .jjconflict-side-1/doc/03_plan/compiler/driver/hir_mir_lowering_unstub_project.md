...
### P0 attempt 2026-06-25 — BLOCKED at the run/verify layer (flag placed, reverted)

A `SIMPLE_UNSTUB_HIR` flag was added to both stub branches (widening the
"real-lower" condition in `lower_and_check_impl` and `lower_to_mir`), confirmed
**inert by default** (a normal check stayed rc=0), then reverted. Two findings
that refine the project:

1. **The stub is reachable only via `driver.compile()`** (the full multi-source
   pipeline: `load_sources_impl → parse_all_impl → lower_and_check_impl →
   lower_to_mir`). Single-file `check` takes the *other* branch
   (`lower_and_check_impl`, `self.ctx.sources.len() <= 0`, ~L389–411), which
   **already lowers for real** via `self.ctx.modules`. So `check` is **not**
   stubbed; the stub matters specifically for the multi-source compile/build/`-c`
   path. (Probes: the `sources.len() > 0` branch never fires for `check FILE`.)

2. **Exercising the stub is blocked in the current tree.** Driving the pipeline
   via `seed run src/compiler/80.driver/main.spl -c <file>` reaches
   `phase1:load_sources` then fails (rc=1) **before phase 3** with seed-interprets-
   the-full-driver errors: `Unknown variable: rt_gc_allocated_bytes while lowering
   gc_init`, and undefined export symbols (`IoError`, `LOG_ERROR`, …). The full
   self-hosted compile pipeline does not run cleanly under the seed interpreter,
   and the native self-hosted build is blocked by bootstrap Stage 3 (always
   fails). So P0's "run the corpus" step has **no working harness** yet.

**Therefore P0 has a prerequisite-of-the-prerequisite (new P-1):** get the
multi-source `driver.compile()` pipeline to *run at all* — either (a) make the
seed interpret the full driver cleanly (fix `gc_init` / export-symbol resolution
under interpretation), or (b) get a green self-hosted build to run it natively.
Until P-1 is met, P0 cannot catalog lowering failures because the stub is never
reached. The `SIMPLE_UNSTUB_HIR` flag (≈2-line condition widening per stub) is
the correct scaffold to re-apply once a harness exists.
