...
### bitwise byte and slice helpers
...
#### get_byte fails closed on invalid byte positions
...
```simple
# @req REQ-SSPEC-UNIT
step("get_byte fails closed on invalid byte positions")
val n = 0x1234567890ABCDEF
assert_equal(get_byte(n, -1), 0)
assert_equal(get_byte(n, 8), 0)
```

</details>

#### set_byte leaves input unchanged on invalid byte positions
...
```simple
# @req REQ-SSPEC-UNIT
step("set_byte leaves input unchanged on invalid byte positions")
val n = 0x1234
assert_equal(set_byte(n, -1, 0xAB), n)
assert_equal(set_byte(n, 8, 0xAB), n)
```

</details>

#### set_byte masks values to one byte
...
```simple
# @req REQ-SSPEC-UNIT
step("set_byte masks values to one byte")
assert_equal(set_byte(0, 0, 0x123), 0x23)
```
...
#### extract_bits fails closed on invalid slices
...
```simple
# @req REQ-SSPEC-UNIT
step("extract_bits fails closed on invalid slices")
assert_equal(extract_bits(0xFF, -1, 1), 0)
assert_equal(extract_bits(0xFF, 0, 0), 0)
assert_equal(extract_bits(0xFF, 63, 2), 0)
```

</details>

#### insert_bits leaves input unchanged on invalid slices
...
```simple
# @req REQ-SSPEC-UNIT
step("insert_bits leaves input unchanged on invalid slices")
val n = 0x55
assert_equal(insert_bits(n, 0xFF, -1, 1), n)
assert_equal(insert_bits(n, 0xFF, 0, 0), n)
assert_equal(insert_bits(n, 0xFF, 63, 2), n)
```
...
- Canonical SPipe generation for source `584c5e8eba2a2a1ee9eae100a73b0aa81aa6e2ab80bdeee75bb65b8665cafc1e`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `584c5e8eba2a2a1ee9eae100a73b0aa81aa6e2ab80bdeee75bb65b8665cafc1e`.
...
source: test/01_unit/lib/bitwise_byte_helpers_spec.spl
mirror: doc/06_spec/01_unit/lib/bitwise_byte_helpers_spec.md (current)
findings: 5 blockers: 0
  narrative=100 structure=100 oracle=100
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/01_unit/lib/bitwise_byte_helpers_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/01_unit/lib/bitwise_byte_helpers_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, evidence, unsupported/limitations, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/01_unit/lib/bitwise_byte_helpers_spec.spl:29:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'get_byte extracts each byte position' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/01_unit/lib/bitwise_byte_helpers_spec.spl:42:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'set_byte writes each byte position' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/01_unit/lib/bitwise_byte_helpers_spec.spl:51:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'set_byte clears the existing byte before writing' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
