# Browser Session Fetch Wasm Chain Specification

> Tests covering BrowserSession fetch WASM promise chain.

| Tests | Active | Skipped | Pending |
|-------|--------|---------|--------:|
| 250 | 250 | 0 | 0 |

<details>
<summary>Full Scenario Manual</summary>

# Browser Session Fetch Wasm Chain Specification

## Scenarios

### BrowserSession fetch WASM promise chain

#### chains fetched arrayBuffer bytes into WebAssembly.instantiate

**Manual warnings:**
- invalid manual visibility metadata: # @manual scenario evidence (expected show, folded, detail, or skip)


- chains fetched arrayBuffer bytes into WebAssembly.instantiate
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:11>instantiate:instantiated:11:1:object`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("chains fetched arrayBuffer bytes into WebAssembly.instantiate")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.instantiate(bytes); }).then(function(result) { out = stages + 'instantiate:' + result.status + ':' + result.module.byteLength + ':' + result.module.sectionCount + ':' + typeof result.instance.exports; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010100",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:11>instantiate:instantiated:11:1:object")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### exposes fetched arrayBuffer instantiated module export descriptors

- exposes fetched arrayBuffer instantiated module export descriptors
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:40>instantiate:instantiated:40:2:tbl:table:answer:global`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes fetched arrayBuffer instantiated module export descriptors")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.instantiate(bytes); }).then(function(result) { var exports = WebAssembly.Module.exports(result.module); out = stages + 'instantiate:' + result.status + ':' + result.module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:40>instantiate:instantiated:40:2:tbl:table:answer:global")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### chains fetched arrayBuffer bytes into WebAssembly function body calls

- chains fetched arrayBuffer bytes into WebAssembly function body calls
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:41>instantiate:instantiated:41:function:42:42`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("chains fetched arrayBuffer bytes into WebAssembly function body calls")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.instantiate(bytes); }).then(function(result) { out = stages + 'instantiate:' + result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports.run + ':' + result.instance.exports.run(40, 2) + ':' + result.instance.exports.run(7, 35); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001070160027f7f017f030201000707010372756e00000a09010700200020016a0b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:41>instantiate:instantiated:41:function:42:42")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### compares fetched arrayBuffer direct and compiled function body calls

- compares fetched arrayBuffer direct and compiled function body calls
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/direct.wasm`
   - Expected: _display_js(value) equals `directArrayBuffer:41:instantiated:41:function:42:42`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing direct fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compiled.wasm`
   - Expected: _display_js(value) equals `directArrayBuffer:41:instantiated:41:function:42:42:compiledArrayBuffer:41:41... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing compiled fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares fetched arrayBuffer direct and compiled function body calls")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var directBytes = 0; var compiledBytes = 0; var compiledModuleBytes = 0; window.fetch('/direct.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { directBytes = bytes.byteLength; return WebAssembly.instantiate(bytes); }).then(function(result) { out = 'directArrayBuffer:' + directBytes + ':' + result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports.run + ':' + result.instance.exports.run(40, 2) + ':' + result.instance.exports.run(7, 35); }); window.fetch('/compiled.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { compiledBytes = bytes.byteLength; return WebAssembly.compile(bytes); }).then(function(module) { compiledModuleBytes = module.byteLength; return WebAssembly.instantiate(module); }).then(function(result) { out = out + ':compiledArrayBuffer:' + compiledBytes + ':' + compiledModuleBytes + ':' + result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports.run + ':' + result.instance.exports.run(40, 2) + ':' + result.instance.exports.run(7, 35); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/direct.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001070160027f7f017f030201000707010372756e00000a09010700200020016a0b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBuffer:41:instantiated:41:function:42:42")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing direct fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compiled.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001070160027f7f017f030201000707010372756e00000a09010700200020016a0b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBuffer:41:instantiated:41:function:42:42:compiledArrayBuffer:41:41:instantiated:41:function:42:42")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing compiled fetch request").to_equal("")
```

</details>

#### compares fetched arrayBuffer direct and compiled multiple function exports

- compares fetched arrayBuffer direct and compiled multiple function exports
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/direct.wasm`
   - Expected: _display_js(value) equals `directArrayBufferMulti:47:instantiated:47:2:function:function:undefined:undef... (full value in folded executable source)`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing direct fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compiled.wasm`
   - Expected: _display_js(value) equals `directArrayBufferMulti:47:instantiated:47:2:function:function:undefined:undef... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing compiled fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares fetched arrayBuffer direct and compiled multiple function exports")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var directBytes = 0; var compiledBytes = 0; var compiledModuleBytes = 0; window.fetch('/direct.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { directBytes = bytes.byteLength; return WebAssembly.instantiate(bytes); }).then(function(result) { var exports = result.instance.exports; out = 'directArrayBufferMulti:' + directBytes + ':' + result.status + ':' + result.module.byteLength + ':' + result.module.functionExportCount + ':' + typeof exports.init + ':' + typeof exports.render + ':' + exports.init() + ':' + exports.render(); }); window.fetch('/compiled.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { compiledBytes = bytes.byteLength; return WebAssembly.compile(bytes); }).then(function(module) { compiledModuleBytes = module.byteLength; return WebAssembly.instantiate(module); }).then(function(result) { var exports = result.instance.exports; out = out + ':compiledArrayBufferMulti:' + compiledBytes + ':' + compiledModuleBytes + ':' + result.status + ':' + result.module.byteLength + ':' + result.module.functionExportCount + ':' + typeof exports.init + ':' + typeof exports.render + ':' + exports.init() + ':' + exports.render(); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/direct.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000030302000007110204696e697400000672656e64657200010a070202000b02000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferMulti:47:instantiated:47:2:function:function:undefined:undefined")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing direct fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compiled.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000030302000007110204696e697400000672656e64657200010a070202000b02000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferMulti:47:instantiated:47:2:function:function:undefined:undefined:compiledArrayBufferMulti:47:47:instantiated:47:2:function:function:undefined:undefined")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing compiled fetch request").to_equal("")
```

</details>

#### compares fetched arrayBuffer direct and compiled table and global exports

- compares fetched arrayBuffer direct and compiled table and global exports
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/direct.wasm`
   - Expected: _display_js(value) equals `directArrayBufferTableGlobal:40:instantiated:40:table:funcref:null:1:2:global... (full value in folded executable source)`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing direct fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compiled.wasm`
   - Expected: _display_js(value) equals `directArrayBufferTableGlobal:40:instantiated:40:table:funcref:null:1:2:global... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing compiled fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares fetched arrayBuffer direct and compiled table and global exports")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var directBytes = 0; var compiledBytes = 0; var compiledModuleBytes = 0; window.fetch('/direct.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { directBytes = bytes.byteLength; return WebAssembly.instantiate(bytes); }).then(function(result) { var table = result.instance.exports.tbl; var global = result.instance.exports.answer; var before = table.get(0); var old = table.grow(1, null); out = 'directArrayBufferTableGlobal:' + directBytes + ':' + result.status + ':' + result.module.byteLength + ':' + table.kind + ':' + table.element + ':' + before + ':' + old + ':' + table.length + ':' + global.kind + ':' + global.valueType + ':' + global.mutable + ':' + global.value; }); window.fetch('/compiled.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { compiledBytes = bytes.byteLength; return WebAssembly.compile(bytes); }).then(function(module) { compiledModuleBytes = module.byteLength; return WebAssembly.instantiate(module); }).then(function(result) { var table = result.instance.exports.tbl; var global = result.instance.exports.answer; var before = table.get(0); var old = table.grow(1, null); out = out + ':compiledArrayBufferTableGlobal:' + compiledBytes + ':' + compiledModuleBytes + ':' + result.status + ':' + result.module.byteLength + ':' + table.kind + ':' + table.element + ':' + before + ':' + old + ':' + table.length + ':' + global.kind + ':' + global.valueType + ':' + global.mutable + ':' + global.value; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/direct.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferTableGlobal:40:instantiated:40:table:funcref:null:1:2:global:i32:false:42")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing direct fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compiled.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferTableGlobal:40:instantiated:40:table:funcref:null:1:2:global:i32:false:42:compiledArrayBufferTableGlobal:40:40:instantiated:40:table:funcref:null:1:2:global:i32:false:42")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing compiled fetch request").to_equal("")
```

</details>

#### compares fetched arrayBuffer direct and compiled memory exports

- compares fetched arrayBuffer direct and compiled memory exports
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/direct.wasm`
   - Expected: _display_js(value) equals `directArrayBufferMemory:25:instantiated:25:131072:65536:4:1:131072:4`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing direct fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compiled.wasm`
   - Expected: _display_js(value) equals `directArrayBufferMemory:25:instantiated:25:131072:65536:4:1:131072:4:compiled... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing compiled fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares fetched arrayBuffer direct and compiled memory exports")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var directBytes = 0; var compiledBytes = 0; var compiledModuleBytes = 0; window.fetch('/direct.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { directBytes = bytes.byteLength; return WebAssembly.instantiate(bytes); }).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[6] = 260; var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); out = 'directArrayBufferMemory:' + directBytes + ':' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + bytes[6] + ':' + old + ':' + grown.length + ':' + grown[6]; }); window.fetch('/compiled.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { compiledBytes = bytes.byteLength; return WebAssembly.compile(bytes); }).then(function(module) { compiledModuleBytes = module.byteLength; return WebAssembly.instantiate(module); }).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[8] = 260; var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); out = out + ':compiledArrayBufferMemory:' + compiledBytes + ':' + compiledModuleBytes + ':' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + bytes[8] + ':' + old + ':' + grown.length + ':' + grown[8]; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/direct.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000503010001070a01066d656d6f72790200",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferMemory:25:instantiated:25:131072:65536:4:1:131072:4")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing direct fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compiled.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000503010001070a01066d656d6f72790200",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferMemory:25:instantiated:25:131072:65536:4:1:131072:4:compiledArrayBufferMemory:25:25:instantiated:25:131072:65536:4:1:131072:4")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing compiled fetch request").to_equal("")
```

</details>

#### compares fetched arrayBuffer direct and compiled memory maximum failures

- compares fetched arrayBuffer direct and compiled memory maximum failures
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/direct.wasm`
   - Expected: _display_js(value) equals `directArrayBufferMemoryMax:26:instantiated:26:65536:65536:1:-1:65536:18`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing direct fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compiled.wasm`
   - Expected: _display_js(value) equals `directArrayBufferMemoryMax:26:instantiated:26:65536:65536:1:-1:65536:18:compi... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing compiled fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares fetched arrayBuffer direct and compiled memory maximum failures")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var directBytes = 0; var compiledBytes = 0; var compiledModuleBytes = 0; window.fetch('/direct.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { directBytes = bytes.byteLength; return WebAssembly.instantiate(bytes); }).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[12] = 274; var zero = memory.grow(0); var fail = memory.grow(1); var after = new Uint8Array(memory.buffer); out = 'directArrayBufferMemoryMax:' + directBytes + ':' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + zero + ':' + fail + ':' + after.length + ':' + after[12]; }); window.fetch('/compiled.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { compiledBytes = bytes.byteLength; return WebAssembly.compile(bytes); }).then(function(module) { compiledModuleBytes = module.byteLength; return WebAssembly.instantiate(module); }).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[13] = 276; var zero = memory.grow(0); var fail = memory.grow(1); var after = new Uint8Array(memory.buffer); out = out + ':compiledArrayBufferMemoryMax:' + compiledBytes + ':' + compiledModuleBytes + ':' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + zero + ':' + fail + ':' + after.length + ':' + after[13]; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/direct.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000050401010101070a01066d656d6f72790200",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferMemoryMax:26:instantiated:26:65536:65536:1:-1:65536:18")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing direct fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compiled.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000050401010101070a01066d656d6f72790200",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferMemoryMax:26:instantiated:26:65536:65536:1:-1:65536:18:compiledArrayBufferMemoryMax:26:26:instantiated:26:65536:65536:1:-1:65536:20")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing compiled fetch request").to_equal("")
```

</details>

#### compares fetched arrayBuffer direct and compiled import calls

- compares fetched arrayBuffer direct and compiled import calls
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/direct.wasm`
   - Expected: _display_js(value) equals `directArrayBufferImport:52:instantiated:1:52:42:1`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing direct fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compiled.wasm`
   - Expected: _display_js(value) equals `directArrayBufferImport:52:instantiated:1:52:42:1:compiledArrayBufferImport:5... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing compiled fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares fetched arrayBuffer direct and compiled import calls")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var directBytes = 0; var compiledBytes = 0; var compiledModuleBytes = 0; var calls = 0; var imports = { env: { foo: function(x) { calls = calls + 1; return x + 2; } } }; window.fetch('/direct.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { directBytes = bytes.byteLength; return WebAssembly.instantiate(bytes, imports); }).then(function(result) { out = 'directArrayBufferImport:' + directBytes + ':' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + result.instance.exports.run(40) + ':' + calls; }); window.fetch('/compiled.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { compiledBytes = bytes.byteLength; return WebAssembly.compile(bytes); }).then(function(module) { compiledModuleBytes = module.byteLength; return WebAssembly.instantiate(module, imports); }).then(function(result) { out = out + ':compiledArrayBufferImport:' + compiledBytes + ':' + compiledModuleBytes + ':' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + result.instance.exports.run(40) + ':' + calls; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/direct.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferImport:52:instantiated:1:52:42:1")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing direct fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compiled.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferImport:52:instantiated:1:52:42:1:compiledArrayBufferImport:52:52:instantiated:1:52:42:2")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing compiled fetch request").to_equal("")
```

</details>

#### compares fetched arrayBuffer direct and compiled missing imports

- compares fetched arrayBuffer direct and compiled missing imports
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/direct.wasm`
   - Expected: _display_js(value) equals `directArrayBufferMissingImport:52:invalid:unsupported-wasm-imports`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing direct fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compiled.wasm`
   - Expected: _display_js(value) equals `directArrayBufferMissingImport:52:invalid:unsupported-wasm-imports:compiledAr... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing compiled fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares fetched arrayBuffer direct and compiled missing imports")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var directBytes = 0; var compiledBytes = 0; var compiledModuleBytes = 0; window.fetch('/direct.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { directBytes = bytes.byteLength; return WebAssembly.instantiate(bytes, {}); }).then(function(result) { out = 'unexpectedDirect:' + result.status; }).catch(function(err) { out = 'directArrayBufferMissingImport:' + directBytes + ':' + err.status + ':' + err.error; }); window.fetch('/compiled.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { compiledBytes = bytes.byteLength; return WebAssembly.compile(bytes); }).then(function(module) { compiledModuleBytes = module.byteLength; return WebAssembly.instantiate(module, {}); }).then(function(result) { out = out + ':unexpectedCompiled:' + result.status; }).catch(function(err) { out = out + ':compiledArrayBufferMissingImport:' + compiledBytes + ':' + compiledModuleBytes + ':' + err.status + ':' + err.error; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/direct.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferMissingImport:52:invalid:unsupported-wasm-imports")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing direct fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compiled.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferMissingImport:52:invalid:unsupported-wasm-imports:compiledArrayBufferMissingImport:52:52:invalid:unsupported-wasm-imports")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing compiled fetch request").to_equal("")
```

</details>

#### compares fetched arrayBuffer direct and compiled export descriptors

- compares fetched arrayBuffer direct and compiled export descriptors
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/direct.wasm`
   - Expected: _display_js(value) equals `directArrayBufferExports:40:instantiated:40:2:tbl:table:answer:global`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing direct fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compiled.wasm`
   - Expected: _display_js(value) equals `directArrayBufferExports:40:instantiated:40:2:tbl:table:answer:global:compile... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing compiled fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares fetched arrayBuffer direct and compiled export descriptors")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var directBytes = 0; var compiledBytes = 0; var compiledModuleBytes = 0; window.fetch('/direct.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { directBytes = bytes.byteLength; return WebAssembly.instantiate(bytes); }).then(function(result) { var exports = WebAssembly.Module.exports(result.module); out = 'directArrayBufferExports:' + directBytes + ':' + result.status + ':' + result.module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); window.fetch('/compiled.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { compiledBytes = bytes.byteLength; return WebAssembly.compile(bytes); }).then(function(module) { compiledModuleBytes = module.byteLength; var exports = WebAssembly.Module.exports(module); out = out + ':compiledArrayBufferExports:' + compiledBytes + ':' + compiledModuleBytes + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; return WebAssembly.instantiate(module); }).then(function(result) { out = out + ':compiledInstantiate:' + result.status + ':' + result.module.byteLength; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/direct.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferExports:40:instantiated:40:2:tbl:table:answer:global")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing direct fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compiled.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferExports:40:instantiated:40:2:tbl:table:answer:global:compiledArrayBufferExports:40:40:2:tbl:table:answer:global:compiledInstantiate:instantiated:40")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing compiled fetch request").to_equal("")
```

</details>

#### compares fetched arrayBuffer direct and compiled import descriptors

- compares fetched arrayBuffer direct and compiled import descriptors
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/direct.wasm`
   - Expected: _display_js(value) equals `directArrayBufferImports:27:instantiated:1:27:1:env:foo:function:object`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing direct fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compiled.wasm`
   - Expected: _display_js(value) equals `directArrayBufferImports:27:instantiated:1:27:1:env:foo:function:object:compi... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing compiled fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares fetched arrayBuffer direct and compiled import descriptors")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var directBytes = 0; var compiledBytes = 0; var compiledModuleBytes = 0; var imports = { env: { foo: function() { return 7; } } }; window.fetch('/direct.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { directBytes = bytes.byteLength; return WebAssembly.instantiate(bytes, imports); }).then(function(result) { var descriptors = WebAssembly.Module.imports(result.module); out = 'directArrayBufferImports:' + directBytes + ':' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind + ':' + typeof result.instance.exports; }); window.fetch('/compiled.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { compiledBytes = bytes.byteLength; return WebAssembly.compile(bytes); }).then(function(module) { compiledModuleBytes = module.byteLength; var descriptors = WebAssembly.Module.imports(module); out = out + ':compiledArrayBufferImports:' + compiledBytes + ':' + module.importCount + ':' + compiledModuleBytes + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind; return WebAssembly.instantiate(module, imports); }).then(function(result) { out = out + ':compiledInstantiate:' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + typeof result.instance.exports; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/direct.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000020b0103656e7603666f6f0000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferImports:27:instantiated:1:27:1:env:foo:function:object")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing direct fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compiled.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000020b0103656e7603666f6f0000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferImports:27:instantiated:1:27:1:env:foo:function:object:compiledArrayBufferImports:27:1:27:1:env:foo:function:compiledInstantiate:instantiated:1:27:object")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing compiled fetch request").to_equal("")
```

</details>

#### compares fetched arrayBuffer direct and compiled invalid header catches

- compares fetched arrayBuffer direct and compiled invalid header catches
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/direct-bad.wasm`
   - Expected: _display_js(value) equals `directArrayBufferInvalid:16:invalid:invalid-wasm-header`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing direct fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compiled-bad.wasm`
   - Expected: _display_js(value) equals `directArrayBufferInvalid:16:invalid:invalid-wasm-header:compiledArrayBufferIn... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing compiled fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares fetched arrayBuffer direct and compiled invalid header catches")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var directBytes = 0; var compiledBytes = 0; window.fetch('/direct-bad.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { directBytes = bytes.byteLength; return WebAssembly.instantiate(bytes); }).then(function(result) { out = 'unexpectedDirect:' + result.status; }).catch(function(err) { out = 'directArrayBufferInvalid:' + directBytes + ':' + err.status + ':' + err.error; }); window.fetch('/compiled-bad.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { compiledBytes = bytes.byteLength; return WebAssembly.compile(bytes); }).then(function(module) { out = out + ':unexpectedCompiled:' + module.byteLength; }).catch(function(err) { out = out + ':compiledArrayBufferInvalid:' + compiledBytes + ':' + err.status + ':' + err.error; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/direct-bad.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d00000000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferInvalid:16:invalid:invalid-wasm-header")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing direct fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compiled-bad.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d00000000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferInvalid:16:invalid:invalid-wasm-header:compiledArrayBufferInvalid:16:invalid:invalid-wasm-header")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing compiled fetch request").to_equal("")
```

</details>

#### compares fetched arrayBuffer direct and compiled fetch error catches

- compares fetched arrayBuffer direct and compiled fetch error catches
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/direct-down.wasm`
   - Expected: "expected first fetch commit error" equals ``
   - Expected: err equals `network-down`
   - Expected: _display_js(value) equals `directArrayBufferFetchError:network-down`
   - Expected: "unexpected first commit js error: {js_err}" equals ``
   - Expected: "missing direct fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compiled-down.wasm`
   - Expected: "expected second fetch commit error" equals ``
   - Expected: err equals `network-down`
   - Expected: _display_js(value) equals `directArrayBufferFetchError:network-down:compiledArrayBufferFetchError:networ... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {js_err}" equals ``
   - Expected: "missing compiled fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 70 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares fetched arrayBuffer direct and compiled fetch error catches")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; window.fetch('/direct-down.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { return WebAssembly.instantiate(bytes); }).then(function(result) { out = 'unexpectedDirect:' + result.status; }).catch(function(err) { out = 'directArrayBufferFetchError:' + err; }); window.fetch('/compiled-down.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { return WebAssembly.compile(bytes); }).then(function(module) { out = out + ':unexpectedCompiled:' + module.byteLength; }).catch(function(err) { out = out + ':compiledArrayBufferFetchError:' + err; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/direct-down.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 0,
            headers: "",
            body: "",
            error: "network-down"
        ))
        match committed:
            Ok(_):
                expect("expected first fetch commit error").to_equal("")
            Err(err):
                expect(err).to_equal("network-down")
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferFetchError:network-down")
                    Err(js_err):
                        expect("unexpected first commit js error: {js_err}").to_equal("")
    nil:
        expect("missing direct fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compiled-down.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 0,
            headers: "",
            body: "",
            error: "network-down"
        ))
        match committed:
            Ok(_):
                expect("expected second fetch commit error").to_equal("")
            Err(err):
                expect(err).to_equal("network-down")
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferFetchError:network-down:compiledArrayBufferFetchError:network-down")
                    Err(js_err):
                        expect("unexpected second commit js error: {js_err}").to_equal("")
    nil:
        expect("missing compiled fetch request").to_equal("")
```

</details>

#### compares fetched arrayBuffer direct and compiled base module metadata

- compares fetched arrayBuffer direct and compiled base module metadata
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/direct.wasm`
   - Expected: _display_js(value) equals `directArrayBufferModule:11:instantiated:11:1:object`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing direct fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compiled.wasm`
   - Expected: _display_js(value) equals `directArrayBufferModule:11:instantiated:11:1:object:compiledArrayBufferModule... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing compiled fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares fetched arrayBuffer direct and compiled base module metadata")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var directBytes = 0; var compiledBytes = 0; window.fetch('/direct.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { directBytes = bytes.byteLength; return WebAssembly.instantiate(bytes); }).then(function(result) { out = 'directArrayBufferModule:' + directBytes + ':' + result.status + ':' + result.module.byteLength + ':' + result.module.sectionCount + ':' + typeof result.instance.exports; }); window.fetch('/compiled.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { compiledBytes = bytes.byteLength; return WebAssembly.compile(bytes); }).then(function(module) { out = out + ':compiledArrayBufferModule:' + compiledBytes + ':' + module.byteLength + ':' + module.sectionCount + ':' + module.hasTypeSection; return WebAssembly.instantiate(module); }).then(function(result) { out = out + ':compiledInstantiate:' + result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/direct.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010100",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferModule:11:instantiated:11:1:object")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing direct fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compiled.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010100",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("directArrayBufferModule:11:instantiated:11:1:object:compiledArrayBufferModule:11:11:1:true:compiledInstantiate:instantiated:11:object")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing compiled fetch request").to_equal("")
```

</details>

#### chains fetched arrayBuffer bytes into multiple WebAssembly function exports

- chains fetched arrayBuffer bytes into multiple WebAssembly function exports
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:47>instantiate:instantiated:47:2:function:function:undefine... (full value in folded executable source)`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("chains fetched arrayBuffer bytes into multiple WebAssembly function exports")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.instantiate(bytes); }).then(function(result) { var exports = result.instance.exports; out = stages + 'instantiate:' + result.status + ':' + result.module.byteLength + ':' + result.module.functionExportCount + ':' + typeof exports.init + ':' + typeof exports.render + ':' + exports.init() + ':' + exports.render(); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000030302000007110204696e697400000672656e64657200010a070202000b02000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:47>instantiate:instantiated:47:2:function:function:undefined:undefined")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### chains fetched arrayBuffer bytes into WebAssembly table and global exports

- chains fetched arrayBuffer bytes into WebAssembly table and global exports
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:40>instantiate:instantiated:40:table:funcref:null:1:2:globa... (full value in folded executable source)`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("chains fetched arrayBuffer bytes into WebAssembly table and global exports")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.instantiate(bytes); }).then(function(result) { var table = result.instance.exports.tbl; var global = result.instance.exports.answer; var before = table.get(0); var old = table.grow(1, null); out = stages + 'instantiate:' + result.status + ':' + result.module.byteLength + ':' + table.kind + ':' + table.element + ':' + before + ':' + old + ':' + table.length + ':' + global.kind + ':' + global.valueType + ':' + global.mutable + ':' + global.value; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:40>instantiate:instantiated:40:table:funcref:null:1:2:global:i32:false:42")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### chains fetched arrayBuffer bytes into WebAssembly memory exports

- chains fetched arrayBuffer bytes into WebAssembly memory exports
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:25>instantiate:instantiated:25:131072:65536:4:1:131072:4`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("chains fetched arrayBuffer bytes into WebAssembly memory exports")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.instantiate(bytes); }).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[6] = 260; var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); out = stages + 'instantiate:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + bytes[6] + ':' + old + ':' + grown.length + ':' + grown[6]; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000503010001070a01066d656d6f72790200",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:25>instantiate:instantiated:25:131072:65536:4:1:131072:4")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### chains fetched arrayBuffer bytes into WebAssembly import calls

- chains fetched arrayBuffer bytes into WebAssembly import calls
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:52>instantiate:instantiated:1:52:42:1`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("chains fetched arrayBuffer bytes into WebAssembly import calls")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; var calls = 0; var imports = { env: { foo: function(x) { calls = calls + 1; return x + 2; } } }; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.instantiate(bytes, imports); }).then(function(result) { out = stages + 'instantiate:' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + result.instance.exports.run(40) + ':' + calls; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:52>instantiate:instantiated:1:52:42:1")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### exposes fetched arrayBuffer instantiated module import descriptors

- exposes fetched arrayBuffer instantiated module import descriptors
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:27>instantiate:instantiated:1:27:1:env:foo:function:object`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes fetched arrayBuffer instantiated module import descriptors")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; var imports = { env: { foo: function() { return 7; } } }; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.instantiate(bytes, imports); }).then(function(result) { var descriptors = WebAssembly.Module.imports(result.module); out = stages + 'instantiate:' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind + ':' + typeof result.instance.exports; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000020b0103656e7603666f6f0000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:27>instantiate:instantiated:1:27:1:env:foo:function:object")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### routes fetched arrayBuffer missing imports through catch

- routes fetched arrayBuffer missing imports through catch
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:52>instantiateImports:invalid:unsupported-wasm-imports`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes fetched arrayBuffer missing imports through catch")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.instantiate(bytes, {}); }).then(function(result) { out = 'unexpected:' + result.status; }).catch(function(err) { out = stages + 'instantiateImports:' + err.status + ':' + err.error; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:52>instantiateImports:invalid:unsupported-wasm-imports")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### chains fetched arrayBuffer compiled modules into WebAssembly table and global exports

- chains fetched arrayBuffer compiled modules into WebAssembly table and global exports
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:40>compile:40>instantiate:instantiated:40:table:funcref:nul... (full value in folded executable source)`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("chains fetched arrayBuffer compiled modules into WebAssembly table and global exports")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.compile(bytes); }).then(function(module) { stages = stages + 'compile:' + module.byteLength + '>'; return WebAssembly.instantiate(module); }).then(function(result) { var table = result.instance.exports.tbl; var global = result.instance.exports.answer; var before = table.get(0); var old = table.grow(1, null); out = stages + 'instantiate:' + result.status + ':' + result.module.byteLength + ':' + table.kind + ':' + table.element + ':' + before + ':' + old + ':' + table.length + ':' + global.kind + ':' + global.valueType + ':' + global.mutable + ':' + global.value; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:40>compile:40>instantiate:instantiated:40:table:funcref:null:1:2:global:i32:false:42")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### chains fetched arrayBuffer compiled modules into WebAssembly memory exports

- chains fetched arrayBuffer compiled modules into WebAssembly memory exports
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:25>compile:25>instantiate:instantiated:25:131072:65536:4:1:... (full value in folded executable source)`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("chains fetched arrayBuffer compiled modules into WebAssembly memory exports")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.compile(bytes); }).then(function(module) { stages = stages + 'compile:' + module.byteLength + '>'; return WebAssembly.instantiate(module); }).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[8] = 260; var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); out = stages + 'instantiate:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + bytes[8] + ':' + old + ':' + grown.length + ':' + grown[8]; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000503010001070a01066d656d6f72790200",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:25>compile:25>instantiate:instantiated:25:131072:65536:4:1:131072:4")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### chains fetched arrayBuffer compiled modules into multiple WebAssembly function exports

- chains fetched arrayBuffer compiled modules into multiple WebAssembly function exports
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:47>compile:47>instantiate:instantiated:47:2:function:functi... (full value in folded executable source)`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("chains fetched arrayBuffer compiled modules into multiple WebAssembly function exports")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.compile(bytes); }).then(function(module) { stages = stages + 'compile:' + module.byteLength + '>'; return WebAssembly.instantiate(module); }).then(function(result) { var exports = result.instance.exports; out = stages + 'instantiate:' + result.status + ':' + result.module.byteLength + ':' + result.module.functionExportCount + ':' + typeof exports.init + ':' + typeof exports.render + ':' + exports.init() + ':' + exports.render(); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000030302000007110204696e697400000672656e64657200010a070202000b02000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:47>compile:47>instantiate:instantiated:47:2:function:function:undefined:undefined")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### chains fetched arrayBuffer compiled modules into WebAssembly import calls

- chains fetched arrayBuffer compiled modules into WebAssembly import calls
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:52>compile:52>instantiate:instantiated:1:52:42:1`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("chains fetched arrayBuffer compiled modules into WebAssembly import calls")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; var calls = 0; var imports = { env: { foo: function(x) { calls = calls + 1; return x + 3; } } }; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.compile(bytes); }).then(function(module) { stages = stages + 'compile:' + module.byteLength + '>'; return WebAssembly.instantiate(module, imports); }).then(function(result) { out = stages + 'instantiate:' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + result.instance.exports.run(39) + ':' + calls; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:52>compile:52>instantiate:instantiated:1:52:42:1")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### routes fetched arrayBuffer compiled missing imports through catch

- routes fetched arrayBuffer compiled missing imports through catch
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:52>compile:52>compileInstantiateImports:invalid:unsupported... (full value in folded executable source)`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes fetched arrayBuffer compiled missing imports through catch")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.compile(bytes); }).then(function(module) { stages = stages + 'compile:' + module.byteLength + '>'; return WebAssembly.instantiate(module, {}); }).then(function(result) { out = 'unexpected:' + result.status; }).catch(function(err) { out = stages + 'compileInstantiateImports:' + err.status + ':' + err.error; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:52>compile:52>compileInstantiateImports:invalid:unsupported-wasm-imports")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### routes fetched invalid arrayBuffer instantiate results through catch

- routes fetched invalid arrayBuffer instantiate results through catch
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/bad.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:16>instantiateInvalid:invalid:invalid-wasm-header`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes fetched invalid arrayBuffer instantiate results through catch")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/bad.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.instantiate(bytes); }).then(function(result) { out = 'unexpected:' + result.status; }).catch(function(err) { out = stages + 'instantiateInvalid:' + err.status + ':' + err.error; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/bad.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d00000000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:16>instantiateInvalid:invalid:invalid-wasm-header")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### routes fetched arrayBuffer instantiate fetch errors through catch

- routes fetched arrayBuffer instantiate fetch errors through catch
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/down.wasm`
   - Expected: "expected fetch commit error" equals ``
   - Expected: err equals `network-down`
   - Expected: _display_js(value) equals `instantiateFetchError:network-down`
   - Expected: "unexpected js error: {js_err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 44 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes fetched arrayBuffer instantiate fetch errors through catch")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; window.fetch('/down.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { return WebAssembly.instantiate(bytes); }).then(function(result) { out = 'unexpected:' + result.status; }).catch(function(err) { out = 'instantiateFetchError:' + err; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/down.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 0,
            headers: "",
            body: "",
            error: "network-down"
        ))
        match committed:
            Ok(_):
                expect("expected fetch commit error").to_equal("")
            Err(err):
                expect(err).to_equal("network-down")
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("instantiateFetchError:network-down")
                    Err(js_err):
                        expect("unexpected js error: {js_err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### chains fetched arrayBuffer bytes into WebAssembly.compile

- chains fetched arrayBuffer bytes into WebAssembly.compile
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:11>compile:11:1:true`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("chains fetched arrayBuffer bytes into WebAssembly.compile")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.compile(bytes); }).then(function(module) { out = stages + 'compile:' + module.byteLength + ':' + module.sectionCount + ':' + module.hasTypeSection; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010100",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:11>compile:11:1:true")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### exposes fetched arrayBuffer compiled module export descriptors

- exposes fetched arrayBuffer compiled module export descriptors
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:40>compile:40:2:tbl:table:answer:global`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes fetched arrayBuffer compiled module export descriptors")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.compile(bytes); }).then(function(module) { var exports = WebAssembly.Module.exports(module); out = stages + 'compile:' + module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:40>compile:40:2:tbl:table:answer:global")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### exposes fetched arrayBuffer compiled module import descriptors

- exposes fetched arrayBuffer compiled module import descriptors
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:27>compile:1:27:1:env:foo:function`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes fetched arrayBuffer compiled module import descriptors")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.compile(bytes); }).then(function(module) { var descriptors = WebAssembly.Module.imports(module); out = stages + 'compile:' + module.importCount + ':' + module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000020b0103656e7603666f6f0000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:27>compile:1:27:1:env:foo:function")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### routes fetched invalid arrayBuffer compile results through catch

- routes fetched invalid arrayBuffer compile results through catch
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/bad.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:16>compileInvalid:invalid:invalid-wasm-header`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes fetched invalid arrayBuffer compile results through catch")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/bad.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.compile(bytes); }).then(function(module) { out = 'unexpected:' + module.byteLength; }).catch(function(err) { out = stages + 'compileInvalid:' + err.status + ':' + err.error; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/bad.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d00000000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:16>compileInvalid:invalid:invalid-wasm-header")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### routes fetched arrayBuffer compile fetch errors through catch

- routes fetched arrayBuffer compile fetch errors through catch
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/down.wasm`
   - Expected: "expected fetch commit error" equals ``
   - Expected: err equals `network-down`
   - Expected: _display_js(value) equals `compileFetchError:network-down`
   - Expected: "unexpected js error: {js_err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 44 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes fetched arrayBuffer compile fetch errors through catch")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; window.fetch('/down.wasm').then(function(r) { return r.arrayBuffer(); }).then(function(bytes) { return WebAssembly.compile(bytes); }).then(function(module) { out = 'unexpected:' + module.byteLength; }).catch(function(err) { out = 'compileFetchError:' + err; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/down.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 0,
            headers: "",
            body: "",
            error: "network-down"
        ))
        match committed:
            Ok(_):
                expect("expected fetch commit error").to_equal("")
            Err(err):
                expect(err).to_equal("network-down")
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileFetchError:network-down")
                    Err(js_err):
                        expect("unexpected js error: {js_err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### chains fetched arrayBuffer bytes through WebAssembly.compile into function body calls

- chains fetched arrayBuffer bytes through WebAssembly.compile into function body calls
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:41>compile:41>instantiate:instantiated:41:function:42:42`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("chains fetched arrayBuffer bytes through WebAssembly.compile into function body calls")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.compile(bytes); }).then(function(module) { stages = stages + 'compile:' + module.byteLength + '>'; return WebAssembly.instantiate(module); }).then(function(result) { out = stages + 'instantiate:' + result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports.run + ':' + result.instance.exports.run(40, 2) + ':' + result.instance.exports.run(7, 35); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001070160027f7f017f030201000707010372756e00000a09010700200020016a0b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:41>compile:41>instantiate:instantiated:41:function:42:42")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### routes invalid WebAssembly.compile results through catch in browser scripts

- routes invalid WebAssembly.compile results through catch in browser scripts
   - Expected: _display_js(value) equals `invalid:invalid-wasm-header:8:0`
   - Expected: "unexpected wasm compile catch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes invalid WebAssembly.compile results through catch in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; WebAssembly.compile('0061736d00000000').catch(function(err) { out = err.status + ':' + err.error; }); var valid = WebAssembly.compile('0061736d01000000'); valid.then(function(module) { out = out + ':' + module.byteLength + ':' + module.sectionCount; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("invalid:invalid-wasm-header:8:0")
    Err(err):
        expect("unexpected wasm compile catch js error: {err}").to_equal("")
```

</details>

#### routes invalid WebAssembly.instantiate results through catch in browser scripts

- routes invalid WebAssembly.instantiate results through catch in browser scripts
   - Expected: _display_js(value) equals `invalid:invalid-wasm-header:invalid:unsupported-wasm-imports:instantiated:8`
   - Expected: "unexpected wasm instantiate catch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes invalid WebAssembly.instantiate results through catch in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; WebAssembly.instantiate('0061736d00000000').catch(function(err) { out = err.status + ':' + err.error; }); var imports = {}; WebAssembly.instantiate('0061736d01000000010401600000020b0103656e7603666f6f0000', imports).catch(function(err) { out = out + ':' + err.status + ':' + err.error; }); WebAssembly.instantiate('0061736d01000000').then(function(result) { out = out + ':' + result.status + ':' + result.module.byteLength; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("invalid:invalid-wasm-header:invalid:unsupported-wasm-imports:instantiated:8")
    Err(err):
        expect("unexpected wasm instantiate catch js error: {err}").to_equal("")
```

</details>

#### routes truncated WebAssembly section results through catch in browser scripts

- routes truncated WebAssembly section results through catch in browser scripts
   - Expected: _display_js(value) equals `compileSection:invalid:invalid-wasm-section:instantiateSection:invalid:invali... (full value in folded executable source)`
   - Expected: "unexpected wasm truncated section catch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes truncated WebAssembly section results through catch in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; WebAssembly.compile('0061736d010000000105').catch(function(err) { out = 'compileSection:' + err.status + ':' + err.error; }); WebAssembly.instantiate('0061736d010000000105').catch(function(err) { out = out + ':instantiateSection:' + err.status + ':' + err.error; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("compileSection:invalid:invalid-wasm-section:instantiateSection:invalid:invalid-wasm-section")
    Err(err):
        expect("unexpected wasm truncated section catch js error: {err}").to_equal("")
```

</details>

#### normalizes decorated WebAssembly hex strings in browser scripts

- normalizes decorated WebAssembly hex strings in browser scripts
   - Expected: _display_js(value) equals `true:true:8:0:wasm32`
   - Expected: "unexpected decorated wasm hex js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("normalizes decorated WebAssembly hex strings in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var decorated = '0x0061_736d 0100_0000'; var module = new WebAssembly.Module(decorated); WebAssembly.validate(decorated) + ':' + module.validated + ':' + module.byteLength + ':' + module.sectionCount + ':' + module.target")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:true:8:0:wasm32")
    Err(err):
        expect("unexpected decorated wasm hex js error: {err}").to_equal("")
```

</details>

#### normalizes prefixless decorated WebAssembly hex strings in browser scripts

- normalizes prefixless decorated WebAssembly hex strings in browser scripts
   - Expected: _display_js(value) equals `true:true:8:0:wasm32`
   - Expected: "unexpected prefixless decorated wasm hex js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("normalizes prefixless decorated WebAssembly hex strings in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var decorated = '0061_736d 0100_0000'; var module = new WebAssembly.Module(decorated); WebAssembly.validate(decorated) + ':' + module.validated + ':' + module.byteLength + ':' + module.sectionCount + ':' + module.target")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:true:8:0:wasm32")
    Err(err):
        expect("unexpected prefixless decorated wasm hex js error: {err}").to_equal("")
```

</details>

#### routes prefixless decorated WebAssembly hex promise results through browser scripts

- routes prefixless decorated WebAssembly hex promise results through browser scripts
   - Expected: _display_js(value) equals `compile:true:8:0:wasm32:instantiate:instantiated:true:8:0:object`
   - Expected: "unexpected prefixless decorated wasm promise js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes prefixless decorated WebAssembly hex promise results through browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var decorated = '0061_736d 0100_0000'; WebAssembly.compile(decorated).then(function(module) { out = 'compile:' + module.validated + ':' + module.byteLength + ':' + module.sectionCount + ':' + module.target; }); WebAssembly.instantiate(decorated).then(function(result) { out = out + ':instantiate:' + result.status + ':' + result.module.validated + ':' + result.module.byteLength + ':' + result.module.sectionCount + ':' + typeof result.instance.exports; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("compile:true:8:0:wasm32:instantiate:instantiated:true:8:0:object")
    Err(err):
        expect("unexpected prefixless decorated wasm promise js error: {err}").to_equal("")
```

</details>

#### compiles decorated WebAssembly hex strings in browser scripts

- compiles decorated WebAssembly hex strings in browser scripts
   - Expected: _display_js(value) equals `true:8:0:wasm32`
   - Expected: "unexpected decorated wasm compile js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compiles decorated WebAssembly hex strings in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var decorated = '0x0061_736d 0100_0000'; WebAssembly.compile(decorated).then(function(module) { out = module.validated + ':' + module.byteLength + ':' + module.sectionCount + ':' + module.target; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:8:0:wasm32")
    Err(err):
        expect("unexpected decorated wasm compile js error: {err}").to_equal("")
```

</details>

#### instantiates decorated WebAssembly hex strings in browser scripts

- instantiates decorated WebAssembly hex strings in browser scripts
   - Expected: _display_js(value) equals `instantiated:true:8:0:object`
   - Expected: "unexpected decorated wasm instantiate js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates decorated WebAssembly hex strings in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var decorated = '0x0061_736d 0100_0000'; WebAssembly.instantiate(decorated).then(function(result) { out = result.status + ':' + result.module.validated + ':' + result.module.byteLength + ':' + result.module.sectionCount + ':' + typeof result.instance.exports; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("instantiated:true:8:0:object")
    Err(err):
        expect("unexpected decorated wasm instantiate js error: {err}").to_equal("")
```

</details>

#### routes decorated WebAssembly hex promise results through browser scripts

- routes decorated WebAssembly hex promise results through browser scripts
   - Expected: _display_js(value) equals `compile:true:8:0:wasm32:instantiate:instantiated:true:8:0:object`
   - Expected: "unexpected decorated wasm promise js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes decorated WebAssembly hex promise results through browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var decorated = '0x0061_736d 0100_0000'; WebAssembly.compile(decorated).then(function(module) { out = 'compile:' + module.validated + ':' + module.byteLength + ':' + module.sectionCount + ':' + module.target; }); WebAssembly.instantiate(decorated).then(function(result) { out = out + ':instantiate:' + result.status + ':' + result.module.validated + ':' + result.module.byteLength + ':' + result.module.sectionCount + ':' + typeof result.instance.exports; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("compile:true:8:0:wasm32:instantiate:instantiated:true:8:0:object")
    Err(err):
        expect("unexpected decorated wasm promise js error: {err}").to_equal("")
```

</details>

#### rejects decorated WebAssembly hex strings with non-hex characters

- rejects decorated WebAssembly hex strings with non-hex characters
   - Expected: _display_js(value) equals `false:false:invalid-wasm-header:0:0`
   - Expected: "unexpected invalid decorated wasm hex js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("rejects decorated WebAssembly hex strings with non-hex characters")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var decorated = '0x0061_736d 0100_000g'; var module = new WebAssembly.Module(decorated); WebAssembly.validate(decorated) + ':' + module.validated + ':' + module.error + ':' + module.byteLength + ':' + module.sectionCount")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("false:false:invalid-wasm-header:0:0")
    Err(err):
        expect("unexpected invalid decorated wasm hex js error: {err}").to_equal("")
```

</details>

#### routes invalid decorated WebAssembly hex promise results through catch in browser scripts

- routes invalid decorated WebAssembly hex promise results through catch in browser scripts
   - Expected: _display_js(value) equals `compile:invalid:invalid-wasm-header:instantiate:invalid:invalid-wasm-header`
   - Expected: "unexpected invalid decorated wasm promise js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes invalid decorated WebAssembly hex promise results through catch in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var decorated = '0x0061_736d 0100_000g'; WebAssembly.compile(decorated).catch(function(err) { out = 'compile:' + err.status + ':' + err.error; }); WebAssembly.instantiate(decorated).catch(function(err) { out = out + ':instantiate:' + err.status + ':' + err.error; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("compile:invalid:invalid-wasm-header:instantiate:invalid:invalid-wasm-header")
    Err(err):
        expect("unexpected invalid decorated wasm promise js error: {err}").to_equal("")
```

</details>

#### rejects separator-only decorated WebAssembly hex strings in browser scripts

- rejects separator-only decorated WebAssembly hex strings in browser scripts
   - Expected: _display_js(value) equals `false:false:invalid-wasm-header:0:0`
   - Expected: "unexpected separator-only decorated wasm hex js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("rejects separator-only decorated WebAssembly hex strings in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var decorated = '  \\t0x__ \\n\\r  '; var module = new WebAssembly.Module(decorated); WebAssembly.validate(decorated) + ':' + module.validated + ':' + module.error + ':' + module.byteLength + ':' + module.sectionCount")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("false:false:invalid-wasm-header:0:0")
    Err(err):
        expect("unexpected separator-only decorated wasm hex js error: {err}").to_equal("")
```

</details>

#### routes separator-only decorated WebAssembly hex promise results through catch in browser scripts

- routes separator-only decorated WebAssembly hex promise results through catch in browser scripts
   - Expected: _display_js(value) equals `compile:invalid:invalid-wasm-header:instantiate:invalid:invalid-wasm-header`
   - Expected: "unexpected separator-only decorated wasm promise js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes separator-only decorated WebAssembly hex promise results through catch in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var decorated = '  \\t0x__ \\n\\r  '; WebAssembly.compile(decorated).catch(function(err) { out = 'compile:' + err.status + ':' + err.error; }); WebAssembly.instantiate(decorated).catch(function(err) { out = out + ':instantiate:' + err.status + ':' + err.error; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("compile:invalid:invalid-wasm-header:instantiate:invalid:invalid-wasm-header")
    Err(err):
        expect("unexpected separator-only decorated wasm promise js error: {err}").to_equal("")
```

</details>

#### rejects odd-length decorated WebAssembly hex strings in browser scripts

- rejects odd-length decorated WebAssembly hex strings in browser scripts
   - Expected: _display_js(value) equals `false:false:invalid-wasm-header:8:0`
   - Expected: "unexpected odd decorated wasm hex js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("rejects odd-length decorated WebAssembly hex strings in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var decorated = '0x0061_736d 0100_0000_0'; var module = new WebAssembly.Module(decorated); WebAssembly.validate(decorated) + ':' + module.validated + ':' + module.error + ':' + module.byteLength + ':' + module.sectionCount")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("false:false:invalid-wasm-header:8:0")
    Err(err):
        expect("unexpected odd decorated wasm hex js error: {err}").to_equal("")
```

</details>

#### routes odd-length decorated WebAssembly hex promise results through catch in browser scripts

- routes odd-length decorated WebAssembly hex promise results through catch in browser scripts
   - Expected: _display_js(value) equals `compile:invalid:invalid-wasm-header:instantiate:invalid:invalid-wasm-header`
   - Expected: "unexpected odd decorated wasm promise js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes odd-length decorated WebAssembly hex promise results through catch in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var decorated = '0x0061_736d 0100_0000_0'; WebAssembly.compile(decorated).catch(function(err) { out = 'compile:' + err.status + ':' + err.error; }); WebAssembly.instantiate(decorated).catch(function(err) { out = out + ':instantiate:' + err.status + ':' + err.error; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("compile:invalid:invalid-wasm-header:instantiate:invalid:invalid-wasm-header")
    Err(err):
        expect("unexpected odd decorated wasm promise js error: {err}").to_equal("")
```

</details>

#### normalizes uppercase decorated WebAssembly hex strings in browser scripts

- normalizes uppercase decorated WebAssembly hex strings in browser scripts
   - Expected: _display_js(value) equals `true:true:8:0:wasm32`
   - Expected: "unexpected uppercase decorated wasm hex js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("normalizes uppercase decorated WebAssembly hex strings in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var decorated = '0X0061_736D 0100_0000'; var module = new WebAssembly.Module(decorated); WebAssembly.validate(decorated) + ':' + module.validated + ':' + module.byteLength + ':' + module.sectionCount + ':' + module.target")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:true:8:0:wasm32")
    Err(err):
        expect("unexpected uppercase decorated wasm hex js error: {err}").to_equal("")
```

</details>

#### routes uppercase decorated WebAssembly hex promise results through browser scripts

- routes uppercase decorated WebAssembly hex promise results through browser scripts
   - Expected: _display_js(value) equals `compile:true:8:0:wasm32:instantiate:instantiated:true:8:0:object`
   - Expected: "unexpected uppercase decorated wasm promise js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes uppercase decorated WebAssembly hex promise results through browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var decorated = '0X0061_736D 0100_0000'; WebAssembly.compile(decorated).then(function(module) { out = 'compile:' + module.validated + ':' + module.byteLength + ':' + module.sectionCount + ':' + module.target; }); WebAssembly.instantiate(decorated).then(function(result) { out = out + ':instantiate:' + result.status + ':' + result.module.validated + ':' + result.module.byteLength + ':' + result.module.sectionCount + ':' + typeof result.instance.exports; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("compile:true:8:0:wasm32:instantiate:instantiated:true:8:0:object")
    Err(err):
        expect("unexpected uppercase decorated wasm promise js error: {err}").to_equal("")
```

</details>

#### normalizes control whitespace decorated WebAssembly hex strings in browser scripts

- normalizes control whitespace decorated WebAssembly hex strings in browser scripts
   - Expected: _display_js(value) equals `true:true:8:0:wasm32`
   - Expected: "unexpected control whitespace decorated wasm hex js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("normalizes control whitespace decorated WebAssembly hex strings in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var decorated = '0x0061\\n736d\\t0100\\r0000'; var module = new WebAssembly.Module(decorated); WebAssembly.validate(decorated) + ':' + module.validated + ':' + module.byteLength + ':' + module.sectionCount + ':' + module.target")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:true:8:0:wasm32")
    Err(err):
        expect("unexpected control whitespace decorated wasm hex js error: {err}").to_equal("")
```

</details>

#### routes control whitespace decorated WebAssembly hex promise results through browser scripts

- routes control whitespace decorated WebAssembly hex promise results through browser scripts
   - Expected: _display_js(value) equals `compile:true:8:0:wasm32:instantiate:instantiated:true:8:0:object`
   - Expected: "unexpected control whitespace decorated wasm promise js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes control whitespace decorated WebAssembly hex promise results through browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var decorated = '0x0061\\n736d\\t0100\\r0000'; WebAssembly.compile(decorated).then(function(module) { out = 'compile:' + module.validated + ':' + module.byteLength + ':' + module.sectionCount + ':' + module.target; }); WebAssembly.instantiate(decorated).then(function(result) { out = out + ':instantiate:' + result.status + ':' + result.module.validated + ':' + result.module.byteLength + ':' + result.module.sectionCount + ':' + typeof result.instance.exports; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("compile:true:8:0:wasm32:instantiate:instantiated:true:8:0:object")
    Err(err):
        expect("unexpected control whitespace decorated wasm promise js error: {err}").to_equal("")
```

</details>

#### normalizes padded decorated WebAssembly hex strings in browser scripts

- normalizes padded decorated WebAssembly hex strings in browser scripts
   - Expected: _display_js(value) equals `true:true:8:0:wasm32`
   - Expected: "unexpected padded decorated wasm hex js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("normalizes padded decorated WebAssembly hex strings in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var decorated = '  \\t0x0061_736d 0100_0000\\n  '; var module = new WebAssembly.Module(decorated); WebAssembly.validate(decorated) + ':' + module.validated + ':' + module.byteLength + ':' + module.sectionCount + ':' + module.target")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:true:8:0:wasm32")
    Err(err):
        expect("unexpected padded decorated wasm hex js error: {err}").to_equal("")
```

</details>

#### routes padded decorated WebAssembly hex promise results through browser scripts

- routes padded decorated WebAssembly hex promise results through browser scripts
   - Expected: _display_js(value) equals `compile:true:8:0:wasm32:instantiate:instantiated:true:8:0:object`
   - Expected: "unexpected padded decorated wasm promise js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes padded decorated WebAssembly hex promise results through browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var decorated = '  \\t0x0061_736d 0100_0000\\n  '; WebAssembly.compile(decorated).then(function(module) { out = 'compile:' + module.validated + ':' + module.byteLength + ':' + module.sectionCount + ':' + module.target; }); WebAssembly.instantiate(decorated).then(function(result) { out = out + ':instantiate:' + result.status + ':' + result.module.validated + ':' + result.module.byteLength + ':' + result.module.sectionCount + ':' + typeof result.instance.exports; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("compile:true:8:0:wasm32:instantiate:instantiated:true:8:0:object")
    Err(err):
        expect("unexpected padded decorated wasm promise js error: {err}").to_equal("")
```

</details>

#### exposes instantiated WebAssembly module export descriptors in browser scripts

- exposes instantiated WebAssembly module export descriptors in browser scripts
   - Expected: _display_js(value) equals `instantiated:40:2:tbl:table:answer:global`
   - Expected: "unexpected instantiated descriptor js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes instantiated WebAssembly module export descriptors in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; WebAssembly.instantiate('0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300').then(function(result) { var exports = WebAssembly.Module.exports(result.module); out = result.status + ':' + result.module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("instantiated:40:2:tbl:table:answer:global")
    Err(err):
        expect("unexpected instantiated descriptor js error: {err}").to_equal("")
```

</details>

#### exposes compiled WebAssembly module export descriptors in browser scripts

- exposes compiled WebAssembly module export descriptors in browser scripts
   - Expected: _display_js(value) equals `40:2:tbl:table:answer:global`
   - Expected: "unexpected compiled export descriptor js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes compiled WebAssembly module export descriptors in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; WebAssembly.compile('0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300').then(function(module) { var exports = WebAssembly.Module.exports(module); out = module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("40:2:tbl:table:answer:global")
    Err(err):
        expect("unexpected compiled export descriptor js error: {err}").to_equal("")
```

</details>

#### compares compiled and instantiated WebAssembly module export descriptors in browser scripts

- compares compiled and instantiated WebAssembly module export descriptors in browser scripts
   - Expected: _display_js(value) equals `compile:40:2:tbl:table:answer:global:instantiate:instantiated:40:2:tbl:table:... (full value in folded executable source)`
   - Expected: "unexpected combined export descriptor js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compiled and instantiated WebAssembly module export descriptors in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var wasm = '0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300'; WebAssembly.compile(wasm).then(function(module) { var exports = WebAssembly.Module.exports(module); out = 'compile:' + module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); WebAssembly.instantiate(wasm).then(function(result) { var exports = WebAssembly.Module.exports(result.module); out = out + ':instantiate:' + result.status + ':' + result.module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("compile:40:2:tbl:table:answer:global:instantiate:instantiated:40:2:tbl:table:answer:global")
    Err(err):
        expect("unexpected combined export descriptor js error: {err}").to_equal("")
```

</details>

#### exposes instantiated WebAssembly module import descriptors in browser scripts

- exposes instantiated WebAssembly module import descriptors in browser scripts
   - Expected: _display_js(value) equals `instantiated:1:1:env:foo:function:object`
   - Expected: "unexpected instantiated import descriptor js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes instantiated WebAssembly module import descriptors in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var imports = { env: { foo: function() { return 7; } } }; WebAssembly.instantiate('0061736d01000000010401600000020b0103656e7603666f6f0000', imports).then(function(result) { var descriptors = WebAssembly.Module.imports(result.module); out = result.status + ':' + result.module.importCount + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind + ':' + typeof result.instance.exports; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("instantiated:1:1:env:foo:function:object")
    Err(err):
        expect("unexpected instantiated import descriptor js error: {err}").to_equal("")
```

</details>

#### exposes compiled WebAssembly module import descriptors in browser scripts

- exposes compiled WebAssembly module import descriptors in browser scripts
   - Expected: _display_js(value) equals `1:27:1:env:foo:function`
   - Expected: "unexpected compiled import descriptor js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes compiled WebAssembly module import descriptors in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; WebAssembly.compile('0061736d01000000010401600000020b0103656e7603666f6f0000').then(function(module) { var descriptors = WebAssembly.Module.imports(module); out = module.importCount + ':' + module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1:27:1:env:foo:function")
    Err(err):
        expect("unexpected compiled import descriptor js error: {err}").to_equal("")
```

</details>

#### compares compiled and instantiated WebAssembly module import descriptors in browser scripts

- compares compiled and instantiated WebAssembly module import descriptors in browser scripts
   - Expected: _display_js(value) equals `compile:1:27:1:env:foo:function:instantiate:instantiated:1:27:1:env:foo:funct... (full value in folded executable source)`
   - Expected: "unexpected combined import descriptor js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compiled and instantiated WebAssembly module import descriptors in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var wasm = '0061736d01000000010401600000020b0103656e7603666f6f0000'; var imports = { env: { foo: function() { return 7; } } }; WebAssembly.compile(wasm).then(function(module) { var descriptors = WebAssembly.Module.imports(module); out = 'compile:' + module.importCount + ':' + module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind; }); WebAssembly.instantiate(wasm, imports).then(function(result) { var descriptors = WebAssembly.Module.imports(result.module); out = out + ':instantiate:' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind + ':' + typeof result.instance.exports; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("compile:1:27:1:env:foo:function:instantiate:instantiated:1:27:1:env:foo:function:object")
    Err(err):
        expect("unexpected combined import descriptor js error: {err}").to_equal("")
```

</details>

#### invokes direct WebAssembly instantiate imports through exported functions

- invokes direct WebAssembly instantiate imports through exported functions
   - Expected: _display_js(value) equals `instantiated:1:42:1`
   - Expected: "unexpected direct instantiate import call js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("invokes direct WebAssembly instantiate imports through exported functions")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var calls = 0; var imports = { env: { foo: function(x) { calls = calls + 1; return x + 2; } } }; WebAssembly.instantiate('0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b', imports).then(function(result) { out = result.status + ':' + result.module.importCount + ':' + result.instance.exports.run(40) + ':' + calls; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("instantiated:1:42:1")
    Err(err):
        expect("unexpected direct instantiate import call js error: {err}").to_equal("")
```

</details>

#### instantiates compiled WebAssembly imports through exported functions

- instantiates compiled WebAssembly imports through exported functions
   - Expected: _display_js(value) equals `instantiated:1:52:42:1`
   - Expected: "unexpected compiled instantiate import call js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates compiled WebAssembly imports through exported functions")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var calls = 0; var imports = { env: { foo: function(x) { calls = calls + 1; return x + 3; } } }; WebAssembly.compile('0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b').then(function(module) { return WebAssembly.instantiate(module, imports); }).then(function(result) { out = result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + result.instance.exports.run(39) + ':' + calls; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("instantiated:1:52:42:1")
    Err(err):
        expect("unexpected compiled instantiate import call js error: {err}").to_equal("")
```

</details>

#### compares direct and compiled WebAssembly import calls in browser scripts

- compares direct and compiled WebAssembly import calls in browser scripts
   - Expected: _display_js(value) equals `direct:instantiated:1:42:1:compiled:instantiated:1:52:42:1`
   - Expected: "unexpected combined import call js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares direct and compiled WebAssembly import calls in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var wasm = '0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b'; var directCalls = 0; var compiledCalls = 0; var directImports = { env: { foo: function(x) { directCalls = directCalls + 1; return x + 2; } } }; var compiledImports = { env: { foo: function(x) { compiledCalls = compiledCalls + 1; return x + 3; } } }; WebAssembly.instantiate(wasm, directImports).then(function(result) { out = 'direct:' + result.status + ':' + result.module.importCount + ':' + result.instance.exports.run(40) + ':' + directCalls; }); WebAssembly.compile(wasm).then(function(module) { return WebAssembly.instantiate(module, compiledImports); }).then(function(result) { out = out + ':compiled:' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + result.instance.exports.run(39) + ':' + compiledCalls; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("direct:instantiated:1:42:1:compiled:instantiated:1:52:42:1")
    Err(err):
        expect("unexpected combined import call js error: {err}").to_equal("")
```

</details>

#### instantiates compiled WebAssembly function export bodies with call arguments

- instantiates compiled WebAssembly function export bodies with call arguments
   - Expected: _display_js(value) equals `instantiated:41:function:42:42`
   - Expected: "unexpected compiled function export body argument js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates compiled WebAssembly function export bodies with call arguments")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; WebAssembly.compile('0061736d0100000001070160027f7f017f030201000707010372756e00000a09010700200020016a0b').then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { out = result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports.run + ':' + result.instance.exports.run(40, 2) + ':' + result.instance.exports.run(7, 35); }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("instantiated:41:function:42:42")
    Err(err):
        expect("unexpected compiled function export body argument js error: {err}").to_equal("")
```

</details>

#### constructs WebAssembly Instance imports through exported functions

- constructs WebAssembly Instance imports through exported functions
   - Expected: _display_js(value) equals `1:52:instantiated:function:42:1`
   - Expected: "unexpected instance constructor import call js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("constructs WebAssembly Instance imports through exported functions")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var calls = 0; var imports = { env: { foo: function(x) { calls = calls + 1; return x + 5; } } }; var module = new WebAssembly.Module('0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b'); var instance = new WebAssembly.Instance(module, imports); module.importCount + ':' + module.byteLength + ':' + instance.status + ':' + typeof instance.exports.run + ':' + instance.exports.run(37) + ':' + calls")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1:52:instantiated:function:42:1")
    Err(err):
        expect("unexpected instance constructor import call js error: {err}").to_equal("")
```

</details>

#### compares constructor and compiled WebAssembly import calls in browser scripts

- compares constructor and compiled WebAssembly import calls in browser scripts
   - Expected: _display_js(value) equals `ctor:1:52:instantiated:function:42:1:compiled:instantiated:1:52:42:1`
   - Expected: "unexpected constructor/compiled import call js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares constructor and compiled WebAssembly import calls in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var wasm = '0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b'; var ctorCalls = 0; var compiledCalls = 0; var ctorImports = { env: { foo: function(x) { ctorCalls = ctorCalls + 1; return x + 5; } } }; var compiledImports = { env: { foo: function(x) { compiledCalls = compiledCalls + 1; return x + 3; } } }; var module = new WebAssembly.Module(wasm); var instance = new WebAssembly.Instance(module, ctorImports); out = 'ctor:' + module.importCount + ':' + module.byteLength + ':' + instance.status + ':' + typeof instance.exports.run + ':' + instance.exports.run(37) + ':' + ctorCalls; WebAssembly.compile(wasm).then(function(module) { return WebAssembly.instantiate(module, compiledImports); }).then(function(result) { out = out + ':compiled:' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + result.instance.exports.run(39) + ':' + compiledCalls; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("ctor:1:52:instantiated:function:42:1:compiled:instantiated:1:52:42:1")
    Err(err):
        expect("unexpected constructor/compiled import call js error: {err}").to_equal("")
```

</details>

#### constructs WebAssembly Instance function export bodies with call arguments

- constructs WebAssembly Instance function export bodies with call arguments
   - Expected: _display_js(value) equals `41:instantiated:function:42:42`
   - Expected: "unexpected instance constructor function export body argument js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("constructs WebAssembly Instance function export bodies with call arguments")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var module = new WebAssembly.Module('0061736d0100000001070160027f7f017f030201000707010372756e00000a09010700200020016a0b'); var instance = new WebAssembly.Instance(module); module.byteLength + ':' + instance.status + ':' + typeof instance.exports.run + ':' + instance.exports.run(40, 2) + ':' + instance.exports.run(7, 35)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("41:instantiated:function:42:42")
    Err(err):
        expect("unexpected instance constructor function export body argument js error: {err}").to_equal("")
```

</details>

#### compares constructor and compiled WebAssembly function export bodies in browser scripts

- compares constructor and compiled WebAssembly function export bodies in browser scripts
   - Expected: _display_js(value) equals `ctor:41:instantiated:function:42:42:compiled:instantiated:41:function:42:42`
   - Expected: "unexpected constructor/compiled function export body js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares constructor and compiled WebAssembly function export bodies in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var wasm = '0061736d0100000001070160027f7f017f030201000707010372756e00000a09010700200020016a0b'; var module = new WebAssembly.Module(wasm); var instance = new WebAssembly.Instance(module); out = 'ctor:' + module.byteLength + ':' + instance.status + ':' + typeof instance.exports.run + ':' + instance.exports.run(40, 2) + ':' + instance.exports.run(7, 35); WebAssembly.compile(wasm).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { out = out + ':compiled:' + result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports.run + ':' + result.instance.exports.run(40, 2) + ':' + result.instance.exports.run(7, 35); }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("ctor:41:instantiated:function:42:42:compiled:instantiated:41:function:42:42")
    Err(err):
        expect("unexpected constructor/compiled function export body js error: {err}").to_equal("")
```

</details>

#### constructs WebAssembly Instance table and global exports in browser scripts

- constructs WebAssembly Instance table and global exports in browser scripts
   - Expected: _display_js(value) equals `40:instantiated:table:funcref:null:1:2:global:i32:false:42`
   - Expected: "unexpected instance constructor table/global export js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("constructs WebAssembly Instance table and global exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var module = new WebAssembly.Module('0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300'); var instance = new WebAssembly.Instance(module); var table = instance.exports.tbl; var global = instance.exports.answer; var before = table.get(0); var old = table.grow(1, null); module.byteLength + ':' + instance.status + ':' + table.kind + ':' + table.element + ':' + before + ':' + old + ':' + table.length + ':' + global.kind + ':' + global.valueType + ':' + global.mutable + ':' + global.value")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("40:instantiated:table:funcref:null:1:2:global:i32:false:42")
    Err(err):
        expect("unexpected instance constructor table/global export js error: {err}").to_equal("")
```

</details>

#### compares constructor and compiled WebAssembly table and global exports in browser scripts

- compares constructor and compiled WebAssembly table and global exports in browser scripts
   - Expected: _display_js(value) equals `ctor:40:instantiated:table:funcref:null:1:2:global:i32:false:42:compiled:inst... (full value in folded executable source)`
   - Expected: "unexpected constructor/compiled table/global export js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares constructor and compiled WebAssembly table and global exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var wasm = '0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300'; var module = new WebAssembly.Module(wasm); var instance = new WebAssembly.Instance(module); var table = instance.exports.tbl; var global = instance.exports.answer; var before = table.get(0); var old = table.grow(1, null); out = 'ctor:' + module.byteLength + ':' + instance.status + ':' + table.kind + ':' + table.element + ':' + before + ':' + old + ':' + table.length + ':' + global.kind + ':' + global.valueType + ':' + global.mutable + ':' + global.value; WebAssembly.compile(wasm).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { var table = result.instance.exports.tbl; var global = result.instance.exports.answer; var before = table.get(0); var old = table.grow(1, null); out = out + ':compiled:' + result.status + ':' + result.module.byteLength + ':' + table.kind + ':' + table.element + ':' + before + ':' + old + ':' + table.length + ':' + global.kind + ':' + global.valueType + ':' + global.mutable + ':' + global.value; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("ctor:40:instantiated:table:funcref:null:1:2:global:i32:false:42:compiled:instantiated:40:table:funcref:null:1:2:global:i32:false:42")
    Err(err):
        expect("unexpected constructor/compiled table/global export js error: {err}").to_equal("")
```

</details>

#### constructs WebAssembly Instance memory exports in browser scripts

- constructs WebAssembly Instance memory exports in browser scripts
   - Expected: _display_js(value) equals `25:instantiated:131072:65536:4:1:131072:4`
   - Expected: "unexpected instance constructor memory export js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("constructs WebAssembly Instance memory exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var module = new WebAssembly.Module('0061736d010000000503010001070a01066d656d6f72790200'); var instance = new WebAssembly.Instance(module); var memory = instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[4] = 260; var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); module.byteLength + ':' + instance.status + ':' + memory.byteLength + ':' + memory.pageSize + ':' + bytes[4] + ':' + old + ':' + grown.length + ':' + grown[4]")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("25:instantiated:131072:65536:4:1:131072:4")
    Err(err):
        expect("unexpected instance constructor memory export js error: {err}").to_equal("")
```

</details>

#### compares constructor and compiled WebAssembly memory exports in browser scripts

- compares constructor and compiled WebAssembly memory exports in browser scripts
   - Expected: _display_js(value) equals `ctor:25:instantiated:131072:65536:4:1:131072:4:compiled:instantiated:25:13107... (full value in folded executable source)`
   - Expected: "unexpected constructor/compiled memory export js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares constructor and compiled WebAssembly memory exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var wasm = '0061736d010000000503010001070a01066d656d6f72790200'; var module = new WebAssembly.Module(wasm); var instance = new WebAssembly.Instance(module); var memory = instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[6] = 260; var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); out = 'ctor:' + module.byteLength + ':' + instance.status + ':' + memory.byteLength + ':' + memory.pageSize + ':' + bytes[6] + ':' + old + ':' + grown.length + ':' + grown[6]; WebAssembly.compile(wasm).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[7] = 260; var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); out = out + ':compiled:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + bytes[7] + ':' + old + ':' + grown.length + ':' + grown[7]; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("ctor:25:instantiated:131072:65536:4:1:131072:4:compiled:instantiated:25:131072:65536:4:1:131072:4")
    Err(err):
        expect("unexpected constructor/compiled memory export js error: {err}").to_equal("")
```

</details>

#### compares constructor and compiled WebAssembly memory maximum failures in browser scripts

- compares constructor and compiled WebAssembly memory maximum failures in browser scripts
   - Expected: _display_js(value) equals `ctor:26:instantiated:65536:65536:1:-1:65536:9:compiled:instantiated:26:65536:... (full value in folded executable source)`
   - Expected: "unexpected constructor/compiled memory maximum failure js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares constructor and compiled WebAssembly memory maximum failures in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var wasm = '0061736d01000000050401010101070a01066d656d6f72790200'; var module = new WebAssembly.Module(wasm); var instance = new WebAssembly.Instance(module); var memory = instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[8] = 265; var zero = memory.grow(0); var fail = memory.grow(1); var after = new Uint8Array(memory.buffer); out = 'ctor:' + module.byteLength + ':' + instance.status + ':' + memory.byteLength + ':' + memory.pageSize + ':' + zero + ':' + fail + ':' + after.length + ':' + after[8]; WebAssembly.compile(wasm).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[9] = 267; var zero = memory.grow(0); var fail = memory.grow(1); var after = new Uint8Array(memory.buffer); out = out + ':compiled:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + zero + ':' + fail + ':' + after.length + ':' + after[9]; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("ctor:26:instantiated:65536:65536:1:-1:65536:9:compiled:instantiated:26:65536:65536:1:-1:65536:11")
    Err(err):
        expect("unexpected constructor/compiled memory maximum failure js error: {err}").to_equal("")
```

</details>

#### constructs WebAssembly Instance multiple function exports in browser scripts

- constructs WebAssembly Instance multiple function exports in browser scripts
   - Expected: _display_js(value) equals `47:2:instantiated:function:function:undefined:undefined`
   - Expected: "unexpected instance constructor multiple function export js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("constructs WebAssembly Instance multiple function exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var module = new WebAssembly.Module('0061736d01000000010401600000030302000007110204696e697400000672656e64657200010a070202000b02000b'); var instance = new WebAssembly.Instance(module); var exports = instance.exports; module.byteLength + ':' + module.functionExportCount + ':' + instance.status + ':' + typeof exports.init + ':' + typeof exports.render + ':' + exports.init() + ':' + exports.render()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("47:2:instantiated:function:function:undefined:undefined")
    Err(err):
        expect("unexpected instance constructor multiple function export js error: {err}").to_equal("")
```

</details>

#### compares constructor and compiled WebAssembly multiple function exports in browser scripts

- compares constructor and compiled WebAssembly multiple function exports in browser scripts
   - Expected: _display_js(value) equals `ctor:47:2:instantiated:function:function:undefined:undefined:compiled:instant... (full value in folded executable source)`
   - Expected: "unexpected constructor/compiled multiple function export js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares constructor and compiled WebAssembly multiple function exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var wasm = '0061736d01000000010401600000030302000007110204696e697400000672656e64657200010a070202000b02000b'; var module = new WebAssembly.Module(wasm); var instance = new WebAssembly.Instance(module); var exports = instance.exports; out = 'ctor:' + module.byteLength + ':' + module.functionExportCount + ':' + instance.status + ':' + typeof exports.init + ':' + typeof exports.render + ':' + exports.init() + ':' + exports.render(); WebAssembly.compile(wasm).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { var exports = result.instance.exports; out = out + ':compiled:' + result.status + ':' + result.module.byteLength + ':' + result.module.functionExportCount + ':' + typeof exports.init + ':' + typeof exports.render + ':' + exports.init() + ':' + exports.render(); }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("ctor:47:2:instantiated:function:function:undefined:undefined:compiled:instantiated:47:2:function:function:undefined:undefined")
    Err(err):
        expect("unexpected constructor/compiled multiple function export js error: {err}").to_equal("")
```

</details>

#### constructs WebAssembly Instance modules with descriptor metadata

- constructs WebAssembly Instance modules with descriptor metadata
   - Expected: _display_js(value) equals `instantiated:40:2:tbl:table:answer:global:instantiated:1:27:1:env:foo:functio... (full value in folded executable source)`
   - Expected: "unexpected instance constructor descriptor metadata js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("constructs WebAssembly Instance modules with descriptor metadata")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var tableGlobal = new WebAssembly.Module('0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300'); var exports = WebAssembly.Module.exports(tableGlobal); var exportedInstance = new WebAssembly.Instance(tableGlobal); var imported = new WebAssembly.Module('0061736d01000000010401600000020b0103656e7603666f6f0000'); var imports = WebAssembly.Module.imports(imported); var importedInstance = new WebAssembly.Instance(imported, { env: { foo: function() { return 7; } } }); exportedInstance.status + ':' + tableGlobal.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind + ':' + importedInstance.status + ':' + imported.importCount + ':' + imported.byteLength + ':' + imports.length + ':' + imports[0].module + ':' + imports[0].name + ':' + imports[0].kind + ':' + typeof importedInstance.exports")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("instantiated:40:2:tbl:table:answer:global:instantiated:1:27:1:env:foo:function:object")
    Err(err):
        expect("unexpected instance constructor descriptor metadata js error: {err}").to_equal("")
```

</details>

#### compares constructor and compiled WebAssembly descriptor metadata in browser scripts

- compares constructor and compiled WebAssembly descriptor metadata in browser scripts
   - Expected: _display_js(value) equals `ctor:40:2:tbl:table:answer:global:27:1:1:env:foo:function:instantiated:object... (full value in folded executable source)`
   - Expected: "unexpected constructor/compiled descriptor metadata js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares constructor and compiled WebAssembly descriptor metadata in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var exportWasm = '0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300'; var importWasm = '0061736d01000000010401600000020b0103656e7603666f6f0000'; var tableGlobal = new WebAssembly.Module(exportWasm); var exports = WebAssembly.Module.exports(tableGlobal); var imported = new WebAssembly.Module(importWasm); var imports = WebAssembly.Module.imports(imported); var importedInstance = new WebAssembly.Instance(imported, { env: { foo: function() { return 7; } } }); out = 'ctor:' + tableGlobal.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind + ':' + imported.byteLength + ':' + imported.importCount + ':' + imports.length + ':' + imports[0].module + ':' + imports[0].name + ':' + imports[0].kind + ':' + importedInstance.status + ':' + typeof importedInstance.exports; WebAssembly.compile(exportWasm).then(function(module) { var exports = WebAssembly.Module.exports(module); out = out + ':compiledExports:' + module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; return WebAssembly.compile(importWasm); }).then(function(module) { var imports = WebAssembly.Module.imports(module); out = out + ':compiledImports:' + module.importCount + ':' + module.byteLength + ':' + imports.length + ':' + imports[0].module + ':' + imports[0].name + ':' + imports[0].kind; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("ctor:40:2:tbl:table:answer:global:27:1:1:env:foo:function:instantiated:object:compiledExports:40:2:tbl:table:answer:global:compiledImports:1:27:1:env:foo:function")
    Err(err):
        expect("unexpected constructor/compiled descriptor metadata js error: {err}").to_equal("")
```

</details>

#### returns invalid WebAssembly Instance metadata for missing imports

- returns invalid WebAssembly Instance metadata for missing imports
   - Expected: _display_js(value) equals `1:invalid:true:unsupported-wasm-imports:object`
   - Expected: "unexpected instance constructor missing import js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("returns invalid WebAssembly Instance metadata for missing imports")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var module = new WebAssembly.Module('0061736d01000000010401600000020b0103656e7603666f6f0000'); var instance = new WebAssembly.Instance(module, {}); module.importCount + ':' + instance.status + ':' + instance.moduleValid + ':' + instance.error + ':' + typeof instance.exports")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1:invalid:true:unsupported-wasm-imports:object")
    Err(err):
        expect("unexpected instance constructor missing import js error: {err}").to_equal("")
```

</details>

#### compares constructor and compiled WebAssembly missing import metadata in browser scripts

- compares constructor and compiled WebAssembly missing import metadata in browser scripts
   - Expected: _display_js(value) equals `ctor:1:27:invalid:true:unsupported-wasm-imports:object:compiledModule:1:27:co... (full value in folded executable source)`
   - Expected: "unexpected constructor/compiled missing import metadata js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares constructor and compiled WebAssembly missing import metadata in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var wasm = '0061736d01000000010401600000020b0103656e7603666f6f0000'; var module = new WebAssembly.Module(wasm); var instance = new WebAssembly.Instance(module, {}); out = 'ctor:' + module.importCount + ':' + module.byteLength + ':' + instance.status + ':' + instance.moduleValid + ':' + instance.error + ':' + typeof instance.exports; WebAssembly.compile(wasm).then(function(module) { out = out + ':compiledModule:' + module.importCount + ':' + module.byteLength; return WebAssembly.instantiate(module, {}); }).then(function(result) { out = out + ':unexpected:' + result.status; }).catch(function(err) { out = out + ':compiledInstantiate:' + err.status + ':' + err.error; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("ctor:1:27:invalid:true:unsupported-wasm-imports:object:compiledModule:1:27:compiledInstantiate:invalid:unsupported-wasm-imports")
    Err(err):
        expect("unexpected constructor/compiled missing import metadata js error: {err}").to_equal("")
```

</details>

#### returns invalid WebAssembly Instance metadata for invalid modules

- returns invalid WebAssembly Instance metadata for invalid modules
   - Expected: _display_js(value) equals `false:invalid-wasm-header:invalid:false:invalid-wasm-module:object`
   - Expected: "unexpected instance constructor invalid module js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("returns invalid WebAssembly Instance metadata for invalid modules")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var module = new WebAssembly.Module('00'); var instance = new WebAssembly.Instance(module); module.validated + ':' + module.error + ':' + instance.status + ':' + instance.moduleValid + ':' + instance.error + ':' + typeof instance.exports")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("false:invalid-wasm-header:invalid:false:invalid-wasm-module:object")
    Err(err):
        expect("unexpected instance constructor invalid module js error: {err}").to_equal("")
```

</details>

#### returns invalid WebAssembly Instance metadata for truncated module sections

- returns invalid WebAssembly Instance metadata for truncated module sections
   - Expected: _display_js(value) equals `false:invalid-wasm-section:0:invalid:false:invalid-wasm-module`
   - Expected: "unexpected instance constructor truncated module js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("returns invalid WebAssembly Instance metadata for truncated module sections")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var module = new WebAssembly.Module('0061736d010000000105'); var instance = new WebAssembly.Instance(module); module.validated + ':' + module.error + ':' + module.sectionCount + ':' + instance.status + ':' + instance.moduleValid + ':' + instance.error")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("false:invalid-wasm-section:0:invalid:false:invalid-wasm-module")
    Err(err):
        expect("unexpected instance constructor truncated module js error: {err}").to_equal("")
```

</details>

#### compares constructor and compiled WebAssembly invalid module metadata in browser scripts

- compares constructor and compiled WebAssembly invalid module metadata in browser scripts
   - Expected: _display_js(value) equals `ctorInvalid:false:invalid-wasm-header:invalid:false:invalid-wasm-module:ctorT... (full value in folded executable source)`
   - Expected: "unexpected constructor/compiled invalid module metadata js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares constructor and compiled WebAssembly invalid module metadata in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var invalidModule = new WebAssembly.Module('00'); var invalidInstance = new WebAssembly.Instance(invalidModule); var truncatedModule = new WebAssembly.Module('0061736d010000000105'); var truncatedInstance = new WebAssembly.Instance(truncatedModule); out = 'ctorInvalid:' + invalidModule.validated + ':' + invalidModule.error + ':' + invalidInstance.status + ':' + invalidInstance.moduleValid + ':' + invalidInstance.error + ':ctorTruncated:' + truncatedModule.validated + ':' + truncatedModule.error + ':' + truncatedModule.sectionCount + ':' + truncatedInstance.status + ':' + truncatedInstance.moduleValid + ':' + truncatedInstance.error; WebAssembly.compile('00').catch(function(err) { out = out + ':compileInvalid:' + err.status + ':' + err.error; return WebAssembly.compile('0061736d010000000105'); }).catch(function(err) { out = out + ':compileTruncated:' + err.status + ':' + err.error; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("ctorInvalid:false:invalid-wasm-header:invalid:false:invalid-wasm-module:ctorTruncated:false:invalid-wasm-section:0:invalid:false:invalid-wasm-module:compileInvalid:invalid:invalid-wasm-header:compileTruncated:invalid:invalid-wasm-section")
    Err(err):
        expect("unexpected constructor/compiled invalid module metadata js error: {err}").to_equal("")
```

</details>

#### returns invalid WebAssembly Instance metadata for missing module arguments

- returns invalid WebAssembly Instance metadata for missing module arguments
   - Expected: _display_js(value) equals `invalid:false:invalid-wasm-module:object`
   - Expected: "unexpected instance constructor missing module js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("returns invalid WebAssembly Instance metadata for missing module arguments")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var instance = new WebAssembly.Instance(); instance.status + ':' + instance.moduleValid + ':' + instance.error + ':' + typeof instance.exports")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("invalid:false:invalid-wasm-module:object")
    Err(err):
        expect("unexpected instance constructor missing module js error: {err}").to_equal("")
```

</details>

#### returns empty WebAssembly Module descriptors for invalid module arguments

- returns empty WebAssembly Module descriptors for invalid module arguments
   - Expected: _display_js(value) equals `false:invalid-wasm-header:0:0:0:0`
   - Expected: "unexpected invalid module descriptor js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("returns empty WebAssembly Module descriptors for invalid module arguments")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var module = new WebAssembly.Module('00'); var exports = WebAssembly.Module.exports(module); var imports = WebAssembly.Module.imports(module); var missingExports = WebAssembly.Module.exports(); var missingImports = WebAssembly.Module.imports(); module.validated + ':' + module.error + ':' + exports.length + ':' + imports.length + ':' + missingExports.length + ':' + missingImports.length")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("false:invalid-wasm-header:0:0:0:0")
    Err(err):
        expect("unexpected invalid module descriptor js error: {err}").to_equal("")
```

</details>

#### returns empty WebAssembly Module descriptors for truncated module sections

- returns empty WebAssembly Module descriptors for truncated module sections
   - Expected: _display_js(value) equals `false:invalid-wasm-section:0:0:0`
   - Expected: "unexpected truncated module descriptor js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("returns empty WebAssembly Module descriptors for truncated module sections")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var module = new WebAssembly.Module('0061736d010000000105'); var exports = WebAssembly.Module.exports(module); var imports = WebAssembly.Module.imports(module); module.validated + ':' + module.error + ':' + module.sectionCount + ':' + exports.length + ':' + imports.length")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("false:invalid-wasm-section:0:0:0")
    Err(err):
        expect("unexpected truncated module descriptor js error: {err}").to_equal("")
```

</details>

#### synthesizes multiple WebAssembly function exports in browser scripts

- synthesizes multiple WebAssembly function exports in browser scripts
   - Expected: _display_js(value) equals `instantiated:47:2:function:function:undefined:undefined`
   - Expected: "unexpected multiple function export js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("synthesizes multiple WebAssembly function exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; WebAssembly.instantiate('0061736d01000000010401600000030302000007110204696e697400000672656e64657200010a070202000b02000b').then(function(result) { var exports = result.instance.exports; out = result.status + ':' + result.module.byteLength + ':' + result.module.functionExportCount + ':' + typeof exports.init + ':' + typeof exports.render + ':' + exports.init() + ':' + exports.render(); }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("instantiated:47:2:function:function:undefined:undefined")
    Err(err):
        expect("unexpected multiple function export js error: {err}").to_equal("")
```

</details>

#### executes WebAssembly function export bodies with call arguments in browser scripts

- executes WebAssembly function export bodies with call arguments in browser scripts
   - Expected: _display_js(value) equals `instantiated:41:function:42:42`
   - Expected: "unexpected function export body argument js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("executes WebAssembly function export bodies with call arguments in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; WebAssembly.instantiate('0061736d0100000001070160027f7f017f030201000707010372756e00000a09010700200020016a0b').then(function(result) { out = result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports.run + ':' + result.instance.exports.run(40, 2) + ':' + result.instance.exports.run(7, 35); }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("instantiated:41:function:42:42")
    Err(err):
        expect("unexpected function export body argument js error: {err}").to_equal("")
```

</details>

#### compares direct and compiled WebAssembly function export bodies in browser scripts

- compares direct and compiled WebAssembly function export bodies in browser scripts
   - Expected: _display_js(value) equals `direct:instantiated:41:function:42:42:compiled:instantiated:41:function:42:42`
   - Expected: "unexpected combined function export body js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares direct and compiled WebAssembly function export bodies in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var wasm = '0061736d0100000001070160027f7f017f030201000707010372756e00000a09010700200020016a0b'; WebAssembly.instantiate(wasm).then(function(result) { out = 'direct:' + result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports.run + ':' + result.instance.exports.run(40, 2) + ':' + result.instance.exports.run(7, 35); }); WebAssembly.compile(wasm).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { out = out + ':compiled:' + result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports.run + ':' + result.instance.exports.run(39, 3) + ':' + result.instance.exports.run(8, 34); }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("direct:instantiated:41:function:42:42:compiled:instantiated:41:function:42:42")
    Err(err):
        expect("unexpected combined function export body js error: {err}").to_equal("")
```

</details>

#### chains compiled WebAssembly modules into instantiate

- chains compiled WebAssembly modules into instantiate
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `fetch>arrayBuffer:11>compile:11>instantiate:instantiated:11:object`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("chains compiled WebAssembly modules into instantiate")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var stages = ''; window.fetch('/mod.wasm').then(function(r) { stages = stages + 'fetch>'; return r.arrayBuffer(); }).then(function(bytes) { stages = stages + 'arrayBuffer:' + bytes.byteLength + '>'; return WebAssembly.compile(bytes); }).then(function(module) { stages = stages + 'compile:' + module.byteLength + '>'; return WebAssembly.instantiate(module); }).then(function(result) { out = stages + 'instantiate:' + result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010100",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("fetch>arrayBuffer:11>compile:11>instantiate:instantiated:11:object")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### instantiates streaming fetch responses with imports

- instantiates streaming fetch responses with imports
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `stream:instantiated:1:42`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates streaming fetch responses with imports")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var imports = { env: { foo: function(x) { return x + 1; } } }; WebAssembly.instantiateStreaming(window.fetch('/mod.wasm'), imports).then(function(result) { out = 'stream:' + result.status + ':' + result.module.importCount + ':' + result.instance.exports.run(41); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("stream:instantiated:1:42")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### exposes instantiateStreaming module export descriptors

- exposes instantiateStreaming module export descriptors
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `streamExportDescriptors:instantiated:40:2:tbl:table:answer:global`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes instantiateStreaming module export descriptors")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.instantiateStreaming(window.fetch('/mod.wasm')).then(function(result) { var exports = WebAssembly.Module.exports(result.module); out = 'streamExportDescriptors:' + result.status + ':' + result.module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("streamExportDescriptors:instantiated:40:2:tbl:table:answer:global")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### exposes instantiateStreaming module import descriptors

- exposes instantiateStreaming module import descriptors
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `streamImportDescriptors:instantiated:1:27:1:env:foo:function:object`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes instantiateStreaming module import descriptors")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var imports = { env: { foo: function() { return 7; } } }; WebAssembly.instantiateStreaming(window.fetch('/mod.wasm'), imports).then(function(result) { var descriptors = WebAssembly.Module.imports(result.module); out = 'streamImportDescriptors:' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind + ':' + typeof result.instance.exports; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000020b0103656e7603666f6f0000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("streamImportDescriptors:instantiated:1:27:1:env:foo:function:object")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### instantiates streaming multiple function exports in browser scripts

- instantiates streaming multiple function exports in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `streamFunctions:instantiated:47:2:function:function:undefined:undefined`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates streaming multiple function exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.instantiateStreaming(window.fetch('/mod.wasm')).then(function(result) { var exports = result.instance.exports; out = 'streamFunctions:' + result.status + ':' + result.module.byteLength + ':' + result.module.functionExportCount + ':' + typeof exports.init + ':' + typeof exports.render + ':' + exports.init() + ':' + exports.render(); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000030302000007110204696e697400000672656e64657200010a070202000b02000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("streamFunctions:instantiated:47:2:function:function:undefined:undefined")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### instantiates streaming WebAssembly function export bodies with call arguments

- instantiates streaming WebAssembly function export bodies with call arguments
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `streamArgBody:instantiated:41:function:42:42`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates streaming WebAssembly function export bodies with call arguments")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.instantiateStreaming(window.fetch('/mod.wasm')).then(function(result) { out = 'streamArgBody:' + result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports.run + ':' + result.instance.exports.run(40, 2) + ':' + result.instance.exports.run(7, 35); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001070160027f7f017f030201000707010372756e00000a09010700200020016a0b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("streamArgBody:instantiated:41:function:42:42")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### instantiates streaming table and global exports in browser scripts

- instantiates streaming table and global exports in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `streamExports:instantiated:40:table:funcref:null:1:2:global:i32:false:42`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates streaming table and global exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.instantiateStreaming(window.fetch('/mod.wasm')).then(function(result) { var table = result.instance.exports.tbl; var global = result.instance.exports.answer; var before = table.get(0); var old = table.grow(1, null); out = 'streamExports:' + result.status + ':' + result.module.byteLength + ':' + table.kind + ':' + table.element + ':' + before + ':' + old + ':' + table.length + ':' + global.kind + ':' + global.valueType + ':' + global.mutable + ':' + global.value; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("streamExports:instantiated:40:table:funcref:null:1:2:global:i32:false:42")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### instantiates streaming memory exports in browser scripts

- instantiates streaming memory exports in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `streamMemory:instantiated:25:131072:65536:4:1:131072:4`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates streaming memory exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.instantiateStreaming(window.fetch('/mod.wasm')).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[5] = 260; var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); out = 'streamMemory:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + bytes[5] + ':' + old + ':' + grown.length + ':' + grown[5]; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000503010001070a01066d656d6f72790200",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("streamMemory:instantiated:25:131072:65536:4:1:131072:4")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### instantiates streaming memory maximum failures in browser scripts

- instantiates streaming memory maximum failures in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `streamMemoryMax:instantiated:26:65536:65536:1:-1:65536:18`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates streaming memory maximum failures in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.instantiateStreaming(window.fetch('/mod.wasm')).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[12] = 274; var zero = memory.grow(0); var fail = memory.grow(1); var after = new Uint8Array(memory.buffer); out = 'streamMemoryMax:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + zero + ':' + fail + ':' + after.length + ':' + after[12]; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000050401010101070a01066d656d6f72790200",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("streamMemoryMax:instantiated:26:65536:65536:1:-1:65536:18")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### routes invalid instantiateStreaming responses through catch

- routes invalid instantiateStreaming responses through catch
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `instantiateStreamInvalid:invalid:invalid-wasm-header`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes invalid instantiateStreaming responses through catch")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.instantiateStreaming(window.fetch('/mod.wasm'), {}).then(function(result) { out = 'unexpected:' + result.status; }).catch(function(err) { out = 'instantiateStreamInvalid:' + err.status + ':' + err.error; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d00000000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("instantiateStreamInvalid:invalid:invalid-wasm-header")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### routes instantiateStreaming fetch errors through catch

- routes instantiateStreaming fetch errors through catch
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: "expected fetch commit error" equals ``
   - Expected: err equals `network-down`
   - Expected: _display_js(value) equals `instantiateStreamFetchError:network-down`
   - Expected: "unexpected js error: {js_err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 44 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes instantiateStreaming fetch errors through catch")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.instantiateStreaming(window.fetch('/mod.wasm'), {}).then(function(result) { out = 'unexpected:' + result.status; }).catch(function(err) { out = 'instantiateStreamFetchError:' + err; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 0,
            headers: "",
            body: "",
            error: "network-down"
        ))
        match committed:
            Ok(_):
                expect("expected fetch commit error").to_equal("")
            Err(err):
                expect(err).to_equal("network-down")
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("instantiateStreamFetchError:network-down")
                    Err(js_err):
                        expect("unexpected js error: {js_err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### routes instantiateStreaming missing imports through catch

- routes instantiateStreaming missing imports through catch
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `instantiateStreamImports:invalid:unsupported-wasm-imports`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes instantiateStreaming missing imports through catch")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.instantiateStreaming(window.fetch('/mod.wasm'), {}).then(function(result) { out = 'unexpected:' + result.status; }).catch(function(err) { out = 'instantiateStreamImports:' + err.status + ':' + err.error; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("instantiateStreamImports:invalid:unsupported-wasm-imports")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### compiles streaming fetch responses into modules

- compiles streaming fetch responses into modules
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `compileStreaming:11:1:true`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compiles streaming fetch responses into modules")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/mod.wasm')).then(function(module) { out = 'compileStreaming:' + module.byteLength + ':' + module.sectionCount + ':' + module.hasTypeSection; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010100",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreaming:11:1:true")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### compares compileStreaming and instantiateStreaming base module metadata in browser scripts

- compares compileStreaming and instantiateStreaming base module metadata in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: _display_js(value) equals `compileStreamModule:11:1:true`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: _display_js(value) equals `compileStreamModule:11:1:true:instantiateStreamModule:instantiated:11:1:object`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming and instantiateStreaming base module metadata in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).then(function(module) { out = 'compileStreamModule:' + module.byteLength + ':' + module.sectionCount + ':' + module.hasTypeSection; }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm')).then(function(result) { out = out + ':instantiateStreamModule:' + result.status + ':' + result.module.byteLength + ':' + result.module.sectionCount + ':' + typeof result.instance.exports; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010100",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamModule:11:1:true")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010100",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamModule:11:1:true:instantiateStreamModule:instantiated:11:1:object")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### compares compileStreaming module and compileStreaming instantiated base metadata in browser scripts

- compares compileStreaming module and compileStreaming instantiated base metadata in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/module.wasm`
   - Expected: _display_js(value) equals `compileStreamModule:11:1:true`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing module fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instance.wasm`
   - Expected: _display_js(value) equals `compileStreamModule:11:1:true:compileStreamInstModule:instantiated:11:1:true:... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instance fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming module and compileStreaming instantiated base metadata in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/module.wasm')).then(function(module) { out = 'compileStreamModule:' + module.byteLength + ':' + module.sectionCount + ':' + module.hasTypeSection; }); WebAssembly.compileStreaming(window.fetch('/instance.wasm')).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { out = out + ':compileStreamInstModule:' + result.status + ':' + result.module.byteLength + ':' + result.module.sectionCount + ':' + result.module.hasTypeSection + ':' + typeof result.instance.exports; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/module.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010100",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamModule:11:1:true")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing module fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instance.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010100",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamModule:11:1:true:compileStreamInstModule:instantiated:11:1:true:object")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instance fetch request").to_equal("")
```

</details>

#### routes invalid compileStreaming responses through catch

- routes invalid compileStreaming responses through catch
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `compileStreamInvalid:invalid-wasm-header`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes invalid compileStreaming responses through catch")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/mod.wasm')).then(function(module) { out = 'unexpected:' + module.byteLength; }).catch(function(err) { out = 'compileStreamInvalid:' + err; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d00000000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamInvalid:invalid-wasm-header")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### compares compileStreaming and instantiateStreaming invalid header rejections in browser scripts

- compares compileStreaming and instantiateStreaming invalid header rejections in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: _display_js(value) equals `compileStreamInvalid:invalid-wasm-header`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: _display_js(value) equals `compileStreamInvalid:invalid-wasm-header:instantiateStreamInvalid:invalid:inv... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming and instantiateStreaming invalid header rejections in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).then(function(module) { out = 'unexpectedCompile:' + module.byteLength; }).catch(function(err) { out = 'compileStreamInvalid:' + err; }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm'), {}).then(function(result) { out = out + ':unexpectedInstantiate:' + result.status; }).catch(function(err) { out = out + ':instantiateStreamInvalid:' + err.status + ':' + err.error; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d00000000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamInvalid:invalid-wasm-header")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d00000000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamInvalid:invalid-wasm-header:instantiateStreamInvalid:invalid:invalid-wasm-header")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### routes truncated section streaming responses through catch

- routes truncated section streaming responses through catch
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: _display_js(value) equals `compileStreamSection:invalid-wasm-section`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: _display_js(value) equals `compileStreamSection:invalid-wasm-section:instantiateStreamSection:invalid:in... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes truncated section streaming responses through catch")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).catch(function(err) { out = 'compileStreamSection:' + err; }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm')).catch(function(err) { out = out + ':instantiateStreamSection:' + err.status + ':' + err.error; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000105",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamSection:invalid-wasm-section")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000105",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamSection:invalid-wasm-section:instantiateStreamSection:invalid:invalid-wasm-section")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### routes compileStreaming fetch errors through catch

- routes compileStreaming fetch errors through catch
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: "expected fetch commit error" equals ``
   - Expected: err equals `network-down`
   - Expected: _display_js(value) equals `compileStreamFetchError:network-down`
   - Expected: "unexpected js error: {js_err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 44 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes compileStreaming fetch errors through catch")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/mod.wasm')).then(function(module) { out = 'unexpected:' + module.byteLength; }).catch(function(err) { out = 'compileStreamFetchError:' + err; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 0,
            headers: "",
            body: "",
            error: "network-down"
        ))
        match committed:
            Ok(_):
                expect("expected fetch commit error").to_equal("")
            Err(err):
                expect(err).to_equal("network-down")
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamFetchError:network-down")
                    Err(js_err):
                        expect("unexpected js error: {js_err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### compares compileStreaming and instantiateStreaming fetch error rejections in browser scripts

- compares compileStreaming and instantiateStreaming fetch error rejections in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: "expected first fetch commit error" equals ``
   - Expected: err equals `network-down`
   - Expected: _display_js(value) equals `compileStreamFetchError:network-down`
   - Expected: "unexpected first commit js error: {js_err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: "expected second fetch commit error" equals ``
   - Expected: err equals `network-down`
   - Expected: _display_js(value) equals `compileStreamFetchError:network-down:instantiateStreamFetchError:network-down`
   - Expected: "unexpected second commit js error: {js_err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 70 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming and instantiateStreaming fetch error rejections in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).then(function(module) { out = 'unexpectedCompile:' + module.byteLength; }).catch(function(err) { out = 'compileStreamFetchError:' + err; }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm'), {}).then(function(result) { out = out + ':unexpectedInstantiate:' + result.status; }).catch(function(err) { out = out + ':instantiateStreamFetchError:' + err; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 0,
            headers: "",
            body: "",
            error: "network-down"
        ))
        match committed:
            Ok(_):
                expect("expected first fetch commit error").to_equal("")
            Err(err):
                expect(err).to_equal("network-down")
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamFetchError:network-down")
                    Err(js_err):
                        expect("unexpected first commit js error: {js_err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 0,
            headers: "",
            body: "",
            error: "network-down"
        ))
        match committed:
            Ok(_):
                expect("expected second fetch commit error").to_equal("")
            Err(err):
                expect(err).to_equal("network-down")
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamFetchError:network-down:instantiateStreamFetchError:network-down")
                    Err(js_err):
                        expect("unexpected second commit js error: {js_err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### exposes compileStreaming module export descriptors

- exposes compileStreaming module export descriptors
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `compileStreamExports:40:2:tbl:table:answer:global`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes compileStreaming module export descriptors")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/mod.wasm')).then(function(module) { var exports = WebAssembly.Module.exports(module); out = 'compileStreamExports:' + module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamExports:40:2:tbl:table:answer:global")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### compares compileStreaming module and instantiateStreaming export descriptors in browser scripts

- compares compileStreaming module and instantiateStreaming export descriptors in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: _display_js(value) equals `compileStreamModuleExports:40:2:tbl:table:answer:global`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: _display_js(value) equals `compileStreamModuleExports:40:2:tbl:table:answer:global:instantiateStreamExpo... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming module and instantiateStreaming export descriptors in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).then(function(module) { var exports = WebAssembly.Module.exports(module); out = 'compileStreamModuleExports:' + module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm')).then(function(result) { var exports = WebAssembly.Module.exports(result.module); out = out + ':instantiateStreamExports:' + result.status + ':' + result.module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamModuleExports:40:2:tbl:table:answer:global")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamModuleExports:40:2:tbl:table:answer:global:instantiateStreamExports:instantiated:40:2:tbl:table:answer:global")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### exposes compileStreaming instantiated module export descriptors

- exposes compileStreaming instantiated module export descriptors
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `compileStreamInstExportDescriptors:instantiated:40:2:tbl:table:answer:global`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes compileStreaming instantiated module export descriptors")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/mod.wasm')).then(function(module) { return WebAssembly.instantiate(module).then(function(result) { var exports = WebAssembly.Module.exports(result.module); out = 'compileStreamInstExportDescriptors:' + result.status + ':' + result.module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamInstExportDescriptors:instantiated:40:2:tbl:table:answer:global")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### compares compileStreaming module and compileStreaming instantiated export descriptors in browser scripts

- compares compileStreaming module and compileStreaming instantiated export descriptors in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/module.wasm`
   - Expected: _display_js(value) equals `compileStreamModuleExports:40:2:tbl:table:answer:global`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing module fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instance.wasm`
   - Expected: _display_js(value) equals `compileStreamModuleExports:40:2:tbl:table:answer:global:compileStreamInstExpo... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instance fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming module and compileStreaming instantiated export descriptors in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/module.wasm')).then(function(module) { var exports = WebAssembly.Module.exports(module); out = 'compileStreamModuleExports:' + module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); WebAssembly.compileStreaming(window.fetch('/instance.wasm')).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { var exports = WebAssembly.Module.exports(result.module); out = out + ':compileStreamInstExports:' + result.status + ':' + result.module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/module.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamModuleExports:40:2:tbl:table:answer:global")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing module fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instance.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamModuleExports:40:2:tbl:table:answer:global:compileStreamInstExports:instantiated:40:2:tbl:table:answer:global")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instance fetch request").to_equal("")
```

</details>

#### compares compileStreaming and instantiateStreaming export descriptors in browser scripts

- compares compileStreaming and instantiateStreaming export descriptors in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: _display_js(value) equals `compileStreamInstExportDescriptors:instantiated:40:2:tbl:table:answer:global`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: _display_js(value) equals `compileStreamInstExportDescriptors:instantiated:40:2:tbl:table:answer:global:... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming and instantiateStreaming export descriptors in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { var exports = WebAssembly.Module.exports(result.module); out = 'compileStreamInstExportDescriptors:' + result.status + ':' + result.module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm')).then(function(result) { var exports = WebAssembly.Module.exports(result.module); out = out + ':instantiateStreamExportDescriptors:' + result.status + ':' + result.module.byteLength + ':' + exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamInstExportDescriptors:instantiated:40:2:tbl:table:answer:global")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamInstExportDescriptors:instantiated:40:2:tbl:table:answer:global:instantiateStreamExportDescriptors:instantiated:40:2:tbl:table:answer:global")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### instantiates compileStreaming table and global exports in browser scripts

- instantiates compileStreaming table and global exports in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `compileStreamExports:instantiated:40:table:funcref:null:1:2:global:i32:false:42`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates compileStreaming table and global exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/mod.wasm')).then(function(module) { return WebAssembly.instantiate(module).then(function(result) { var table = result.instance.exports.tbl; var global = result.instance.exports.answer; var before = table.get(0); var old = table.grow(1, null); out = 'compileStreamExports:' + result.status + ':' + result.module.byteLength + ':' + table.kind + ':' + table.element + ':' + before + ':' + old + ':' + table.length + ':' + global.kind + ':' + global.valueType + ':' + global.mutable + ':' + global.value; }); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamExports:instantiated:40:table:funcref:null:1:2:global:i32:false:42")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### compares compileStreaming and instantiateStreaming table and global exports in browser scripts

- compares compileStreaming and instantiateStreaming table and global exports in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: _display_js(value) equals `compileStreamExports:instantiated:40:table:funcref:null:1:2:global:i32:false:42`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: _display_js(value) equals `compileStreamExports:instantiated:40:table:funcref:null:1:2:global:i32:false:... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming and instantiateStreaming table and global exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { var table = result.instance.exports.tbl; var global = result.instance.exports.answer; var before = table.get(0); var old = table.grow(1, null); out = 'compileStreamExports:' + result.status + ':' + result.module.byteLength + ':' + table.kind + ':' + table.element + ':' + before + ':' + old + ':' + table.length + ':' + global.kind + ':' + global.valueType + ':' + global.mutable + ':' + global.value; }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm')).then(function(result) { var table = result.instance.exports.tbl; var global = result.instance.exports.answer; var before = table.get(0); var old = table.grow(1, null); out = out + ':instantiateStreamExports:' + result.status + ':' + result.module.byteLength + ':' + table.kind + ':' + table.element + ':' + before + ':' + old + ':' + table.length + ':' + global.kind + ':' + global.valueType + ':' + global.mutable + ':' + global.value; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamExports:instantiated:40:table:funcref:null:1:2:global:i32:false:42")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamExports:instantiated:40:table:funcref:null:1:2:global:i32:false:42:instantiateStreamExports:instantiated:40:table:funcref:null:1:2:global:i32:false:42")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### compares compileStreaming and instantiateStreaming function export bodies in browser scripts

- compares compileStreaming and instantiateStreaming function export bodies in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: _display_js(value) equals `compileStreamArgBody:instantiated:41:function:42:42`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: _display_js(value) equals `compileStreamArgBody:instantiated:41:function:42:42:instantiateStreamArgBody:... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming and instantiateStreaming function export bodies in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { out = 'compileStreamArgBody:' + result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports.run + ':' + result.instance.exports.run(40, 2) + ':' + result.instance.exports.run(7, 35); }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm')).then(function(result) { out = out + ':instantiateStreamArgBody:' + result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports.run + ':' + result.instance.exports.run(40, 2) + ':' + result.instance.exports.run(7, 35); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001070160027f7f017f030201000707010372756e00000a09010700200020016a0b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamArgBody:instantiated:41:function:42:42")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001070160027f7f017f030201000707010372756e00000a09010700200020016a0b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamArgBody:instantiated:41:function:42:42:instantiateStreamArgBody:instantiated:41:function:42:42")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### compares compileStreaming and instantiateStreaming multiple function exports in browser scripts

- compares compileStreaming and instantiateStreaming multiple function exports in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: _display_js(value) equals `compileStreamFunctions:instantiated:47:2:function:function:undefined:undefined`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: _display_js(value) equals `compileStreamFunctions:instantiated:47:2:function:function:undefined:undefine... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming and instantiateStreaming multiple function exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { var exports = result.instance.exports; out = 'compileStreamFunctions:' + result.status + ':' + result.module.byteLength + ':' + result.module.functionExportCount + ':' + typeof exports.init + ':' + typeof exports.render + ':' + exports.init() + ':' + exports.render(); }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm')).then(function(result) { var exports = result.instance.exports; out = out + ':instantiateStreamFunctions:' + result.status + ':' + result.module.byteLength + ':' + result.module.functionExportCount + ':' + typeof exports.init + ':' + typeof exports.render + ':' + exports.init() + ':' + exports.render(); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000030302000007110204696e697400000672656e64657200010a070202000b02000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamFunctions:instantiated:47:2:function:function:undefined:undefined")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000030302000007110204696e697400000672656e64657200010a070202000b02000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamFunctions:instantiated:47:2:function:function:undefined:undefined:instantiateStreamFunctions:instantiated:47:2:function:function:undefined:undefined")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### instantiates compileStreaming multiple function exports in browser scripts

- instantiates compileStreaming multiple function exports in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `compileStreamFunctions:instantiated:47:2:function:function:undefined:undefined`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates compileStreaming multiple function exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/mod.wasm')).then(function(module) { return WebAssembly.instantiate(module).then(function(result) { var exports = result.instance.exports; out = 'compileStreamFunctions:' + result.status + ':' + result.module.byteLength + ':' + result.module.functionExportCount + ':' + typeof exports.init + ':' + typeof exports.render + ':' + exports.init() + ':' + exports.render(); }); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000030302000007110204696e697400000672656e64657200010a070202000b02000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamFunctions:instantiated:47:2:function:function:undefined:undefined")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### instantiates compileStreaming WebAssembly function export bodies with call arguments

- instantiates compileStreaming WebAssembly function export bodies with call arguments
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `compileStreamArgBody:instantiated:41:function:42:42`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates compileStreaming WebAssembly function export bodies with call arguments")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/mod.wasm')).then(function(module) { return WebAssembly.instantiate(module).then(function(result) { out = 'compileStreamArgBody:' + result.status + ':' + result.module.byteLength + ':' + typeof result.instance.exports.run + ':' + result.instance.exports.run(40, 2) + ':' + result.instance.exports.run(7, 35); }); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001070160027f7f017f030201000707010372756e00000a09010700200020016a0b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamArgBody:instantiated:41:function:42:42")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### instantiates compileStreaming memory exports in browser scripts

- instantiates compileStreaming memory exports in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `compileStreamMemory:instantiated:25:131072:65536:4:1:131072:4`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates compileStreaming memory exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/mod.wasm')).then(function(module) { return WebAssembly.instantiate(module).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[7] = 260; var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); out = 'compileStreamMemory:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + bytes[7] + ':' + old + ':' + grown.length + ':' + grown[7]; }); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000503010001070a01066d656d6f72790200",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamMemory:instantiated:25:131072:65536:4:1:131072:4")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### compares compileStreaming and instantiateStreaming memory exports in browser scripts

- compares compileStreaming and instantiateStreaming memory exports in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: _display_js(value) equals `compileStreamMemory:instantiated:25:131072:65536:4:1:131072:4`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: _display_js(value) equals `compileStreamMemory:instantiated:25:131072:65536:4:1:131072:4:instantiateStre... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming and instantiateStreaming memory exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[9] = 260; var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); out = 'compileStreamMemory:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + bytes[9] + ':' + old + ':' + grown.length + ':' + grown[9]; }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm')).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[10] = 262; var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); out = out + ':instantiateStreamMemory:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + bytes[10] + ':' + old + ':' + grown.length + ':' + grown[10]; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000503010001070a01066d656d6f72790200",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamMemory:instantiated:25:131072:65536:4:1:131072:4")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d010000000503010001070a01066d656d6f72790200",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamMemory:instantiated:25:131072:65536:4:1:131072:4:instantiateStreamMemory:instantiated:25:131072:65536:6:1:131072:6")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### instantiates compileStreaming memory maximum failures in browser scripts

- instantiates compileStreaming memory maximum failures in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `compileStreamMemoryMax:instantiated:26:65536:65536:1:-1:65536:20`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates compileStreaming memory maximum failures in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/mod.wasm')).then(function(module) { return WebAssembly.instantiate(module).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[13] = 276; var zero = memory.grow(0); var fail = memory.grow(1); var after = new Uint8Array(memory.buffer); out = 'compileStreamMemoryMax:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + zero + ':' + fail + ':' + after.length + ':' + after[13]; }); }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000050401010101070a01066d656d6f72790200",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamMemoryMax:instantiated:26:65536:65536:1:-1:65536:20")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### compares compileStreaming and instantiateStreaming memory maximum failures in browser scripts

- compares compileStreaming and instantiateStreaming memory maximum failures in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: _display_js(value) equals `compileStreamMax:instantiated:26:65536:65536:1:-1:65536:22`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: _display_js(value) equals `compileStreamMax:instantiated:26:65536:65536:1:-1:65536:22:instantiateStreamM... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming and instantiateStreaming memory maximum failures in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[14] = 278; var zero = memory.grow(0); var fail = memory.grow(1); var after = new Uint8Array(memory.buffer); out = 'compileStreamMax:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + zero + ':' + fail + ':' + after.length + ':' + after[14]; }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm')).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[15] = 280; var zero = memory.grow(0); var fail = memory.grow(1); var after = new Uint8Array(memory.buffer); out = out + ':instantiateStreamMax:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + zero + ':' + fail + ':' + after.length + ':' + after[15]; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000050401010101070a01066d656d6f72790200",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamMax:instantiated:26:65536:65536:1:-1:65536:22")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000050401010101070a01066d656d6f72790200",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamMax:instantiated:26:65536:65536:1:-1:65536:22:instantiateStreamMax:instantiated:26:65536:65536:1:-1:65536:24")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### exposes compileStreaming module import descriptors

- exposes compileStreaming module import descriptors
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `compileStreamImports:1:27:1:env:foo:function`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes compileStreaming module import descriptors")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/mod.wasm')).then(function(module) { var descriptors = WebAssembly.Module.imports(module); out = 'compileStreamImports:' + module.importCount + ':' + module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000020b0103656e7603666f6f0000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamImports:1:27:1:env:foo:function")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### compares compileStreaming module and instantiateStreaming import descriptors in browser scripts

- compares compileStreaming module and instantiateStreaming import descriptors in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: _display_js(value) equals `compileStreamModuleImports:1:27:1:env:foo:function`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: _display_js(value) equals `compileStreamModuleImports:1:27:1:env:foo:function:instantiateStreamImports:i... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming module and instantiateStreaming import descriptors in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var imports = { env: { foo: function() { return 7; } } }; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).then(function(module) { var descriptors = WebAssembly.Module.imports(module); out = 'compileStreamModuleImports:' + module.importCount + ':' + module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind; }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm'), imports).then(function(result) { var descriptors = WebAssembly.Module.imports(result.module); out = out + ':instantiateStreamImports:' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind + ':' + typeof result.instance.exports; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000020b0103656e7603666f6f0000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamModuleImports:1:27:1:env:foo:function")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000020b0103656e7603666f6f0000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamModuleImports:1:27:1:env:foo:function:instantiateStreamImports:instantiated:1:27:1:env:foo:function:object")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### exposes compileStreaming instantiated module import descriptors

- exposes compileStreaming instantiated module import descriptors
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `compileStreamInstImportDescriptors:instantiated:1:27:1:env:foo:function:object`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes compileStreaming instantiated module import descriptors")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var imports = { env: { foo: function() { return 7; } } }; WebAssembly.compileStreaming(window.fetch('/mod.wasm')).then(function(module) { return WebAssembly.instantiate(module, imports); }).then(function(result) { var descriptors = WebAssembly.Module.imports(result.module); out = 'compileStreamInstImportDescriptors:' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind + ':' + typeof result.instance.exports; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000020b0103656e7603666f6f0000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamInstImportDescriptors:instantiated:1:27:1:env:foo:function:object")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### compares compileStreaming module and compileStreaming instantiated import descriptors in browser scripts

- compares compileStreaming module and compileStreaming instantiated import descriptors in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/module.wasm`
   - Expected: _display_js(value) equals `compileStreamModuleImports:1:27:1:env:foo:function`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing module fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instance.wasm`
   - Expected: _display_js(value) equals `compileStreamModuleImports:1:27:1:env:foo:function:compileStreamInstImports:i... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instance fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming module and compileStreaming instantiated import descriptors in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var imports = { env: { foo: function() { return 7; } } }; WebAssembly.compileStreaming(window.fetch('/module.wasm')).then(function(module) { var descriptors = WebAssembly.Module.imports(module); out = 'compileStreamModuleImports:' + module.importCount + ':' + module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind; }); WebAssembly.compileStreaming(window.fetch('/instance.wasm')).then(function(module) { return WebAssembly.instantiate(module, imports); }).then(function(result) { var descriptors = WebAssembly.Module.imports(result.module); out = out + ':compileStreamInstImports:' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind + ':' + typeof result.instance.exports; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/module.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000020b0103656e7603666f6f0000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamModuleImports:1:27:1:env:foo:function")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing module fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instance.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000020b0103656e7603666f6f0000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamModuleImports:1:27:1:env:foo:function:compileStreamInstImports:instantiated:1:27:1:env:foo:function:object")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instance fetch request").to_equal("")
```

</details>

#### compares compileStreaming and instantiateStreaming import descriptors in browser scripts

- compares compileStreaming and instantiateStreaming import descriptors in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: _display_js(value) equals `compileStreamInstImportDescriptors:instantiated:1:27:1:env:foo:function:object`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: _display_js(value) equals `compileStreamInstImportDescriptors:instantiated:1:27:1:env:foo:function:objec... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming and instantiateStreaming import descriptors in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var imports = { env: { foo: function() { return 7; } } }; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).then(function(module) { return WebAssembly.instantiate(module, imports); }).then(function(result) { var descriptors = WebAssembly.Module.imports(result.module); out = 'compileStreamInstImportDescriptors:' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind + ':' + typeof result.instance.exports; }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm'), imports).then(function(result) { var descriptors = WebAssembly.Module.imports(result.module); out = out + ':instantiateStreamImportDescriptors:' + result.status + ':' + result.module.importCount + ':' + result.module.byteLength + ':' + descriptors.length + ':' + descriptors[0].module + ':' + descriptors[0].name + ':' + descriptors[0].kind + ':' + typeof result.instance.exports; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000020b0103656e7603666f6f0000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamInstImportDescriptors:instantiated:1:27:1:env:foo:function:object")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d01000000010401600000020b0103656e7603666f6f0000",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamInstImportDescriptors:instantiated:1:27:1:env:foo:function:object:instantiateStreamImportDescriptors:instantiated:1:27:1:env:foo:function:object")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### instantiates compileStreaming modules with imports

- instantiates compileStreaming modules with imports
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `compileStreamImport:1:52:42:1`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("instantiates compileStreaming modules with imports")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var calls = 0; var imports = { env: { foo: function(x) { calls = calls + 1; return x + 4; } } }; WebAssembly.compileStreaming(window.fetch('/mod.wasm')).then(function(module) { return WebAssembly.instantiate(module, imports); }).then(function(result) { out = 'compileStreamImport:' + result.module.importCount + ':' + result.module.byteLength + ':' + result.instance.exports.run(38) + ':' + calls; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamImport:1:52:42:1")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### compares compileStreaming and instantiateStreaming import calls in browser scripts

- compares compileStreaming and instantiateStreaming import calls in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: _display_js(value) equals `compileStreamImport:1:52:42:1`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: _display_js(value) equals `compileStreamImport:1:52:42:1:instantiateStreamImport:1:52:42:2`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming and instantiateStreaming import calls in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; var calls = 0; var imports = { env: { foo: function(x) { calls = calls + 1; return x + 4; } } }; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).then(function(module) { return WebAssembly.instantiate(module, imports); }).then(function(result) { out = 'compileStreamImport:' + result.module.importCount + ':' + result.module.byteLength + ':' + result.instance.exports.run(38) + ':' + calls; }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm'), imports).then(function(result) { out = out + ':instantiateStreamImport:' + result.module.importCount + ':' + result.module.byteLength + ':' + result.instance.exports.run(38) + ':' + calls; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamImport:1:52:42:1")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamImport:1:52:42:1:instantiateStreamImport:1:52:42:2")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### routes compileStreaming instantiate missing imports through catch

- routes compileStreaming instantiate missing imports through catch
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/mod.wasm`
   - Expected: _display_js(value) equals `compileStreamInstantiateImports:invalid:unsupported-wasm-imports`
   - Expected: "unexpected js error: {err}" equals ``
   - Expected: "unexpected commit error: {err}" equals ``
   - Expected: "missing fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 43 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("routes compileStreaming instantiate missing imports through catch")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/mod.wasm')).then(function(module) { return WebAssembly.instantiate(module, {}); }).then(function(result) { out = 'unexpected:' + result.status; }).catch(function(err) { out = 'compileStreamInstantiateImports:' + err.status + ':' + err.error; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/mod.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamInstantiateImports:invalid:unsupported-wasm-imports")
                    Err(err):
                        expect("unexpected js error: {err}").to_equal("")
            Err(err):
                expect("unexpected commit error: {err}").to_equal("")
    nil:
        expect("missing fetch request").to_equal("")
```

</details>

#### compares compileStreaming and instantiateStreaming missing import rejections in browser scripts

- compares compileStreaming and instantiateStreaming missing import rejections in browser scripts
   - Expected: _display_js(value) equals `queued`
   - Expected: "unexpected queue error: {err}" equals ``
   - Expected: _display_js(value) equals ``
   - Expected: "unexpected pre-commit js error: {err}" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/compile.wasm`
   - Expected: _display_js(value) equals `compileStreamMissingImports:invalid:unsupported-wasm-imports`
   - Expected: "unexpected first commit js error: {err}" equals ``
   - Expected: "unexpected first commit error: {err}" equals ``
   - Expected: "missing compile fetch request" equals ``
   - Expected: request.kind equals `fetch`
   - Expected: request.url equals `https://example.com/instantiate.wasm`
   - Expected: _display_js(value) equals `compileStreamMissingImports:invalid:unsupported-wasm-imports:instantiateStrea... (full value in folded executable source)`
   - Expected: "unexpected second commit js error: {err}" equals ``
   - Expected: "unexpected second commit error: {err}" equals ``
   - Expected: "missing instantiate fetch request" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 68 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares compileStreaming and instantiateStreaming missing import rejections in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val queued = session.eval_script("var out = ''; WebAssembly.compileStreaming(window.fetch('/compile.wasm')).then(function(module) { return WebAssembly.instantiate(module, {}); }).then(function(result) { out = 'unexpectedCompile:' + result.status; }).catch(function(err) { out = 'compileStreamMissingImports:' + err.status + ':' + err.error; }); WebAssembly.instantiateStreaming(window.fetch('/instantiate.wasm'), {}).then(function(result) { out = out + ':unexpectedInstantiate:' + result.status; }).catch(function(err) { out = out + ':instantiateStreamMissingImports:' + err.status + ':' + err.error; }); 'queued'")
match queued:
    Ok(value):
        expect(_display_js(value)).to_equal("queued")
    Err(err):
        expect("unexpected queue error: {err}").to_equal("")
match session.eval_script("out"):
    Ok(value):
        expect(_display_js(value)).to_equal("")
    Err(err):
        expect("unexpected pre-commit js error: {err}").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/compile.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamMissingImports:invalid:unsupported-wasm-imports")
                    Err(err):
                        expect("unexpected first commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected first commit error: {err}").to_equal("")
    nil:
        expect("missing compile fetch request").to_equal("")

match session.take_pending_request():
    Some(request):
        expect(request.kind).to_equal("fetch")
        expect(request.url).to_equal("https://example.com/instantiate.wasm")
        val committed = session.commit_network_response(BrowserResponse.create(
            request_id: request.id,
            kind: "fetch",
            url: request.url,
            status: 200,
            headers: "Content-Type: application/wasm\n",
            body: "0061736d0100000001060160017f017f020b0103656e7603666f6f0000030201000707010372756e00010a08010600200010000b",
            error: ""
        ))
        match committed:
            Ok(_):
                match session.eval_script("out"):
                    Ok(value):
                        expect(_display_js(value)).to_equal("compileStreamMissingImports:invalid:unsupported-wasm-imports:instantiateStreamMissingImports:invalid:unsupported-wasm-imports")
                    Err(err):
                        expect("unexpected second commit js error: {err}").to_equal("")
            Err(err):
                expect("unexpected second commit error: {err}").to_equal("")
    nil:
        expect("missing instantiate fetch request").to_equal("")
```

</details>

#### exposes Module imports and exports descriptors in browser scripts

- exposes Module imports and exports descriptors in browser scripts
   - Expected: _display_js(value) equals `2:tbl:table:answer:global:1:env:foo:function`
   - Expected: "unexpected descriptor js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes Module imports and exports descriptors in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var tableGlobal = new WebAssembly.Module('0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300'); var exports = WebAssembly.Module.exports(tableGlobal); var imported = new WebAssembly.Module('0061736d01000000010401600000020b0103656e7603666f6f0000'); var imports = WebAssembly.Module.imports(imported); exports.length + ':' + exports[0].name + ':' + exports[0].kind + ':' + exports[1].name + ':' + exports[1].kind + ':' + imports.length + ':' + imports[0].module + ':' + imports[0].name + ':' + imports[0].kind")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("2:tbl:table:answer:global:1:env:foo:function")
    Err(err):
        expect("unexpected descriptor js error: {err}").to_equal("")
```

</details>

#### constructs Table and Global objects in browser scripts

- constructs Table and Global objects in browser scripts
   - Expected: _display_js(value) equals `2:1:slot0:slot1:i32:true:7`
   - Expected: "unexpected table/global js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("constructs Table and Global objects in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var table = new WebAssembly.Table({ element: 'externref', initial: 1, maximum: 2 }); table.set(0, 'slot0'); var old = table.grow(1, 'slot1'); var global = new WebAssembly.Global({ value: 'i32', mutable: true }, 7); table.length + ':' + old + ':' + table.get(0) + ':' + table.get(1) + ':' + global.valueType + ':' + global.mutable + ':' + global.value")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("2:1:slot0:slot1:i32:true:7")
    Err(err):
        expect("unexpected table/global js error: {err}").to_equal("")
```

</details>

#### exposes instantiated WebAssembly table and global exports in browser scripts

- exposes instantiated WebAssembly table and global exports in browser scripts
   - Expected: _display_js(value) equals `instantiated:table:funcref:null:1:2:global:i32:false:42`
   - Expected: "unexpected instantiated table/global export js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes instantiated WebAssembly table and global exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; WebAssembly.instantiate('0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300').then(function(result) { var table = result.instance.exports.tbl; var global = result.instance.exports.answer; var before = table.get(0); var old = table.grow(1, null); out = result.status + ':' + table.kind + ':' + table.element + ':' + before + ':' + old + ':' + table.length + ':' + global.kind + ':' + global.valueType + ':' + global.mutable + ':' + global.value; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("instantiated:table:funcref:null:1:2:global:i32:false:42")
    Err(err):
        expect("unexpected instantiated table/global export js error: {err}").to_equal("")
```

</details>

#### compares direct and compiled WebAssembly table and global exports in browser scripts

- compares direct and compiled WebAssembly table and global exports in browser scripts
   - Expected: _display_js(value) equals `direct:instantiated:40:table:funcref:null:1:2:global:i32:false:42:compiled:in... (full value in folded executable source)`
   - Expected: "unexpected combined table/global export js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares direct and compiled WebAssembly table and global exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var wasm = '0061736d010000000404017000010606017f00412a0b0710020374626c010006616e737765720300'; WebAssembly.instantiate(wasm).then(function(result) { var table = result.instance.exports.tbl; var global = result.instance.exports.answer; var before = table.get(0); var old = table.grow(1, null); out = 'direct:' + result.status + ':' + result.module.byteLength + ':' + table.kind + ':' + table.element + ':' + before + ':' + old + ':' + table.length + ':' + global.kind + ':' + global.valueType + ':' + global.mutable + ':' + global.value; }); WebAssembly.compile(wasm).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { var table = result.instance.exports.tbl; var global = result.instance.exports.answer; var before = table.get(0); var old = table.grow(1, null); out = out + ':compiled:' + result.status + ':' + result.module.byteLength + ':' + table.kind + ':' + table.element + ':' + before + ':' + old + ':' + table.length + ':' + global.kind + ':' + global.valueType + ':' + global.mutable + ':' + global.value; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("direct:instantiated:40:table:funcref:null:1:2:global:i32:false:42:compiled:instantiated:40:table:funcref:null:1:2:global:i32:false:42")
    Err(err):
        expect("unexpected combined table/global export js error: {err}").to_equal("")
```

</details>

#### exposes instantiated WebAssembly memory exports in browser scripts

- exposes instantiated WebAssembly memory exports in browser scripts
   - Expected: _display_js(value) equals `instantiated:25:131072:65536:4:1:131072:4`
   - Expected: "unexpected instantiated memory export js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("exposes instantiated WebAssembly memory exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; WebAssembly.instantiate('0061736d010000000503010001070a01066d656d6f72790200').then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[3] = 260; var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); out = result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + bytes[3] + ':' + old + ':' + grown.length + ':' + grown[3]; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("instantiated:25:131072:65536:4:1:131072:4")
    Err(err):
        expect("unexpected instantiated memory export js error: {err}").to_equal("")
```

</details>

#### compares direct and compiled WebAssembly memory exports in browser scripts

- compares direct and compiled WebAssembly memory exports in browser scripts
   - Expected: _display_js(value) equals `direct:instantiated:25:131072:65536:4:1:131072:4:compiled:instantiated:25:131... (full value in folded executable source)`
   - Expected: "unexpected combined memory export js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares direct and compiled WebAssembly memory exports in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var wasm = '0061736d010000000503010001070a01066d656d6f72790200'; WebAssembly.instantiate(wasm).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[3] = 260; var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); out = 'direct:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + bytes[3] + ':' + old + ':' + grown.length + ':' + grown[3]; }); WebAssembly.compile(wasm).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[5] = 260; var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); out = out + ':compiled:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + bytes[5] + ':' + old + ':' + grown.length + ':' + grown[5]; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("direct:instantiated:25:131072:65536:4:1:131072:4:compiled:instantiated:25:131072:65536:4:1:131072:4")
    Err(err):
        expect("unexpected combined memory export js error: {err}").to_equal("")
```

</details>

#### compares direct and compiled WebAssembly memory maximum failures in browser scripts

- compares direct and compiled WebAssembly memory maximum failures in browser scripts
   - Expected: _display_js(value) equals `direct:instantiated:26:65536:65536:1:-1:65536:14:compiled:instantiated:26:655... (full value in folded executable source)`
   - Expected: "unexpected direct/compiled memory maximum failure js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("compares direct and compiled WebAssembly memory maximum failures in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var out = ''; var wasm = '0061736d01000000050401010101070a01066d656d6f72790200'; WebAssembly.instantiate(wasm).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[10] = 270; var zero = memory.grow(0); var fail = memory.grow(1); var after = new Uint8Array(memory.buffer); out = 'direct:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + zero + ':' + fail + ':' + after.length + ':' + after[10]; }); WebAssembly.compile(wasm).then(function(module) { return WebAssembly.instantiate(module); }).then(function(result) { var memory = result.instance.exports.memory; var bytes = new Uint8Array(memory.buffer); bytes[11] = 272; var zero = memory.grow(0); var fail = memory.grow(1); var after = new Uint8Array(memory.buffer); out = out + ':compiled:' + result.status + ':' + result.module.byteLength + ':' + memory.byteLength + ':' + memory.pageSize + ':' + zero + ':' + fail + ':' + after.length + ':' + after[11]; }); out")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("direct:instantiated:26:65536:65536:1:-1:65536:14:compiled:instantiated:26:65536:65536:1:-1:65536:16")
    Err(err):
        expect("unexpected direct/compiled memory maximum failure js error: {err}").to_equal("")
```

</details>

#### mutates WebAssembly Global values in browser scripts

- mutates WebAssembly Global values in browser scripts
   - Expected: _display_js(value) equals `7:9:i32:true:3:false`
   - Expected: "unexpected global mutation js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("mutates WebAssembly Global values in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var global = new WebAssembly.Global({ value: 'i32', mutable: true }, 7); var before = global.value; global.value = 9; var immutable = new WebAssembly.Global({ value: 'i32', mutable: false }, 3); before + ':' + global.value + ':' + global.valueType + ':' + global.mutable + ':' + immutable.value + ':' + immutable.mutable")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("7:9:i32:true:3:false")
    Err(err):
        expect("unexpected global mutation js error: {err}").to_equal("")
```

</details>

#### preserves WebAssembly Table slots when grow exceeds maximum

- preserves WebAssembly Table slots when grow exceeds maximum
   - Expected: _display_js(value) equals `2:1:-1:slot0:slot1`
   - Expected: "unexpected table grow limit js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("preserves WebAssembly Table slots when grow exceeds maximum")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var table = new WebAssembly.Table({ element: 'externref', initial: 1, maximum: 2 }); table.set(0, 'slot0'); var old = table.grow(1, 'slot1'); var fail = table.grow(1, 'slot2'); table.length + ':' + old + ':' + fail + ':' + table.get(0) + ':' + table.get(1)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("2:1:-1:slot0:slot1")
    Err(err):
        expect("unexpected table grow limit js error: {err}").to_equal("")
```

</details>

#### fills and searches Uint8Array values in browser scripts

- fills and searches Uint8Array values in browser scripts
   - Expected: _display_js(value) equals `0:4:4:0:1:2:3:true:false`
   - Expected: "unexpected uint8 js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("fills and searches Uint8Array values in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(5); b.fill(260, 1, 4); b[0] + ':' + b[1] + ':' + b[3] + ':' + b[4] + ':' + b.indexOf(4) + ':' + b.indexOf(4, 2) + ':' + b.indexOf(4, -2) + ':' + b.includes(4) + ':' + b.includes(5)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("0:4:4:0:1:2:3:true:false")
    Err(err):
        expect("unexpected uint8 js error: {err}").to_equal("")
```

</details>

#### joins and reverses Uint8Array values in browser scripts

- joins and reverses Uint8Array values in browser scripts
   - Expected: _display_js(value) equals `1-4-255-7:7,255,4,1:7/255/4/1`
   - Expected: "unexpected uint8 join reverse js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("joins and reverses Uint8Array values in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; var before = b.join('-'); var returned = b.reverse(); before + ':' + b.join(',') + ':' + returned.join('/')")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1-4-255-7:7,255,4,1:7/255/4/1")
    Err(err):
        expect("unexpected uint8 join reverse js error: {err}").to_equal("")
```

</details>

#### searches Uint8Array values backward in browser scripts

- searches Uint8Array values backward in browser scripts
   - Expected: _display_js(value) equals `3:2:3:0:-1`
   - Expected: "unexpected uint8 lastIndexOf js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("searches Uint8Array values backward in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(5); b[0] = 4; b[1] = 7; b[2] = 260; b[3] = 4; b[4] = 0; b.lastIndexOf(4) + ':' + b.lastIndexOf(4, 2) + ':' + b.lastIndexOf(4, -2) + ':' + b.lastIndexOf(4, -4) + ':' + b.lastIndexOf(5)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("3:2:3:0:-1")
    Err(err):
        expect("unexpected uint8 lastIndexOf js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype lastIndexOf edge ranges with apply

- dispatches Uint8Array prototype lastIndexOf edge ranges with apply
   - Expected: _display_js(value) equals `4:2:3:-1`
   - Expected: "unexpected uint8 lastIndexOf apply edge dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype lastIndexOf edge ranges with apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(6); b[0] = 4; b[1] = 7; b[2] = 4; b[3] = -1; b[4] = 4; b[5] = 0; Uint8Array.prototype.lastIndexOf.apply(b, [4, -2]) + ':' + Uint8Array.prototype.lastIndexOf.apply(b, [4, -3]) + ':' + Uint8Array.prototype.lastIndexOf.apply(b, [-1, -2]) + ':' + Uint8Array.prototype.lastIndexOf.apply(b, [-1, -4])")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("4:2:3:-1")
    Err(err):
        expect("unexpected uint8 lastIndexOf apply edge dispatch js error: {err}").to_equal("")
```

</details>

#### stringifies Uint8Array values in browser scripts

- stringifies Uint8Array values in browser scripts
   - Expected: _display_js(value) equals `1,4,255,7:1,4,255,7:1|4|255|7`
   - Expected: "unexpected uint8 toString js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("stringifies Uint8Array values in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; b.toString() + ':' + b.toLocaleString() + ':' + b.join('|')")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1,4,255,7:1,4,255,7:1|4|255|7")
    Err(err):
        expect("unexpected uint8 toString js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype string helpers with apply

- dispatches Uint8Array prototype string helpers with apply
   - Expected: _display_js(value) equals `1|4|255|7:1,4,255,7:1,4,255,7:1,4,255,7:1,4,255,7`
   - Expected: "unexpected uint8 string apply dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype string helpers with apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; Uint8Array.prototype.join.apply(b, ['|']) + ':' + Uint8Array.prototype.join.apply(b, []) + ':' + Uint8Array.prototype.toString.apply(b, []) + ':' + Uint8Array.prototype.toLocaleString.call(b) + ':' + Uint8Array.prototype.toLocaleString.apply(b, [])")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1|4|255|7:1,4,255,7:1,4,255,7:1,4,255,7:1,4,255,7")
    Err(err):
        expect("unexpected uint8 string apply dispatch js error: {err}").to_equal("")
```

</details>

#### reads Uint8Array values by relative index in browser scripts

- reads Uint8Array values by relative index in browser scripts
   - Expected: _display_js(value) equals `1:4:7:255:undefined`
   - Expected: "unexpected uint8 at js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("reads Uint8Array values by relative index in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; b.at(0) + ':' + b.at(1) + ':' + b.at(-1) + ':' + b.at(-2) + ':' + b.at(9)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1:4:7:255:undefined")
    Err(err):
        expect("unexpected uint8 at js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype at and reverse with complementary call and apply

- dispatches Uint8Array prototype at and reverse with complementary call and apply
   - Expected: _display_js(value) equals `255:undefined:7,255,4,1:true:7`
   - Expected: "unexpected uint8 at reverse complementary dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype at and reverse with complementary call and apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; var neg = Uint8Array.prototype.at.call(b, -2); var miss = Uint8Array.prototype.at.call(b, 9); var returned = Uint8Array.prototype.reverse.apply(b, []); neg + ':' + miss + ':' + b.toString() + ':' + (returned === b) + ':' + returned.at(0)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("255:undefined:7,255,4,1:true:7")
    Err(err):
        expect("unexpected uint8 at reverse complementary dispatch js error: {err}").to_equal("")
```

</details>

#### copies Uint8Array values within the same storage in browser scripts

- copies Uint8Array values within the same storage in browser scripts
   - Expected: _display_js(value) equals `1,7,9,7,9:7`
   - Expected: "unexpected uint8 copyWithin js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("copies Uint8Array values within the same storage in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(5); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; b[4] = 9; var returned = b.copyWithin(1, -2); b.toString() + ':' + returned.at(1)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1,7,9,7,9:7")
    Err(err):
        expect("unexpected uint8 copyWithin js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype mutating and search helpers with call and apply in browser scripts

- dispatches Uint8Array prototype mutating and search helpers with call and apply in browser scripts
   - Expected: _display_js(value) equals `0-4-4-4-0:true:2:0,4,4,4,0:0,4,0,4,0:4`
   - Expected: "unexpected uint8 prototype helper dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype mutating and search helpers with call and apply in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(5); Uint8Array.prototype.fill.call(b, 260, 1, 4); var search = Uint8Array.prototype.includes.call(b, 4) + ':' + Uint8Array.prototype.indexOf.apply(b, [4, 2]); var joined = Uint8Array.prototype.join.call(b, '-'); var reversed = Uint8Array.prototype.reverse.call(b); var reversedJoin = reversed.join(','); var copied = Uint8Array.prototype.copyWithin.apply(b, [1, -2]); joined + ':' + search + ':' + reversedJoin + ':' + b.join(',') + ':' + copied.at(1)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("0-4-4-4-0:true:2:0,4,4,4,0:0,4,0,4,0:4")
    Err(err):
        expect("unexpected uint8 prototype helper dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype search edge ranges with complementary call and apply

- dispatches Uint8Array prototype search edge ranges with complementary call and apply
   - Expected: _display_js(value) equals `true:false:false:4:3`
   - Expected: "unexpected uint8 search edge complementary dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype search edge ranges with complementary call and apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(6); b[0] = 4; b[1] = 7; b[2] = 4; b[3] = -1; b[4] = 4; b[5] = 0; Uint8Array.prototype.includes.apply(b, [4, -2]) + ':' + Uint8Array.prototype.includes.apply(b, [-1, -2]) + ':' + Uint8Array.prototype.includes.apply(b, [7, -2]) + ':' + Uint8Array.prototype.indexOf.call(b, 4, -3) + ':' + Uint8Array.prototype.indexOf.call(b, -1, -4)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:false:false:4:3")
    Err(err):
        expect("unexpected uint8 search edge complementary dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype fill edge ranges with apply

- dispatches Uint8Array prototype fill edge ranges with apply
   - Expected: _display_js(value) equals `1,254,254,254,5,6:true:254`
   - Expected: "unexpected uint8 fill edge apply dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype fill edge ranges with apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(6); b[0] = 1; b[1] = 2; b[2] = 3; b[3] = 4; b[4] = 5; b[5] = 6; var returned = Uint8Array.prototype.fill.apply(b, [-2, -5, -2]); b.toString() + ':' + (returned === b) + ':' + returned.at(3)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1,254,254,254,5,6:true:254")
    Err(err):
        expect("unexpected uint8 fill edge apply dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype copyWithin edge ranges with call and apply

- dispatches Uint8Array prototype copyWithin edge ranges with call and apply
   - Expected: _display_js(value) equals `1,2,1,2,3,6:2,3,1,2,3,6:true:true`
   - Expected: "unexpected uint8 copyWithin edge dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype copyWithin edge ranges with call and apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(6); b[0] = 1; b[1] = 2; b[2] = 3; b[3] = 4; b[4] = 5; b[5] = 6; var viaCall = Uint8Array.prototype.copyWithin.call(b, -4, 0, 3); var afterCall = b.toString(); var viaApply = Uint8Array.prototype.copyWithin.apply(b, [0, -3, -1]); afterCall + ':' + b.toString() + ':' + (viaCall === b) + ':' + (viaApply === b)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1,2,1,2,3,6:2,3,1,2,3,6:true:true")
    Err(err):
        expect("unexpected uint8 copyWithin edge dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches remaining Uint8Array prototype helpers with call and apply in browser scripts

- dispatches remaining Uint8Array prototype helpers with call and apply in browser scripts
   - Expected: _display_js(value) equals `1,4,255,7,9:undefined:2:7:0,1:0=1,1=4`
   - Expected: "unexpected uint8 remaining prototype helper dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches remaining Uint8Array prototype helpers with call and apply in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var dst = new Uint8Array(5); dst[0] = 1; dst[4] = 9; var src = new Uint8Array(3); src[0] = 260; src[1] = -1; src[2] = 7; var setRet = Uint8Array.prototype.set.call(dst, src, 1); var last = Uint8Array.prototype.lastIndexOf.call(dst, 255); var str = Uint8Array.prototype.toString.call(dst); var at = Uint8Array.prototype.at.apply(dst, [-2]); var keys = Uint8Array.prototype.keys.call(dst); var k0 = keys.next(); var k1 = keys.next(); var entries = Uint8Array.prototype.entries.apply(dst, []); var e0 = entries.next(); var e1 = entries.next(); str + ':' + setRet + ':' + last + ':' + at + ':' + k0.value + ',' + k1.value + ':' + e0.value[0] + '=' + e0.value[1] + ',' + e1.value[0] + '=' + e1.value[1]")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1,4,255,7,9:undefined:2:7:0,1:0=1,1=4")
    Err(err):
        expect("unexpected uint8 remaining prototype helper dispatch js error: {err}").to_equal("")
```

</details>

#### checks Uint8Array values with callback predicates in browser scripts

- checks Uint8Array values with callback predicates in browser scripts
   - Expected: _display_js(value) equals `true:true:false`
   - Expected: "unexpected uint8 callback js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("checks Uint8Array values with callback predicates in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; b.some(function(v, i, arr) { return v == 255 && i == 2 && arr.at(3) == 7; }) + ':' + b.every(function(v) { return v >= 0; }) + ':' + b.every(function(v) { return false; })")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:true:false")
    Err(err):
        expect("unexpected uint8 callback js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype some with call in browser scripts

- dispatches Uint8Array prototype some with call in browser scripts
   - Expected: _display_js(value) equals `true`
   - Expected: "unexpected uint8 prototype some dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype some with call in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; Uint8Array.prototype.some.call(b, function(v, i, arr) { return v == 255 && i == 2 && arr.at(3) == 7; })")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true")
    Err(err):
        expect("unexpected uint8 prototype some dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype every with call in browser scripts

- dispatches Uint8Array prototype every with call in browser scripts
   - Expected: _display_js(value) equals `true:false`
   - Expected: "unexpected uint8 prototype every dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype every with call in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; Uint8Array.prototype.every.call(b, function(v, i, arr) { return arr.at(i) == v && v >= 0; }) + ':' + Uint8Array.prototype.every.call(b, function(v, i, arr) { return i < 2 || arr.at(i) < 10; })")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:false")
    Err(err):
        expect("unexpected uint8 prototype every dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype predicates with apply in browser scripts

- dispatches Uint8Array prototype predicates with apply in browser scripts
   - Expected: _display_js(value) equals `true:false`
   - Expected: "unexpected uint8 prototype predicate apply js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype predicates with apply in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; var some = Uint8Array.prototype.some.apply(b, [function(v, i, arr) { return v == 255 && i == 2 && arr.at(3) == 7; }]); var every = Uint8Array.prototype.every.apply(b, [function(v, i, arr) { return arr.at(i) == v && v < 255; }]); some + ':' + every")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:false")
    Err(err):
        expect("unexpected uint8 prototype predicate apply js error: {err}").to_equal("")
```

</details>

#### iterates Uint8Array values with side-effect callbacks in browser scripts

- iterates Uint8Array values with side-effect callbacks in browser scripts
   - Expected: _display_js(value) equals `266:0=4;1=255;2=7;:undefined`
   - Expected: "unexpected uint8 forEach js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("iterates Uint8Array values with side-effect callbacks in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(3); b[0] = 260; b[1] = -1; b[2] = 7; var total = 0; var seen = ''; var returned = b.forEach(function(v, i, arr) { total = total + v; seen = seen + i + '=' + arr.at(i) + ';'; }); total + ':' + seen + ':' + returned")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("266:0=4;1=255;2=7;:undefined")
    Err(err):
        expect("unexpected uint8 forEach js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype forEach with call and apply in browser scripts

- dispatches Uint8Array prototype forEach with call and apply in browser scripts
   - Expected: _display_js(value) equals `532:0=4;1=255;2=7;:undefined:undefined`
   - Expected: "unexpected uint8 prototype forEach dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype forEach with call and apply in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(3); b[0] = 260; b[1] = -1; b[2] = 7; var total = 0; var seen = ''; var ret1 = Uint8Array.prototype.forEach.call(b, function(v, i, arr) { total = total + v; seen = seen + i + '=' + arr.at(i) + ';'; }); var ret2 = Uint8Array.prototype.forEach.apply(b, [function(v, i, arr) { total = total + arr.at(i); }]); total + ':' + seen + ':' + ret1 + ':' + ret2")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("532:0=4;1=255;2=7;:undefined:undefined")
    Err(err):
        expect("unexpected uint8 prototype forEach dispatch js error: {err}").to_equal("")
```

</details>

#### finds Uint8Array values with callback predicates in browser scripts

- finds Uint8Array values with callback predicates in browser scripts
   - Expected: _display_js(value) equals `4:1:undefined:-1`
   - Expected: "unexpected uint8 find js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("finds Uint8Array values with callback predicates in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; b.find(function(v, i, arr) { return i; }) + ':' + b.findIndex(function(v, i, arr) { return i; }) + ':' + b.find(function(v) { return false; }) + ':' + b.findIndex(function(v) { return false; })")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("4:1:undefined:-1")
    Err(err):
        expect("unexpected uint8 find js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype find helpers with call and apply in browser scripts

- dispatches Uint8Array prototype find helpers with call and apply in browser scripts
   - Expected: _display_js(value) equals `255:3:undefined:-1`
   - Expected: "unexpected uint8 prototype find dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype find helpers with call and apply in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; var found = Uint8Array.prototype.find.call(b, function(v, i, arr) { return v == 255 && i == 2 && arr.at(3) == 7; }); var idx = Uint8Array.prototype.findIndex.call(b, function(v, i, arr) { return arr.at(i) == 7; }); var miss = Uint8Array.prototype.find.apply(b, [function(v) { return v == 99; }]); var missIdx = Uint8Array.prototype.findIndex.apply(b, [function(v) { return v == 99; }]); found + ':' + idx + ':' + miss + ':' + missIdx")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("255:3:undefined:-1")
    Err(err):
        expect("unexpected uint8 prototype find dispatch js error: {err}").to_equal("")
```

</details>

#### finds Uint8Array values from the end with callback predicates in browser scripts

- finds Uint8Array values from the end with callback predicates in browser scripts
   - Expected: _display_js(value) equals `4:4:undefined:-1`
   - Expected: "unexpected uint8 findLast js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("finds Uint8Array values from the end with callback predicates in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(5); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; b[4] = 260; b.findLast(function(v, i, arr) { return v == 4 && arr.at(i) == 4; }) + ':' + b.findLastIndex(function(v, i, arr) { return v == 4 && arr.at(i) == 4; }) + ':' + b.findLast(function(v) { return v == 99; }) + ':' + b.findLastIndex(function(v) { return v == 99; })")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("4:4:undefined:-1")
    Err(err):
        expect("unexpected uint8 findLast js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype findLast helpers with call and apply in browser scripts

- dispatches Uint8Array prototype findLast helpers with call and apply in browser scripts
   - Expected: _display_js(value) equals `4:4:undefined:-1`
   - Expected: "unexpected uint8 prototype findLast dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype findLast helpers with call and apply in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(5); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; b[4] = 260; var found = Uint8Array.prototype.findLast.call(b, function(v, i, arr) { return v == 4 && i > 1 && arr.at(2) == 255; }); var idx = Uint8Array.prototype.findLastIndex.call(b, function(v, i, arr) { return v == 4 && i > 1 && arr.at(i) == 4; }); var miss = Uint8Array.prototype.findLast.apply(b, [function(v) { return v == 99; }]); var missIdx = Uint8Array.prototype.findLastIndex.apply(b, [function(v) { return v == 99; }]); found + ':' + idx + ':' + miss + ':' + missIdx")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("4:4:undefined:-1")
    Err(err):
        expect("unexpected uint8 prototype findLast dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype transform helpers with apply in browser scripts

- dispatches Uint8Array prototype transform helpers with apply in browser scripts
   - Expected: _display_js(value) equals `8,255,16,19:255,7,8:554:19016511008:4,255,7,8`
   - Expected: "unexpected uint8 prototype transform apply dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype transform helpers with apply in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 260; b[1] = -1; b[2] = 7; b[3] = 8; var mapped = Uint8Array.prototype.map.apply(b, [function(v, i, arr) { return v + i + arr.at(i); }]); var filtered = Uint8Array.prototype.filter.apply(b, [function(v, i, arr) { return i && arr.at(0) == 4; }]); var reduced = Uint8Array.prototype.reduce.apply(b, [function(acc, v, i, arr) { return acc + v + i + arr.at(i); }, 0]); var reducedRight = Uint8Array.prototype.reduceRight.apply(b, [function(acc, v, i, arr) { return acc * 1000 + v + i + arr.at(i); }, 0]); mapped.toString() + ':' + filtered.toString() + ':' + reduced + ':' + reducedRight + ':' + b.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("8,255,16,19:255,7,8:554:19016511008:4,255,7,8")
    Err(err):
        expect("unexpected uint8 prototype transform apply dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array callback helpers over nonzero-offset views

- dispatches Uint8Array callback helpers over nonzero-offset views
   - Expected: _display_js(value) equals `true:2:4:0=4:true;1=255:true;2=7:true;3=8:true;:0:false:8,255,16,19:0:false:2... (full value in folded executable source)`
   - Expected: "unexpected uint8 view callback helper js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array callback helpers over nonzero-offset views")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var base = new Uint8Array(7); base[0] = 10; base[1] = 20; base[2] = 260; base[3] = -1; base[4] = 7; base[5] = 8; base[6] = 9; var view = base.subarray(2, 6); var seen = ''; var mapped = view.map(function(v, i, arr) { seen = seen + i + '=' + v + ':' + (arr === view) + ';'; return v + i + arr.at(i); }); var filtered = Uint8Array.prototype.filter.call(view, function(v, i, arr) { return i > 0 && arr.at(0) == 4; }); var reduced = Uint8Array.prototype.reduce.call(view, function(acc, v, i, arr) { return acc + v + i + arr.at(i); }, 0); var reducedRight = Uint8Array.prototype.reduceRight.call(view, function(acc, v, i, arr) { return acc + v + i + arr.at(i); }, 0); var some = Uint8Array.prototype.some.call(view, function(v, i, arr) { return i == 1 && v == 255 && arr === view; }); var every = Uint8Array.prototype.every.call(view, function(v, i, arr) { return arr.at(i) == v; }); var found = Uint8Array.prototype.find.call(view, function(v, i, arr) { return i == 2 && arr.at(i) == 7; }); var foundIndex = Uint8Array.prototype.findIndex.call(view, function(v, i, arr) { return i == 3 && v == 8; }); var each = ''; var eachTotal = 0; var eachReturned = Uint8Array.prototype.forEach.call(view, function(v, i, arr) { eachTotal = eachTotal + v; each = each + i + '=' + arr.at(i) + ':' + (arr === view) + ';'; }); base[3] = 99; (view.buffer === base.buffer) + ':' + view.byteOffset + ':' + view.length + ':' + seen + ':' + mapped.byteOffset + ':' + (mapped.buffer === view.buffer) + ':' + mapped.toString() + ':' + filtered.byteOffset + ':' + (filtered.buffer === view.buffer) + ':' + filtered.toString() + ':' + reduced + ':' + reducedRight + ':' + some + ':' + every + ':' + found + ':' + foundIndex + ':' + eachTotal + ':' + each + ':' + eachReturned + ':' + view.toString() + ':' + base.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:2:4:0=4:true;1=255:true;2=7:true;3=8:true;:0:false:8,255,16,19:0:false:255,7,8:554:554:true:true:7:3:274:0=4:true;1=255:true;2=7:true;3=8:true;:undefined:4,99,7,8:10,20,4,99,7,8,9")
    Err(err):
        expect("unexpected uint8 view callback helper js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array reverse search helpers over nonzero-offset views

- dispatches Uint8Array reverse search helpers over nonzero-offset views
   - Expected: _display_js(value) equals `true:2:5:4:3:255:4:undefined:-1:4,255,7,44,8:10,20,4,255,7,44,8,90`
   - Expected: "unexpected uint8 view reverse search helper js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array reverse search helpers over nonzero-offset views")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var base = new Uint8Array(8); base[0] = 10; base[1] = 20; base[2] = 260; base[3] = -1; base[4] = 7; base[5] = 260; base[6] = 8; base[7] = 90; var view = base.subarray(2, 7); var direct = view.findLast(function(v, i, arr) { return v == 4 && arr === view && arr.at(i) == 4; }); var directIndex = view.findLastIndex(function(v, i, arr) { return v == 4 && arr === view && arr.at(i) == 4; }); var callFound = Uint8Array.prototype.findLast.call(view, function(v, i, arr) { return i < 4 && v == 255 && arr.at(i) == 255; }); var applyIndex = Uint8Array.prototype.findLastIndex.apply(view, [function(v, i, arr) { return v == 8 && arr.at(i) == 8; }]); var miss = Uint8Array.prototype.findLast.apply(view, [function(v) { return v == 99; }]); var missIndex = Uint8Array.prototype.findLastIndex.call(view, function(v) { return v == 99; }); base[5] = 44; (view.buffer === base.buffer) + ':' + view.byteOffset + ':' + view.length + ':' + direct + ':' + directIndex + ':' + callFound + ':' + applyIndex + ':' + miss + ':' + missIndex + ':' + view.toString() + ':' + base.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:2:5:4:3:255:4:undefined:-1:4,255,7,44,8:10,20,4,255,7,44,8,90")
    Err(err):
        expect("unexpected uint8 view reverse search helper js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array mutating and search helpers over nonzero-offset views

- dispatches Uint8Array mutating and search helpers over nonzero-offset views
   - Expected: _display_js(value) equals `true:2:5:true:true:1:2:3:0:true:true:true:70,4,70,4,4:10,20,70,4,70,4,4,80`
   - Expected: "unexpected uint8 view mutating/search helper js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array mutating and search helpers over nonzero-offset views")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var base = new Uint8Array(8); base[0] = 10; base[1] = 20; base[2] = 30; base[3] = 40; base[4] = 50; base[5] = 60; base[6] = 70; base[7] = 80; var view = base.subarray(2, 7); var fillRet = Uint8Array.prototype.fill.call(view, 260, 1, 4); var includes = Uint8Array.prototype.includes.call(view, 4); var first = Uint8Array.prototype.indexOf.call(view, 4); var second = Uint8Array.prototype.indexOf.apply(view, [4, 2]); var last = Uint8Array.prototype.lastIndexOf.call(view, 4); var lastFrom = Uint8Array.prototype.lastIndexOf.apply(view, [30, -4]); var tail = Uint8Array.prototype.includes.apply(view, [70, -1]); var copyRet = Uint8Array.prototype.copyWithin.apply(view, [0, 2, 5]); var revRet = Uint8Array.prototype.reverse.call(view); (view.buffer === base.buffer) + ':' + view.byteOffset + ':' + view.length + ':' + (fillRet === view) + ':' + includes + ':' + first + ':' + second + ':' + last + ':' + lastFrom + ':' + tail + ':' + (copyRet === view) + ':' + (revRet === view) + ':' + view.toString() + ':' + base.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:2:5:true:true:1:2:3:0:true:true:true:70,4,70,4,4:10,20,70,4,70,4,4,80")
    Err(err):
        expect("unexpected uint8 view mutating/search helper js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array sort and iterators over nonzero-offset views

- dispatches Uint8Array sort and iterators over nonzero-offset views
   - Expected: _display_js(value) equals `true:2:5:30:true:true:0,1:255,30:0=255:255,30:255,30,8,7,4:10,20,255,30,8,7,4,90`
   - Expected: "unexpected uint8 view sort/iterator helper js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array sort and iterators over nonzero-offset views")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var base = new Uint8Array(8); base[0] = 10; base[1] = 20; base[2] = 30; base[3] = 260; base[4] = -1; base[5] = 7; base[6] = 8; base[7] = 90; var view = base.subarray(2, 7); var firstValue = Uint8Array.prototype.values.call(view).next(); var sortRet = Uint8Array.prototype.sort.call(view); var cmpRet = Uint8Array.prototype.sort.apply(view, [function(x, y) { return y - x; }]); var keys = Uint8Array.prototype.keys.call(view); var k0 = keys.next(); var k1 = keys.next(); var values = Uint8Array.prototype.values.apply(view, []); var v0 = values.next(); var v1 = values.next(); var entries = Uint8Array.prototype.entries.call(view); var e0 = entries.next(); var sym = Uint8Array.prototype[Symbol.iterator].apply(view, []); var s0 = sym.next(); var s1 = sym.next(); (view.buffer === base.buffer) + ':' + view.byteOffset + ':' + view.length + ':' + firstValue.value + ':' + (sortRet === view) + ':' + (cmpRet === view) + ':' + k0.value + ',' + k1.value + ':' + v0.value + ',' + v1.value + ':' + e0.value[0] + '=' + e0.value[1] + ':' + s0.value + ',' + s1.value + ':' + view.toString() + ':' + base.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:2:5:30:true:true:0,1:255,30:0=255:255,30:255,30,8,7,4:10,20,255,30,8,7,4,90")
    Err(err):
        expect("unexpected uint8 view sort/iterator helper js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array range and string helpers over nonzero-offset views

- dispatches Uint8Array range and string helpers over nonzero-offset views
   - Expected: _display_js(value) equals `true:2:5:5:30-4-255-7-8:30,4,255,7,8:30:8:255:undefined:true:3:3:4,255,7:4,99... (full value in folded executable source)`
   - Expected: "unexpected uint8 view range/string helper js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array range and string helpers over nonzero-offset views")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var base = new Uint8Array(8); base[0] = 10; base[1] = 20; base[2] = 30; base[3] = 260; base[4] = -1; base[5] = 7; base[6] = 8; base[7] = 90; var view = base.subarray(2, 7); var joined = view.join('-'); var str = view.toString(); var a0 = view.at(0); var an1 = view.at(-1); var an3 = view.at(-3); var miss = view.at(9); var nested = view.subarray(1, -1); var nestedBefore = nested.toString(); nested[1] = 99; (view.buffer === base.buffer) + ':' + view.byteOffset + ':' + view.byteLength + ':' + view.length + ':' + joined + ':' + str + ':' + a0 + ':' + an1 + ':' + an3 + ':' + miss + ':' + (nested.buffer === view.buffer) + ':' + nested.byteOffset + ':' + nested.length + ':' + nestedBefore + ':' + nested.toString() + ':' + view.toString() + ':' + base.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:2:5:5:30-4-255-7-8:30,4,255,7,8:30:8:255:undefined:true:3:3:4,255,7:4,99,7:30,4,99,7,8:10,20,30,4,99,7,8,90")
    Err(err):
        expect("unexpected uint8 view range/string helper js error: {err}").to_equal("")
```

</details>

#### reduces Uint8Array values with accumulator callbacks in browser scripts

- reduces Uint8Array values with accumulator callbacks in browser scripts
   - Expected: _display_js(value) equals `535`
   - Expected: "unexpected uint8 reduce js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("reduces Uint8Array values with accumulator callbacks in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(3); b[0] = 260; b[1] = -1; b[2] = 7; b.reduce(function(acc, v, i, arr) { return acc + v + i + arr.at(i); }, 0)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("535")
    Err(err):
        expect("unexpected uint8 reduce js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype reduce with call in browser scripts

- dispatches Uint8Array prototype reduce with call in browser scripts
   - Expected: _display_js(value) equals `535`
   - Expected: "unexpected uint8 prototype reduce dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype reduce with call in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(3); b[0] = 260; b[1] = -1; b[2] = 7; Uint8Array.prototype.reduce.call(b, function(acc, v, i, arr) { return acc + v + i + arr.at(i); }, 0)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("535")
    Err(err):
        expect("unexpected uint8 prototype reduce dispatch js error: {err}").to_equal("")
```

</details>

#### reduces Uint8Array values from the right in browser scripts

- reduces Uint8Array values from the right in browser scripts
   - Expected: _display_js(value) equals `16511008`
   - Expected: "unexpected uint8 reduceRight js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("reduces Uint8Array values from the right in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(3); b[0] = 260; b[1] = -1; b[2] = 7; b.reduceRight(function(acc, v, i, arr) { return acc * 1000 + v + i + arr.at(i); }, 0)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("16511008")
    Err(err):
        expect("unexpected uint8 reduceRight js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype reduceRight with call in browser scripts

- dispatches Uint8Array prototype reduceRight with call in browser scripts
   - Expected: _display_js(value) equals `16511008`
   - Expected: "unexpected uint8 prototype reduceRight dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype reduceRight with call in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(3); b[0] = 260; b[1] = -1; b[2] = 7; Uint8Array.prototype.reduceRight.call(b, function(acc, v, i, arr) { return acc * 1000 + v + i + arr.at(i); }, 0)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("16511008")
    Err(err):
        expect("unexpected uint8 prototype reduceRight dispatch js error: {err}").to_equal("")
```

</details>

#### constructs Uint8Array values from indexed sources in browser scripts

- constructs Uint8Array values from indexed sources in browser scripts
   - Expected: _display_js(value) equals `4,255:4,255:4,0,9:3:0`
   - Expected: "unexpected uint8 from js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("constructs Uint8Array values from indexed sources in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var src = new Uint8Array(2); src[0] = 260; src[1] = -1; var copied = Uint8Array.from(src); var mapped = Uint8Array.from([260, -1, 7], function(v, i) { return v + i; }); copied.toString() + ':' + src.toString() + ':' + mapped.toString() + ':' + mapped.length + ':' + mapped.at(1)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("4,255:4,255:4,0,9:3:0")
    Err(err):
        expect("unexpected uint8 from js error: {err}").to_equal("")
```

</details>

#### constructs Uint8Array values from nonzero-offset views with mapper copies

- constructs Uint8Array values from nonzero-offset views with mapper copies
   - Expected: _display_js(value) equals `true:2:3:false:0:3:99,255,16:4,44,7:10,20,4,44,7,8`
   - Expected: "unexpected uint8 from view mapper js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("constructs Uint8Array values from nonzero-offset views with mapper copies")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var base = new Uint8Array(6); base[0] = 10; base[1] = 20; base[2] = 260; base[3] = -1; base[4] = 7; base[5] = 8; var view = base.subarray(2, 5); var copy = Uint8Array.from(view, function(v, i) { return v + i + view.at(i); }); copy[0] = 99; base[3] = 44; (view.buffer === base.buffer) + ':' + view.byteOffset + ':' + view.length + ':' + (copy.buffer === view.buffer) + ':' + copy.byteOffset + ':' + copy.byteLength + ':' + copy.toString() + ':' + view.toString() + ':' + base.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:2:3:false:0:3:99,255,16:4,44,7:10,20,4,44,7,8")
    Err(err):
        expect("unexpected uint8 from view mapper js error: {err}").to_equal("")
```

</details>

#### constructs Uint8Array values from varargs in browser scripts

- constructs Uint8Array values from varargs in browser scripts
   - Expected: _display_js(value) equals `4,255,7:3:4:255:0:`
   - Expected: "unexpected uint8 of js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("constructs Uint8Array values from varargs in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var made = Uint8Array.of(260, -1, 7); var empty = Uint8Array.of(); made.toString() + ':' + made.length + ':' + made.at(0) + ':' + made.at(1) + ':' + empty.length + ':' + empty.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("4,255,7:3:4:255:0:")
    Err(err):
        expect("unexpected uint8 of js error: {err}").to_equal("")
```

</details>

#### maps Uint8Array values into a new coerced array in browser scripts

- maps Uint8Array values into a new coerced array in browser scripts
   - Expected: _display_js(value) equals `8,255,16:4,255,7:3:255`
   - Expected: "unexpected uint8 map js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("maps Uint8Array values into a new coerced array in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(3); b[0] = 260; b[1] = -1; b[2] = 7; var mapped = b.map(function(v, i, arr) { return v + i + arr.at(i); }); mapped.toString() + ':' + b.toString() + ':' + mapped.length + ':' + mapped.at(1)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("8,255,16:4,255,7:3:255")
    Err(err):
        expect("unexpected uint8 map js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype map with call in browser scripts

- dispatches Uint8Array prototype map with call in browser scripts
   - Expected: _display_js(value) equals `8,255,16:4,255,7:3:255`
   - Expected: "unexpected uint8 prototype map dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype map with call in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(3); b[0] = 260; b[1] = -1; b[2] = 7; var mapped = Uint8Array.prototype.map.call(b, function(v, i, arr) { return v + i + arr.at(i); }); mapped.toString() + ':' + b.toString() + ':' + mapped.length + ':' + mapped.at(1)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("8,255,16:4,255,7:3:255")
    Err(err):
        expect("unexpected uint8 prototype map dispatch js error: {err}").to_equal("")
```

</details>

#### filters Uint8Array values into a new coerced array in browser scripts

- filters Uint8Array values into a new coerced array in browser scripts
   - Expected: _display_js(value) equals `255,7,8:4,255,7,8:3:7`
   - Expected: "unexpected uint8 filter js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("filters Uint8Array values into a new coerced array in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 260; b[1] = -1; b[2] = 7; b[3] = 8; var filtered = b.filter(function(v, i) { return i; }); filtered.toString() + ':' + b.toString() + ':' + filtered.length + ':' + filtered.at(1)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("255,7,8:4,255,7,8:3:7")
    Err(err):
        expect("unexpected uint8 filter js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype filter with call in browser scripts

- dispatches Uint8Array prototype filter with call in browser scripts
   - Expected: _display_js(value) equals `255,7,8:4,255,7,8:3:7`
   - Expected: "unexpected uint8 prototype filter dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype filter with call in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 260; b[1] = -1; b[2] = 7; b[3] = 8; var filtered = Uint8Array.prototype.filter.call(b, function(v, i, arr) { return i && arr.at(0) == 4; }); filtered.toString() + ':' + b.toString() + ':' + filtered.length + ':' + filtered.at(1)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("255,7,8:4,255,7,8:3:7")
    Err(err):
        expect("unexpected uint8 prototype filter dispatch js error: {err}").to_equal("")
```

</details>

#### slices Uint8Array ranges into returned typed arrays in browser scripts

- slices Uint8Array ranges into returned typed arrays in browser scripts
   - Expected: _display_js(value) equals `4,255,7:3:4:255,7:2:1,4,255,7,9`
   - Expected: "unexpected uint8 slice js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("slices Uint8Array ranges into returned typed arrays in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(5); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; b[4] = 9; var sub = b.subarray(1, -1); var sl = b.slice(-3, -1); sub.toString() + ':' + sub.length + ':' + sub.at(0) + ':' + sl.toString() + ':' + sl.length + ':' + b.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("4,255,7:3:4:255,7:2:1,4,255,7,9")
    Err(err):
        expect("unexpected uint8 slice js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype range helpers with complementary call and apply

- dispatches Uint8Array prototype range helpers with complementary call and apply
   - Expected: _display_js(value) equals `4,255,7:3:4:255,7,8:3:1,4,255,7,8,9`
   - Expected: "unexpected uint8 range complementary dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype range helpers with complementary call and apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(6); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; b[4] = 8; b[5] = 9; var sub = Uint8Array.prototype.subarray.apply(b, [-5, -2]); var sl = Uint8Array.prototype.slice.call(b, -4, -1); sub.toString() + ':' + sub.length + ':' + sub.at(0) + ':' + sl.toString() + ':' + sl.length + ':' + b.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("4,255,7:3:4:255,7,8:3:1,4,255,7,8,9")
    Err(err):
        expect("unexpected uint8 range complementary dispatch js error: {err}").to_equal("")
```

</details>

#### sets Uint8Array ranges from another typed array in browser scripts

- sets Uint8Array ranges from another typed array in browser scripts
   - Expected: _display_js(value) equals `1,4,255,7,9:undefined:5:4,255,7`
   - Expected: "unexpected uint8 set js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("sets Uint8Array ranges from another typed array in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var dst = new Uint8Array(5); dst[0] = 1; dst[4] = 9; var src = new Uint8Array(3); src[0] = 260; src[1] = -1; src[2] = 7; var returned = dst.set(src, 1); dst.toString() + ':' + returned + ':' + dst.length + ':' + src.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1,4,255,7,9:undefined:5:4,255,7")
    Err(err):
        expect("unexpected uint8 set js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype set with apply in browser scripts

- dispatches Uint8Array prototype set with apply in browser scripts
   - Expected: _display_js(value) equals `1,2,4,255,8,6:undefined:4,255,8:6`
   - Expected: "unexpected uint8 set apply dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype set with apply in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var dst = new Uint8Array(6); dst[0] = 1; dst[1] = 2; dst[2] = 3; dst[3] = 4; dst[4] = 5; dst[5] = 6; var src = new Uint8Array(3); src[0] = 260; src[1] = -1; src[2] = 8; var returned = Uint8Array.prototype.set.apply(dst, [src, 2]); dst.toString() + ':' + returned + ':' + src.toString() + ':' + dst.length")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1,2,4,255,8,6:undefined:4,255,8:6")
    Err(err):
        expect("unexpected uint8 set apply dispatch js error: {err}").to_equal("")
```

</details>

#### sorts Uint8Array values numerically in browser scripts

- sorts Uint8Array values numerically in browser scripts
   - Expected: _display_js(value) equals `1,4,7,255:1,4,7,255:1:255`
   - Expected: "unexpected uint8 sort js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("sorts Uint8Array values numerically in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 260; b[1] = -1; b[2] = 7; b[3] = 1; var returned = b.sort(); returned.toString() + ':' + b.toString() + ':' + returned.at(0) + ':' + returned.at(3)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1,4,7,255:1,4,7,255:1:255")
    Err(err):
        expect("unexpected uint8 sort js error: {err}").to_equal("")
```

</details>

#### sorts Uint8Array values with comparator callbacks in browser scripts

- sorts Uint8Array values with comparator callbacks in browser scripts
   - Expected: _display_js(value) equals `255,7,4,1:255,7,4,1:255:1`
   - Expected: "unexpected uint8 comparator sort js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("sorts Uint8Array values with comparator callbacks in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; var returned = b.sort(function(x, y) { return y - x; }); returned.toString() + ':' + b.toString() + ':' + returned.at(0) + ':' + returned.at(3)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("255,7,4,1:255,7,4,1:255:1")
    Err(err):
        expect("unexpected uint8 comparator sort js error: {err}").to_equal("")
```

</details>

#### copies Uint8Array values in reverse order without mutating source

- copies Uint8Array values in reverse order without mutating source
   - Expected: _display_js(value) equals `9,255,4,1:1,4,255,7:false:false:0:4`
   - Expected: "unexpected uint8 toReversed js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("copies Uint8Array values in reverse order without mutating source")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; var copy = b.toReversed(); copy[0] = 9; copy.toString() + ':' + b.toString() + ':' + (copy === b) + ':' + (copy.buffer === b.buffer) + ':' + copy.byteOffset + ':' + copy.byteLength")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("9,255,4,1:1,4,255,7:false:false:0:4")
    Err(err):
        expect("unexpected uint8 toReversed js error: {err}").to_equal("")
```

</details>

#### copies Uint8Array values in sorted order without mutating source

- copies Uint8Array values in sorted order without mutating source
   - Expected: _display_js(value) equals `9,4,7,255:4,255,7,1:false:false:0:4`
   - Expected: "unexpected uint8 toSorted js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("copies Uint8Array values in sorted order without mutating source")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 260; b[1] = -1; b[2] = 7; b[3] = 1; var copy = b.toSorted(); copy[0] = 9; copy.toString() + ':' + b.toString() + ':' + (copy === b) + ':' + (copy.buffer === b.buffer) + ':' + copy.byteOffset + ':' + copy.byteLength")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("9,4,7,255:4,255,7,1:false:false:0:4")
    Err(err):
        expect("unexpected uint8 toSorted js error: {err}").to_equal("")
```

</details>

#### copies Uint8Array comparator sorted values without mutating source

- copies Uint8Array comparator sorted values without mutating source
   - Expected: _display_js(value) equals `255,7,4,1:1,4,255,7:false:false`
   - Expected: "unexpected uint8 comparator toSorted js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("copies Uint8Array comparator sorted values without mutating source")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; var copy = b.toSorted(function(x, y) { return y - x; }); copy.toString() + ':' + b.toString() + ':' + (copy === b) + ':' + (copy.buffer === b.buffer)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("255,7,4,1:1,4,255,7:false:false")
    Err(err):
        expect("unexpected uint8 comparator toSorted js error: {err}").to_equal("")
```

</details>

#### copies Uint8Array values with one replaced index without mutating source

- copies Uint8Array values with one replaced index without mutating source
   - Expected: _display_js(value) equals `9,4,3:1,2,3:false:false:0:3`
   - Expected: "unexpected uint8 with js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("copies Uint8Array values with one replaced index without mutating source")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(3); b[0] = 1; b[1] = 2; b[2] = 3; var copy = b.with(1, 260); copy[0] = 9; copy.toString() + ':' + b.toString() + ':' + (copy === b) + ':' + (copy.buffer === b.buffer) + ':' + copy.byteOffset + ':' + copy.byteLength")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("9,4,3:1,2,3:false:false:0:3")
    Err(err):
        expect("unexpected uint8 with js error: {err}").to_equal("")
```

</details>

#### copies Uint8Array values with a negative replacement index

- copies Uint8Array values with a negative replacement index
   - Expected: _display_js(value) equals `1,2,255:1,2,3:255`
   - Expected: "unexpected uint8 negative with js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("copies Uint8Array values with a negative replacement index")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(3); b[0] = 1; b[1] = 2; b[2] = 3; var copy = b.with(-1, -1); copy.toString() + ':' + b.toString() + ':' + copy.at(-1)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1,2,255:1,2,3:255")
    Err(err):
        expect("unexpected uint8 negative with js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype sort with call in browser scripts

- dispatches Uint8Array prototype sort with call in browser scripts
   - Expected: _display_js(value) equals `1,4,7,255:1,4,7,255:1:255`
   - Expected: "unexpected uint8 prototype sort dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype sort with call in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 260; b[1] = -1; b[2] = 7; b[3] = 1; var returned = Uint8Array.prototype.sort.call(b); returned.toString() + ':' + b.toString() + ':' + returned.at(0) + ':' + returned.at(3)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1,4,7,255:1,4,7,255:1:255")
    Err(err):
        expect("unexpected uint8 prototype sort dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype sort with apply in browser scripts

- dispatches Uint8Array prototype sort with apply in browser scripts
   - Expected: _display_js(value) equals `1,4,7,255:1,4,7,255:1:255`
   - Expected: "unexpected uint8 prototype sort apply dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype sort with apply in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = -1; b[1] = 1; b[2] = 260; b[3] = 7; var returned = Uint8Array.prototype.sort.apply(b, []); returned.toString() + ':' + b.toString() + ':' + returned.at(0) + ':' + returned.at(3)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1,4,7,255:1,4,7,255:1:255")
    Err(err):
        expect("unexpected uint8 prototype sort apply dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype comparator sort with call in browser scripts

- dispatches Uint8Array prototype comparator sort with call in browser scripts
   - Expected: _display_js(value) equals `255,7,4,1:255,7,4,1:255:1`
   - Expected: "unexpected uint8 prototype comparator sort dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype comparator sort with call in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; var returned = Uint8Array.prototype.sort.call(b, function(x, y) { return y - x; }); returned.toString() + ':' + b.toString() + ':' + returned.at(0) + ':' + returned.at(3)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("255,7,4,1:255,7,4,1:255:1")
    Err(err):
        expect("unexpected uint8 prototype comparator sort dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype comparator sort with apply in browser scripts

- dispatches Uint8Array prototype comparator sort with apply in browser scripts
   - Expected: _display_js(value) equals `255,7,4,1:255,7,4,1:255:1`
   - Expected: "unexpected uint8 prototype comparator sort apply dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype comparator sort with apply in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 1; b[1] = 260; b[2] = -1; b[3] = 7; var returned = Uint8Array.prototype.sort.apply(b, [function(x, y) { return y - x; }]); returned.toString() + ':' + b.toString() + ':' + returned.at(0) + ':' + returned.at(3)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("255,7,4,1:255,7,4,1:255:1")
    Err(err):
        expect("unexpected uint8 prototype comparator sort apply dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype toReversed with call and apply

- dispatches Uint8Array prototype toReversed with call and apply
   - Expected: _display_js(value) equals `11,12,255,4,3:11,8,13,4,3:3,4,255,8,11:false:false:false:false`
   - Expected: "unexpected uint8 prototype toReversed dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype toReversed with call and apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(5); b[0] = 3; b[1] = 260; b[2] = -1; b[3] = 8; b[4] = 11; var callCopy = Uint8Array.prototype.toReversed.call(b); var applyCopy = Uint8Array.prototype.toReversed.apply(b, []); callCopy[1] = 12; applyCopy[2] = 13; callCopy.toString() + ':' + applyCopy.toString() + ':' + b.toString() + ':' + (callCopy === b) + ':' + (applyCopy === b) + ':' + (callCopy.buffer === b.buffer) + ':' + (applyCopy.buffer === b.buffer)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("11,12,255,4,3:11,8,13,4,3:3,4,255,8,11:false:false:false:false")
    Err(err):
        expect("unexpected uint8 prototype toReversed dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype toSorted with call and apply

- dispatches Uint8Array prototype toSorted with call and apply
   - Expected: _display_js(value) equals `3,12,8,11,255:255,11,13,4,3:3,4,255,8,11:false:false:false:false`
   - Expected: "unexpected uint8 prototype toSorted dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype toSorted with call and apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(5); b[0] = 3; b[1] = 260; b[2] = -1; b[3] = 8; b[4] = 11; var callCopy = Uint8Array.prototype.toSorted.call(b); var applyCopy = Uint8Array.prototype.toSorted.apply(b, [function(x, y) { return y - x; }]); callCopy[1] = 12; applyCopy[2] = 13; callCopy.toString() + ':' + applyCopy.toString() + ':' + b.toString() + ':' + (callCopy === b) + ':' + (applyCopy === b) + ':' + (callCopy.buffer === b.buffer) + ':' + (applyCopy.buffer === b.buffer)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("3,12,8,11,255:255,11,13,4,3:3,4,255,8,11:false:false:false:false")
    Err(err):
        expect("unexpected uint8 prototype toSorted dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype with with call and apply

- dispatches Uint8Array prototype with with call and apply
   - Expected: _display_js(value) equals `7,12,255,8:3,4,11,13:3,4,255,8:false:false:false:false`
   - Expected: "unexpected uint8 prototype with dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype with with call and apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(4); b[0] = 3; b[1] = 260; b[2] = -1; b[3] = 8; var callCopy = Uint8Array.prototype.with.call(b, 0, 7); var applyCopy = Uint8Array.prototype.with.apply(b, [-2, 11]); callCopy[1] = 12; applyCopy[3] = 13; callCopy.toString() + ':' + applyCopy.toString() + ':' + b.toString() + ':' + (callCopy === b) + ':' + (applyCopy === b) + ':' + (callCopy.buffer === b.buffer) + ':' + (applyCopy.buffer === b.buffer)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("7,12,255,8:3,4,11,13:3,4,255,8:false:false:false:false")
    Err(err):
        expect("unexpected uint8 prototype with dispatch js error: {err}").to_equal("")
```

</details>

#### iterates Uint8Array keys and values in browser scripts

- iterates Uint8Array keys and values in browser scripts
   - Expected: _display_js(value) equals `0:1:true:4:255:true`
   - Expected: "unexpected uint8 iterator js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("iterates Uint8Array keys and values in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(2); b[0] = 260; b[1] = -1; var keys = b.keys(); var values = b.values(); var k0 = keys.next(); var k1 = keys.next(); var k2 = keys.next(); var v0 = values.next(); var v1 = values.next(); var v2 = values.next(); k0.value + ':' + k1.value + ':' + k2.done + ':' + v0.value + ':' + v1.value + ':' + v2.done")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("0:1:true:4:255:true")
    Err(err):
        expect("unexpected uint8 iterator js error: {err}").to_equal("")
```

</details>

#### iterates Uint8Array entries in browser scripts

- iterates Uint8Array entries in browser scripts
   - Expected: _display_js(value) equals `0=4:1=255:true`
   - Expected: "unexpected uint8 entries iterator js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("iterates Uint8Array entries in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(2); b[0] = 260; b[1] = -1; var entries = b.entries(); var e0 = entries.next(); var e1 = entries.next(); var e2 = entries.next(); e0.value[0] + '=' + e0.value[1] + ':' + e1.value[0] + '=' + e1.value[1] + ':' + e2.done")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("0=4:1=255:true")
    Err(err):
        expect("unexpected uint8 entries iterator js error: {err}").to_equal("")
```

</details>

#### iterates Uint8Array values through Symbol iterator in browser scripts

- iterates Uint8Array values through Symbol iterator in browser scripts
   - Expected: _display_js(value) equals `function:function:4:255`
   - Expected: "unexpected uint8 symbol iterator js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("iterates Uint8Array values through Symbol iterator in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(2); b[0] = 260; b[1] = -1; var iterator = b[Symbol.iterator](); var iter2 = iterator[Symbol.iterator](); var i0 = iter2.next(); var i1 = iterator.next(); typeof b[Symbol.iterator] + ':' + typeof iterator[Symbol.iterator] + ':' + i0.value + ':' + i1.value")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("function:function:4:255")
    Err(err):
        expect("unexpected uint8 symbol iterator js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype iterators with complementary call and apply

- dispatches Uint8Array prototype iterators with complementary call and apply
   - Expected: _display_js(value) equals `0:1:4:255:0=4:4`
   - Expected: "unexpected uint8 iterator complementary dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype iterators with complementary call and apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(3); b[0] = 260; b[1] = -1; b[2] = 7; var keys = Uint8Array.prototype.keys.apply(b, []); var k0 = keys.next(); var k1 = keys.next(); var values = Uint8Array.prototype.values.apply(b, []); var v0 = values.next(); var v1 = values.next(); var entries = Uint8Array.prototype.entries.call(b); var e0 = entries.next(); var sym = Uint8Array.prototype[Symbol.iterator].apply(b, []); var s0 = sym.next(); k0.value + ':' + k1.value + ':' + v0.value + ':' + v1.value + ':' + e0.value[0] + '=' + e0.value[1] + ':' + s0.value")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("0:1:4:255:0=4:4")
    Err(err):
        expect("unexpected uint8 iterator complementary dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype Symbol iterator with call in browser scripts

- dispatches Uint8Array prototype Symbol iterator with call in browser scripts
   - Expected: _display_js(value) equals `4:255:true`
   - Expected: "unexpected uint8 prototype symbol iterator dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype Symbol iterator with call in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(2); b[0] = 260; b[1] = -1; var iterator = Uint8Array.prototype[Symbol.iterator].call(b); var first = iterator.next(); var second = iterator.next(); var third = iterator.next(); first.value + ':' + second.value + ':' + third.done")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("4:255:true")
    Err(err):
        expect("unexpected uint8 prototype symbol iterator dispatch js error: {err}").to_equal("")
```

</details>

#### reports Uint8Array view buffer metadata in browser scripts

- reports Uint8Array view buffer metadata in browser scripts
   - Expected: _display_js(value) equals `1:1:1:3:3:8:4,255,0:0,4,255,0,0,0,0,0`
   - Expected: "unexpected uint8 metadata js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("reports Uint8Array view buffer metadata in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var buffer = new ArrayBuffer(8); var full = new Uint8Array(buffer); full[1] = 260; full[2] = -1; var view = new Uint8Array(buffer, 1, 3); Uint8Array.BYTES_PER_ELEMENT + ':' + view.BYTES_PER_ELEMENT + ':' + view.byteOffset + ':' + view.byteLength + ':' + view.length + ':' + view.buffer.byteLength + ':' + view.toString() + ':' + full.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1:1:1:3:3:8:4,255,0:0,4,255,0,0,0,0,0")
    Err(err):
        expect("unexpected uint8 metadata js error: {err}").to_equal("")
```

</details>

#### reports Uint8Array prototype metadata in browser scripts

- reports Uint8Array prototype metadata in browser scripts
   - Expected: _display_js(value) equals `true:object:1:function:function:function`
   - Expected: "unexpected uint8 prototype metadata js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("reports Uint8Array prototype metadata in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("(Uint8Array.prototype === Uint8Array.prototype) + ':' + typeof Uint8Array.prototype + ':' + Uint8Array.prototype.BYTES_PER_ELEMENT + ':' + typeof Uint8Array.prototype.subarray + ':' + typeof Uint8Array.prototype.values + ':' + typeof Uint8Array.prototype[Symbol.iterator]")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:object:1:function:function:function")
    Err(err):
        expect("unexpected uint8 prototype metadata js error: {err}").to_equal("")
```

</details>

#### evaluates strict equality for browser script values

- evaluates strict equality for browser script values
   - Expected: _display_js(value) equals `true:true:true:true:true`
   - Expected: "unexpected strict equality js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("evaluates strict equality for browser script values")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("(1 === 1) + ':' + ('a' === 'a') + ':' + (1 !== 2) + ':' + (window === window) + ':' + (document === document)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:true:true:true:true")
    Err(err):
        expect("unexpected strict equality js error: {err}").to_equal("")
```

</details>

#### detects ArrayBuffer view objects in browser scripts

- detects ArrayBuffer view objects in browser scripts
   - Expected: _display_js(value) equals `true:true:false:false:false`
   - Expected: "unexpected arraybuffer isView js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("detects ArrayBuffer view objects in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var buffer = new ArrayBuffer(8); var bytes = new Uint8Array(buffer); var data = new DataView(buffer); ArrayBuffer.isView(bytes) + ':' + ArrayBuffer.isView(data) + ':' + ArrayBuffer.isView(buffer) + ':' + ArrayBuffer.isView({}) + ':' + ArrayBuffer.isView(null)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:true:false:false:false")
    Err(err):
        expect("unexpected arraybuffer isView js error: {err}").to_equal("")
```

</details>

#### dispatches ArrayBuffer prototype slice with call and apply in browser scripts

- dispatches ArrayBuffer prototype slice with call and apply in browser scripts
   - Expected: _display_js(value) equals `function:4:4,255,7,8:3:255,7,8:4:255,7,8,9:1,4,5,7,8,9`
   - Expected: "unexpected arraybuffer prototype slice js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches ArrayBuffer prototype slice with call and apply in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var buffer = new ArrayBuffer(6); var bytes = new Uint8Array(buffer); bytes[0] = 1; bytes[1] = 260; bytes[2] = -1; bytes[3] = 7; bytes[4] = 8; bytes[5] = 9; var direct = buffer.slice(1, 5); var viaCall = ArrayBuffer.prototype.slice.call(buffer, -4, -1); var viaApply = ArrayBuffer.prototype.slice.apply(buffer, [2]); var directView = new Uint8Array(direct); var callView = new Uint8Array(viaCall); var applyView = new Uint8Array(viaApply); bytes[2] = 5; (typeof ArrayBuffer.prototype.slice) + ':' + direct.byteLength + ':' + directView.toString() + ':' + viaCall.byteLength + ':' + callView.toString() + ':' + viaApply.byteLength + ':' + applyView.toString() + ':' + bytes.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("function:4:4,255,7,8:3:255,7,8:4:255,7,8,9:1,4,5,7,8,9")
    Err(err):
        expect("unexpected arraybuffer prototype slice js error: {err}").to_equal("")
```

</details>

#### dispatches ordinary function call and apply with explicit receivers in browser scripts

- dispatches ordinary function call and apply with explicit receivers in browser scripts
   - Expected: _display_js(value) equals `15:29`
   - Expected: "unexpected ordinary function call/apply js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches ordinary function call and apply with explicit receivers in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var add = function(a, b) { return this.base + a + b; }; var callReceiver = []; callReceiver.base = 10; var applyReceiver = []; applyReceiver.base = 20; var applyArgs = [4, 5]; var called = add.call(callReceiver, 2, 3); var applied = add.apply(applyReceiver, applyArgs); called + ':' + applied")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("15:29")
    Err(err):
        expect("unexpected ordinary function call/apply js error: {err}").to_equal("")
```

</details>

#### dispatches ordinary function call and apply with object literal receivers in browser scripts

- dispatches ordinary function call and apply with object literal receivers in browser scripts
   - Expected: _display_js(value) equals `15:29`
   - Expected: "unexpected object literal function call/apply js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches ordinary function call and apply with object literal receivers in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script('var add = function(a, b) { return this.base + a + b; }; var applyArgs = [4, 5]; var called = add.call({ base: 10 }, 2, 3); var applied = add.apply({ base: 20 }, applyArgs); called + ":" + applied')
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("15:29")
    Err(err):
        expect("unexpected object literal function call/apply js error: {err}").to_equal("")
```

</details>

#### dispatches ordinary function bind with receiver and partial arguments in browser scripts

- dispatches ordinary function bind with receiver and partial arguments in browser scripts
   - Expected: _display_js(value) equals `19:5,7,9:function`
   - Expected: "unexpected ordinary function bind js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches ordinary function bind with receiver and partial arguments in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script('var add = function(a, b, c) { return this.base + a + b + c; }; var bound = add.bind({ base: 10 }, 2); var bytes = new Uint8Array(3); bytes[0] = 1; bytes[1] = 2; bytes[2] = 3; var mapper = function(v, i) { return this.offset + v + i; }.bind({ offset: 4 }); var mapped = bytes.map(mapper); bound(3, 4) + ":" + mapped.toString() + ":" + typeof bound')
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("19:5,7,9:function")
    Err(err):
        expect("unexpected ordinary function bind js error: {err}").to_equal("")
```

</details>

#### dispatches bound function call and apply without replacing the bound receiver

- dispatches bound function call and apply without replacing the bound receiver
   - Expected: _display_js(value) equals `19:23:function`
   - Expected: "unexpected bound function call/apply js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches bound function call and apply without replacing the bound receiver")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script('var add = function(a, b, c) { return this.base + a + b + c; }; var bound = add.bind({ base: 10 }, 2); var ignored = { base: 99 }; var viaCall = bound.call(ignored, 3, 4); var viaApply = bound.apply(ignored, [5, 6]); viaCall + ":" + viaApply + ":" + typeof bound')
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("19:23:function")
    Err(err):
        expect("unexpected bound function call/apply js error: {err}").to_equal("")
```

</details>

#### dispatches chained function bind with accumulated partial arguments

- dispatches chained function bind with accumulated partial arguments
   - Expected: _display_js(value) equals `20:21:function`
   - Expected: "unexpected chained function bind js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches chained function bind with accumulated partial arguments")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script('var add = function(a, b, c, d) { return this.base + a + b + c + d; }; var first = add.bind({ base: 10 }, 1); var second = first.bind({ base: 99 }, 2, 3); var viaDirect = second(4); var viaCall = second.call({ base: 50 }, 5); viaDirect + ":" + viaCall + ":" + typeof second')
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("20:21:function")
    Err(err):
        expect("unexpected chained function bind js error: {err}").to_equal("")
```

</details>

#### dispatches chained function bind with apply arguments

- dispatches chained function bind with apply arguments
   - Expected: _display_js(value) equals `25:function`
   - Expected: "unexpected chained function bind apply js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches chained function bind with apply arguments")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script('var add = function(a, b, c, d, e) { return this.base + a + b + c + d + e; }; var first = add.bind({ base: 10 }, 1); var second = first.bind({ base: 99 }, 2); var args = [3, 4, 5]; var applied = second.apply({ base: 50 }, args); applied + ":" + typeof second')
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("25:function")
    Err(err):
        expect("unexpected chained function bind apply js error: {err}").to_equal("")
```

</details>

#### binds arguments object through function call apply and bind

- binds arguments object through function call apply and bind
   - Expected: _display_js(value) equals `call:2:1:2:1|apply:2:3:4:3|bound:2:5:6:5|bound:2:5:7:5`
   - Expected: "unexpected function arguments binding js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("binds arguments object through function call apply and bind")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script('var inspect = function(a) { return this.base + ":" + arguments.length + ":" + arguments[0] + ":" + arguments[1] + ":" + a; }; var viaCall = inspect.call({ base: "call" }, 1, 2); var viaApply = inspect.apply({ base: "apply" }, [3, 4]); var bound = inspect.bind({ base: "bound" }, 5); var viaBound = bound(6); var viaBoundApply = bound.apply({ base: "ignored" }, [7]); viaCall + "|" + viaApply + "|" + viaBound + "|" + viaBoundApply')
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("call:2:1:2:1|apply:2:3:4:3|bound:2:5:6:5|bound:2:5:7:5")
    Err(err):
        expect("unexpected function arguments binding js error: {err}").to_equal("")
```

</details>

#### dispatches function apply with array-like argument objects

- dispatches function apply with array-like argument objects
   - Expected: _display_js(value) equals `plain:3:2:3:4|bound:3:5:6:7`
   - Expected: "unexpected function array-like apply js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches function apply with array-like argument objects")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script('var add = function(a, b, c) { return this.base + ":" + arguments.length + ":" + a + ":" + b + ":" + c; }; var args = { 0: 2, 1: 3, 2: 4, length: 3 }; var viaApply = add.apply({ base: "plain" }, args); var bound = add.bind({ base: "bound" }, 5); var viaBoundApply = bound.apply({ base: "ignored" }, { 0: 6, 1: 7, length: 2 }); viaApply + "|" + viaBoundApply')
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("plain:3:2:3:4|bound:3:5:6:7")
    Err(err):
        expect("unexpected function array-like apply js error: {err}").to_equal("")
```

</details>

#### dispatches Uint8Array prototype helpers with call and apply in browser scripts

- dispatches Uint8Array prototype helpers with call and apply in browser scripts
   - Expected: _display_js(value) equals `4,255,7:255,7,9:1`
   - Expected: "unexpected uint8 prototype dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches Uint8Array prototype helpers with call and apply in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var bytes = new Uint8Array(5); bytes[0] = 1; bytes[1] = 260; bytes[2] = -1; bytes[3] = 7; bytes[4] = 9; var sub = Uint8Array.prototype.subarray.call(bytes, 1, 4); var sl = Uint8Array.prototype.slice.apply(bytes, [2, 5]); var it = Uint8Array.prototype.values.call(bytes); var first = it.next(); sub.toString() + ':' + sl.toString() + ':' + first.value")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("4,255,7:255,7,9:1")
    Err(err):
        expect("unexpected uint8 prototype dispatch js error: {err}").to_equal("")
```

</details>

#### preserves Uint8Array subarray shared buffer writes in browser scripts

- preserves Uint8Array subarray shared buffer writes in browser scripts
   - Expected: _display_js(value) equals `1:3:3:true:4,255,4:1,4,255,4,5:255`
   - Expected: "unexpected uint8 subarray shared buffer js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("preserves Uint8Array subarray shared buffer writes in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(5); b[0] = 1; b[1] = 2; b[2] = 3; b[3] = 4; b[4] = 5; var sub = b.subarray(1, 4); sub[0] = 260; b[2] = -1; sub.byteOffset + ':' + sub.byteLength + ':' + sub.length + ':' + (sub.buffer === b.buffer) + ':' + sub.toString() + ':' + b.toString() + ':' + sub.at(1)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1:3:3:true:4,255,4:1,4,255,4,5:255")
    Err(err):
        expect("unexpected uint8 subarray shared buffer js error: {err}").to_equal("")
```

</details>

#### keeps Uint8Array slice copied buffer writes isolated in browser scripts

- keeps Uint8Array slice copied buffer writes isolated in browser scripts
   - Expected: _display_js(value) equals `0:3:3:false:4,3,4:1,2,255,4,5:3`
   - Expected: "unexpected uint8 slice copied buffer js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("keeps Uint8Array slice copied buffer writes isolated in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(5); b[0] = 1; b[1] = 2; b[2] = 3; b[3] = 4; b[4] = 5; var sl = b.slice(1, 4); sl[0] = 260; b[2] = -1; sl.byteOffset + ':' + sl.byteLength + ':' + sl.length + ':' + (sl.buffer === b.buffer) + ':' + sl.toString() + ':' + b.toString() + ':' + sl.at(1)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("0:3:3:false:4,3,4:1,2,255,4,5:3")
    Err(err):
        expect("unexpected uint8 slice copied buffer js error: {err}").to_equal("")
```

</details>

#### keeps Uint8Array prototype slice call and apply copied buffers isolated

- keeps Uint8Array prototype slice call and apply copied buffers isolated
   - Expected: _display_js(value) equals `false:false:0:0:3:3:4,3,4:3,255,5:1,2,3,99,5,6:4:255`
   - Expected: "unexpected uint8 prototype slice copied buffer js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("keeps Uint8Array prototype slice call and apply copied buffers isolated")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(6); b[0] = 1; b[1] = 2; b[2] = 3; b[3] = 4; b[4] = 5; b[5] = 6; var callSlice = Uint8Array.prototype.slice.call(b, 1, 4); var applySlice = Uint8Array.prototype.slice.apply(b, [2, 5]); callSlice[0] = 260; applySlice[1] = -1; b[3] = 99; (callSlice.buffer === b.buffer) + ':' + (applySlice.buffer === b.buffer) + ':' + callSlice.byteOffset + ':' + applySlice.byteOffset + ':' + callSlice.length + ':' + applySlice.length + ':' + callSlice.toString() + ':' + applySlice.toString() + ':' + b.toString() + ':' + callSlice.at(0) + ':' + applySlice.at(1)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("false:false:0:0:3:3:4,3,4:3,255,5:1,2,3,99,5,6:4:255")
    Err(err):
        expect("unexpected uint8 prototype slice copied buffer js error: {err}").to_equal("")
```

</details>

#### copies Uint8Array slice bytes from nonzero-offset views in browser scripts

- copies Uint8Array slice bytes from nonzero-offset views in browser scripts
   - Expected: _display_js(value) equals `true:false:false:false:0:0:0:2:2:2:4,50:30,255:99,50:30,77,50,60:10,20,30,77,... (full value in folded executable source)`
   - Expected: "unexpected uint8 view slice copied buffer js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("copies Uint8Array slice bytes from nonzero-offset views in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var base = new Uint8Array(7); base[0] = 10; base[1] = 20; base[2] = 30; base[3] = 40; base[4] = 50; base[5] = 60; base[6] = 70; var view = base.subarray(2, 6); var direct = view.slice(1, 3); var callSlice = Uint8Array.prototype.slice.call(view, 0, 2); var applySlice = Uint8Array.prototype.slice.apply(view, [-3, -1]); direct[0] = 260; callSlice[1] = -1; applySlice[0] = 99; view[1] = 77; (view.buffer === base.buffer) + ':' + (direct.buffer === view.buffer) + ':' + (callSlice.buffer === view.buffer) + ':' + (applySlice.buffer === view.buffer) + ':' + direct.byteOffset + ':' + callSlice.byteOffset + ':' + applySlice.byteOffset + ':' + direct.length + ':' + callSlice.length + ':' + applySlice.length + ':' + direct.toString() + ':' + callSlice.toString() + ':' + applySlice.toString() + ':' + view.toString() + ':' + base.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:false:false:false:0:0:0:2:2:2:4,50:30,255:99,50:30,77,50,60:10,20,30,77,50,60,70")
    Err(err):
        expect("unexpected uint8 view slice copied buffer js error: {err}").to_equal("")
```

</details>

#### copies nested Uint8Array view slice bytes into isolated buffers

- copies nested Uint8Array view slice bytes into isolated buffers
   - Expected: _display_js(value) equals `true:false:0:2:2:255,7:123,7:4,66,7:30,4,66,7,8:10,20,30,4,66,7,8,90`
   - Expected: "unexpected nested uint8 view slice copied buffer js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("copies nested Uint8Array view slice bytes into isolated buffers")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var base = new Uint8Array(8); base[0] = 10; base[1] = 20; base[2] = 30; base[3] = 260; base[4] = -1; base[5] = 7; base[6] = 8; base[7] = 90; var view = base.subarray(2, 7); var nested = view.subarray(1, -1); var copy = nested.slice(1); var copyBefore = copy.toString(); copy[0] = 123; nested[1] = 66; (nested.buffer === view.buffer) + ':' + (copy.buffer === nested.buffer) + ':' + copy.byteOffset + ':' + copy.byteLength + ':' + copy.length + ':' + copyBefore + ':' + copy.toString() + ':' + nested.toString() + ':' + view.toString() + ':' + base.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("true:false:0:2:2:255,7:123,7:4,66,7:30,4,66,7,8:10,20,30,4,66,7,8,90")
    Err(err):
        expect("unexpected nested uint8 view slice copied buffer js error: {err}").to_equal("")
```

</details>

#### copies nested Uint8Array view slice edge ranges into isolated buffers

- copies nested Uint8Array view slice edge ranges into isolated buffers
   - Expected: _display_js(value) equals `false:false:false:0,0,0:4,2,2:4,2,2:40,250,4,255|4,255|250,4:123,250,4,255:4,... (full value in folded executable source)`
   - Expected: "unexpected nested uint8 view slice edge range copied buffer js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("copies nested Uint8Array view slice edge ranges into isolated buffers")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var base = new Uint8Array(9); base[0] = 1; base[1] = 2; base[2] = 30; base[3] = 40; base[4] = 250; base[5] = 260; base[6] = -1; base[7] = 80; base[8] = 90; var view = base.subarray(2, 8); var nested = view.subarray(1, -1); var all = nested.slice(); var tail = nested.slice(-2); var middle = nested.slice(1, 3); var before = all.toString() + '|' + tail.toString() + '|' + middle.toString(); all[0] = 123; tail[1] = 124; middle[0] = 125; nested[1] = 66; (all.buffer === nested.buffer) + ':' + (tail.buffer === nested.buffer) + ':' + (middle.buffer === nested.buffer) + ':' + all.byteOffset + ',' + tail.byteOffset + ',' + middle.byteOffset + ':' + all.byteLength + ',' + tail.byteLength + ',' + middle.byteLength + ':' + all.length + ',' + tail.length + ',' + middle.length + ':' + before + ':' + all.toString() + ':' + tail.toString() + ':' + middle.toString() + ':' + nested.toString() + ':' + view.toString() + ':' + base.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("false:false:false:0,0,0:4,2,2:4,2,2:40,250,4,255|4,255|250,4:123,250,4,255:4,124:125,4:40,66,4,255:30,40,66,4,255,80:1,2,30,40,66,4,255,80,90")
    Err(err):
        expect("unexpected nested uint8 view slice edge range copied buffer js error: {err}").to_equal("")
```

</details>

#### copies overlapping Uint8Array subarray windows through set in browser scripts

- copies overlapping Uint8Array subarray windows through set in browser scripts
   - Expected: _display_js(value) equals `1,2,1,2,3,6:undefined:1,2,1`
   - Expected: "unexpected uint8 overlapping set js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("copies overlapping Uint8Array subarray windows through set in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var b = new Uint8Array(6); b[0] = 1; b[1] = 2; b[2] = 3; b[3] = 4; b[4] = 5; b[5] = 6; var src = b.subarray(0, 3); var returned = b.set(src, 2); b.toString() + ':' + returned + ':' + src.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1,2,1,2,3,6:undefined:1,2,1")
    Err(err):
        expect("unexpected uint8 overlapping set js error: {err}").to_equal("")
```

</details>

#### writes Uint8Array set bytes through nonzero-offset source and target views

- writes Uint8Array set bytes through nonzero-offset source and target views
   - Expected: _display_js(value) equals `2:5:1:3:undefined:undefined:undefined:10,4,255,77,123:1,2,10,4,255,77,123,8:1... (full value in folded executable source)`
   - Expected: "unexpected uint8 set view window js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("writes Uint8Array set bytes through nonzero-offset source and target views")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var base = new Uint8Array(8); base[0] = 1; base[1] = 2; base[2] = 3; base[3] = 4; base[4] = 5; base[5] = 6; base[6] = 7; base[7] = 8; var target = base.subarray(2, 7); var srcBase = new Uint8Array(5); srcBase[0] = 10; srcBase[1] = 260; srcBase[2] = -1; srcBase[3] = 77; srcBase[4] = 88; var src = srcBase.subarray(1, 4); var ret1 = target.set(src, 1); var callSrc = srcBase.subarray(0, 2); var ret2 = Uint8Array.prototype.set.call(target, callSrc, 0); var applySrc = srcBase.subarray(3, 5); var ret3 = Uint8Array.prototype.set.apply(target, [applySrc, 3]); src[0] = 99; target[4] = 123; target.byteOffset + ':' + target.length + ':' + src.byteOffset + ':' + src.length + ':' + ret1 + ':' + ret2 + ':' + ret3 + ':' + target.toString() + ':' + base.toString() + ':' + srcBase.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("2:5:1:3:undefined:undefined:undefined:10,4,255,77,123:1,2,10,4,255,77,123,8:10,99,255,77,88")
    Err(err):
        expect("unexpected uint8 set view window js error: {err}").to_equal("")
```

</details>

#### copies Uint8Array constructor bytes from nonzero-offset views

- copies Uint8Array constructor bytes from nonzero-offset views
   - Expected: _display_js(value) equals `false:0:3:3:77,255,50:77,255,50:4,88,50:10,20,4,88,50,60`
   - Expected: "unexpected uint8 constructor view copy js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("copies Uint8Array constructor bytes from nonzero-offset views")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var base = new Uint8Array(6); base[0] = 10; base[1] = 20; base[2] = 260; base[3] = -1; base[4] = 50; base[5] = 60; var view = base.subarray(2, 5); var copy = new Uint8Array(view); copy[0] = 77; view[1] = 88; var bufferView = new Uint8Array(copy.buffer); (copy.buffer === view.buffer) + ':' + copy.byteOffset + ':' + copy.buffer.byteLength + ':' + copy.length + ':' + copy.toString() + ':' + bufferView.toString() + ':' + view.toString() + ':' + base.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("false:0:3:3:77,255,50:77,255,50:4,88,50:10,20,4,88,50,60")
    Err(err):
        expect("unexpected uint8 constructor view copy js error: {err}").to_equal("")
```

</details>

#### reports ArrayBuffer and typed-array constructor metadata in browser scripts

- reports ArrayBuffer and typed-array constructor metadata in browser scripts
   - Expected: _display_js(value) equals `Uint8Array:3:ArrayBuffer:1:DataView:1:true:true:true:true:true:function`
   - Expected: "unexpected constructor metadata js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("reports ArrayBuffer and typed-array constructor metadata in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("Uint8Array.name + ':' + Uint8Array.length + ':' + ArrayBuffer.name + ':' + ArrayBuffer.length + ':' + DataView.name + ':' + DataView.length + ':' + (Uint8Array.prototype.constructor === Uint8Array) + ':' + (ArrayBuffer.prototype === ArrayBuffer.prototype) + ':' + (ArrayBuffer.prototype.constructor === ArrayBuffer) + ':' + (DataView.prototype === DataView.prototype) + ':' + (DataView.prototype.constructor === DataView) + ':' + typeof DataView.prototype.getUint8")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("Uint8Array:3:ArrayBuffer:1:DataView:1:true:true:true:true:true:function")
    Err(err):
        expect("unexpected constructor metadata js error: {err}").to_equal("")
```

</details>

#### reads and writes ArrayBuffer bytes through DataView in browser scripts

- reads and writes ArrayBuffer bytes through DataView in browser scripts
   - Expected: _display_js(value) equals `16909060:-2:4,3,2,1,254,255,255,255:0:8`
   - Expected: "unexpected dataview js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("reads and writes ArrayBuffer bytes through DataView in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var buffer = new ArrayBuffer(8); var view = new DataView(buffer); view.setUint32(0, 16909060, true); view.setInt32(4, -2, true); var bytes = new Uint8Array(buffer); view.getUint32(0, true) + ':' + view.getInt32(4, true) + ':' + bytes.toString() + ':' + view.byteOffset + ':' + view.byteLength")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("16909060:-2:4,3,2,1,254,255,255,255:0:8")
    Err(err):
        expect("unexpected dataview js error: {err}").to_equal("")
```

</details>

#### dispatches DataView prototype helpers with call and apply in browser scripts

- dispatches DataView prototype helpers with call and apply in browser scripts
   - Expected: _display_js(value) equals `258:-1:16909060:255,2,1,2,3,4`
   - Expected: "unexpected dataview prototype dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches DataView prototype helpers with call and apply in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var buffer = new ArrayBuffer(6); var view = new DataView(buffer); DataView.prototype.setUint16.call(view, 1, 258, true); DataView.prototype.setInt8.apply(view, [0, -1]); DataView.prototype.setUint32.call(view, 2, 16909060, false); var bytes = new Uint8Array(buffer); DataView.prototype.getUint16.apply(view, [1, true]) + ':' + DataView.prototype.getInt8.call(view, 0) + ':' + DataView.prototype.getUint32.apply(view, [2, false]) + ':' + bytes.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("258:-1:16909060:255,2,1,2,3,4")
    Err(err):
        expect("unexpected dataview prototype dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches remaining DataView prototype helpers with call and apply in browser scripts

- dispatches remaining DataView prototype helpers with call and apply in browser scripts
   - Expected: _display_js(value) equals `4:-2:-3:4,254,255,255,255,255,253,0`
   - Expected: "unexpected remaining dataview prototype dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches remaining DataView prototype helpers with call and apply in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var buffer = new ArrayBuffer(8); var view = new DataView(buffer); DataView.prototype.setUint8.call(view, 0, 260); DataView.prototype.setInt16.apply(view, [1, -2, true]); DataView.prototype.setInt32.call(view, 3, -3, false); var bytes = new Uint8Array(buffer); DataView.prototype.getUint8.apply(view, [0]) + ':' + DataView.prototype.getInt16.call(view, 1, true) + ':' + DataView.prototype.getInt32.apply(view, [3, false]) + ':' + bytes.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("4:-2:-3:4,254,255,255,255,255,253,0")
    Err(err):
        expect("unexpected remaining dataview prototype dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches DataView prototype helpers with complementary call and apply

- dispatches DataView prototype helpers with complementary call and apply
   - Expected: _display_js(value) equals `258:-1:16909060:1,2,255,4,3,2,1`
   - Expected: "unexpected dataview complementary dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches DataView prototype helpers with complementary call and apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var buffer = new ArrayBuffer(7); var view = new DataView(buffer); DataView.prototype.setUint16.apply(view, [0, 258, false]); DataView.prototype.setInt8.call(view, 2, -1); DataView.prototype.setUint32.apply(view, [3, 16909060, true]); var bytes = new Uint8Array(buffer); DataView.prototype.getUint16.call(view, 0, false) + ':' + DataView.prototype.getInt8.apply(view, [2]) + ':' + DataView.prototype.getUint32.call(view, 3, true) + ':' + bytes.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("258:-1:16909060:1,2,255,4,3,2,1")
    Err(err):
        expect("unexpected dataview complementary dispatch js error: {err}").to_equal("")
```

</details>

#### dispatches remaining DataView prototype helpers with complementary call and apply

- dispatches remaining DataView prototype helpers with complementary call and apply
   - Expected: _display_js(value) equals `4:-2:-3:4,254,255,255,255,255,253,0`
   - Expected: "unexpected remaining dataview complementary dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches remaining DataView prototype helpers with complementary call and apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var buffer = new ArrayBuffer(8); var view = new DataView(buffer); DataView.prototype.setUint8.apply(view, [0, 260]); DataView.prototype.setInt16.call(view, 1, -2, true); DataView.prototype.setInt32.apply(view, [3, -3, false]); var bytes = new Uint8Array(buffer); DataView.prototype.getUint8.call(view, 0) + ':' + DataView.prototype.getInt16.apply(view, [1, true]) + ':' + DataView.prototype.getInt32.call(view, 3, false) + ':' + bytes.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("4:-2:-3:4,254,255,255,255,255,253,0")
    Err(err):
        expect("unexpected remaining dataview complementary dispatch js error: {err}").to_equal("")
```

</details>

#### reads signed ArrayBuffer bytes through DataView in browser scripts

- reads signed ArrayBuffer bytes through DataView in browser scripts
   - Expected: _display_js(value) equals `-1:255:-2:65534:-32768:32768:255,254,255,128,0,0`
   - Expected: "unexpected signed dataview js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("reads signed ArrayBuffer bytes through DataView in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var buffer = new ArrayBuffer(6); var view = new DataView(buffer); view.setInt8(0, -1); view.setInt16(1, -2, true); view.setInt16(3, -32768, false); var bytes = new Uint8Array(buffer); view.getInt8(0) + ':' + view.getUint8(0) + ':' + view.getInt16(1, true) + ':' + view.getUint16(1, true) + ':' + view.getInt16(3, false) + ':' + view.getUint16(3, false) + ':' + bytes.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("-1:255:-2:65534:-32768:32768:255,254,255,128,0,0")
    Err(err):
        expect("unexpected signed dataview js error: {err}").to_equal("")
```

</details>

#### windows DataView bytes over ArrayBuffer offsets in browser scripts

- windows DataView bytes over ArrayBuffer offsets in browser scripts
   - Expected: _display_js(value) equals `2:4:3:258:-1:1,2,3,2,1,255,0,0`
   - Expected: "unexpected dataview window js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("windows DataView bytes over ArrayBuffer offsets in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var buffer = new ArrayBuffer(8); var bytes = new Uint8Array(buffer); bytes[0] = 1; bytes[1] = 2; bytes[2] = 3; bytes[3] = 4; bytes[4] = 5; var view = new DataView(buffer, 2, 4); view.setUint16(1, 258, true); view.setInt8(3, -1); view.byteOffset + ':' + view.byteLength + ':' + view.getUint8(0) + ':' + view.getUint16(1, true) + ':' + view.getInt8(3) + ':' + bytes.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("2:4:3:258:-1:1,2,3,2,1,255,0,0")
    Err(err):
        expect("unexpected dataview window js error: {err}").to_equal("")
```

</details>

#### dispatches windowed DataView prototype helpers with call and apply

- dispatches windowed DataView prototype helpers with call and apply
   - Expected: _display_js(value) equals `2:4:3:258:-1:1,2,3,2,1,255,0,0`
   - Expected: "unexpected dataview window prototype js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches windowed DataView prototype helpers with call and apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var buffer = new ArrayBuffer(8); var bytes = new Uint8Array(buffer); bytes[0] = 1; bytes[1] = 2; bytes[2] = 3; bytes[3] = 4; bytes[4] = 5; var view = new DataView(buffer, 2, 4); DataView.prototype.setUint16.apply(view, [1, 258, true]); DataView.prototype.setInt8.call(view, 3, -1); view.byteOffset + ':' + view.byteLength + ':' + DataView.prototype.getUint8.call(view, 0) + ':' + DataView.prototype.getUint16.apply(view, [1, true]) + ':' + DataView.prototype.getInt8.call(view, 3) + ':' + bytes.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("2:4:3:258:-1:1,2,3,2,1,255,0,0")
    Err(err):
        expect("unexpected dataview window prototype js error: {err}").to_equal("")
```

</details>

#### dispatches windowed DataView 32-bit prototype helpers with call and apply

- dispatches windowed DataView 32-bit prototype helpers with call and apply
   - Expected: _display_js(value) equals `2:8:16909060:-3:10,20,4,3,2,1,255,255,255,253,30,40`
   - Expected: "unexpected dataview window 32-bit prototype js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches windowed DataView 32-bit prototype helpers with call and apply")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var buffer = new ArrayBuffer(12); var bytes = new Uint8Array(buffer); bytes[0] = 10; bytes[1] = 20; bytes[10] = 30; bytes[11] = 40; var view = new DataView(buffer, 2, 8); DataView.prototype.setUint32.apply(view, [0, 16909060, true]); DataView.prototype.setInt32.call(view, 4, -3, false); view.byteOffset + ':' + view.byteLength + ':' + DataView.prototype.getUint32.call(view, 0, true) + ':' + DataView.prototype.getInt32.apply(view, [4, false]) + ':' + bytes.toString()")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("2:8:16909060:-3:10,20,4,3,2,1,255,255,255,253,30,40")
    Err(err):
        expect("unexpected dataview window 32-bit prototype js error: {err}").to_equal("")
```

</details>

#### encodes browser TextEncoder bytes into WASM validation buffers

- encodes browser TextEncoder bytes into WASM validation buffers
   - Expected: _display_js(value) equals `utf-8:utf-8:3:97,115,109:true:asm`
   - Expected: "unexpected text codec wasm header js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("encodes browser TextEncoder bytes into WASM validation buffers")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var encoder = new TextEncoder(); var decoder = new TextDecoder('utf8'); var header = new Uint8Array(8); var letters = encoder.encode('asm'); header[0] = 0; Uint8Array.prototype.set.call(header, letters, 1); header[4] = 1; header[5] = 0; header[6] = 0; header[7] = 0; encoder.encoding + ':' + decoder.encoding + ':' + letters.length + ':' + header[1] + ',' + header[2] + ',' + header[3] + ':' + WebAssembly.validate(header) + ':' + decoder.decode(header.slice(1, 4))")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("utf-8:utf-8:3:97,115,109:true:asm")
    Err(err):
        expect("unexpected text codec wasm header js error: {err}").to_equal("")
```

</details>

#### reports partial browser TextEncoder writes before WASM validation

- reports partial browser TextEncoder writes before WASM validation
   - Expected: _display_js(value) equals `3:3:109,111,100:mod:false`
   - Expected: "unexpected partial text codec wasm validation js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("reports partial browser TextEncoder writes before WASM validation")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var encoder = new TextEncoder(); var decoder = new TextDecoder(); var dst = new Uint8Array(3); var written = encoder.encodeInto('module', dst); written.read + ':' + written.written + ':' + dst.toString() + ':' + decoder.decode(dst) + ':' + WebAssembly.validate(dst)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("3:3:109,111,100:mod:false")
    Err(err):
        expect("unexpected partial text codec wasm validation js error: {err}").to_equal("")
```

</details>

#### writes TextEncoder encodeInto output through nonzero-offset Uint8Array views

- writes TextEncoder encodeInto output through nonzero-offset Uint8Array views
   - Expected: _display_js(value) equals `4:4:2:4:109,111,100,117:1,2,109,111,100,117,7,8:modu:false`
   - Expected: "unexpected nonzero-offset text encoder encodeInto js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("writes TextEncoder encodeInto output through nonzero-offset Uint8Array views")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var encoder = new TextEncoder(); var decoder = new TextDecoder(); var base = new Uint8Array(8); base[0] = 1; base[1] = 2; base[6] = 7; base[7] = 8; var view = base.subarray(2, 6); var written = encoder.encodeInto('module', view); written.read + ':' + written.written + ':' + view.byteOffset + ':' + view.length + ':' + view.toString() + ':' + base.toString() + ':' + decoder.decode(view) + ':' + WebAssembly.validate(view)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("4:4:2:4:109,111,100,117:1,2,109,111,100,117,7,8:modu:false")
    Err(err):
        expect("unexpected nonzero-offset text encoder encodeInto js error: {err}").to_equal("")
```

</details>

#### reports TextDecoder constructor option properties in browser scripts

- reports TextDecoder constructor option properties in browser scripts
   - Expected: _display_js(value) equals `utf-8:true:true:false:false:asm`
   - Expected: "unexpected text decoder option property js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("reports TextDecoder constructor option properties in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var configured = new TextDecoder('utf8', { fatal: true, ignoreBOM: true }); var defaults = new TextDecoder(); var bytes = new Uint8Array(3); bytes[0] = 97; bytes[1] = 115; bytes[2] = 109; configured.encoding + ':' + configured.fatal + ':' + configured.ignoreBOM + ':' + defaults.fatal + ':' + defaults.ignoreBOM + ':' + configured.decode(bytes)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("utf-8:true:true:false:false:asm")
    Err(err):
        expect("unexpected text decoder option property js error: {err}").to_equal("")
```

</details>

#### replaces invalid browser TextDecoder bytes before WASM validation

- replaces invalid browser TextDecoder bytes before WASM validation
   - Expected: _display_js(value) equals `65533:(:195,40:false`
   - Expected: "unexpected invalid text decoder wasm validation js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("replaces invalid browser TextDecoder bytes before WASM validation")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var bytes = new Uint8Array(2); bytes[0] = 195; bytes[1] = 40; var text = new TextDecoder().decode(bytes); text.charCodeAt(0) + ':' + text.charAt(1) + ':' + bytes.toString() + ':' + WebAssembly.validate(bytes)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("65533:(:195,40:false")
    Err(err):
        expect("unexpected invalid text decoder wasm validation js error: {err}").to_equal("")
```

</details>

#### replaces truncated browser TextDecoder bytes before WASM validation

- replaces truncated browser TextDecoder bytes before WASM validation
   - Expected: _display_js(value) equals `65533:226,130:false`
   - Expected: "unexpected truncated text decoder wasm validation js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("replaces truncated browser TextDecoder bytes before WASM validation")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var bytes = new Uint8Array(2); bytes[0] = 226; bytes[1] = 130; var text = new TextDecoder().decode(bytes); text.charCodeAt(0) + ':' + bytes.toString() + ':' + WebAssembly.validate(bytes)")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("65533:226,130:false")
    Err(err):
        expect("unexpected truncated text decoder wasm validation js error: {err}").to_equal("")
```

</details>

#### shares WebAssembly Memory buffer bytes with typed array views in browser scripts

- shares WebAssembly Memory buffer bytes with typed array views in browser scripts
   - Expected: _display_js(value) equals `1:131072:131072:4:2:1`
   - Expected: "unexpected wasm memory view js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("shares WebAssembly Memory buffer bytes with typed array views in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var memory = new WebAssembly.Memory({ initial: 1, maximum: 2 }); var bytes = new Uint8Array(memory.buffer); bytes[4] = 260; var view = new DataView(memory.buffer); view.setUint16(6, 258, true); var old = memory.grow(1); var grown = new Uint8Array(memory.buffer); old + ':' + memory.buffer.byteLength + ':' + grown.length + ':' + grown[4] + ':' + grown[6] + ':' + grown[7]")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1:131072:131072:4:2:1")
    Err(err):
        expect("unexpected wasm memory view js error: {err}").to_equal("")
```

</details>

#### dispatches prototype helpers over WebAssembly Memory buffer views

- dispatches prototype helpers over WebAssembly Memory buffer views
   - Expected: _display_js(value) equals `4:255:7:258:2:1:65536`
   - Expected: "unexpected wasm memory prototype dispatch js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("dispatches prototype helpers over WebAssembly Memory buffer views")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var memory = new WebAssembly.Memory({ initial: 1, maximum: 2 }); var bytes = new Uint8Array(memory.buffer); var src = new Uint8Array(3); src[0] = 260; src[1] = -1; src[2] = 7; Uint8Array.prototype.set.apply(bytes, [src, 5]); var view = new DataView(memory.buffer); DataView.prototype.setUint16.call(view, 9, 258, true); bytes[5] + ':' + bytes[6] + ':' + bytes[7] + ':' + DataView.prototype.getUint16.apply(view, [9, true]) + ':' + bytes[9] + ':' + bytes[10] + ':' + memory.buffer.byteLength")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("4:255:7:258:2:1:65536")
    Err(err):
        expect("unexpected wasm memory prototype dispatch js error: {err}").to_equal("")
```

</details>

#### preserves prototype-dispatched WebAssembly Memory bytes after grow

- preserves prototype-dispatched WebAssembly Memory bytes after grow
   - Expected: _display_js(value) equals `1:131072:1,4,255:2,1:131072`
   - Expected: "unexpected wasm memory prototype grow js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("preserves prototype-dispatched WebAssembly Memory bytes after grow")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var memory = new WebAssembly.Memory({ initial: 1, maximum: 2 }); var before = new Uint8Array(memory.buffer); var src = new Uint8Array(3); src[0] = 1; src[1] = 260; src[2] = -1; Uint8Array.prototype.set.call(before, src, 12); var view = new DataView(memory.buffer); DataView.prototype.setUint16.apply(view, [20, 258, true]); var old = memory.grow(1); var after = new Uint8Array(memory.buffer); old + ':' + memory.buffer.byteLength + ':' + after[12] + ',' + after[13] + ',' + after[14] + ':' + after[20] + ',' + after[21] + ':' + after.length")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("1:131072:1,4,255:2,1:131072")
    Err(err):
        expect("unexpected wasm memory prototype grow js error: {err}").to_equal("")
```

</details>

#### preserves prototype-dispatched WebAssembly Memory bytes after failed grow

- preserves prototype-dispatched WebAssembly Memory bytes after failed grow
   - Expected: _display_js(value) equals `-1:65536:4,255:2,1:258`
   - Expected: "unexpected wasm memory prototype failed grow js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("preserves prototype-dispatched WebAssembly Memory bytes after failed grow")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var memory = new WebAssembly.Memory({ initial: 1, maximum: 1 }); var bytes = new Uint8Array(memory.buffer); var src = new Uint8Array(2); src[0] = 260; src[1] = -1; Uint8Array.prototype.set.apply(bytes, [src, 30]); var view = new DataView(memory.buffer); DataView.prototype.setUint16.call(view, 40, 258, true); var fail = memory.grow(1); var after = new Uint8Array(memory.buffer); fail + ':' + memory.buffer.byteLength + ':' + after[30] + ',' + after[31] + ':' + after[40] + ',' + after[41] + ':' + DataView.prototype.getUint16.apply(new DataView(memory.buffer), [40, true])")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("-1:65536:4,255:2,1:258")
    Err(err):
        expect("unexpected wasm memory prototype failed grow js error: {err}").to_equal("")
```

</details>

#### preserves WebAssembly Memory state when grow exceeds maximum in browser scripts

- preserves WebAssembly Memory state when grow exceeds maximum in browser scripts
   - Expected: _display_js(value) equals `65536:65536:1:-1:65536`
   - Expected: "unexpected wasm memory grow limit js error: {err}" equals ``


<details>
<summary>Executable SSpec</summary>

Runnable source: 13 lines folded for reproduction.
Reproduction: this block contains the complete executable scenario source.

```simple
# @req REQ-SSPEC-UNIT
step("preserves WebAssembly Memory state when grow exceeds maximum in browser scripts")
var session = BrowserSession.new()
session.open_html(
    "https://example.com/webgpu-wasm.html",
    "<html><body>WASM GPU</body></html>"
)
val result = session.eval_script("var memory = new WebAssembly.Memory({ initial: 1, maximum: 1 }); var before = memory.buffer.byteLength; var first = memory.grow(0); var fail = memory.grow(1); var after = memory.buffer.byteLength; var bytes = new Uint8Array(memory.buffer); bytes.length + ':' + before + ':' + first + ':' + fail + ':' + after")
match result:
    Ok(value):
        expect(_display_js(value)).to_equal("65536:65536:1:-1:65536")
    Err(err):
        expect("unexpected wasm memory grow limit js error: {err}").to_equal("")
```

</details>

## At a Glance

| Field | Value |
|-------|-------|
| Category | Standard Library |
| Status | Active |
| Source | `test/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.spl` |
| Updated | 2026-08-26 |
| Generator | `simple spipe-docgen` (Simple) |

## Overview

Tests covering BrowserSession fetch WASM promise chain.
- BrowserSession fetch WASM promise chain

## Scenario Summary

| Metric | Count |
|--------|------:|
| Total scenarios | 250 |
| Active scenarios | 250 |
| Slow scenarios | 0 |
| Skipped scenarios | 0 |
| Pending scenarios | 0 |


</details>

<!-- sspec-maintain:traceability:start -->
## Traceability

Requirements covered by the scenarios in this manual:

- `REQ-SSPEC-UNIT`
<!-- sspec-maintain:traceability:end -->

<!-- sspec-maintain:provenance:start -->
## Generation history

- Canonical SPipe generation for source `73c1d9d322fa46d9bbe6d2bc5e59006b84ca20945734dacf5c6ac87750686596`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `73c1d9d322fa46d9bbe6d2bc5e59006b84ca20945734dacf5c6ac87750686596`.
<!-- sspec-maintain:provenance:end -->

<!-- sspec-maintain:scorecard:start -->
## SSpec documentization scorecard

Source SHA-256: `73c1d9d322fa46d9bbe6d2bc5e59006b84ca20945734dacf5c6ac87750686596`  
Analyzer: `1`; rules: `ssdoc-rules/1`  
Raw score: **92/100**; effective score: **92/100**; blockers: **0**.

SSpec documentization score: 92/100
source: test/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.spl
mirror: doc/06_spec/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.md (current)
findings: 5 blockers: 0
  narrative=100 structure=100 oracle=100
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.spl:31:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'chains fetched arrayBuffer bytes into WebAssembly.instantiate' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.spl:76:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'exposes fetched arrayBuffer instantiated module export descriptors' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/01_unit/lib/common/web/browser_session_fetch_wasm_chain_spec.spl:121:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'chains fetched arrayBuffer bytes into WebAssembly function body calls' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
<!-- sspec-maintain:scorecard:end -->
