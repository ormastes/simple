...
#### keeps unknown backend fallback on the public creation API
...
#### canonicalizes cpu SIMD aliases before selecting the backend

- var engine = Engine2D create with priority
...
#### skips unknown entries before selecting cpu SIMD
...
#### falls back to CPU after unknown priority entries
...
#### does not claim explicit native framebuffer backends from dimensions
