...
val decoded = deflate_decompress_eof(out.body_bytes).unwrap()
expect(decoded.len()).to_equal(payload.len())
```
...
expect(_get_header(out.headers, "content-length")).to_equal(out.body_bytes.len().to_string())
```
...
expect(_get_header(out1.headers, "content-encoding")).to_equal("gzip")

val resp2 = _resp_with_byte_body(payload)
val out2 = compress_response_for(resp2, accept)
expect(_get_header(out2.headers, "content-encoding")).to_equal("gzip")

val resp3 = _resp_with_byte_body(payload)
val out3 = compress_response_for(resp3, accept)
expect(_get_header(out3.headers, "content-encoding")).to_equal("gzip")
```
