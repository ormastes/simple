...
step("passes the layered firmware smoke")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("writes, reads, garbage-collects, trims, and survives a power cycle")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("drives the full admin-and-IO-queue controller lifecycle")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("guards garbage collection against data loss and the write-cliff")
...
val (out, err, code) = _shell("mkdir -p build/test-artifacts; SIMPLE_TIMEOUT_SECONDS=300 \"" + SIMPLE_BINARY + "\" run " + FW + "/gc_safety_check.spl > build/test-artifacts/nvme_gc_safety.out 2> build/test-artifacts/nvme_gc_safety.err && grep -F 'GC SAFETY OK' build/test-artifacts/nvme_gc_safety.out")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("preserves committed data across power loss and a full journal")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("relocates data safely during wear-leveling and read-disturb scrub")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("preserves the bad-block table and wear history across a Format NVM")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("sandboxes dynamic policy hooks so a malicious hook cannot corrupt the FTL (req 7)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("corrects one silent payload bit and fails double-bit corruption closed (gap-closure P3)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("moves multi-block host data through segmented PRP descriptors (gap-closure P4)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("bounds host writes behind a DRAM arena span with no partial media update (gap-closure P5)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("fails closed when task-pool allocation is unavailable")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("schedules NAND ops across channels in parallel with per-channel serialization (gap-closure P2)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("reconstructs a failed channel from XOR parity with no data loss (RAIN, gap-closure P8)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("rebuilds a failed channel inside the live FTL with no logical data loss (RAIN wired, P8)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("throttles under sustained thermal load and recovers when cool (gap-closure P7)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("rejects IO queue deletion while commands or completions are still pending")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("backs off instead of dropping work when queue metadata is corrupted")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("preserves legacy HIL work when queue metadata is corrupted")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("verifies the allocator and GC-reserve safety (Alloc.lean)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("verifies that recovery preserves the committed prefix (Recover.lean)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("verifies the garbage-collection data-loss guard (Gc.lean)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("verifies the sandboxed policy-hook safety properties (Hooks.lean, req 7)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("verifies the flash-controller status state machine (Fmc.lean, gap-closure P1)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("verifies XOR-parity channel reconstruction (Rain.lean, gap-closure P8)")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("has no conflict markers in the firmware status docs")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("keeps rv32 P9 production docs aligned with the hardened scalar floor")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("keeps rv64 P9 production docs honest about the missing real-boot ELF")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("keeps firmware self-test assertion counts current in production docs")
...
```simple
# @req REQ-SSPEC-SYSTEM
step("keeps the Cosmos+ OpenSSD platform descriptor executable and honest")
...
- Canonical SPipe generation for source `96b7fefa24a295dce95f0702efa563b292e177cfa881a5ee19e14eca9ef39bc1`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `96b7fefa24a295dce95f0702efa563b292e177cfa881a5ee19e14eca9ef39bc1`.
...
source: test/03_system/app/nvme_firmware/nvme_firmware_simulation_spec.spl
mirror: doc/06_spec/03_system/app/nvme_firmware/nvme_firmware_simulation_spec.md (current)
findings: 5 blockers: 0
  narrative=100 structure=100 oracle=70
  traceability=100 evidence=70 coverage=100 maintainability=80
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/03_system/app/nvme_firmware/nvme_firmware_simulation_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, unsupported/limitations
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/03_system/app/nvme_firmware/nvme_firmware_simulation_spec.spl:1:1: advice SSDOC-ORA-003 [oracle] (-30): 34 unexplained numeric expected value(s)
  why: Reviewers need to know why a magic expected value is authoritative.
  improve: Name the authoritative expected value or add a '# oracle:' explanation.
test/03_system/app/nvme_firmware/nvme_firmware_simulation_spec.spl:73:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'passes the layered firmware smoke' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/03_system/app/nvme_firmware/nvme_firmware_simulation_spec.spl:89:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'writes, reads, garbage-collects, trims, and survives a power cycle' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/03_system/app/nvme_firmware/nvme_firmware_simulation_spec.spl:122:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'drives the full admin-and-IO-queue controller lifecycle' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
