...
Tests the SDoctest tool that extracts and runs code examples from documentation
comments. Verifies that doctest blocks are correctly parsed, executed, and that
expected outputs are validated against actual results.
...
val sum = create_pair_sum(3, 7)
expect sum == 10
expect create_pair_sum(1, 2) == 3
```
