...
| Category | Compiler |
| Status | Active |
| Source | `test/unit/compiler/mir/bitfield_mir_spec.spl` |
| Updated | 2026-08-26 |
...
check(code.contains("bitfield"))
    ...
    it "supports bool fields (1 bit)":
        step("supports bool fields (1 bit)")
        val code = """
        bitfield Status(u8):
            ready: bool
            error: bool
...
```simple
# @req REQ-SSPEC-UNIT
step("recognizes bitfield definitions")
val code = """
bitfield Flags(u32):
    enabled: bool
    priority: u4
...
```simple
# @req REQ-SSPEC-UNIT
step("supports custom bit widths")
val code = """
...
```simple
# @req REQ-SSPEC-UNIT
step("supports reserved/padding fields")
val code = """
...
```simple
# @req REQ-SSPEC-UNIT
step("handles multiple field updates")
val code = """
bitfield Status(u8):
    ready: bool
    error: bool
...
```simple
# @req REQ-SSPEC-UNIT
step("generates correct getter mask")
...
```simple
# @req REQ-SSPEC-UNIT
step("generates correct setter mask")
# For field at offset 4, width 3:
...
```simple
# @req REQ-SSPEC-UNIT
step("implements hardware control register")
val code = """
...
```simple
# @req REQ-SSPEC-UNIT
step("implements peripheral status register")
val code = """
...
```simple
# @req REQ-SSPEC-UNIT
step("implements DMA configuration")
val code = """
...
```simple
# @req REQ-SSPEC-UNIT
step("implements x86 EFLAGS register")
val code = """
...
```simple
# @req REQ-SSPEC-UNIT
step("detects overflow (too many bits)")
val code = """
...
```simple
# @req REQ-SSPEC-UNIT
step("requires valid backing type")
val code = """
...
```simple
# @req REQ-SSPEC-UNIT
step("lowers field read to shift and mask")
...
```simple
# @req REQ-SSPEC-UNIT
step("lowers field write to clear and set")
...
```simple
# @req REQ-SSPEC-UNIT
step("optimizes away zero shifts for reads")
...
```simple
# @req REQ-SSPEC-UNIT
step("optimizes away zero shifts for writes")
# For field at offset 0, width 1:
...
- Canonical SPipe generation for source `2cdc99085bf9328ec6d8e4208c043016c936ba9b3e2e6cb575724d0b95eeb5b8`; maintenance tool `1`, rules `ssdoc-rules/1`.

Source SHA-256: `2cdc99085bf9328ec6d8e4208c043016c936ba9b3e2e6cb575724d0b95eeb5b8`.
...
source: test/unit/compiler/mir/bitfield_mir_spec.spl
mirror: doc/06_spec/unit/compiler/mir/bitfield_mir_spec.md (current)
findings: 5 blockers: 0
  narrative=100 structure=100 oracle=100
  traceability=100 evidence=70 coverage=100 maintainability=70
  cache=not-used suppressed=0
  lint-owned related rules=SPIPE001,SPIPE002,SPIPE003,SPIPE004,SPIPE005,SPIPE006,SPIPE007
doc/06_spec/unit/compiler/mir/bitfield_mir_spec.md:1:1: advice SSDOC-MNT-005 [maintainability] (-10): generated manual lacks verification or troubleshooting guidance
  why: Operators need recovery and evidence interpretation guidance.
  improve: Author verification and recovery facts in SSpec and regenerate.
doc/06_spec/unit/compiler/mir/bitfield_mir_spec.md:1:1: warning SSDOC-MNT-008 [maintainability] (-20): manual is missing: purpose, audience, scope, assumptions/preconditions, primary workflow, unsupported/limitations, recovery/troubleshooting
  why: A test dump is not a complete professional specification manual.
  improve: Author the missing facts in SSpec and regenerate through canonical SPipe docgen.
test/unit/compiler/mir/bitfield_mir_spec.spl:18:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'recognizes bitfield definitions' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/unit/compiler/mir/bitfield_mir_spec.spl:29:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'supports bool fields (1 bit)' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
test/unit/compiler/mir/bitfield_mir_spec.spl:40:1: warning SSDOC-EVD-001 [evidence] (-10): visible scenario 'supports custom bit widths' has no retained capture or evidence
  why: Professional manuals need retained observable evidence.
  improve: Capture typed user/operator-facing evidence or explain why the oracle is complete.
