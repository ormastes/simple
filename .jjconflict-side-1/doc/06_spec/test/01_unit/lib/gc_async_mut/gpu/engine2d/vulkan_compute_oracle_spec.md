...
#### Vulkan draw_image uploads source pixels into the device framebuffer

- Create the Vulkan backend; if unavailable, assert zero devices honestly
- var vk = VulkanBackend create
   - Expected: rt_vulkan_device_count() equals `0`
- Blit a 2x2 image and verify device readback matches the source
- vk clear
- vk draw image
   - Expected: px[8 * w + 8] equals `src[0]`
   - Expected: px[8 * w + 9] equals `src[1]`
   - Expected: px[9 * w + 8] equals `src[2]`
   - Expected: px[9 * w + 9] equals `src[3]`
   - Expected: px[7 * w + 7] equals `bg`
...
#### Vulkan draw_image clipping matches the SoftwareBackend reference

- Create the Vulkan backend; if unavailable, assert zero devices honestly
- var vk = VulkanBackend create
   - Expected: rt_vulkan_device_count() equals `0`
- Render the same partially clipped image through Vulkan and SoftwareBackend
- var sw = SoftwareBackend create
- sw init
- vk clear
- sw clear
- vk draw image
- sw draw image
- Every readback pixel matches the software clipping contract
   - Expected: pixel_mismatches(vk.read_pixels(), sw.read_pixels()) equals `0`
- An active Engine2D clip rectangle is also honored
- vk clear
- sw clear
- vk set clip
- sw set clip
- vk draw image
- sw draw image
   - Expected: pixel_mismatches(clipped, sw.read_pixels()) equals `0`
   - Expected: clipped[2 * w + 0] equals `bg`
   - Expected: clipped[2 * w + 1] equals `src[2]`
   - Expected: clipped[3 * w + 1] equals `bg`
- An active Engine2D mask is also honored
- vk clear clip
- sw clear clip
- vk set mask
- sw set mask
...
- vk draw image
- sw draw image
   - Expected: pixel_mismatches(masked, sw.read_pixels()) equals `0`
   - Expected: masked[2 * w + 0] equals `src[1]`
   - Expected: masked[2 * w + 1] equals `bg`
...
- **Plan:** [doc/03_plan/ui/production_readiness_master_plan_2026-07-02.md](doc/03_plan/ui/production_readiness_master_plan_2026-07-02.md)
