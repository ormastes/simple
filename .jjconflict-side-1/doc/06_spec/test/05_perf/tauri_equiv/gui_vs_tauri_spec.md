...
WARNING — the numbers in make_tauri_bench_ok() below are SYNTHETIC FIXTURES,
not measured. They exist only to exercise the report schema and the
ratio/threshold pass-fail logic; do NOT read them as a real Simple-vs-Tauri
benchmark result. Real measured mobile open-time (Simple GUI vs a native
baseline app, via `am start -W` on a device-class emulator) lives in
doc/09_report/mobile_tauri_android_live_mdi_event_evidence_2026-06-06.md.
This desktop harness still needs a real measuring runner (workflow_driver.spl
currently simulates operations); tracked as a separate cleanup.
