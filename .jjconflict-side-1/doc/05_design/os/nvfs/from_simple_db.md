# Simple DB Filesystem Requirements for nvfs

- **Audience:** the parallel nvfs (NVMe-aware filesystem) design agent/track
- **Author:** Simple DB track (SStack Phase 3, `.sstack/simple-db-nvfs-storage/`)
- **Status:** v0 — initial upfront contribution, derived from the Simple DB research doc (`doc/01_research/simple_db_research.md`) and the Simple DB engine design (`../simple_db_design.md`)
...
| Clone-range | Page-version clone during checkpoint compaction | Not required | Simple DB-only; NVFS offers `arena_clone_range` as a P1 primitive, Simple DB pays for it |
...
## Workload profile (what Simple DB actually does to storage)

To help NVFS size its primitives correctly, here's Simple DB's I/O profile:
...
Simple DB expects to drive NVFS near the device's 4K-random-write durability envelope during sustained OLTP. Benchmark discipline requires NVFS to honor the SSD-iq critique (see research §8): NVFS must expose `fs_caps().write_amp_hint` so Simple DB can report realistic WAF numbers without guessing.
...
- `Durability ∈ { BUFFERED, DURABLE_ON_RETURN, DURABLE_GROUP_COMMIT }`. Simple DB WAL uses `DURABLE_GROUP_COMMIT`, which NVFS implements by coalescing concurrent appends until one NVMe Flush / FUA completes.
...
- **Failure modes:** media error → Simple DB fails the WAL write and retries a new arena (crash-safe); out-of-space → Simple DB seals current WAL arena and opens a new one.
...
- Simple DB MVCC uses this exactly: a sealed checkpoint arena cannot be discarded until every transaction snapshot predating its seal has completed (research §11.3).
- **Failure modes:** `DiscardBlocked(held_generations)` → Simple DB delays vacuum and retries.
...
- Copy `len` bytes from `(src, src_off)` to `(dst, dst_off)`. On capable devices, NVFS implements via NVMe `Copy` or ZNS `Zone Append`; on baseline NVMe, it falls back to read-then-write in NVFS (no bounce through Simple DB).
- Used for: checkpoint compaction (fold live page-versions into the sealed arena), snapshot materialization (M4+).
- **Atomicity:** clone is not atomic; Simple DB sequences clone → pmap update → old-arena discard.
...
- Simple DB caches `FsCaps` at mount and refreshes at each checkpoint (5–60 s cadence). Changes between refreshes are safe — Simple DB never assumes a capability it did not observe at cache time.
...
- Simple DB's `Durability::DURABLE_ON_RETURN` maps to `arena_fua_append`; `DURABLE_GROUP_COMMIT` maps to `arena_append + arena_flush` with coalescing (see S2).
...
- If `fs_caps().supports_zns`, NVFS may implement `arena_append_aligned` on `DB_WAL` using `Zone Append` (implicit offsets, no head-of-line blocking). Simple DB must not assume this — the returned offset is still monotonic in either mode.

### S-stretch-2. FDP PIDs per storage class

- If `fs_caps().supports_fdp`, NVFS maps each `StorageClass` to a distinct Placement ID so device-side GC can separate WAL from page-main from temp. Simple DB benefits without changing its API usage.

### S-stretch-3. Namespace-per-class

- On multi-namespace controllers, NVFS may dedicate a namespace per class, delivering hardware-level isolation (most powerful but rarely available). Simple DB takes this as a pure perf win.
...
- If exposed, NVFS may register the WAL in-flight buffer in CMB (commit-latency reduction). Simple DB is indifferent at the API level.

### S-stretch-6. Dataset Management (DSM) trim on arena_discard

- NVFS SHOULD issue DSM trim when a discarded arena's LBAs are released back to the free pool. Simple DB does not observe this directly but benefits via sustained WAF.

---

## Non-requirements (explicit, to help NVFS scope)

Simple DB does **not** need these; NVFS need not implement them for Simple DB's sake:

- **POSIX permissions / ACLs.** Simple DB is one trusted process; capability-based access through the Simple DB capsule is sufficient.
- **Hard links, symlinks.** Arena IDs are the only naming level Simple DB uses.
- **Case-insensitive paths.** Simple DB names arenas by `(relation_oid, fork_tag, epoch)`, never by user-supplied strings.
- **xattrs.** All metadata Simple DB needs is in its own catalog (`META_DURABLE` arena).
- **mmap semantics.** Simple DB reads explicitly; no page-fault-driven I/O at M1.
- **Concurrent writers on one arena.** Simple DB serializes writers per arena; WAL has at most one active writer via a per-node lock.
...
- **Fsync-on-close for every arena.** Simple DB calls `arena_flush` / `arena_seal` explicitly.
- **Crash-consistent `arena_create`.** If `arena_create` crashes mid-creation, NVFS may garbage-collect orphan arenas at mount; Simple DB will re-create on the next operation.
...
- **M1–M2 (nogc_sync_mut):** Simple DB issues all NVFS calls synchronously from the engine's WAL-writer thread and checkpoint thread. NVFS may still coalesce internally, but Simple DB sees blocking calls.
- **M3 (nogc_async_mut):** Simple DB introduces an AIO path (io_uring_cmd). NVFS must offer an async variant of `arena_read` / `arena_append_aligned` that returns a completion handle. Signatures to be defined in a later revision of this doc; for now, document the migration plan in §Open coordination.
- **M4+ (nogc_async_mut):** async checkpoint, async vacuum. No new NVFS primitives needed — same async variants as M3.

Simple DB never uses `gc_async_mut` or `nogc_async_mut_noalloc` against NVFS.

---

## Alignment with research doc and design docs

| Research section | This doc section | Simple DB design section |
|---|---|---|
