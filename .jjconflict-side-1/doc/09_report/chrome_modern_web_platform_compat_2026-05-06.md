...
- `test/01_unit/lib/gc_async_mut/gpu/browser_engine/browser_renderer_dom_events_spec.spl`
- `test/01_unit/lib/gc_async_mut/gpu/browser_engine/browser_renderer_hit_test_events_spec.spl`
...
Basic selector support now includes bounded universal `*` matching in the fallback pixel renderer alongside existing type, class, id, compound, and selector-list coverage; composed tree matching, non-`div` target rendering, and specificity parity are still not claimed.
...
CSS custom property support now resolves `var(--name)` and `var(--name, fallback)` from `:root` rules, element inline styles, and all CSS rule blocks for `background-color` and `background` shorthand, with recursive var() fallback resolution; nested `var()` beyond single depth, cyclic variables, typed registration, and cascade parity are still not claimed.
CSS `@supports` feature queries are now flattened via `br_flatten_supports_blocks`, covering `@supports (property: value)` application, unsupported property rejection, and `not` negation; complex combinators (`and`/`or`) and `selector()` queries are still not claimed.
Descendant combinator support now includes bounded fallback pixel matching for ancestor `tag/.class/#id` selectors targeting nested `div/.class/#id` nodes with sibling rejection; arbitrary selector chains, non-`div` target rendering, composed tree matching, DOM style-block parity, and specificity parity are still not claimed.
Child combinator support now includes bounded fallback pixel matching for direct `body > div/.class/#id` rules and ancestor `tag/.class/#id > div/.class/#id` rules with nested descendant rejection; arbitrary selector chains, non-`div` target rendering, DOM style-block parity, and specificity parity are still not claimed.
...
104/104 WPT-shaped CSS tests pass across 9 spec files in `test/03_system/feature/web_platform/css/`. Categories: selectors and colors (57), scorecard (20), transforms (5), animations (5), custom properties (4), object-fit (4), @supports (3), scrollbar (3), sticky positioning (3). New coverage added for `@supports` feature queries with `br_flatten_supports_blocks`, CSS `var()` resolution across all CSS rule blocks (not just `:root`), and sticky positioning rendering.

## Remaining Gaps

- WPT CSS subset is 104 curated cases, not a complete WPT import. A practical next step is expanding the subset or adding a fixture importer for broader CSS/HTML rendering cases.
