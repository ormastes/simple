...
- **Test:** `bin/simple test/01_unit/memleak/c_runtime_leak_spec.spl` — 30/30 pass
- **ASan:** `ASAN_OPTIONS=detect_leaks=1 build/asan/simple <cmd>` — 42 paths, zero leaks
- **Build:** `cmake -B build/asan ... && ninja -C build/asan`

## Test File

`test/01_unit/memleak/c_runtime_leak_spec.spl` — 30 sspec tests covering all 9 categories.
