...
- `bin/simple test test/01_unit/compiler/interpreter/tiered_jit_hotspot_spec.spl --mode=interpreter --clean`
  produced no useful output for roughly 90 seconds, then reported
  `Resource temporarily unavailable (os error 11)`.
- Process inspection showed thousands of nested
  `bin/simple test test/01_unit/compiler/mir_opt/general_patterns_backend_recommendation_spec.spl --mode=interpreter --clean`
  processes.
- Additional recursive process chains were observed for:
  ...
  - `bin/simple test test/03_system/infra/heavy_work_preflight_spec.spl`
...
- Rerun the hotspot unit specs and `test/05_perf/compiler/jit_hotspot_plan_bench.spl`
  on a clean host.
...
```bash
src/compiler_rust/target/debug/simple test test/01_unit/compiler/interpreter/tiered_jit_hotspot_spec.spl --mode=interpreter --clean --progress none
...
```bash
bin/simple test test/01_unit/compiler/interpreter/tiered_jit_hotspot_spec.spl --mode=interpreter --clean --progress none
# PASS: 51 tests
