...
- Source repair (2026-08-16): implemented in the isolated B-10–B-14 lane. The browser retains six arena owners with twelve constructor-owned backing stores, resets logical counts, preserves GUI ownership, uses counted pane hit testing, and exposes capacity/reuse receipts. The system SSpec contains a real render/navigation/edit/wheel loop. B-14 remains open because no admitted pure-Simple runtime was available to compile the sources or provide external allocator evidence; in-band counters alone do not close NFR-011.
