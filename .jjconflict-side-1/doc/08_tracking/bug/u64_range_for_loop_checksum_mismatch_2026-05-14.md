...
`while` loops in `test/05_perf/collections/collection_simple.spl` to canonical
...
```bash
timeout 150s env SIMPLE_COLLECTION_BENCH_SOURCE_CLOSURE=1 SIMPLE_COLLECTION_BENCH_CLEAN=1 SIMPLE_COLLECTION_BENCH_CPU=native test/05_perf/collections/run_collection_benchmarks.shs
```
