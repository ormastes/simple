...
Status: FIXED (2026-05-03). Root cause: `bind_args_with_values` in `interpreter_call/core/arg_binding.rs` wa
...
- `test/01_unit/os/crypto/sha384_kat_spec.spl` — KAT spec, includes a
  `_sha384_diag_big_sigma0_h0` regression guard that fails if this bug
  comes back in `_logical_shr64`.
- `test/01_unit/os/crypto/sha512_kat_spec.spl` — added 2026-05-02; verifies
