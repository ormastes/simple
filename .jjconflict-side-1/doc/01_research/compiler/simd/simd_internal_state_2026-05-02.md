...
*Cross-ref: `doc/04_architecture/language/portable_simd_fp_modules.md` — the PSFM architecture document defines REQ-PSFM-001–011 for this capability registry.*
...
*Cross-refs: `doc/04_architecture/ml/gpu_acceleration_plan.md` (GPU acceleration architecture); `doc/04_architecture/ml/compositor_gpu_architecture.md` (three-layer GPU/compositor model).*
...
*Cross-ref: `doc/01_research/compiler/simd/cpu_simd_scheduling.md` — TBB-style work-stealing scheduler design for `@simd` kernels; complements the vectorizer's cost model.*
