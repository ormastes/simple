...
- Prior research in `doc/01_research/domain/t32_mcp_cli_async_and_window_ux.md` recommends not building MCP status around transient GUI chrome. It prioritizes explicit state queries, capturable status windows, AREA output, and only then message-line text.
...
- Important constraint from `doc/01_research/local/ui_access_protocol.md`: v1 intentionally materializes from `UISession`, `UITree`, widget registry, and `SurfaceManager` rather than external OS accessibility APIs.
