...
- [`src/app/dashboard/main.spl`](src/app/dashboard/main.spl) now routes heavy dashboard commands through [`src/app/dashboard/framework_policy.spl`](src/app/dashboard/framework_policy.spl).
- [`src/app/dashboard/framework_policy.spl`](src/app/dashboard/framework_policy.spl) isolates `collect`, `export`, `query`, `compare`, `serve`, `gui`, and `agents --gui` into child workers.
- [`src/os/crash_policy.spl`](src/os/crash_policy.spl) defines default fault domains and runtime budgets:
...
- [`src/os/desktop/app_manifest.spl`](src/os/desktop/app_manifest.spl) already imports crash-policy defaults for launcher metadata.
- Rust logging startup was hardened in:
  - [`src/compiler_rust/log/src/lib.rs`](src/compiler_rust/log/src/lib.rs)
  - [`src/compiler_rust/driver/src/log.rs`](src/compiler_rust/driver/src/log.rs)
  - [`src/compiler_rust/driver/src/cli/init.rs`](src/compiler_rust/driver/src/cli/init.rs)
...
4. Container workflow documentation is stale.
   Multiple docs and compose files still point to `docker/Dockerfile.*`, but the actual maintained Dockerfiles live under [`tools/docker`](tools/docker).

5. Existing dashboard system spec is stale-path oriented.
   [`test/03_system/infrastructure/dashboard_system_spec.spl`](test/03_system/infrastructure/dashboard_system_spec.spl) still targets `doc/dashboard/...` and direct `.spl` execution instead of the current `simple dashboard` CLI path and current `doc/10_metrics/dashboard/...` layout.

## Existing Runtime Enforcement Mechanisms

- Shell-level resource caps:
  - [`src/lib/nogc_sync_mut/io/process_limit_enforcer.spl`](src/lib/nogc_sync_mut/io/process_limit_enforcer.spl)
  - [`src/lib/nogc_sync_mut/io/process_ops.spl`](src/lib/nogc_sync_mut/io/process_ops.spl)
...
- Rust watchdog enforcement:
  - [`src/compiler_rust/compiler/src/watchdog.rs`](src/compiler_rust/compiler/src/watchdog.rs)
...
- The practical container path in this repo is direct `docker build` / `docker run` using [`tools/docker/Dockerfile.test-isolation`](tools/docker/Dockerfile.test-isolation).
