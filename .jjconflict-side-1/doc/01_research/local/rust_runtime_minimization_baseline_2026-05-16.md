...
- TUI smoke fixture found: `test/03_system/llm_dashboard_tui_smoke.spl` — runs in interpreter mode, wraps a TUI app with key input; not a compiled native binary.
- Network smoke fixture found: `test/05_perf/webserver/native_tcp_bind_variable_smoke.spl` — binds TCP in interpreter mode; not a compiled native binary.
- No file I/O standalone smoke fixture found.
- No pre-built `.native` or `.stripped` artifacts exist in `test/03_system/` or `test/02_integration/`.
...
- `test/03_system/exe_size_budget_spec.spl`: checks total stripped size ≤ 12 MB budget. Prints `nm` top symbols as diagnostic on failure, but does **not** assert absence of Rust symbols.
- `test/01_unit/compiler/linker/object_emitter_spec.spl`: checks `file_size > 0` for object files.
- `test/01_unit/app/build/.spipe_matchers_bootstrap_spec.spl`: has commented-out `binary_size` assertions (all lines start with `#`).
- `test/05_perf/port_algorithms/run_port_algorithm_benchmarks.shs`: uses `nm` and `readelf` for relocation analysis, not Rust-specific.
