...
- A module that exists in one family but is imported by another = import debt; fix with a facade or import rewrite, never a wider cross-family fallback (`doc/03_plan/import_warning_debt_2026-05-13.md`). The seed std `core/traits.spl` defines Rust-style `Read/Write/Seek` (no `Close`) with different signatures — do NOT conflate with `common/io`.
