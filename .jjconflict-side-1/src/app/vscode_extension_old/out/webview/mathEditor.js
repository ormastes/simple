...
var MathEditorWebview = (() => {
  ...
  // src/math/webview/mathEditorWebview.ts
  var mathEditorWebview_exports = {};
  __export(mathEditorWebview_exports, {
    boot: () => boot,
  ...
  // src/math/webview/simpleLang.ts
  ...
  var MATH_PREFIXES = /* @__PURE__ */ new Set(["m", "loss", "nograd"]);
  ...
  // src/math/webview/mathEditorWebview.ts
  var MATH_BLOCK_REGEX = /\b(?<prefix>m|loss|nograd)\{([^}]*(?:\{[^}]*\}[^}]*)*)\}/gs;
  ...
  var MathPlaceholderWidget = class extends WidgetType {
    ...
    toDOM() {
      ...
      wrap.textContent = this.prefix === "m" ? `m{ ${this.content} }` : `${this.prefix}{ ${this.content} }`;
      return wrap;
    }
    ignoreEvent() {
      return false;
  ...
  function buildMathDecorations(view, renderedBlocks) {
  ...
  function createEditor(parent, initialText, onEdit, onSelectionChange) {
    let renderedBlocks = /* @__PURE__ */ new Map();
    let isApplyingSync = false;
    let editTimer = null;
    const mathPlugin = ViewPlugin.define((view2) => ({
      decorations: buildMathDecorations(view2, renderedBlocks),
      update(update) {
        if (update.docChanged || update.selectionSet) {
  ...
  function boot(vsCodeApi) {
    ...
    let editor = null;
    const renderedBlocksMap = /* @__PURE__ */ new Map();
  ...
  return __toCommonJS(mathEditorWebview_exports);
})();
//# sourceMappingURL=mathEditor.js.map
