import { buildHighlightedSourcePreview, escapeForHtml, serializeForScript, type MathPreviewPanelState, type MathSyncPanelState } from './mathPanelShared';
...
function panelShell(title: string, subtitle: string, body: string): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    <div class="panel-shell">
        <div class="panel-header">
            <div>
                <div class="panel-title">${escapeForHtml(title)}</div>
                <div class="panel-subtitle">${escapeForHtml(subtitle)}</div>
            </div>
        </div>
        ${body}
    </div>
...
export function buildMathPreviewPanelHtml(state: MathPreviewPanelState): string {
    const block = state.block;
    const rendered = block
        ? `<div class="math-preview">
                <div class="math-indicator">${escapeForHtml(`${block.indicator} ${block.label}`.trim())}</div>
                <div class="math-rendered">${escapeForHtml(block.pretty)}</div>
            </div>`
        : `<div class="empty-state">Move the caret into a math block to render it.</div>`;

    const source = block
        ? `<div class="math-source">${escapeForHtml(block.content)}</div>`
        : `<div class="empty-state">No active math block.</div>`;

    const statusClass = `status-${state.statusKind}`;
    return panelShell(
        'Math Preview',
        'Char-based preview for the active math block',
        `
        <div class="panel-grid">
            <div class="card">
                <div class="section-label">Status</div>
                <div class="status ${statusClass}">${escapeForHtml(state.statusMessage)}</div>
            </div>
            <div class="card">
...
export function buildMathSyncPanelHtml(state: MathSyncPanelState): string {
    const initialState = serializeForScript(state);
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    <div class="panel-shell">
        ...
        <div class="panel-grid" style="grid-template-columns: minmax(0, 1fr) minmax(280px, 0.9fr); align-items: start;">
            ...
            <div class="card" style="display:flex; flex-direction:column; gap:12px;">
                <div class="section-label">Status</div>
                <div id="render-status" class="status ${state.statusKind === 'error' ? 'status-error' : state.statusKind === 'ok' ? 'status-ok' : 'status-info'}">${escapeForHtml(state.statusMessage)}</div>
                <div class="section-label">Rendered</div>
                <div id="rendered" class="math-preview">
                    ${state.activeBlock ? `
                        <div class="math-indicator">${escapeForHtml(`${state.activeBlock.indicator} ${state.activeBlock.label}`.trim())}</div>
                        <div class="math-rendered">${escapeForHtml(state.activeBlock.pretty)}</div>
                ...
                <div id="source-preview" class="math-source">
                    ${state.activeBlock
                        ? buildHighlightedSourcePreview(state.activeBlock.content, state.activeSelectionStart, state.activeSelectionEnd)
