...
impl Context {
    ...
    pub fn alloc_with_usage<T: Clone + Default>(&self, len: usize, usage: BufferUsage) -> GpuResult<Buffer<T>> {
    ...
    /// Launch a kernel.
    pub fn launch(&self, _kernel: &CompiledKernel, _global_size: [u32; 3], local_size: [u32; 3]) -> GpuResult<()> {
    ...
    pub fn launch_1d(&self, kernel: &CompiledKernel, global_size: u32, local_size: u32) -> GpuResult<()> {
    ...
    pub fn execute_on_buffers<T, F>(&self, input: &Buffer<T>, output: &mut Buffer<T>, f: F) -> GpuResult<()>
    where
        T: Clone + Default,
        F: Fn(&T) -> T,
    {
    ...
    pub fn execute_binary<T, F>(&self, a: &Buffer<T>, b: &Buffer<T>, output: &mut Buffer<T>, f: F) -> GpuResult<()>
    where
        T: Clone + Default,
        F: Fn(&T, &T) -> T,
    {
        if a.len() != b.len() || a.len() != output.len() {
            return Err(GpuError::InvalidArgument("All buffer sizes must match".to_string()));
        }
...
mod tests {
    ...
    fn test_execute_on_buffers() {
        ...
        ctx.execute_on_buffers(&input, &mut output, |x| x * 2.0).unwrap();
    ...
    fn test_execute_binary() {
        ...
        ctx.execute_binary(&a, &b, &mut output, |x, y| x + y).unwrap();
