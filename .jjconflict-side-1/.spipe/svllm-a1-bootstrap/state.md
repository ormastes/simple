...
- [x] AC-6: 19 unit tests under `test/01_unit/lib/gc_async_mut/svllm/model_executor/model_loader/` — all passing
...
- [x] AC-9: Submodule registered at `examples/svllm` — gitlink stabilized at `160000 commit 4de0f9256cc8354b6409469c69553a79e2f4d5bf`; working tree restored via `git checkout HEAD -- .` in quiet repo (2026-05-19). Outer `git status` clean; `LICENSE`, `README.md`, `doc/`, `src/` all present.
...
### 4-spec
Specs:
- `test/01_unit/lib/gc_async_mut/svllm/model_executor/model_loader/weight_utils_spec.spl` — dtype + size helpers (8 tests, all pass)
...
### 7-verify
- ✅ `bin/simple test test/01_unit/lib/gc_async_mut/svllm/` — 19/19 pass
...
## Known Issues
- ~~Submodule gitlink race~~ — **RESOLVED 2026-05-19**: gitlink held at `160000 commit 4de0f92...`; working tree restored cleanly.
- Bundled commits — parallel `/dev` tracks share jj working copy; per-phase commit hygiene requires serializing `/dev` runs
- Commits `162b39016c`, `ac1564c8c4`, `bdb49da17d`, and the most recent submodule-fix commit may need `jj abandon` or `git reset --hard` in a quiet session (separate ship-phase cleanup, does not block A1 closure)

## Pipeline Status: CLOSED (2026-05-19)
All 9 ACs are marked `[x]`. No blocking issues remain for Phase A1.

## Next Steps (Phase A2+, separate /dev runs)
1. Phase A2: real safetensors parser + tensor_pack writer, then connect to packer CLI
2. Phase A3: streaming loader + pinned DRAM staging
3. (Optional) Quiet-repo commit history cleanup: `jj abandon` the 3 race-polluted commits
