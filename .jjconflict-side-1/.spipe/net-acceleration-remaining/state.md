...
## Acceptance Criteria
- [x] AC-1: TCP socket state machine — TcpState with SYN_SENT/CONNECTING/ESTABLISHED/CLOSE_WAIT transitions, active-open stays CONNECTING until ESTABLISHED
- [x] AC-2: Socket publication — passive-open accepted connections queued only after ESTABLISHED, socket table tracks state
- [x] AC-3: Net backend capabilities — NetBackendCapabilities with portable/sendfile/zero-copy/packet-io/sriov/rdma capability tiers
- [x] AC-4: IoDriver capability exposure — net_capabilities() exposes runtime backend capabilities to library users
- [x] AC-5: Network DMA buffers — RX/TX buffers using shared DMA descriptor model (same as file/block/VirtIO-GPU)
- [x] AC-6: SR-IOV capability gating — SR-IOV reporting gated on isolation-domain state, trusted vs isolated distinction
- [x] AC-7: HTTP server capability reporting — backend capability output on startup, throughput tier selection
- [x] AC-8: QEMU test timeout — timeout-safe test phases, exact phase identification on failure, clean QEMU termination
- [x] AC-9: Benchmark fixture — HTTP server benchmark with capability output, latency/throughput baseline
- [x] AC-10: Verification spec — 20 tests covering TCP states, capabilities, DMA, HTTP reporting, timeouts
...
- test/01_unit/os/net_acceleration_spec.spl — 20 tests
