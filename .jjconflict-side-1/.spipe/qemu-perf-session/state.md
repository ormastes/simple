...
- [ ] AC-3: `separate_qemu_tests()` recognizes `test/03_system/*qemu*` and `test/03_system/*baremetal*` paths; system specs have `@session:`/`@arch:` markers
...
**Routing gap:**
- `separate_qemu_tests()` only checks `/qemu/` or `/baremetal/` in path — misses `test/03_system/*qemu*` entirely.
...
**Skipped tests (AC-2):** 192 skipped tests are dominated by `test/01_unit/compiler/coverage/branch_coverage_*` — scaffolding for branch coverage, not perf-related. Expected skips.
...
- **D-4: Fix routing gap in separate_qemu_tests()** — Currently only checks `/qemu/` or `/baremetal/` in path. Must also match `test/03_system/*qemu*` and `test/03_system/*baremetal*` patterns.
...
|--------|------|------|--------------|
| test_classification | `src/lib/nogc_sync_mut/test_runner/test_classification.spl` | Fix `separate_qemu_tests()` to recognize `test/03_system/*qemu*` and `test/03_system/*baremetal*` paths | Modified |
...
| system_spec_markers | Multiple `test/03_system/*_in_qemu_spec.spl` files | Add `# @session:mutating` and `# @arch:x86_64` marker comments to top of each spec | Modified |

### Dependency Map

```
test/03_system/*_in_qemu_spec.spl
...
2. separate_qemu_tests(files) -> (normal_files, qemu_files)
   - NOW also matches test/03_system/*qemu* and test/03_system/*baremetal*
...
### Spec Files
- `test/01_unit/lib/test_runner/test_classification_system_routing_spec.spl` — 8 specs covering AC-3 (separate_qemu_tests routing + classify_test markers)
- `test/01_unit/qemu/qemu_harness_acquire_or_spawn_spec.spl` — 9 specs covering AC-4, AC-5, AC-6, AC-7 (broker-first acquire, release, kernel sharing, backward compat)
- `test/01_unit/app/test_daemon/test_daemon_qemu_sharing_spec.spl` — 2 new specs added covering AC-6 (kernel-path session sharing)
...
| AC-3 | `test_classification_system_routing_spec.spl` | "routes test/03_system/*_in_qemu_spec.spl to qemu bucket" | Failing (no impl) |
| AC-3 | `test_classification_system_routing_spec.spl` | "routes test/03_system/*baremetal* to qemu bucket" | Failing (no impl) |
| AC-3 | `test_classification_system_routing_spec.spl` | "routes test/03_system/replay_qemu* to qemu bucket" | Failing (no impl) |
| AC-3 | `test_classification_system_routing_spec.spl` | "does not route non-qemu system specs to qemu bucket" | Failing (no impl) |
| AC-3 | `test_classification_system_routing_spec.spl` | "mixed bucket — normal, /qemu/, and test/03_system/*qemu* all routed" | Failing (no impl) |
...
- AC-3: `separate_qemu_tests()` already routes `test/03_system/*qemu*` and `test/03_system/*baremetal*` paths (verified in source)
...
- **Broker spec (6/6 passed):** `bin/simple run test/01_unit/app/test_daemon/test_daemon_qemu_broker_spec.spl` — all 6 examples pass. Session acquire, reuse, eviction, release, cleanup, shutdown all work correctly.
