...
> Commands route to Simple (.spl) apps by default. Some legacy hybrid commands still expose explicit Rust guards; `test` is pure-Simple and fails closed.
...
## Overview
Commands route to Simple (.spl) apps by default. Some legacy hybrid commands
still expose explicit Rust guards; `test` is pure-Simple and fails closed.

## Dispatch Pattern
1. Check env guard (SIMPLE_<CMD>_RUST) -> use Rust fallback for hybrid commands
2. Check for Rust-only flags -> use Rust fallback for hybrid commands
3. Find Simple app at src/app/<tool>/main.spl -> dispatch
4. Fall back to Rust implementation

## Migrated Commands (13 total)
fmt, lint, spipe-docgen, context, mcp, verify, dashboard,
coverage, depgraph, lsp, dap, test, test-daemon
...
- expect guard starts with
- expect guard ends with
...
#### guard count tracks hybrid and legacy pure commands
...
```simple
val guards = [
    "SIMPLE_FMT_RUST", "SIMPLE_LINT_RUST",
...
- expect unique count == guards len
...
#### EDGE: total migrated command count is 13
...
- expect dispatch commands contains
...
```simple
val pure_simple = ["coverage", "depgraph", "lsp", "dap", "test", "test-daemon"]
...
```simple
val hybrid = ["fmt", "lint", "spipe-docgen", "context",
...
val hybrid = ["fmt", "lint", "spipe-docgen", "context",
              "mcp", "verify", "dashboard"]
val pure = ["coverage", "depgraph", "lsp", "dap", "test", "test-daemon"]
expect hybrid.len() + pure.len() == 13
```
...
- expect check args len
- expect check args contains
...
```simple
# Priority for hybrid commands: env guard > rust-only flags > simple app > rust fallback
val env_set = true
