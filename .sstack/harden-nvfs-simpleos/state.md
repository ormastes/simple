...
3. Copied the hardened connector into the worktree. `.sstack/` is gitignored (verified `.gitignore:133`) — state.md stays local-only by design. The spec file was already on origin via the wip-snapshot, so the diff in the worktree was exactly 1 file.
