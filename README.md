...
- **LLM-safe collections ("the dataframe way")**: LLM-written code often turns collection work into hidden O(n²) — `.contains` in loops, nested key-matching loops, growing `seen` lists, sort-to-take-top-k. Simple lints these patterns (COLL002/015/016/020–022), names the typed frame call to switch to (`std.common.frame`: `semi_join`, `index_by`, `group_by_key`, `distinct_by`, `top_k_by`, …), auto-fixes the provable cases, and ships an opt-in collection profiler that recommends the switch with evidence. Guide: [doc/07_guide/language/collections/dataframe_way.md](doc/07_guide/language/collections/dataframe_way.md)
