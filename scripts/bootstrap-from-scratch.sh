...
#
# Usage:
#   ./scripts/bootstrap-from-scratch.sh [options]
#
# Options:
...
bootstrap_in_freebsd_vm() {
    ...
    run_in_freebsd_vm "cd ~/simple && scripts/bootstrap-from-scratch-freebsd.sh ${SKIP_VERIFY:+--skip-verify} ${KEEP_ARTIFACTS:+--keep-artifacts} ${VERBOSE:+--verbose}"
    local result=$?
    ...
    if [ "$QEMU_FREEBSD" = true ]; then
        if [ -z "$QEMU_VM_PATH" ]; then
            detect_qemu_vm || {
                ...
                err "Or run: bin/simple scripts/download_qemu.spl freebsd"
                exit 1
            }
        fi
