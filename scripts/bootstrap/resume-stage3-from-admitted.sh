...
source_output=${1:?usage: resume-stage3-from-admitted.sh OUTPUT_DIR [--jobs=N]}
shift
jobs=${SIMPLE_BOOTSTRAP_BUILD_JOBS:-}
if [ -n "${SIMPLE_BOOTSTRAP_BUILD_JOBS+x}" ]; then jobs_source=env; else jobs_source=default; fi
for option in "$@"; do
  case "$option" in
    --jobs=*) jobs=${option#*=}; jobs_source=cli ;;
    *) echo "error: unknown Stage 3 resume option: $option" >&2; exit 2 ;;
  esac
done
. "$root/scripts/bootstrap/bootstrap-build-jobs-policy.shs"
bootstrap_build_jobs_resolve "$jobs" "$jobs_source" "" \
  "${SIMPLE_BOOTSTRAP_MAX_BUILD_JOBS:-16}" || exit 2
...
external_output_root=/mnt/data/.simple/bootstrap
bootstrap_stage3_resume_output_path "$source_output" "$root" \
  "$external_output_root" || {
  echo "error: Stage 3 resume output is outside canonical allowlisted roots: $source_output" >&2
  exit 2
}
...
# The Stage-2 admission receipts are immutable: take a fresh Stage-3-pre
# authority snapshot and compare it before admitting either Stage-2 binary or
# touching an existing candidate/recovery tree.  The output directory is
# external to the source checkout, so its own presence cannot satisfy this
# binding by accident.
bootstrap_stage3_source_snapshot "$resume_source_check" "$root" || exit 1
bootstrap_stage3_git_state "$root" "$resume_git_check" || exit 1
bootstrap_stage3_tool_authority_snapshot "$resume_tool_check" "$path" "$root" || exit 1

# Do not follow either copy of the Stage-2 compiler.  This check is
# intentionally after the fresh source/git/tool snapshots, so an authority
# drift is rejected without using a substituted compiler or deleting evidence.
for stage2_required in "$stage2" "$admitted"; do
  [ -f "$stage2_required" ] && [ -x "$stage2_required" ] && \
    [ ! -L "$stage2_required" ] || exit 1
  [ "$(bootstrap_stage3_canonical_file "$stage2_required")" = \
    "$stage2_required" ] || exit 1
done
cmp -s "$source_before" "$resume_source_check" || exit 1
cmp -s "$git_before" "$resume_git_check" || exit 1
cmp -s "$tool_before" "$resume_tool_check" || exit 1
...
bootstrap_stage3_retain_candidate "$candidate" previous
for old in "$stage3_transcript" "$stage3_log" "$stage3_sanity" "$manifest"; do
  if [ -e "$old" ]; then cp -p "$old" "$archive/$(basename "$old")"; fi
done
...
bootstrap_stage_sanity() (
  ...
  if [ "$version_status" -eq 0 ] && [ "$version_expect_status" -eq 0 ] && \
    ...
    [ "$frontend_status" -eq 0 ] && [ "$frontend_bootstrap_status" -eq 0 ] && \
    [ "$before" = "$after" ]; then sanity_status=pass; fi
