...
if [ "${can_full_bootstrap}" -eq 1 ]; then
  ...
  bootstrap_stage3_run_transcribed \
    ...
    SIMPLE_BOOTSTRAP_DIAG="${SIMPLE_BOOTSTRAP_DIAG:-0}" \
    SIMPLE_COMPILER_TRACE="${SIMPLE_COMPILER_TRACE:-0}" \
    SIMPLE_KEEP_LLVM_IR="${SIMPLE_KEEP_LLVM_IR:-0}" \
    SIMPLE_MEM_SNAPSHOT="${SIMPLE_MEM_SNAPSHOT:-0}" \
    SIMPLE_BUILD_PROGRESS_EVENTS="${build_progress_events}" \
    SIMPLE_BINARY="${stage2_seed_absolute}" -- \
