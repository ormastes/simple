...
Options:
  ...
  --resume-stage3-from-admitted=<output>
                     ...
                     using a new isolated recovery transcript/evidence lane.
...
target=""
verbose=0
jobs="${SIMPLE_BOOTSTRAP_BUILD_JOBS:-}"
if [ -n "${SIMPLE_BOOTSTRAP_BUILD_JOBS+x}" ]; then
  jobs_source=env
else
  jobs_source=default
fi
...
bootstrap_build_jobs_resolve "${jobs}" "${jobs_source}" "" \
  "${SIMPLE_BOOTSTRAP_MAX_BUILD_JOBS:-16}" || exit 1
jobs=${bootstrap_build_jobs_effective}
...
if [ -n "${resume_stage3_output}" ]; then
  ...
  if [ "${bootstrap_build_jobs_source}" = cli ]; then
    exec sh "$(dirname -- "$0")/resume-stage3-from-admitted.sh" \
      "${resume_stage3_output}" \
      --jobs="${bootstrap_build_jobs_requested}"
  fi
  # Default and environment authority are inherited directly so the recovery
  # receipt records the real source rather than relabelling it as forwarded CLI.
  exec sh "$(dirname -- "$0")/resume-stage3-from-admitted.sh" \
    "${resume_stage3_output}"
fi
...
if [ "${stop_after_stage2}" -eq 1 ]; then
  [ "${full_cli}" -eq 0 ] && [ "${release_tests}" -eq 0 ] && \
    [ "${diagnostic_sweep}" -eq 0 ] || {
    echo "error: --stop-after-stage2 is incompatible with Stage 3/4, deploy, release, or diagnostic-sweep options" >&2
    exit 1
  }
  case "${target}" in
    simpleos-x86_64|x86_64-simpleos)
      echo "error: --stop-after-stage2 is unavailable for the SimpleOS target lane" >&2
      exit 1
      ;;
  esac
...
bootstrap_authority_target_requested=${SIMPLE_BOOTSTRAP_AUTHORITY_TARGET_ROOT:-\
"${repo_root}/src/compiler_rust/target"}
bootstrap_authority_target_root=$(bootstrap_stage3_prepare_authority_target_root \
  "${bootstrap_authority_target_requested}") || {
  echo "error: invalid bootstrap authority target root: ${bootstrap_authority_target_requested}" >&2
  echo "Use an absolute, non-symlink path whose parent already exists." >&2
  exit 1
}
...
host_cpus=${bootstrap_build_jobs_host_cpus}
echo "Native build jobs: ${jobs} (requested: ${bootstrap_build_jobs_requested}; source: ${bootstrap_build_jobs_source}; host CPUs: ${host_cpus}; configured max: ${bootstrap_build_jobs_configured_max})"
selfhost_jobs="${jobs}"
...
seed_bin="${bootstrap_authority_target_root}/bootstrap/simple${exe_suffix}"
native_all_lib="${bootstrap_authority_target_root}/bootstrap/${archive_prefix}simple_native_all${archive_suffix}"
compiler_backfill_lib="${bootstrap_authority_target_root}/bootstrap/${archive_prefix}simple_compiler_backfill${archive_suffix}"
rust_authority_lock_root="${bootstrap_authority_target_root}/.bootstrap-authority-locks"
rust_authority_generation_root="${bootstrap_authority_target_root}/bootstrap.generations"
rust_authority_current_marker="${bootstrap_authority_target_root}/bootstrap.current.env"
rust_authority_compatibility_path="${bootstrap_authority_target_root}/bootstrap"
...
if [ "${private_rust_authority_mode}" -eq 0 ] \
...
prepare_rust_authority_workspace() {
  ...
  if [ "${private_rust_authority_mode}" -eq 1 ] \
     && [ -d "${rust_authority_root}/private.generations" ] \
     && [ ! -L "${rust_authority_root}/private.generations" ]; then
    chmod -R u+w "${rust_authority_root}/private.generations" || {
      echo "error: could not thaw prior private Rust generation namespace" >&2
      exit 1
    }
  fi
...
elif [ "${full_bootstrap}" -eq 1 ] \
  && { [ "${private_rust_authority_mode}" -eq 1 ] \
...
if [ "${rust_rebuilt}" -eq 1 ] || [ "${compiler_backfill_rebuilt}" -eq 1 ]; then
  ...
  rust_publication_generation_root=${rust_authority_generation_root}
  if [ "${private_rust_authority_mode}" -eq 1 ]; then
    rust_publication_generation_root="${rust_authority_root}/private.generations"
  fi
  bootstrap_stage3_prepare_seed_generation \
    "${rust_authority_profile_dir}" "${rust_publication_generation_root}" \
...
if [ -n "${resume_stage4_output}" ]; then
  ...
  stage3_jobs_receipt="${stage3_provenance_dir}/effective-build-jobs.env"
  if [ "${stop_after_stage2}" -eq 0 ]; then
    [ ! -e "${stage3_jobs_receipt}" ] && [ ! -L "${stage3_jobs_receipt}" ] || {
      echo "error: Stage 3 effective build jobs receipt collision: ${stage3_jobs_receipt}" >&2
      exit 1
    }
  fi
  ...
  rm -f "${stage3_provenance_manifest}" \
    "${stage3_source_before}" "${stage3_source_after}" "${stage2_source_after}" \
    "${stage3_git_before}" "${stage3_git_after}" "${stage2_git_after}" \
  ...
  if [ "${private_rust_authority_mode}" -eq 1 ]; then
    runtime_origin_absolute=$(bootstrap_stage3_physical_directory \
      "${private_rust_authority_path}") || {
      echo "error: missing private Rust runtime authority" >&2
      exit 1
    }
  else
    bootstrap_acquire_rust_authority || exit 1
  ...
  if [ "${private_rust_authority_mode}" -eq 0 ]; then
    bootstrap_release_rust_authority || {
  ...
  if [ -n "${SIMPLE_BOOTSTRAP_TEST_PRIVATE_RUST_PUBLICATION_FIXTURE:-}" ]; then
    echo "Private Rust fixture: build, publication, and admission complete."
    exit 0
  fi
  ...
  # Resume and manifest verification reconstruct the admitted Stage 2 command
  # from its transcript. Bind the required large-closure stack explicitly so
  # the hashed argv, executed argv, and canonical transcript stay identical.
  stage2_compile_stack_mib=64
  ...
  bootstrap_build_jobs_write_receipt "${stage3_jobs_receipt}" \
    bootstrap-from-scratch || exit 1
  stage3_jobs_receipt_sha256_before=$(
    bootstrap_stage3_hash_file "${stage3_jobs_receipt}"
  )
  echo "  Stage 3 effective jobs: ${stage3_jobs_receipt} (${stage3_jobs_receipt_sha256_before})"
  ...
  if [ "${stage3_ok}" -ne 1 ]; then
    ...
    BSTAGE3_JOBS_RECEIPT="$(absolute_path "${stage3_jobs_receipt}")"
    BSTAGE3_JOBS_RECEIPT_SHA256_BEFORE="${stage3_jobs_receipt_sha256_before}"
    BSTAGE3_JOBS_EFFECTIVE="${selfhost_jobs}"
...
"${bootstrap_provenance_bundle_fingerprint_before}"
    ...
    export BSTAGE3_ROOT BSTAGE3_MANIFEST BSTAGE3_PLATFORM BSTAGE3_BACKEND \
      ...
      BSTAGE3_JOBS_RECEIPT BSTAGE3_JOBS_RECEIPT_SHA256_BEFORE \
...
run_logged stage4-redeploy-gate run_timeout_kill 180 env \
  SIMPLE_BOOTSTRAP_TEST_BUILD_THREADS="${jobs}" \
...
if [ "${deploy}" -eq 1 ]; then
  stage4_verify_candidate_provenance \
    "${stage4_provenance}" "${full_bin}" "${repo_root}" || {
    echo "ERROR: deploy refused - Stage 4 provenance is stale for the current worktree" >&2
    exit 1
  }
  ...
  if ! stage4_verify_candidate_provenance \
      "${stage4_provenance}" "${full_bin}" "${repo_root}"; then
    echo "ERROR: deploy refused - source/tree identity changed during deployment" >&2
    if [ "${backup_created}" -eq 1 ] && [ -x "${prev_bin}" ]; then
      restore_tmp="${deploy_dir}/.simple${exe_suffix}.identity-restore.$$"
      install -m755 "${prev_bin}" "${restore_tmp}"
      mv "${restore_tmp}" "${deployed_bin}"
    else
      rm -f "${deployed_bin}"
  ...
  [ "${backup_created}" -eq 1 ] && [ -f "${prev_bin}" ] && [ ! -L "${prev_bin}" ] && backup_hash="$(hash_file "${prev_bin}")"
  {
    ...
    echo "stage3_current_acceptance_receipt_sha256=$(bootstrap_stage3_hash_file "${stage3_acceptance_receipt}")"
    echo "repository_root=$(bootstrap_stage3_manifest_value repository_root "${stage4_provenance}")"
