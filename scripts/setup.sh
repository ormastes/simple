...
# Default to a self-hosted CLI when one is available. Prefer a freshly bootstrapped
# full CLI under build/bootstrap/full/<triple>/simple, then fall back to the
# deployed self-hosted binary in bin/release. The live Rust build outputs remain
# available as an explicit opt-in for debugging/bootstrap work.
# Set SIMPLE_SETUP_PREFER_RUST=1 to prefer src/compiler_rust/target/bootstrap/simple
# or src/compiler_rust/target/release/simple when they exist. FreeBSD also has a
# checked-in seed at bin/freebsd/simple.
preferred_runtime=""
if [ "${os}" != "windows" ]; then
  pure_full_runtime="${repo_root}/build/bootstrap/full/${PLATFORM}/simple"
  if [ -x "${pure_full_runtime}" ]; then
    preferred_runtime="${pure_full_runtime}"
  elif [ -n "${SIMPLE_SETUP_PREFER_RUST:-}" ] && [ "${os}" = "${host_runtime_os}" ]; then
    if [ -x "${repo_root}/src/compiler_rust/target/bootstrap/simple" ]; then
...
# Prefer the Rust release binary when present. Some local bin/simple links can
# point at bootstrap builds that do not reliably run MCP entrypoints.
...
generate_mcp_launcher "t32_mcp_server" \
  "src/app/t32_mcp_frontend/main.spl" \
