...
CACHE_ROOT="\${REPO_ROOT}/.simple/cache/${name}"
CACHE_FILE="\${CACHE_ROOT}/main.smf"
STDERR_LOG="\${REPO_ROOT}/.simple/logs/${name}_stderr.log"
mkdir -p "\$CACHE_ROOT" "\$(dirname "\$STDERR_LOG")"
. "\${REPO_ROOT}/config/mcp/mcp_startup_lib.sh"

mcp_debug_init
mcp_find_runtime

SOURCE_SIMPLE_LIB="\${REPO_ROOT}/src"

export SIMPLE_LOG="\${SIMPLE_LOG:-error}"
export SIMPLE_BINARY="\$MCP_RUNTIME"
export SIMPLE_TIMEOUT_SECONDS="\${SIMPLE_TIMEOUT_SECONDS:-86400}"
export SIMPLE_MEMORY_LIMIT_MB="\${SIMPLE_MEMORY_LIMIT_MB:-\${SIMPLE_TEST_MEMORY_LIMIT_MB:-256}}"
${extra_env}
