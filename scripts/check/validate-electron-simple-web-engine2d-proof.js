...
if (proof.blur_or_tolerance_used !== false) {
  ...
  reason = `unexpected-proof-source-file-${proofSourceFileStatus}`;
...
emit('electron_simple_web_engine2d_electron_user_agent', electronUserAgent);
emit('electron_simple_web_engine2d_electron_process_version', electronProcessVersion);
emit('electron_simple_web_engine2d_chrome_process_version', chromeProcessVersion);
...
emit('electron_simple_web_engine2d_captured_argb_height', jsonIntegerTextOrBlank(capturedArgb.height));
...
emit('electron_simple_web_engine2d_captured_argb_checksum', capturedArgbChecksum);
emit('electron_simple_web_engine2d_captured_argb_weighted_checksum', capturedArgbWeightedChecksum);
