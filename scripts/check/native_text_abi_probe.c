/* Behavioural probe for the native-runtime producer contracts the compiler's
 * `text` extern ABI depends on.
 *
 ...
 *
 * Contract 3 -- the same byte-count rule, plus nil-on-unreadable, on
 *   rt_file_read_text_rv. That is the spelling natively compiled Simple code
 *   actually calls; checking only the (ptr, len) form left the real call path
 *   unchecked and this gate green over a live truncation.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
extern int64_t rt_file_read_text_rv(int64_t path_value);
...
int main(int argc, char** argv) {
    ...
    /* Contract 3 -- the SAME two properties on the RuntimeValue spelling.
     * `nm` on any binary produced by `native-build --runtime-bundle
     * core-c-bootstrap` lists rt_file_read_text_rv and does NOT list
     * rt_file_read_text, so the (ptr, len) contract above is checked on a
     * symbol compiled Simple code never calls. When the strlen() truncation was
     * repaired in the two (ptr, len) copies the _rv sibling was missed and this
     * gate stayed green over a live defect: measured 2026-09-07, a natively
     * built FileFingerprint.from_file saw content-len=7 for a 1032-byte `.o`. */
    int64_t path_value = rt_string_new((const uint8_t*)path, path_len);
    int64_t text_rv = rt_file_read_text_rv(path_value);
    if (text_rv == 3) {
        report("binary-read-length-rv", 0,
               "rt_file_read_text_rv returned nil for a readable file");
    } else {
        int64_t rv_len = rt_string_len(text_rv);
        snprintf(detail, sizeof(detail),
                 "rt_file_read_text_rv text_len=%lld expected=8 (pre-fix strlen truncation stops at the NUL and gives 5)",
                 (long long)rv_len);
        report("binary-read-length-rv", rv_len == 8, detail);
    }

    /* An unreadable path must be NIL, not an empty string: every Simple caller
     * spells `?? ""` or `if val content = ...`, which fire on nil only, so the
     * byte-level sha256 fallback in FileFingerprint.from_file was dead code
     * while this returned rt_string_new(NULL, 0). */
    {
        char missing[600];
        snprintf(missing, sizeof(missing), "%s.does-not-exist", path);
        int64_t missing_value =
            rt_string_new((const uint8_t*)missing, (uint64_t)strlen(missing));
        int64_t missing_rv = rt_file_read_text_rv(missing_value);
        snprintf(detail, sizeof(detail),
                 "rt_file_read_text_rv(missing)=%lld expected=3 (RT_NIL; pre-fix returned an empty string)",
                 (long long)missing_rv);
        report("missing-path-is-nil-rv", missing_rv == 3, detail);
    }
