...
if ($SelfTest) {
    ...
    if (@($descriptors | Where-Object CollectorNonceEcho).Count -ne 1 -or
        @($descriptors | Where-Object CollectorNonceEcho).Guest -ne 'x86_64') {
        throw 'matrix self-test: only the source-proven x86_64 collector nonce echo may run'
    }
