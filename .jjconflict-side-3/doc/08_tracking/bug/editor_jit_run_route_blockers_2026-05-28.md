...
- VReg(5) investigation (item 1) — **diagnostic tooling added**. Auto-dump now
  fires on any "Copy: source vreg" compile error, printing post-inline MIR to
  stderr without requiring `SIMPLE_NO_STUB_FALLBACK=1` or `SIMPLE_DUMP_MIR`.
  Static analysis of the codegen + inliner ruled out missing `vreg_values.insert`
  in any MirInst arm. Revised hypothesis: the "JIT-only" claim likely reflects
  DCE omitting `file_modified_time` from the AOT editor build rather than a true
  JIT-vs-AOT backend divergence — `compile_function_body` and
  `inline_small_pure_functions` are shared. The bug likely surfaces after
  multi-level inlining (e.g. `file_exists_mock` → `file_exists` → caller),
  where the Copy-from-Return synthesis in `remap_terminator` (mir_inline.rs)
  creates a VReg whose definition is in a predecessor block not reachable via
  the Variable-based SSA sync. Root cause NOT yet confirmed empirically.
  Next step: run `SIMPLE_LIB=$(pwd)/src bin/simple run src/app/editor/main.spl
  -- --tui` against the seed binary and read the auto-dump from stderr. Do NOT
  add a silent fallback to `compile_copy` — that would silently copy garbage.
