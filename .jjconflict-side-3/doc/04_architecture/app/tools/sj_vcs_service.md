...
business layer per `doc/04_architecture/compiler/mdsoc/mdsoc_architecture_tobe.md`).
...
`scripts/setup/setup.shs`, jj install/upgrade scripts. These predate the daemon and must not depend on it.
...
| `test/01_unit/lib/service/` | Service base unit specs (Phase-4) |
| `test/03_system/sj_concurrency_spec.spl` | AC-6 concurrency smoke test (Phase-4) |
