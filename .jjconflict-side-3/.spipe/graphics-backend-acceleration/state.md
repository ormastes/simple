...
- AC-9: Tauri-equivalent benchmark (test/05_perf/tauri_equiv/) runs startup, windows, scroll, IPC, memory; reports vs baseline.
- AC-10: Chrome-equivalent benchmark (test/05_perf/web_render_chrome/) runs input→script→style/layout→paint/raster→composite phases; normalized to INP-style timing.
- AC-11: All benchmarks write structured output to `test/05_perf/graphics_2d/` and `test/05_perf/local_gpu_check/`; report_spec.spl parses and prints pass/fail vs thresholds.
...
**Existing benchmark infrastructure** — `test/05_perf/graphics_2d/`:
...
**GPU local check** — `test/05_perf/local_gpu_check/`:
- `gpu_perf_compare.spl` — GPU backend comparison runner
- `run_gpu_check.spl` — entry point

**Tauri-equivalent benchmark** — `test/05_perf/tauri_equiv/`:
- `simple_app.spl`, `workflow_driver.spl`, `report_spec.spl`

**Chrome web-render benchmark** — `test/05_perf/web_render_chrome/`:
- `chrome_runner.spl`, `report_spec.spl`, `simple_runner.spl`

**Perf timing library** — `test/05_perf/bench/lib/timing.spl` — reusable wall-clock helpers
...
- `test/05_perf/bench/lib/timing.spl` — wall-clock timing helpers for all new benchmarks
- `test/05_perf/graphics_2d/scene_format.spl` — shared scene fixture (C + Simple use same format)
- `test/05_perf/graphics_2d/report_spec.spl` — structured pass/fail reporting; extend for new ACs
...
- REQ-1 (AC-1): C reference bench (`c_reference/bench_2d.c`) and Simple bench (`bench_2d_gpu.spl`) must share scene fixtures via `scene_format.spl`; both report μs/frame per kernel — area: `test/05_perf/graphics_2d/`
...
- REQ-3 (AC-3): `backend_cuda.spl` + `ffi_cuda.spl` must execute a fill kernel via CUDA Driver API and report timing vs CPU reference — area: `src/lib/gc_async_mut/gpu/engine2d/`, `test/05_perf/graphics_2d/`
...
- REQ-9 (AC-9): `test/05_perf/tauri_equiv/` workflow driver runs startup/windows/scroll/IPC/memory; report_spec produces pass/fail vs thresholds
- REQ-10 (AC-10): `test/05_perf/web_render_chrome/` runner collects input→composite phases; normalized to INP-style (ms) vs Chrome baseline
- REQ-11 (AC-11): All benchmarks write structured output readable by `report_spec.spl`; thresholds defined in spec — area: `test/05_perf/graphics_2d/`, `test/05_perf/local_gpu_check/`
...
| bench_2d_gpu | `test/05_perf/graphics_2d/bench_2d_gpu.spl` | Drive fill/blit/alpha/scroll kernels; write structured μs/frame output | Modified |
| scene_format | `test/05_perf/graphics_2d/scene_format.spl` | Shared fixture format for C + Simple runners | Reused (verify) |
| report_spec | `test/05_perf/graphics_2d/report_spec.spl` | Parse structured output; check vs thresholds; pass/fail | Modified |
| gpu_perf_compare | `test/05_perf/local_gpu_check/gpu_perf_compare.spl` | Cross-backend perf comparison; write to local_gpu_check/ | Modified |
...
| tauri_equiv_driver | `test/05_perf/tauri_equiv/workflow_driver.spl` | Run startup/windows/scroll/IPC/memory phases; write structured output | Modified |
| chrome_equiv_runner | `test/05_perf/web_render_chrome/simple_runner.spl` | Collect input→composite phases; normalize to INP-style ms | Modified |
...
5. MEASURE + COMPARE
   bench_2d_gpu.spl / c_reference/bench_2d.c:
     ...
     → write JSON/TSV to test/05_perf/graphics_2d/
   report_spec.spl:
...
| File | AC | Description |
|------|----|-------------|
| `test/05_perf/graphics_2d/backend_probe_spec.spl` | AC-1 (probe) | Strict probe, no silent fallback, GLSL exclusion, BackendProbeResult field contract |
| `test/05_perf/graphics_2d/vulkan_spirv_spec.spl` | AC-2 | Vulkan SPIR-V shader format, pipeline creation, GLSL exclusion from selection |
| `test/05_perf/graphics_2d/cuda_smoke_spec.spl` | AC-3 | CUDA kernel dispatch, device framebuffer, sync_readback, pixel hash match vs CPU |
| `test/05_perf/graphics_2d/metal_smoke_spec.spl` | AC-4 | Metal MTLComputePipelineState, dispatch, macOS-gated correctness via readback |
| `test/05_perf/graphics_2d/webgpu_real_spec.spl` | AC-5 | WebGPU adapter enumeration, no silent CPU fallback, wgsl shader format |
| `test/05_perf/graphics_2d/cpu_simd_spec.spl` | AC-6, AC-12 | SIMD kernel names, SimdHitCounts per frame, scalar/SIMD parity, target features |
| `test/05_perf/graphics_2d/wm_frame_pacing_spec.spl` | AC-7 | FramePacingCounters fields, event sleep, dirty-rect, idle CPU measurable |
| `test/05_perf/web_render_chrome/web_paint_cache_spec.spl` | AC-8 | Style cache invalidation, paint-command batching, glyph cache, scroll damage |
| `test/05_perf/tauri_equiv/gui_vs_tauri_spec.spl` | AC-9 | Tauri benchmark workflows (9), pass/fail per workflow, memory reporting |
| `test/05_perf/web_render_chrome/chrome_vs_simple_spec.spl` | AC-10 | Chrome pipeline phases (5), INP-style timing, pixel hash, reference_kind |
| `test/05_perf/graphics_2d/shared_helpers_spec.spl` | AC-11 | BenchFrameRecord canonical schema, output paths, FramePacingCounters dedup |
| `test/05_perf/graphics_2d/no_duplication_spec.spl` | AC-12 | No parallel stacks, single probe entry, shared schema, single SimdHitCounts |
| `test/05_perf/graphics_2d/c_vs_simple_2d_spec.spl` | AC-1 (perf) | C vs Simple 2D BenchFrameRecord, NFR ratio 1250, 4 scene fixtures |
| `test/05_perf/graphics_2d/optimization_plugin_spec.spl` | AC-10 (plugin) | auto_vectorize + simd_lowering provider event structure and accumulation |
...
- 2026-05-16 spec: Created 14 BDD spec files (~193 it-blocks). All 12 ACs covered. No imports of non-existent modules — specs use local sentinel structs and constants. Split: 10 files in test/05_perf/graphics_2d/, 3 in test/05_perf/web_render_chrome/, 1 in test/05_perf/tauri_equiv/.
