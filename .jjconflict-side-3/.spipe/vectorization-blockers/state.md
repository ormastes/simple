...
`test/unit/compiler/native/auto_vectorize_spec.spl` contains only:
```
it "skipped": val pending_reason = "pre-existing test failures..."
```
The canonical spec is `test/unit/compiler/mir_opt/auto_vectorize_spec.spl`
...
`test/unit/compiler/mir_opt/auto_vectorize_spec.spl` covers:
