...
docker run --rm \
  ...
  simple-test-isolation:latest test/01_unit/test_file.spl
```
...
- Documentation: `doc/07_guide/container_testing.md`
...
ulimit -v 131072 && \     # 128MB virtual memory limit
  ...
  simple test test/01_unit/test_file.spl
```
...
1. **Structured Output (JSON/XML):**
   ```json
   {
     "test_file": "test/01_unit/string_spec.spl",
     "passed": 42,
     "failed": 1,
     "skipped": 0,
     "duration_ms": 234
...
1. **Batch Tests in Containers:** Group multiple test files per container
   ```bash
   docker run --rm ... test test/01_unit/*.spl  # Run all unit tests together
   ```
...
2. **Keep Containers Running:** Use long-lived containers with exec
   ...
   # Execute tests
   docker exec test-runner simple test test/01_unit/file1.spl
   docker exec test-runner simple test test/01_unit/file2.spl
...
1. **Local Development:** Process isolation (fast iteration)
   ```bash
   simple test test/01_unit/  # Native execution
   ```

2. **Pre-Commit Verification:** Container isolation (matches CI)
   ```bash
   docker run --rm -v $(pwd):/workspace:ro ... test test/01_unit/changed_spec.spl
   ```
...
│ bin/simple test --parallel test/01_unit/std/        │
...
1. **Measure First:**
   ...
   docker run --rm ... test --verbose test/01_unit/test_file.spl
   ```
