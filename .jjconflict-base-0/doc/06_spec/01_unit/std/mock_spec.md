...
1. mock policy init with patterns
   - Expected: r1 is true
2. mock policy init with patterns
   - Expected: r2 is true
3. mock policy init with patterns
   - Expected: r3 is false
...
val r1 = mock_policy_matches_any_pattern("app.cache.redis")
expect(r1).to_equal(true)
mock_policy_init_with_patterns(["*.cache.*", "*.db.*"])
val r2 = mock_policy_matches_any_pattern("app.db.postgres")
expect(r2).to_equal(true)
mock_policy_init_with_patterns(["*.cache.*", "*.db.*"])
val r3 = mock_policy_matches_any_pattern("app.service.user")
expect(r3).to_equal(false)
```
...
1. s1 set
   - Expected: has_exists is true
   - Expected: has_missing is false
...
```simple
val s1 = Stub.create("Config")
s1.set("exists", 1)
val has_exists = s1.has("exists")
expect(has_exists).to_equal(true)
val s2 = Stub.create("Config")
val has_missing = s2.has("missing")
expect(has_missing).to_equal(false)
```
