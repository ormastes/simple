...
- SimpleOS support is evidenced through
  `doc/09_report/simpleos_multicore_green_evidence_2026-06-07.md` and the
  report index. The host cross-language profile must stay linked to that
  SimpleOS evidence, but must not conflate interpreter-run SimpleOS
  scheduler/carrier checks with host runtime-pool M:N profile rows.
...
- Verify SimpleOS evidence is linked but not conflated with host M:N rows
...
step("Verify SimpleOS evidence is linked but not conflated with host M:N rows")
...
expect(report_index).to_contain("simpleos_multicore_green_evidence_2026-06-07.md")
expect(report_index).to_contain("Current SimpleOS green-carrier evidence")
expect(report_index).to_contain("interpreter-run refresh for cooperative, multicore scheduler, green-channel wake, and final handoff blocker specs")
expect(simpleos_report).to_contain("must not be conflated with host runtime-pool profile evidence")
expect(simpleos_report).to_contain("simpleos_cooperative_green_spec.spl")
expect(simpleos_report).to_contain("simpleos_multicore_green_spec.spl")
expect(simpleos_report).to_contain("simpleos_green_channel_wake_spec.spl")
expect(simpleos_report).to_contain("simpleos_green_hardware_handoff_blocker_spec.spl")
```
