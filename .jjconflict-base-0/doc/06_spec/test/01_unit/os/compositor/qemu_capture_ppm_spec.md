...
<!-- sdn-diagram:id=qemu_capture_ppm_spec.arch -->
<details class="sdn-source">
<summary>SDN source</summary>

```sdn id=qemu_capture_ppm_spec.arch hash=sha256:auto render=ascii
@layout dag
@direction LR

qemu_capture_ppm_spec -> std
qemu_capture_ppm_spec -> os
```
...
```ascii generated-from=qemu_capture_ppm_spec.arch hash=sha256:auto
...
| Status | Active |
| Source | `test/01_unit/os/compositor/qemu_capture_ppm_spec.spl` |
| Updated | 2026-06-01 |
...
#### AC-3: screendump probe preserves qemu_vm backend on invalid socket
...
```simple
val result = probe_qemu_vm_screendump("/nonexistent/qmp.sock", "/tmp/test_ppm.ppm")
expect(result.success).to_equal(false)
expect(result.backend_name).to_equal("qemu_vm")
```
...
#### AC-3: screendump probe reports zero mismatches before live capture
...
```simple
val result = probe_qemu_vm_screendump("/nonexistent/qmp.sock", "/tmp/test_ppm.ppm")
expect(result.sample_mismatch_count).to_equal(0)
expect(result.scene_mismatch_count).to_equal(0)
```
