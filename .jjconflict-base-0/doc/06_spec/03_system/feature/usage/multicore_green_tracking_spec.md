...
- Verify the report index promotes the current freshbin profile evidence
   ...
   - Expected: absent_in_text(report_index, "prior live QEMU final-handoff claim is therefore not refreshed by this report") equals `1`
...
expect(report_index).to_contain("release-visible gate remains the fast")
expect(report_index).to_contain("simpleos_green_hardware_handoff_blocker_spec.spl")
expect(report_index).to_contain("2026-06-14 opt-in final AP ring/user QEMU refresh")
expect(report_index).to_contain("x86_64 freestanding runtime ABI now exports `rt_string_char_code_at`, `rt_for_iterable`, and no-op `rt_pool_safepoint`")
expect(report_index).to_contain("the linker preserves the boot `_entry32` ELF entry")
expect(report_index).to_contain("probe-only final handoff clears IF while keeping IOPL=3")
...
expect(report_index).to_contain("non-final live SSpec wrapper passed 3 scenarios")
expect(report_index).to_contain("opt-in final live SSpec wrapper passed 3 scenarios in 74244ms")
expect(report_index).to_contain("HW_HANDOFF_PASS=true`, `USER_ENTRY_PASS=true`, and `USER_SYSCALL_PASS=true")
expect(report_index).to_contain("LLVM backend is unavailable in this driver build")
expect(absent_in_text(report_index, "prior live QEMU final-handoff claim is therefore not refreshed by this report")).to_equal(1)
...
val profile_negative = rt_file_read_text("test/05_perf/profile_scripts/profile_report_contract_negative_test.shs") ?? ""
...
expect(requirements).to_contain("`MulticoreGreenHandle.used_runtime_pool()` and the row proves public")
expect(requirements).to_contain("`counter_delta=<submitted>/<completed>,pending=0,busy=0,blocked=0`")
expect(requirements).to_contain("`used_runtime_pool()` plus public counter-delta evidence")
expect(nfr).to_contain("plus public runtime-pool counter drain evidence")
expect(nfr).to_contain("`counter_delta=<submitted>/<completed>,pending=0,busy=0,blocked=0`")
expect(nfr).to_contain("public `counter_delta=submitted/completed,pending=0,busy=0,blocked=0`")
...
expect(profile_contract).to_contain("require_not_contains")
expect(profile_contract).to_contain("'Pure Simple `multicore_green_spawn`/`rt_pool_*` candidate row'")
expect(profile_contract).to_contain("\"extern fn rt_pool_join\"")
expect(profile_contract).to_contain("\"rt_pool_join(\"")
expect(profile_contract).to_contain("\"val handle = handles[index]\"")
...
expect(profile_contract).to_contain("simpleos_multicore_green_evidence_2026-06-07.md")
expect(profile_contract).to_contain("Current SimpleOS green-carrier evidence")
expect(profile_contract).to_contain("interpreter-run refresh for cooperative, multicore scheduler, green-channel wake, and final handoff blocker specs")
...
expect(sys_test_plan).to_contain("without depending on a volatile pushed commit id")
expect(sys_test_plan).to_contain("2026-06-14 status-alignment slice")
...
expect(agent_plan).to_contain("without depending on a volatile pushed commit id")
expect(agent_plan).to_contain("2026-06-14 status-alignment slice")
...
expect(perf).to_contain("Bun, Java, and Erlang rows are emitted when")
expect(perf).to_contain("C and Go remain the required native baselines")
...
expect(row).to_contain("release wrapper remains probe-required")
