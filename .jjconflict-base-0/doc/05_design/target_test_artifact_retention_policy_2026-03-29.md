...
- `target/test-artifacts/<spec-relative-path>/`
- `target/test-artifacts/<spec-relative-path>/scenarios/...`
- `build/coverage/`
- `build/artifacts/`
- `build/bootstrap/`
...
- Rust test runner writes per-spec evidence to `target/test-artifacts/`:
  - `src/compiler_rust/driver/src/cli/test_runner/artifact.rs`
  - `src/compiler_rust/driver/src/cli/test_runner/execution.rs`
- Rust test runner plans per-scenario evidence under `target/test-artifacts/.../scenarios/`:
...
- `target/test-artifacts/` for per-spec and per-scenario test evidence
...
- never commit `target/test-artifacts/`
...
- test runner owns `target/test-artifacts/`
...
- `simple test`:
  - leaves `target/test-artifacts/` in place for the latest run
  - removes temporary wrappers unless `--keep-artifacts`
- future `simple test --clean-artifacts`:
  - deletes `target/test-artifacts/`
...
1. Add `simple test --clean-artifacts` that deletes `target/test-artifacts/` only.
