...
`test/01_unit/compiler/mir_opt/cipher/pattern_engine_spec.spl` adds a provider-contract test:
...
`test/01_unit/compiler/interpreter/mir_ssa_phi_intrinsic_spec.spl` adds interpreter coverage:

- `__simple_ssa_phi` stores the first incoming value into the destination as a
  deterministic linear fallback.

`test/01_unit/compiler/mir_opt/general_patterns_backend_recommendation_spec.spl` adds recommendation coverage:
...
`test/01_unit/compiler/mir_opt/pass_descriptor_spec.spl` adds backend pipeline coverage:
...
`test/01_unit/compiler/interpreter/tiered_jit_hotspot_spec.spl` adds plan coverage:
...
`test/05_perf/compiler/jit_hotspot_plan_bench.spl` adds benchmark coverage:
