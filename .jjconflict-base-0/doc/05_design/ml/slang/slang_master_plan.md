...
slang is a vLLM-like LLM serving tool for Simple / SimpleOS. It provides:
...
slang **mirrors vLLM's module layout and naming** unless doing so breaks MDSOC+. See `~/.claude/plans/to-make-good-file-virtual-stardust.md` → "Naming & Architecture Rule" for the full rule. Concretely:

```
src/lib/gc_async_mut/slang/
...
Product binary + integration tests live in a separate GitHub repo (`ormastes/slang`, public, MIT) added at `examples/07_ml/slang/` as a git submodule.
...
| A2 | Packer CLI | `src/app/slang_pack/` consumes safetensors, emits tensor-pack | R3 atomic publish, R1 append_only |
...
5. **nvfs is a client contract, not a hard dep.** slang builds on top of today's FAT32 as a bring-up substrate; the `nvfs_client` shim hides the difference. Production perf requires nvfs; functional correctness does not.
...
- **Filesystem contract slang depends on:** sealed objects, pinned extents, async range reads into pinned buffers. See `../nvfs/slang_requirements.md` for the precise API.
...
- Throughput baseline recorded under `doc/10_metrics/slang_baseline.md`
...
- Design drift from research doc — this file is the single source of truth for slang; any deviation updates here.
