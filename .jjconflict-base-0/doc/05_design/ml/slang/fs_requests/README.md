# slang → nvfs Feature Request Log

**Secondary channel** for filesystem feature requests that surface during slang implementation. The **primary** channel is the upfront design-doc contribution at `../../nvfs/slang_requirements.md`. Use this directory only when:
...
If the discovery changes the design rather than just reporting a perf gap, promote it into `slang_requirements.md` and delete/supersede the entry here.
...
- **Proposed mapping:** `fs_<verb>(<args>)` per `../../nvfs/slang_requirements.md` §R<n>
...
```bash
bin/simple bug-add --id=FS-<nnn> --title="<same title>" --path=doc/05_design/slang/fs_requests/<file>
```
...
1. Edit `../../nvfs/slang_requirements.md` to reflect the new or updated requirement.
...
(Empty on first PR; populated as slang implementation progresses.)
