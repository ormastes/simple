...
`vmm_device_mapping` is the explicit address-space transaction owner. It plans
the physical page envelope with checked arithmetic, chooses or validates a
lower-user page range, rejects VMA and stale/huge-PTE collisions before
mutation, reserves exact `VMA_DEVICE` provenance, and maps USER|WRITABLE|UC|NX
leaves through `vmm_map_page_in`. Failure detaches only leaves installed by the
transaction and removes metadata when rollback succeeds. Strict unmap requires
the exact virtual extent and physical page provenance, restores earlier PTEs if
a later detach fails, and never calls PMM for device leaves.

`device_mapping_resources` is the lifecycle authority ledger for production
syscalls 88/89. A slot is reserved before page-table mutation, then committed
with immutable task, BDF, BAR, requested-length, virtual-envelope, and physical
provenance. Exact lookup prevents a caller from unmapping another task or a
different extent. Reserved and live entries both block fork/exec. The existing
deterministic `DeviceGrant.bar0_cap` is descriptive ABI metadata, not security
authority; generation-aware exact BDF/BAR authorization remains owned by the
kernel capability manager.
Creation requires that live generation-aware capability; release requires the
exact immutable owner record and deliberately remains possible after
revocation. Release transitions LIVE→RELEASING before PTE mutation, returns to
LIVE on unmap failure, and clears only after the updated vmspace is registered.
PCI sizing probes are serialized and quiesced by temporarily disabling IO,
memory decode, and bus mastering while BAR dwords are sampled and restored.
The live implementation is split at the MDSOC layer boundary:
`pci_bar_live_resolver` alone owns PCI configuration mutation and feeds the
pure aperture resolver, while `syscall_device_bar` alone coordinates explicit
vmspace and lifecycle-ledger transactions. The syscall hub owns authorization
and dispatch but never probes hardware or mutates page tables.

Intermediate page-table pages allocated before a map failure are not yet
reclaimed because the current page-table API has no empty-table transaction.
They remain owned by the process address space and are reclaimed at teardown;
this is bounded metadata leakage within the process lifetime, not MMIO frame
ownership. Syscall integration must still register the returned exact extent
and invoke this same owner for explicit release and task cleanup.

Modern VirtIO-GPU BAR consumption is grouped by the pure
`virtio_gpu_bar_mapping_plan` owner. It merges all capability intervals for a
BAR into one checked envelope, so common/notify/ISR rows sharing a page cannot
create overlapping VMAs. The live adapter maps each envelope through syscall
88, derives capability pointers only by containment within returned user VAs,
and rolls back already-mapped BARs through syscall 89 on later failure. Physical
BAR addresses never enter the userspace mapping request.
