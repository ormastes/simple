...
**Runs per measurement:** 1 | **Warmup (in-process):** 10 | **fib(N):** 35 | **CPU workers:** 100 | **Go GOMAXPROCS:** 100 | **OS thread workers:** 100 | **Cooperative green workers:** 10 | **Multicore green workers:** 100 | **Fanout workers:** 1000 | **Fanout cooperative green workers:** 200 | **Fanout multicore green workers:** 1000 | **Fanout iters:** 32 | **Fanout stress workers:** 512 | **Fanout stress iters:** 1 | **Per-run timeout:** 30s
**Profile script:** `scripts/check/check-cross-language-perf.shs`
**Report path:** `doc/09_report/cross_language_perf_2026-06-11_turn.md`
...
|------------------------|--------------|--------------|--------------|--------------|
| Simple (interpreter)   |       38.0 B |      175.0 B |      51.9 MB |              |
| Simple (SMF loader)    |       3.0 KB |       3.7 KB |      51.9 MB |              |
| Simple (native)        |       2.7 MB |       2.7 MB |         none |              |
...
| Erlang                 |      900.0 B |      900.0 B |      BEAM VM |              |

## Cold Startup — hello world (1 runs avg)

| Language               |     Avg (ms) |         Mode |              |              |
|------------------------|--------------|--------------|--------------|--------------|
| Simple (interpreter)   |       31.551 |    interpret |              |              |
| Simple (SMF loader)    |       19.907 |          smf |              |              |
| Simple (native)        |        6.922 |       native |              |              |
| C (gcc -O2)            |        5.458 |       native |              |              |
| Go (compiled)          |       57.919 |       native |              |              |
| Python                 |       30.163 |    interpret |              |              |
| Bun                    |      165.643 |          JIT |              |              |
| Java                   |      132.846 |    JIT (JVM) |              |              |
| Erlang                 |     1452.381 |      BEAM VM |              |              |

## Warm Throughput — fib(35) in process (10 warmup + 1 measured)

| Language               |     Avg (ms) |                                    Notes |
|------------------------|--------------|------------------------------------------|
| Simple (interpreter)   |       88.157 | tree-walk (outer-process, no in-proc timing) |
| Simple (SMF loader)    |       87.344 | bytecode (outer-process, no in-proc timing) |
| Simple (native)        |       58.415 |           AOT via Cranelift (in-process) |
| C (gcc -O2)            |       13.333 |                             baseline AOT |
| Go                     |       52.540 |                                  SSA AOT |
| Python                 |     1600.314 |                         CPython bytecode |
| Bun                    |       65.710 |        JavaScriptCore JIT (steady-state) |
| Java                   |       51.344 |            HotSpot C2 JIT (steady-state) |
| Erlang                 |      115.553 |                    BEAM (single-process) |
...
| Simple (SMF loader)    |      120.990 |    std thread_spawn fork-join (bytecode) |
| Simple (native)        |         fail |        thread_spawn fork-join OS threads |
| Simple cooperative green (interp) |       47.533 | cooperative_green_spawn cooperative queue |
| Simple cooperative green (SMF) |         fail | cooperative_green_spawn cooperative queue (segfault) |
| Simple cooperative green (native) |         fail | cooperative_green_spawn cooperative queue |
| Simple multicore green (SMF) |      120.742 | multicore_green runtime pool candidate (pool_used=100/100, parallelism=64/64, queue_model=work_stealing) |
| Simple multicore green (native) |       99.021 | multicore_green runtime pool candidate (pool_used=100/100, parallelism=64/64, queue_model=work_stealing) |
| C (pthreads)           |       10.634 |                               OS threads |
| Go                     |       10.104 |           goroutines + chan result (M:N) |
| Python                 |     3029.536 |                          threading (GIL) |
| Bun                    |      367.943 |                           worker_threads |
| Java                   |      165.492 |                               ThreadPool |
| Erlang                 |     1367.141 |                    lightweight processes |
...
|------------------------|--------------|------------------------------------------|
| Simple (interpreter)   |          n/a |          extern thread FFI not supported |
| Simple (SMF loader)    |      103.113 |                  std thread_spawn fanout |
| Simple (native)        |         fail | OS-thread fork-join fanout (checksum mismatch) |
| Simple cooperative green (interp) |       46.476 |                 cooperative queue fanout |
| Simple cooperative green (SMF) |         fail |      cooperative queue fanout (segfault) |
| Simple cooperative green (native) |         fail |                 cooperative queue fanout |
| Simple multicore green (SMF) |       33.566 | multicore_green runtime pool fanout (pool_used=1000/1000, parallelism=64/64, queue_model=work_stealing) |
| Simple multicore green (native) |       21.578 | multicore_green runtime pool fanout (pool_used=1000/1000, parallelism=64/64, queue_model=work_stealing) |
| C (pthreads)           |       57.398 |              one OS thread per tiny task |
| Go                     |        8.210 |        goroutine per tiny task + channel |
| Python                 |      175.206 |            threading per tiny task (GIL) |
| Bun                    |         fail |              worker_thread per tiny task |
| Java                   |      101.709 |   cached ThreadPool future per tiny task |
| Erlang                 |     1331.072 |               BEAM process per tiny task |

## Simple vs Go vs C Large Fanout Stress — spawn 512 tiny workers (1 runs avg)

| Language               |     Avg (ms) |                        Concurrency model |
|------------------------|--------------|------------------------------------------|
| C (pthreads)           |       36.273 |                  pthread per stress task |
| Simple multicore green (native) |       15.261 | multicore_green stress fanout (pool_used=512/512, parallelism=64/64, queue_model=work_stealing) |
| Go                     |        7.200 |          goroutine per stress task (M:N) |

## Parallel Artifact Footprint

| Language               |         Binary |     Per-thread |                Notes |
|------------------------|----------------|----------------|----------------------|
| Simple (native)        |         2.7 MB | OS default (8MB) |        Cranelift AOT |
| Simple (SMF)           |         7.0 KB | OS default (8MB) |    + runtime 51.9 MB |
| Simple cooperative green (native) |         2.7 MB | current OS thread |    cooperative queue |
| Simple cooperative green (SMF) |         8.3 KB | current OS thread |    + runtime 51.9 MB |
| Simple multicore green (native) |         2.7 MB |   runtime pool |      multicore_green |
| Simple multicore green (SMF) |        10.4 KB |   runtime pool |    + runtime 51.9 MB |
...
|------------------------|----------------|----------------|----------------------|
| Simple (native)        |         4.2 MB |         2.8 MB |                ~15KB |
| Simple multicore green |         4.0 MB |         2.8 MB |                ~12KB |
| C (pthreads)           |         2.0 MB |         1.8 MB |                 ~2KB |
| Go                     |         2.5 MB |         2.5 MB |                 ~0KB |
| Python                 |        10.2 MB |        10.2 MB |                 ~0KB |
| Java                   |        48.2 MB |        42.8 MB |                ~56KB |
