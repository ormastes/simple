...
- Native mode does not currently provide real evidence for pipe-forward short grammar. `test/01_unit/compiler_core/parser/short_grammar_combined_spec.spl --mode=native` can pass via codegen stub fallback after reporting `Pipe forward (|>) requires interpreter mode for function dispatch`.
...
- `test/01_unit/compiler_core/parser/short_grammar_interpreter_spec.spl`: interpreter-focused combinations for placeholder lambda, numbered placeholder, method reference, comprehension, `??`, and pipe/compose.
- `test/01_unit/compiler_core/parser/short_grammar_native_spec.spl`: native-safe short forms without pipe-forward dispatch.
