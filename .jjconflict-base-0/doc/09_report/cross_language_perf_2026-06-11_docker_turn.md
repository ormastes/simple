...
**Report path:** `doc/09_report/cross_language_perf_2026-06-11_docker_turn.md`
...
|------------------------|--------------|--------------|--------------|--------------|
| Simple (interpreter)   |       24.834 |    interpret |              |              |
| Simple (SMF loader)    |       22.239 |          smf |              |              |
| Simple (native)        |        4.275 |       native |              |              |
| C (gcc -O2)            |        4.518 |       native |              |              |
| Go (compiled)          |       63.780 |       native |              |              |
| Python                 |       23.258 |    interpret |              |              |

## Warm Throughput — fib(35) in process (10 warmup + 1 measured)

| Language               |     Avg (ms) |                                    Notes |
|------------------------|--------------|------------------------------------------|
| Simple (interpreter)   |       92.743 | tree-walk (outer-process, no in-proc timing) |
| Simple (SMF loader)    |       83.663 | bytecode (outer-process, no in-proc timing) |
| Simple (native)        |       57.457 |           AOT via Cranelift (in-process) |
| C (gcc -O2)            |       14.705 |                             baseline AOT |
| Go                     |       55.381 |                                  SSA AOT |
| Python                 |     1631.321 |                         CPython bytecode |
...
| Simple (SMF loader)    |      153.908 |    std thread_spawn fork-join (bytecode) |
| Simple (native)        |         fail |        thread_spawn fork-join OS threads |
| Simple cooperative green (interp) |       56.248 | cooperative_green_spawn cooperative queue |
...
| Simple multicore green (SMF) |      142.828 | multicore_green runtime pool candidate (pool_used=100/100, parallelism=64/64, queue_model=work_stealing) |
| Simple multicore green (native) |      150.434 | multicore_green runtime pool candidate (pool_used=100/100, parallelism=64/64, queue_model=work_stealing) |
| C (pthreads)           |       12.968 |                               OS threads |
| Go                     |        9.914 |           goroutines + chan result (M:N) |
| Python                 |     3090.011 |                          threading (GIL) |
...
|------------------------|--------------|------------------------------------------|
| Simple (interpreter)   |          n/a |          extern thread FFI not supported |
| Simple (SMF loader)    |      107.997 |                  std thread_spawn fanout |
| Simple (native)        |         fail | OS-thread fork-join fanout (checksum mismatch) |
| Simple cooperative green (interp) |       40.390 |                 cooperative queue fanout |
...
| Simple multicore green (SMF) |       34.139 | multicore_green runtime pool fanout (pool_used=1000/1000, parallelism=64/64, queue_model=work_stealing) |
| Simple multicore green (native) |       18.156 | multicore_green runtime pool fanout (pool_used=1000/1000, parallelism=64/64, queue_model=work_stealing) |
| C (pthreads)           |       63.908 |              one OS thread per tiny task |
| Go                     |        8.712 |        goroutine per tiny task + channel |
| Python                 |      146.076 |            threading per tiny task (GIL) |

## Simple vs Go vs C Large Fanout Stress — spawn 512 tiny workers (1 runs avg)

| Language               |     Avg (ms) |                        Concurrency model |
|------------------------|--------------|------------------------------------------|
| C (pthreads)           |       38.134 |                  pthread per stress task |
| Simple multicore green (native) |       14.594 | multicore_green stress fanout (pool_used=512/512, parallelism=64/64, queue_model=work_stealing) |
| Go                     |        6.405 |          goroutine per stress task (M:N) |
