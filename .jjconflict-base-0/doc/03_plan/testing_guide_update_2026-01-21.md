...
**Date**: 2026-01-21
**Status**: Planning
**Goal**: Update `doc/07_guide/test.md` to reflect new directory structure and testing infrastructure
...
**Target Document**: `doc/07_guide/test.md` (currently 750 lines)
...
```bash
# Create backup
cp doc/07_guide/test.md doc/07_guide/test.md.backup

# Verify current document
wc -l doc/07_guide/test.md
# Should be ~750 lines
```
...
```bash
# Check changes
git diff doc/07_guide/test.md | grep "^-" | grep "tests/"
git diff doc/07_guide/test.md | grep "^+" | grep "test/"
```
...
- `doc/07_guide/directory_structure.md` - Directory structure reference
```
...
npx markdownlint doc/07_guide/test.md

# 3. Check internal links
grep -n "](#" doc/07_guide/test.md
# Verify all anchors exist

# 4. Verify code blocks
grep -c '```' doc/07_guide/test.md
# Should be even number

# 5. Check path references
grep -E "(tests/|simple/test/|simple/std_lib/test/)" doc/07_guide/test.md
# Should return NO matches (all updated to new paths)

grep -E "(test/rust/|test/lib/std/)" doc/07_guide/test.md
...
```bash
# Review changes
git diff doc/07_guide/test.md | less

# Stage changes
git add doc/07_guide/test.md
...
```bash
# Restore backup
cp doc/07_guide/test.md.backup doc/07_guide/test.md
