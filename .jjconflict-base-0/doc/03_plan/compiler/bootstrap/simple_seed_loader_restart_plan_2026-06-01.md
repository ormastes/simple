...
- 2026-07-04 focused restart evidence:
  ...
  - 2026-07-25 native-build parent split: `src/app/cli/native_build_main.spl` no longer imports `app.io._CliCompile.compile_targets` in the parent entry; it always spawns `native_build_worker.spl` after local help handling. `simple_seed run src/app/cli/native_build_main.spl --help` now returns in 0.05s. The worker-parent `bootstrap_main.spl` shard still timed out at 180s with only the seed warning, zero cached objects, and no output binary, so the next fix belongs inside `native_build_worker.spl` / `cli_native_build` early worker progress.
