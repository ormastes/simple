...
- The old `test/01_unit/os/compositor/web_surface_blit_test.spl` still stalled
  under `bin/simple test` and left an orphaned `bin/simple run` child until
  cleaned up.
- The coverage was converted to
  `test/01_unit/os/compositor/web_surface_blit_spec.spl` and bounded to smaller
  fixture dimensions.
- Verification:
  - `SIMPLE_LIB=src timeout 45s bin/simple check test/01_unit/os/compositor/web_surface_blit_spec.spl`
    passed.
  - `SIMPLE_LIB=src timeout 45s bin/simple test test/01_unit/os/compositor/web_surface_blit_spec.spl --mode=interpreter --clean --format json`
...
- Added focused coverage to
  `test/01_unit/os/compositor/simpleos_gui_shared_wm_adapter_spec.spl` proving
...
0. Initial reconciliation:
   - Dirty SimpleOS lane files found:
     ...
     `test/03_system/os_network_spec.spl`, and
   ...
   - Dirty net acceleration lane files found:
     ...
     `test/03_system/net_tcp_socket_data_path_spec.spl`, and
     `test/03_system/net_rdma_transport_spec.spl`.
   - Dirty SPM lane files found:
     `test/01_unit/os/kernel/ipc/spm_init_register_spec.spl` and
     `test/01_unit/os/kernel/ipc/spm_port_spec.spl`.
   - No source edits have been made in this recovery pass yet.
1. SimpleOS focused non-live verification:
   - `SIMPLE_LIB=src bin/simple check test/01_unit/os/kernel/arch/x86_32_paging_timer_spec.spl`
     passed.
   - `SIMPLE_LIB=src bin/simple test test/01_unit/os/kernel/arch/x86_32_paging_timer_spec.spl --mode=interpreter --clean --format json`
     passed `4/4` in 1525 ms.
   - `SIMPLE_LIB=src bin/simple check test/03_system/os_network_spec.spl`
     passed.
   - `SIMPLE_LIB=src bin/simple test test/03_system/os_network_spec.spl --mode=interpreter --clean --format json`
...
2. Net acceleration focused verification:
   - `SIMPLE_LIB=src bin/simple check src/os/crypto/crc32.spl src/os/services/netstack/socket.spl src/os/services/netstack/tcp_connection.spl src/os/services/netstack/tcp_state_machine.spl test/03_system/net_tcp_socket_data_path_spec.spl test/03_system/net_rdma_transport_spec.spl`
     passed.
   - `SIMPLE_LIB=src bin/simple test test/03_system/net_tcp_socket_data_path_spec.spl --mode=interpreter --clean --format json`
     passed `6/6` in 407 ms.
   - `SIMPLE_LIB=src bin/simple test test/03_system/net_rdma_transport_spec.spl --mode=interpreter --clean --format json`
...
3. SPM focused IPC verification:
   - `SIMPLE_LIB=src bin/simple check test/01_unit/os/kernel/ipc/spm_init_register_spec.spl test/01_unit/os/kernel/ipc/spm_port_spec.spl`
     passed.
   - `SIMPLE_LIB=src bin/simple test test/01_unit/os/kernel/ipc/spm_init_register_spec.spl --mode=interpreter --clean --format json`
     passed `4/4` in 2261 ms.
   - `SIMPLE_LIB=src bin/simple test test/01_unit/os/kernel/ipc/spm_port_spec.spl --mode=interpreter --clean --format json`
