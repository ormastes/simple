...
### Maintenance Note — Lean/BYL/RISC-V Regeneration

For ongoing hardening, keep generated Lean, generated BYL, and handwritten
constraint/theorem files in separate artifacts. Regeneration may replace
generated files, but it must preserve the exported names consumed by manual
proofs or update the generated contract and manual proofs in the same change.
RISC-V formal evidence must use both applicable layers: the RTL/RVFI/SBY lane
for generated hardware sidecars and the Lean/BYL lane for higher-level resource,
capability, scheduler, starvation, and race-condition models.
