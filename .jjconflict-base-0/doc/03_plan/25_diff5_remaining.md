...
|---|---------|--------------------|
| 13 | Type Inference | Finalize HM design in `doc/05_design/type_inference.md`, add HIR-typed nodes, and wire a unification engine (start with expressions only). |
...
| 24 | GC-Managed Memory (default T) | Land GC-backed allocator as default path and route codegen allocations through GC ABI (see `doc/05_design/memory.md`). |
