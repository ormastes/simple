...
The explicit-space transaction is now implemented in the working tree with
collision preflight, atomic leaf rollback, strict provenance unmap, and a hosted
integration fixture. Focused source checks pass and the pure planner passes
14/14. The bootstrap runner cannot currently execute the PMM-backed transaction
fixture: it reports `no examples executed` after PMM initialization, and the
unchanged `memory_leveling_vmm_effects_spec.spl` reproduces the same environment
failure. This remains provisional until both fixtures execute under a rebuilt
pure-Simple runner; syscall 88 migration remains open.

## Live syscall progress

Syscalls 88/89 are implemented provisionally around exact BDF/BAR capability
checks, checked ABI decoding, restored PCI BAR probing, the pure aperture
resolver, explicit `ProcessVmSpace` mapping, and a pre-reserved immutable
ownership ledger. Exit cleanup consumes the same strict unmap transaction.
The deterministic `bar0_cap` field is not treated as security authority; live
capability generation remains kernel-owned. Focused leaf checks and pure tests
pass, while the aggregate syscall owners exceed the current bootstrap check
timeout and therefore still need rebuilt pure-Simple compile/runtime evidence.
VirtIO-GPU callers have not yet migrated, so this bug remains open.
