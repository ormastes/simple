...
`test/01_unit/os/crypto/streebog_kat_spec.spl` had hash-comparison assertions failing
...
`bin/simple test test/01_unit/os/crypto/streebog_kat_spec.spl` → **7 passed, 0 failed**.
...
The `test/01_unit/os/crypto/streebog_kat_spec.spl` expected values were stale/wrong.
The spec was updated with correct RFC 6986 vectors verified against `gostcrypto`.

## Related

- `test/01_unit/os/crypto/streebog_kat_spec.spl` — KAT spec (now correct, 7/7 pass)
