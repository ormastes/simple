...
5. Existing dashboard system spec is stale-path oriented.
   [`test/system/infrastructure/dashboard_system_spec.spl`](test/system/infrastructure/dashboard_system_spec.spl) still targets `doc/dashboard/...` and direct `.spl` execution instead of the current `simple dashboard` CLI path and current `doc/10_metrics/dashboard/...` layout.
