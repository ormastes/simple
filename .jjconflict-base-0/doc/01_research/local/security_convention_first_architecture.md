...
- RISC-V baremetal support now converts generated `pmp_region|base|size|permissions|locked` lowering rows into concrete `pmpaddrN` and `pmpcfgN` CSR write plans with NAPOT address encoding and locked read/write/execute config bytes.
- Hosted remote `SecurityContext` sessions now have a concrete Redis deployment store using the existing Redis client, SDN-compatible session row payloads, Redis TTLs, and bounded revocation tombstones.
- Hosted remote `SecurityContext` sessions now have concrete quorum behavior over replicated adapters with configurable read/write quorum counts, topology export, revocation-wins merge policy, and latest-active-expiry merge policy.
- Hosted KMS/HSM-backed remote `SecurityContext` signing now has a generic JSON-over-HTTPS provider contract with opaque key handles, bearer/TLS transport metadata, external sign/verify response handling, fail-closed malformed/error responses, and token validation through the existing adapter seam.
- Hosted KMS/HSM-backed remote `SecurityContext` signing now has vendor request builders for AWS KMS Sign/Verify, Google Cloud KMS asymmetricSign, Azure Key Vault sign/verify, and PKCS#11 HSM gateway sign/verify, all using opaque key handles and transport credentials instead of raw signing keys.
