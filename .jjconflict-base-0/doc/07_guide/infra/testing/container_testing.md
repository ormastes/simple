...
docker run --rm -v $(pwd):/workspace:ro \
  --memory=512m --cpus=1.0 \
  simple-test-isolation:latest test test/01_unit/std/string_spec.spl
...
2. **Quick local test:** `bin/simple test test/01_unit/std/string_spec.spl`
3. **Verify in container:** `scripts/local-container-test.sh quick test/01_unit/std/string_spec.spl`
...
docker run --rm -v $(pwd):/workspace:ro \
  --memory=512m --cpus=1.0 \
  simple-test-isolation:latest test test/01_unit/

# Integration tests (standard tier)
docker run --rm -v $(pwd):/workspace:ro \
  --memory=1g --cpus=2.0 \
  simple-test-isolation:latest test test/02_integration/

# System tests (slow tier)
docker run --rm -v $(pwd):/workspace:ro \
  --memory=2g --cpus=4.0 \
  simple-test-isolation:latest test test/03_system/
```
...
```bash
scripts/local-container-test.sh quick test/01_unit/std/string_spec.spl  # Single test
...
TEST_PROFILE=standard scripts/ci-test.sh test/01_unit/   # Unit tests, standard profile
...
| Tests pass locally, fail CI | Run exact CI command: `scripts/ci-test.sh test/01_unit/` |
...
# Inside container:
simple test test/01_unit/failing_spec.spl --verbose
```
