...
| SIMD | Source-present in library/tests (`test/03_system/feature/scilib/*simd*`, `test/02_integration/lib/simd_stdlib_spec.spl`). Not board-verified here. |
...
```bash
SIMPLE_LIB=src timeout 120s bin/simple test test/03_system/hardware/riscv64_fpga/preflight_spec.spl --mode=interpreter --clean
SIMPLE_LIB=src timeout 120s bin/simple test test/03_system/hardware/riscv64_fpga/hardware_inventory_spec.spl --mode=interpreter --clean
SIMPLE_LIB=src timeout 120s bin/simple test test/03_system/hardware/riscv64_fpga/jtag_unbind_spec.spl --mode=interpreter --clean
```
