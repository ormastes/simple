...
use simple_common::smf::{
    ...
    SECTION_FLAG_READ, Platform, SMF_FLAG_EXECUTABLE, SMF_MAGIC,
};
...
fn build_smf_with_code_for_target(
    ...
    // Zero-initialize first so that repr(C) padding bytes (e.g. bytes 20-23 between
...
pub fn generate_smf_bytes_for_target(return_value: i32, gc: Option<&Arc<dyn GcAllocator>>, target: Target) -> Vec<u8> {
    ...
    let code_bytes = match target.arch {
        ...
        TargetArch::Wasm32 | TargetArch::Wasm64 => {
            // WASM targets require LLVM backend, not native code generation
            // This path should not be reached for WASM as it uses emit_object from LLVM
            // Return empty vec as placeholder (will be replaced by LLVM-generated WASM)
            Vec::new()
        }
    };
...
/// NOTE: Any uninitialized padding bytes in `value` are serialized as-is. Callers must
/// ensure structs are zero-initialized (e.g. via `std::mem::zeroed()`) before field
/// assignment so that padding bytes are deterministically 0x00.
