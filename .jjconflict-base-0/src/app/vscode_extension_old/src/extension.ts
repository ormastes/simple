...
export function activate(context: vscode.ExtensionContext) {
    console.log('Simple Language Old extension activating...');

    // Create output channel for LSP communication
    outputChannel = vscode.window.createOutputChannel('Simple Language Old Server', { log: true });
    ...
    fallbackSemanticTokensProvider = new SimpleSemanticTokensProvider();
    fallbackSemanticTokensRegistration = vscode.languages.registerDocumentSemanticTokensProvider(
        { language: 'simple' },
        fallbackSemanticTokensProvider,
        TOKEN_LEGEND
    );
    context.subscriptions.push(fallbackSemanticTokensRegistration);
    fallbackDiagnosticsProvider = new SimpleDiagnosticsProvider();
    ...
    console.log('Simple Language Old extension activated');
}
...
async function startLspClient(context: vscode.ExtensionContext) {
    ...
    client.start().then(() => {
        ...
        vscode.window.showWarningMessage(
            'Simple LSP server not available. Syntax highlighting and other basic features still work.',
            'Show Output'
        ).then(selection => {
...
function disableFallbackProviders(): void {
