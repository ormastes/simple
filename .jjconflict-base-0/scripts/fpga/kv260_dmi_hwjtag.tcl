# kv260_dmi_hwjtag.tcl — Vivado hw_jtag RAW-mode DMI batch over the BSCANE2 tunnel.
#
# Board-proven path (KV260, rv32 debug bitstream G_DEBUG_JTAG=true):
#   host -> FT4232H chA -> xck26 PS TAP -> BSCANE2 USER4 -> bscane2_tap_bridge
#   (v1 raw TMS/TDI-pair framing) -> tck->clk CDC -> Simple TAP/DTM/DMI/DM/hart.
#
# TCK MUST be 1 MHz (CDC proven <= core_clk/8 ~= 3 MHz; the ~15 MHz hw_jtag
# default reads ALL-ZEROS). OpenOCD's use_bscan_tunnel framing is structurally
# incompatible with the v1 bridge — raw hw_jtag only.
#
# Invocation:
#   vivado -mode batch -notrace -source kv260_dmi_hwjtag.tcl -tclargs \
#       <idle_cycles> <op:addr:data> [<op:addr:data> ...]
#   op = 1 read / 2 write ; addr = 7-bit DMI reg ; data = 32-bit hex (no 0x)
#   A trailing NOP is appended automatically; DMI is pipelined so OPi_RESULT is
#   the completion of the i-th issued op (addr/data echo the read result).
# Reference: doc/07_guide/hardware/fpga/simple_riscv_jtag_debugging.md
