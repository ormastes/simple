...
- test/01_unit/compiler/custom_primitive_sffi_spec.spl — 20 tests

### 7-verify
Syntax checks pass for the custom primitive metadata and SFFI spec files.
`bin/simple test test/01_unit/compiler/custom_primitive_sffi_spec.spl --mode=interpreter --clean` now reports `Passed: 20`, `Failed: 0` after converting the manual checks into runner-discovered examples.
