...
> Scope: test/dbfs/, test/03_system/db_*, test/05_perf/bench/simple_db_*, test/01_unit/lib/*/simple_db_if/, doc/05_design/*, doc/08_tracking/feature_request/simple_db_requests.md
...
|------|-------------|------------|--------|
| test/03_system/db_sdn_spec.spl | 3 | 10 | Real — covers SDN table import/export, @cover annotation targets `src/lib/nogc_sync_mut/database/sdn_table.spl` at 60% |
| test/03_system/simple_db_nvfs_constants_spec.spl | 1 | 4 | Real — nvfs constant validation |

---

### 1.3 Unit Tests (`test/01_unit/lib/*/simple_db_if/`)
...
### 1.4 Perf Benchmarks (`test/05_perf/bench/simple_db_*.spl`)
...
- **Phase 2 (Done — 2026-05-20):** `PrefixIndex`, `TextIndex`, `PageSummary`/`PageSummaryIndex`, `filter_in` with OR semantics; regression spec at `test/05_perf/bench/db_accel_index/db_accel_index_spec.spl`.
