...
if (!target) {
  console.warn(
    `@simple-lang/t32-mcp-server: unsupported platform ${platform}. ` +
...
if (fs.existsSync(binPath)) {
  console.log("@simple-lang/t32-mcp-server: binary already exists, skipping download.");
  process.exit(0);
}

console.log(`@simple-lang/t32-mcp-server: downloading Simple compiler for ${target}...`);
...
try {
  ...
  console.log("@simple-lang/t32-mcp-server: installed successfully.");
} catch (err) {
  console.warn(
    `@simple-lang/t32-mcp-server: failed to download binary. ` +
