...
- **Caret terminal signal teardown** — isolate interactive HUP/INT/TERM/WINCH
  ...
  restore the preceding handlers after cursor, screen, and raw-mode cleanup,
  cover real Windows raw/resize input, panic/contract/assert cleanup, BEGIN
  rollback, and exact-target concurrent teardown safely.
...
- **LLVM/Clang 23.1 provider and SimpleOS browser path** — add a signed
  ...
  compilation, bootstrap evidence, freestanding custom-property/backdrop
  admission probes, and cross-LLM operator guidance. The Rust in-process LLVM
  binding remains blocked upstream at LLVM 22.
