...
inductive MemoryOperation
  ...
  | ThreadSpawn : ThreadId → ThreadId → MemoryOperation
  | ThreadJoin : ThreadId → ThreadId → MemoryOperation
...
example : ∃ exec : Execution, dataRaceFree exec := by
  val tid1 := ThreadId.mk 0
  val tid2 := ThreadId.mk 1
  val loc := LocationId.mk 0
  val lock := LockId.mk 0

  val op1 := (OperationId.mk 0, MemoryOperation.LockAcquire lock tid1)
  val op2 := (OperationId.mk 1, MemoryOperation.Write loc tid1)
  val op3 := (OperationId.mk 2, MemoryOperation.LockRelease lock tid1)
  val op4 := (OperationId.mk 3, MemoryOperation.LockAcquire lock tid2)
  val op5 := (OperationId.mk 4, MemoryOperation.Read loc tid2)
  val op6 := (OperationId.mk 5, MemoryOperation.LockRelease lock tid2)
...
example : ∃ exec : Execution, hasDataRace exec := by
  ...
  val op1 := (OperationId.mk 0, MemoryOperation.Write loc tid1)
  val op2 := (OperationId.mk 1, MemoryOperation.Read loc tid2)
