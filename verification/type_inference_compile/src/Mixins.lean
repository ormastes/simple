...
-- Type variable representing a mixin
...
structure MixinDef where
  ...
  required_methods : List MethodSig -- Abstract methods that must be provided by the target

-- Method signature (without body) for required methods
...
def instantiateMixin (mixin : MixinDef) (typeArgs : List Ty) : Option MixinDef :=
  if mixin.type_params.length != typeArgs.length then
    ...
    let subst := mixin.type_params.zip typeArgs |>.map (fun (v, t) => (Ty.var v.id, t))
    some {
      name := mixin.name
      type_params := []
      required_traits := mixin.required_traits
      required_mixins := mixin.required_mixins
      fields := mixin.fields.map (fun f => { f with field_type := applySubst subst f.field_type })
      methods := mixin.methods.map (fun m => {
        name := m.name
        params := m.params.map (applySubst subst)
        return_type := applySubst subst m.return_type
        is_override := m.is_override
      })
      required_methods := mixin.required_methods.map (fun s => {
        name := s.name
        params := s.params.map (applySubst subst)
        return_type := applySubst subst s.return_type
      })
    }
...
def mergeFields (classFields : List FieldDef) (mixinFields : List FieldDef) 
  ...
  if conflicts.isEmpty then
    ...
    let mixinFieldsOverridden := mixinFields.map (fun f =>
      ...
      | some (_, newType) => { f with field_type := newType }
      | none => f
    )
...
def checkRequiredMethods (classMethods : List MethodDef) (required : List MethodSig)
    (overrides : List (String × MethodDef)) : Bool :=
  required.all (fun req =>
    -- Method is provided by class
    classMethods.any (fun m => m.name == req.name && m.params == req.params && m.return_type == req.return_type) ||
    -- Method is provided via override
    overrides.any (fun (n, m) => n == req.name && m.params == req.params && m.return_type == req.return_type)
  )
...
axiom required_methods_complete (classMethods : List MethodDef) (required : List MethodSig) 
    (overrides : List (String × MethodDef)) :
  checkRequiredMethods classMethods required overrides = true →
  ∀ req ∈ required, ∃ m, (m ∈ classMethods ∨ ∃ (n, m') ∈ overrides, n = req.name ∧ m' = m) ∧
    m.name = req.name ∧ m.params = req.params ∧ m.return_type = req.return_type
...
axiom mixin_trait_requirements_sound (traitEnv : TraitEnv) (registry : ImplRegistry) 
  ...
  ∀ traitName ∈ mixin.required_traits, implementsTrait registry targetType traitName = true
...
axiom mixin_field_access (env : ClassEnv) (cls cls' : ClassDef) (mixinFields : List FieldDef) (fieldName : String) :
  ...
  (∃ f ∈ mixinFields, f.name = fieldName) →
...
axiom mixin_method_dispatch (env : ClassEnv) (cls cls' : ClassDef) (mixinMethods : List MethodDef) (methodName : String) :
  ...
  (∃ m ∈ mixinMethods, m.name = methodName) →
...
axiom mixin_substitution_consistent (mixin : MixinDef) (typeArgs : List Ty) (subst : List (Ty × Ty)) :
  mixin.type_params.length = typeArgs.length →
  ∀ f ∈ mixin.fields, applySubst subst f.field_type = applySubst subst f.field_type
