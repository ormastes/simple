...
begin
    ...
    process(clk, reset_n)
        variable imem_rdata : std_logic_vector(31 downto 0);
            ...
            if load_issue_reg = '1' then
                ...
                case load_pending_funct3_reg is
                    when "000" =>
                        if XLEN = 64 then
                            if load_pending_rd_reg /= 0 then
                                regs(load_pending_rd_reg) <= std_logic_vector(resize(signed(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(2 downto 0))) * 8)(7 downto 0)), XLEN));
                                debug_last_load_value_reg <= std_logic_vector(resize(signed(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(2 downto 0))) * 8)(7 downto 0)), XLEN));
                            end if;
                        elsif load_pending_rd_reg /= 0 then
                            regs(load_pending_rd_reg) <= std_logic_vector(resize(signed(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(1 downto 0))) * 8)(7 downto 0)), XLEN));
                            debug_last_load_value_reg <= std_logic_vector(resize(signed(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(1 downto 0))) * 8)(7 downto 0)), XLEN));
                        end if;
                    when "001" =>
                        if XLEN = 64 then
                            if load_pending_rd_reg /= 0 then
                                regs(load_pending_rd_reg) <= std_logic_vector(resize(signed(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(2 downto 0))) * 8)(15 downto 0)), XLEN));
                                debug_last_load_value_reg <= std_logic_vector(resize(signed(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(2 downto 0))) * 8)(15 downto 0)), XLEN));
                            end if;
                        elsif load_pending_rd_reg /= 0 then
                            regs(load_pending_rd_reg) <= std_logic_vector(resize(signed(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(1 downto 0))) * 8)(15 downto 0)), XLEN));
                            debug_last_load_value_reg <= std_logic_vector(resize(signed(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(1 downto 0))) * 8)(15 downto 0)), XLEN));
                        end if;
                    when "010" =>
                        if load_pending_rd_reg /= 0 then
                            regs(load_pending_rd_reg) <= std_logic_vector(resize(signed(dmem_rdata(31 downto 0)), XLEN));
                            debug_last_load_value_reg <= std_logic_vector(resize(signed(dmem_rdata(31 downto 0)), XLEN));
                        end if;
                    when "011" =>
                        if XLEN = 64 then
                            if load_pending_rd_reg /= 0 then
                                regs(load_pending_rd_reg) <= dmem_rdata;
                                debug_last_load_value_reg <= dmem_rdata;
                            end if;
                        else
                            trap_reg <= '1';
                        end if;
                    when "100" =>
                        if XLEN = 64 then
                            if load_pending_rd_reg /= 0 then
                                regs(load_pending_rd_reg) <= std_logic_vector(resize(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(2 downto 0))) * 8)(7 downto 0), XLEN));
                                debug_last_load_value_reg <= std_logic_vector(resize(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(2 downto 0))) * 8)(7 downto 0), XLEN));
                            end if;
                        elsif load_pending_rd_reg /= 0 then
                            regs(load_pending_rd_reg) <= std_logic_vector(resize(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(1 downto 0))) * 8)(7 downto 0), XLEN));
                            debug_last_load_value_reg <= std_logic_vector(resize(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(1 downto 0))) * 8)(7 downto 0), XLEN));
                        end if;
                    when "101" =>
                        if XLEN = 64 then
                            if load_pending_rd_reg /= 0 then
                                regs(load_pending_rd_reg) <= std_logic_vector(resize(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(2 downto 0))) * 8)(15 downto 0), XLEN));
                                debug_last_load_value_reg <= std_logic_vector(resize(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(2 downto 0))) * 8)(15 downto 0), XLEN));
                            end if;
                        elsif load_pending_rd_reg /= 0 then
                            regs(load_pending_rd_reg) <= std_logic_vector(resize(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(1 downto 0))) * 8)(15 downto 0), XLEN));
                            debug_last_load_value_reg <= std_logic_vector(resize(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(1 downto 0))) * 8)(15 downto 0), XLEN));
                        end if;
                    when "110" =>
                        if XLEN = 64 then
                            if load_pending_rd_reg /= 0 then
                                regs(load_pending_rd_reg) <= std_logic_vector(resize(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(2 downto 0))) * 8)(31 downto 0), XLEN));
                                debug_last_load_value_reg <= std_logic_vector(resize(shift_right(unsigned(dmem_rdata), to_integer(unsigned(addr_i(2 downto 0))) * 8)(31 downto 0), XLEN));
                            end if;
                        else
                            trap_reg <= '1';
                        end if;
                ...
                imem_rdata := imem_rdata_raw;
                instr_size_bytes := 4;
                if rv64c_is_compressed(imem_rdata_raw) then
                    imem_rdata := rv64c_decompress(imem_rdata_raw(15 downto 0));
                    instr_size_bytes := 2;
                    if imem_rdata = x"00000000" then
                        trap_reg <= '1';
                    end if;
                end if;
                if imem_rdata = x"01F01013" and semi_phase_reg = 0 then
                    semi_phase_reg <= 1;
                    pc_reg <= std_logic_vector(unsigned(pc_reg) + to_unsigned(instr_size_bytes, XLEN));
                elsif imem_rdata = x"00100073" and semi_phase_reg = 1 then
                    semi_phase_reg <= 2;
                    semi_op_reg <= regs(10);
                    semi_param_reg <= regs(11);
                    pc_reg <= std_logic_vector(unsigned(pc_reg) + to_unsigned(instr_size_bytes, XLEN));
                elsif imem_rdata = x"40705013" and semi_phase_reg = 2 then
                    semi_phase_reg <= 0;
                    semi_trigger_reg <= '1';
                ...
                elsif imem_rdata = x"00100073" and semi_phase_reg = 0 then
                    halt_reg <= '1';
                else
                ...
                if imem_rdata(6 downto 0) = "0110111" then
                    ...
                    decode_trap_halt_control_bits := "00000000000000000000000000000" & '0' & '0' & '1';
                elsif imem_rdata(6 downto 0) = "0011011" then
                    ...
                    decode_trap_halt_control_bits := "00000000000000000000000000000" & '0' & '0' & '1';
                elsif imem_rdata(6 downto 0) = "0111011" then
                ...
                case decode_dispatch_class_bits(3 downto 0) is
                    ...
                    when "0011" =>
                        ...
                        decode_execute_control_bits := "0000000000000000000000" & imem_rdata(25 downto 20) & imem_rdata(30 downto 30) & imem_rdata(14 downto 12);
                        ...
                        case decode_dispatch_class is
                                ...
                                if decode_writeback_rd_i /= 0 then
                                    if XLEN = 64 and decode_execute_is_reg = '0' then
                                        case decode_execute_alu_op is
                                            ...
                                            when "0010" => regs(decode_writeback_rd_i) <= std_logic_vector(resize(signed(std_logic_vector(shift_left(unsigned(regs(rs1_i)(31 downto 0)), to_integer(unsigned(imem_rdata(24 downto 20)))))), XLEN));
                                            when "0110" => regs(decode_writeback_rd_i) <= std_logic_vector(resize(signed(std_logic_vector(shift_right(unsigned(regs(rs1_i)(31 downto 0)), to_integer(unsigned(imem_rdata(24 downto 20)))))), XLEN));
                                            when "0111" => regs(decode_writeback_rd_i) <= std_logic_vector(resize(shift_right(signed(regs(rs1_i)(31 downto 0)), to_integer(unsigned(imem_rdata(24 downto 20)))), XLEN));
                                            when others => trap_reg <= '1';
                                        end case;
                                    else
                                        trap_reg <= '1';
                            ...
                            when "0101" =>
                                ...
                                if XLEN = 64 then
                                    ...
                                    translated_addr_v := std_logic_vector(addr_i);
                                    translated_fault_v := '0';
                                end if;
                            ...
                            when "1011" =>
                                if imem_rdata(14 downto 12) = "000" then
                                    if imem_rdata = x"00100073" then
                                        halt_reg <= '1';
                                    elsif imem_rdata = x"00000073" then
                                    ...
                                    elsif imem_rdata = x"30200073" then
                                        ...
                                        pc_reg <= csr_mepc_reg;
                                    elsif imem_rdata = x"10200073" then
                                        ...
                                        csr_mstatus_reg(8) <= '0';
                                        pc_reg <= csr_sepc_reg;
                                    elsif imem_rdata(31 downto 25) = "0001001" then
                                        pc_reg <= std_logic_vector(unsigned(pc_reg) + to_unsigned(instr_size_bytes, XLEN));
                                    else
                                        trap_reg <= '1';
                                    end if;
                                    ...
                                    case imem_rdata(31 downto 20) is
                                    ...
                                    if trap_reg = '0' then
                                        case imem_rdata(14 downto 12) is
