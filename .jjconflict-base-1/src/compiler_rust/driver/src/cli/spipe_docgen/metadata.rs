...
pub fn extract_metadata(spipe_doc: &mut SspecDoc) {
    ...
    if metadata.id.is_none() && !spipe_doc.feature_ids.is_empty() {
        metadata.id = Some(spipe_doc.feature_ids[0].clone());
    }

    spipe_doc.metadata = metadata;
}
