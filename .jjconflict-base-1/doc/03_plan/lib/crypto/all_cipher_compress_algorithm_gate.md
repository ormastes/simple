...
complete or faster-than-baseline evidence.
 ...
 ```bash
 test/05_perf/port_algorithms/run_cipher_compress_gate.shs --list
 ```
...
- Verified `test/01_unit/lib/crypto/poly1305_rfc8439_spec.spl`, `test/01_unit/os/crypto/chacha20_poly1305_spec.spl`, and `test/01_unit/lib/crypto/chacha20_poly1305_rfc8439_spec.spl` all pass in interpreter mode with `--no-cache`.
- Restored the documented `test/05_perf/port_algorithms/run_cipher_compress_gate.shs` runner.
...
- Adler-32 now defers reduction across a 355328-byte chunk when using `u64` accumulators, and `test/01_unit/os/crypto/adler32_spec.spl` locks a 6000-byte pattern that crosses the classic 5552-byte reduction window. This is a correctness-preserving source-level cleanup, not enough to close the performance gate.
...
### Tier 2: Full In-Repo Correctness
 ...
 ```bash
 CIPHER_COMPRESS_SCOPE=all test/05_perf/port_algorithms/run_cipher_compress_gate.shs
 ```
 
 Stop condition:
 
 - All discovered `test/01_unit/**/{crypto,compress,compression,huffman,lz77}*spec.spl` tests pass, excluding only explicitly documented pre-existing bugs.
...
`doc/03_plan/agent_tasks/pure_simple_lz4_zstd_speed_parity.md`.
 ...
 - Keep dependency-free references in `test/05_perf/port_algorithms` so the gate runs on a fresh developer machine.
...
- Added `test/05_perf/port_algorithms/run_cipher_compress_gate.shs`.
