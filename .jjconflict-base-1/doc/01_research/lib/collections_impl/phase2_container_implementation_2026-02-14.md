...
# Run single test
scripts/docker-test.sh run test/01_unit/std/string_spec.spl
...
```bash
# Fast profile
docker run --memory=128m --cpus=1.0 simple-test-isolation:latest test test/01_unit/

# Intensive profile
docker run --memory=4g --cpus=4.0 simple-test-full:latest test test/02_integration/qemu/
```
  ...
  test-full:
    ...
    steps:
      ...
      - name: Run integration tests
        run: scripts/docker-test.sh run-full test/02_integration/
```
...
test-full:
  ...
  dependencies:
    - build-images
  script:
    - scripts/docker-test.sh run-full test/02_integration/
```
...
Running test: test/01_unit/std/string_spec.spl
...
- ✅ Single test execution works in container
  - Verified with `test/01_unit/std/string_spec.spl`
