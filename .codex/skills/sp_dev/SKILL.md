...
When `$sp_dev` is used for LLM Caret agent/team/advisor/goal work, check `doc/07_guide/app/llm/llm_caret_agent_teams.md`, `doc/07_guide/app/llm/llm_caret_embedded_tmux.md`, `src/app/llm_caret/agent_plan.spl`, `src/app/llm_caret/agent_files.spl`, `src/app/llm_caret/agent_vcs.spl`, `src/app/llm_caret/agent_discovery.spl`, `src/app/llm_caret/agent_mailbox.spl`, `src/app/llm_caret/agent_tui.spl`, `src/app/llm_caret/agent_tmux.spl`, `src/app/llm_caret/agent_runtime.spl`, `test/03_system/app/llm_caret/feature/llm_caret_agent_tui_handoff_spec.spl`, and `test/03_system/app/llm_caret/feature/llm_caret_embedded_tmux_tui_spec.spl` first. Treat the existing feature as static prompt/argv planning with existing-file hash snapshots, `jj diff --name-only` changed-file discovery, simple MCP/plugin manifest parsing, plugin install argv planning, pure `btw`/`side` team mailbox routing, TUI-readable handoff rendering, embedded tmux-style process panes with caller-supplied CPU/memory usage, non-persistent team launch, and explicit agent, skill, MCP server, and plugin capability lists. Use the canonical SPipe/LLM Caret capability field names `agent_paths`, `skill_paths`, `mcp_servers`, and `plugins` in state files, docs, generated manuals, and helper names; new aliases for the same concepts are stale-process-doc risk. Do not claim persistent live supervisor, background VCS watching, plugin install execution, live MCP registry discovery, live process sampling, or live team transport unless a later lane implements and verifies them.
