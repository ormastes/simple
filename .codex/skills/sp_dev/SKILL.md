...
When `$sp_dev` is used for LLM Caret agent/team/advisor/goal work, check `doc/07_guide/app/llm/llm_caret_agent_teams.md`, `src/app/llm_caret/agent_plan.spl`, `src/app/llm_caret/agent_files.spl`, and `src/app/llm_caret/agent_runtime.spl` first. Treat the existing feature as static prompt/argv planning with existing-file hash snapshots, non-persistent team launch, and explicit agent, skill, MCP server, and plugin capability lists; do not claim persistent live supervisor, VCS-wide diff capture, plugin install, MCP discovery, or team chat unless a later lane implements and verifies them.
