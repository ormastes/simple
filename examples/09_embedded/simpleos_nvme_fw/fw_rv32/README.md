...
payload-window ECC floor, a fixed-capacity write-back map-cache floor, a fixed band allocator floor,
and a fixed-capacity journal-ring floor, written to run inside the bare-metal rv32 boot path with
...
- ✅ `logic.spl` / `entry.spl` are `check`-clean, array-free, and the RAIN+ECC+map-cache+band+journal-ring logic is
...
ECC floor, fixed map cache, fixed band allocator, and fixed journal ring (host-verified), now wired through the rv32 boot hook. A standalone `native-build` of it alone still
