...
journal-ring floor, a fixed HIL command/queue floor, a fixed admin/format/fw-log floor, a fixed reactor floor, a fixed policy/target floor, and a fixed DRAM/durability floor, written to run inside the bare-metal rv32 boot path with
...
- ✅ `logic.spl` / `entry.spl` are `check`-clean, array-free, and the RAIN+ECC+scheduler+power-thermal+map-cache+band+journal-ring+HIL+admin-format-fw-log+reactor+policy-target+DRAM-durability logic is
