...
re-expression of the RAIN reconstruct, SECDED ECC floor, fixed scheduler, fixed power/thermal model, fixed map cache, fixed band allocator, fixed journal ring, and fixed HIL command/queue, `bin/simple check`-clean, host-verified, and wired through
