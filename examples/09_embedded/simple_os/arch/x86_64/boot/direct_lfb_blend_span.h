    ...
    RuntimeValue src_offset, RuntimeValue count)
{
    ...
    if ((uint64_t)framebuffer_addr != registered_addr ||
        ...
        !registered_pitch || x >= registered_width || y >= registered_height ||
    ...
    if (!items) return 0;
    volatile uint32_t *dst = (volatile uint32_t *)(uintptr_t)registered_addr +
                             (uint64_t)y * (registered_pitch / 4u) + x;
