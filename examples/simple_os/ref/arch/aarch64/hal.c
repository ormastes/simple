/*
 * hal.c — HAL implementation for AArch64
 *
 * Thin wrappers mapping the generic HAL interface to AArch64-specific drivers.
 * Uses PL011 UART, generic timer, and MSR for IRQ/MMU control.
 */
...
void hal_timer_init(uint32_t freq_hz)     { timer_init(); (void)freq_hz; }
uint64_t hal_timer_ticks(void)            { return timer_get_ticks(); }
uint32_t hal_timer_ms(void)               { return (uint32_t)timer_get_ms(); }

void hal_irq_enable(void)                 { __asm__ volatile("msr daifclr, #0xf"); }
void hal_irq_disable(void)               { __asm__ volatile("msr daifset, #0xf"); }
...
void hal_halt(void)                       { __asm__ volatile("wfe"); }
void hal_panic_halt(void)                 { __asm__ volatile("msr daifset, #0xf"); for(;;) __asm__ volatile("wfe"); }
