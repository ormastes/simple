...
typedef struct {
    ...
    int64_t value1;
    int64_t value2;
} RiscvNativeTcpTriple;

extern RiscvNativeTcpPair native_tcp_bind(int64_t addr_ptr, int64_t addr_len) __attribute__((weak));
extern RiscvNativeTcpTriple native_tcp_accept(int64_t handle) __attribute__((weak));
extern RiscvNativeTcpPair native_tcp_read(int64_t handle, int64_t buf_ptr, int64_t buf_len) __attribute__((weak));
extern RiscvNativeTcpPair native_tcp_write(int64_t handle, int64_t data_ptr, int64_t data_len) __attribute__((weak));
extern int64_t native_tcp_close(int64_t handle) __attribute__((weak));
extern int64_t native_tcp_set_backlog(int64_t handle, int64_t backlog) __attribute__((weak));

#define RISCV_RT_NET_MAX_SOCKETS 8U
#define RISCV_RT_NET_RX_CAPACITY 65536U
#define RISCV_RT_NET_ENOSYS 38
#define RISCV_RT_NET_EMFILE 24
#define RISCV_RT_NET_EBADF 9
#define RISCV_RT_NET_EINVAL 22

enum {
    RISCV_RT_NET_SLOT_FREE = 0,
    RISCV_RT_NET_SLOT_SOCKET = 1,
    RISCV_RT_NET_SLOT_LISTENER = 2,
    RISCV_RT_NET_SLOT_STREAM = 3
};
...
typedef struct {
    ...
    uint16_t backlog;
    uint32_t rx_len;
    uint8_t rx_buf[RISCV_RT_NET_RX_CAPACITY];
} RiscvRtNetSocket;

static RiscvRtNetSocket g_riscv_rt_net_sockets[RISCV_RT_NET_MAX_SOCKETS];

static uint8_t riscv_rt_net_value_to_byte(RuntimeValue value)
{
    return (uint8_t)(simpleos_raw_or_encoded_int(value) & 0xffU);
}

static int riscv_rt_net_backend_ready(void)
{
    return native_tcp_bind &&
           native_tcp_accept &&
           native_tcp_read &&
           native_tcp_write &&
           native_tcp_close &&
...
static RuntimeValue riscv_rt_net_empty_bytes(void)
{
    RuntimeArray *a = (RuntimeArray *)rv_alloc(sizeof(RuntimeArray));
    if (!a) return NIL_VALUE;
    a->hdr.type = HEAP_ARRAY;
    a->hdr.size = (uint32_t)sizeof(RuntimeArray);
    a->len = 0;
    a->cap = 0;
    a->items = runtime_array_inline_items(a);
    return ENCODE_PTR(a);
...
static RuntimeValue riscv_rt_net_bytes_to_array(const uint8_t *buf, uint32_t len)
{
    RuntimeArray *a = (RuntimeArray *)rv_alloc(sizeof(RuntimeArray) + ((size_t)len * sizeof(RuntimeValue)));
    if (!a) return NIL_VALUE;
    a->hdr.type = HEAP_ARRAY;
    ...
    a->cap = len;
    a->items = runtime_array_inline_items(a);
    for (uint32_t i = 0; i < len; i++) a->items[i] = ENCODE_INT((int64_t)buf[i]);
    return ENCODE_PTR(a);
}

static int riscv_rt_net_find_free_slot(void)
{
    for (uint32_t i = 0; i < RISCV_RT_NET_MAX_SOCKETS; i++) {
        if (g_riscv_rt_net_sockets[i].state == RISCV_RT_NET_SLOT_FREE) return (int)i;
    }
    return -1;
}

static void riscv_rt_net_reset_slot(RiscvRtNetSocket *slot)
{
    if (!slot) return;
    slot->state = RISCV_RT_NET_SLOT_FREE;
    slot->handle = -1;
    slot->port = 0;
    slot->backlog = 0;
    slot->rx_len = 0;
...
static int riscv_rt_net_fd_ok(int fd)
{
    return fd >= 0 &&
           fd < (int)RISCV_RT_NET_MAX_SOCKETS &&
           g_riscv_rt_net_sockets[fd].state != RISCV_RT_NET_SLOT_FREE;
}

static int64_t riscv_rt_net_negated_error(int64_t err)
{
    return err > 0 ? -err : err;
}

static uint32_t riscv_rt_net_make_addr(char *buf, uint32_t cap, uint16_t port)
{
    static const char prefix[] = "0.0.0.0:";
    uint32_t len = 0;
    if (cap == 0) return 0;
    for (uint32_t i = 0; prefix[i] != 0 && len + 1U < cap; i++) buf[len++] = prefix[i];
    char digits[5];
    ...
    while (digits_len > 0 && len + 1U < cap) buf[len++] = digits[--digits_len];
    buf[len] = 0;
    return len;
}

static void riscv_rt_net_consume(RiscvRtNetSocket *slot, uint32_t count)
{
    ...
    __builtin_memmove(slot->rx_buf, slot->rx_buf + count, slot->rx_len - count);
    slot->rx_len -= count;
}

static int64_t riscv_rt_net_fill(RiscvRtNetSocket *slot, uint32_t need)
{
    if (!slot) return -RISCV_RT_NET_EBADF;
    if (!riscv_rt_net_backend_ready()) return -RISCV_RT_NET_ENOSYS;
    while (slot->rx_len < need) {
        if (slot->rx_len >= RISCV_RT_NET_RX_CAPACITY) return -RISCV_RT_NET_EINVAL;
        uint32_t space = RISCV_RT_NET_RX_CAPACITY - slot->rx_len;
        RiscvNativeTcpPair read_result =
            native_tcp_read(slot->handle,
                            (int64_t)(uintptr_t)(slot->rx_buf + slot->rx_len),
                            (int64_t)space);
        if (read_result.value1 != 0) return riscv_rt_net_negated_error(read_result.value1);
        if (read_result.value0 <= 0) break;
        slot->rx_len += (uint32_t)read_result.value0;
    }
    return (int64_t)slot->rx_len;
}

int64_t rt_net_socket(int64_t proto)
{
    (void)proto;
    if (!riscv_rt_net_backend_ready()) return ENCODE_INT(-RISCV_RT_NET_ENOSYS);
    int slot_index = riscv_rt_net_find_free_slot();
    if (slot_index < 0) return ENCODE_INT(-RISCV_RT_NET_EMFILE);
    riscv_rt_net_reset_slot(&g_riscv_rt_net_sockets[slot_index]);
    g_riscv_rt_net_sockets[slot_index].state = RISCV_RT_NET_SLOT_SOCKET;
    return ENCODE_INT(slot_index);
}

int64_t rt_net_init(void)
{
    return ENCODE_INT(riscv_rt_net_backend_ready() ? 0 : -RISCV_RT_NET_ENOSYS);
}
...
{
    int fd = (int)sock_fd;
    if (!riscv_rt_net_fd_ok(fd)) return ENCODE_INT(-RISCV_RT_NET_EBADF);
    if (port_num < 0 || port_num > 65535) return ENCODE_INT(-RISCV_RT_NET_EINVAL);
    g_riscv_rt_net_sockets[fd].port = (uint16_t)port_num;
    return ENCODE_INT(0);
}
...
int64_t rt_net_listen(int64_t sock_fd, int64_t backlog)
{
    ...
    RiscvRtNetSocket *slot = &g_riscv_rt_net_sockets[fd];
    char addr[32];
    uint32_t addr_len = riscv_rt_net_make_addr(addr, sizeof(addr), slot->port);
    if (slot->handle >= 0) {
        (void)native_tcp_close(slot->handle);
        slot->handle = -1;
    }

    RiscvNativeTcpPair bind_result = native_tcp_bind((int64_t)(uintptr_t)addr, (int64_t)addr_len);
    if (bind_result.value1 != 0) return ENCODE_INT(riscv_rt_net_negated_error(bind_result.value1));

    slot->handle = bind_result.value0;
    slot->backlog = backlog > 0 ? (uint16_t)backlog : 0;
    int64_t backlog_rc = native_tcp_set_backlog(slot->handle, backlog);
    if (backlog_rc != 0) {
        (void)native_tcp_close(slot->handle);
        slot->handle = -1;
        return ENCODE_INT(riscv_rt_net_negated_error(backlog_rc));
    }
    slot->state = RISCV_RT_NET_SLOT_LISTENER;
    slot->rx_len = 0;
    return ENCODE_INT(0);
}
...
int64_t rt_net_accept(int64_t sock_fd)
{
    ...
    RiscvRtNetSocket *listener = &g_riscv_rt_net_sockets[fd];
    if (listener->state != RISCV_RT_NET_SLOT_LISTENER || listener->handle < 0) {
        return ENCODE_INT(-RISCV_RT_NET_EBADF);
    }

    RiscvNativeTcpTriple accept_result = native_tcp_accept(listener->handle);
    if (accept_result.value2 != 0) return ENCODE_INT(riscv_rt_net_negated_error(accept_result.value2));

    int slot_index = riscv_rt_net_find_free_slot();
    if (slot_index < 0) {
        (void)native_tcp_close(accept_result.value0);
        return ENCODE_INT(-RISCV_RT_NET_EMFILE);
    }

    riscv_rt_net_reset_slot(&g_riscv_rt_net_sockets[slot_index]);
    g_riscv_rt_net_sockets[slot_index].state = RISCV_RT_NET_SLOT_STREAM;
    g_riscv_rt_net_sockets[slot_index].handle = accept_result.value0;
    return ENCODE_INT(slot_index);
}
...
int64_t rt_net_close(int64_t sock_fd)
{
    ...
    if (!riscv_rt_net_fd_ok(fd)) return ENCODE_INT(-RISCV_RT_NET_EBADF);
    RiscvRtNetSocket *slot = &g_riscv_rt_net_sockets[fd];
    if (slot->handle >= 0 && native_tcp_close) {
        int64_t rc = native_tcp_close(slot->handle);
        if (rc != 0) {
            riscv_rt_net_reset_slot(slot);
            return ENCODE_INT(riscv_rt_net_negated_error(rc));
        }
    }
    riscv_rt_net_reset_slot(slot);
    return ENCODE_INT(0);
...
int64_t rt_net_send_bytes(int64_t sock_fd, RuntimeValue data_rv)
{
    ...
    if (!riscv_rt_net_fd_ok(fd)) return ENCODE_INT(-RISCV_RT_NET_EBADF);
    if (!riscv_rt_net_backend_ready()) return ENCODE_INT(-RISCV_RT_NET_ENOSYS);
    if (!IS_HEAP(data_rv)) return ENCODE_INT(-RISCV_RT_NET_EINVAL);

    RiscvRtNetSocket *slot = &g_riscv_rt_net_sockets[fd];
    if (slot->state != RISCV_RT_NET_SLOT_STREAM || slot->handle < 0) return ENCODE_INT(-RISCV_RT_NET_EBADF);

    RuntimeArray *arr = (RuntimeArray *)DECODE_PTR(data_rv);
    if (!arr || arr->hdr.type != HEAP_ARRAY) return ENCODE_INT(-RISCV_RT_NET_EINVAL);
    if (arr->len == 0) return ENCODE_INT(0);

    uint32_t len = (uint32_t)arr->len;
    uint8_t *buf = (uint8_t *)rv_alloc(len > 0 ? len : 1U);
    if (!buf) return ENCODE_INT(-12);
    RuntimeValue *items = runtime_array_items(arr);
    for (uint32_t i = 0; i < len; i++) buf[i] = riscv_rt_net_value_to_byte(items[i]);

    uint32_t sent = 0;
    while (sent < len) {
        RiscvNativeTcpPair write_result =
            native_tcp_write(slot->handle,
                             (int64_t)(uintptr_t)(buf + sent),
                             (int64_t)(len - sent));
        if (write_result.value1 != 0) return ENCODE_INT(riscv_rt_net_negated_error(write_result.value1));
        if (write_result.value0 <= 0) break;
        sent += (uint32_t)write_result.value0;
    }
...
{
    int fd = (int)sock_fd;
    if (!riscv_rt_net_fd_ok(fd)) return NIL_VALUE;
    if (max_len <= 0) return riscv_rt_net_empty_bytes();

    RiscvRtNetSocket *slot = &g_riscv_rt_net_sockets[fd];
    if (slot->state != RISCV_RT_NET_SLOT_STREAM || slot->handle < 0) return NIL_VALUE;
    (void)riscv_rt_net_fill(slot, 1U);
    if (slot->rx_len == 0) return riscv_rt_net_empty_bytes();

    uint32_t take = slot->rx_len;
    if ((uint64_t)take > (uint64_t)max_len) take = (uint32_t)max_len;
    RuntimeValue out = riscv_rt_net_bytes_to_array(slot->rx_buf, take);
    riscv_rt_net_consume(slot, take);
    return out;
}
...
{
    int fd = (int)sock_fd;
    if (!riscv_rt_net_fd_ok(fd)) return rt_string_from_cstr("");

    RiscvRtNetSocket *slot = &g_riscv_rt_net_sockets[fd];
    if (slot->state != RISCV_RT_NET_SLOT_STREAM || slot->handle < 0) return rt_string_from_cstr("");

    uint32_t line_end = 0;
    int found_newline = 0;
    for (;;) {
        for (line_end = 0; line_end < slot->rx_len; line_end++) {
            if (slot->rx_buf[line_end] == (uint8_t)'\n') {
                found_newline = 1;
                break;
            }
        }
        if (found_newline || slot->rx_len >= RISCV_RT_NET_RX_CAPACITY) break;
        int64_t fill_rc = riscv_rt_net_fill(slot, slot->rx_len + 1U);
        if (fill_rc <= 0 || (uint32_t)fill_rc == slot->rx_len) break;
    }

    if (!found_newline) line_end = slot->rx_len;
    while (line_end > 0 && slot->rx_buf[line_end - 1U] == (uint8_t)'\r') line_end--;

    RuntimeValue out = rt_string_new((RuntimeValue)(uintptr_t)slot->rx_buf, (RuntimeValue)line_end);
    uint32_t consume = found_newline ? (line_end + 1U) : line_end;
    if (found_newline && consume <= slot->rx_len && slot->rx_buf[consume - 1U] != (uint8_t)'\n') {
        consume = line_end;
        while (consume < slot->rx_len && slot->rx_buf[consume] != (uint8_t)'\n') consume++;
        if (consume < slot->rx_len) consume++;
    }
    riscv_rt_net_consume(slot, consume);
    return out;
}
...
RuntimeValue rt_net_recv_ssh_plain_packet_payload(int64_t sock_fd)
{
    ...
    if (riscv_rt_net_fill(slot, 5U) < 5) return riscv_rt_net_empty_bytes();

    uint32_t packet_length =
        ((uint32_t)slot->rx_buf[0] << 24) |
        ((uint32_t)slot->rx_buf[1] << 16) |
        ((uint32_t)slot->rx_buf[2] << 8) |
        (uint32_t)slot->rx_buf[3];
    uint32_t padding_length = (uint32_t)slot->rx_buf[4];
    if (packet_length < 2U || packet_length > 35000U || padding_length + 1U > packet_length) {
        return riscv_rt_net_empty_bytes();
    }

    uint32_t total = 4U + packet_length;
    if (riscv_rt_net_fill(slot, total) < (int64_t)total) return riscv_rt_net_empty_bytes();

    uint32_t payload_length = packet_length - padding_length - 1U;
    RuntimeValue out = riscv_rt_net_bytes_to_array(slot->rx_buf + 5U, payload_length);
    riscv_rt_net_consume(slot, total);
    return out;
}
