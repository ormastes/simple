/*
 * SimpleOS RISC-V32 Baremetal Runtime Stubs
 *
 * Provides a complete freestanding runtime for the Simple language compiler
 * targeting RISC-V32 bare-metal (QEMU virt, M-mode, 16550 UART at 0x10000000).
 *
 * CRITICAL 32-bit considerations:
 *   - RuntimeValue is STILL int64_t (compiler ABI uses 64-bit tagged values)
 *   - On RV32, int64_t args are passed in register pairs (a0+a1, a2+a3, etc.)
 *   - Pointers are 32-bit: sizeof(void*) == 4
 *   - ENCODE_PTR: upper 32 bits are 0
 *   - DECODE_PTR: mask to 32-bit before casting to pointer
 *   - No SBI (M-mode boot with -bios none) — use wfi for halt
 *
 * Sections:
 ...
 * 3. RuntimeValue tagging
 *
 * RuntimeValue is int64_t even on 32-bit targets (compiler ABI).
 * On RV32, int64_t args are passed in register pairs (a0+a1, a2+a3).
 * Pointers are 32-bit — ENCODE_PTR upper 32 bits are 0,
 * DECODE_PTR masks to 32-bit before casting.
 * =================================================================== */
...
/* 32-bit pointer encoding: upper 32 bits are 0, tag in low 3 bits */
#define ENCODE_PTR(p)  ((RuntimeValue)((uint64_t)((uint32_t)(uintptr_t)(p)) | TAG_HEAP))
/* 32-bit pointer decoding: mask off tag, then truncate to 32-bit pointer */
#define DECODE_PTR(v)  ((void*)((uint32_t)((uint64_t)(v) & ~TAG_MASK)))
...
static void *_heap_alloc(size_t sz)
{
    ...
    if (_heap_off + sz > sizeof(_heap)) {
        ...
        /* No SBI on M-mode — just halt with wfi */
...
RuntimeValue rt_string_new(RuntimeValue data, RuntimeValue len_val)
{
    ...
    s->len = (uint32_t)len;
    const char *src = (const char *)(uintptr_t)(uint32_t)data;
...
{
    const char *spec = (const char *)(uintptr_t)(uint32_t)fmt_ptr_rv;
...
void rt_print_str(RuntimeValue str)
{
    ...
    if (str != 0) {
        RuntimeString *s = (RuntimeString *)(uintptr_t)(uint32_t)str;
...
static int64_t _pci_enumerate(uint64_t mode, uint64_t index, uint64_t buf_addr)
{
    ...
    if (mode == 1) {
        ...
        uint8_t *buf = (uint8_t *)(uintptr_t)(uint32_t)buf_addr;
        int i = (int)index;
...
/* _pci_enumerate mode 4: single-field query
 * Args: (80, 4, device_index, field_selector, 0, 0)
 * Fields: 0=bus, 1=device, 2=func, 3=class, 4=subclass, 5=vendor, 6=devid, 7=bar0 */
static int64_t _pci_field_query(int index, int field)
{
    if (_pci_cache_count == 0) _pci_scan();
    if (index < 0 || index >= _pci_cache_count) return -22;
    switch (field) {
        case 0: return _pci_cache[index].bus;
        case 1: return _pci_cache[index].dev;
        case 2: return _pci_cache[index].func;
        case 3: return _pci_cache[index].class_code;
        case 4: return _pci_cache[index].subclass;
        case 5: return _pci_cache[index].vendor;
        case 6: return _pci_cache[index].device;
        case 7: return _pci_cache[index].bar0;
        default: return -22;
    }
}

static int32_t _syscall_dispatch(uint32_t id, uint32_t a0, uint32_t a1,
                                  uint32_t a2, uint32_t a3, uint32_t a4)
{
    (void)a3; (void)a4;
    switch (id) {
        case 0: { /* Exit — no SBI in M-mode, just halt */
            for(;;) __asm__ volatile("wfi");
            return 0;
        }
        ...
        case 80: {
            /* PCI enumerate: mode in a0 */
            if (a0 == 4) return (int32_t)_pci_field_query((int)a1, (int)a2);
            return (int32_t)_pci_enumerate((uint64_t)a0, (uint64_t)a1, (uint64_t)a2);
        }
        case 82: return (int32_t)_pci_enumerate(5, (uint64_t)a0, 0); /* DeviceGrant */
        case 83: return (int32_t)a0; /* MapBar — identity mapped */
        case 84: { void *p = _heap_alloc(a0 > 0 ? a0 : 4096); return (int32_t)(uintptr_t)p; }
        case 86: return 0; /* NVMe init: no-op on riscv32 virt (no NVMe), return success */
        default: return -38; /* ENOSYS */
    }
}

/* Called from the M-mode trap handler in crt0.S when an ecall is triggered.
 * The Simple compiler's syscall() function uses ecall even in M-mode.
 * This wrapper receives the 32-bit register values from the trap frame. */
int32_t _trap_syscall(uint32_t id, uint32_t a0, uint32_t a1,
                      uint32_t a2, uint32_t a3, uint32_t a4)
{
    return _syscall_dispatch(id, a0, a1, a2, a3, a4);
}

/* Also provide the full 64-bit versions as link-time symbols */
int64_t userlib__syscall_raw__syscall(uint64_t id, uint64_t a0, uint64_t a1,
                                       uint64_t a2, uint64_t a3, uint64_t a4)
{
    int32_t r = _syscall_dispatch((uint32_t)id, (uint32_t)a0, (uint32_t)a1,
                                  (uint32_t)a2, (uint32_t)a3, (uint32_t)a4);
    /* Zero-extend positive results (heap pointers with bit 31 set),
       sign-extend only small negative error codes (-1..-255) */
    if (r >= -255 && r < 0) return (int64_t)r;  /* sign-extend error codes */
    return (int64_t)(uint32_t)r;  /* zero-extend everything else */
}
...
RuntimeValue rt_mmio_read_u8(RuntimeValue addr)  { return (RuntimeValue)(uint64_t)*(volatile uint8_t  *)(uintptr_t)(uint32_t)(uint64_t)addr; }
RuntimeValue rt_mmio_read_u16(RuntimeValue addr) { return (RuntimeValue)(uint64_t)*(volatile uint16_t *)(uintptr_t)(uint32_t)(uint64_t)addr; }
RuntimeValue rt_mmio_read_u32(RuntimeValue addr) { return (RuntimeValue)(uint64_t)*(volatile uint32_t *)(uintptr_t)(uint32_t)(uint64_t)addr; }
/* 64-bit MMIO read on 32-bit: read low word only */
RuntimeValue rt_mmio_read_u64(RuntimeValue addr) { return (RuntimeValue)(uint64_t)*(volatile uint32_t *)(uintptr_t)(uint32_t)(uint64_t)addr; }
RuntimeValue rt_mmio_write_u8(RuntimeValue addr, RuntimeValue val)  { *(volatile uint8_t  *)(uintptr_t)(uint32_t)(uint64_t)addr = (uint8_t)(uint64_t)val; return 0; }
RuntimeValue rt_mmio_write_u16(RuntimeValue addr, RuntimeValue val) { *(volatile uint16_t *)(uintptr_t)(uint32_t)(uint64_t)addr = (uint16_t)(uint64_t)val; return 0; }
RuntimeValue rt_mmio_write_u32(RuntimeValue addr, RuntimeValue val) { *(volatile uint32_t *)(uintptr_t)(uint32_t)(uint64_t)addr = (uint32_t)(uint64_t)val; return 0; }
/* 64-bit MMIO write on 32-bit: write low word only */
RuntimeValue rt_mmio_write_u64(RuntimeValue addr, RuntimeValue val) { *(volatile uint32_t *)(uintptr_t)(uint32_t)(uint64_t)addr = (uint32_t)(uint64_t)val; return 0; }
...
/* CPU control — M-mode (no SBI) */
RuntimeValue rt_hlt(void)  { __asm__ volatile("wfi"); return NIL_VALUE; }
RuntimeValue rt_sti(void)  { return NIL_VALUE; }
...
/* rdtime is not available in M-mode on all implementations; use mcycle instead */
RuntimeValue rt_rdtsc(void) { uint32_t lo; __asm__ volatile("rdcycle %0" : "=r"(lo)); return ENCODE_INT((int64_t)lo); }
...
RuntimeValue rt_read_cr3(RuntimeValue a) { (void)a; return ENCODE_INT(0); /* No S-mode satp in M-mode */ }
RuntimeValue rt_write_cr3(RuntimeValue a) { (void)a; return NIL_VALUE; }
RuntimeValue rt_read_cr2(RuntimeValue a) { (void)a; return ENCODE_INT(0); }
 ...
 * 15. _boot_init entry point
 *
 * Called from crt0.S before spl_start(). Initializes UART, scans PCI.
 * crt0.S provides the real _start that sets up stack and zeroes BSS.
 * =================================================================== */
...
void _boot_init(void)
{
    ...
    serial_puts("[BOOT] RuntimeValue: tagged 64-bit (register pairs on RV32)\r\n");
...
/* Provide _c_start as fallback if crt0.S is not linked (weak) */
void _c_start(void) __attribute__((weak));
void _c_start(void)
{
    _uart_init();
    serial_puts("SimpleOS RISC-V32 boot (C fallback)\r\n");
    _pci_scan();
    if (spl_start) {
        spl_start();
    } else {
    ...
    serial_puts("[BOOT] riscv32 boot complete\r\n");
    for(;;) __asm__ volatile("wfi");
}
...
/* ===================================================================
 * 17. All stubs (matching riscv64 complete list)
 * =================================================================== */
...
RuntimeValue rt_hash(RuntimeValue val)
{
    ...
    if (IS_INT(val)) { int64_t n = DECODE_INT(val); for (int i = 0; i < 8; i++) { h ^= (uint8_t)(n & 0xFF); h *= 1099511628211ULL; n >>= 8; } }
        ...
        else { uint32_t p = (uint32_t)(uintptr_t)hdr; for (int i = 0; i < 4; i++) { h ^= (uint8_t)(p & 0xFF); h *= 1099511628211ULL; p >>= 8; } }
    }
...
RuntimeValue rt_function_not_found(RuntimeValue name_ptr, RuntimeValue name_len) {
    ...
    if (name_ptr) { const char *p = (const char *)(uintptr_t)(uint32_t)name_ptr; int64_t len = (int64_t)name_len; for (int64_t i = 0; i < len && i < 128; i++) serial_putchar(p[i]); }
    serial_puts("\r\n");
    return NIL_VALUE;
}
...
/* End of RISC-V32 baremetal_stubs.c */
