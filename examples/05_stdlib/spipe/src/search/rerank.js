  ...
  return Object.freeze({
    rerankRrfPageV1(request) {
      ...
      for (let index = 0; index < normalizedRecords.length; index += 1) {
        ...
        const trace = record.acceptedTrace;
        const feature = record.featureMatch;
        const component = record.componentMatch;
        const recency = record.recency;
        const status = record.status;

        const raw = rawResult.value.hits[index].rawScoreUnits;
        ...
        if ([traceDelta, featureDelta, componentDelta, recencyDelta, statusDelta, positiveCap].includes(null)) return fail('arithmetic_overflow');
        ...
        ranked.push({ documentId: record.documentId, rawScoreUnits: raw, adjustedScoreUnits,
          rawHit: rawResult.value.hits[index].rawHit,
