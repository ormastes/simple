...
function inferKind(path, value) {
  ...
  const first = normalizeText(path).replaceAll("\\", "/").split("/")[0];
  const match = first.match(/^(\d+)[_-](.+)$/);
  return (match ? match[2] : first) || "document";
}

function metadataMarker(source, lines) {
  for (const line of lines) {
    const parsed = parseMarker(line, "artifact");
...
export function parseMarkdownArtifact(input, options = {}) {
  ...
  const front = frontMatter(normalized);
  diagnostics.push(...front.diagnostics);
  const marker = metadataMarker(normalized, lines);
  ...
  const artifact = {
    ...
    trust: normalizeText(metadata.trust) || "untrusted_data",
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const markerLine = index + 1 < lines.length ? parseMarker(lines[index + 1], "section") : null;
