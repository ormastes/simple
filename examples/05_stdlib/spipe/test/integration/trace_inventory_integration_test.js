...
test("trace inventory composes supplied records without filesystem, provider, or authority access", () => {
  ...
  const result = createTraceInventoryV1(deepFreeze({ snapshot_uid: uid("V", "5"), project_uid, revision_id: "git-integration", nodes: [artifact, section, requirement].sort((a, b) => a.uid.localeCompare(b.uid)), edges: [{ edge_type: "evidence_for", from_uid: artifact_uid, to_uid: requirement_uid, source_uid: artifact_uid, origin: "explicit", asserted_status: "accepted" }] }));
