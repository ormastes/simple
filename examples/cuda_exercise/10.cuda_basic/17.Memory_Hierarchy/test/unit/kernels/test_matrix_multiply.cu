...
extern __global__ void demonstrate_strided_access(float* data, int stride, int N);
extern __global__ void demonstrate_coalesced_access(float* data, int N);

// Forward declarations of helper functions from matrix_multiply.cu
extern void run_memory_hierarchy_benchmark(int N);
extern void demonstrate_memory_patterns();

template<typename KernelFunc>
extern float benchmark_kernel(KernelFunc kernel, const float* d_A, const float* d_B,
                             float* d_C, int N, dim3 gridSize, dim3 blockSize,
                             const char* kernel_name, int iterations);
...
// Test the benchmark function from matrix_multiply.cu
TEST_F(MatrixMultiplyTest, BenchmarkFunctionTest) {
    ...
    // Shared memory should be faster than naive (though not guaranteed on all hardware)
    std::cout << "Naive time: " << naive_time << " ms" << std::endl;
    std::cout << "Shared time: " << shared_time << " ms" << std::endl;
...
// Test the run_memory_hierarchy_benchmark function
TEST_F(MatrixMultiplyTest, MemoryHierarchyBenchmark) {
    // Run the full benchmark with a small matrix size
    std::cout << "\n--- Running Memory Hierarchy Benchmark ---" << std::endl;
    ASSERT_NO_THROW(run_memory_hierarchy_benchmark(256));
    std::cout << "--- Benchmark Complete ---\n" << std::endl;
}

// Test memory access pattern demonstration kernels
TEST_F(MatrixMultiplyTest, MemoryAccessPatterns) {
    const int N = 1024;
    const int stride = 32;
